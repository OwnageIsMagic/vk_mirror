...
function ppShow(pp_tag, nofix) {
    ...
    var pp_top = '<div onclick="ppHide(1)" class="ppHead' + postfix + '"><div class="ppChooseTop" onmouseover="pp_over=1" onmouseout="pp_over=0; ppRemove()" id="ppChooseWord">' +
        options[j] + '</div></div>';
    if (toUp) {
        pp_menu.innerHTML =
            '<table cellpadding="0" cellspacing="0"><tr><td class="ppMenuSide"></td><td><div class="ppMenuBottom"></div><div class="ppMenuBottom2"></div><div onmouseout="ppRemove();" class="ppMenuBodyUp">' +
            ppOptions(pp_tag, true) + '</div>' + pp_top + '</td><td class="ppMenuSide"></td></tr></table>';
        pp_menu.style.top = (coords[1] + 18 - height[0] + ie_y_fix) + 'px';
    } else {
        pp_menu.innerHTML = '<table cellpadding="0" cellspacing="0"><tr><td class="ppMenuSide"></td><td>' + pp_top + '<div onmouseout="ppRemove();" class="ppMenuBody">' +
            ppOptions(pp_tag) + '</div><div class="ppMenuBottom2"></div><div class="ppMenuBottom"></div></td><td class="ppMenuSide"></td></tr></table>';
...
function ppOptions(pp_tag, toUp) {
    ...
    for (var i = 0; i < options.length; i++) {
        ...
        str = (toUp ? '' : str) + '<div id="ppItem' + i + '" onclick="ppClick(' + i + ',\'' + pp_tag + '\')" onmouseover="ppOverItem(this,' + i + ',\'' + pp_tag +
            '\')" onmouseout="ppOutItem(this,' + i + ')" class="' + cls + '">' + options[i] + '</div>' + (toUp ? str : '');
        if (i == 3 && options[-1]) {
            cls = j == -1 ? 'ppItemOver' : 'ppItem';
            str = (toUp ? '' : str) + '<div id="ppItem-1" onclick="ppClick(-1,\'' + pp_tag + '\')" onmouseover="ppOverItem(this,-1,\'' + pp_tag +
                '\')" onmouseout="ppOutItem(this,-1)" class="' + cls + '">' + options[-1] + '</div>' + (toUp ? str : '');
            custom_f = true;
        }
    }
...
function ppOverItem(obj, i, pp_tag) {
    ...
    if (pp_selected[pp_tag] != i && (ge('ppItem' + pp_selected[pp_tag]) && ge('ppItem' + pp_selected[pp_tag])
            .className == 'ppItemOver')) {
        ge('ppItem' + pp_selected[pp_tag])
            .className = 'ppItem';
    }
...
function dpShow(pp_tag) {
    ...
    var str = '<div class="dpHead' + postfix + '" onclick="ppClick(' + pp_lists_id + ',\'' + pp_tag + '\')" onmouseout="dpRemove(\'' + pp_tag +
        '\'); dp_over=0;" onmouseover="dp_over=1"><div class="dpChooseTop">' + options[pp_lists_id] + '</div></div>';
...
function dpItems(pp_tag, toUp) {
    ...
    each(k, function(i, v) {
        ...
        str = (toUp ? '' : str) + '<div id="dp' + v + '" onclick="dpCheck(' + v + ',\'' + pp_tag + '\')" onmouseover="dpOverItem(' + v + ',\'' + pp_tag +
            '\')" onmouseout="dpOutItem(' + v + ',\'' + pp_tag + '\')" class="' + cl_name + '"><div>' + friends_lists[v] + '</div></div>' + (toUp ? str : '');
    });
    ...
    if (dp_checked[pp_tag][n]) {
        ...
        if (pp_selected[pp_tag] == pp_lists_id) {
            if (!dpHasChecks(pp_tag)) {
                pp_selected[pp_tag] = 0;
                ge('pp_' + pp_tag)
                    .innerHTML = options[pp_selected[pp_tag]];
                ge('ppChooseWord')
                    .innerHTML = options[pp_selected[pp_tag]];
            }
        }
...
function onCustomPrivacySave(pp_tag, friends, friends_info, list_id, list_name, minus_friends, minus_friends_info) {
    ...
    if (!isArray(minus_friends)) {
        ...
        ge('pp_' + pp_tag)
            .innerHTML = options[pp_friends_id];
...
function setPrivacy(pp_tag, privacy) {
    ...
    ge('pp_' + pp_tag)
        .innerHTML = pp_options[privacy[0]];
        ...
        if (!customMB[key]) {
            ...
            function init() {
                ...
                customMB[key].content('<div class="plus_friends"><h4 class="header_plus_friends">' + options.who_can_see_title + '</h4>' +
                    '<div style="padding:3px 0;">' + options.show_to.replace('{friends}', '<a href="" id="plus_friends_type_' + key + '" onclick="showCustomMenu(\'' +
                        key + '\');return false;">' + l[plus_type] + '</a>') + '</div>' +
