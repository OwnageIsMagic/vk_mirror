function AudioPage(e, t, i, o) {
    ...
    if (AudioUtils.toggleAudioHQBodyClass(), addTemplates({
            audio_row: i.audioRowTpl
        }), this.options = i, this._container = ge(e), this._onSilentLoaded = {}, this._readyAudio = o, cur.canAudioAddToGroup = this.options.canAudioAddToGroup, this.ap =
        getAudioPlayer(), ap.langs = i.langs, extend(cur.lang || {}, i.langs), this.ap.getCurrentAudio() && delete this._readyAudio, t.type) {
        var r = getAudioPlayer()
            .hasPlaylist(t.type, t.ownerId, t.albumId);
        r ? t = r : (t = new AudioPlaylist(t), t.loadSilent())
    } else ap.friendBlock = t.blocks, t = this.ap.getCurrentPlaylist(), t.mergeWith({
        blocks: ap.friendBlock
    });
    if (isObject(i.myPlaylist)) {
        var d = new AudioPlaylist(i.myPlaylist);
        this._myPlaylist = d.getId()
    }
    this._updateAdditionalBlocksAndRightMenu(t), this._initSearch(), this._initScroll(), this.ap.setStatusExportInfo(i.exp), this.onShow(t), data(e, "audioPage", this);
    var n = this;
    this.isLayer() || (cur.destroy.push(function() {
        ap.off(n), delete n
    }), cur.module = "audio"), this.ap.on(this, AudioPlayer.EVENT_PLAY, function(e, t, i) {
        var o = this.getCurrentPlaylist(),
            a = !this.isLayer() && this.ap.layer && this.ap.layer.isShown();
        t && 0 != i && o.indexOfAudio(e) >= 0 && (a || this.scrollToTrack())
...
AudioPage.address = "audio", AudioPage.onSearchFocused = function(e) {
    ...
    function e() {
        return !t.isLayer() && ap.layer && ap.layer.isShown()
    }
    ...
    window.Notifier && Notifier.addRecvClbk("audio_hq_label", "audio", function() {
        ...
        if (!e() && ((t.keyCode != KEY.CTRL || cur.ctrlMouseDown) && (delete cur.ctrlMouseDown, delete cur.ctrlPressed), cur.ctrlPressed)) {
            ...
            i.showHQLabel(!o), window.Notifier && Notifier.lcSend("audio_hq_label"), delete cur.ctrlPressed
        }
    }), addEvent(window.document, "keydown", this._audioSeekKeyEventHandler = function(t) {
        var i = getAudioPlayer();
        if ((!t.target || !(inArray(t.target.tagName.toLowerCase(), ["input", "textarea"]) && "" != val(t.target) || hasClass(t.target, "fc_editable"))) && i.isPlaying() &&
            inArray(t.keyCode, [KEY.RIGHT, KEY.LEFT]) && !e()) {
            var o = AudioUtils.asObject(i.getCurrentAudio()),
                a = 10 / o.duration,
                s = i.getCurrentProgress() + (t.keyCode == KEY.RIGHT ? a : -a);
            s = Math.max(0, Math.min(1, s)), i.seek(s)
        }
    ...
    var t = this.ap.hasStatusExport(),
        i = geByClass1("_audio_page_player_status", this._container);
    return toggleClass(i, "audio_page_player_btn_enabled", t), !1
}, AudioPage.prototype.updateStatusExport = function(e, t) {
    e && checkbox(e);
    var i, o, t = intval(t),
        a = this.ap.getStatusExportInfo() || {};
    if (t) a[t] ? (delete a[t], i = !1) : (a[t] = 1, i = !0);
    else if (this.ap.hasStatusExport()) {
        for (var s in a) delete a[s];
        i = !1
    } else t = vk.id, a[t] = 1, i = !0;
    this.ap.setStatusExportInfo(a);
    var r = vk.audioParams.addHash;
    if (r) {
        t != vk.id && t || checkbox("currinfo_audio", this.ap.hasStatusExport()), this.updateStatusExportControls();
    ...
    return ajax.post("al_audio.php", d, {
        ...
        onDone: function(t, i, o, a) {
            var d = getAudioPlayer(),
                n = d.getPlaylist(AudioPlaylist.TYPE_ALBUM, s, t);
            n.clean(), n.mergeWith({
                list: a,
                hasMore: !1
            }), d.deletePlaylist(d.getPlaylist(AudioPlaylist.TYPE_RECOM, s, "album" + t)), each(e, function(e, t) {
                var i = {};
                i[AudioUtils.AUDIO_ITEM_INDEX_ALBUM_ID] = t, d.updateAudio(s + "_" + t, i)
            }), each(d.getPlaylists(), function(e, t) {
                t.getType() == AudioPlaylist.TYPE_ALBUM && t.getOwnerId() == s && d.deletePlaylist(t)
            });
    ...
    for (var i = e.length, o = [], a = 0; i > a; a++) {
        var s = e[a];
...
}, AudioPage.prototype.editAlbum = function(e, t, i) {
    function o() {
...
}, AudioPage.prototype.showAlbumRecoms = function(e, t, i, o) {
    return nav.go({
        0: nav.objLoc[0],
        section: "recoms",
        album_id: o
    ...
    if (t) {
        var i = this.ap.getPlaylist(AudioPlaylist.TYPE_ALBUM, this.options.oid, AudioPlaylist.ALBUM_ALL);
        i.addAudio(t, 0);
        var o = this.ap.getCurrentPlaylist();
        o && o.getSelf() == i && o.addAudio(t);
        var a = this.getCurrentPlaylist();
        a == i && this.switchToSection(a)
    }
    ...
    var t = AudioUtils.getAudioFromEl(e, !0),
        i = AudioUtils.asObject(this.ap.getCurrentAudio());
    i && i.fullId == t.fullId && this.ap.playNext();
    var o = AudioUtils.getAudioExtra(t)
        .recom,
        a = {
    ...
    s && s.getType() == AudioPlaylist.TYPE_RECOM && (cur._audioAddRestoreInfo[t.fullId].removedCurrentPos = s.removeAudio(t))
}, AudioPage.prototype.restoreRecommendation = function(e) {
    var t = AudioUtils.getAudioFromEl(e, !0),
        i = AudioUtils.getAudioExtra(t)
        .recom,
        o = {
    ...
    nav.objLoc.audio_id && (o.recommendation_type = "query"), nav.objLoc.album_id && (o.recommendation_type = "album"), ajax.post("al_audio.php", o), removeClass(e,
        "audio_deleted");
    var a = cur._audioAddRestoreInfo[t.fullId].removedCurrentPos,
        s = this.ap.getCurrentPlaylist();
    a >= 0 && s && s.getType() == AudioPlaylist.TYPE_RECOM && s.addAudio(AudioUtils.getAudioFromEl(e), a), delete cur._audioAddRestoreInfo[t.fullId]
}, AudioPage.prototype.deleteAudio = function(e, t, i) {
    function o() {
    ...
    var t = toggleClass(e, "audio_page_player_btn_enabled");
    this.ap.toggleRepeatCurrentAudio(t)
}, AudioPage.prototype.toggleShuffle = function(e) {
    var t = this.getCurrentPlaylist();
    t.isShuffled() ? (removeClass(e, "audio_page_player_btn_enabled"), t.shuffle(0)) : (addClass(e, "audio_page_player_btn_enabled"), t.shuffle(irand(1, 999999))), this.switchToSection(
        t)
}, AudioPage.onFilterRemoved = function(e, t) {
    var i = AudioPage(t);
    switch (e) {
        case "performer":
            var o = "audio_search_type_" + intval(i.isLayer()),
                a = window.radioBtns[o];
            radiobtn(a.els[0], 0, o);
            break;
        case "sort":
            var s = i._searchSortFilter.options.defaultItems[0][0];
            i._searchSortFilter.selectItem(s, !1);
            break;
        case "lyrics":
            removeClass(geByClass1("_audio_fltr_with_lyrics", i._container), "on")
    }
    i.onSearchFiltersChanged()
    ...
    each(["performer", "lyrics", "sort"], function(t, i) {
        uiSearch.toggleFilter(s.searchInputEl, i, a[i], !!e[i])
    ...
    new Suggester(this.searchInputEl, {
            ...
            sidePadding: "47px",
            onSelect: function(e) {
                val(t.searchInputEl, e[3])
            },
            onChoose: function(e) {
                val(t.searchInputEl, e[3]), t.searchAudios(e[3])
            }
    ...
    if (!this._friendSearchInProgress) {
        ...
        each(geByClass("_audio_friend", this._container), function() {
            i._shownFriends.push(domData(this, "id"))
        }), ajax.post("al_audio.php", {
            act: "more_friends",
    ...
    if (this.isLayer() ? s = r == this.ap.getCurrentPlaylist() : (d = e.getType() == AudioPlaylist.TYPE_POPULAR && this.options.md, s = this.options.reorderHash && e.getType() ==
        ...
        var n = this.isLayer() ? {} : {
            ...
            onDragEnter: function(t, o) {
                var a = i(t);
                a != e.getAlbumId() && (addClass(o, "audio_item_drag_over_album"), addClass(t, "audio_album_drop"))
            },
            onDragLeave: function(e, t) {
                removeClass(t, "audio_item_drag_over_album"), removeClass(e, "audio_album_drop")
            },
            onDragDrop: function(t, o) {
                removeClass(o, "audio_item_drag_over_album"), removeClass(t, "audio_album_drop");
                var s = i(t),
                    r = domData(o, "full-id");
    ...
    if (this.isLayer()) {
        ...
        var t = (getXY(e)[1], getSize(e)[1] + getSize(this.searchInputEl)[1]),
            ...
            r = getSize(s)[1],
            d = Math.max(a, r);
        d > t ? setStyle(e, "padding-bottom", d - t + i - 1) : setStyle(e, "padding-bottom", null), setTimeout(function() {
            var e = getAudioPlayer()
                .layer;
            e && e.sb.update()
        }, 1)
    }
}, AudioPage.prototype._initAutoList = function(e, t, i) {
    var o = this,
        a = getAudioPlayer(),
    ...
    this._autoList = new AutoList(s, {
        ...
        onRendered: function() {
            if (i(!1), o._updateLayerRowsBottomPadding(), a.updateCurrentPlaying(), !l)
                if (l = !0, o.isLayer()) {
        ...
        onNeedRows: function(t, l, u, _) {
            function c(e) {
                if (e) {
                    if (0 == l && (s.innerHTML = ""), e.getType() == AudioPlaylist.TYPE_FEED && e != a.getCurrentPlaylist()) g = e.getItemsList()
                        ...
                        h = e.getAudiosList()
                            .slice(l, l + d);
                        var i = e.getType() == AudioPlaylist.TYPE_SEARCH && a.getCurrentPlaylist() != e,
                            r = !1;
                        if (i) {
                        ...
                        each(h, function(t, o) {
                            i && l + t == e.getLocalFoundCount() && g.push("<h3>" + langNumeric(e.getTotalCount(), a.langs.audio_global_search_found, !0) +
            ...
            var h, g = [];
            n = l, _ || i(!0), l > 0 && (show(r), lockButton(r)), e.load(l, function(e) {
                c(e)
            })
        }
    }), cur.destroy.push(function() {
    ...
    if (this.isLayer()) {
        ...
        return setStyle(i, {
            ...
            if (cur.audioCancelMenuScroll) a = o = 0;
                ...
                var n = t.scrollTop - o;
                a -= n, a = Math.max(getSize(t)[1] - getSize(e)[1], a), a = Math.min(0, a)
            }
            setStyle(e, "top", a), o = t.scrollTop, delete cur.audioCancelMenuScroll;
            var l = geByClass1("_audio_padding_cont", r._container);
            t.scrollTop > 0 ? (setStyle(l, {
                "padding-top": s[1]
            }), addClass(i, "fixed")) : (setStyle(l, {
                "padding-top": null
            }), removeClass(i, "fixed"))
        })
    }
    ...
    var t = geByClass1("ui_search_fltr_control", this._container),
        i = geByClass1("ui_search_fltr", i);
    hideProgress(i), e && showProgress(i), hasClass(t, "shown") || toggleClass(gpeByClass("_wrap", this.searchInputEl), "ui_search_loading", e)
    ...
    toggleClass(o, "audio_search_rows", e.getType() == AudioPlaylist.TYPE_SEARCH), toggleClass(o, "audio_popular_rows", e.getType() == AudioPlaylist.TYPE_POPULAR), toggleClass(
        ...
        if (a) {
            var s = geByClass1("_ui_item_audio_" + e.getId(), i._getMenuEl());
            if (!s) switch (e.getType()) {
                case AudioPlaylist.TYPE_POPULAR:
                    s = geByClass1("_ui_item_audio_popular_" + i.options.oid, i._getMenuEl());
                    break;
                case AudioPlaylist.TYPE_RECOM:
                    s = geByClass1("_ui_item_audio_recoms_" + i.options.oid, i._getMenuEl());
                    break;
                case AudioPlaylist.TYPE_FEED:
                    s = geByClass1("_ui_item_audio_feed_" + i.options.oid, i._getMenuEl())
            }
            if (s && isVisible(s) ? (uiRightMenu.switchMenu(s), uiRightMenu.showProgress(s)) : i._hideMenuItemProgress(), !s && e.getType() == AudioPlaylist.TYPE_ALBUM &&
                e.getOwnerId() != vk.id) {
                var r = geByClass1("_audio_friend_" + e.getOwnerId(), i._container);
                r && showProgress(r)
            }
            e.getType() == AudioPlaylist.TYPE_SEARCH && uiSearch.showProgress(i.searchInputEl)
        } else data(o, "playlist", e), unlockButton(geByClass1("audio_more_friends_btn")), i._hideMenuItemProgress(), i._updateAdditionalBlocksAndRightMenu(e, t), i._updateEmptyPlaceholder(
            e), uiSearch.hideProgress(i.searchInputEl), i._initSorter(e)
    ...
    var e = this._getForeignTogglerEl(),
        ...
        o = (i.getAlbumId() + "")
        .replace(/[a-z]*/, ""),
        a = this.ap.getPlaylist(AudioPlaylist.TYPE_POPULAR, vk.id, (t ? "foreign" : "") + o);
    ...
    if (e.getAudiosCount()) hide(t);
        ...
        var i = "",
            o = e.getType();
        i = o == AudioPlaylist.TYPE_ALBUM ? e.getOwnerId() == vk.id ? AudioPlayer.getLang("audio_no_rec_load_msg")
    ...
    toggle(g, e.getType() == AudioPlaylist.TYPE_POPULAR), setTimeout(function() {
            uiRightMenu.hideSliding(i._getMenuEl())
        ...
        this._blocks || {}, e.getType() == AudioPlaylist.TYPE_SEARCH && d.search && (this._blocks[e.getId()] = d.search), each(d, function(e, t) {
            i._blocks[e] = i._blocks[e] || t
    ...
    e == getAudioPlayer()
        .getCurrentPlaylist() && p.push("friends"), each(p, function(e, t) {
            if (t) {
                var o = "right";
                0 == t.indexOf("popular_") && (o = "top");
                var a = geByClass1("_audio_additional_blocks_wrap_" + o, i._container),
                    s = geByClass1("_audio_additional_block_" + t, i._container);
                !s && i._blocks[t] && (s = se(rs(i.options.additionalBlockSectionTpl, {
                    block_id: t
                })), val(s, i._blocks[t]), a.appendChild(s)), show(s), show(a)
            }
    ...
    var e = !1,
        t = !1;
    this._prevSearchPlaylistId = null, cur.nav.push(this._nav_func = function(i, o, a, s) {
        if (each(cur._audioAddRestoreInfo || {}, function(e, t) {
                ("deleted" == t.state || "recom_hidden" == t.state) && getAudioPlayer()
        ...
        if (d) {
            ...
            if (l != vk.id && !inArray(i[0], ["audios" + l]) || l == vk.id && !inArray(i[0], ["audios" + l, "audio"])) return !0
        }
    ...
    var t = this._myPlaylist ? this.ap.getPlaylist(this._myPlaylist) : this.ap.getPlaylist(AudioPlaylist.TYPE_ALBUM, vk.id, AudioPlaylist.ALBUM_ALL);
    this.switchToSection(t);
    var i = t.getAudiosList()[0];
    i && (this._readyAudio = i, this._initPlayer(!0), cur.audioPage && (cur.audioPage._readyAudio = i, cur.audioPage._initPlayer(!0))), window.tooltips && tooltips.hideAll(),
    ...
    function i(e, t) {
        ...
        var i, o = s.getDurationType(),
            a = intval(e.duration);
        return i = o && 1 != t ? "-" + formatTime(Math.round(a - t * a)) : formatTime(Math.round(t * a))
    }

    function o() {
        if (a._trackSlider) {
            var e = AudioUtils.asObject(a._readyAudio || s.getCurrentAudio()),
                t = e.ownerId != a.options.oid;
            p !== t && (toggle(g, t), p = t), cur._audioAddRestoreInfo = cur._audioAddRestoreInfo || {};
            var i = cur._audioAddRestoreInfo[e.fullId];
            addClass(g, "no_transition"), toggleClass(g, "audio_player_btn_added", !(!i || "added" != i.state)), removeClassDelayed(g, "no_transition"), toggleClass(h,
    ...
    }), this.ap.on(this, AudioPlayer.EVENT_ADDED, function(e, t) {
        e = AudioUtils.asObject(e), e && e.fullId == t && addClass(g, "audio_player_btn_added")
    }), this.ap.on(this, AudioPlayer.EVENT_REMOVED, function(e, t) {
        e = AudioUtils.asObject(e), e && e.fullId == t && removeClass(g, "audio_player_btn_added")
    }), this.ap.on(this, AudioPlayer.EVENT_PLAY, function(e, s, r) {
        delete a._readyAudio, data(d, "audio", e), o(), t(e), addClass(c, "audio_playing"), s && !cur.audioStartReadyAudio && (a._trackSlider.setBackValue(0), a._trackSlider
            .setValue(0), _.innerHTML = i(e, 0), n.setAttribute("title", ""), n.titleSet = !1)
    }), this.ap.on(this, AudioPlayer.EVENT_PAUSE, function(e) {
        removeClass(c, "audio_playing")
    }), this.ap.on(this, AudioPlayer.EVENT_BUFFERED, function(e, t) {
        a._trackSlider.setBackValue(t)
    }), this.ap.on(this, AudioPlayer.EVENT_VOLUME, function(e, t) {
        a._volumeSlider.setValue(t)
    }), this.ap.on(this, AudioPlayer.EVENT_UPDATE, function(e, t) {
        e && t && (_.innerHTML = i(e, t))
    }), this.ap.on(this, AudioPlayer.EVENT_PROGRESS, function(e, t) {
        a._trackSlider.setValue(t), _.innerHTML = i(e, t)
    }));
    var y = this.ap.getCurrentAudio() || this._readyAudio;
    y && (domData(d, "audio", JSON.stringify(y)), t(y), toggleClass(c, "audio_playing", this.ap.isPlaying()), _.innerHTML = i(y, 1), e && (this._trackSlider.setValue(0), this._trackSlider
        ...
        if (a) {
            ...
            if (e || !(c > u || u > c + n)) {
                ...
                t.isLayer() ? (cur.audioCancelMenuScroll = !0, r.scrollTop = h, setTimeout(function() {
                    cur.audioCancelMenuScroll = !0, r.scrollTop = h, t.getLayer()
    ...
    if (this.ap.isPlaying()) this.ap.pause();
        ...
        var t = this.getCurrentPlaylist(),
            i = this.ap.getCurrentPlaylist(),
            o = this.ap.getCurrentAudio(),
            a = this._readyAudio ? this._readyAudio : o;
        if (a = a ? a : t.getAudioAt(0), AudioUtils.isClaimedAudio(a)) {
        ...
        var d; - 1 != t.indexOfAudio(a) ? d = t : i && -1 != i.indexOfAudio(a) ? d = i : (d = new AudioPlaylist(AudioPlaylist.TYPE_TEMP, vk.id), d.addAudio(a)), delete this._readyAudio,
