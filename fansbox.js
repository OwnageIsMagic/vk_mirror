var FansBox = {
    init: function(box, opts) {
        ...
        box.setOptions({
            width: opts.intro ? 630 : 620,
            bodyStyle: opts.intro ? 'padding: 0px' : 'margin-top: 50px',
            onShow: function() {
                                ...
                                ajax.post(cur.fnbxPage, {
                                    ...
                                    onDone: function(result) {
                                        ...
                                        cur.fnbxIdolsIndex[oid] = new vkIndexer(cur.fnbxIdolsCache[oid].all, function(obj) {
                                            ...
                                            if (cur.fnbxOwnerId == oid && curBox()
                                                .tbId == cur.fnbxId && curBox()
                                                .tbVis == 'idols') {
        ...
        placeholderSetup('fans_idol_search_inp', {
            back: true
        });
        box.tbcOnLoad();
    ...
    moreIdols: function(force) {
        ...
        var list = cur.fnbxIdolsCache[oid].all,
            q = trim(val('fans_idol_search_inp'));
    ...
    genIdolEmpty: function(q) {
        ...
        var text = trim(q) ? (getLang('fans_idols_not_found') + '<br><br>' + getLang('groups_you_can_find')
            .replace('{term}', lnk + clean(q) + '</a>')
            .replace('{link}', lnk)
    ...
    genIdolRow: function(row, name) {
        var oid = row[0],
            evs = row[6] ? ' onmouseover="FansBox.bigphOver(this, ' + oid + ')"' : '',
...
<div class="fans_idol_row inl_bl">\
  ...
  <div class="fans_idol_info fl_l">\
    <div class="fans_idol_name"><a class="fans_idol_lnk" href="', href, '">', name,
            '</a></div>\
    <div class="fans_idol_status">', status, '</div><div class="fans_idol_size">', size, '</div>\
    <div id="fans_idol_sub', oid,
            '" class="button_blue fans_idol_sub"', ((row[1] || !btns) ? non : ''), '>\
      <button onclick="FansBox.subscribe(this, ', oid, ')">', getLang(
                'public_subscribe'), '</button>\
    </div>\
    <div id="fans_idol_feedact', oid, '" class="fans_idol_space"', (row[1] && btns ? '' : non), '>',
            feed_act, '</div>\
        ...
        ajax.post(cur.fnbxPage, {
            ...
            showProgress: function() {
                ...
                show(domNS(el) || domPN(el)
                    .appendChild(ce('span', {
                        className: 'progress_inline fans_idol_unsub_p'
                    })));
            },
            hideProgress: function() {
                show(el);
    ...
    onScroll: function() {
        ...
        var bt = lastWindowHeight,
            objMore = ge('fans_more_link' + curBox()
                .tbCur);
    ...
    bigphOver: function(obj, uid) {
        ...
        var o = obj.firstChild,
            ch = cur.fnbxPhCache[uid];
        if (o.tagName != 'A' || o.className != 'fans_fanph') {
            (o = obj.insertBefore(ce('a', {
                className: 'fans_fanph',
                href: ch && ch._id ? ('/photo' + ch._id + '?all=1') : ('/albums' + uid),
                innerHTML: '<span class="fans_fanph_label">' + getLang('global_photo_full_size') + '</span>'
            }), obj.firstChild))
            .onclick = FansBox.bigphClick.pbind(uid);
            o._uid = uid;
        }
    ...
    bigphClick: function(uid, ev) {
        ...
        var ch = cur.fnbxPhCache[uid],
            t = curBox()
            .tbCur,
            o = cur.fnbxPhShown[uid],
            obj = domPN(o);
        if (!o || !obj) return;
        if (ch === undefined) {
        ...
        extend(ch, {
            ...
            onHide: function() {
                cur.fnbxLoaded = function() {
                    ...
                    domPN(cont)
                        .replaceChild(cur.fnbxBack.cont, cont);
                ...
                showBox(cur.fnbxPage, {
                    act: cur.fnbxAct || 'box',
                    tab: cur.fnbxBack.initial,
                    oid: cur.fnbxBack.oid
                }, {
                    cache: 1
                });
            }
