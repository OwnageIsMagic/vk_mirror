...
function editComment(oid, cid) {
    ...
    ajax.onDone = function(obj, text) {
        ...
        commtext.innerHTML = '<textarea id="comment' + cid + 'edit" style="' + padding_left + 'width: ' + width + '; height: ' + commtext.offsetHeight + 'px; margin-top: ' +
            margin_top + '; margin-left: ' + margin_left + '; line-height: 14px; margin-bottom: 0px;" onkeydown="if (event.keyCode == 27) cancelEditComment(' + cid +
            '); else if (event.keyCode == 13 && event.ctrlKey) doEditComment(' + oid + ', ' + cid + ')">' + text + '</textarea>' +
...
function cancelEditComment(cid) {
    ...
    separator(comment)
        .style.height = '15px';
    geByClass('text', comment)[0].innerHTML = ge('comment' + cid + 'text')
        .innerHTML;
}
    ...
    Ajax.Post({
        ...
        onDone: function(res, text) {
            ...
            if (s) {
                ...
                box.innerHTML = ge('photo_' + pid + '_desc_old')
                    .innerHTML;
            }
        },
...
function gotPhotoInfo(ajax, res) {
    ...
    if (r[8] && ge('to_album_link')) ge('to_album_link')
        .innerHTML = r[8];
    ...
    ge('photolikes_wrap')
        .innerHTML = r[10];
    setStyle('photolikes_wrap', 'display', ge('photolikes')
...
function failedPhotoInfo(ajax, res) {
    ge('photoinfo')
        .innerHTML = "<div style='text-align:center;'>" + photo_error_occurred + "</div>";
}
...
function showPhoto(params) {
    ...
    ge('myphotolink')
        .innerHTML = "<img src='" + new_url + "' id='myphoto' galleryimg='no'>";
    ge('myphoto')
        .onload = function() {
            ge('myphoto')
                .onload = null;
    ...
    if (browser.msie) {
        var ov_height = parseInt(ge('photocaptionleft')
            .clientHeight) + parseInt(ge('photoinfo')
            .clientHeight) + 20;
        var overlay = "<div style='position:absolute; top: 0px; left:-10px; width:628px; height:" + ov_height +
            "px; background-color: #F7F7F7; z-index:1300; opacity: 0.80;filter:alpha(opacity=80);'></div>";
        ge('ph_overlay')
            .innerHTML = overlay;
    } else {
        ge('photocaptionleft')
            .className = 'ph_info_dis';
        ge('photoinfo')
            .className = 'ph_info_dis';
    }
    var progress =
        "<div style='position:absolute; top: -55px; left:-10px; width:628px; background-color: transparent; text-align:center; padding: 20px; z-index:300'><img src='images/progress7.gif'/></div>";
    ge('pr_overlay')
        .innerHTML = progress;
    return true;
}
    ...
    Ajax.Post({
        ...
        onDone: function(res, text) {
            ...
            if (ge('actionMessage' + pid)) {
                ...
                if (sure) {
                    box.innerHTML = text + '<div id="photo_' + pid + '_desc_old" style="display: none">' + ge('photo_' + pid + '_desc_old')
                        .innerHTML + '</div>';
                } else {
...
function showText(obj, mess) {
    ...
    if (mess == 1) {
        ge(obj)
            .innerHTML = "<h4>" + photo_rotating + "<img src='http://vkontakte.ru/images/upload.gif'></h4>";
    }
    if (mess == 2) {
        ge(obj)
            .innerHTML = "<div id='uploadingInner'><h4>" + photo_uploading_photo + "<img src='http://vkontakte.ru/images/upload.gif'></h4><p style='margin-top:11px'>" +
            photo_dont_close + "</p></div>";
    }
}
...
function generateUploadList(photos) {
    for (var i in photos) {
        if (ge('uploader_list')) ge('uploader_list')
            .innerHTML += '<li id="photo' + i +
            '" class="default clearFix" style="border-bottom: 1px solid #DAE2E8; margin: 0px; height: 22px; z-index:0; line-height: 0px;">\
      <div id="progress' + i +
            '" style="position:absolute; background-color: #F0F2F5; width: 0; height: 22px;"></div>\
      <div class="name fl_l" style="position:relative; z-index:1; padding-left: 5px; margin-top: 10px; width: 300px; text-align: left;">' +
            photos[i] +
            '</div>\
      <div class="select fl_l" style="position:relative; z-index:1; margin-top: 2px; width: 40px; text-align: center; display:none">\
        <div style="background-color: rgb(196, 210, 225); width: 17px; height: 17px; margin: auto;" id="imgX' +
            i + '" onmouseover="changeX(\'imgX' + i + '\', 0)" onmouseout="changeX(\'imgX' + i + '\', 1)" onclick="cancelImageUploading(' + i +
            ');">\
...
function likePhoto(oid, photo_id, hash) {
    ...
    Ajax.Send('like.php?act=a_do_like', {
        ...
        onSuccess: function(o, t) {
            ...
            res.stats = res.stats.replace('{link}', '<a href="#" onclick="return showLikesBox(' + oid + ', ' + photo_id + ');">')
                .replace('{/link}', '</a>');
