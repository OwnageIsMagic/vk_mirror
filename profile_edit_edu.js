var ProfileEditorEdu = {
        ...
        return ce('div', {
            ...
            innerHTML: '<div id="uni_content' + uni_id + '">' +
                ...
                this.genOneRow('u', 'university', uni_id, getLang('select_university'), '',
                    '<span class="add_item_link fl_l" id="u_add_uni_to_db' + uni_id + '"><b><a href="" onclick="return ProfileEditor.addItemBox(\'uni\', ' + uni_id +
                    ')">' + getLang('profileEdit_uni_not_in_db') + '</a></b></span>') +
                this.genOneRow('u', 'faculty', uni_id, getLang('select_faculty'), '',
                    '<span class="add_item_link fl_l" id="u_add_fac_to_db' + uni_id + '"><b><a href="" onclick="return ProfileEditor.addItemBox(\'faculty\', ' +
                    uni_id + ')">' + getLang('profileEdit_fac_not_in_db') + '</a></b></span>') +
                this.genOneRow('u', 'chair', uni_id, getLang('select_chair'), '',
                    '<span class="add_item_link fl_l" id="u_add_chair_to_db' + uni_id + '"><b><a href="" onclick="return ProfileEditor.addItemBox(\'chair\', ' +
                    uni_id + ')">' + getLang('profileEdit_chair_not_in_db') + '</a></b></span>') +
    ...
    genOneSchoolRow: function(id) {
        ...
        return '<div class="pedit_edu_row" id="row_' + key + '"><div class="label fl_l ta_r">' + getLang('select_school') + '</div>' +
            ...
            '</div>' +
            '<span class="add_item_link fl_l" id="s_add_school_to_db' + id + '"><b><a onclick="ProfileEditor.addItemBox(\'school\', ' + id + ')">' + getLang(
                'profileEdit_school_not_in_db') + '</a></b></span>' +
            '</div>';
    },
    ...
    addObjectToDB: function(country_val, rows, initer, handler) {
        ...
        addBox.addButton(getLang('global_cancel'), function() {
            addBox.hide();
        }, 'no');
    ...
    addToDB: function(obj, id) {
        cur.addBox = showBox('al_profileEdit.php', {
            act: 'a_add_' + obj + '_box',
            id: id
        });
        return false;
    ...
    addUniversity: function() {
        ...
        ge('unis')
            .appendChild(this.genUniRow(new_uni.id));
    ...
    addSchool: function() {
        ...
        ge('schools')
            .appendChild(this.genSchoolRow(new_school.id));
    ...
    deleteUniversity: function(id) {
        ...
        if (intval(ge('u_university' + id)
            ...
            hide('uni_content' + id);
            ge('uni_deleted' + id)
                .firstChild.innerHTML = getLang('profileEdit_uni_will_be_deleted');
            show('uni_deleted' + id);
        } else {
    ...
    deleteSchool: function(id) {
        ...
        if (intval(ge('s_school' + id)
            ...
            ge('school_deleted' + id)
                .firstChild.innerHTML = getLang('profileEdit_school_will_be_deleted');
        ...
        if (isVisible('uni')) {
            ...
            doneHandler = function(response) {
                ...
                for (var i = 0; i < to_remove.length; ++i) {
                    ...
                    ge('uni' + cur.unis[index].id)
                        .parentNode.removeChild(ge('uni' + cur.unis[index].id));
                    cur.unis[index] = cur.unis[cur.unis.length - 1];
            ...
            doneHandler = function(response) {
                ...
                for (var i = 0; i < to_remove.length; ++i) {
                    var index = to_remove[i];
                    ge('school' + cur.schools[index].id)
                        .parentNode.removeChild(ge('school' + cur.schools[index].id));
                    cur.schools[index] = cur.schools[cur.schools.length - 1];
                    for (var j = i + 1; j < to_remove.length; ++j) {
