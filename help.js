var Help = {
    ...
    submitCCForm: function() {
        ...
        var params = {
            act: 'a_cc_request',
            content_name: ge('ccform_content_name')
                .value,
            content_links: ge('ccform_content_links')
                .value,
            possessor: (legal ? 1 : 0),
            name: ge('ccform_' + infix + '_name')
                .value,
            email: ge('ccform_' + infix + '_email')
                .value,
            country: (legal ? cur.legalCountry : cur.naturalCountry)
                .val(),
            city: (legal ? cur.legalCity : cur.naturalCity)
                .val(),
            region: ge('ccform_' + infix + '_region')
                .value,
            address: ge('ccform_' + infix + '_address')
                .value,
            repr_name: (legal ? ge('ccform_legal_reprname')
                .value : ''),
            repr_pos: (legal ? ge('ccform_legal_reprpos')
                .value : '')
        };
        ...
        if (params.country < 1) {
            (legal ? cur.legalCountry : cur.naturalCountry)
            .focus();
            return false;
        }
        if (params.city < 1) {
            (legal ? cur.legalCity : cur.naturalCity)
            .focus();
            return false;
        }
        if (params.address < 9) {
    ...
    uploadComplete: function(mid, photo, server, thumb) {
        ...
        ge('ccform_doc_photo')
            .value = photo;
        ge('ccform_doc_server')
            .value = server;
        show('doc_photos');
        ge('doc_photos')
            .innerHTML = '<div id="photo"><img id="photo_img" src="' + thumb +
