...
(function(i, s, c, d, n, b) {
    ...
    function y() {
        ...
        var D = {
            ...
            B = '{"a":[1,2,3]}';
        if (JSON && typeof JSON.stringify === "function" && JSON.stringify(D)
            .replace((/\s/g), "") === B) {
            return JSON
        }
        if (Object.toJSON) {
    ...
    easyXDM.Rpc = function(D, C) {
        ...
        var B = v(g(D)
            .concat([new easyXDM.stack.RpcBehavior(this, C), {
                callback: function(G) {
                    if (D.onReady) {
                        D.onReady(G)
                    }
...
function getVotes(from_box, count) {
    ...
    window.getVotesBox.removeButtons()
        .addButton({
...
(function() {
    ...
    function installApp(app_id) {
        ...
        Ajax.Send('apps.php', {
            ...
            if (response.result) {
                ...
                if (ge('app_message')) ge('app_message')
                    .innerHTML = response.result;
                if (response.links) {
                    if (ge('summary_links')) ge('summary_links')
                        .innerHTML = response.links;
    ...
    window.showPaymentBox = function(votes) {
        ...
        text += '  <div class="error" style="margin: 0px; background-color: #F9F6E7; border: 1px solid #D4BC4C;">' + window.lang_not_enough_votes +
            ' <a href="payments.php?act=addfunds" onclick="return getVotes(1)">' + window.lang_get_votes + '</a></div>';
        text += '</div>';
        text +=
            '<div id="a_money_to_votes" style="display: none; background-color: #F9F6E7; border: 1px solid #D4BC4C; padding: 8px 11px; margin: 10px 0px 0px; text-align: center;">';
        ...
        text += '</div>';
        text += '<div style="margin-top:10px; vertical-align:top; font-size:9px; background:#F7F7F7; border:1px solid #DAE1E8; padding:5px 10px">' + window.lang_votes_agreement +
            '</div>';
    ...
    window.onVotesChange = function(obj) {
        ...
        if (ge('votesToAddFlex')) {
            ge('votesToAddFlex')
                .innerHTML = langNumeric(value, votes_flex);
        }
        if (value <= window.userBalance) {
            ...
            ge('a_money_to_votes_text')
                .innerHTML = lang_money_to_votes.replace('{money}', '<b>' + m + '</b>')
                .replace('{votes}', '<b>' + v + '</b>');
    ...
    function addVotes() {
        ...
        var votes = intval(ge('votesToAdd')
            .value);
        if (!votes) {
                ...
                if (result.error) {
                    ...
                    if (result.balance != undefined) {
                        ...
                        if (ge('votes_balance_str')) {
                            ge('votes_balance_str')
                                .innerHTML = result.votes_balance_str;
                        }
                    }
                    setTimeout(function() {
                        paymentBox.hide()
