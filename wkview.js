var WkView = {
    ...
    edit: function() {
        ...
        ajax.post('wkview.php', params, {
            ...
            onDone: function(content, js, info, options) {
                ...
                ge('wk_layer_title')
                    .innerHTML = options.lang['pages_edit_title'];
                show('wk_summary');
                eval(js);
    ...
    updateShadow: function() {
        ...
        if (toolbox) {
            if (wkcur.fixedTop) {
                ...
                ge('wk_top_toolbox_shadow')
                    .style.top = (size[1] - 1) + 'px';
            }
        }
    },
    ...
    show: function(title, html, options, script, ev) {
        ...
        if (options.type == 'wall') {
            ...
            if (options) {
                content = '<div class="wk_text wk_wiki_content' + (options.className ? ' ' + options.className : '') + '" onclick="return WkView.wikiClick(this, event);">' +
                    content + '</div>';
            }
        }
  ...
  <div id="wk_box" onclick="wkcur.wkClicked = true;">\
    ...
    <div id="wk_summary" class="fl_l"><span class="summary" id="wk_layer_title">' + title +
                '</span></div>\
    <div id="wk_content">' + content +
                '</div>\
    <div class="clear"></div>\
  </div>\
  \
...
<div id="wk_left_wrap"><div id="wk_left" style="' +
                hideLeft +
                '" class="wk_left no_select"></div></div>\
...
<div class="no_select" id="wk_left_nav" style="' +
                hideLeft + '" ' +
                'onmouseover="WkView.activate(wkcur.wkLeft)" onmouseout="WkView.deactivate(wkcur.wkLeft)" onmousedown="wkcur.wkClicked = true; WkView.back();" onselectstart="return cancelEvent(event);"></div>\
<div class="no_select" id="wk_right_nav" ' +
                'onmouseover="WkView.activate(wkcur.wkRight)" onmouseout="WkView.deactivate(wkcur.wkRight)" onmousedown="if (!wkcur.noClickHide) { wkcur.wkClicked = true; WkView.hide(); }"></div>\
...
<div id="wk_right_arrow_bg" class="no_select" onclick="return WkView.navigate(this, event, 1);" onmouseover="WkView.activateArrow(true, event)" onmouseout="WkView.deactivateArrow(true, event)"><div class="wk_arrow_bg_inner"></div></div>';
    ...
    wysiwygInit: function() {
        ...
        extendedToolBar +=
            'table,table_delete,insert_row_before,insert_row_after,insert_col_before,insert_col_after,delete_row,delete_col,col_width,sub,sup,citate,outdent,indent,pre,character,hider';
        ...
        var editorParams = {
            ...
            html: ge('pages_html')
                .value,
            wiki: ge('pages_body') ? ge('pages_body')
                .value : '',
            height: 326,
    ...
    likeOut: function() {
        if (!wkcur.liked) {
            setTimeout(animate.pbind(ge('wk_like_icon'), {
                opacity: 0.4
            }, 200, false), 1);
        }
    },
    ...
    wallBeforeInitPost: function(opts) {
        ...
        while (true) {
            ...
            postEl = postEl || ge('reply_fakebox' + post) || ge('reply_box' + post) || ge('replies' + post) || ge('feedback_row_wall' + post) || (wkcur.hl_reply && (ge(
                'post' + wkcur.hl_reply) || ge('feedback_row_wall_reply' + wkcur.hl_reply))) || ge('feed_rows');
            if (!postEl) {
                break;
            }
            size = getSize(postEl);
        ...
        if (opts.options.wall_tpl) {
            ...
            extend(wkcur, {
                ...
                wallMyDeleted: {},
                tsDiff: opts.options.wall_tpl && opts.options.wall_tpl.abs_timestamp ? Math.round((vkNow() / 1000 - opts.options.wall_tpl.abs_timestamp) / 900.0) *
                    900 : 0,
                wallMyOpened: {},
                wallMyReplied: {},
                wallMyRepliesCnt: 0
        ...
        ajax.post('al_wall.php', {
            ...
            onDone: function(msg, additional, need_confirm) {
                ...
                if (del && hasClass(del, 'wl_post_deleted')) {
                    ...
                    p.parentNode.insertBefore(ce('div', {
                        id: 'post_del' + post,
                        innerHTML: msg,
                        className: 'wl_post_deleted'
                    }), p.nextSibling);
                    hide(p);
                }
    ...
    wallUpdateReplies: function() {
        ...
        if (!wkcur.reverse && wkcur.offset > 0) {
            if (wkcur.offset > 100) {
                label = langNumeric(100, wkcur.lang.wall_show_n_of_m_last)
                    .replace('{count}', wkcur.count);
            } else {
    ...
    likesTab: function(tab) {
        ajax.post('wkview.php', {
            ...
            cache: 1,
            showProgress: setStyle.pbind('wk_likes_tabs_prg', {
                visibility: 'visible'
            }),
            hideProgress: setStyle.pbind('wk_likes_tabs_prg', {
                visibility: 'hidden'
            }),
    ...
    likesBigphOver: function(obj, uid) {
        ...
        if (o.tagName != 'A' || o.className != 'wk_likes_likerph') {
            o = obj.insertBefore(ce('a', {
                className: 'wk_likes_likerph',
                href: ch && ch._id ? ('/photo' + ch._id + '?all=1') : ('/albums' + uid),
                innerHTML: '<span class="wk_likes_likerph_label">' + wkcur.lang.global_photo_full_size + '</span>'
            }), obj.firstChild);
