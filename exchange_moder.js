var ExchangeModer = {
    ...
    processRequest: function(action, requestId) {
        ...
        if ((action === 'disapprove') || (action === 'force_disapprove')) {
            params.moder_rules = cur.uiReasonsControls[requestId].getSelectedItems()
                .join(',');
        }
        ...
        ajax.post('/exchangemoder', params, {
            onDone: function(response, js) {
                ...
                if (response.requests) {
                    ge('exchange_moder_requests_container')
                        .innerHTML += response.requests;
                }
                ...
                if (response.button) {
                    ge('exchange_moder_button_container')
                        .innerHTML = response.button;
                }
                ge('exchange_moder_summary')
                    .innerHTML = response.summary;
                if (js) {
                    eval('(function(){' + js + ';})()');
    ...
    doRefund: function(hash) {
        ...
        var refundAmount = parseInt(ge('exchange_moder_refund_amount')
            .value);
        if (!refundAmount) {
            showFastBox(getLang('global_error'), '������� ����� �����������');
            return;
        ...
        var refundComment = ge('exchange_moder_refund_comment')
            .value;
        if (!refundComment) {
            showFastBox(getLang('global_error'), '������� �����������');
            return;
    ...
    doUpdateCheckMoneyQueue: function(hash) {
        ajax.post('/exchangemoder', {
            ...
            onDone: function(html) {
                ge('exchange_moder_queue_container')
                    .innerHTML = html;
            },
            showProgress: function() {
    ...
    doCheckMoney: function(hash) {
        ajax.post('/exchangemoder', {
            act: 'a_check_money',
            union_id: ge('exchange_moder_check_money_union_id')
                .value,
            hash: hash
        }, {
            onDone: function() {
                ge('exchange_moder_check_money_union_id')
                    .value = '';
