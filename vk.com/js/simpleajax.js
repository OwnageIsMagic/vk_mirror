...
function show_add_wall_media(type) {
    ...
    if (ge('reply_to')) {
        if (type == 'status') {
            ge('reply_to')
                .value = -1;
            if (ge('group_status') && !ge('group_status')
                .value) ge('reply_to')
                .value = 0;
        }
    }
...
onDomReady(function() {
    if (wall_post_types.length) {
        ...
        for (var i in wall_post_types) {
            ...
            rows += '<a onfocus="this.blur()" id="wall_post_type' + i + '" style="background-image: url(' + row.icon + '); background-position: ' + row.bgpos[0] + 'px ' +
                row.bgpos[1] + 'px;' + spec_style + '" href="' + row.url + '">' + row.label.replace(/\s/g, '&nbsp;') + '</a>';
            if (status_menu && row.type != 'app' /*&& row.type != 'graffiti'*/ ) {
                status_rows += '<a onfocus="this.blur()" id="wall_post_type_status' + i + '" style="background-image: url(' + row.icon + '); background-position: ' + row.bgpos[
                    0] + 'px ' + row.bgpos[1] + 'px;' + spec_style + '" href="' + row.url + '">' + row.label.replace(/\s/g, '&nbsp;') + '</a>';
            }
        }
        if (wall_menu) {
            ge('add_wall_rows')
                .innerHTML = '<table cellspacing="0" cellpadding="0"><tr><td class="add_wall_side"><div>&nbsp;</div></td><td><div class="rows">' + rows +
                '</div><div class="add_wall_bottom"></div><div class="add_wall_bottom2"></div></td><td class="add_wall_side"><div>&nbsp;</div></td></tr></table>';
        }
        if (status_menu) {
            ge('add_wall_rows_status')
                .innerHTML = '<table cellspacing="0" cellpadding="0"><tr><td class="add_wall_side"><div>&nbsp;</div></td><td><div class="rows">' + status_rows +
                '</div><div class="add_wall_bottom"></div><div class="add_wall_bottom2"></div></td><td class="add_wall_side"><div>&nbsp;</div></td></tr></table>';
        }
        for (var i in wall_post_types) {
...
function showGraffiti(grid, owner, elem) {
    ...
    graffitiBox.addControlsText(ge('graffiti_author' + grid)
        .innerHTML);
    graffitiBox.setOptions({
        title: ge('graffiti_title' + owner)
            .value,
        fullPageLink: elem.href
    });

    graffitiBox.content('<div style="width: 586px; overflow: hidden; height: 293px; background: #fff url(' + base_domain +
            'images/progress7.gif) no-repeat 50% 50%" onclick="graffitiBox.hide()"><img src="' + ge('graffiti_src' + grid)
            .value + '" /></div>')
        .show();
    return false;
}
...
function showWallPhoto(phid, owner, elem) {
    ...
    if (!wallImages[phid]) {
        ...
        wallImages[phid].src = ge('wallphoto_src' + phid)
            .value;
    }
    setTimeout(function() {
        ...
        wallPhotoBox.addControlsText(ge('wallphoto_author' + phid)
            .innerHTML);
        wallPhotoBox.setOptions({
            width: (intval(width) > 410) ? (width + 20) : 430,
            title: ge('wallphoto_title' + owner)
                .value,
            fullPageLink: elem.href
        });
        if (wallImages[phid].height) {
            wallPhotoBox.content('<div style="height: ' + wallImages[phid].height +
                    'px; text-align: center;" onclick="wallPhotoBox.hide()"><img style="background: #fff url(' + base_domain +
                    'images/progress7.gif) no-repeat 50% 50%" src="' + ge('wallphoto_src' + phid)
                    .value + '" /></div>')
                .show();
        } else {
            wallPhotoBox.content('<div class="box_loader"></div>')
                .show();
...
function postMedia(media, media_id, to_id, hash, options) {
    ...
    var reply_to = ge('reply_to') ? ge('reply_to')
        .value : 0;
    var el = reply_to == -1 ? ge('status_field') : ge('reply_field');
            ...
            Ajax.Post({
                ...
                onDone: function(obj, text) {
                    if (text && ge('uploadedGraffiti')) {
                        ge('uploadedGraffiti')
                            .innerHTML = '<img style="width: 586px; height: 293px" src="' + text + '" />';
                    } else if (box) {
...
window.onbeforeunload = function() {
    ...
    if (ge('graffitiDraw') && drawBox.isVisible()) ge('graffitiDraw')
        .confirmWhenExit();
...
function showApp(app_id, to_id, from_id, loc, href, photo) {
    ...
    showAppBox.removeButtons()
        .addButton({
            label: global_close,
            onClick: showAppBox.hide
        })
        .addControlsText('<a href="' + href + '">' + ge('go_to_app_label')
            .value + '</a>');
