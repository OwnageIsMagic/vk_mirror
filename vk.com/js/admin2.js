...
function report(wid, act, hash, pp, url) {
    ...
    ajax.onDone = function(obj, text) {
        ...
        if (json && json.html) {
            ge('wid' + wid)
                .innerHTML = "<div id='msg'>" + json.html + "</div>";
            if (json.html == "�������������� ��� ����������.") {
                msg = ge('wid' + wid)
    ...
    ajax.onFail = function(obj, text) {
        ge('wid' + wid)
            .innerHTML = "<div id='msg'>" + getLang('admin2_server_error') + " (" + text + ")</div>";
    };
    if (!url) url = 'admin';
    ajax.post('/' + url + '.php', {
        'act': act,
...
function changeName(id) {
    ...
    if (ge('typeId' + id)
        .value >= 0) {
        ge('itemType' + id)
            .innerHTML = "<select class='select' name='type" + id + "' id='type" + id + "' style='width:180px'><option id='" + id + "sc0' value = '0'>" + getLang('admin2_school') +
            "</option><option id='" + id + "sc1' value = '1'>" + getLang('admin2_gymnasium') + "</option><option id='" + id + "sc2' value = '2'>" + getLang('admin2_liceum') +
            "</option><option id='" + id + "sc3' value = '3'>" + getLang('admin2_internat') + "</option><option id='" + id + "sc4' value = '4'>" + getLang('admin2_evening') +
            "</option><option id='" + id + "sc5' value = '5'>" + getLang('admin2_music') + "</option><option id='" + id + "sc6' value = '6'>" + getLang('admin2_sports') +
            "</option><option id='" + id + "sc7' value = '7'>" + getLang('admin2_artistic') + "</option><option id='" + id + "sc13' value = '13'>" + getLang('admin2_art') +
            "</option><option id='" + id + "sc14' value = '14'>" + getLang('admin2_garten') + "</option><option id='" + id + "sc9' value = '9'>" + getLang('admin2_prof_liceum') +
            "</option><option id='" + id + "sc8' value = '8'>" + getLang('admin2_colledge') + "</option><option id='" + id + "sc10' value = '10'>" + getLang('admin2_tech') +
            "</option><option id='" + id + "sc12' value = '12'>" + getLang('admin2_spec_school') + "</option><option id='" + id + "sc11' value = '11'>" + getLang(
                'admin2_prof_tech') + "</option><option id='" + id + "sc15' value = '15'>" + getLang('admin2_prof') + "</option><option id='" + id + "sc16' value = '16'>" +
            getLang('admin2_driving_sch') + "</option></select>";
        ge(id + 'sc' + ge('typeId' + id)
                .value)
            .selected = "selected";
    }
    if (ge('typeId' + id)
        .value == -1) {
        fullname = ge('itemFullNameValue' + id)
            .value;
        ge('itemFullName' + id)
            .innerHTML = "<div>" + getLang('admin2_full_univ_name') + "</div><input type='text' style='width:400px; margin:10px 0px 5px 0px' value='" + fullname +
            "' id='newFullName" + id + "'>";
    }
    obj.innerHTML = "<input type='text' style='width:400px; margin:10px 0px 5px 0px' value='" + name + "' id='newName" + id + "'>";
    ge('changeLink' + id)
        .innerHTML = "<div id='oldChangeLink" + id + "' style='display: none'>" + ge('changeLink' + id)
        .innerHTML + "</div><a href='javascript: changeReady(" + id + ");'>" + getLang('admin2_ready') + "</a> <span>|</span> <a href='javascript: changeCancel(" + id + ");'>" +
        getLang('admin2_cancel') + "</a> <span>|</span> <a href='javascript: searchName(" + id + ", 1);'>" + getLang('admin2_search') + "</a>";
    ge('newName' + id)
        .focus();
}
...
function changeCancel(id) {
    ge('itemName' + id)
        .innerHTML = ge('trueItemName' + id)
        .value;
    ge('changeLink' + id)
        .innerHTML = ge('oldChangeLink' + id)
        .innerHTML; // "<a href='javascript: changeName("+id+");'>" + getLang('admin2_edit') + "</a> <span>|</span> <b style='font-weight:normal;' id='itemList"+id+"'><a href='javascript: searchName("+id+", -1);'>" + getLang('admin2_list') + "</a></b> <span>|</span> <a href='javascript: searchName("+id+", 0);'>" + getLang('admin2_search') + "</a>";
    if (ge('typeId' + id)
        .value >= 0) {
        ge('itemType' + id)
            .innerHTML = ge('typeName' + id)
            .value;
    }
    if (ge('typeId' + id)
        .value == -1) {
...
function showRecentHistory(hash) {
    ...
    mb.loadContent('names_admin.php', params)
        .show();
    return false;
}
...
function changeReady(id) {
    ...
    if (ge('typeId' + id)
        ...
        ge('typeId' + id)
            ...
            .value;
        ge('itemType' + id)
            .innerHTML = ge(id + 'sc' + ge('typeId' + id)
                .value)
            .innerHTML;
    }
    if (ge('typeId' + id)
        .value == -1) {
        ge('itemFullNameValue' + id)
            .value = ge('newFullName' + id)
            .value;
        ge('itemFullName' + id)
            .innerHTML = "";
    }
...
function searchName(id, newName) {
    ...
    if (newName == 1) {
        ajax.name = ge('newName' + id)
            .value;
    } else {
        ajax.name = ge('trueItemName' + id)
            .value;
    }
    if (newName == -1) {
        ajax.name = "";
        ge('itemList' + id)
            .innerHTML = "<a href=\"javascript: hideList(" + id + ");\">" + getLang('admin2_hide_list') + "</a>";
    } else if (newName == 0) {
        ge('itemSearch' + id)
            .innerHTML = "<a href=\"javascript: hideList(" + id + ");\">" + getLang('admin2_hide_list') + "</a>";
    }
...
function hideList(id) {
    ge('searchRes' + id)
        .innerHTML = "";
    ge('itemList' + id)
        .innerHTML = "<a href='javascript: searchName(" + id + ", -1);'>" + getLang('admin2_list') + "</a>";
    ge('itemSearch' + id)
        .innerHTML = "<a href='javascript: searchName(" + id + ", 0);'>" + getLang('admin2_search') + "</a>";
    ge('itemSearchNum' + id)
        .innerHTML = "<a href='javascript: searchNameNum(" + id + ");'>" + getLang('admin_added_items_digit_search') + "</a>";
}
...
function searchSuccess(ajaxObj, responseText) {
    ...
    if (json && json.html) {
        ...
        ge('searchRes' + id)
            .innerHTML = getLang('admin2_server_error');
    }
}
...
function added(id, act, cb) {
    ...
    ajax.parent = ge('parentId' + id)
        .value;
    ajax.type = ge('typeId' + id)
        .value;
    ajax.name = ge('trueItemName' + id)
        .value;
    if (ajax.type == -1) {
        ajax.fullname = ge('itemFullNameValue' + id)
            .value;
    } else {
        ajax.fullname = '';
    }
...
function feedback_box(item_id, obj_id) {
    ...
    box.addButton({
        ...
        onClick: function(btn) {
            ...
            var ajax = new Ajax(),
                feedBackText = ge('feedback_' + item_id)
                .value;

            hide('admin_feedback_error');
            if (feedBackText.match(/\{[a-z]+\}/)) {
                ge('admin_feedback_error')
                    .innerHTML = getLang('admin_feedback_bad_text');
            ...
            query.hash = ge('feedback_' + item_id + '_hash')
                .value;
            ajax.post(url, query);
        }
    });
...
function loadFeedbackText(item_id, result, item_type) {
    ...
    if (typeof result !== 'undefined') {
        ...
        ge('feedback_' + item_id + '_item_type')
            .value = item_type;
        ge('feedback_' + item_id + '_result')
            .value = result;
    } else {
        ajaxParams.obj_id = ge('feedback_' + item_id + '_obj_id')
            .value;
        ajaxParams.item_type = ge('feedback_' + item_id + '_item_type')
            .value;
        ajaxParams.result = ge('feedback_' + item_id + '_result')
            .value;
    }

    ajax.onDone = function(ajaxRes, response) {
        response = eval('(' + response + ')') || {};
        ge('feedback_' + item_id)
...
function load_details(id, type) {
    if (parseInt(ge('details_' + id + '_' + type + '_visible')
            .innerHTML)) {
        ge('details_' + id + '_' + type + '_visible')
            .innerHTML = '0';
        ge('details_' + id + '_' + type)
            .style.display = 'none';
    } else {
        ge('details_' + id + '_' + type + '_visible')
            .innerHTML = '1';
        ...
        ge('details_' + id + '_' + type + '_text')
            .innerHTML = '<img style="margin: 30px 0px" src="/images/progress7.gif"/>';
        var ajax = new Ajax();
        ajax.onDone = function(obj, text) {
            ge('details_' + id + '_' + type)
...
function init() {
    ...
    if (ge('new_pages')) {
        text = ge('new_pages')
            .innerHTML;
        ge('pages_top')
            .innerHTML = '<ul class="pageList"><li><img style="vertical-align: -3px; display: none;" id="loading_pages_up" src="/images/upload.gif"/></li>' + text + '</ul>';
        if (text.length > 0)
            ge('pages_bottom')
            .innerHTML = '<ul class="pageList"><li><img style="vertical-align: -3px; display: none;" id="loading_pages_down" src="/images/upload.gif"/></li>' + text + '</ul>';
        else
            ge('pages_bottom')
            .innerHTML = '';
    }
...
var filter_set = function(index, value) {
    ...
    if ((is_set) && (!value)) {
        ...
        ge('filter_' + index)
            .className = 'filter_btn_off';
    } else if ((!is_set) && (value)) {
        filter_current -= power_of_two;
        ge('filter_' + index)
            .className = 'filter_btn_on';
    }
}
...
function added_items_page(page) {
    ...
    ajax.onDone = function(obj, text) {
        ...
        if (filters_row) {
            ge('pageAdmin')
                .insertBefore(filters_row, ge('pageAdmin')
                    .firstChild);
        }
        init();
    }
    ...
    if (ge('page')
        ...
        ajax.onDone = function(obj, text) {
            ...
            if (text.charAt(0) == 'e') {
                ...
                ge('informer_moders_table')
                    .innerHTML = text.substr(1);
            ...
            ge('addingModeratorMessageCont')
                .style.display = 'block';
            ge('addingModeratorMessageCont')
                .innerHTML = '<div id="addingModeratorMessage" class="' + class_name + '">' + message_text + '</div>';
            setTimeout(function() {
                (new effects.fader())
                .fade(ge('addingModeratorMessage'), [0xFFFFFF, 0xDDDDDD], 3000);
            }, 2500);
        }
        ajax.post('admin.php', {
...
function remove_informer_moder(id) {
    ...
    ajax.onDone = function(obj, text) {
        if (text.charAt(0) == 'e') {
            ...
            ge('moderator' + id + 'dellink')
                .innerHTML = '<a href="#" onclick="return remove_informer_moder(' + id + ')">�������</a>';
        } else if (text.charAt(0) == 'g') {
            ge('moderator' + id)
                .className += ' removed';
            var prev = ge('moderator' + id)
                .previousSibling;
            var has_prev = false;
            if (prev) {
                if (prev.tagName) {
                    if (prev.tagName.toString()
                        .toLowerCase() == 'div') {
                        has_prev = true;
                    }
                }
            ...
            if (has_prev) {
                ...
                ge('moderator' + id)
                    .style.borderTopColor = '#ded8b1';
            }
            ge('moderator' + id + 'del')
                .style.display = 'inline';
            ge('moderator' + id + 'dellink')
                .innerHTML = '<a href="#" onclick="return return_informer_moder(' + id + ')">������</a>';
        } else {
            ge('moderator' + id + 'dellink')
                .innerHTML = '<a href="#" onclick="return remove_informer_moder(' + id + ')">�������</a>';
        }
...
function return_informer_moder(id) {
    ...
    ajax.onDone = function(obj, text) {
        if (text.charAt(0) == 'e') {
            ...
            ge('moderator' + id)
                .className = ge('moderator' + id)
                .className.replace('removed', '');
            var prev = ge('moderator' + id)
                .previousSibling;
            var has_prev = false;
            if (prev) {
...
function appOpFailed(ajaxObj, responseText) {
    ge('appoptions' + ajaxObj.aid)
        .innerHTML = "<div id='msg'>" + getLang('admin2_server_error') + "</div>";
}
...
var Datepicker = function(el, options) {
    ...
    var updateDate = function(date, mode, init) {
        ...
        if (mode == "m") {
            ...
            dateInput.value = date_format.replace('{day}', date.d)
                .replace('{month}', winToUtf(months[date.m - 1]))
                .replace('{year}', date.y);
        }
        _t.hide();
        dateVal.value = Math.floor((new Date(date.y, date.m - 1, date.d, hour, min))
            .getTime() / 1000);
    ...
    var html = '<input type="hidden" name="' + id + '" id="' + id + '"/>' +
        ...
        '<div id="' + id + '_cal_box" style="display:none;position:absolute;"><iframe id="' + id +
        '_cal_frame" style="border:0;position:absolute;z-index:100;"></iframe><div id="' + id + '_cal_div" style="position:absolute;z-index:200"></div></div>';
...
var Timepicker = function(el, options) {
    ...
    var html = '<input type="hidden" name="' + id + '" id="' + id + '"/>' +
        options.format.replace('{hour}', '<div class="fl_l"><input type="hidden" id="' + id + '_hour_input" value="' + h + '"/></div>')
        .replace('{min}', '<div class="fl_l"><input type="hidden" id="' + id + '_min_input" value="' + m + '"/></div>') +
