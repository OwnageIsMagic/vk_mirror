var MediaPoster = function(postType, target, preview, msgBox, toId, types, isAdmin) {
    ...
    each(types, function(i, v) {
        ...
        rows += '<a onfocus="this.blur()" id="' + id + '_post_type' + i + '" style="background-image: url(' + ico + '); background-position: ' + bgpos + spec_style +
            '" href="' + url + '">' + v[1].replace(/\s/g, '&nbsp;') + '</a>';
    });

    var prefix = vk.al ? 'add_media' : 'add_wall';
    var html = '<table cellspacing="0" cellpadding="0"><tr><td class="' + prefix + '_side"><div>&nbsp;</div></td><td><div class="rows">' + rows + '</div><div class="' + prefix +
        '_bottom"></div><div class="' + prefix + '_bottom2"></div></td><td class="' + prefix + '_side"><div>&nbsp;</div></td></tr></table>';
    if (!ge(id + '_add_menu')) {
        var toAppend = document.body;
        toAppend.appendChild(ce('div', {
            id: id + '_menu',
            className: prefix + '_menu',
            innerHTML: '<div id="' + id + '_header" class="' + prefix + '_header"><div>' + target.innerHTML + '</div></div><div id="' + id + '_rows" class="' +
                prefix + '_rows">' + html + '</div>'
        }));
    } else {
        ge(id + '_rows')
            .innerHTML = html;
    ...
    function postAttach(media, media_id, toId, hash, options, target, preview) {
        ...
        if (own_reply) {
            params.own_reply = ge('own_reply_post_raw')
                .value;
        }
        if (ge('mainFeed')) {
        ...
        if (media == 'poll') {
            params.question = ge(postType + '_poll_question')
                .value;
            var answers = [],
                answer;
            for (var i = 1; i <= 10; i++) {
                if (answer = trim(ge(postType + '_poll_answer' + i)
                        .value)) {
                    answers.push(answer);
                }
    ...
    var showGraffitiBox = function(toId, title, post_hash) {
        ...
        boxes['graffiti'].removeButtons()
            .loadContent('graffiti.php', {
                act: 'a_draw_box',
                to_id: toId
            }, true)
            .show();
        ...
        window.graffitiUploaded = function() {
            ...
            var options = {
                ...
                onCaptchaHide: function(done) {
                    box.setOptions({
                        ...
                        onHide: function() {
                            if (!boxes['graffiti_warning']) {
                                ...
                                bw.content(window.lang_graffiti_cancel_confirm)
                                    .addButton({
                                        label: global_cancel,
                                        style: 'button_no',
                                        onClick: function() {
                    ...
                    box.content('<div id="uploadedGraffiti" style="width: 586px; height: 293px; background: #fff url(' + base_domain +
                            'images/progress7.gif) no-repeat 50% 50%; overflow: hidden"></div>')
                        .show();
                }
            };
            ...
            Ajax.Post({
                ...
                onDone: function(obj, text) {
                    if (text) {
                        ...
                        box.content(global_unknown_error)
                            .removeButtons()
                            .addButton({
                                label: global_cancel,
                                style: 'button_no',
    ...
    var showPhotoBox = function(toId, title, href, post_hash, isAdmin) {
        ...
        window.newPostedUploaded = function(phid, thumb_x, thumb_s) {
            ...
            if (ge('upload_new_file')) {
                ge('upload_new_file')
                    .innerHTML = '<input type="file" name="' + ge('upload_new_file')
                    .firstChild.name + '" onchange="uploadNew()" style="font-size: 11px" />';
            }
    ...
    var pollConstructor = null,
        pollHTML = '<div><div class="row"><div class="label">{question}</div><div class="labeled"><input id="' + postType +
        '_poll_question" type="text" style="margin-bottom: 8px;"/></div></div><div class="row"><div class="label">{answers} <nobr><a href="#" id="' + postType + '_poll_inc">' +
        getLang('global_add')
        .toLowerCase() + '</a><span class="divider" id="' + postType + '_poll_split"> | </span><a href="#" id="' + postType + '_poll_dec" class="disabled">' + getLang(
            'global_delete')
        .toLowerCase() + '</a></nobr></div><div class="labeled">{answers_inputs}</div><div class="clear"></div></div>',
        ansHTML = '',
        numAnswers = 2;
    for (var i = 1; i <= 10; i++) {
    ...
    var showPollBox = function(toId, labels, hash) {
        ...
        if (!pollConstructor) {
            ...
            ge(postType + '_attach')
                .parentNode.insertBefore(pollConstructor, ge(postType + '_attach'));
            ge(postType + '_attach')
                .parentNode.insertBefore(ce('div', {
        ...
        var html = pollHTML.replace('{question}', labels[1])
            .replace('{answers}', labels[2])
            .replace('{answers_inputs}', ansHTML);
        var tmpDiv = ce('div', {
            innerHTML: html
        }, {
            position: 'absolute',
        ...
        animate(tmpDiv1, {
            ...
            addEvent(ge(postType + '_poll_question'), 'keyup', function() {
                var q = this.value;
                ge(postType + '_poll_question_preview')
                    .innerHTML = q ? (': <span class="pollQ">' + (q.length > 40 ? q.substr(0, 40) + '...' : q) + '</span>') : '';
            });
            numAnswers = 2;
            addEvent(ge(postType + '_poll_inc'), 'click', function() {
                ...
                ge(postType + '_poll_inc')
                    .className = numAnswers >= 10 ? 'disabled' : '';
                ge(postType + '_poll_dec')
                    .className = numAnswers <= 2 ? 'disabled' : '';
                return false;
            });
    ...
    var setPreview = function(type, info, media_id, toId, hash, options) {
        ...
        switch (type) {
            ...
            case 'app':
                ...
                break;
            case 'share':
                obj = '<div class="attach_url fl_l"><span>' + getLang('wall_link_label') + ': <a href="' + base_domain + 'away.php?to=' + encodeURIComponent(info[1]) +
                    '" target="_blank">' + info[0] + '</a></span></div>';
        ...
        preview.innerHTML = '<div class="attach_box clearFix">' + obj +
            '<div class="attach_x fl_l"><div class="iconX" onmouseover="overX(this);" onmouseout="outX(this);" onclick="cancelMedia(\'' + preview.id + '\', \'' + id +
            '\')"></div></div></div>';
        ...
        while (wiki_value && (matches_url = wiki_value.match(rx))) {
            ...
            if (matches_url[4].match(/vkontakte.ru|vk.com|vkontakte.com|vk.cc/)) {
                var query = matches_url[5],
                    media_matches = null,
                    media_type = false;
                if ((media_matches = query.match(/#photo\/(\-?\d+)_(\d+)/)) || (media_matches = query.match(/photo(\-?\d+)_(\d+)/)) || (media_matches = query.match(
                        /photos.php\?oid=\-?\d+&act=show&id=(\-?\d+)_(\d+)/))) {
                    media_type = 'photo';
    ...
    function shareLoadPreview(url) {
        if (!sharePreview) {
            ...
            data(preview, 'sharePreview', sharePreview);
            ge(postType + '_attach')
                .parentNode.insertBefore(sharePreview, ge(postType + '_attach'));
            ge(postType + '_attach')
                .parentNode.insertBefore(ce('div', {
    ...
    function shareShowPreview() {
        if (share_data.parse_failed) {
            ...
            var html = (share_data.images && share_data.images[0] ? '<img src="' + share_data.images[0] + '" class="fl_l tt_thumb" ' + share_data.imagesStyles[0] + '/>' : '') +
                (share_data.title ? '<h4 class="tt_header">' + share_data.title + '</h4>' : '') + (share_data.description ? '<div class="descr">' + share_data.description +
...
function initMentions(el) {
    ...
    var _tmp = function() {
        ...
        if (!el.id.indexOf('status')) {
            onsubmit = function() {
                if (ge('group')) {
                    postGroupStatus(ge('mid')
                        .value, window.wall_phash);
                } else {
                    postStatus(ge('mid')
                        .value, window.wall_phash);
                }
