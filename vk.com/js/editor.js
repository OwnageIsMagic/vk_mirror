var EditBox = {
    ...
    init: function() {
        ...
        if (!browser.msie) {
            ...
            if (!ge("editFrame")
                .contentDocument) addEvent(frame, 'load', onload);
            else onload();
        } else {
    ...
    editorEvent: function(event, args) {
        if (event == (browser.msie ? "keydown" : "keypress")) {
            var key = String.fromCharCode((browser.msie ? args[0].keyCode : args[0].charCode))
                .toLowerCase();
            if (args[0].ctrlKey) {
                for (btn in this.buttons) {
    ...
    refreshToolbar: function() {
        for (btn in this.buttons) {
            ...
            var val = (this.buttons[btn].tag) ? this.findParent(this.buttons[btn].tag, this.getRng()
                .commonAncestorContainer) : this.doc.queryCommandState(btn);
...
var checkLoad = function() {
    ...
    if (tags && tags[0]) {
        var ta = tags[0];
        EditBox.txt = (ta.value || ta.textContent || "")
            .replace(/\r\n/g, "<br/>")
            .replace(/\n/g, "<br/>");
        ta.value = "";
        // image size - 150x8
