...
function genPlaceStr(address) {
    ...
    if (address.since && address.until && address.since != 1000000 && address.until != 1000000) {
        result.push(getLang('profileEdit_years_from_to')
            .replace('{start}', address.since)
            .replace('{end}', address.until));
    } else if (address.since && address.since != 1000000) {
        result.push(getLang('profileEdit_years_from')
            .replace('%s', address.since));
    } else if (address.until && address.until != 1000000) {
        result.push(getLang('profileEdit_years_to')
            .replace('%s', address.until));
    }
    if (result.length) {
        return '<div>' + result.join(', ') + '</div>';
    }
...
onDomReady(function() {
    ...
    for (var i in placeCategories) {
        ge('addresses')
            .innerHTML += '<div id="category' + i + '" class="placeCategory">' +
...
function showEditAddressBox(address, title, label, ignoreChanges) {
    ...
    if (address.id > 0) {
        ...
        ge('add_address_link')
            .parentNode.insertBefore(editBox.parentNode.removeChild(editBox), ge('add_address_link'));
        hide('add_address_link');
        editingItem = -1;
    }
