...
var Index = {
    ...
    submitJoin: function() {
        var btn = ge('ij_submit'),
            phone = trim(val('ijp_input')
                .replace(/[^\d]/g, ''));
        if (phone.length < 5) {
            return elfocus('ijp_input');
        }
        hide('ij_error');
        ajax.post('al_register.php', {
            act: 'join_phone',
            phone: phone,
            hash: cur.hash
        }, {
    ...
    submitJoinStart: function() {
        var fname = trim(val('ij_first_name')),
        ...
        ajax.post('join.php', {
            ...
            showProgress: lockButton.pbind('ij_submit'),
            hideProgress: unlockButton.pbind('ij_submit')
        });
    },
    ...
    fbJoin: function(context) {
        ...
        var oauthUrl = 'https://graph.facebook.com/v2.7/oauth/authorize?client_id=' + cur.fbApp + '&redirect_uri=' + encodeURIComponent(redirectUri) +
            '&scope=email,user_birthday&display=popup&state=' + cur.fbState;
...
var JoinPhotoview = {
    ...
    updateHeight: function() {
        var h = cur.jpvBox.offsetHeight + 110,
    ...
    show: function(index, ev) {
        ...
        cur.jpvShown = true;
        if (!cur.jpvFixed) {
            cur.jpvFixed = bodyNode.appendChild(ce('div', {
                className: 'jpv_fixed fixed jpv_dark',
                innerHTML: '\
...
<div id="jpv_box" onclick="cur.jpvClicked = true;">\
  <a class="fl_r jpv_close_link" onclick="JoinPhotoview.hide()">' +
...
<div class="no_select" id="jpv_left_nav" ' +
                'onmouseover="JoinPhotoview.activate(cur.jpvLeft)" onmouseout="JoinPhotoview.deactivate(cur.jpvLeft)" onmousedown="JoinPhotoview.show(cur.jpvIndex - 1 + vk.rtl * 2, event); cur.jpvClicked = true;" onselectstart="return cancelEvent(event);"></div>\
<div class="no_select" id="jpv_right_nav" ' +
                'onmouseover="JoinPhotoview.activate(cur.jpvClose)" onmouseout="JoinPhotoview.deactivate(cur.jpvClose)" onmousedown="JoinPhotoview.onClick(event); cur.jpvClicked = true;"></div>\
      ';
        ...
        cur.jpvSummary.innerHTML = ((count > 1) ? getLang('index_screen_m_of_n')
            .replace('{num}', cur.jpvIndex + 1)
            .replace('{count}', count) : getLang('photos_view_one_photo'));
        cur.jpvCurPhoto = ph;
