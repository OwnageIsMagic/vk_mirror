    ...
    flistBox: function(box, friendsData, selectedFriends, rowTemplate, saveLang, hash) {
        ...
        if (cur.flistTooltip) {
            if (cur.flistTooltipRight) {
                var hintLeft = '<td class="priv_h_arrow_td" style="vertical-align: top;"><div class="priv_h_arrow" style="margin-top: 50px;"><img src="/images/ddtooltip' +
                    (window.devicePixelRatio >= 2 ? '_2x' : '') + '.png" width="16" height="11" class="priv_h_arrow_img png" /></div></td>';
                var hintRight = '';
            } else {
                var hintRight =
                    '<td class="priv_h_arrow_td" style="vertical-align: top;"><div class="priv_h_arrow_l" style="margin-top: 50px;"><img src="/images/ddtooltip' + (window.devicePixelRatio >=
                        2 ? '_2x' : '') + '.png" width="16" height="11" class="priv_h_arrow_img png" /></div></td>';
                var hintLeft = '';
            }
  ...
  <tr class="priv_h_bottom_sh">\
    ...
    <td colspan="5"><div class="priv_h_bottom2"></div></td>\
        ...
        if (cur.flistCountStr && cur.flistSelectedCnt > 0) {
            ge('flist_sel_summary')
                .innerHTML = langNumeric(cur.flistSelectedCnt, cur.flistCountStr);
        }
    ...
    flistShowOne: function(u, ins) {
        ...
        var row = ce('div', {
            id: 'flist_cell' + u[0],
            innerHTML: cur.flistTpl.replace('%id%', u[0])
                .replace('%name%', uname)
                .replace('%photo%', u[2])
        });
        if (ins) {
    ...
    flistSearch: function(obj, event) {
        ...
        setTimeout(function() {
            ...
            if (str) {
                ...
                cur.flistSelection = {
                    re: new RegExp('(' + str.replace(cur.flistIndex.delimiter, '|')
                        .replace(/[\/\\\(\)\[\]\{\}\*,]/g, '')
                        .replace(/^\||\|$/g, '') + ')', 'gi'),
                    val: '<em>$1</em>'
                };
            ...
            if (el.nextSibling) {
                if (v == Privacy.listsType) {
                    ...
                    for (var i in p[2]) {
                        var cat_id = -p[2][i],
                            color = (cat_id - 1) % 8 + 1;
                        if (_lists[cat_id]) str.push(cat_id < 100 ? '<a href="/friends?section=list' + cat_id + '" class="group' + color + '">' + _lists[cat_id] + '</a>' :
                            '<span class="group' + color + '">' + _lists[cat_id] + '</span>');
                    }
    ...
    choose: function(ev, val, list) {
        ...
        if (val == Privacy.customType) {
            ...
            return showTabbedBox('al_friends.php', {
                act: 'select_friends_box',
                Checked: checked
            }, {
                stat: ['ui_controls.js']
        ...
        if (cur.privSelIndex !== false) {
            ...
            if (cur.privSelIndex == Privacy.listsType && cur.privacy[cur.privSel][0] != Privacy.listsType) {
                if (Privacy.toup) {
                    ...
                    setStyle(cur.privEl, {
                        top: cur.privFromY - getSize(cur.privEl)[1]
                    });
                } else {
                    hide(el.nextSibling);
                }
    ...
    show: function(el, ev, key, delta) {
        ...
        if (!cur.privEl) {
            cur.privEl = bodyNode.appendChild(ce('div', {
                id: 'privacy_dropdown',
                innerHTML: '\
      ...
      <div class="bottom"></div><div class="bottom2"></div>\
        ...
        if (types[Privacy.listsType] && hasLists) {
            ...
            for (var i in lists) {
                ...
                html.push('<div class="l_item' + sel + '" id="privacy_l_item' + _i + '" onclick="Privacy.choose(event, ' + Privacy.listsType + ', ' + _i +
                    ')"><div class="privacy_item_icon"></div>' + lists[i] + '</div>');
            }
            html.push('</div>');
        }
        html.push('</div>');
        ...
        if (helpText) {
            ...
            if (!cur.privHelper) {
                cur.privHelper = bodyNode.appendChild(ce('div', {
                    id: 'privacy_helper',
                    innerHTML: '\
<table cellspacing="0" cellpadding="0">\
  <tr class="priv_h_top_sh">\
  ...
  <tr>\
    <td class="priv_h_arrow_td"><div class="priv_h_arrow"><img src="/images/ddtooltip' +
                        (window.devicePixelRatio >= 2 ? '_2x' : '') +
                        '.png" width="16" height="11" class="priv_h_arrow_img png" /></div></td>\
    <td class="priv_h_side_td2"><div class="priv_h_side"></div></td>\
    <td class="priv_h_side_td"><div class="priv_h_side"></div></td>\
    <td><div class="priv_h_rows"' +
                        (hw ? ' style="width: ' + hw + 'px"' : '') +
                        '></div></td>\
    <td class="priv_h_side_td"><div class="priv_h_side"></div></td>\
    <td class="priv_h_side_td2"><div class="priv_h_side"></div></td>\
  </tr>\
  ...
  <tr class="priv_h_bottom_sh">\
    ...
    <td colspan="5"><div class="priv_h_bottom2"></div></td>\
                ...
                addEvent(cur.privHelper, 'mouseover', function() {
                    clearTimeout(cur.hidePrivacyTimer);
                });
            }
        ...
        var sz = getSize(cur.privEl),
            h = Math.max(sz[1], hsz[1])
        ...
        if (helpText) {
            var hx = (xy[0] - dx) + (vk.rtl ? (-hsz[0]) : sz[0]),
                hy = 0;
            if (Privacy.toup) {
    ...
    each(options.actions_menu, function() {
        ...
        if (!this[0]) {
            menuItem.onMouseOver = function() {
                ...
                setStyle(self.olistSelectMenu.header, {
                    width: getSize(this)[0]
                });
                setStyle(self.olistSelectMenu.rows, {
                    minWidth: getSize(this)[0]
                });
                if (browser.msie) {
                    setStyle(self.olistSelectMenu.rows, {
                        width: getSize(this)[0] + 20
                    });
                }
...
extend(OList.prototype, {
    ...
    ignoreOnClick: function(target, id) {
        ...
        ajax.post('/al_feed.php?misc', {
            act: checked ? 'a_ignore_owner' : 'a_unignore_owner',
            owner_id: id,
            hash: hash
        }, {
            onDone: function() {
    ...
    renderList: function(pattern, offset, sel) {
        ...
        if (!offset && !html.length) {
            html.push('<div class="olist_empty">' + (pattern ? getLang('global_search_not_found')
                .replace('{search}', pattern) : self.noSelMsg) + '</div>');
        }
        re(this.moreEl);
        html = html.join(' ');
