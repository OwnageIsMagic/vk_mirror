var AppsCheck = {
    ...
    changeSummary: function() {
        ...
        if (cur.section == 'blocked' || cur.section == 'requests' || cur.section == 'comments' || cur.section == 'reports') {
            ...
            if (cur.section == 'reports') {
                ...
                res += '<span class="divider">|</span><span class="app_check_actions"><a href="#" onclick="AppsCheck.switchReports(); return false;">' + label +
                    '</a></span>'
            }
            sum.innerHTML = res;
            if (ge('tab_counter_' + cur.section)) {
                ge('tab_counter_' + cur.section)
                    .innerHTML = (cur.tabCount) ? '+' + cur.tabCount : '';
                toggleClass(ge('tab_' + cur.section), 'count', !!cur.tabCount);
            }
        }
    },
    ...
    ddShow: function(aid, obj, event) {
        ...
        var html =
            '<div id="apps_lists_menu" class="apps_lists_menu" onmouseout="AppsCheck.ddHide(this);" onmouseover="AppsCheck.ddActive(this);">\
...
<table cellspacing="0" cellpadding="0"><tbody><tr><td class="apps_lists_shad_l"><div></div></td><td><div class="apps_lists_shad_t2"></div><div class="apps_lists_shad_t"></div><div class="apps_lists_rows"><div id="rows3">';
        ...
        for (i in cur.rules) {
            html += '<a onclick="AppsCheck.checkRule(this, ' + aid + ', ' + (i - 1) + ');" class="' + ((cur.selectedRules & (1 << parseInt(i - 1))) ? 'checked' : '') +
                '">' + cur.rules[i] + '</a>';
        }
        html +=
            '</div></div><div class="apps_lists_shad_b"></div><div class="apps_lists_shad_b2"></div></td><td class="apps_lists_shad_r"><div> </div></td></tr></tbody></table></div></div>';
            ...
            onDone: function(title, text) {
                ...
                slideUp(ge('app' + id), 200, function() {
                    ...
                    if (!cur.totalCount) {
                        ...
                        ge('apps_check_content')
                            .innerHTML = '<div class="app_msg" id="no_apps">' + msg + '</div>';
                    }
                });
            },
            onFail: function() {
    ...
    showEditReply: function(post) {
        ...
        ge('reply_button' + post)
            .onclick = this.sendReply.pbind(post);
        cur.editing = post;
        elfocus(rf);
    },
            ...
            onDone: function(text) {
                if (!text.length) {
                    ...
                    showFastBox({
                        ...
                        }, '<iframe src="app' + app_id + '?check=1" style="width: 100%; height: ' + h +
    ...
    removeAutoanswer: function(id) {
        ...
        ajax.post('apps_check', {
            ...
            onDone: function(text) {
                ...
                cur.deletedAutoanswers[id] = ge('autoanswer_row' + id)
                    .innerHTML;
                if (text) {
                    ge('autoanswer_row' + id)
                        .innerHTML = text;
                }
            },
            onFail: function() {
        ...
        ajax.post('apps_check', {
            ...
            onDone: function() {
                ...
                if (cur.deletedAutoanswers && cur.deletedAutoanswers[id]) {
                    ge('autoanswer_row' + id)
                        .innerHTML = cur.deletedAutoanswers[id];
    ...
    editAutoanswer: function(id) {
        ...
        var new_label = ge('answer_content' + id)
            .value;
        ajax.post('apps_check', {
            act: 'edit_autoanswer',
            from: cur.section,
    ...
    addAutoanswer: function(id) {
        ...
        var name = ge('add_answer_label')
            .value;
        var cont = ge('add_answer_text')
            .value;
        if (!name || !cont) {
    ...
    cancelAutoanswer: function(id) {
        slideUp('edit_autoanswer' + id, 200, function() {
            if (cur.autoanswers[id]) {
                ge('answer_content' + id)
                    .value = cur.autoanswers[id];
            } else {
                ge('answer_content' + id)
                    .value = '';
            }
    ...
    toggleCollections: function(el, enabled) {
        ...
        var cnt = 0,
            rows = geByClass('apps_collection_row', list);
        for (var i in rows) {
            setStyle(rows[i], {
                zIndex: null,
                left: null,
    ...
    removeCollectionApp: function(collection_id, aid, hash) {
        ajax.post('apps_check', {
            ...
            onDone: function(html) {
                ...
                list.innerHTML = html;
                if (geByClass('apps_cat_row', ge('app_search_list'))
                    .length) {
                    cur.sorter = qsorter.init('app_search_list', {
                        onReorder: cur.reorderApps,
                        xsize: 4,
                        width: 148,
                        height: 209 + (browser.msie8 ? 2 : 0)
                    });
                }
            },
            onFail: function(err) {
