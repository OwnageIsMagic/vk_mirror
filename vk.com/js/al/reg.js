function initRegSteps(step) {
    ...
    extend(cur, {
        ...
        move: function(to_step) {
            ...
            show(to_node);
            cur.m_to_height = geByClass1('borders', to_node)
                .offsetHeight;
    ...
    cur.genDays = function(year, month) {
        ...
        uiSex = new Dropdown(ge('sex'), cur.selData.sexes, {
            ...
            onChange: function(val) {
                ...
                if (val) {
                    ge('birth_date_label')
                        .innerHTML = langSex(val, cur.lang.birth_date_label);
                }
            }
        });
    ...
    ge('step1_save')
        .onclick = function() {
            var fnameRaw = trim(ge('fname')
                    .value),
                lnameRaw = trim(ge('lname')
                    .value);
            var name = cleanName(fnameRaw, lnameRaw);
            fname = name[0];
            lname = name[1];

            var sex = intval(ge('sex')
                .value);
    ...
    ge('step2_save')
        .onclick = function() {
            ...
            params = addFields(params, 's_country', 's_city', 'school', 'school_type', 's_class', 's_start', 's_fin', 's_grad', 'u_country', 'u_city', 'university', 'faculty',
                'chair', 'edu_form', 'edu_status', 'grad');
