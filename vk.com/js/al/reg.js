function initRegSteps(e) {
    extend(cur, {
        ...
        move: function(e) {
            ...
            cur.moving || (cur.m_height = cur.stepsWrap.offsetHeight - 2, cur.m_g = 115 * (cur.step - 1), cur.m_c = 632 * (1 - cur.step), cur.m_cur = cur.step, cur.moving =
                setInterval(function() {
                    if (cur.m_time += 13, cur.anim_v) {
                        if (cur.m_time > cur.v_time) {
                            if (cur.m_height = cur.m_to_height, cur.anim_v = !1, cur.anim_h = cur.m_to_c != cur.m_c)
                                for (var e = 1; 3 >= e; ++e) show("reg_step" + e + "_c"), browser.opera_mobile ? ge("reg_step" + e + "_c")
                                    .style.visibility = "visible" : ge("reg_step" + e + "_c")
                                    .style.height = "auto";
                    ...
                    if (cur.anim_h && (cur.m_time > cur.h_time ? (cur.m_g = cur.m_to_g, cur.m_c = cur.m_to_c, cur.anim_h = !1, cur.anim_v = cur.m_to_height !=
                                cur.m_height, cur.m_time -= cur.h_time) : (cur.m_g = cur.anim(cur.m_time, cur.m_from_g, cur.m_to_g - cur.m_from_g, cur.h_time),
                                cur.m_c = cur.anim(cur.m_time, cur.m_from_c, cur.m_to_c - cur.m_from_c, cur.h_time)), cur.stepsContent.style.marginLeft = cur.m_c +
                            "px", cur.stepsGlass.style.marginLeft = cur.m_g + "px", cur.stepsGlassContent.style.marginLeft = -cur.m_g - 2 + "px", cur.m_cur =
                            Math.floor(cur.m_g / 115) + 1, cur.m_to <= cur.m_cur && cur.m_cur < cur.m_next && (removeClass(ge("reg_step" + cur.m_next),
                                "passed"), --cur.m_next), cur.m_to > cur.m_cur && cur.m_cur == cur.m_next && (addClass(ge("reg_step" + cur.m_next), "passed"), ++
                                cur.m_next), cur.stepsPoint.style.marginLeft = cur.m_g + 47 + "px"), !cur.anim_v && !cur.anim_h) {
                        cur.step = cur.m_to, nav.setLoc("regstep" + cur.step), clearInterval(cur.moving), cur.moving = !1;
                        for (var e = 1; 3 >= e; ++e) e < cur.step ? browser.opera_mobile ? ge("reg_step" + e + "_c")
                            .style.visibility = "hidden" : ge("reg_step" + e + "_c")
                            .style.height = "10px" : e > cur.step && hide("reg_step" + e + "_c");
                        cur.stepsWrap.style.height = "auto", cur.stepsContent.style.width = 632 * cur.step + "px", cur.stepsWrap.style.overflow = "visible", 3 ==
            ...
            if (t.style.position = "absolute", t.style.left = "-5000px", show(t), cur.m_to_height = geByClass1("borders", t)
                .offsetHeight, cur.m_from_g = cur.m_g, cur.m_to_g = 115 * (cur.m_to - 1), cur.m_from_c = cur.m_c, cur.m_to_c = 632 * (1 - cur.m_to), cur.anim_v = cur.m_to_height >
                cur.m_height, cur.stepsWrap.style.height = cur.m_height + "px", cur.stepsWrap.style.overflow = "hidden", cur.stepsContent.style.width = "5000px", t.style
                .position = "static", t.style.left = "auto", cur.anim_h = cur.m_to_height <= cur.m_height)
                for (var r = 1; 3 >= r; ++r) show("reg_step" + r + "_c"), browser.opera_mobile ? ge("reg_step" + r + "_c")
                    .style.visibility = "visible" : ge("reg_step" + r + "_c")
                    .style.height = "auto";
            cur.h_time = 2 * Math.abs(cur.m_to_c - cur.m_c) / 5, cur.v_time = Math.abs(cur.m_to_height - cur.m_height) / 2, cur.h_time > 500 && (cur.h_time = 500), cur
                .v_time > 500 && (cur.v_time = 500)
        },
        finish: function(e) {
        ...
        uploadSucceed: function(e, t, r, s) {
            ...
            c.src = s, cur.updatePhoto = function() {
                if (c.height) {
                    ge("photo_file")
                        .innerHTML = '<input id="photo" class="inputFile" type="file" onchange="cur.uploadPhoto()" name="photo" />', hide("upload_progress");
                    var e = ge("reg_photo");
        ...
        uploadError: function(e) {
            ge("photo_error")
                .innerHTML = e, show("photo_error"), hide("upload_progress"), ge("photo_file")
                .innerHTML = '<input id="photo" class="inputFile" type="file" onchange="cur.uploadPhoto()" name="photo" />'
        }
    });
    for (var t = 1; 3 >= t; ++t) t < cur.step ? browser.opera_mobile ? ge("reg_step" + t + "_c")
        .style.visibility = "hidden" : ge("reg_step" + t + "_c")
        .style.height = "10px" : t > cur.step && hide("reg_step" + t + "_c");
    cur.stepsContent.style.width = 632 * cur.step + "px", cur.stepsWrap.style.overflow = "visible", cur.m_height = cur.stepsWrap.offsetHeight - 2, cur.m_g = 115 * (cur.step - 1),
        cur.m_c = 632 * (1 - cur.step), cur.stepsPoint.style.marginLeft = cur.m_g + 47 + "px", show(cur.stepsPoint), cur.stepsGlass.style.marginLeft = cur.m_g + "px", cur.stepsGlassContent
        .style.marginLeft = -cur.m_g - 2 + "px", show(cur.stepsGlass), cur.stepsContent.style.marginLeft = cur.m_c + "px", show(cur.stepsContent), selectsData.setCountries(cur.selData
            .countries_list);
    ...
    cur.getLastDay = function(e, t) {
        ...
        onChange: function(e) {
            e = intval(e), e && (ge("birth_date_label")
                .innerHTML = langSex(e, cur.lang.birth_date_label))
        }
    });
    var S = function(e, t) {
    ...
    s = new Dropdown(ge("bday"), cur.genDays(cur.selData.byear, cur.selData.bmonth), {
        ...
        .onclick = function() {
            ...
            r = e(r, "s_country", "s_city", "school", "school_type", "s_class", "s_start", "s_fin", "s_grad", "u_country", "u_city", "university", "faculty", "chair", "edu_form",
                "edu_status", "grad"), r = t(r, "school_custom", "university_custom", "faculty_custom", "chair_custom"), ajax.post("register.php", r, {
        ...
        .onclick = function() {
            ...
            each([u, o, i, n, l, _, g, h, p, m, y, d, f, v, w, D, b], Selector.prototype.destroy), cleanElems("step1_save", "step2_save", "step3_save", "reg_step1_c",
                "reg_step2_c", "reg_step3_c", "uni_progress", "school_progress", "city_progress")
