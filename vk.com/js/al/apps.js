...
var vkApp = function(cont, options, params, onInit) {
    ...
    if (params.hash.indexOf('#') != -1) {
        var cut = params.hash.split('#')
            .pop();
        if ((cut || '')
            .substr(0, 1) == vk.navPrefix) {
            params.hash = '';
        } else {
            params.hash = cut;
        }
    ...
    this.funcs = {
        ...
        showLeadsPaymentBox: function(lead_id) {
            showBox('al_apps.php', {
                act: 'show_payment_box',
                aid: options.aid,
                offers: isArray(lead_id) ? lead_id.join(",") : (intval(lead_id) || 1)
            }, {
                params: {
                    dark: 1
                }
    ...
    switch (self.options.type) {
        ...
        case 2: // Flash Wrapper App
            ...
            res = renderFlash(self.cont, opts, {
                allowFullScreen: true,
...
var Apps = { // can be removed soon
    ...
    init: function(obj, appTpl) {
        ...
        if (ddAppsGenres) {
            ...
            cur.genrestt = new ElementTooltip(gpeByClass('apps_type_link_wrap', ddAppsGenres), {
                ...
                onShow: function(content) {
                    ...
                    each(geByClass(SELECTED_CLS), function() {
                        removeClass(this, SELECTED_CLS)
                    });
        ...
        ajax.post(Apps.address, query, {
            cache: 1,
            onDone: (function(data, opts, preload) {
                    if (opts) {
                        opts = eval('(' + opts + ')');
                    ...
                    cur.curList = 'all';
                    cur.appsList = obj[cur.curList] ? obj : {
                        all: []
                    };
                    if ((cur.section == 'catalog' || cur.section == 'list') && !cur.searchStr) {
                        cur.sectionCount = 0;
                    } else {
                        cur.sectionCount = (cur.appsList[cur.curList] || [])
                            .length;
                    }
    ...
    parseEvent: function(ev) {
        ...
        var c = ce('div', {
            className: 'apps_feed_animate_cont'
        });
        c.appendChild(el);
    ...
    scrollCheck: function() {
        var searchBar = ge('apps_search_wrap'),
            searchWrap = ge('apps_search_rel');
        if (searchBar && searchWrap) {
            var pos = getXY(searchWrap)[1],
                h = getSize(searchBar)[1],
    ...
    searchApps: function(str, type) {
        ...
        cur.sectionCount = (cur.appsList && cur.appsList[cur.curList]) ? Apps.filter(cur.appsList[cur.curList])
            .length : 0;
        this.filterTimeout = setTimeout((function() {
                hide(cur.sShowMore);
                cur.searchOffset = 0;
    ...
    showRows: function(force) {
        ...
        if (!list || !list.length) {
            ...
            if (!cur.searchStr && cur.appsList[defaultList].length <= cur.deletedCount) {
                if (cur.id == vk.id) {
                    msg = getLang('apps_youhavenoapps') + "<br /><a onclick=\"Apps.showSummaryProgress(); return nav.go(this, event);\" href=\"apps?act=catalog\">" +
                        getLang('apps_viewallapps') + " &raquo;</a>";
                } else if (cur.gid) {
                    msg = getLang('apps_noappsingroup2');
                    if (cur.isGroupAdmin) msg += "<br /><a onclick=\"Apps.showSummaryProgress() return nav.go(this, event);\" href=\"apps?act=catalog&gid=" + cur.gid +
                        "\">" + getLang('apps_viewallapps') + " &raquo;</a>";
                } else {
                    msg = getLang('apps_no_apps_found')
                        .split('{query}')
        ...
        ajax.post(Apps.address, query, {
            onDone: function(res, preload, options) {
                ...
                if (cur.section == 'catalog' && (cur.searchSort || nav.objLoc.sort || cur.searchType || nav.objLoc.type || val || nav.objLoc.q)) {
                    nav.setLoc(extend(nav.objLoc, {
                        sort: cur.searchSort == 0 ? null : cur.searchSort,
                        type: cur.searchType == 0 ? null : cur.searchType,
                        q: val ? val : null
                    }));
                }
            },
    ...
    removeApp: function(aid, hash, force) {
                ...
                onDone: function(text, data, padCounter, tabCounter) {
                    ...
                    if (cur.section == 'catalog' && force == 2) {
                        ...
                        if (geByClass1('app_deleted_layer', el)) {
                            ...
                            setStyle(textEl, {
                                marginTop: 64 - getSize(textEl)[1] / 2
                            })
                        } else {
        ...
        if (cur.section == 'catalog' && force == 2) {
            var box = showFastBox({
                ...
                dark: 1,
                bodyStyle: 'padding: 20px; line-height: 160%;'
            }, getLang('apps_quit_confirm'), getLang('apps_remove'), function() {
    ...
    selectTab: function(tab, label) {
        ...
        if (!el && label) {
            ge('apps_tabs')
                .appendChild(se('<li id="tab_' + tab +
                    '" class="active_link"><a class="apps_inactive_tab"><b class="tl1"><b></b></b><b class="tl2"></b><b class="tab_word">' + label + '</b></a></li>'));
            el = ge('tab_' + tab);
        }
        if (el) {
    ...
    switchTab: function(tab, event, search) {
        ...
        if (cur.preload && cur.preload[tab]) {
            var title = cur.preload[tab].title,
                html = cur.preload[tab].html,
                js = cur.preload[tab].js,
                _back = cur._back,
                wNode = ge('wrap3'),
                tNode = ge('title');
            if (_back) {
    ...
    showTypeSwitched: function(type) {
        ...
        var label = (type < 2 ? getLang('apps_cat_other') : cur.appsGenresData[type] || getLang('apps_cat_other'))
            .replace(/\s+/g, '&nbsp;') + '<span class="apps_genre_arrow"></span>';
        ge('apps_type_genres')
            .innerHTML = label;
    ...
    showSettingsRows: function(type) {
        ...
        toggle(ge('app_actions'), !cur.searchStr);
        toggleClass(ge('apps_summary')
            .parentNode, 'no_results', !list || !list.length);
        if (cur.section == 'manage') {
                ...
                if (!apps.length && (cur.shownCounters[i] < cur.totalCounters[i] || !cur.totalCounters[i])) {
                    ...
                    if (summaries[i]) {
                        switch (i) {
                            ...
                            case 0:
                            default:
                                summaries[i].innerHTML = (cur.section == 'manage') ? cur.totalCounters[i] : getLang((cur.searchStr ? 'apps_X_apps_found' : 'apps_X_apps'),
                                    cur.totalCounters[i], true);
                                break;
                        }
                    }
                }
    ...
    rejectRequest: function(lnk, rid, hash) {
        var block = ge("apps_request_row_" + rid),
            y = getSize(geByClass1('apps_request_info_wrap', block))[1];
        cur['req_' + rid + '_back'] = block.innerHTML;
        ajax.post('al_apps.php', {
            act: 'a_reject_request',
            rid: rid,
            hash: hash
        }, {
            onDone: function(text, isNew) {
                ...
                var msgBlock = geByClass1('apps_request_removed', block),
                    newY = getSize(msgBlock)[1];
    ...
    requestsBanUser: function(obj, mid, hash) {
        var msgBlock = obj.parentNode.parentNode,
            y = getSize(msgBlock)[1];
        ajax.post('al_apps.php', {
            act: 'a_request_ban_user',
            mid: mid,
            hash: hash
        }, {
    ...
    recountAddVotes: function(obj) {
        ...
        ge('add_votes')
            .innerHTML = langNumeric(add_val, votes_flex);
    ...
    recountWithdrawVotes: function(obj) {
        ...
        ge('withdraw_votes')
            .innerHTML = langNumeric(withdraw_val, votes_flex);
        if (withdraw_val > 0) {
            ge('app_pay_add')
                .value = 0;
        ...
        if (isPush) {
            ...
            ajax.post('/al_apps.php', {
                ...
                hideProgress: function() {
                    ...
                    geByClass1('apps_i_btn_bg', runButton)
                        .innerHTML = sentMessage;
    ...
    showWinInstructions: function(event, downloadLink, helpLink) {
        setTimeout(function() {
            var html = rs(trim(cur.winInstrTpl), {
    ...
    onFeaturedClick: function(rowEl) {
        ...
        // scroll
        if (geByClass('apps_featured_row')
            .length > 5) {
            ...
            setStyle(geByClass1('apps_featured_shadow_top'), {
                opacity: newTop == 0 ? 0 : 1
            });
            setStyle(geByClass1('apps_featured_shadow_bottom'), {
                opacity: newTop == floor ? 0 : 1
            });
        }
            ...
            for (var i = 0; i < imgs.length; i++) {
                ...
                img.onload = function() {
                    ...
                    if (loaded >= imgs.length) {
                        cur.appSsScrollbar = new Scrollbar('apps_i_th_contr', {
                            prefix: 'app_',
                            horizontal: true
                        });
                    }
    ...
    showMoreRecent: function(obj) {
        ...
        each(geByClass('apps_recent_row_hidden', ge('apps_recent_list')), function(i, el) {
            re(el);
        });
    ...
    initCat: function() {
        ...
        cur.appsImagesLoader = imagesLoader(ge('apps_wrap'), {
            skip_process_load: true,
            use_iframe: true
        });
    ...
    updateScrollWrapWidth: function(rowsWrap) {
        var cCount = geByClass('apps_cat_row', rowsWrap)
            .length;
        if (rowsWrap.id == 'apps_new_rows_wrap' && ge('apps_new_hidden')) {
            cCount -= geByClass('apps_cat_row', 'apps_new_hidden')
                .length;
        }
    ...
    slideAppsCatalog: function(type, direction, extra) {
        ...
        if (rowsLO == 0) {
            if (geByClass('apps_cat_row', rowsScroll)
                .length > 5) {
    ...
    showMoreNew: function(obj) {
        ...
        if (!cur.shownNew) {
            ...
            if (ge('apps_new_hidden')) {
                rowsWrap.appendChild(cf(ge('apps_new_hidden')
                    .innerHTML));
    ...
    blacklistInit: function(box, owners, options) {
        ...
        function renderList(pattern, offset) {
            ...
            if (!offset && !html.length) {
                html.push('<div class="olist_empty">' + (pattern ? getLang('global_search_not_found')
                    .replace('{search}', clean(pattern)) : options.lang['apps_blacklist_empty']) + '</div>');
            }
            re(moreEl);
            html = html.join(' ');
    ...
    rateOver: function(el) {
        ...
        for (var k = 1; k <= 5; k++) {
            ...
            if (row) {
                ...
                geByClass1('app_rate_percent', row)
                    .innerHTML = percent + '%';
                geByClass1('app_rate_cnt', row)
                    .innerHTML = rateCntText;
                geByClass1('app_rate_stars', row)
                    .className = 'app_rate_stars fl_l ' + classname;
            }
        }
        label += lines.reverse()
            .join('');
    ...
    notifications: (function() {
        ...
        function setOrDecrementCounter(el, num, plus) {
            ...
            animateCount(el, num > 0 ? (plus ? '+' : '') + num : '', {
                str: true
            });
            return num > 0;
        }
        ...
        row.prototype = {
            ...
            remove: function(hash) {
                ...
                ajax.post('al_apps.php', {
                    ...
                    onDone: function(html) {
                        this.nodes.remove = domFC(ce('div', {
                            'innerHTML': html
                        }));
                        this.show('remove');
                        if (!this.old && !readed[this.frame.app][this.id]) {
        ...
        function frame(app, hash, el, count) {
            ...
            this.container = ce('div', {
                'className': optClassNames.container
            });
            this.content = ce('div', {
                'className': optClassNames.content
            });
            this.loader = ce('div', {
                'className': optClassNames.loaderMax
            });
        ...
        frame.prototype = {
            ...
            counter: function(num) {
                ...
                animateCount(this.counterWrap, num, {
                    str: ''
                });
