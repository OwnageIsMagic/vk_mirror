...
<div style="margin: 8px 0 8px 0;">\
  ...
  <div id="edit_progress' + cid +
    ...
    saveTemplate: function(tid) {
        ...
        var title = trim(ge('add_template_title')
                .value),
            text = trim(ge('add_template_text')
                .value);
        if (!title) {
    ...
    deleteTemplate: function() {
        ...
        var box = showFastBox({
            title: cur.lang['delete_template_title'],
            width: 430
        }, cur.lang['delete_template_confirm'], cur.lang['delete'], function() {
            var tid = cur.selectedTemplate;
    ...
    selectTemplate: function(tid) {
        ...
        var front = (txtarea.value)
            .substring(0, strPos - text.length);
        var back = (txtarea.value)
            .substring(strPos - text.length, txtarea.value.length);
        txtarea.value = front + text + back;
        txtarea.scrollTop = scrollPos;

        if (!txtarea.autosize) autosizeSetup(txtarea, {
            minHeight: 42,
            maxHeight: 100
        });
        ...
        ge('template_title')
            .innerHTML = '<a href="#" onclick="return Tickets.deselectTemplate(' + tid + ')">' + template.title + '</a>';
        setStyle('edit_template', {
            display: (vk.id == intval(template.author_id) || cur.canEditTemplates) ? 'inline-block' : 'none'
        });
    ...
    deselectTemplate: function(tid) {
        if (cur.templates[tid] && ge('tickets_reply')
            .value == replaceEntities(cur.templates[tid]['text'].replace(/<br>/g, "\n"))) {
            ge('tickets_reply')
                .setValue('');
            if (!ge('tickets_reply')
                .autosize) autosizeSetup('tickets_reply', {
                minHeight: 42,
                maxHeight: 100
            });
            ge('tickets_reply')
        ...
        ge('template_title')
            .innerHTML = cur.lang.template_title;
        hide('edit_template');
    ...
    doPass: function(section, text, box) {
        if (!box) {
            var info = ge('tickets_info_title'),
                pr = se('<div class="fl_l"><img src="/images/upload.gif"/></div>');
        }
    ...
    passTo: function(el, id, no_autoanswer) {
        ...
        var text = '<div class="msg" style="margin-bottom: 15px;">' + msg + '</div><div style="line-height: 160%">' + getLang('support_sure_pass')
            .replace('{section}', val(el)) + '<br>';
        ...
        text += '</div>';
        text += '\
<div class="tickets_add_comm">' + getLang('support_comment') +
            '</div>\
<textarea id="tickets_pass_comm" onkeypress="if (curBox()) curBox().changed = 1; onCtrlEnter(event, Tickets.doPass.pbind(' + id +
            ', val(\'tickets_pass_comm\'), curBox()))"></textarea>';
        ...
        if (!cur.isMobileTicket) {
            if (id == 16 || id == 17 || id == 18) {
                cur.sendPayFormCheck = new Checkbox(ge('support_send_payform'), {
                    label: cur.lang.send_pay_form,
                    width: 400
                });
            }
        }
        ...
        autosizeSetup('tickets_pass_comm', {
            minHeight: 40,
            maxHeight: 200
        });
                    ...
                    handler = function() {
                        ...
                        stManager.add('upload.js', function() {
                            if (opts.photoCallback) {
                                ...
                                Tickets.addScreen(Tickets.initPhotoUpload.pbind('tis_add_data', {
                                    hideOnStart: true,
                                    target: target
                                }));
                            }
                    ...
                    handler = function() {
                        ...
                        stManager.add('upload.js', function() {
                            if (opts.oneClick) {
                                ...
                                Tickets.addDoc(Tickets.initDocUpload.pbind('tis_add_data', {
                                    hideOnStart: true,
                                    target: target
                                }));
                            }
...
<div class="fl_l"><div class="page_attach_progress_wrap" style="margin-top: 3px; margin-bottom: 4px;">\
  <div id="upload' +
    ...
    sortModerStats: function(el, field) {
        ...
        if (field != cur.sort) {
            each(geByClass('table_header_upper_span', el.parentNode), function(i, v) {
                removeClass(v, 'sorted');
            });
        ...
        switch (nav.objLoc.act) {
            ...
            case 'all':
                ...
                if (ge('tickets_extra_options') && params.opened) {
                    ...
                    params.photo_server = ge('tickets_photo')
                        .value;
                    params.id100 = ge('tickets_id')
                        .value;
                    params.id1000 = ge('tickets_id1000')
                        .value;
                    params.nospam_pid = ge('tickets_nospam_pid')
                        .value;
                    params.cdn = ge('tickets_cdn')
                        .value;
            ...
            case 'history':
                if (query.q) {
                    if (ge('search_subtab')) {
                        ...
                        ge('search_subtab')
                            .className = 'tickets_subtab1 active';
                    }
                } else {
                    if (hasClass('search_subtab', 'active')) {
                        ge('search_subtab')
                            .className = 'tickets_subtab1 hidden';
            ...
            onDone: function(cont, script) {
                switch (nav.objLoc.act) {
                    ...
                    case 'all':
                        ...
                        if (script) eval(script);
                        delete nav.objLoc.offset;
                        each(['q', 'good', 'opened', 'download', 'from_support', 'photo_server', 'id100', 'nospam_pid', 'time_from', 'time_to', 'mobile',
                            'browser', 'id1000', 'https', 'cdn', 'no_category'
                        ], function(i, v) {
                            if (query[v]) {
    ...
    saveBug: function() {
        ...
        var doSave = function() {
            ...
            if (ge('tickets_closed_autoanswer_addressing_m')) {
                query.addressing_m = ge('tickets_closed_autoanswer_addressing_m')
                    .value;
            }
            if (ge('tickets_closed_autoanswer_addressing_f')) {
                query.addressing_f = ge('tickets_closed_autoanswer_addressing_f')
                    .value;
            }
            query.no_autoanswer = cur.hideAutoanswer.val();
            query.answer_text = ge('tickets_send_autoanswer')
                .value;
            var attachs = [],
        ...
        var box = showFastBox({
            title: cur.lang['bind_title'],
            width: 430,
            bodyStyle: "line-height: 160%;"
        }, cur.lang['sure_bind'], cur.lang['do_bind'], function() {
            if (doSave() && curBox()) {
                curBox()
                    .content('<div style="height:100px; background: url(/images/progress7.gif) 50% 50% no-repeat;"></div>');
                curBox()
                    .setOptions({
                        bodyStyle: 'padding: 0px;'
                    });
        ...
        cur.ticketsAutoMedia = Tickets.initAddMedia(ge('tis_add_lnk_auto')
            .firstChild, 'tis_preview_auto', cur.mediaTypes, {
                limit: 5,
                oneClick: cur.oneClickUpload,
                photoCallback: cur.addAutoReply,
                target: 'auto'
            });
        cur.hideAutoanswer = new Checkbox(ge('support_ignore_autoanswer'), {
    ...
    bindTicket: function(bid, hash) {
        ...
        cur.hideAutoanswer = new Checkbox(ge('support_ignore_autoanswer'), {
            label: cur.lang.no_autoanswer,
            onChange: toggle.pbind(ge('support_sure_bind'), this.val)
        });
        return false;
    },
    ...
    unbindTicket: function(bid, hash, el) {
        ...
        cur.unbindBox = showFastBox({
                title: getLang('support_delete_bind'),
                width: 430,
                onHide: function() {
                    removeEvent(document, 'keydown', enterUnbind);
                }
            }, getLang('support_delete_text')
            .replace('{title}', cur.bug_link || ''), getLang('support_delete'), doUnbind, getLang('global_cancel'));
    },
    ...
    updateModerStats: function(id, hash, type, offset, is_spec) {
        ...
        ge('support_moders_period_stats' + id)
            .innerHTML = '<div class="tickets_detailed_loading"><div>';
        ajax.post(nav.objLoc[0], {
            act: 'detailed_stats',
            mid: id,
            type: type,
    ...
    restoreDraft: function(ticket_id) {
        ...
        if (val(txt)
            .length < draftv.length) {
            val(txt, draftv);
            txt.autosize.update();
        }
        if ((draft.medias || [])
            .length && !((cur.ticketsNewMedia || {})
                .chosenMedias || [])
            .length) {
            var m = [];
