var Offers = {
    initOfficesMenu: function(event) {
        ...
        if (location.hash.indexOf('#/') != -1 || location.hash.indexOf('#!') != -1) {
            realLocation = location.hash.replace('#/', '')
                .replace('#!', '');
        } else {
    ...
    updateActionButtons: function(offerId) {
        ajax.post('/offers?act=a_get_actions_buttons', {
            ...
            onDone: function(response) {
                ge('offers_status_btn')
                    .parentNode.parentNode.innerHTML = response;
            },
            onFail: function(msg) {
                var errorMessage = msg ? msg : getLang('ads_error_unexpected_error_try_later');
                showFastBox(getLang('ads_cant_start_offer_box_title'), errorMessage);
    ...
    updateCostInfo: function(newValue, newText) {
        ...
        geByTag1('a', 'offers_info_cost')
            .innerHTML = newText[0];
        geByClass1('offers_preview_votes', 'offers_preview')
            .innerHTML = newText[0];
    ...
    onLinkTypeClick: function(elem) {
        ...
        hide('offers_edit_link_id_row', 'offers_edit_link_url_row', 'offers_edit_link_object_complete', 'offers_edit_link_object_edit', 'offers_edit_link_id_go_wrap',
            'offers_edit_link_url_go_wrap');
        var uiData = false;
        switch (type) {
            case 'group':
    ...
    editMetricTitle: function(id) {
        ...
        ge('offer_metric_title_input_' + id)
            .focus();
    },
    ...
    saveMetricTitle: function(id) {
        var metricTitle = ge('offer_metric_title_input_' + id)
            .value;
        if (metricTitle.length) {
            ge('offer_metric_title_link_' + id)
                .innerHTML = Offers.getMetricCaption(metricTitle);
    ...
    generateMetricRow: function(id, metricType, metricTitle) {
        ...
        elem.innerHTML =
            ...
            '<div class="offer_metric_title fl_l"><a id="offer_metric_title_link_' + id + '" onclick="Offers.editMetricTitle(' + id + ')">' + metricTitleText +
            '</a><input onblur="Offers.saveMetricTitle(' + id + ')" id="offer_metric_title_input_' + id + '" type="text" placeholder="�������� �������" value="' +
            metricTitleValue + '" maxlength="100"></div>' +
    ...
    addMetric: function(metricId, metricType, metricTitle, doNotEdit) {
        ...
        ge('offers_edit_metrics_container')
            .appendChild(newRow);
    ...
    removeCurrentMetric: function(hash, force) {
        ...
        if (force) {
            ...
            cur.confirmRemoveMetricBox = showFastBox(getLang('global_action_confirmation'), getLang('ads_offers_remove_metric_confirmation'), getLang('Delete'), Offers.removeCurrentMetric
                .pbind(hash, true), getLang('box_cancel'));
        }
    },
    saveCurrentMetric: function(hash) {
        ajax.post('/offers?act=a_save_metric', {
            title: ge('offer_metric_edit_box_metric_title')
                .value,
        ...
        if (show_text) {
            ajax.post('/offers?act=a_save_metric_link', {
                ...
                onDone: function(result, msg, section) {
                    ...
                    ge('offers_metric_link_text')
                        .innerHTML = displayValue;
                },
                onFail: function(msg) {
                    ge('offers_metric_link_error')
                        .innerHTML = msg;
    ...
    updateDefaultInstruction: function() {
        ...
        text = text.replace('{name}', cur.uiLinkId.val_full()[1])
            .replace('{votes}', getLang('global_n_votes', cur.uiCostSelector.val()))
            .replace(/&quot;/g, '"');
    ...
    editOnParamUpdate: function(paramName, paramValue, delayed) {
        ...
        switch (paramName) {
            ...
            case 'descr':
                ...
                var remainLength = targetElem.getAttribute('maxlength') - targetElem.value.replace("\r", '')
                    .length;
    ...
    getOfferTargetData: function() {
        ...
        each(geByClass('offers_edit_metric_row'), function(index, elemRow) {
            ...
            var metric = [id, type, ge('offer_metric_title_input_' + id)
                .value
            ].join(';;;');
            metrics.push(metric);
        });
        ...
        switch (event.target.index) {
            ...
            case 1:
                ...
                var saveOfficesList = function() {
                    cur.offices_list = ge('offers_offices_list')
                        .value;
                    curBox()
                        .hide();
                };
                showFastBox({
                    ...
                    '<div>�������� ID ��������� ��������, �� ������� ���� �������� ��������, � ��������� �������:<br>[ID ��������]<span style="color: #888; font-size: 0.8em; margin: 0 2px;">���</span>[�������� ��� "-"]<span style="color: #888; font-size: 0.8em; margin: 0 2px;">���</span>[������ ��� "-"]<span style="color: #888; font-size: 0.8em; margin: 0 2px;">���</span>[��������� ��� "-"]</div><textarea id="offers_offices_list" style="width: 100%; height: 200px; margin-top: 4px;">' +
                    (cur.offices_list ? cur.offices_list : '') + '</textarea>',
                    '���������', saveOfficesList);
                break;
        }
    },
    ...
    submitExportStatsForm: function() {
        ...
        var isExportMethodHtml = (Ads.getNamespace('offers_export_stats')
            .export_method.value == 0);
        var topUnionIdParam = (cur.topUnionId ? '&union_id=' + cur.topUnionId : '');
        var downloadWatcherCookieName = 'dwcookie';
        var downloadWatcherCookieValue = Math.random();
        var postFormParams = {
        ...
        if (isExportMethodHtml) {
            ...
            postFields['start_time'] = cur.exportParamsData.start_time.year + ('0' + cur.exportParamsData.start_time.month)
                .slice(-2) + ('0' + cur.exportParamsData.start_time.day)
                .slice(-2);
            postFields['end_time'] = cur.exportParamsData.stop_time.year + ('0' + cur.exportParamsData.stop_time.month)
    ...
    createExportSubmitButton: function(element) {
        ...
        (new Image())
        .src = '/images/upload_inv.gif'; // pre-load animation
