var Restore = {
    ...
    uploadError: function(code, type) {
        ...
        setTimeout(showFastBox(getLang('global_error'), err)
            .hide, 2000);
    ...
    uploadComplete: function(photo, photo_id, photo_hash, type) {
        ...
        ge(prefix + 'input')
            .disabled = cur.images_count[type] >= 2 ? true : false;

        show(prefix + 'photos');
        ge(prefix + 'photos')
            .innerHTML += '<div id="photo' + index + '"><img id="photo_img' + index + '" src="' + photo +
            '" /><span onmouseover="this.className=\'over\';" onmouseout="this.className=\'\';" onclick="Restore.deleteImage(' + type + ', ' + index + ')" id="del_link' +
            index + '">' + getLang('global_delete') + '</span></div>';
    },
    deleteImage: function(type, index) {
        ...
        if (cur.images[index].deleted) {
            ...
            ge('del_link' + index)
                .innerHTML = getLang('global_delete');
        } else {
            ...
            ge(prefix + 'input')
                .disabled = false;
            ge('del_link' + index)
                .innerHTML = getLang('global_dont_delete');
        }
    },
    ...
    submitPageLink: function() {
        ...
        ajax.post('al_restore.php', params, {
            onDone: function(msg, title) {
                ge('error')
                    .innerHTML = (title ? '<b>' + title + '</b><br>' : '') + msg;
                show('error');
            },
            onFail: function() {
                unlockButton(btn);
        ...
        ajax.post('/al_restore.php', params, {
            onDone: function(result, msg, input, login_error) {
                ...
                if (code == -1) {
                    ...
                    Restore.phone_confirm_box = showFastBox(getLang('restore_confirmation'), '<div id="phone_confirm_box">' + ge('phone_confirm')
    ...
    confirmPhoneSend: function() {
        var code = trim(ge('phone_confirm_code')
            .value);
    ...
    initDropdowns: function() {
        ...
        cur.uiCity = new CitySelect(ge('reg_city'), ge('reg_city_row'), {
            width: 210,
            show: function(container) {
                show(container);
                ge('request_reg_info')
                    .style.bottom = ge('reg_year_row')
                    .clientHeight + 'px';
            },
        ...
        cur.uiCountry = new CountrySelect(ge('reg_country'), ge('reg_country_row'), {
            width: 210,
            citySelect: cur.uiCity
        });
        cur.uiYear = new Dropdown(ge('reg_year'), reg_years, {
            width: 210
        });
