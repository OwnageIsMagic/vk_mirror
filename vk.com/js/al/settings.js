var Settings = {
    ...
    getsect: function() {
        var current = ge('settings_filters')
            .firstChild
    ...
    saveSmsNotify: function(btn) {
        ...
        if (cur.daytime.checked()) {
            params.daytime_from = ge("daytime_from")
                .value;
            params.daytime_to = ge("daytime_to")
                .value;
        } else {
    ...
    initNotify: function() {
        ...
        var uiDaytimeFrom = new Dropdown(ge("daytime_from"), ["00:00", "01:00", "02:00", "03:00", "04:00", "05:00", "06:00", "07:00", "08:00", "09:00", "10:00", "11:00",
            "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00", "20:00", "21:00", "22:00", "23:00"
        ], {
            selectedItems: cur.options.time_from,
            width: 60
        });
        var uiDaytimeTo = new Dropdown(ge("daytime_to"), ["00:00", "01:00", "02:00", "03:00", "04:00", "05:00", "06:00", "07:00", "08:00", "09:00", "10:00", "11:00",
            "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00", "20:00", "21:00", "22:00", "23:00"
        ], {
    ...
    openTwitterSettings: function() {
        if (!Settings.twitterMB) {
            var edit_content =
                '<div class="info_msg" style="margin:-5px 0px 14px 0px; _margin-top:0px; font-weight: normal; line-height: 150%"><img align="left" src="/images/twitter_sync.png" style="margin: 3px 9px 20px 0px;" /><b>Twitter</b><br />' +
                getLang('settings_twitter_desc') + '</div>';
            Settings.twitterMB = new MessageBox({
                title: getLang('settings_status_export'),
                progress: 'socialProgress',
                selfDestruct: false,
                onHide: function() {
                    if (twitterRequst) clearInterval(twitterRequst);
                }
            });
            Settings.twitterMB.addButton(getLang('global_cancel'), Settings.twitterMB.hide, 'gray');
            Settings.twitterMB.content(edit_content + '<div align="center">' + getLang('settings_external_site_request') + '<div>')
                .show();
            show('socialProgress');
            var twitterRequst = setInterval(function() {
                ajax.post('/al_settings.php', {
                    act: 'a_get_twitter_login_url',
                    'new': Settings.twitterRequestNew
                }, {
                    onDone: function(result) {
                        if (result && result.data) {
                            clearInterval(twitterRequst);
                            hide('socialProgress');
                            Settings.twitterMB.addButton(getLang('settings_auth_in_twitter'), function() {
    ...
    openLiveJournalSettings: function() {
        slideDown(ge('lj_data'), 200, function() {
            elfocus('lj_login');
        });
    ...
    getFacebookName: function() {
        var nameService3 = setInterval(function() {
            ajax.post('/al_settings.php', {
                act: 'a_get_facebook_name'
            }, {
                onDone: function(result) {
                    if (result && result.login && result.name) {
                        clearInterval(nameService3);
                        var ns = ge('name_service3');
                        ns.href = result.name[0] == '+' ? 'http://facebook.com/app_scoped_user_id/' + result.name.substring(1) :
                            'http://facebook.com/profile.php?id=' + result.name;
                        ns.innerHTML = result.login;
                    } else if (result && result.login == undefined) {
                        clearInterval(nameService3);
                        ge('name_service3')
                            .innerHTML = '<span style="color: #800">' + result + '</span>';
                    }
                }
            });
        }, 1000);
    ...
    clearSocialExport: function(service) {
        ...
        var clrBox = showFastBox({
            title: getLang(title),
            dark: 1,
            bodyStyle: 'padding: 20px; line-height: 160%;'
        }, msg, getLang('global_continue'), function() {
            ajax.post('/settings', {
                act: 'a_clear_social_export',
                service: service,
                hash: cur.options.social_hash
    ...
    submitServices: function() {
        var params = {
                hash: cur.options.services_hash,
        ...
        each(geByClass('checkbox', ge('settings_services'), 'div'), function() {
            var name = this.getAttribute('name'),
                isOn = this.className.indexOf('on') != -1;
    ...
    OTPAuthGetTrusted: function(force) {
        ...
        if (force || geByTag1('img', el)) {
            ...
            ajax.post(vk.loginscheme + '://' + vk.host + '/al_login.php', {
    ...
    OTPCreateAppPassword: function(btn, hash) {
        ...
        ajax.post('al_settings.php', {
            ...
            onDone: function(title, html, table) {
                ...
                ge('settings_app_passwords_table_wrap')
                    .innerHTML = table;
            ...
            onFail: function(msg) {
                if (msg) {
                    ge('settings_app_passwords_error')
                        .innerHTML = msg;
    ...
    OTPRemoveAppPassword: function(link, id, hash) {
        ...
        ajax.post('al_settings.php', {
            ...
            onDone: function() {
                ...
                if (geByTag('tr', 'settings_app_passwords_table_wrap')
                    .length <= 1) {
                    ge('settings_app_passwords_table_wrap')
                        .innerHTML = '';
    ...
    showMsg: function(msg) {
        ge('settings_result')
            .innerHTML = '<div id="settings_save_msg" class="msg">' + msg + '</div>';
        if (!msg) {
    ...
    nameMoved: function(btn) {
        ajax.post('al_settings.php', {
            act: 'a_hide_name_moved_hint'
        }, {
            onDone: function(msg) {
                animate('settings_name_moved_row', {
                    height: 0
                }, 200, function() {
...
<form action="' + vk.loginscheme + '://login.vk.com/" method="POST" target="reset_sessions_frame">\
  ...
  <input name="reset_hash" value="' + cur.options.reset_hash + '" type="hidden" />' + addParams +
    ...
    showUserClientTT: function(el, i) {
        var text = '';
        if (hasClass(el.parentNode.parentNode, 'old_session')) text = '<div style="font-weight:bold; margin-bottom:5px;">' + getLang('settings_session_terminated') +
            '</div>';
    ...
    doCheckAddress: function() {
        ...
        ajax.post('al_settings.php', {
            ...
            onFail: function(msg) {
                cur.addrChecked = -1;
                btnEl.innerHTML = msg;
                setStyle('settings_addr_disabler', {
                    width: btnWEl.offsetWidth,
                    height: btnWEl.offsetHeight,
                    display: 'block'
                });
                return true;
            },
    ...
    init: function() {
        ...
        var addrEl = ge('settings_addr'),
            addrTarget = ge('settings_addr_table');
