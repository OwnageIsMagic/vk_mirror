var Dev = {
    ...
    switchPage: function(page, edit, opts) {
        ...
        ajax.post('/dev/' + page, pageOpts, {
            onDone: function(title, text, acts, top_section, edit_sections, isPage, isSection, opts, js, bodyClass, parent_section) {
                ...
                ge('dev_page_acts')
                    .innerHTML = acts;
                ge('dev_page_sections')
                    .innerHTML = edit_sections;
    ...
    wrapObject: function(obj, rootNode, objName, parentContext) {
        ...
        switch (typeof obj) {
            case 'object':
                ...
                if (Object.prototype.toString.call(obj) == '[object Array]') {
                    ...
                    html += '<span class="dev_result_block"><span class="dev_result_lbracket" onclick="Dev.btHide(this);">[</span>' + items.join(', ') +
                    ...
                    if (rootNode) {
                        ...
                        html += '<span class="dev_result_block"><span id="dev_wrap_open_' + cur.wrapNum +
                            '" class="dev_result_bracket" onclick="Dev.btHide(this);">{</span><br/>' + res + '<span id="dev_wrap_close_' + cur.wrapNum +
                            '" class="dev_result_bracket" onclick="Dev.btHide(this);">}</span></span>';
                        cur.wrapNum += 1;
                    }
                }
                break;
        ...
        if (rootNode && obj.response && obj.response['upload_url'] && Dev.checkUploadUrl(obj.response['upload_url'])) {
            html += '<div class="dev_upload_form"><form id="dev_file_submit" action="' + clean(obj.response['upload_url']) +
    ...
    onSearchSelect: function(ev) {
        ...
        if (curSel) {
            ...
            var methodStr = (val(firstSel) || '')
                .replace(/<[^>]*>/g, '');
            if (methodStr && methodStr.replace(/[\. ]/g, '')
                .toLowerCase()
                .indexOf(q.replace(/[\. ]/g, '')
                    .toLowerCase()) === 0) {
                if (methodStr.split('.')[0].toLowerCase()
    ...
    paletteDown: function(ev, down, y, noChangeColor) {
        ...
        t = (t == 360) ? 0 : t;
        ge('dev_colors')
            .style.backgroundColor = "rgb(" + Dev.hsv2rgb(t, 100, 100) + ")";
        if (!noChangeColor) {
    ...
    mainPageSliderInit: function(items) {
        ...
        var backgrounds = '',
            buttons = '';
        for (var i in items) {
            backgrounds += '<div class="dev_main_featured_banners_bg_item dev_main_featured_banners_bg_' + items[i].className + '" id="dev_main_featured_banners_bg_' + i +
                '"></div>';
            buttons += '<div class="dev_main_featured_banners_change_button" id="dev_main_featured_banners_button_' + i + '" onclick="Dev.mainPageSliderChange(' + i +
                ', \'auto\')">' + items[i].title + '</div>';
        }
