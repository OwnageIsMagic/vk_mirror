    ...
    cur.subscribeMobileLaunch = function(event) {
        ...
        ajax.post('/ads?act=a_subscribe_mobile_launch', {
            ...
            onDone: function(msg) {
                ...
                ge('ads_mobile_intro_subscribe_button')
...
Ads.openInnerTable = function(id, bindingId) {
    ...
    if (id in tab.contentDep && tab.contentDep[id]) {
        ...
        ajax.post('/ads?act=a_get_client_children', {
            client_id: id,
            ads_types: ads_types
        }, {
            onDone: onDone,
...
Ads.createStaticDropdown = function(elem, bindingId, values, params) {
    ...
    params.updateHeader = function(i, t, pp) {
        ...
        if (elem.className.substring(0, 'ads_types'.length) == 'ads_types') {
            ...
            toggle(geByClass1('client_choose_row_' + bindingId), promoted_posts_mode || (geByClass1('grouping_ads_' + bindingId)
                .uiDropdown.value == 2));
            ...
            grouping_ads_el.uiDropdown.options.updateHeader(promoted_posts_mode ? (grouping_ads_el.uiDropdown.items[3] ? 3 : 2) : grouping_ads_el.uiDropdown.value,
                undefined, promoted_posts_mode);
        } else
        //
        // grouping_time
        //
        ...
        switch (valueGeneralType) {
            case 'cost_per_click':
                ...
                if (!minValue || valueFloat < minValue) {
                    return getLang(minErrorLangKey)
                        .replace('{money}', getLang('global_money_amount_rub', minValue));
                }
                if (!maxValue || valueFloat > maxValue) {
                    return getLang(maxErrorLangKey)
                        .replace('{money}', getLang('global_money_amount_rub', maxValue));
                }
            ...
            case 'limit':
                ...
                if (parseInt(value) < cur.unionsLimits.limit_min) {
                    return getLang('ads_error_limit_min_value')
                        .replace('{money}', getLang((additionalParams.is_votes_limit ? 'global_n_votes' : 'global_money_amount_rub'), cur.unionsLimits.limit_min));
                }
...
Ads.addOfficeFormatFunctions = function(options) {
    ...
    var limitFormat = function(data, rown, coln) {
        ...
        try {
            ...
            if (uid) {
                wrap = "<img id=\"inline_edit_progress" + uid + "\" src=\"/images/upload.gif\" style=\"display: none;\" /><a id=\"inline_edit_value" + uid +
                    "\" >{value}</a>";
            }
        } catch (e) {
            // Nothing
...
Ads.changeDemographyView = function(name, unionId, updateOnly) {
    ...
    var navElems = [
        ge('ads_demography_navigation_tab_bars_all')
        .parentNode,
        ge('ads_demography_navigation_tab_graphs')
        .parentNode
    ];
...
Ads.changeDemographyAge = function(name) {
    // Change navigation
    var allNav = ge('ads_demography_navigation_tab_age_all')
        .parentNode;
    var femaleNav = ge('ads_demography_navigation_tab_age_female')
        .parentNode;
    var maleNav = ge('ads_demography_navigation_tab_age_male')
        .parentNode;
    switch (name) {
        case 'all':
...
Ads.loadDemography = function(unionId) {
    ...
    function onComplete(response) {
        ...
        if (response && response.graphs_html && response.graphs_js) {
            ...
            ge('ads_demography_graphs_wrap2')
                .innerHTML = response.graphs_html;
...
Ads.toggleRetargetingInput = function(id, show) {
    ...
    Ads.retargetingInputChanged = function(id) {
        var new_val = trim(ge('ads_retargeting_' + id)
            .value);
...
Ads.sendContactsForm = function(button) {
    ...
    ajaxParams.country = ge('country')
        .value;
    ajaxParams.city = ge('city')
        .value;
    ajaxParams.agency = ge('agency')
        .value;
    ajaxParams.dating = ge('dating')
        .value;
    ajaxParams.medicine = ge('medicine')
        .value;
