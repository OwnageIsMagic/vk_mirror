var Profile = {
        ...
        photoRemove: function(lnk, photo_id, ev) {
            ...
            ajax.post('al_profile.php', {
                ...
                onDone: function(about, photos) {
                    val(ge('profile_photos_about') || ge('profile_photos_module')
                        .appendChild(ce('div', {
                            className: 'msg',
                            id: 'profile_photos_about'
                        })), about);
        ...
        setupSMS: function(hash) {
            ...
            showBox('al_mobile.php', {
                act: 'configure_sms_notifications',
                mid: cur.oid,
                hash: hash
            }, {
                params: {
        ...
        toggleFriend: function(btn, hash, act, ev, sure) {
            ...
            ajax.post('al_friends.php', {
                ...
                onDone: function(text, vis, ttText, ttScript, doReload) {
                    ...
                    var tt = (ge('profile_am_subscribed') || {})
                        .tt;
                    if (tt && tt.hide) {
                        tt.hide({
                            fasthide: 1
        ...
        frDropdownPreload: function(el, sh) {
            ...
            ajax.post('al_friends.php', {
                ...
                onDone: function(html, js) {
                    ...
                    var firstChild = domFC(ce('div', {
                        innerHTML: html
                    }));
                    if (firstChild) domPN(el)
                        .insertBefore(firstChild, el);
                    eval(js);
                },
                cache: 1
            });
        ...
        frListsDDShow: function(ev) {
            ...
            for (var j = 0, i; j < 5; ++j) {
                ...
                if (cur.options.publicLists[i]) {
                    elems.push('<a class="ddreq_item ddreq_cat' + ((cats & (1 << parseInt(i))) ? ' checked' : '') + '" onclick="Profile.frListsCheck(this, ' + i + ');">' + cur
                        .options.publicLists[i] + '</a>');
                }
            }
            for (var i in cur.options.userLists) {
                if (i < 25) {
                    ...
                    elems.push('<a class="ddreq_item ddreq_cat' + ((cats & (1 << parseInt(i))) ? ' checked' : '') + '" onclick="Profile.frListsCheck(this, ' + i + ');">' +
                        lname + '</a>');
                }
            }
        ...
        showHideGiftsBox: function(ev) {
            Profile.hideOther(-1);
            if (cur.viewAsBox) return cur.viewAsBox();
            var msg = getLang('profile_sure_hide_gifts')
                .replace('{link}', '<a href="/settings">')
                .replace('{/link}', '</a>')
                .replace('{link1}', '<a href="/settings?act=privacy">')
                .replace('{/link1}', '</a>');
        ...
        showNewGift: function(giftId, src) {
            ...
            var onload = function() {
                ...
                if (firstPic) {
                    ...
                    setStyle(a, {
                        marginLeft: -getSize(firstPic)[0] + 'px'
                    });
        ...
        showFans: function(ev) {
            ...
            return !showTabbedBox('al_fans.php', {
                act: 'show_fans_box',
                oid: cur.oid
            }, {
                cache: 1,
...
<table cellspacing="0" cellpadding="0">\
  ...
  <tr><td><div class="bottom"></div></td></tr>\
        ...
        init: function(opts) {
            ...
            (cur._back ? cur._back.hide : cur.destroy)
            .push(function(c) {
                clearTimeout((c || cur)
                    .clHintTimer);
                clearTimeout((c || cur)
        ...
        inviteHintUpdate: function() {
            ...
            geByClass1('top_pointer', hint.tt.container)
                .style.margin = '0px ' + r + ' 0px ' + l;
        },
        ...
        giftSend: function() {
            ...
            ajax.post('al_gifts.php', {
                ...
                onDone: function(data, text, giftsCnt, giftsModule, giftThumb) {
                    ...
                    text = text.replace('<b>', '<div class="top_result_header">')
                        .replace('</b><br>', '</div>');
                    showDoneBox(text, {
                        out: 1500
                    });
        ...
        publishPhotos: function(photos) {
            ...
            ajax.post('/al_wall.php', query, {
                onDone: function(wall) {
                    ...
                    showDoneBox('<div style="font-weight: bold">' + getLang('profile_photos_published') + '</div>', {
                        w: 320
                    });
        ...
        initPhotosUploadBox: function() {
            ...
            cur.photoUploadBox = showFastBox({
                    title: getLang('profile_photo_upload_title'),
                    width: 410,
                    dark: 1,
                    onHide: function() {
                ...
                }, '<div class="profile_photo_upload_box"><div>' + getLang('profile_photos_uploading') +
                '</div><div id="profile_photo_upload_label"></div><div class="profile_progress_wrap"><div id="profile_photo_progress" class="profile_progress" style="width: 0%;"></div></div></div>',
                getLang('global_cancel'),
                function() {
                    cur.photoUploadBox.hide();
                    delete cur.photoUploadBox;
        ...
        initPhotosUpload: function() {
            ...
            var photos_data = (cur.options || {})
                .photos_upload,
                area = ge('profile_photos_upload') || ge('profile_photos_extra_link'),
                el = se(
                    '<div id="profile_photo_uploader" onclick="cancelEvent(event); return false;" style="position: absolute; height: 100%; width: 100%; cursor: pointer;"></div>'
                );
            ...
            cur.phUplInd = Upload.init('profile_photo_uploader', photos_data.url, photos_data.params, {
                ...
                onUploadStart: function(info, res) {
                    ...
                    if (info.num === 0) {
                        txt = (info.totalCount > 1) ? getLang('profile_photos_uploading_x')
                            .replace('%s', info.totalCount) : getLang('profile_photo_uploading');
                    } else {
                        txt = getLang('profile_photos_uploaded_x', info.num)
                            .replace('{count}', info.totalCount);
                    }
                    ge('profile_photo_upload_label')
                        .innerHTML = txt;
                },
                onUploadComplete: function(info, res) {
                    if (info.totalCount > 1 && info.totalCount == info.num + 1) {
                        ge('profile_photo_upload_label')
                            .innerHTML = getLang('profile_photos_uploaded_x', info.totalCount)
                            .replace('{count}', info.totalCount);
                    }
                ...
                onUploadProgress: function(info, bytesLoaded, bytesTotal) {
                    ...
                    animate(ge('profile_photo_progress'), {
                        width: newWidth + 'px'
                    }, 200);
                },
                ...
                onCheckComplete: function(i) {
                    try {
                        ...
                        ge('uploader_lite' + i)
                            .configureServer(Upload.uploadUrls[i], escape(postData.join('&')));
                    } catch (e) {
                        debugLog(e);
                    }
                },
