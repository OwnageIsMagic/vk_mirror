var Calendar = {
        ...
        if (a[0]) {
            ...
            for (var r in a) {
                ...
                if (i = new Date(i.getFullYear(), i.getMonth(), i.getDate()), 0 > d) {
                    ...
                    10 > g && (g = "0" + g);
                    var h = v + ":" + g;
                    u = +s > +new Date ? getLang("events_calendar_future_event")
                        .replace("{time}", h) : getLang("events_calendar_past_event")
                        .replace("{time}", h)
                } else {
                    var u, _ = "";
                    if (s.getFullYear() != i.getFullYear()) {
                        var f = o.getFullYear() - s.getFullYear();
                        u = getLang(i > o ? "events_calendar_past_birthday" : i.toString() == o.toString() ? "events_calendar_recent_birthday" :
                            "events_calendar_future_birthday", f)
                    } else u = getLang("events_celebrates_birthday")
                }
                t += cur.boxRowTmpl.replace("{fid}", d)
                    .replace("{class}", _)
                    .replace("{name}", c)
                    .replace(/{href}/g, l)
                    .replace("{photo}", n)
                    .replace("{label}", u.toLowerCase())
                    .replace("{actions}", "")
            }
            t += "</div>";
            var y = getLang("events_calendar_day_box_title")
                .replace("{day}", o.getDate() + " " + getLang("month" + (o.getMonth() + 1) + "_of") + " " + o.getFullYear());
            showFastBox({
                title: y,
                width: 400,
        ...
        if (t[0]) {
            ...
            for (var s in t) {
                var o = t[s][4],
                    i = t[s][2],
                    u = t[s][0];
                r += '<a class="' + d + " bd_user_cell_" + s + '" href="' + i + '" onclick="return nav.go(this, event);"><img class="bd_cell_img" src="' + o +
                    '" onmouseover="Calendar.calTrailOn(this, ' + u + ", " + e + ');" onmouseout="Calendar.calTrailOff(this, ' + u + ');"/></a>', 1 == s && (r += "<br/>")
            }
            n && (r += '<a class="bd_day_more" href="" onclick="Calendar.calShowMore(' + e + '); return cancelEvent(event);"><span class="bd_day_more_num">+' + n +
                "</span></a>")
        }
        return r
    },
    calTrailOn: function(e, a, t) {
        ...
        if (0 > a) {
            ...
            10 > o && (o = "0" + o);
            var i = s + ":" + o;
            d = +n > +new Date ? getLang("events_calendar_future_event")
                .replace("{time}", i) : getLang("events_calendar_past_event")
                .replace("{time}", i)
        ...
        var _ = '<a href="' + r[2] + '" onclick="return nav.go(this, event);">' + r[1] + "</a>",
            ...
            g = function() {
                ...
                showTooltip(e.parentNode, {
                    text: '<a href="' + r[2] + '" onclick="return nav.go(this, event);"><img class="photo" src="' + r[5] +
                        '"/></a><div class="info"><div class="info_inner"><div class="name' + t + '">' + _ + "</div>" + d + "</div></div>",
                    dir: "auto",
                    className: "cal_tt",
                    slide: 15,
                    hasover: 1,
    ...
    calGetMonth: function(e) {
        window.tooltips && tooltips.hideAll(), cur.calMon += e, cur.calMon > 12 ? (cur.calMon = 1, cur.calYear++) : cur.calMon < 1 && (cur.calMon = 12, cur.calYear--), ge(
                "bd_calendar_header")
            .innerHTML = getLang("Month" + cur.calMon) + " " + cur.calYear;
        var a = new Date(cur.calYear, cur.calMon, 0)
            .getDate(),
        ...
        for (var s = 0; l > s; s++) {
            for (var o = "", i = 0; 7 > i; i++) {
                ...
                if (u > 0 && a >= u) {
                    ...
                    o += '<td class="bd_day_cell day' + (i + 1) + " " + _ + '">            <div class="bd_day_num">' + u + '</div>            <div class="bd_day_events">' +
                        h + "</div>          </td>"
        ...
        return ge("bd_calendar_table_wrap")
            .innerHTML = '<table class="bd_day_table" cellpadding="0" cellspacing="0" align="center">      <tr>       <td class="bd_day_head day1">' + getLang("events_mon") +
            '</td>       <td class="bd_day_head day2">' + getLang("events_tue") + '</td>       <td class="bd_day_head day3">' + getLang("events_wed") +
            '</td>       <td class="bd_day_head day4">' + getLang("events_thu") + '</td>       <td class="bd_day_head day5">' + getLang("events_fri") +
            '</td>       <td class="bd_day_head day6">' + getLang("events_sat") + '</td>       <td class="bd_day_head day7">' + getLang("events_sun") + "</td>      </tr>" +
            c + "</table>", !1
    },
