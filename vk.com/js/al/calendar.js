var Calendar = {
    ...
    calShowMore: function(el, i, day) {
        ...
        p.innerHTML = '<a href="" onclick="Calendar.calShowMore(this, ' + i + ', ' + day + '); return cancelEvent(event);">' + getLang('events_calendar_show_more') +
            '</a>';
        e.innerHTML = '<a href="' + item[2] + '" onclick="return nav.go(this, event);"><img width="50" src="' + item[4] + '" onmouseover="Calendar.calTrailOn(this, ' +
            item[0] + ', ' + day + ');" onmouseout="Calendar.calTrailOff(this, ' + item[0] + ');"/></a>';
    },
    calTrailOn: function(el, fid, day) {
        ...
        if (fid < 0) {
            ...
            if (+bDay > +(new Date())) {
                ...
                text = getLang('events_calendar_past_event')
                    .replace('{time}', time);
            }
        } else {
        ...
        var showTT = function() {
            ...
            showTooltip(el.parentNode, {
                content: '<div class="wrap"><div class="content" align="center"><a href="' + item[2] + '" onclick="return nav.go(this, event);"><img src="' +
                    item[5] + '"/></a><div class="name">' + name + '</div>' + text + '</div></div>',
                className: 'rich wall_tt cal_tt',
                slide: 15,
                shift: [w > 25 ? 43 : 58, 3, w > 25 ? 40 : 3],
                toup: true
            });
        };
        im.src = item[5];
    ...
    calGetMonth: function(shift) {
                ...
                if (day > 0 && day <= days) {
                    ...
                    if (calEvents[0]) {
                        ...
                        if (calEvents.length > 4) {
                            showMore = '<a href="" onclick="Calendar.calShowMore(this, 4, ' + day + '); return cancelEvent(event);">' + getLang('events_calendar_show_more') +
                                '</a>';
                        ...
                        for (var i in calEvents) {
                            ...
                            dayEvents += '<a class="' + calLayout + ' bd_user_cell_' + i + '" href="' + href +
                                '" onclick="return nav.go(this, event);"><img class="bd_cell_img" src="' + photo + '" onmouseover="Calendar.calTrailOn(this, ' + fid + ', ' +
                                day + ');\" onmouseout="Calendar.calTrailOff(this, ' + fid + ');"/></a>';
                        }

                    }

                    rowHTML += '<td class="bd_day_cell day' + (weekDay + 1) + ' ' + dayClass + '">\
            <div class="bd_day_num fl_l">' + day +
                        '</div><div class="bd_day_text fl_l">' + dayText + '</div>\
            <div class="bd_day_events clear">' + dayEvents +
                        '</div>\
        ...
        ge('bd_calendar_table_wrap')
            .innerHTML = '<table class="bd_day_table" cellpadding="0" cellspacing="0" align="center">\
      <tr>\
       <td class="bd_day_head day1">' + getLang(
                'events_mon') + '</td>\
       <td class="bd_day_head day2">' + getLang('events_tue') + '</td>\
       <td class="bd_day_head day3">' + getLang(
                'events_wed') + '</td>\
       <td class="bd_day_head day4">' + getLang('events_thu') + '</td>\
       <td class="bd_day_head day5">' + getLang(
                'events_fri') + '</td>\
       <td class="bd_day_head day6">' + getLang('events_sat') + '</td>\
       <td class="bd_day_head day7">' + getLang(
                'events_sun') + '</td>\
      </tr>' + rows + '</table>';
