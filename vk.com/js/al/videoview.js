var Videoview = {
        ...
        playerCallback: {
            ...
            onVideoPlayProgress: function(oid, vid, hash, time_progress, time_total) {
                ...
                if (mvcur.adData) {
                    ...
                    if (mvcur.adData.stat_link_half && !mvcur.adData.view_complete_half && time_progress >= (time_total / 2)) {
                        ajax.post(mvcur.adData.stat_link_half, {}, {
                            onDone: function() {},
                            onFail: function() {
                                return true;
                            }
                    ...
                    if (mvcur.adData.stat_link_full && !mvcur.adData.view_complete_full && time_progress >= (time_total * 0.9)) {
                        ajax.post(mvcur.adData.stat_link_full, {}, {
                            onDone: function() {},
                            onFail: function() {
                                return true;
                            }
            ...
            onViewSegmentsChanged: function(oid, vid, segments, hash) {
                ...
                if (prev && prev.ts && ((new Date()
                        .getTime()) - prev.ts) > 1000 * 60 * 60 * 24) {
                    prev = null;
                    ls.remove(ls_key);
                }
        ...
        removeVideo: function() {
            ...
            if (mv && mv.deleteFromAllAlbumsHash) {
                ajax.post('/al_video.php', {
                    ...
                    oid: mv.oid,
                    target_id: vk.id,
                    hash: mv.deleteFromAllAlbumsHash
                }, {});
        ...
        initAddButton: function() {
            ...
            function initTooltip(addToDefaultList) {
                mvcur.addButtonTT = mvcur.addButtonTT || new ElementTooltip(btn, {
                    ...
                    onFirstTimeShow: function(content) {
                        var playlistsHtml = '<div id="mv_pl_tt">' + (noPublicAdd ? '<div class="mv_tt_private_only">' + getLang('video_only_private_video') +
                            '</div>' : '') + '<div class="mv_tt_playlists' + (noPublicAdd ? ' with_border' : '') + '">';
                        each(mvcur.mvData.playlists, function(i, pl) {
                            var privateIcon = pl.private ? '<span class="video_album_private_icon" onmouseover="showTooltip(this,{black:1,text: \'' +
                                getLang('video_album_is_private_tt') + '\', shift:[13, 3, 0]})"></span>' : '';
                            playlistsHtml += '<div class="mv_tt_playlist" data-title="' + clean(pl.title) + '" data-added="' + (+pl.added) + '" data-id="' +
                                pl.id + '" data-disabled="' + (+pl.disabled) + '"><span></span>' + privateIcon + '</div>';
                        });
                        playlistsHtml += '</div>';
                        playlistsHtml += '<div class="mv_tt_add_playlist" onclick="Videoview.addPlaylist(' + noPublicAdd +
                            ')"><span class="mv_tt_plus_icon"></span>' + (noPublicAdd ? getLang('video_add_private_album') : getLang('video_add_album')) +
                            '</div></div>';
            ...
            function togglePlaylist() {
                ...
                ajax.post('/video', {
                    ...
                    oid: mvcur.mvData.oid,
                    vid: mvcur.mvData.vid,
                    hash: this.checked ? mvcur.mvData.playlistAddHash : mvcur.mvData.playlistRemoveHash,
                    playlist_id: this.playlistId,
                    add: +this.checked,
                    info: Video.isInCatalogue() ? Videocat.getCurrentPlaylistId() : ''
                }, {
        ...
        actionInfo: function() {
            return ge('mv_action_info') || mvcur.mvWide.appendChild(ce('div', {
                id: 'mv_action_info'
            }));
        },
        ...
        show: function(ev, videoRaw, listId, options) {
            ...
            if (options.prevLoc) {
                ...
                setTimeout(function() {
                    ...
                    setTimeout(window.comScoreUDM && comScoreUDM.pbind(locProtocol + '//' + location.host + '/al_video.php?comscorekw=pageview_candidate', referrer),
                        10);
                }, 0);
            }
            ...
            if (!mvcur.mvFixed) {
                ...
                if (vk.rtl) {
                    ...
                    var minimizeBtn =
                        '<div class="divider fl_r">|</div><div onmouseover="Videoview.activate(this, 2);" onmouseout="Videoview.deactivate(this, 2);" onclick="return Videoview.minimize(event);" class="mv_top_button fl_r">' +
                        getLang('global_min') + '</div>';
                }
 ...
 \
    ...
    ';
        ...
        hide: function(noLoc, force, ev, closeButtonClick) {
            ...
            if (mvcur.mvData.duration > 60 && !force && !mvcur.finished) {
                var openTime = new Date()
                    ...
                    closeText = getLang('video_are_you_sure_close');
                if (openTime > 30000 && closeText != 'are you sure close' && !browser.safari_mobile) {
                    ...
                    box = showFastBox({
                        ...
                        dark: 1,
                        forceNoBtn: 1
                    }, closeText, getLang('box_yes'), function() {
                        box.hide();
            ...
            vkImage()
                .src = locProtocol + '//www.tns-counter.ru/V13a****pladform_ru/ru/CP1251/tmsec=pladform_videovk-playerend/' + Videoview.playerCallback.randomNumber();
        ...
        setLocation: function(noLocChange) {
            ...
            var m = mvcur.videoRaw.match(/^(-?\d+)(photo|video)?_/),
                owner = intval(m[1]),
                nl;
            ...
            //  }
            //} else {
            var videoLocation = 'video' + mvcur.videoRaw,
                nl;
            if (mvcur.listId) {
        ...
        highlightComment: function(el) {
            ...
            if (top < 0 || top > lastWindowHeight - 200) {
                animate(mvLayerWrap, {
                    scrollTop: mvLayerWrap.scrollTop + top - 50
                }, 300, hlfunc);
            } else {
                hlfunc();
            }
        ...
        commAction: function(act, comm, hash, from) {
            ...
            ajax.post('al_video.php', {
                ...
                videoview: 1,
                from: from
            }, Videoview.commParams(comm, from));
        },

        comments: function(showcomm) {
            if (showcomm) {
                var frst = domFC(ge('mv_comments'))
                    .id || '';
                if (!isVisible('mv_comments_header') ||
                    (domFC(ge('mv_comments_header')) || {})
                    .tagName == 'IMG' ||
        ...
        commentClick: function(el, event, from) {
            var comm = el.id.replace('mv_comment', ''),
                cmnt = comm.split('_');
        ...
        commentTo: function(comm, toId, event) {
            var cmnt = (comm || '')
                .split('_'),
                commId = cmnt[1],
                replyNameOld = mvcur.mvReplyTo && mvcur.mvReplyNames[mvcur.mvReplyTo[0]] || '',
                replyName = mvcur.mvReplyNames[toId] || '',
                rf = ge('mv_comment'),
                tl = ge('mv_reply_to_title'),
                asGroup = ge('mv_reply_as_group');
            ...
            if (comm) {
                ...
                setStyle(tl, {
                    maxWidth: ge('mv_comment_submit')
                        .offsetWidth - domPN(ge('mv_comment_send'))
                        .offsetWidth - (asGroup ? (asGroup.offsetWidth + 10) : 0) - ge('mv_add_media')
                        .offsetWidth - 21
                });
            } else {
        ...
        sendComment: function(ev, stickerId) {
            ...
            if (stickerId) {
                ...
                var params = comp ? Composer.getSendParams(comp, Videoview.sendComment) : {
        ...
        likeUpdate: function(my, count, title, nott) {
            ...
            var icon = ge('mv_like_icon');
            var tt = (icon ? icon.parentNode.tt : false) || {},
                opts = clone(tt.opts || {}),
                newleft = (my ? 0 : -36);
            ...
            if (window.mvcur && mvcur.statusVideo) {
                ...
                if (wallCount) {
                    ...
                    (ge('like_real_count_wall' + mv.videoRaw) || {})
                    .value = count;
        ...
        showAddDialog: function(videoRaw) {
            ...
            setTimeout(function() {
                ...
                var addBox = showBox('/al_video.php', {
        ...
        likeShare: function(hash) {
            ...
            var el = ge('like_share_video' + mv.videoRaw),
                was = isChecked(el);
            checkbox(el);
            ajax.post('like.php', {
            ...
            ajax.post('al_video.php', {
                ...
                onDone: function(row) {
                    if (from == 'list' && cur.restoreRaw && cur.restoreRaw[oid + '_' + vid]) {
                        ...
                        setStyle(geByClass1('video_row_icon_delete', rowCont), {
                            opacity: 0.8
                        });
                        var skipClear = true;
                    } else {
                    ...
                    if (cur.claimedVideoText) {
                        ge('video_player')
                            .innerHTML = cur.claimedVideoText;
        ...
        deleteVideo: function(vid, oid, hash, sure, from, obj, callback) {
            ...
            ajax.post('al_video.php', {
                ...
                showProgress: obj ? addClass.pbind(obj, 'loading') : false,
                hideProgress: obj ? removeClass.pbind(obj, 'loading') : false
            });
        },
        deleteVideoOnClaim: function(vid, oid, hash, sure, from, obj) {
            Videoview.deleteVideo(vid, oid, hash, sure, from, obj, function(text) {
                if (from == 'videoviewer') {
                    ...
                    cur.claimedVideoText = ge('video_player')
                        .innerHTML;
                    ge('video_player')
                        .innerHTML = text;
                }
            ...
            ajax.post('al_video.php', {
                ...
                onDone: function(type, title, text, do_button, cancel_button) {
                    ...
                    if (type == 'sure') {
                        ...
                        if (from == 'list') {
                            ge('video_row' + oid + '_' + vid)
                                .innerHTML = '<div class="video_row">' + text + '</div>';
        ...
        confirmTag: function(tagId) {
            var actionCont = ge('mv_action_info');
            ge('mv_approve')
                .innerHTML = '<div style="text-align: center; padding-top: 4px;"><img src="/images/upload.gif"></div>';
            ajax.post('al_video.php', {
                act: 'confirm_tag',
                video: mvcur.mvData.videoRaw,
                tag_id: tagId,
        ...
        showVideo: function(title, html, js, desc, info, controlsLine, opt) {
            ...
            if (opt.pl_list && Object.keys) {
                ...
                if (plFullId.split('_')[1] == -2 /* system album all*/ ) {
                    if (Videocat.getCurrentPlaylistId()
            ...
            if (cur.mvComments[mvcur.videoRaw]) {
                domPN(cms)
                    .replaceChild(cur.mvComments[mvcur.videoRaw], cms);
            }
            ...
            if (mvcur.mvData.noControls) {
                ...
                if (viewsCountEl && viewsCountEl.textContent && viewsCountEl.textContent.indexOf('(') != -1) {
                    addEvent(viewsCountEl, 'mouseover', function() {
                        showTooltip(viewsCountEl, {
                            text: getLang('video_views_count_tt_text'),
                            black: true,
                            shift: [0, 5, 0]
                        });
                    });
        ...
        addToClubPlaylistBoxInit: function(box, clubs, hash) {
                ...
                ajax.post('/video', {
                    ...
                    onDone: function(playlists, added) {
                        ...
                        each(playlists, function(i, pl) {
                            playlistsHtml += '<div class="video_pl_cb" data-id="' + pl.id + '" data-title="' + clean(pl.title) + '" data-added="' + (+pl.added) +
                                '"></div>';
                        });
                        playlistsHtml += '<div>';
            ...
            var saveButton = box.removeButtons()
                .addButton(getLang('Save'), function(btn) {
                    if (!selectedGid) {
                        return;
                    }
                    var pl = [];
        ...
        setTitle: function(len) {
            ...
            len = len || 590;
            ge('mv_min_title')
                .innerHTML = Videoview._isCurrentVideoPublished() ? title : '';
            setStyle(ge('mv_min_title'), {
                maxWidth: Math.max(0, len - 60)
            });
        ...
        expandDescr: function(toggleEl) {
            ...
            ge('mv_descr_field')
                .innerHTML = fullTextEl.innerHTML;
        ...
        updateComposer: function() {
            ...
            if (cur.mvYourComment) {
                domPN(yc)
                    .replaceChild(cur.mvYourComment, yc);
        ...
        createDD: function(obj, items, callback) {
            ...
            if (!dd || !hasClass(dd, 'mv_controls_dd')) {
                var text = geByClass1('mv_dd_text', obj)
                    .innerHTML;
                var dd = se('<div onmouseover="videoview.showDD(ge(\'' + obj.id +
                    '\'), this);" onmouseout="videoview.hideDD(500);" onclick="videoview.hideDD(-1);" class="mv_controls_dd fixed">' +
                ...
                for (var i in items) {
                    var el = ce('a', {
                        className: 'mvs_act',
                        onclick: callback.pbind(items[i][0]),
                        innerHTML: '<span class="mvs_act_text">' + items[i][1] + '</span>'
                    });
                    acts.appendChild(el)
                }
        ...
        togglePlaylistsBlock: function(doShow, noAnim, userAction) {
            ...
            var isFlash = !!ge('video_player'),
                MV_TRANSITION_CLS = 'mv_player_dimensions_transition';
            ...
            if (userAction) {
                ajax.post('/al_video.php', {
                    act: 'a_videoview_stat',
                    action: doShow ? 'show_playlist_block' : 'hide_playlist_block'
                });
            }
        ...
        updatePlaylistBoxPosition: function() {
            ...
            var left = mvpos[0] + mvsize[0] - blockWidth,
                top = mvpos[1];
        ...
        editInline: function(ev, noreq) {
            if (((ev || window.event || {})
                    .target || {})
                .tagName == 'A' || !window.mvcur || mvcur.mvEditing || !ge('mv_description')) return;
            ...
            var onDone = function(text) {
                ...
                var el = ge('mv_description')
                    .appendChild(ce('div', {
                        innerHTML: '\
<div style="margin: ' + mrg + '">\
<textarea id="mv_edit_text"' + taStyle +
                            ' onkeydown="onCtrlEnter(event, videoview.saveInline)" onkeyup="checkTextLength(mvcur.mvCaptionLimit, this, ge(\'mv_caption_warn\'));" placeholder="' +
                            getLang('video_edit_desc_intro') + '">' + text + '</textarea>\
  <div id="mv_caption_warn"></div>\
        ...
        cancelInline: function() {
            ...
            show(ge('mv_description')
                .firstChild);
            re(ge('mv_description')
...
<a class="mv_finish_suggestions_item" onclick="videoCallback([\'' + suggestionsCallback + '\', \'' + item.vid +
                        '\']); return false;" href="//vk.com/video' + item.vid + '" title="' + item.title +
                        '">\
  <div class="mv_finish_suggestions_item_thumb" style="background-image: url(' + item.thumb +
                        ')"></div>\
  <div class="mv_finish_suggestions_item_title">' + item.title + '</div>\
  <div class="mv_finish_suggestions_item_views">' + item.views +
                        '</div>\
</a>\
      ';
                });
...
<div class="mv_external_finish" id="mv_external_finish">\
  <div class="mv_finish_header">\
    <div id="mv_finish_subscribe" class="fl_r mv_finish_subscribe ' +
                ...
                '">\
      <button id="mv_finish_subscribe_btn" class="mv_finish_subscribe_btn fl_l" onclick="Videoview.onExternalVideoSubscribe()">' + (isSubscribed ?
                    getLang('video_view_subscribed_msg') : getLang('video_view_subscribe_to_author')) + '</button>\
      <a href="' + mv.authorHref +
                '" target="_blank" class="fl_r"><img class="mv_finish_author_img" src="' + mv.authorPhoto +
                '"></a>\
    </div>\
    <div id="mv_finish_title" class="mv_finish_title" style="' + (isMinimized ? 'display:none' : '') + '">' + mv.title +
                '</div>\
  </div>\
  <div id="mv_finish_actions" class="mv_finish_actions ' + (extendedActions ? 'mv_finish_actions_extended' : '') + ' ' + (!canAdd ?
                    'mv_finish_actions_cant_add' : '') + ' ' + (minActions ? 'mv_finish_actions_no_content' : '') + '">\
    <div class="mv_finish_like ' + (isLiked ?
                    'selected' : '') +
                '" onclick="Videoview.onExternalVideoLike()">\
      <div class="mv_finish_like_icon mv_finish_icon"></div>\
      <div class="mv_finish_liked_icon mv_finish_icon"></div>\
      <div class="mv_finish_like_text">' +
    ...
    <div class="mv_finish_share" onclick="Videoview.onExternalVideoShare()">\
      ...
      <div class="mv_finish_share_text">' +
                getLang('video_share_with_friends') + '</div>\
    </div>\
    <div class="mv_finish_add ' + (isAdded ? 'selected' : '') +
                '" onclick="Videoview.onExternalVideoAdd()">\
