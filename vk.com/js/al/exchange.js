var Exchange = {
    ...
    searchCommunity: function() {
        ...
        ajax.post('/exchange?act=community_search' + (cur.post_id ? '&ad_id=' + cur.post_id : '&union_id=' + cur.union_id), query, {
            ...
            onDone: function(rows, showMore) {
                ...
                if (query['offset'] > 0) {
                    var tbl = ge('exchange_comm_search_table')
                        .tBodies[0];
                    if (rows) {
                        if (!browser.msie) {
    ...
    getPage: function(offset, wrap) {
        ...
        ajax.post(url, extend(obj, {
            ...
            onDone: function(res) {
                ge(wrap || 'exchange_requests_table_wrap')
                    .innerHTML = res;
    ...
    deleteRequest: function(gid, ad_id, request_id, from_office, hash, show_comment) {
        ...
        var box = showFastBox({
            ...
            getLang('ads_posts_delete') + '</button></div></div>', getLang('ads_posts_delete'), doDeleteRequest, getLang('global_cancel'));
        if (show_comment) {
            show('exchange_box_comment_wrap');
            placeholderSetup('exchange_box_comment', {
                back: true
            });
    ...
    editPost: function() {
        ...
        return wall.editPost(cur.postRaw, {
            from: 'exchange'
        }, false, Exchange.showFullPost);
    },
    showFullPost: function() {
        removeClass('exchange_post_msg_wrap', 'short');
        setStyle('exchange_post_msg', {
            maxHeight: 'none'
        });
    },
    slideFullPost: function() {
        ...
        animate(ge('exchange_post_msg'), {
            maxHeight: realSize
        }, 200, Exchange.showFullPost);
        animate(ge('exchange_post_msg_more'), {
            height: 0
        }, 200);
    },
    ...
    createUnion: function(btn, hash) {
        ajax.post('/exchange?act=a_new_union', {
            hash: hash
        }, {
            showProgress: lockButton.pbind(btn),
            hideProgress: unlockButton.pbind(btn),
    ...
    createClient: function(union_id, hash) {
        ...
        ajax.post('/exchange?act=a_create_client', {
            union_id: union_id,
            hash: hash,
            name: name
        }, {
    ...
    changePeriod: function(period, event) {
        ...
        delete params[0];
        ajax.post('/exchange', extend(params, {
            period: period,
            load: 1
        }), {
            cache: 1,
            ...
            onDone: function(rows, tabs) {
                ge('exchange_clients_list')
                    .innerHTML = rows;
                ge('exchange_stats_period_tabs')
                    .innerHTML = tabs;
            }
        });
    },
    openHelpBox: function(type, unionId) {
        showBox('/exchange?act=a_help_text_box', {
            type: type,
            union_id: unionId
        }, {
            params: {
    ...
    initExportStats: function(topUnionId, watchControls, union_created_date, hash) {
        ...
        Ads.getNamespace('exchange_export_stats')
    ...
    submitExportStatsForm: function() {
        ...
        var isExportMethodHtml = (Ads.getNamespace('exchange_export_stats')
