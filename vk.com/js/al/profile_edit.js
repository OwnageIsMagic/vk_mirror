var ProfileEditor = {
    ...
    showMsg: function(msg) {
        ge('pedit_result')
            .innerHTML = '<div class="msg" id="pedit_msg">' + msg + '</div>';
        show('pedit_result');
        animate(ge('pedit_msg'), {
    ...
    initGeneral: function() {
        ...
        var handlePartnerChange = function(value) {
            ...
            if (intval(value) && statusVal > 1 && cur.uiStatus.val() < 6) {
                if (intval(value) == peditData.partner) {
                    if (ge('pedit_status_detail_content')
                        .innerHTML) show('pedit_status_detail');
                } else {
                ...
                if (value > 1 && value < 6 || value == 7) {
                    ProfileEditor.updateFriendsList(function() {
                        ...
                        if (peditData.ptitles[value][cur.uiSex.val()].length) {
                            ge('pedit_status_ptitle')
                                .innerHTML = peditData.ptitles[value][cur.uiSex.val()] + ':' || '&nbsp;';
        ...
        if (peditData.sex) {
            ...
            if (peditData.status > 1 && peditData.status != 6) {
                if (peditData.ptitles[peditData.status][peditData.sex].length) {
                    ge('pedit_status_ptitle')
                        .innerHTML = peditData.ptitles[peditData.status][peditData.sex] + ':' || '&nbsp;';
                    show('pedit_status_ptitle');
                }
                show('pedit_status_partner');
            }
        ...
        cur.uiBday = new Dropdown(ge('pedit_bday'), ProfileEditor.generateDays(peditData.byear, peditData.bmonth)
            .slice(intval(peditData.bday) ? 1 : 0), {
                width: 55,
                multiselect: false,
    ...
    initContacts: function() {
        ...
        ge('pedit_email')
            .value = cur.options.email;
        ge('pedit_mobile')
            .value = cur.options.mobile;
        ge('pedit_home')
            .value = cur.options.home;
        ge('pedit_website')
            .value = cur.options.website;
        ge('pedit_skype')
            .value = cur.options.skype;
        ...
        switch (section) {
            ...
            case 'contacts':
                ...
                return ge('pedit_email')
                    .value != peditData.email ||
                    ge('pedit_mobile')
                    .value != peditData.mobile ||
                    ge('pedit_home')
                    .value != peditData.home ||
                    ge('pedit_website')
                    .value != peditData.website ||
                    ge('pedit_skype')
                    .value != (peditData.skype || '') ||
                    (parseInt(ge('pedit_country')
                        .value) || '') != (cur.selectData['country_val'][0] || '') ||
                    (parseInt(ge('pedit_city')
            ...
            case 'interests':
                ...
                each(['activities', 'interests', 'music', 'movies', 'tv', 'books', 'games', 'quotes', 'about'], function(k, field) {
                    ...
                    result |= (cur.interests_data[field].replace(/\x0d/g, '') != val(el)
                        .replace(/\x0d/g, ''));
                });
                return result;
                break;
            ...
            case 'military':
                ...
                for (var i = 0; i < cur.militaries.length; ++i) {
                    ...
                    } else if (cur.militaries[i].id < 0 && isVisible('content' + cur.militaries[i].id) && intval(ge('unit' + cur.militaries[i].id)
                            .value)) {
                        military_change = true;
                    }
                }
    ...
    checkChanges: function(showBox) {
        ...
        if (message) {
            return winToUtf(message.replace(/<\/?b>/g, '')
                .replace(/<br\s*\/?>/g, '\n'));
        }
    },
    ...
    updateDays: function(year, month) {
        ...
        cur.uiBday.setData(ProfileEditor.generateDays(year, month)
            .slice(intval(cur.options.bday) ? 1 : 0));
        if (cur.onUpdateDays) {
    ...
    doUpdateFriendsList: function() {
        ...
        data[gs][0][1] = getLang((status == 5) ? 'profileEdit_main_sel_rel_with_m' : (gs ? 'profileEdit_main_sel_relat' : 'profileEdit_main_sel_rel_with_' + (sex == 1 ?
            'm' : 'f')));
    ...
    onFriendsLoaded: function(type, selected) {
        ...
        var name = type + cur.relGuid,
            id = 'pedit_' + name;
        var html = '<div class="pedit_relation_input"><div class="fl_l"><input type="hidden" id="' + id + '" name="' + name +
            '"/></div><div class="fl_r" style="padding-top: 8px;"><a href="" onclick="ProfileEditor.removeRelation(\'' + type + '\', ' + cur.relGuid + ');return false;">' +
            getLang('global_delete') + '</a></div>';
        var t = 0;
        if (withDate) {
            t = selected ? parseInt(selected[3]) : 0;
            html += '<div class="pedit_relation_date clear_fix clear"><input class="pedit_date_field" type="hidden" id="' + id + '_date" value="' + t +
                '"><div class="pedit_bday fl_l"><input type="hidden" id="' + id + '_day"/></div><div class="pedit_bmonth fl_l"><input type="hidden" id="' + id +
                '_month"/></div><div class="pedit_byear fl_l"><input type="hidden" id="' + id + '_year"/></div></div>';
        }
        wrap.innerHTML = html;
        ge(cur.conts[type])
            .appendChild(wrap);
    ...
    switchUnits: function() {
        ...
        ge('pedit_units_toggler')
            .innerHTML = curMeasure == 1 ? getLang('profileEdit_switch_to_meters') : getLang('profileEdit_switch_to_inches');
        ge('pedit_height_title')
            .innerHTML = curMeasure == 1 ? getLang('profileEdit_height_in') : getLang('profileEdit_height_cm');
        ge('pedit_weight_title')
            .innerHTML = curMeasure == 1 ? getLang('profileEdit_weight_lbs') : getLang('profileEdit_weight_kg');
    },
        ...
        if (peditData.family !== false) {
            ...
            each(geByClass('pedit_relation_input', ge('pedit_grandparents')), function() {
                ...
                if (params['grandparents[' + i + ']'] == -1 && params['grandparents_custom[' + i + ']']) {
                    ...
                    var n = name.split(/\s+/);
                    params['grandparents_custom[' + i + ']'] = cleanName(n[0] || '', n[1] || '')
                        .join(' ');
                }
                i++;
            });
            ...
            each(geByClass('pedit_relation_input', ge('pedit_parents')), function() {
                ...
                if (params['parents[' + i + ']'] == -1 && params['parents_custom[' + i + ']']) {
                    ...
                    var n = name.split(/\s+/);
                    params['parents_custom[' + i + ']'] = cleanName(n[0] || '', n[1] || '')
                        .join(' ');
                }
                i++;
            });
            ...
            each(geByClass('pedit_relation_input', ge('pedit_siblings')), function() {
                ...
                if (params['siblings[' + i + ']'] == -1 && params['siblings_custom[' + i + ']']) {
                    ...
                    var n = name.split(/\s+/);
                    params['siblings_custom[' + i + ']'] = cleanName(n[0] || '', n[1] || '')
                        .join(' ');
                }
                i++;
            });
            ...
            each(geByClass('pedit_relation_input', ge('pedit_children')), function() {
                ...
                if (params['children[' + i + ']'] == -1 && params['children_custom[' + i + ']']) {
                    ...
                    var n = name.split(/\s+/);
                    params['children_custom[' + i + ']'] = cleanName(n[0] || '', n[1] || '')
                        .join(' ');
                }
                i++;
            });
            ...
            each(geByClass('pedit_relation_input', ge('pedit_grandchildren')), function() {
                ...
                if (params['grandchildren[' + i + ']'] == -1 && params['grandchildren_custom[' + i + ']']) {
                    ...
                    var n = name.split(/\s+/);
                    params['grandchildren_custom[' + i + ']'] = cleanName(n[0] || '', n[1] || '')
                        .join(' ');
                }
                i++;
            });
    ...
    saveContacts: function(btn) {
        ...
        for (var i in fields) {
            params[fields[i]] = ge('pedit_' + fields[i])
                .value;
        }
            ...
            ajax.post('/al_profileEdit.php', {
                ...
                onDone: function(result) {
                    if (result.msg) {
                        ...
                        ge('export_service_' + service)
                            .innerHTML = (result.msg_sett) ? result.msg_sett : result.msg;
                ...
                onDone: function(result) {
                    ...
                    if (result.msg) {
                        ...
                        if (result.error == 1) {
                            ...
                            ge('pedit_lj_error')
                                .innerHTML = cur.lang['settings_lj_wrong_password'];
                            show('pedit_lj_error');
                        }
                    }
                }
    ...
    selectEduTab: function(tab, isInit) {
        ...
        if (tab == 'education') {
            cur.filter_uni = 0;
            removeClass(ge('sum_tab_schools')
                .parentNode, 'summary_tab');
            addClass(ge('sum_tab_schools')
                .parentNode, 'summary_tab_sel');
            removeClass(ge('sum_tab_unis')
                .parentNode, 'summary_tab_sel');
            addClass(ge('sum_tab_unis')
                .parentNode, 'summary_tab');
            show('school');
            hide('uni');
            hide('pedit_result');
            ...
            ge('summary')
                .innerHTML = getLang('profileEdit_can_edit_schools');
            ...
            if (!isVisible('schools')) {
                ...
                if (cur.schoolsCount) {
                    for (var i = 0; i < cur.schools.length; ++i) {
                        ge('schools')
                            .appendChild(ProfileEditorEdu.genSchoolRow(cur.schools[i].id));
            ...
            ge('summary')
                .innerHTML = getLang('profileEdit_can_edit_unis');

            if (!isVisible('primary_uni')) {
                ge('primary_uni')
                    .appendChild(ProfileEditorEdu.genUniRow(cur.primary_uni.id, true));
                ...
                if (cur.unisCount) {
                    for (var i = 0; i < cur.unis.length; ++i) {
                        ge('unis')
                            .appendChild(ProfileEditorEdu.genUniRow(cur.unis[i].id));
