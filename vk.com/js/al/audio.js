var Audio = {
    ...
    init: function(obj, audioTpl) {
        ...
        ajax.post(Audio.address, query, {
            onDone: (function(data, opts) {
                    ...
                    cur.sectionCount = (cur.audiosList[cur.curList] || [])
                        .length;
                    if (cur.canEdit && nav.objLoc.act != 'popular' && nav.objLoc.act != 'recommendations' && nav.objLoc.act != 'feed' && !browser.mobile && cur
                        .allAudiosIndex == 'all' && !cur.shuffled && !cur.club) {
                        if (cur.sectionCount) {
                            var opts = {
                                onReorder: Audio.onAudioReorder,
                                onMouseDown: Audio.onDragStart,
                                onMouseUp: Audio.onDragEnd,
                                noMoveCursor: 1
                            };
                            if (cur.audioAlbumsWrap) {
                                extend(opts, {
                                    target: cur.audioAlbumsWrap,
                                    onDragOver: Audio.onDragOver,
                                    onDragOut: Audio.onDragOut
                                });
                            }
                        ...
                        callback: function() {
                            ...
                            if (!cur.q || cur.club) {
                                if (cur.curSection == 'recommendations' || cur.curSection == 'popular' || cur.curSection == 'feed') {
                                    ...
                                    audioPlayer.genPlaylist(cur.audiosList[cur.curList], false, {
                                        my: (cur.curList == 'all' && cur.oid == vk.id ? 'my' : false)
                                    });
                                }
                            } else if (!query.club) {
    ...
    startEvents: function() {
        ...
        if (Audio.fixedScroll) {
            ...
            setStyle(cur.audioEl.bar, {
                top: headH + headT
            });
            setStyle(cur.audioEl.cont, {
                paddingTop: contentY
            });
            setStyle(cur.audioEl.filters, {
                top: contentY
            });
    ...
    showRows: function(opts) {
        ...
        var _a = window.audioPlayer;
        if (!list || !list.length) {
            if (__cur.shownAudios == 0 && (__cur.album_id || (!Audio.allAudios(from_pad)
                    .length && !__cur.searchStr))) {
                var msg;
                if (Audio.allAudios(from_pad)
                    .length) {
                    msg = (__cur.album_id) ? getLang('audio_album_no_recs') : getLang('audio_no_audios_found')
                        .split('{query}')
                        .join('<b>' + cur.searchStr.replace(/([<>&#]*)/g, '') + '</b>');
                } else {
                    if ((__cur.oid > 0 || from_pad) && !(__cur.audioFriend && __cur.allAudiosIndex !== 'all')) {
                        if ((__cur.id == vk.id || from_pad) && __cur.allAudiosIndex == 'all') {
                            msg = getLang('audio_no_rec_load_msg')
                                .split('{link}')
                                .join('<a href="#" onclick="Audio.addAudio(); return false">')
                                .split('{/link}')
                                .join('</a>');
                        } else {
                ...
                var style = from_pad && window._pads && _pads.content ? ' style="height: ' + (getSize(_pads.content)[1] - 110 - ge('pad_playlist')
                    .offsetTop) + 'px;"' : '';
            ...
            if (from_pad && window.Pads && Pads.updateHeight) {
                Pads.updateHeight();
            } else if (__cur.canEdit && nav.objLoc.act != 'popular' && nav.objLoc.act != 'recommendations' && nav.objLoc.act != 'feed' && !browser.mobile && !__cur.searchStr &&
                __cur.sectionCount && __cur.allAudiosIndex == 'all' && !__cur.shuffled && !__cur.club) {
                if (start > 0) {
        ...
        var aid = currentAudioId(),
            clubList = (__cur.curSection && __cur.curSection.substr(0, 4) == 'club');
    ...
    toggleSearchFriends: function(show, el) {
        var inpWrap = el.nextSibling,
            inp = geByTag1('input', inpWrap);
    ...
    searchFriends: function(opts) {
        ...
        var __cur = opts.from_pad ? window._pads && _pads.cur : window.cur,
            from_pad = !!opts.from_pad,
            str = trim(val(__cur.friendsSearch)),
            inp = opts.el,
            event = opts.event;
        ...
        this.searchFTimeout = setTimeout((function() {
                Audio.searchFriendsRequest({
                    from_pad: from_pad,
                    str: __cur.searchFriendsStr
                });
            })
            .bind(this), 150);
    },
        ...
        ajax.post(Audio.address, query, {
            onDone: function(cont, friends) {
                ...
                if (ge(__cur.audioMoreFriensId)) {
                    toggle(ge(__cur.audioMoreFriensId)
                        .parentNode, !str);
                }
                if (from_pad) {
                    Audio.updatePadFiltersHeight();
    ...
    searchRequest: function(opts) {
        ...
        var query = {
            ...
            id: __cur.id,
            gid: __cur.gid,
            performer: __cur.searchTypeMenu ? __cur.searchTypeMenu.val() : 0
        };
        if (!from_pad) {
            ...
            onDone: function(res, preload, options) {
                ...
                if (!__cur.sectionCount) {
                    ...
                    if (!__cur.searchCount && !res && !preload) {
                        msg = getLang('audio_no_audios_found')
                            .split('{query}')
                            .join('<b>' + __cur.searchStr.replace(/([<>&#]*)/g, '') + '</b>');
                        var style = from_pad && window._pads && _pads.content ? ' style="height: ' + (getSize(_pads.content)[1] - 110 - ge('pad_playlist')
                            .offsetTop) + 'px;"' : '';
                        __cur.aContent.innerHTML = '<div id="not_found" class="info_msg"' + style + '>' + msg + '</div>';
                        hide(__cur.showMore);
    ...
    handleFilterPos: function(force) {
        ...
        var headH = cur.audioEl.head.clientHeight,
            audioNavH = cur.audioEl.bar.offsetHeight,
            contentY = headH + audioNavH,
            st = Math.max(0, scrollGetY()),
            wh = window.lastWindowHeight || 0,
            pos = 0,
            filt = ge('side_panel'),
            filtPos = getXY(filt)[1],
            filtY = getSize(filt)[1],
            sf = ge('side_filters'),
            sfPos = (getStyle(sf, 'position') == 'fixed') ? parseInt(getStyle(sf, 'top')) : getXY(sf)[1],
            sfY = getSize(sf)[1],
            bottomPad = Math.max(0, st + wh - filtY - contentY),
            tooBig = (filtPos + filtY - sfPos - sfY < 20),
            lastPos = cur.filterLastPos || 100,
            lastSt = cur.lastSt || 0;

        if (!tooBig) {
            addClass(sf, 'fixed');
            pos = (wh > sfY + contentY || bottomPad > 0) ? Math.min(contentY, wh - sfY - bottomPad) : Math.max(Math.min(contentY, lastPos + lastSt - st), wh - sfY -
                bottomPad);
        } else {
        ...
        if (!browser.mozilla && !browser.msie && (cur.lastWW !== lastWindowWidth || force)) {
            ...
            cur.audioEl.head.style.left = ge('side_bar')
    ...
    updateList: function(opts) {
        ...
        this.filterTimeout = setTimeout((function() {
                ...
                var c = Audio.allAudios(from_pad)
                    .length;
                if (str) {
    ...
    selectPerformer: function(opts) {
        ...
        var __cur = opts.from_pad ? window._pads && _pads.cur : window.cur,
            from_pad = !!opts.from_pad,
            event = opts.event,
            name = replaceEntities(opts.name || '');
    ...
    searchAudios: function(opts) {
        ...
        if (str) {
            ...
            __cur.audiosList[__cur.curList] = res.sort(function(a, b) {
                return a._order - b._order
            });
            ...
            if (str) {
                ...
                str = trim(str.replace(/\)/g, '')
                    .replace(/&/, '&amp;'));
                __cur.selection = {
                    re: new RegExp('(\\s|^)(' + str.replace(__cur.audiosIndex.delimiter, '|')
                        .replace(/(^\||\|$|\?)/g, '') + ')', 'gi'),
                    val: '$1<span>$2</span>'
                };
            }
            ...
            audioPlayer.genPlaylist(__cur.audiosList[__cur.curList], false, {
                my: (__cur.curList == 'all' && (__cur.oid == vk.id || from_pad) ? 'my' : false),
                htitle: __cur.justShuffled && pl && pl.htitle
            });
    ...
    changeAllIndex: function(opts) {
        ...
        this.indexAll({
            ...
            callback: opts.clbk || function() {
                ...
                audioPlayer.genPlaylist(__cur.audiosList[__cur.curList], from_pad && index == 'all' && !currentAudioId(), {
                    my: (index == 'all' && (__cur.oid == vk.id || from_pad) ? 'my' : false)
                });
    ...
    changeHTitle: function(opts) {
        ...
        if (__cur.curSection && !__cur.curSection.indexOf('album')) {
            ...
            htitle = getLang('audio_title_search')
                .replace('{q}', clean(val(__cur.aSearch)));
            pad_htitle = getLang('audio_title_search')
                .replace('{q}', '<b>' + clean(val(__cur.aSearch)) + '</b>');
        ...
        this.qTimeout = setTimeout(function() {
            if (cur.allAudiosIndex != 'all') {
                if (cur.allAudiosIndex && cur.allAudiosIndex.substr(0, 5) == 'owner') {
                    extend(nav.objLoc, {
                        owner: cur.audioFriend
                    });
                } else {
                    extend(nav.objLoc, {
                        friend: cur.audioFriend
                    });
                }
            } else {
    ...
    animateAdded: function(el, speed) {
        var c = se(
            '<div class="audio_add_wrap anim fl_r"><div class="audio_add_anim_wrap"><div class="audio_add_anim clear_fix"><div class="audio_add fl_l"></div><div class="audio_add done fl_l"></div></div></div></div>'
        );
        el.parentNode.replaceChild(c, el);
        el = c;
        var anim = geByClass1('audio_add_anim', el),
            add = anim.firstChild,
            added = add.nextSibling;
        animate(anim, !vk.rtl ? {
            left: '-15px'
        } : {
            right: '-15px'
    ...
    addShareAudio: function(opts) {
        ...
        var __cur = opts.from_pad ? window._pads && _pads.cur : window.cur,
            ...
            gid = opts.gid,
            top = opts.top || '',
            from_choose = opts.from_choose,
            from_wall = opts.from_wall,
            ev = opts.ev;
            ...
            onDone: function(data, res) {
                            ...
                            if (tip && !tip.inited) {
                                ...
                                addEvent(a, 'click', function() {
                                    ...
                                    var share_q = {
                                        act: 'share_audio',
                                        audio: res.audio,
                                        status: tip.status ? tip.status : 0,
                                        check: hasClass(a, 'on') ? 1 : 0,
                                        hash: res.hash
                                    };
                ...
                if (from_choose) {
                    if (cur.module != 'audio') {
                        ...
                        __cur.ignoreEqual = true;
                        setTimeout(Audio.clearSearch.pbind({
                            from_pad: from_pad
                        }), 0);
                    }
                }
    ...
    deleteAddedAudio: function(opts) {
        ...
        var el = opts.el,
            ...
            oid = id.split('_')[0],
            aid = id.split('_')[1],
            added_oid = opts.added_oid,
            added_aid = opts.added_aid,
            hash = opts.hash;
        if (!el) {
            return false;
        }
    ...
    loadAlbum: function(opts) {
        ...
        if (!album_id && !showAlbums && (__cur.oid == vk.id) && isVisible(__cur.audioMoreFriensId) && !__cur.searchFriendsStr) {
            __cur.shownFriends = [];
            Audio.showMoreFriends({
                from_pad: from_pad,
                onShow: addClass.pbind(ge((from_pad ? 'pad_' : '') + 'album0'), 'loading'),
                onHide: removeClass.pbind(ge((from_pad ? 'pad_' : '') + 'album0'), 'loading')
            });
        }
        hide(__cur.searchInfoCont);
        if (__cur.allAudiosIndex != 'all') {
        ...
        if (__cur.audioWrap) {
            toggleClass(__cur.audioWrap, 'audio_no_recs', !Audio.allAudios(from_pad)
                .length && __cur.allAudiosIndex == 'all' && __cur.oid <= 0);
        }
        if (filter) {
        ...
        if (__cur.oid == vk.id || from_pad) {
            ajax.post(Audio.address, {
                act: 'list_stats',
                albums: (album_id || showAlbums) ? 1 : 0
            });
        }
    },
    ...
    toggleAlbums: function(opts) {
        ...
        var __cur = opts.from_pad ? window._pads && _pads.cur : window.cur,
            from_pad = !!opts.from_pad,
            down = isVisible(__cur.audioAlbums),
            icon = __cur.albumsFilter && geByClass1('icon', __cur.albumsFilter);
        if (!from_pad) {
            Audio.updateAlbumsStart();
        }
    ...
    addAudio: function(params, event) {
        if (cur.uploadBanned) {
            setTimeout(showFastBox({
                    title: getLang('audio_no_upload_title'),
                    bodyStyle: 'padding: 20px; line-height: 160%;',
                    dark: 1
                }, getLang('audio_claims_no_upload'))
                .hide, 5000);
            if (event) cancelEvent(event);
            return false;
        }
        showBox(Audio.address, extend(params || {}, {
            act: 'new_audio',
            gid: cur.gid
        }), {
            params: {
    ...
    mixAllAudios: function(opts) {
        ...
        var all_list = Audio.allAudios(from_pad),
    ...
    loadRecommendations: function(opts) {
        ...
        if (__cur.preloadJSON) {
            ...
            var aid = currentAudioId(),
                needs_update = __cur.recsOffset && (aid && __cur.recommendIds && indexOf(__cur.recommendIds, aid) != -1);
        ...
        ajax.post(Audio.address, query, {
            onDone: function(rows, preload, json, preload_json, options, ownersRows) {
                ...
                if (json) {
                    ...
                    var aid = currentAudioId(),
                        needs_update = query.offset && (aid && __cur.recommendIds && indexOf(__cur.recommendIds, aid) != -1);
                    audioPlayer.genPlaylist(__cur.recommendAudios, needs_update);
                    if (query.audio_id) {
    ...
    loadPopular: function(opts) {
        ...
        if (__cur.preloadJSON) {
            ...
            var aid = currentAudioId(),
                needs_update = __cur.popularOffset && (aid && __cur.popularIds && indexOf(__cur.popularIds, aid) != -1);
        ...
        var offset = __cur.popularOffset,
            query = {
                act: 'get_popular',
                offset: offset
            },
        ...
        ajax.post(Audio.address, query, {
            onDone: function(rows, preload, json, preload_json, options, genres) {
                ...
                if (json) {
                    ...
                    var aid = currentAudioId(),
                        needs_update = query.offset && (aid && __cur.popularIds && indexOf(__cur.popularIds, aid) != -1);
                    audioPlayer.genPlaylist(__cur.popularAudios[genre], needs_update);
                }
                removeClass(__cur.showMore, 'loading');
                if (offset) {
            ...
            onDone: function(rows, script) {
                ...
                if (rows) {
                    if (update) {
                        ...
                        var au = ce('div'),
                            par = geByClass1('wall_module', __cur.aContent),
                            row;
                        au.innerHTML = rows;
                        if (par && update) {
                            par.innerHTML = '';
                ...
                var aid = currentAudioId(),
                    needs_update = query.offset && (aid && __cur.feedIds && indexOf(__cur.feedIds, aid) != -1 && window.audioPlaylist && audioPlaylist.address &&
                        audioPlaylist.address.indexOf('act=feed') != -1);
    ...
    reorderPlaylist: function(aid, before_id, after_id) {
        each([window.audioPlaylist, cur.nextPlaylist], function(i, e) {
            ...
            if (list && list[aid] && after_id && list[after_id] && after_id != aid) {
                var next_id = list[aid]._next,
                    prev_id = list[aid]._prev;
                if (next_id != aid) {
    ...
    hideRecommendation: function(id, q, hash, event) {
        ...
        if (el) {
            ...
            var title_wrap = el && geByClass1('title_wrap', el),
                performer = clean(title_wrap && (geByTag1('a', title_wrap) || {})
                    .innerHTML) || '',
                title = clean(title_wrap && (geByClass1('lyrics_link', title_wrap) || geByClass1('title', title_wrap) || {})
                    .innerHTML) || '';
    ...
    selectCommunity: function(opts, ev) {
        ...
        mentionOver(ge((from_pad ? 'pad_' : '') + 'audio_community' + id), {
            shift: [47, 7, 7]
        });
        each(geByClass('audio_filter', __cur.albumFilters), function(i, e) {
            removeClass(e, 'selected');
    ...
    playCurrent: function(el, hash, ev) {
        var _a = window.audioPlayer,
            aid = currentAudioId(),
            audioId = el.getAttribute('data-audio');
        ...
        var prg = el.nextSibling || el.parentNode.appendChild(ce('span', {
            className: 'progress_inline current_audio_prg'
        }));
        stManager.add(['audioplayer.css', 'audioplayer.js'], ajax.post.pbind(Audio.address, {
    ...
    loadFriendsAudios: function(opts) {
        ...
        if (!__cur.audiosList[index]) {
            ...
            if (index != 'all' && !ge((from_pad ? 'pad_' : '') + 'audio_friend' + id) && !owner) {
                __cur.shownFriends = [];
                var txt = domFC(ge(__cur.audioMoreFriensId)),
                    prg = domLC(ge(__cur.audioMoreFriensId));
                Audio.showMoreFriends({
                    from_pad: from_pad,
                    onShow: function() {
                        hide(txt);
    ...
    showMoreFriends: function(opts) {
        ...
        var __cur = opts.from_pad ? window._pads && _pads.cur : window.cur,
            from_pad = !!opts.from_pad,
            friend = opts.friend,
            onShow = opts.onShow || function() {},
            onHide = opts.onHide || function() {};
        ...
        ajax.post(Audio.address, query, {
            onDone: function(cont, friends, reset) {
                ...
                if (cont) {
                    ...
                    var old = ce('div', {
                            innerHTML: '<div id="audio_friends_old" style="position: absolute; top: ' + list.offsetTop + 'px">' + list.innerHTML + '</div>'
                        })
                        .firstChild;
                    ...
                    list.innerHTML = cont;
                    var oldRows = geByClass('audio_friend', old),
                        oldCnt = oldRows.length,
                        newCnt = geByClass('audio_friend', list)
                        .length;
                    if (oldCnt > newCnt) {
    ...
    showAudioClaimWarning: function(aid, claim_id, title, reason) {
        ...
        if (reason == 'crap') {
            claimText = getLang(claim_id >= 0 ? 'audio_crap_warning_text' : 'audio_crap_warning') || getLang(claim_id > 0 ? 'audio_claim_warning_objection' : (claim_id ==
                0 ? 'audio_claim_warning_text' : 'audio_claim_warning'));
        ...
        claimText = claimText.split('{audio}')
            .join('<b>' + title + '</b>');
        claimText = claimText.split('{objection_link}')
            .join('<a href="/help?act=cc_objection&claim=' + claim_id + '&content=audio' + aid + '">' + getLang('audio_claim_objection') + '</a>');
        claimText = claimText.split('{delete_link}')
            .join('<a href="#" onclick="Audio.deleteAudio(\'' + aid + '\'); return false;">' + getLang('audio_claim_delete') + '</a>');
    ...
    deleteAudio: function(id) {
        ...
        var a = (cur.audios || {})[aid] || [],
            lyrics = isVisible(ge('lyrics' + id)),
            _tw = el && geByClass1('title_wrap', el),
            performer = a[5] || _tw && (geByTag1('a', _tw) || {})
            .innerHTML || '',
            title = a[6] || _tw && (geByClass1('title', _tw) || {})
            .innerHTML || '';
        el.innerHTML = rs(cur.deletedTpl, {
            audio_id: id,
            performer: performer.split('<span>')
                .join('')
                .split('</span>')
                .join(''),
            title: title,
    ...
    deleteAll: function(object_id, from_id, to_id, hash) {
        var box = showFastBox({
            ...
            dark: 1
        }, getLang('audio_delete_all') || '', getLang('global_delete'), function(btn) {
            ajax.post(Audio.address, {
                act: 'delete_all',
                object_id: object_id,
                to_id: to_id,
    ...
    updatePadFiltersHeight: function() {
        var filters = ge('pad_side_filters'),
            td = ge('pad_filters_panel'),
            curSz = getSize(td)[1],
            minSz = getSize(_pads.content)[1],
    ...
    initAlbumsSort: function() {
        if (!cur.canEdit || browser.mobile || !cur.audioAlbumsWrap || cur.audioAlbumsWrap.sorter) return;
        sorter.init(cur.audioAlbumsWrap, {
            onMouseDown: Audio.hideAlbumsTT,
            onReorder: Audio.onAlbumReorder,
            noMoveCursor: 1
        });
    },
    ...
    editAlbum: function(aid) {
        ...
        var box = showTabbedBox(Audio.address, {
            act: 'edit_album_box',
            album_id: aid,
            oid: cur.oid
        }, {
    ...
    saveAlbum: function(box, aid, audio_ids) {
        ...
        var query = {
            ...
            name: albumName,
            gid: cur.gid,
            Audios: audio_ids.join(','),
            hash: cur.hashes.save_album_hash
        };
        ajax.post(Audio.address, query, {
            showProgress: lockButton.pbind(btn),
    ...
    deleteAlbum: function(aid, hash) {
        var box = showFastBox({
            title: getLang('audio_delete_album_title'),
            dark: 1
        }, getLang('audio_delete_album_are_you_sure'), getLang('audio_delete_album_button'), function(btn) {
            ajax.post(Audio.address, {
                act: 'delete_album',
                album_id: aid,
                hash: hash,
    ...
    onAudioReorder: function(audio, before, after) {
        var aid = audio.id.replace('audio', '')
            .split('_')[1];
        var before_id = (before && before.id || '')
            .replace('audio', '')
            .split('_')[1];
    ...
    onDragStart: function(el) {
        ...
        cur.dragStartTimer = setTimeout(function() {
            ...
            hide('album_add');
            var ids = el.id.substr(5),
                id = parseInt(ids.split('_')[1]);
    ...
    onDragEnd: function(el, target) {
        ...
        show('album_add');
        if (target) {
            var new_album, full_id = el.id.substr(5),
                aid = parseInt(full_id.split('_')[1]);
    ...
    onDragOut: function(el, target) {
        ...
        cur.dragOutTimer = setTimeout(function() {
            setStyle(el, {
                opacity: .8
            });
        }, 200);
    ...
    moreCatalog: function(opts) {
        ...
        if (hasClass(obj, 'audio_performer_shown')) {
            ...
            var exclude = [];
            var nodes = ge(__cur.popularPerformersId)
                .childNodes;
            for (var i in nodes) {
