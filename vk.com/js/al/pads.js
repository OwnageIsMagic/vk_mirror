var Pads = {
    ...
    getCache: function(id, set) {
        var c = _pads.cache[id],
            t = vkNow();
        if ((c || {})
            .tim && (t - c.tim) < 86400000 || (t - intval(c)) < 20000) return c;
    ...
    reposition: function() {
        ...
        setStyle(_pads.wrap, {
            left: getXY(ge('l_' + _pads.shown), true)[0] + (vk.rtl ? (-Pads.coords[_pads.shown].w - 7) : 143)
        });
    },
    getEl: function(id) {
        var el;
    ...
    setPos: function(id) {
        var d = Pads.coords[id],
            ...
            topOffset = 33 + barHeight,
            topPad = getSize(ge('page_header'))[1] + 6;
        if (id == 'mus') {
            var w3 = ge('wrap3'),
                posStyle = getStyle(ge('page_header'), 'position'),
                y = scrollGetY(),
                playBtn = ge('head_play_btn'),
                gp = ge('gp'),
                reverse = isVisible(gp) && hasClass(gp, 'reverse'),
                fixed = (posStyle == 'fixed' || posStyle != 'fixed' && y >= topOffset && isVisible(gp) || reverse || !ge('head_play_btn')),
                l, r, t, b;
            ...
            if (!fixed && !reverse && browser.mac) { // :(
                setTimeout(function() {
                    ...
                    setTimeout(function() {
                        _pads.wrap.style.position = '';
                        _pads.wrap.style.top = nt + 'px';
                    }, 0);
                }, 0);
            }
            ...
            var c = {},
                wasvis = isVisible(_stlSide),
                obj = Pads.coords[id];
            if (!wasvis) show(_stlSide);
            c.fixpos = intval((_fMenuLnks[id] || {})
                .offsetTop + (_fMenu || {})
                .offsetTop);
    ...
    onScroll: function(cacheFix, id) {
        ...
        if ((id || _pads.shown) == 'mus') {
            ...
            if (reverse) {
                ...
                setStyle(_pads.wrap, {
                    bottom: fixed ? '15px' : 'auto',
                    top: fixed ? 'auto' : topPad + barHeight
                });
                toggleClass(_pads.wrap, 'fixed', fixed);
                toggleClass(_pads.wrap, 'pad_no_fixed', !fixed);
            ...
            if (_fixedNav) y = 0;
            var top = _pads.cur.fixtop,
                arrtop = _pads.cur.basearrtop - y,
                cf = (cacheFix === true);
    ...
    show: function(id, ev) {
        ...
        if (id == 'game' || id == 'mus' && !_pads.gpClicked) {
            ...
            if (loadEl) {
                if (c) {
                    ...
                    loadEl.innerHTML = '<img style="position: absolute; margin-top: 3px; margin-left: ' + ((s[0] + wDiff) / 2 - 16) + 'px;" src="/images/upload_inv' + (
                        window.devicePixelRatio >= 2 ? '_2x' : '') + '.gif" width="32" height="8"/>&nbsp;';
                }
            }
        ...
        var row = domFC(ge('pad_rows')),
            needClean = (id == 'nws' && vk.counts[id] > 0);
    ...
    deinit: function() {
        ...
        if (((_pads.cur.tipEl || {})
                .tt || {})
            .destroy) _pads.cur.tipEl.tt.destroy();
        _pads.cur = 0;
    },
    hide: function() {
    ...
    handleFilterPos: function() {
        var filters = ge('pad_side_filters'),
            filtersWrap = ge('pad_side_filters_wrap'),
            lastPos = _pads.cur.filterLastPos || 0,
            lastSt = _pads.cur.lastSt || 0,
            pos,
            st = _pads.content.scrollTop,
            fSz = getSize(filters)[1],
            fwSz = getSize(filtersWrap)[1];
    ...
    scroll: function() {
        if (_pads.shown == 'mus') {
            var more = ge('pad_more_audio'),
                searchMore = ge('pad_more_search_link');
    ...
    showTip: function(id, over) {
        var place = geByClass1('pad_' + _pads.shown + '_btns'),
            el = _pads.cur.tipEl;
        ...
        showTooltip(el, {
            ...
            hasover: 1,
            onCreate: function() {
                var els = Array.prototype.slice.apply(geByClass('radiobtn', ge('pad_submit_hint_opts'))),
                    val = _pads.cur.subm ? 1 : 0;
                addClass(els[val], 'on');
    ...
    updateHeight: function(id) {
        var el = ge('pad_rows'),
            lh = intval(window.lastWindowHeight);
        if (!el) {
            _pads.restrict = [0, (_pads.height = _pads.wrap.offsetHeight)];
            return;
        }

        var wh, ct = _pads.content.offsetTop,
            oh = _pads.content.offsetHeight;
    ...
    frDone: function(uid, fail, text) {
        ...
        var el = ge('pad_fr' + uid),
            btns = geByClass1('pad_fr_btns', el) || geByClass1('pad_fr_result', el);
        domPN(btns)
            .replaceChild(ce('div', {
                innerHTML: text,
    ...
    frPhotoOver: function(el, uid) {
        ...
        if (!_pads.frPhotoCache) extend(_pads, {
            frPhotoCache: {},
            frPhotoShown: {}
        });
        ...
        if (o.tagName != 'A' || o.className != 'pad_fr_phbig') {
            ...
            .onclick = Pads.frPhotoClick.pbind(uid);
            o._uid = uid;
        }
    ...
    phDone: function(ph, fail, text) {
        ...
        var el = ge('pad_ph' + ph),
            btns = geByClass1('pad_ph_btns', el) || geByClass1('pad_ph_result', el);
        if (!el) return;

        geByClass1('pad_ph_lnk', el)
            .href = '/photo' + ph;
        domPN(btns)
            .replaceChild(ce('div', {
                innerHTML: text,
    ...
    vidDone: function(vid, fail, text) {
        ...
        var el = ge('pad_vid' + vid),
            btns = geByClass1('pad_vid_btns', el) || geByClass1('pad_vid_result', el);
        if (!el) return;
    ...
    grDone: function(gid, fail, res, block, text) {
        if (fail) {
            ...
            if (block > 0) {
                ...
                text += '<div class="pads_gr_clubinv_block"><a onclick="Pads.grProcess(this, ' + gid + ', -2, 1)">' + val('groups_block_clubinv')
                    .replace('{club}', '<b>' + val(domPS(ge('pads_clubinv' + gid))) + '</b>') + '</a></div>';
            }
        }
        delete(_pads.cur.savedcnts[gid]);

        var el = ge('pad_gr' + gid),
            btns = geByClass1('pad_gr_btns', el) || geByClass1('pad_gr_result', el);
        if (!fail && block < 0) {
            text = btns.oldtext;
        }
        domPN(btns)
    ...
    grProcess: function(el, gid, res, block) {
        ...
        var act = (res == -2) ? 'spam' : (res ? 'enter' : 'leave'),
            context = (res == -1) ? '_decline' : ((res == 2) ? '_unsure' : '');
    ...
    msgAnswer: function(el, msg, ev) {
        ...
        if (t) {
            ...
            if (d[1]) {
                rcpts = '<a onclick="showBox(\'al_im.php\', {act: \'a_show_members_box\', chat: ' + (intval(d[0]) - 2e9) +
                    '}, {stat: [\'im.css\', \'boxes.css\'], params: {dark: 1}})" class="pad_msg_rcpts fl_l">' + getLang('mail_chat_X_rcpts', intval(d[1])) + '</a>';
            }
            var c = geByClass1('pad_msg_wrap', el)
                .appendChild(ce('div', {
                    className: 'pad_msga pad_msgr',
                    innerHTML: val('pad_msg_reply_tpl')
                        .replace('{field}', '\
<textarea onkeydown="Pads.onSubmit(this, event, Pads.msgSend.pbind(geByTag1(\'button\', this.parentNode), ' + msg +
                            '))" id="pad_msg_field' + msg +
                            '" class="pad_msg_field"></textarea>\
<div class="pad_msg_btns clear_fix">\
  <div class="pad_msg_btn button_blue fl_l">\
    <button onclick="Pads.msgSend(this, ' +
                            msg + ')" onmouseover="Pads.showTip(' + msg + ', 1)" onmouseout="Pads.showTip(' + msg + ', 0)">' + getLang('global_send') +
                            '</button>\
  </div>' + rcpts + '\
</div>')
                }));
            ...
            if (!_pads.layerBG) {
                _pads.layerBG = bodyNode.appendChild(ce('div', {
                    id: 'pad_layer_bg',
                    className: 'fixed',
            ...
            var dy = c.offsetTop + c.offsetHeight - _pads.content.scrollTop - _pads.content.offsetHeight + 15,
                d = function() {
                    Pads.update();
                    elfocus(t);
                };
            if (dy > 0) {
                animate(_pads.content, {
                    scrollTop: (_pads.content.scrollTop + dy)
                }, 200, d);
            } else {
                d();
            }
    ...
    msgDone: function(msg, fail, text) {
        ...
        if (fail) {
            var f = geByClass1('pad_msg_error', c),
                h = 0;
            if (f) {
                h -= f.offsetHeight;
            } else {
            ...
            var w = geByClass1('pad_msg_wrap', c),
                a = geByClass1('pad_msga', c),
                n = se(text);
            if (a) {
                w.replaceChild(n, a);
            } else {
    ...
    nwsAnswer: function(nws, ev, opts) {
        ...
        if (op) {
            ...
            if (opts.disabled) {
                var c = geByClass1('pad_nws_cont', el)
                    .appendChild(ce('div', {
                        className: 'pad_nwsa pad_nwsr',
                        innerHTML: val('pad_nws_dis_tpl')
                            .replace('{text}', opts.disabled)
                    }));
            } else {
                var c = geByClass1('pad_nws_cont', el)
                    .appendChild(ce('div', {
                        ...
                        innerHTML: val('pad_nws_reply_tpl')
                            .replace('{field}', '\
<textarea onkeydown="Pads.onSubmit(this, event, Pads.nwsSend.pbind(geByTag1(\'button\', this.parentNode), \'' +
                                nws + '\'))" id="pad_nws_field' + nws + '" class="pad_nws_field" placeholder="' + opts.ph + '">' + opts.greet +
                                '</textarea>\
<div class="pad_nws_btns clear_fix">\
  <div class="pad_nws_btn button_blue fl_l">\
    <button onclick="Pads.nwsSend(this, \'' +
                                nws + '\')" onmouseover="Pads.showTip(\'' + nws + '\', 1)" onmouseout="Pads.showTip(\'' + nws + '\', 0)">' + opts.btn +
                                '</button>\
  </div>\
</div>')
                    }));
    ...
    nwsDone: function(nws, fail, text) {
        ...
        if (fail) {
            var f = geByClass1('pad_nws_error', c),
                h = 0;
            if (f) {
                h -= f.offsetHeight;
            } else {
            ...
            var w = geByClass1('pad_nws_cont', c),
                a = geByClass1('pad_nwsa', c),
                n = se(text);
            if (a) {
                w.replaceChild(n, a);
            } else {
    ...
    nwsSend: function(el, nws) {
        ...
        var opts = _pads.editing[nws],
            item = val('pad_nwsd' + nws),
            text = trim(val('pad_nws_field' + nws));
    ...
    onAudioReorder: function(audio, before, after) {
        ...
        if (aid && padPlist && padPlist[aid]) {
            ...
            if (pl && pl.sorter && pl.sorter.elems) {
                var newStart = (pl.sorter.elems[0].id || '')
                    .substr(5);
    ...
    setAudioCurPos: function(speed) {
        var _a = window.audioPlayer,
            audio_id = (_a && _a.isPlaylistGlobal()) ? ls.get('audio_id') || currentAudioId() : currentAudioId(),
            topPad = getSize(ge('page_header'))[1] + 6;
        if (!audio_id) return;
    ...
    playLastStatus: function() {
        ...
        if (cur.nextPlaylist && cur.nextPlaylist.start) {
            ...
            if (m && m[1] && _a.statusData && _a.statusData[m[1]]) {
                ...
                ge('pad_footer_text')
                    .innerHTML = data.audio_goto_user;
                hide('pad_more_audio');
            }
        }
    },
    audioRow: function(audio, opts) {
        opts = opts || {};
        var aid = audio.full_id || audio[0] + '_' + audio[1],
            actions = '';
        var performer = audio[5].replace(/\\$/g, "\\&#36;");
        var _a = window.audioPlayer,
            cur_aid = (_a && _a.isPlaylistGlobal()) ? ls.get('audio_id') || currentAudioId() : currentAudioId();
        if (vk.id != audio[0] || audio[9] && intval(audio[9])) {
            if (cur.addedIds && cur.addedIds[audio[0] + '_' + audio[1]]) {
                ...
                actions += '<div class="audio_add_wrap fl_r" onmouseover="Audio.rowActive(this, \'' + getLang('audio_add_to_audio') +
                    '\', [9, 5, 0]);" onmouseout="Audio.rowInactive(this);" onclick="Pads.addAudio(this, ' + audio[1] + ', ' + audio[0] +
                    '); return cancelEvent(event);"><div class="audio_add"></div></div>';
            }
        }
        ...
        return rs(Pads.audioTpl, {
            ...
            playtime: audio[3],
            duration: audio[4],
            attr: 'href="/search?c[q]=' + encodeURIComponent(performer.replace(/(<span>|<\/span>)/g, '')) +
                '&c[section]=audio&c[performer]=1" onclick="if (checkEvent(event)) return; Audio.selectPerformer({from_pad: true, event: event, name: \'' + clean(
                    performer.replace(/(<span>|<\/span>)/g, '')) + '\'}); return false"',
            actions: actions,
            author: '',
    ...
    showAudios: function() {
        var self = this,
            __cur = window._pads && _pads.cur,
            pl = ge('pad_playlist'),
            _a = window.audioPlayer,
            pList = _a.recsLoaded && cur.nextPlaylist ? cur.nextPlaylist || {} : ((!window.audioPlaylist || window.curNotifier && (padPlData = ls.get('pad_pldata')) &&
                padPlData.source == curNotifier.instance_id) ? ls.get('pad_playlist') || window.audioPlaylist : padAudioPlaylist());
        ...
        if (pList) {
            var el = domLC(pl),
                ...
                first_id = '',
                showRowsCnt = 25,
                rowsCnt = geByClass('audio', pl)
                .length,
                _a, audio_id = ((_a = window.audioPlayer) && _a.isPlaylistGlobal()) ? ls.get('audio_id') || currentAudioId() : currentAudioId();

            if (el) {
                first_id = geByClass1('audio', pl)
                    .id.substr(5)
                    .replace('_pad', '');
                var last_id = el.id.substr(5)
                    .replace('_pad', '');
            ...
            if (!first_id) {
                ...
                if (aid) {
                    ...
                    do {
                        var rowEl = se(Pads.audioRow(pList[aid], {
                            thisTab: thisTab
                        }));
                ...
                if (pList.htitle) {
                    ge('pad_footer_text')
                        .innerHTML = '<span class="audio_album_length">' + pList.htitle + '</span>';
                }
                if (!el && !_a.recsLoaded) Pads.setAudioCurPos();
            }
            Pads.update();
