var html5video = {
    ...
    initHTML5Video: function(vars, fixedWidth, fixedHeight) {
        ...
        ge(video_box_id)
            .innerHTML = html5video.htmlCode();
        ge(video_box_id)
            .style.padding = '0px';

        var actsEl = ge('video_actions'),
            is_ext = intval(vars.is_ext);
        if (!is_ext && !vars.nolikes && !vars.min_controls) {
            ge('popup_actions')
                .appendChild(se('<div id="vid_like" class="vid_like fl_l ' + (vars.liked ? 'selected' : '') +
            ...
            if (vars.viewer_id !== vars.oid && vars.can_add) {
                ge('popup_actions')
                    .appendChild(se('<div id="vid_add" class="vid_add fl_l ' + (vars.added ? 'selected' : '') +
        ...
        if (!is_ext && vars.ads_link) {
            ge('popup_actions')
                .appendChild(se(
                    ...
                    (vars.lang_ads_link || 'Advertiser\'s Site') + '</div></div></div>'));
        ...
        if (false && vars.can_rotate && this.transformAvailable()) {
            actsEl.insertBefore(se(
                '<div id="rotate_btn" class="rotate_button fl_l" onmouseover="html5video.rotateOver(this, event)" onmouseout="html5video.rotateOut(this, event)" onclick="html5video.rotateVideo()"></div>'
            ), ge('quality_btn'));
        ...
        if (!vars.nologo) {
            actsEl.appendChild(se('<div id="logo_btn" class="logo ' + (vars.is_vk ? 'vk ' : '') +
        ...
        if (vars.timeline_thumbs) {
            ...
            ge('menu_controls')
                .appendChild(se('<div id="video_preview_tip"><div id="video_preview_tip_img_wrap">' + spritesheets +
    ...
    initVideoLinks: function() {
        ...
        setStyle(ge('quality_panel_wrap'), {
            top: -4 - linksCount * 22
        });
        setStyle(ge('quality_bk'), {
            height: linksCount * 22 - 5
        });
    ...
    showFinishLayer: function() {
      ...
      <div id="vid_finish_title" class="vid_finish_title">' +
            vars.md_title +
            '</div>\
      <div id="vid_finish_actions" class="vid_finish_actions clear_fix">\
        <div id="vid_finish_like" class="vid_finish_like fl_l ' + (
                html5video.liked ? 'selected' : '') +
            '" onclick="html5video.onLike(2)"><div id="vid_finish_like_bg" class="vid_finish_like_bg"></div><div id="vid_finish_like_fg" class="vid_finish_like_fg"></div><div id="vid_finish_liked_fg" class="vid_finish_liked_fg"></div></div>\
        ...
        ' +
            (vars.viewer_id !== vars.oid && vars.can_add ? '<div id="vid_finish_add" class="vid_finish_add fl_l ' + (html5video.added ? 'selected' : '') +
      ...
      <div>\
        ...
        ge('html5_player')
            .insertBefore(layer, ge('menu_layer'));
    ...
    showFinishExtendedLayer: function() {
      ...
      <div id="vid_finish_actions" class="vid_finish_actions vid_finish_extended_actions clear_fix">\
        <div id="vid_finish_like" class="vid_finish_like vid_finish_extended_like fl_l ' +
            (html5video.liked ? 'selected' : '') + '"' + (!canAdd ? ' style="width:230px"' : '') +
            ...
            vars.lang_like + '</div></div>\
        ' + (canAdd ? '<div id="vid_finish_add" class="vid_finish_add vid_finish_extended_add fl_l ' + (html5video.added ?
                    'selected' : '') +
    ...
    showNextVideoLayer: function() {
      ...
      <div id="vid_next_video" class="vid_next_video" onmouseover="html5video.nextThumbOver()" onmouseout="html5video.nextThumbOut()" onclick="html5video.nextVideo(\'' +
            nextVideoData.vid + '\', true, false)">\
        <div class="vid_next_video_caption">' + vars.lang_next +
            '</div>\
        <div class="vid_next_video_thumb" style="background-image: url(' + nextVideoData.thumb +
            ')"></div>\
        ...
        <div class="vid_next_video_info">\
          <div class="vid_next_video_title">' +
            nextVideoData.title + '</div>\
          <div class="vid_next_video_views">' + nextVideoData.views +
            '</div>\
        </div>\
    ...
    showSuggestionsLayer: function() {
        ...
        <div class="vid_suggestions_item fl_l" onclick="html5video.nextVideo(\'' + this.vid +
          ...
          <div class="vid_suggestions_item_views">' + this.views +
                '</div>\
        </div>';
        });
        contentHtml += '</div>';
    ...
    nextTimerStart: function() {
        ...
        html5video.nextTimerStarted = new Date()
            .getTime();
        html5video.nextTimerCtx = ge('vid_next_video_timer_canvas')
            .getContext('2d');
    ...
    nextTimerTick: function() {
        var progress = (new Date()
            .getTime() - html5video.nextTimerStarted) / 10000;
        if (progress < 1) {
    ...
    updTime: function() {
        ...
        if (video) {
            var c = video.currentTime || 0,
                d = video.duration || vars.duration || 0,
                percent = Math.min(100, Math.max(0, (d > 0 ? 100 * c / d : 0)));
            setStyle(ge('progress_line'), {
                width: percent + '%'
            });
            ge('curtime')
                .innerHTML = html5video.reversed && html5video.minSize ? html5video.formatTime(d - c) : html5video.formatTime(c);
            ge('duration')
    ...
    updVol: function() {
        ...
        setStyle(ge('volume_line'), {
            width: Math.min(100, Math.max(0, vol * 100)) + '%'
        });
        ge('the_video')
            .volume = vol;
    },
  ...
  <div id="html5_player">\
            ...
            <tr>\
              ...
              <td style="padding:11px 0 0 14px" id="time_label_cell">\
                <div id="time_label" class="time_label" onclick="html5video.onTimeClick(this)">\
                  <span id="curtime" class="time1_text">' +
            this.formatTime(this.vars.duration || 0) + '</span>\
                  <span id="duration" class="time2_text">' + this.formatTime(this.vars.duration || 0) +
            '</span>\
                </div>\
              </td>\
    ...
    showTip: function(text, x, offsetY) {
        ge('video_tip')
            .innerHTML = text;
        show('video_tip_wrap');
        var w = getSize('video_tip')[0],
            s = getSize('html5_player')[0],
            l = x - w / 2,
            a = w / 2 - 3;
        if (l + w > s - 10) {
            a -= s - w - l - 10;
    ...
    onErr: function(e) {
        ...
        topError('<b>Video loading error:</b> ' + msg, {
            dt: 5
        });
    },
    ...
    onPause: function() {
        ...
        if (ge('play_button')) ge('play_button')
    ...
    onVolMove: function(e) {
        ...
        xy = getXY(ge('volume_line')), video = ge('the_video'),
            percent = html5video.volLineW ? ((vertical ? xy[1] - html5video.defY(e) : html5video.defX(e) - xy[0]) + (fullScreenApi.isFullScreen() ? getXY(ge('html5_player'))[
                vertical] : 0)) / html5video.volLineW : 0;
