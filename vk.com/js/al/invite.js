var inviter = {
    ...
    checkLang: function() {
        var fname = trim(ge('inv_fname')
            .value);
        var lname = trim(ge('inv_lname')
            .value);
        var hasrus = true;
    ...
    init: function(opts) {
        ...
        if (!geByClass('user', ge('invites_invites_wrap'))
            .length) {
    ...
    send: function(force) {
        ...
        var fnameRaw = trim(ge('inv_fname')
                .value),
            lnameRaw = trim(ge('inv_lname')
                .value);
        var name = cleanName(fnameRaw, lnameRaw);
        fname = name[0];
        lname = name[1];

        var sex = isVisible('inv_sex_row') ? intval(ge('inv_sex')
                .value) : -1,
            phone = ge('inv_mobile')
            .value;
        ...
        var params = {
            ...
            sex: sex,
            phone: phone,
            lang: isVisible('inv_lang_row') ? ge('inv_lang')
                .value : 0
        };
        if (curBox()) {
            params.from = 'box';
        ...
        if (isVisible('inv_base_info')) {
            extend(params, {
                bday: ge('inv_bday')
                    .value,
                bmonth: ge('inv_bmonth')
                    .value,
                byear: ge('inv_byear')
                    .value,
                country: ge('inv_country')
                    .value,
                city: ge('inv_city')
                    .value
            });
        ...
        if (isVisible('inv_school_info')) {
            extend(params, {
                scountry: ge('inv_scountry')
                    .value,
                scity: ge('inv_scity')
                    .value,
                sschool: ge('inv_sschool')
                    .value,
                sschool_custom: ge('inv_sschool_custom')
                    .value,
                sschool_type: ge('inv_sschool_type')
                    .value
            });
            if (isVisible('inv_sdetails')) {
                extend(params, {
                    sclass: ge('inv_sclass')
                        .value,
                    sstart: ge('inv_sstart')
                        .value,
                    sfinish: ge('inv_sfinish')
                        .value,
                    sgraduation: ge('inv_sgraduation')
                        .value
                });
            }
        }
        if (isVisible('inv_uni_info')) {
            extend(params, {
                ucountry: ge('inv_ucountry')
                    .value,
                ucity: ge('inv_ucity')
                    .value,
                uuniversity: ge('inv_uuniversity')
                    .value,
                uuniversity_custom: ge('inv_uuniversity_custom')
                    .value,
                ufaculty: ge('inv_ufaculty')
                    .value,
                ufaculty_custom: ge('inv_ufaculty_custom')
                    .value,
                uchair: ge('inv_uchair')
                    .value,
                uchair_custom: ge('inv_uchair_custom')
                    .value
            });
            if (isVisible('inv_udetails')) {
                extend(params, {
                    ueduform: ge('inv_uedu_form')
                        .value,
                    uedustatus: ge('inv_uedu_status')
                        .value,
                    ugraduation: ge('inv_ugraduation')
                        .value
                });
            }
        }
