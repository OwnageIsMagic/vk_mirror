var WkView = {
    ...
    wikiClick: function(e, r, o) {
        ...
        if (t && "A" == t.tagName) {
            var i = t.href,
                w = i.match(/^\/(page[^?]*)(\?.*)?$/);
            if (w) var a = w[1],
                l = w[4] ? q2ajx(w[4].substr(1)) : {};
            else {
                var w = i.match(/https?:\/\/([a-zA-Z0-9\-_\.]+\.)?(vk\.com|vkontakte\.ru)\/([^?]*)(\?.*)?$/i);
                if (!w || !w[3]) return !0;
                var a = w[3].split("/"),
                    l = w[4] ? q2ajx(w[4].substr(1)) : {}
            }
    ...
    edit: function() {
        ...
        wkcur.note ? (params.nid = wkcur.nid, params.note = 1) : params.id = wkcur.pid, wkcur.from && (params.from = wkcur.from), ajax.post("wkview.php", params, {
            ...
            onDone: function(content, js, info, options) {
                wkcur.wkContent.innerHTML = content, hide(wkcur.wkRight), addClass(wkcur.wkBox, "wk_view_no_background"), eval(js), addEvent(wkLayerWrap,
                    "scroll", WkView.onScroll), addEvent(window, "resize", WkView.onResize), WkView.onScroll(), WkView.updateSize()
            }
        })
    },
    ...
    saveInfo: function(autosave) {
        ...
        return params.Body = wkcur.editor.val(), params.Body === !1 ? !1 : (params = wkcur.note ? extend(params, {
                ...
                oid: wkcur.oid,
                id: wkcur.pid
            }), wkcur.pageTitle && (params.title = val(wkcur.pageTitle)), "page_new" == wkcur.type && (params.page_new = 1), wkcur.from && (params.from = wkcur.from),
            autosave && (wkcur.lockAutoSave || wkcur.note && curBox()) ? !1 : (wkcur.lockAutoSave = !0, void ajax.post(wkcur.note ? "wkview.php" : "al_pages.php",
                params, {
                    onDone: function(text, data) {
                        ...
                        if ("market" == wkcur.from && data) ge("group_edit_market_wiki")
                            .innerHTML = data.html, eval(data.js);
                        else if (data) {
                            if (data.nid && (wkcur.nid = data.nid), data.raw && (wkcur.wkRaw = data.raw), data.created && cur.chooseMedia) return nav.setLoc(
                                extend(nav.objLoc, {
                                    w: "note" + data.raw
                                })), autosave || WkView.hide(!1, !0), cur.pbNoteAdded = data, cur.chooseMedia("note", data.raw, data), !0;
                            var title = ge("share_note_title" + data.raw);
                            if (title && !autosave && (title.innerHTML = data.title), wkcur.note && wkcur.toStatus && !autosave) return WkView.hide(!1, !0),
                                wkcur.reloadOnSave && (boxRefreshCoords(boxLoader), show(boxLoader), show(boxLayerWrap), nav.reload({
                                    onDone: function() {
    ...
    initWkBox: function(title, html, options, script) {
        ...
        options.wkRaw && (box.wkRaw = options.wkRaw), options.type && (box.type = options.type), options.commonClass && addClass(box.bodyNode, options.commonClass), script &&
            eval(script), box.setOptions({
                onHide: function(e) {
    ...
    show: function(title, html, options, script, ev) {
        ...
        if (toQueue && (layerQueue.push(), options.queue = !1), (window.wkcur || {})
            .shown && wkcur.root && nav.setLoc(wkcur.prevLoc), window.wkcur = {
                historyLen: hlen,
                _hide: [],
                _show: []
        ...
        if (cur.cancelTooltip = !0, window.tooltips && tooltips.hideAll(), boxQueue.hideAll(), isVisible(wkLayerWrap) || (otherList = !0, addEvent(window, "resize", WkView
            ...
            WkView.setLocation(options.noLocChange, toQueue), referrer == document.URL && (referrer = ""), setTimeout(window.comScoreUDM && comScoreUDM.pbind(locProtocol +
                "//" + location.host + "/wkview.php?comscorekw=pageview_candidate", referrer), 10), options.prevLoc && (wkcur.prevLoc = options.prevLoc)
        }
        ...
        "wall" == options.type ? (addEvent(window, "resize", WkView.onResize), WkView.wallBeforeInitPost(options)) : options.edit ? addEvent(window, "resize", WkView.onResize) :
            options && (content = '<div class="wk_text wk_wiki_content' + (options.className ? " " + options.className : "") +
                '" onclick="return WkView.wikiClick(this, event);">' + content + "</div>"), wkcur.wkCont ? wkcur.wkContent.innerHTML = content : (wkLayer.innerHTML =
                '<div class="wk_cont">  <div id="wk_box" onclick="wkcur.wkClicked = true;">    <div id="wk_loader"></div>    <a id="wk_close_link" href="javascript: return false;" class="fl_r wk_close_link" onclick="return WkView.hide(false, true, event);">      ' +
                getLang("global_close") + '    </a>    <div id="wk_summary" class="fl_l"><span class="summary" id="wk_layer_title">' + title +
                '</span></div>    <div id="wk_content" tabindex="0">' + content +
                '</div>    <div class="clear"></div>  </div>  </div><div id="wk_left_wrap"><div id="wk_left" style="' + hideLeft +
                '" class="wk_left no_select"></div></div><div class="wk_left_nav no_select" id="wk_left_nav" style="' + hideLeft +
                '" onmousedown="wkcur.wkClicked = true; WkView.back();" onselectstart="return cancelEvent(event);"></div><div class="wk_right_nav no_select" id="wk_right_nav" onmousedown="if (!wkcur.noClickHide) { wkcur.wkClicked = true; WkView.hide(); }"><div id="wk_right" class="wk_close no_select"><div class="wk_close_inner"></div></div></div><div id="wk_left_arrow_bg" class="wk_arrow_bg no_select" onclick="return WkView.navigate(this, event, -1);" onmouseover="WkView.preloadArrow(false)"><div class="wk_arrow_bg_inner"></div><div id="wk_left_arrow" class="wk_arrow no_select"></div></div><div id="wk_right_arrow_bg" class="wk_arrow_bg no_select" onclick="return WkView.navigate(this, event, 1);" onmouseover="WkView.preloadArrow(true)"><div class="wk_arrow_bg_inner"></div><div id="wk_right_arrow" class="wk_arrow no_select"></div></div>',
                options.nocross && re(geByClass1("wk_close_inner", wkLayer)), extend(wkcur, {
    ...
    hide: function(e, r, o) {
        if (window.wkcur && (r || wkcur.shown)) {
            ...
            if (clearTimeout(wkcur.autosaveTimeout), clearTimeout(wkcur.showT), r || !wkcur.edit || !wkcur.editor || !wkcur.editor.changed) {
                ...
                var i = !1;
                if (isVisible(wkLayerWrap) && (setTimeout(layerQueue.pop, 0), i = layerQueue.count() && "wiki" == layerQueue._layers[layerQueue._layers.length - 1][0] && !
                        layerQueue._bl, i || (layers.wraphide(wkLayerWrap), layers.fullhide = !1)), window.tooltips && tooltips.destroy(this), removeEvent(document,
                    ...
                    var w = scrollGetY();
                    Math.abs(w - cur.wkStartScroll) > 500 && scrollToY(cur.wkStartScroll, 0)
                }
                return delete wkcur.historyLen, getAudioPlayer()
                    .updateCurrentPlaying(), cur.gifAutoplayScrollHandler && cur.gifAutoplayScrollHandler(), !1
            }
            if (wkcur.note && wkcur.toStatus && !e) return void WkView.saveInfo();
            var a = showFastBox(wkcur.lang.pages_close_title, wkcur.lang.pages_close_text, getLang("box_yes"), function() {
                a.hide(), WkView.hide(e, !0)
            }, getLang("box_no"))
        }
    },
    cmp: function(e, r) {
    ...
    onKeyDown: function(e) {
        ...
        if (!(cur.pvShown || wkcur.edit || e.keyCode != KEY.LEFT && e.keyCode != KEY.RIGHT)) {
            ...
            return r && ("TEXTAREA" == r.tagName || "INPUT" == r.tagName || "DIV" == r.tagName && r.contentEditable && "inherit" != r.contentEditable) ? !0 : (WkView.navigate(
                null, e, e.keyCode == KEY.RIGHT ? 1 : -1), cancelEvent(e))
        }
        if (WkView.canEdit) {
    ...
    onResize: function() {
        ...
        var w = !1,
            a = wkcur.mvVeryBig;
        wkcur.mvVeryBig = t > 1280 ? 2 : t > 807 ? 1 : !1, w = a != wkcur.mvVeryBig, WkView.onScroll(!1, !0), WkView.updateHeight(), WkView.updateArrows(), setTimeout(
    ...
    updateArrows: function() {
        ...
        var r = getSize(wkcur.wkBox),
            ...
            w = i - 10,
            a = getSize(wkcur.wkLeftArrow)[0] || getSize(wkcur.wkRightArrow)[0];
        ...
        if (l[0] || l[1] || n) {
            ...
            l[0] || n && wkcur.introControlsCur ? (show(wkcur.wkLeftArrow), show(wkcur.wkLeftArrowBg), setStyle(wkcur.wkLeftArrowBg, {
                ...
                left: (lastWindowWidth - e - o) / 2 - w + (w - a) / 2,
                top: s
                ...
                setStyle(wkcur.wkRightArrowBg, {
                    ...
                    left: (lastWindowWidth - e - o) / 2 + o + (w - a) / 2,
                    top: s
    ...
    updateSize: function(e) {
        if (wkcur.wkCont) {
            var r = getSize(wkcur.wkCont),
                ...
                w = i + 10,
                a = wkLayer.offsetHeight - r[1] + i + 90;
            wkcur.wkCont.style.top = i + "px", wkcur.wkRight.style.top = Math.max(w - wkLayerWrap.scrollTop, 16) + "px", wkcur.wkLeftArrowBg.style.paddingTop = wkcur.wkRightArrowBg
                .style.paddingTop = w + "px", wkcur.wkLeftArrowBg.style.paddingBottom = wkcur.wkRightArrowBg.style.paddingBottom = a + "px", e || (onBodyResize(), WkView.onResize())
        }
    },
    ...
    likeShare: function(e) {
        ...
        if (checkbox(o), ajax.post("like.php", {
            ...
            w = hasClass(ge("wk_like_icon"), "my_like");
        else var a = ge("like_real_count_" + r),
            i = a ? a.value : val("like_count" + r),
            w = hasClass(ge("like_icon" + r), "my_like");
        WkView.likeUpdate(!0, intval(i) + (w ? 0 : 1))
    },
    ...
    likeUpdate: function(e, r, o) {
        ...
        if (i) {
            var w = t.tt || {},
                a = clone(w.opts || {}),
                l = domByClass(w.container, "_value"),
                n = domByClass(w.container, "_content"),
                s = domByClass(w.container, "_title");
            o && s && val(s, o), w && (w.likeInvalidated = !0), l && (l.value = r), wkcur.likes = r, animateCount(i, r), wkcur.liked = e, toggleClass(t, "my_like", e),
                toggleClass(t, "no_likes", !r), toggleClass(n, "me_hidden", !e), w.el && (r ? o === !1 ? w.destroy && w.destroy() : isVisible(w.container) || o || tooltips
                    .show(w.el, extend(a, {
                        showdt: 0
                    })) : w.hide())
        }
    ...
    extPageSubscribe: function(e, r, o, t) {
        if (!buttonLocked(e)) {
            ...
            if (r > 0) ajax.post("al_friends.php", {
                ...
                var a = ajax.post.pbind("al_groups.php", {
    ...
    wallAfterInitPost: function() {
        var e = wkcur.post,
            r = ge("reply_field" + e);
        cur.wkSubscribed && void 0 !== cur.wkSubscribed[intval(e)] && toggleClass(geByClass1("_wk_subscribe_btn", wkcur.wkBox), "secondary", cur.wkSubscribed[intval(e)]),
            delete cur.editing, r && placeholderInit(r, {
                editable: 1
        ...
        if (wkcur.postInited) {
            ...
            if (i && !hasClass(i, "wl_post_reply_form_forbidden")) {
                var w = ge("wl_reply_form_wrap"),
                    a = getXY(w, !0)[1],
                    l = getSize(i);
                if (formH = l[1], e && wkcur.fixedBottom === !1 && r - formH < a + 20) wkLayerWrap.scrollTop += a + 20 - (r - formH);
                else if (isVisible(w) && r - formH < a) {
        ...
        Wall.moreReplies(wkcur.post, o, t, {
            ...
            onDone: function(r, o, t) {
                extend(wkcur, {
                    ...
                    }), e && (wkLayerWrap.scrollTop += i.offsetHeight - w, setTimeout(Wall.scrollHighlightReply.pbind("post" + e), 0)), WkView.wallUpdateReplies(),
                    wkcur.loadingReplies = !1
            },
            onFail: function() {
                wkcur.loadingReplies = !1
    ...
    wallUpdateReplies: function() {
        ...
        if (wkcur.count && t && !isVisible(t.firstChild) && show(t.firstChild), i) {
            var w = ge("wl_reply_form_wrap");
            i.parentNode != w && w.appendChild(i)
        }
    ...
    likesInit: function() {
        extend(wkcur, {
            ...
            }), WkView.initSTL(), WkView.likesTabInit(), addEvent(wkLayerWrap, "scroll", WkView.onScroll), addEvent(window, "resize", WkView.onResize), onBodyResize(), cur
            .wallTpl = cur.wallTpl || {}, cur.wallInited ? (wkcur._oldReplyForm = cur.wallTpl.reply_form, cur.wallTpl.reply_form = wkcur.wall_opts.wall_tpl.reply_form) :
            Wall.initWallOptions(wkcur.wall_opts), void 0 === cur.options ? cur.options = {
                reply_names: {}
                ...
                removeEvent(wkLayerWrap, "scroll", WkView.onScroll), removeEvent(window, "resize", WkView.onResize), wkcur._oldReplyForm && (cur.wallTpl.reply_form =
                    wkcur._oldReplyForm, wkcur._oldReplyForm = !1)
            })
    },
    ...
    likesMore: function() {
        ...
        isButtonLocked(e) || ajax.post("wkview.php", {
            ...
            onDone: function(r, o, t, i, w) {
                var a = ge("wk_likes_rows");
                if (a) {
                    if (w)
                        for (var l = geByClass("wk_likes_hidden", a), n = 0, s = l.length; s > n; ++n) a.appendChild(l[n]), removeClass(l[n], "wk_likes_hidden");
                    a.appendChild(cf(r)), wkcur.offset = o, t ? WkView.likesPreload() : hide(e), WkView.updateHeight(), i && extend(cur.options.reply_names, i)
                }
            },
            showProgress: lockButton.pbind(e),
            hideProgress: unlockButton.pbind(e),
        ...
        if (!r.cached) {
            ...
            if (t) {
                ...
                if (t && "/query" == t.substr(-6)) {
                    var w = clone(nav.objLoc);
                    delete w[0], delete w.w, i.query = JSON.stringify(w)
                }
    ...
    stlOnScroll: function(e) {
        var r = wkLayerWrap.scrollTop,
            ...
            t = wkcur.lSTLWas || r > o,
            i = 0;
        if (wkcur.lSTL.style.marginTop = r + "px", t ? (1 !== wkcur.lSTLShown && (show(wkcur.lSTL), wkcur.lSTLShown = 1), wkcur.lSTLWas && r > 500 && (wkcur.lSTLWas = 0),
                r > o ? (i = (r - o) / o, wkcur.lSTLWasSet && (wkcur.lSTLWasSet = 0, val(domLC(wkcur.lSTL), getLang("global_to_top")), removeClass(domLC(wkcur.lSTL),
                    "down"))) : (i = (o - r) / o, wkcur.lSTLWas && (wkcur.lSTLWasSet || (wkcur.lSTLWasSet = 1, val(domLC(wkcur.lSTL), ""), addClass(domLC(wkcur.lSTL),
                    "down"))))) : 0 !== wkcur.lSTLShown && (hide(wkcur.lSTL), wkcur.lSTLShown = 0), wkcur.wkLeft && wkcur.wkLeftNav) {
