var Gifts = {
    ...
    onReceiverChange: function() {
        ...
        val('gft_box_to', getLang((cur.sel && cur.sel.length > 1) ? 'gifts_receivers' : 'gifts_receiver')
            .replace(':', ''));
    },
...
<div class="tb_title">' + getLang('gifts_send_gift') +
            '</div>\
<div id="gifts_do_send_wrap">\
' + (left > 0 ? '<div class="info_msg" style="margin-bottom: 10px;">' + getLang('gifts_free_send_left', left) +
                '</div>' : '') + '\
<div class="gs_box_wrap">\
  <img class="gs_box_img" src="images/gift/' + num +
            '/256.jpg" />\
</div>\
<div class="gft_send">\
  <div class="gft_box_label" id="gft_box_to">' + getLang('gifts_receiver')
            .replace(':', '') +
            '</div>\
  <div class="wdd clear_fix" id="gft_box_dd">\
    <div class="fl_r wdd_arr"></div>\
    <input type="text" class="wdd_text fl_l" placeholder="' +
            clean(getLang('gifts_choose_recipients')) +
            '" id="gft_box_inp" onfocus="this.focused=1" onblur="this.focused=\'\'" />\
    <br class="clear" />\
  </div>\
  <div class="fl_r" id="gs_warn"></div>\
  <div class="gft_box_label">' +
            getLang('gifts_your_message')
            .replace(':', '') +
            '</div>\
...
' + (bonus > 0 ? '<div class="gs_bonus">' + getLang('gifts_rate_increase')
                .replace('{gifts_count}', getLang(bonus, 'gifts_X_votes')) + '</div>' : '') + '\
' + (tip ? '<div class="gs_tip">' + tip + '</div>' : '') +
            '\
</div>\
    ', getLang('box_send'), Gifts.sendGift, getLang('global_cancel'));
        setStyle(domNS(box.bodyNode), {
            border: '0px'
        });
    ...
    sendGift: function() {
        if (cur.giftsSendConfirmed) {
            ...
            var sendBtn = geByClass1('button_blue', cur.confirmBox.bodyNode.nextSibling)
                .firstChild;
        } else {
            var mids = [],
            ...
            var query = {
                act: 'do_send',
                text: ge('gs_msg')
                    .value, //(isVisible('gs_msg_wrap') ? ge('gs_msg').value : ''),
                mids: mids,
                number: cur.gift,
            ...
            var sendBtn = geByClass1('button_blue', curBox()
            ...
            onDone: function(data, text) {
                if (data === 'confirm') {
                    ...
                    cur.confirmBox = showFastBox({
                        title: cur.giftsTitle,
                        dark: 1,
                        bodyStyle: 'padding: 20px; line-height: 160%;'
                    }, text, getLang('box_send'), cur.sendBoxGift, getLang('global_cancel'));
                    return;
                }
    ...
    initGiftsBox: function(box, opts) {
        box.setOptions({
            width: 620,
            bodyStyle: 'margin-top: 50px; padding: 0px',
            onShow: function() {
        ...
        extend(cur, {
            ...
            gftbxMid: opts.mid,
            gftbxId: box.tbId,
            gftbxAddLnk: ge('tb_' + box.tbId + '_prg')
                .parentNode.insertBefore(se('<div class="gifts_add_lnk fl_r"><a href="/gifts?act=send" onclick="return Gifts.showGiftBox(cur.oid, event);">' +
                    getLang('gifts_send_gift') + '</a></div>'), ge('tb_' + box.tbId + '_prg')),
            ...
            lSTL: boxLayerWrap.appendChild(ce('div', {
                ...
                el: box.bodyNode,
                onclick: cancelEvent,
                onmousedown: Gifts.lSTLDown,
                sc: Gifts.onGiftsScroll
            })),
            lSTLShown: 0,
            lSTLWas: 0,
            lSTLWasSet: 0
    ...
    onGiftsScroll: function() {
        ...
        var bt = lastWindowHeight,
            objMore = ge('gifts_more_link' + curBox()
                .tbCur);
