...
<div class="fl_l summary_tab' + ((curTab == i) ? '_sel' : '') + '">\
  <a class="summary_tab2"' + tabHref +
            ' onclick="if (checkEvent(event)) return; cur.onTabSwitch[' + tabSwitch + '](\'' + i +
            '\');geByClass1(\'summary_tab_sel\', this.parentNode.parentNode).className=\'fl_l summary_tab\';this.parentNode.className=\'fl_l summary_tab_sel\';return false;">\
    <div class="summary_tab3"><nobr>' +
            tabs[i].label + '</nobr></div>\
  </a>\
</div>\
    ');
    }
    geByClass1('summary_tabs', box.bodyNode)
        .innerHTML = html.join('');
}
