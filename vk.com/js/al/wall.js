var FullWall = {
    getsect: function() {
        var current = ge('full_wall_filters')
            .firstChild;
    ...
    go: function(el, ev) {
        ...
        var result = nav.go(el, ev, {
            onFail: FullWall.failed.pbind(current)
        });
        if (result === false) {
        ...
        cur.nav.push(function(changed, current, next) {
            ...
            ajax.post('al_wall.php', {
                ...
                onDone: function(summary, rows, names, href, start, offset, perpage, count, preload, queue) {
                    ...
                    ge('wall_' + (next.own ? 'all' : 'own') + '_filter')
                        .className = '';
    ...
    updateSummary: function(count) {
        ...
        if (count > 1) {
            ge('fw_summary')
                .innerHTML = getLang('wall_x_posts_at_all', count, true);
        } else if (count > 0) {
            ge('fw_summary')
                .innerHTML = getLang('wall_one_post');
        } else {
            ge('fw_summary')
                .innerHTML = getLang('wall_no_posts');
        }
        if (ds) ge('fw_summary')
            .appendChild(ds);
    },
    noArrowNav: function() {
        return cur.__focused || (ge('own_reply_field') || {})
            .focused || cur.editingPost;
    },
    ...
    replyFieldUpdated: function() {
        ...
        var newt = Emoji.val(cur.addField),
    ...
    sendReply: function(ev, stickerId) {
        ...
        if (stickerId) {
            ...
            var params = composer ? Composer.getSendParams(composer, FullWall.sendReply) : {
                message: trim(Emoji.val(rf))
            };
            if (params.delayed) {
                return;
            }
    ...
    replyTo: function(post, toMsgId, toId, event) {
        ...
        var replyName = cur.options.reply_names[toId],
            cancel = '<span class="fwr_no_to" onclick="FullWall.cancelReplyTo(); elfocus(\'fwr_text\');"></span>';
            ...
            onDone: function(msg, additional, need_confirm) {
                if (need_confirm && act == 'delete') {
                    var box = showFastBox(msg, need_confirm, getLang('global_delete'), function() {
                        box.hide();
                        FullWall.replyAction(act, post, hash, 1);
                    }, getLang('box_cancel'));
                    return;
                }
                var p = root ? ge('fw_post') : ge('post' + post);
        ...
        stManager.add(['ui_controls.js', 'datepicker.js', 'datepicker.css'], function() {
            if (!cur.wallDP) {
                ...
                cur.wallDP = new Datepicker(ge('wall_datesearch'), {
                    width: 140,
                    resfmt: 'plain',
                    addRows: nav.objLoc.day ?
                        '<tr><td class="wall_cal_clear" colspan="7"><a onclick="nav.change({day: false, offset: false, search: nav.objLoc.q ? false : 1})" id="wall_cal_clear_lnk">' +
                        getLang('wall_clear_date_filter') + '</a></td></tr>' : '',
                    onUpdate: function() {
                        if (cur.wallSD != val('wall_datesearch')) {
                            var nd = val('wall_datesearch')
                                .split('.');
                            nav.change({
            ...
            ge('wall_datesearch_cal_box')
                .style[vk.rtl ? 'right' : 'left'] = Math.floor((ge('wall_datesearch_lnk')
                    .offsetWidth - ge('wall_datesearch_cal_div')
                    .offsetWidth) / 2 + ge('wall_datesearch_lnk')[vk.rtl ? 'offsetRight' : 'offsetLeft']) + 'px';
            ge('wall_datesearch_cal_box')
                .style.marginTop = '-2px';
        });
    },
