var WriteBox = {
    ...
    show: function(box, opts) {
        ...
        if (opts.emojiRcnt && !cur.mbRcntEmoji) {
            ...
            for (var a = opts.emojiRcnt, i = 0, l = a.length; i < l; ++i) {
                var code = a[i];
                if (!code) continue;
                html.push('<a id="mbe_rc_em_' + code + '" class="mbe_rc_emojibtn" onmousedown="Emoji.addEmoji(cur.emojiWId, \'' + code +
        ...
        cur.emojiWId = Emoji.init(cur.mbField, {
            ...
            onStickerSend: function(stNum) {
                var text = trim(Emoji.editableVal(cur.mbField)),
                    media = cur.mbMedia.getMedias();
                ...
                ajax.post('/al_mail.php', {
                    ...
                    onFail: function(failText) {
                        setTimeout(showFastBox(getLang('global_error'), failText)
                            .hide, 3000);
                        return true;
                    }
                });
        ...
        if (!window._mbFriends) { // is used in sharebox.js too!
            ajax.post('hints.php', {
                ...
                onDone: function(arr, stickersList) {
            ...
            if (WideDropdown.init('mail_box_dd', {
                    ...
                    onChange: function(act) {
                        ...
                        if (sel < 1 && !cur.mbHidden) {
                            cssAnim(cur.mbAva, extend({
                                opacity: 0
                            }, WriteBox.mrg(-26)), {
                                duration: d,
                                transition: t,
                                func: f
                            }, hide.pbind(cur.mbAva));
                            cssAnim(ge('mail_box_topic'), WriteBox.mrg(0), {
                                duration: d,
                                transition: t,
                                func: f
                            });
                            ...
                            cssAnim(ge('mail_box_topic'), WriteBox.mrg(26), {
                                duration: d,
                                transition: t,
                                func: f
                            });
        ...
        stManager.add(['page.js', 'page.css'], function() {
            cur.mbMedia = initAddMedia('mail_box_add_link', 'mail_box_added_row', cur.mbMediaTypes, {
                mail: 1,
                nocl: 1,
                editable: 1,
                sortable: 1,
    ...
    restoreDraft: function(needPeer) {
        ...
        if (draft) {
            if (!WriteBox.editableHasVal(cur.mbField)) {
                if (cur.mbEditable) {
                    val(cur.mbField, clean(draft.txt || '')
                        .replace(/\n/g, '<br/>'));
                    if (window.Emoji) {
            ...
            if ((draft.medias || [])
                ...
                ajax.post('al_im.php', {
                    ...
                    onDone: function(resp) {
                        if (!cur.mbField || WriteBox.getPeer() != peer || !(resp || [])
                            .length) return;
                        each(resp, function() {
    ...
    toFull: function(ev, peer) {
        ...
        if (cur.mbMedia.chosenMedias) {
            var meds = cur.mbMedia.getMedias(),
                media = [];
    ...
    showToFull: function() {
        ...
        if (mid > 2e9) {
            val('mail_box_to_full', '<a href="/im?sel=c' + (mid - 2e9) + '" onclick="return WriteBox.toFull(event, ' + mid + ')">' + getLang('mail_im_to_multidialog') +
                '</a>');
            show('mail_box_to_full');
        } else {
            if (!mid || !sex || !text) return;
            val('mail_box_to_full', ('<a href="/im?sel=' + mid + '" onclick="return WriteBox.toFull(event, ' + mid + ')">' + getLang('mail_go_to_dialog') + '</a>')
                .replace('%s', text));
            show('mail_box_to_full');
        }
    },
    ...
    extractEmoji: function() {
        ...
        for (var code in newRc) {
            ...
            rchtml += '<a id="mbe_rc_em_' + code + '" class="mbe_rc_emojibtn" onmousedown="Emoji.addEmoji(cur.emojiWId, \'' + code +
                '\', this); return cancelEvent(event);">' + Emoji.getEmojiHTML(code, false, true) + '</a>';
            ml -= 22;
        }
        cur.mbRcntEmoji = (rchtml + val(rcCont))
            .split('a><a')
            .slice(0, 7)
            .join('a><a');
        ...
        animate(rcCont, {
            ...
            onComplete: function() {
                var emjs = geByClass('mbe_rc_emojibtn', rcCont)
                    .slice(7);
                for (var i in emjs) {
                    re(emjs[i]);
                }
