var GroupsList = {
    ...
    enter: function(el, gid, hash, act, past) {
        ...
        var key = GroupsList.rand(),
            value = GroupsList.rand();
            ...
            onDone: function(newRow, added) {
                ...
                if (shown) {
                    if (cur.switchedInvite && act == 'decline' && !past) {
                        var switchedHtml = getLang('groups_event_left') + ' <a onclick="GroupsList.spam(this.parentNode, ' + gid + ', \'' + hash +
                            '\', cur.switchedInvite)">' + getLang('its_spam') + '</a>';
                        if (ge('clubinv' + gid)) switchedHtml += '<div class="group_row_blockinv"><a onclick="GroupsList.spam(domPN(domPN(this)), ' + gid +
                            ', \'' + hash + '\', cur.switchedInvite, 1)">' + getLang('groups_block_clubinv')
                            .replace('{club}', '<b>' + val(domPS(ge('clubinv' + gid))) + '</b>') + '</a></div>';
                ...
                var status = geByClass1('group_row_status', row),
                    actions = geByClass1('group_row_actions', row),
                    old = status.basehtml;
...
<div class="group_row_buttons">\
  <div class="group_row_button button_blue fl_l">\
    <button onclick="GroupsList.enter(this, ' +
                        gid + ', \'' + hash + '\', \'join\')">' + getLang('groups_event_go_btn') +
                        '</button>\
  </div>\
  <div class="group_row_button button_blue fl_l">\
    <button onclick="GroupsList.enter(this, ' + gid + ', \'' +
                        hash + '\', \'unsure\')">' + getLang('groups_event_maybe_btn') +
                        '</button>\
  </div>\
  <div class="group_row_button button_cancel fl_l">\
    <div class="button" onclick="GroupsList.enter(this, ' +
                        gid + ', \'' + hash + '\', \'decline\')">' + getLang('groups_event_cant_btn') + '</div>\
  </div>\
</div>';
                } else if (act) {
                    if (past) {
                        ...
                        if (act == 'decline') {
                            actions.innerHTML = '<a onclick="GroupsList.enter(this.parentNode, ' + gid + ', \'' + hash + '\', \'join\', true)">' + getLang(
                                'groups_return_event') + '</a>';
                        } else {
                            actions.innerHTML = '<a onclick="GroupsList.enter(this.parentNode, ' + gid + ', \'' + hash + '\', \'decline\', true)">' + getLang(
                                'groups_remove_event') + '</a>';
                        }
                    } else {
                        var acts = '<a onclick="GroupsList.enter(this.parentNode, ' + gid + ', \'' + hash + '\', \'undecided\')">' + getLang(
                            'groups_event_change') + '</a>';
                        if (act == 'unsure') {
                            var statHTML = getLang('groups_unsure_event');
                        } else if (act == 'decline') {
                            var statHTML = getLang('groups_event_left');
                            acts += '<span class="divider">|</span><a onclick="GroupsList.spam(this.parentNode, ' + gid + ', \'' + hash + '\')">' + getLang(
                                'its_spam') + '</a>';
                            if (ge('clubinv' + gid)) acts += '<div class="group_row_blockinv"><a onclick="GroupsList.spam(domPN(domPN(this)), ' + gid + ', \'' +
                                hash + '\', false, 1)">' + getLang('groups_block_clubinv')
                                .replace('{club}', '<b>' + val(domPS(ge('clubinv' + gid))) + '</b>') + '</a></div>';
                        } else {
                ...
                if (tab == 'inv') {
                    ...
                    if (row) {
                        var statusOne = geByClass1('group_row_status', row),
                            actionsOne = geByClass1('group_row_actions', row);
    ...
    switchInvite: function(gid, prepareActs) {
        ...
        if (prepareActs) {
            cur.switchedInvite = ce('div', {
                className: 'gle_invites_actions',
                innerHTML: '<div class="progress_inline"></div>'
            });
        ...
        if (cur.scrollList.invCount) {
            ge('gle_inv_summary')
                .innerHTML = langNumeric(cur.scrollList.invCount, cur.scrollList.summaries.inv);
        } else {
            ge('gle_inv_summary')
                .innerHTML = cur.scrollList.summaries.invEmpty;
        }
        return true;
    },
    ...
    leave: function(el, gid, hash, pp) {
        ...
        ajax.post('al_groups.php', {
            ...
            onDone: function(newRow) {
                ...
                var acts = '<a onclick="GroupsList.cancel(this.parentNode, ' + gid + ', \'' + hash + '\')">' + getLang('global_cancel') +
                    '</a><span class="divider">|</span><a onclick="GroupsList.spam(this.parentNode, ' + gid + ', \'' + hash + '\')">' + getLang('its_spam') +
                    '</a>';
                ...
                if (shown) {
                    if (cur.switchedInvite) {
                        cur.switchedInvite.innerHTML = text + ' <a onclick="GroupsList.spam(this.parentNode, ' + gid + ', \'' + hash +
                            '\', cur.switchedInvite)">' + getLang('its_spam') + '</a>';
                    }
                    return;
                }
            ...
            onDone: function(respText) {
                ...
                if (applyCont) {
                    if (block < 0) {
                        ...
                        if (block > 0) {
                            respText += '<div class="group_row_blockinv"><a onclick="GroupsList.spam(domPN(domPN(this)), ' + gid + ', \'' + hash +
                                '\', cur.switchedInvite, -1)">' + getLang('global_cancel') + '</div>';
                        } else if (ge('clubinv' + gid)) {
                            respText += '<div class="group_row_blockinv"><a onclick="GroupsList.spam(domPN(domPN(this)), ' + gid + ', \'' + hash +
                                '\', cur.switchedInvite, 1)">' + getLang('groups_block_clubinv')
                                .replace('{club}', '<b>' + val(domPS(ge('clubinv' + gid))) + '</b>') + '</a></div>'
                        }
                ...
                if (block < 0) {
                    ...
                    if (block) {
                        ...
                        actions.innerHTML = '<div class="group_row_blockinv"><a onclick="GroupsList.spam(domPN(domPN(this)), ' + gid + ', \'' + hash +
                            '\', false, -1)">' + getLang('global_cancel') + '</a></div>';
                    } else {
                        status.basehtml = status.innerHTML;
                        actions.basehtml = actions.innerHTML;
    ...
    cancel: function(el, gid, hash) {
        ...
        ajax.post('al_groups.php', {
            ...
            onDone: function() {
                ...
                if (elem) {
                    var name = elem[0],
                        q = trim(cur.scrollList.query.value);
                    if (q) {
                    ...
                    row.parentNode.replaceChild(ce('div', {
                            innerHTML: newRow
                        })
                        .firstChild, row);
                } else {
    ...
    locNav: function(changed, oldLoc, newLoc) {
        var changedTab = changed.tab,
            tab = changedTab || 'groups';
        delete(changed.tab);
        if (!isEmpty(changed) || changedTab === undefined) return;

        ge('groups_tab_' + cur.scrollList.tab)
            .className = '';
        ...
        if (cur.scrollList.events) {
            toggle(cur.scrollList.events, tab == 'groups' && ge('gle_list_cont')
                .innerHTML);
        ...
        if (cur.scrollList.invites || cur.scrollList.events && ge('gle_list_cont')
            .innerHTML) {
    ...
    init: function(opts) {
        ...
        extend(cur, {
            ...
            scrollList: {
                ...
                perpage: 20,
                offset: ge('groups_list_' + opts.tab)
                    .childNodes.length,
    ...
    initAdvancedSearchBlock: function(opts) {
        ...
        var options = {
            big: true,
            width: Math.floor((getSize(searchWrapEl)[0] - getSize(advancedBtnEl)[0] - 44) / 2)
        };
        ...
        new Dropdown(ge('groups_search_group_type'), [
            [0, getLang('search_filter_any')],
            [1, getLang('search_Group')],
            [2, getLang('search_Public')],
            [3, getLang('search_Event')]
        ], extend(options, {
            selectedItem: 0,
        ...
        new Dropdown(ge('groups_search_group_sort'), [
            [0, getLang('groups_order_members')],
            [1, getLang('groups_order_popularity')],
            [6, getLang('groups_order_relevance')]
        ], extend(options, {
            selectedItem: 0,
    ...
    load: function(force, forceTab) {
        ...
        ajax.post(cur.scrollList.url, extend(cur.scrollList.params, {
            tab: tab
        }), {
            onDone: function(result) {
                ...
                cur.scrollList.cache[tab] = {
                    all: []
                };
                var processed = cur.scrollList.processed[tab],
                    gid;
    ...
    getHighlight: function(q) {
        var indxr = cur.scrollList.index[cur.scrollList.tab],
    ...
    updateEvents: function() {
        if (!ge('gle_list_cont')
            .innerHTML && !cur.scrollList.invites || cur.scrollList.tab != 'groups') return;
        toggle(cur.scrollList.events, !cur.scrollList.query.value && ge('gle_list_cont')
            .innerHTML);
        cur.scrollList.eventsShow = false;
        if (isVisible(cur.scrollList.eventsMore)) {
            hide(cur.scrollList.eventsMore);
    ...
    showMore: function(force) {
        ...
        var tab = cur.scrollList.tab,
            list = cur.scrollList.cache[tab].all;
        ...
        if (!cur.loadingShown) {
            ...
            setTimeout(function() {
                ...
                setStyle(searchInputEl, {
                    width: q ? (528 - advancedToggleBtnWidth) : ''
                }); // todo: too hacky
            }, 1);
        }
        ...
        if (q) {
            ...
            if (list === undefined) {
                var tmp = cur.scrollList.index[tab].search(q),
                    mp = {};
                list = [];
        ...
        var cont = ge(cur.scrollList.prefix + tab),
            more = ge(cur.scrollList.prefix + tab + '_more');
        if (!len) {
            if (q && tab == 'groups') {
                if (refresh) {
        ...
        var start = refresh ? 0 : cur.scrollList.offset,
            end = Math.min(len, start + cur.scrollList.perpage);
        var html = [];
        ...
        cur.searchTimeout = setTimeout(function() {
            ...
            ajax.post('al_groups.php', GroupsList.extendWithAdvancedParams({
                ...
                onDone: function(count, rows, summary, finish) {
                    if (cur.searchStr != q) return;
                    if (count) {
                        var last = geByClass('group_list_row', cont)
                            .pop();
                        if (last) {
    ...
    recomMore: function(ev) {
        ...
        var preload = ge('group_recom_preload'),
            sz = getSize(preload),
            lnk = ge('group_recom_more'),
            isVis = isVisible(preload);
        if (isVis) {
