...
Payments.masterbankVerifySumDialog = function() {
    cur.mb_verify_sum_dialog = showFastBox(getLnag('payments_verify_sum_header'), mb_verify_sum_dialog_content, getLang('box_send'), Payments.masterbankSubmitVerifySum,
        getLang('box_cancel'));
    setTimeout(function() {
        ge('verify_sum_input')
            .focus()
...
Payments.masterbankSubmitPaymentForm = function(random_auth, min_amount) {
    ...
    if (!random_auth) {
        var sum = parseFloat(ge('input_sum')
                .value.replace(/[^\d.,]/g, ""))
            .toFixed(2);
        ...
        var originalSum = parseFloat(ge('input_money')
                .value.replace(/[^\d.,]/g, ""))
            .toFixed(2);
        if (cur.isAdsPayment && originalSum < min_amount) {
            var error = getLang('payments_min_amount_limit_rubles')
                .replace('%s', min_amount);
...
Payments.scrollTo = function(el) {
    offset = Payments.getOffset(ge(el))
        .top;
...
Payments.paymentCheckAmount = function(o) {
    ...
    v = v.replace(/[,��]/, ".");
    var i = parseInt(v);
    if (i >= 100000) v = parseInt(i.toString()
        .substr(0, 5)) + Math.floor((v - i) * 100) / 100;
    if (o.value != v)
        o.value = v;
}
...
Payments.adsConfirmGoPaymentSystem = function(func) {
    var confirmMessage = getLang('ads_payments_confirm_go')
        .replace('{office_id}', ge('payment_account_id_view')
            .innerHTML);
...
var MoneyTransfer = {
    ...
    send: function() {
        ...
        if (!amount || amount <= 0) {
            ...
            error = getLang('payments_money_transfer_error_min_amount')
                .replace('%d', cur.paymentsOptions.minAmount);
        } else if (cur.paymentsOptions.maxAmount && amount > cur.paymentsOptions.maxAmount) {
            error = getLang('payments_money_transfer_error_max_amount')
                .replace('%d', cur.paymentsOptions.maxAmount);
        }
        if (error != false) {
