var PhotoCaptcha = {
    init: function(c, t) {
        ...
        for (var p in cur.pcPhotos) vkImage()
            .src = cur.pcPhotos[p]
    },
    show: function(c) {
        if (void 0 !== c || cur.pcShowing || (c = (cur.pcCur + 1) % cur.pcPhotos.length), cur.pcPhotos[c]) {
            ...
            if (c != cur.pcCur) {
                ...
                var t, p = cur.pcWrapEl.appendChild(domFC(ce("div", {
                    innerHTML: '<table class="pcpt_photo" cellspacing="0" cellpadding="0" style="position: absolute; opacity: 0;">  <tr><td class="pcpt_cell" onclick="PhotoCaptcha.show()"><img src="' +
                        cur.pcPhotos[c] + '" class="pcpt_big_img" /></td></tr></table>'
                })));
                setStyle(domPS(p), {
                    position: "absolute",
                    marginLeft: 0
    ...
    select: function(c) {
        c !== cur.pcSel && (void 0 !== cur.pcSel && removeClass(ge("pcpt_user" + cur.pcSel), "pcpt_user_sel"), addClass(ge("pcpt_user" + c), "pcpt_user_sel"), cur.pcSel =
            c)
    },
    other: function() {
        ajax.post("pcaptcha.php", {
            ...
            onDone: function(c, t, p) {
                delete cur.pcSel, extend(cur, {
                    ...
                    '<table class="pcpt_photo" cellspacing="0" cellpadding="0">  <tr><td class="pcpt_cell" onclick="PhotoCaptcha.show()"><img src="' + cur.pcPhotos[
                        0] + '" class="pcpt_big_img" /></td></tr></table>'), val(ge("pcpt_thumbs"), t), val(ge("pcpt_names"), p)
            }
        })
    }
};
