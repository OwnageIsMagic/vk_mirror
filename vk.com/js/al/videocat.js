window.Videocat = window.Videocat || {
    ...
    getPopularShownVids: function() {
        ...
        for (var i = from;; ++i) {
            ...
            var vid = (item.getAttribute('id') || '')
                .replace(/^videocat_video_item_/, '');
            vids.push(vid);
        }
        ...
        stManager.add(['video.js', 'videoview.js', 'notifier.js'], function() {
            ...
            if (playlistId == 'ugc_popular') {
                if (cur.popular_qid) {
                    ...
                    vkImage()
                        .src = '//go.imgsmail.ru/vk?pxn=vic&qid=' + cur.popular_qid + '&vid=' + videoId + '&p=' + pos + '&t=0';
                }
    ...
    buildPlaylistBlock: function(playlistId, force) {
        ...
        playlistId = (window.Video && (Video.isCurrentChannel() || Video.isCurrentCategory() || Video.isCurrentSectionAlbum() || Video.isInVideosList())) ? ('all_' +
            playlistId) : playlistId;
            ...
            ajax.post('al_video.php', {
                ...
                onDone: function(html, list) {
                    ...
                    if (list && list[playlistId]) {
                        Videocat.lists[playlistId] = Videocat.lists[playlistId] || {
                            list: []
                        };
    ...
    nextVideo: function() {
        ...
        if (playlist.loaded && playlist.loaded.vid == nextVideo.vid) {
            ...
            videoview.show(null, nextVideo.vid, loadData.listId, extend(loadData.options, {
                playlistId: playlist.id
            }));
    ...
    showMore: function(btn, countToShow, type) {
        ...
        if (type == 'ugc_popular' && cur.popular_qid) {
            ...
            vkImage()
                .src = '//go.imgsmail.ru/vk?pxn=vs&qid=' + cur.popular_qid + vids;
        }
        ...
        if (window.Video) {
            if (Video.isCurrentChannel()) {
                ...
                var albumTitle = 'Album',
                    albumId = cur.vSection.substr('album_'.length);
