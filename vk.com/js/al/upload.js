    ...
    var Upload = {
        ...
        initFlash: function(iUpload, obj) {
            ...
            var opts = {
                ...
                preventhide: true,
                width: '100%',
                height: browser.safari || browser.msie || browser.chrome && intval(browser.version) < 17 ? (options.flashHeight ? options.flashHeight + 'px' : (cur.flash_lite &&
            ...
            if (!options.flash_lite) {
                ...
                if (options.flat_button) {
                    obj.innerHTML = '<div id="uploader' + iUpload + '" style="position: relative;"><div id="lite_upload' + iUpload +
                        ...
                        options.lang['button_browse'] + '</button></div>';
                } else {
                    obj.innerHTML = '<button id="uploader' + iUpload + '" class="flat_button upload_btn ' + (options.buttonClass || '') +
                        '" style="position: relative;"><div id="lite_upload' + iUpload +
                        '" style="position: absolute; height: 100%; width: 100%; z-index: 10000; cursor: pointer;"></div>' + options.lang['button_browse'] + '</button>';
                }
            } else {
        ...
        initForm: function(iUpload, obj) {
            ...
            var html = '<form action="' + this.uploadUrls[_iu] + '" enctype="multipart/form-data" method="post" id="file_uploader_form' + _iu + '" target="upload_iframe' +
                _iu + '" style="text-align: center; width: 100%;">';
            if (options.signed) {
                extend(vars, {
                    _jsonp: jsonpManager.reg(function(r) {
            ...
            var input = '<input type="file" class="file" size="28" onchange="' + onInpChange + '" name="' + options['file_name'] + '" style="cursor: pointer;"' + (options.accept ?
                    ' accept="' + options.accept + '"' : '') +
                '/>\
  \
  </form><iframe style="position: absolute; visibility: hidden; width: 1px; height: 1px;" id="upload_iframe' + _iu + '" name="upload_iframe' + _iu +
                '"></iframe>';
            if (options.label) {
                html += options.label.split('{file}')
                    .join(input);
            } else {
                html += input;
            }
        ...
        buttonOver: function(iUpload) {
            ...
            if (options.flash_lite) {
                ...
                addClass(ge('lite_upload' + iUpload)
                    .nextSibling, 'hover');
            }
        },
        buttonOut: function(iUpload) {
            ...
            if (options.flash_lite) {
                ...
                removeClass(ge('lite_upload' + iUpload)
                    .nextSibling, 'hover');
                removeClass(ge('lite_upload' + iUpload)
                    .nextSibling, 'active');
            }
        ...
        initFileApi: function(iUpload, obj, dropbox) {
            ...
            if (options.chooseBox) {
                input = '<input class="file" type="file" size="28" onchange="Upload.onFileApiSend(' + iUpload + ', this.files);"' + (options.multiple ? ' multiple="true"' :
                    '') + (options.accept ? ' accept="' + options.accept + '"' : '') + ' name="' + options['file_name'] + '" style="cursor: pointer;"/>';
            } else if (options.uploadButton) {
                if (!options.lang['button_browse']) {
                    options.lang['button_browse'] = '������� ����';
                }
                input = '<button class="flat_button upload_btn fl_l ' + (options.buttonClass || '') + '" onclick="this.nextSibling.nextSibling.click()">' + options.lang[
                        'button_browse'] +
                    ...
                    iUpload + ', this.files); Upload.onUploadFileSelected(' + iUpload + ')"' + (options.multiple ? ' multiple="true"' : '') + (options.accept ? ' accept="' +
                        options.accept + '"' : '') + ' name="' + options['file_name'] + '" style="visibility: hidden; position: absolute;"/><br class="clear" />';
            } else {
                if (!options.lang['button_browse']) {
                    options.lang['button_browse'] = '������� ����';
                }
                if (options.flat_button) {
                    input = '<button class="upload_btn flat_button button_big_width button_big ' + (options.buttonClass || '') + '" onclick="this.nextSibling.click()">' +
                        options.lang['button_browse'] + '</button><input class="file" type="file" size="28" onchange="Upload.onFileApiSend(' + iUpload + ', this.files);"' +
                        (options.multiple ? ' multiple="true"' : '') + (options.accept ? ' accept="' + options.accept + '"' : '') + ' name="' + options['file_name'] +
                        '" style="visibility: hidden; position: absolute;"/>';
                } else {
                    input = '<button class="flat_button upload_btn ' + (options.buttonClass || '') + '" onclick="this.nextSibling.click()">' + options.lang['button_browse'] +
                        '</button><input class="file" type="file" size="28" onchange="Upload.onFileApiSend(' + iUpload + ', this.files);"' + (options.multiple ?
                            ' multiple="true"' : '') + (options.accept ? ' accept="' + options.accept + '"' : '') + ' name="' + options['file_name'] +
                        '" style="visibility: hidden; position: absolute;"/>';
                }
            }
            if (options.label) {
                obj.innerHTML = options.label.split('{file}')
                    .join(input);
            } else {
                obj.innerHTML = input;
            }
            ...
            if (options.uploadButton && options.setUploadAction) {
                options.setUploadAction(iUpload, function() {
                    Upload.onFileApiSend(iUpload, geByTag1('input', obj)
                        .files);
                });
            } else if (options.uploadButton) {
                ge(options.uploadButton)
        ...
        onUploadError: function(info, result) {
            ...
            if (options.signed) {
                if (options.onUploadComplete) {
                    options.onUploadComplete(info, '{"error":"ERR_CLIENT_UPLOAD_FAIL: upload request fail, code \\"' + result.replace(/([\\\"])/g, '\\$1')
        ...
        checkFileType: function(filename, fileTypes) {
            ...
            each(fileTypes.split(';'), function(i, type) {
                type = type.substr(1)
                    .toLowerCase();
                if (filename.substr(-type.length)
        ...
        onFileApiSend: function(i, files, force) {
            ...
            if (!options.multi_progress) this.onUploadStart({
                ind: i,
                fileName: (files[0].fileName || files[0].name || '')
                    .replace(/[&<>"']/g, '')
            });
            ...
            if (options.max_files) {
                ...
                if (options.max_files - curCount < files.length && options.lang && options.lang.max_files_warning) {
                    ...
                    if (options.force_max_files) {
                        max_files = Math.min(max_files, options.max_files - (cur.savedPhotos || [])
                            .length);
                    }
        ...
        onPhotoAdd: function(i, files) {
            ...
            each(files, function(n, file) {
                ...
                var photo = ce('div', {
                    ...
                    innerHTML: '<span class="photos_add_img photos_add_wait" id="photos_add_cont' + photoId + '" onmouseover="PhotosAdd.thumbOver(' +
                        photoId + ', this);" onmouseout="PhotosAdd.thumbOut(' + photoId + ');"><span class="photos_add_s" id="photos_add_s' + photoId +
        ...
        uploadFileChunked: function(uplId, file, url) {
            ...
            function _uploadChunk(pointerStart, pointerEnd) {
                var chunk = (file.slice || file.webkitSlice || file.mozSlice)
                    .call(file, pointerStart, pointerEnd + 1);
