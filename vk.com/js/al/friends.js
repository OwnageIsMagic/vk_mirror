var Friends = {
    ...
    showMore: function(clear, start, end, plain) {
        ...
        if (cur.section == 'requests') {
            if (cur.suggRev) {
                list = (cur.friendsList['sugg_requests'] || [])
                    .slice();
        ...
        if (!list || !list.length) {
            if (cur.shownFriends == 0 && !cur.searchCount) {
                ...
                if (cur.searchStr) {
                    ...
                    ge('search_ph')
                        .innerHTML = cur.searchStr.replace(/([<>&#]*)/g, '');
                } else {
                ...
                if (cur.curList.substr(0, 4) == 'list') {
                    if (cur.filterIds) {
                        ...
                        text = text.replace('{link}', '<a onclick="Friends.clearFilter(true);">')
                            .replace('{/link}', '</a>');
                    } else {
                        text = cur.summaryLang['list_not_found'];
                        text = text.replace('{link}', '<a onclick="Friends.editList(-1);">')
                            .replace('{/link}', '</a>');
                    }
                } else if (cur.filter) {
            ...
            if (suggSwitch && suggSwitch == iSt) {
                if (cur.suggRev) {
                    ...
                    if (nav.objLoc.sort != 'date') {
                        summaryText += '<span class="divide">|</span><span><a href="/friends?section=' + cur.section + '&sort=date">' + cur.summaryLang[
                            'friends_sort_by_date'] + '</a></span>';
                    } else {
                        summaryText += '<span class="divide">|</span><span><a href="/friends?section=' + cur.section + '">' + cur.summaryLang['friends_sort_by_common'] +
                            '</a></span>';
                    }
                } else {
        ...
        cur.fContent.appendChild(ce('div', {
            innerHTML: html.join('')
        }));
        if (cur.shownFriends >= cur.sectionCount) {
            hide('show_more');
    ...
    showSection: function(section) {
        ...
        if (vk.id == cur.oid) {
            if (cur.section == 'requests' || cur.section == 'all_requests' || cur.section == 'out_requests') {
                ...
                toggle(ge('sum_tab_requests')
                    .parentNode, cur.requestsCount > 0 || cur.suggCount > 0);
                toggle(ge('sum_tab_all_requests')
                    .parentNode, cur.allRequestsCount > 0);
                toggle(ge('sum_tab_out_requests')
                    .parentNode, cur.outRequestsCount > 0);
                show('tab_requests');
    ...
    search: function(str, type, callback, filter) {
        ...
        if (str) {
            ...
            if (str) {
                ...
                cur.selection = {
                    re: new RegExp('(' + str.replace(index.delimiter, '|')
                        .replace(/(^\||\|$|\?)/g, '') + ')', 'gi'),
                    val: '<em>$1</em>'
                };
            }
    ...
    changeSummary: function() {
        ...
        if ((cur.section == 'requests' && cur.requestsCount > 50) || cur.section == 'all_requests') {
            if (!cur.suggRev || !cur.suggCount) {
                if (nav.objLoc.sort != 'date') {
                    html += '<span class="divide">|</span><span><a href="/friends?section=' + cur.section + '&sort=date">' + cur.summaryLang['friends_sort_by_date'] +
                        '</a></span>';
                } else {
                    html += '<span class="divide">|</span><span><a href="/friends?section=' + cur.section + '">' + cur.summaryLang['friends_sort_by_common'] +
                        '</a></span>';
                }
            }
            ...
            for (var i in cur.friendsList[cur.curList]) {
                ...
                if (row[0] == mid) {
                    ...
                    var block = ge('user_block' + mid),
                        link = '';
                    if (invited) {
                        link = '<a href="" onclick="return Friends.inviteToGroup(event, ' + gid + ', ' + mid + ', 1, \'' + row[11] + '\')">' + getLang(
                            'friends_cancel_invite') + '</a>';
                    } else {
                        link = '<a href="" onclick="return Friends.inviteToGroup(event, ' + gid + ', ' + mid + ', 0, \'' + row[11] + '\')">' + getLang(
                            'friends_send_invite') + '</a>';
                    }
    ...
    processRequest: function(mid, add, sugg) {
        ...
        while (len--) {
            if (reqs[len][0] == mid) {
                ...
                val(geByClass1('tab_word', ge('tab_requests')), cur.summaryLang['requests_title'] + (cur.requestsCount ? ('<span class="count">+' + cur.requestsCount +
                    '</span>') : ''));
    ...
    onRemoveFriend: function(mid, res) {
        var needUpdateView = (cur.friendsList[cur.curList] || [])
            .length < 10;
        ...
        if (fr && block) {
            ...
            block.parentNode.replaceChild(ce('div', {
                    innerHTML: cur.friendsTpl(fr, cur.section, hasClass(block, 'user_block_first') ? ' user_block_first' : '')
                        .join('')
                })
                .firstChild, block);
        } else {
    ...
    ttActsMenu: function(obj, ev, uid, hash) {
        ...
        html.push('<a class="friends_tt_item" onclick="return showWriteMessageBox(event, ' + uid + ')"><div class="friends_tt_icon friends_tt_msg"></div>' + cur.summaryLang[
            'global_write_msg'] + '</a>');
        if (info) {
            html.push('<a class="friends_tt_item" onclick="return Friends.ttActToggle(this, ' + uid + ', \'' + hash +
                '\', 0);"><div class="friends_tt_icon friends_tt_cancel"></div>' + cur.summaryLang['friends_add_cancel'] + '</a>');
        } else if (!cur.friends[uid]) {
            html.push('<a class="friends_tt_item" onclick="return Friends.ttActToggle(this, ' + uid + ', \'' + hash +
                '\', 1);"><div class="friends_tt_icon friends_tt_add"></div>' + cur.summaryLang['friends_add_act'] + '</a>');
        }
        html.push('<a class="friends_tt_item" onclick="return Friends.ttActGift(' + uid + ');"><div class="friends_tt_icon friends_tt_gift"></div>' + cur.summaryLang[
            'friends_gift_act'] + '</a>');
        html.push('</div>');
        showTooltip(obj, {
            text: html.join(''),
    ...
    ddShow: function(uid, obj, ev) {
        ...
        for (var j = 0, i; j < 5; ++j) {
            ...
            if (cur.publicLists[i]) {
                elems.push('<a class="friends_dd_item' + ((cats & (1 << parseInt(i))) ? ' checked' : '') + '" onclick="Friends.checkCat(this, ' + uid + ', ' + i + ');">' +
                    cur.publicLists[i] + '</a>');
            }
        }
    ...
    ddShowCustom: function(obj, event, elems, ddId) {
        ...
        var htmlH = ['<div id="friends_dd_menu_', ddId, '" onmouseout="Friends.ddHide(this, ', ddId, ');" onmouseover="Friends.ddActive(this, ', ddId,
            ');"><div class="lists_header', headerAdd, '"><div>', text, '</div></div><div class="lists_body" ', styleAdd,
        ...
        var htmlF =
            '</div></div><div class="lists_shad_b"></div><div class="lists_shad_b2"></div></td><td class="lists_shad_r"><div> </div></td></tr></tbody></table></div></div>';
    ...
    onAllRequestsRemove: function() {
        return nav.go('/friends?section=all_requests', false, {
            nocur: true
        });
    },
    ...
    editListClient: function(listId, id, title, friendsList) {
        ...
        if (listId == 0) {
            var html = '<a onmousedown="return nav.change({\'0\':\'al_friends.php\', section:\'list' + id + '\'});" id="section_list' + id + '" class="side_filter">' +
                title + '</a>';
            ge('sections_block')
                .insertBefore(ce('div', {
                    innerHTML: html
    ...
    searchForFriends: function() {
        showBox('al_friends.php', {
            ...
            stat: ['friends_search.js', 'friends_search.css'],
            dark: 1
        });
    },
    ...
    hideSuggestion: function(mid, hash, el) {
        ...
        var controlsCont = controls.parentNode;
        (el || controls)
        .innerHTML = '<div align="center"><img src="/images/upload.gif"></div>';
        ajax.post('al_friends.php', {
            act: 'hide_suggestion',
            mid: mid,
            hash: hash,
    ...
    bigphOver: function(obj, uid) {
        ...
        if (o.tagName != 'A' || o.className != 'friends_bigph') {
            (o = obj.insertBefore(ce('a', {
                className: 'friends_bigph',
                href: ch && ch._id ? ('/photo' + ch._id + '?all=1') : ('/albums' + uid),
                innerHTML: '<span class="friends_bigph_label">' + getLang('global_photo_full_size') + '</span>'
            }), obj.firstChild))
            .onclick = Friends.bigphClick.pbind(uid);
            o._uid = uid;
        }
...
extend(Friends, {
    ...
    activeDeleteRow: function(post, tt) {
        ...
        if (tt) showTooltip(ge('delete_row' + post), {
            text: tt,
            showdt: 500,
            black: 1
        });
