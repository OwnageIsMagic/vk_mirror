var SiteStats = {
    toggleSmallGraphs: function(e, s) {
        ...
        removeClass(t, "selected"), addClass(e.target, "selected"), nav.setLoc(e.target.getAttribute("href")
            .replace(/^\//, "")), each(geByClass("site_stats_small_graphs", ge("site_stats_page")), hide), each(geByTag("img", ge("site_stats_small_graphs_" + s)),
            function(e, s) {
    ...
    updateReferrersTable: function(e) {
        return e ? (e = extend(cur.statsTableParams || {}, e), e.act = "referrers", e.upd = 1, ajax.post("site_stats.php", e, {
            onDone: function(e) {
                cur.statsTableParams = e.params, ge("site_stats_navigation")
                    .innerHTML = e.nav, "visitors" == e.params.value_type ? (removeClass("table_value_type1", "graph_menu_item_sel"), addClass(
                        "table_value_type2", "graph_menu_item_sel")) : (removeClass("table_value_type2", "graph_menu_item_sel"), addClass(
                        "table_value_type1", "graph_menu_item_sel"));
                var s = "keywords" == e.params.page ? ge("searchers_sel") : ge("sources_sel");
                if (s) {
                    var a = geByTag("a", s);
                    e.params.searcher = e.params.searcher || 0;
                    for (var t in a) t == e.params.searcher ? addClass(a[t], "site_stats_tbl_control_active") : removeClass(a[t],
                        "site_stats_tbl_control_active")
                }
                var r = cur.statsTable._curSortOrder,
                    n = cur.statsTable.curPage;
                cur.statsTable.setOptions(e.options), cur.statsTable.setContent(e.content), cur.statsTable.applyData(), cur.statsTable._initSearchHashes(),
                    void 0 !== r && cur.statsTable.setSortingOrder(r[0], r[1]), cur.statsTable.goToPage(n);
                var o = nav.objLoc;
