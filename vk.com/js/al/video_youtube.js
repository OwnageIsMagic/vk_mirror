...
<div id="video_yt_ui" class="no_select">\
  <!--div id="video_yt_poster" style="background-image: url(' + vars.jpg +
            ')"></div-->\
\
...
\
  <div id="video_yt_actions" class="clear_fix hidden">\
    <div id="video_yt_like" class="fl_l video_yt_like ' +
            (vars.liked ? 'selected' : '') +
            '" onmouseover="VideoYoutube.onLikeOver(this)" onmouseout="VideoYoutube.onLikeOut(this)" onclick="VideoYoutube.onLikeClick(this)">\
    ...
    <div id="video_yt_add" class="fl_l video_yt_add ' +
            (vars.added ? 'selected' : '') +
            '" onmouseover="VideoYoutube.onAddOver(this)" onmouseout="VideoYoutube.onAddOut(this)" onclick="VideoYoutube.onAddClick(this, 1)" style="' + (!vars.can_add ?
                'display:none' : '') +
            '">\
    ...
    <table cellspacing="0" cellpadding="0" border="0" id="video_yt_controls_table">\
      <tr>\
        ...
        <td style="padding-left:14px" id="video_yt_time_td">\
          <div id="video_yt_time_label" class="video_yt_time_label ' +
            (VideoYoutube.cur.reverseTime ? 'video_yt_time_label_reversed' : '') +
            '" onclick="VideoYoutube.onTimeLabelClick(this)">\
            <span id="video_yt_time_current" class="video_yt_time_current">' + curTime +
            '</span>\
            <span id="video_yt_time_duration" class="video_yt_time_duration"> / ' + duration +
            '</span>\
          </div>\
        </td>\
        ...
        <td style="padding-left:9px;' +
            (fullscreenAvailable ? '' : 'display:none') +
            '">\
...
\
  <div id="video_yt_popup" class="hidden" onclick="VideoYoutube.togglePlay()" ondblclick="VideoYoutube.toggleFullscreen()">\
    ...
    <div class="video_yt_popup_title" style="' +
            (vars.no_title ? 'display:none' : '') + '">' + vars.title + '</div>\
    <a href="' + vars.author_href +
            '" target="_blank" onclick="event.stopPropagation()" class="video_yt_popup_author">' + vars.author_name +
            '</a>\
  </div>\
\
  <div id="video_yt_tip">\
    ...
    getQualityItemsHtml: function(qualities) {
        ...
        each(qualities, function(i, quality) {
            ...
            qualityItemsHtml += '\
<div class="video_yt_quality_item ' + (quality == 'auto' ? 'selected' : '') + '" onclick="VideoYoutube.setQuality(\'' + quality +
                '\', this)">\
  <span class="video_yt_quality_item_text">' + qualityValue + '</span>\
  ' + (hdLabel ? '<span class="video_yt_quality_item_' +
                    hdLabel + '_label video_yt_icon"></span>' : '') + '\
</div>\
      ';
        });
    ...
    setQuality: function(res, btn) {
        ...
        each(ge('video_yt_quality_dropdown')
            .children,
            function(i, item) {
    ...
    updateProgress: function(newTime) {
        ...
        ge('video_yt_time_current')
            .innerHTML = VideoYoutube.formatTime(reverseTime ? (duration - curTime) : curTime);
        ge('video_yt_time_duration')
            .innerHTML = ' / ' + VideoYoutube.formatTime(duration);
    },
    ...
    updateQualityLabel: function(quality) {
        ...
        ge('video_yt_quality_btn_value')
            .textContent = qualityValue;
    ...
    onQualityLabelOut: function() {
        VideoYoutube.cur.hideQualityDDTimer = setTimeout(function() {
            ...
            if (VideoYoutube.cur.isFS && VideoYoutube.cur.state == YT.PlayerState.PLAYING && new Date()
                .getTime() - VideoYoutube.cur.lastMouseMove > 2000) {
                VideoYoutube.toggleControls(false);
            }
        }, 3000);
    },
    ...
    onVolumeDrag: function(event, dontPrevent) {
        ...
        if (browser.msie8) {
            ...
            var volumeRect = ge('video_yt_volumebar')
    ...
    onMouseMove: function(event) {
        clearTimeout(VideoYoutube.cur.hideControlsTimeoutId);
        VideoYoutube.cur.lastMouseMove = new Date()
            .getTime();
    ...
    if (fullscreenApi.supportsFullscreen) {
        ...
        fullscreenApi.fullscreenElement = function() {
            ...
            fullscreenApi.requestFullscreen = function(el) {
                return (this.prefix === '') ? el.requestFullscreen() : (el[this.prefix + 'RequestFullscreen'] || el[this.prefix + 'RequestFullScreen'])
                    .call(el);
            }
