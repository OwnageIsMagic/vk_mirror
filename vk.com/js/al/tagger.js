    ...
    var _, L, I, X, Y, H, O, q, F = {},
        ...
        k = function(t, e) {
            j = extend(j, t), each(t, function(t) {
                ...
                if (e.length < 2) "n" == e || "s" == e ? (this.style.left = h + t.left + intval(t.width / 2) - 5 + "px", this.style.top = a + t.top + ("n" == e ? 0 : t.height) -
                    5 + "px") : (this.style.left = h + t.left + ("w" == e ? 0 : t.width) - 5 + "px", this.style.top = a + t.top + intval(t.height / 2) - 5 + "px");
                else {
                    var i = e.charAt(0),
                        n = e.charAt(1);
                    this.style.left = h + t.left + ("w" == n ? 0 : t.width) - 5 + "px", this.style.top = a + t.top + ("n" == i ? 0 : t.height) - 5 + "px"
                }
            }), e || (r && (r.style.width = m(50 * R / t.width) + "px", r.style.height = m(50 * W / t.height) + "px", r.style.marginLeft = -u(50 * t.left / t.width) + "px", r.style
                .marginTop = -u(50 * t.top / t.width) + "px"), o && (o.style.width = m(100 * R / t.width) + "px", o.style.height = m(100 * W / t.height) + "px", o.style.marginLeft = -
        ...
        U = function(i) {
            return q = Q(t), X = P(i), O = extend({}, j), d = l = c = !1, i.target == L ? B = 1 : i.target == I || i.target == t ? B = 2 : i.target == r || i.target == o ? (B = i.target,
                j.width && j.height || k({
                    left: 0,
                    top: 0,
                    width: w(100, w(R, W)),
        ...
        V = function(e) {
            ...
            if (1 == B) {
                ...
                2 == B.length ? (x = "n" == B.charAt(0) ? -1 : 1, E = "w" == B.charAt(1) ? -1 : 1) : (x = "n" == B ? -1 : "s" == B ? 1 : 0, E = "w" == B ? -1 : "e" == B ? 1 : 0),
                    x && f + y(x) * c < S / 2 && (x = -x, X[1] = j.top + (x > 0 ? j.height : 0), c = n[1] - X[1]), E && g + y(E) * l < N / 2 && (E = -E, X[0] = j.left + (E > 0 ? j
                        .width : 0), l = n[0] - X[0]), vsign = x ? y(x) : 0, hsign = E ? y(E) : 0, b > 0 && g + hsign * l < (f + vsign * c) * b && (E ? l = hsign * m((f + vsign *
                        c) * b - g) : c = vsign * u(g / b - f)), A > 0 && g + hsign * l > (f + vsign * c) * A && (x ? c = vsign * m((g + hsign * l) / A - f) : l = hsign * u(f * A -
                        g)), x && (f += y(x) * c, S > f ? (a -= x > 0 ? 0 : S - f - c, f = S) : a += x > 0 ? 0 : c), E && (g += y(E) * l, N > g ? (h -= E > 0 ? 0 : N - g - l, g =
                        N) : h += E > 0 ? 0 : l);
                var C = 0,
                    M = 0,
                    T = 0,
                    L = 0;
                0 > h ? (C = h, h = 0) : g > R - h && (C = R - h - g), C && (g += C, b > 0 && b * f > g && (L = u(g / b) - f, f += L, a -= x > 0 ? 0 : L)), 0 > a ? (M = a, a = 0) :
                    f > W - a && (M = W - a - f), M && (f += M, A > 0 && g > A * f && (T = u(f * A) - g, g += T, h -= E > 0 ? 0 : T)), k({
                        left: h,
                        top: a,
                        width: g,
                        height: f
                    }), X[0] = j.left + (E > 0 ? j.width : 0), X[1] = j.top + (x > 0 ? j.height : 0), x = x > 0 ? "s" : 0 > x ? "n" : "", E = E > 0 ? "e" : 0 > E ? "w" : "", B !=
                    x + E && (B = x + E, K())
            }
            return 3 == B && (n[0] -= q[0], n[1] -= q[1], n[0] = w(R, p(0, n[0])), n[1] = w(W, p(0, n[1])), K((y((X[0] - n[0]) * (X[1] - n[1]) * (.5 - z % 2)) > 0 ? "nw" : "ne") +
                "-resize"), k({
        ...
        Z = function(h) {
            ...
            return show(_, I), each(F, function() {
                    fadeTo(this, 200, .3)
                }), hide(s), B = 0, removeEvent(bodyNode, "mousemove", V), removeEvent(bodyNode, "mouseup", Z), removeEvent(bodyNode, "dragend", Z), e.onFinish && e.onFinish(),
                cancelEvent(h)
        };
    return function() {
        if (R = T.width, W = T.height, !R || !W) return void(++g < 50 && setTimeout(arguments.callee, 100));
        var d = getSize(t);
        if (R = d[0], W = d[1], f.style.position = "relative", h = t.offsetLeft, a = t.offsetTop, C = w(R, C), i = intval(C / 2), M = w(W, M), n = intval(M / 2), N = w(N, C),
            S = w(S, M), b > 0 && S * b > N ? N = m(S * b) : A > 0 && N > S * A && (S = m(N / A)), E && (e.preview50 && (r = ge(e.preview50)
                .appendChild(ce("img", {
                    src: t.src
                })), addEvent(r, "mousedown", U)), e.preview100 && (o = ge(e.preview100)
                .appendChild(ce("img", {
                    src: t.src
        ...
        resize: function(e, r) {
            ...
            var o = e / d,
                s = r / l;
            R = e, W = r, C = w(R, C), i = intval(C / 2), M = w(W, M), n = intval(M / 2), N = w(N, C), S = w(S, M), b > 0 && S * b > N ? N = m(S * b) : A > 0 && N > S * A &&
                (S = m(N / A)), h = t.offsetLeft, a = t.offsetTop, setStyle(I, {
                    left: h,
                    top: a,
                    width: e,
                    height: r
                    ...
                    width: e,
                    height: r
                }), j.width && (j.left = u(o * c.left), j.width = u(o * c.width), j.top = u(s * c.top), j.height = u(s * c.height), j.width < N && (j.width = N), j.height <
                    S && (j.height = S), k(j))
        },
        rotate: function(t) {
            if (t % 2) {
                var e = b;
                b = 1 / A, A = 1 / e, e = N, N = S, S = e, j.width && (j.width < N && (j.left = p(0, j.left - u((N - j.width) / 2)), j.width = N), j.height < S && (j.top =
                    p(0, j.top - u((S - j.height) / 2)), j.height = S), j.width < j.height * b && (j.height = u(j.width / b)), j.width > j.height * A && (j.width =
                    u(j.height * A)), k(j))
            }
