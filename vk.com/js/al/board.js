var Board = {
    createTopic: function(oid, hash) {
        var title = trim(val('bnt_title')),
            ...
            question, answers = [];
        var chosen = cur.boardNewMedia.chosenMedias || {},
            media = [],
            types = [];
        ...
        if (cur.pollPreview) {
            ...
            for (var el = ge('bnt_poll_answers')
                    .firstChild; el; el = el.nextSibling) {
            ...
            if (!answers.length) {
                return elfocus(ge('bnt_poll_answers')
                    .firstChild);
            }
        }
    ...
    attachPoll: function() {
        ...
        cur.pollPreview = prev.parentNode.insertBefore(ce('div', {
            id: 'bnt_poll',
            innerHTML: '\
<div class="bnt_header">' + getLang('voting_topic') +
                '</div>\
...
<nobr class="no_select fl_r" onselectstart="return false;" ondblclick="return false;">\
  <a id="bnt_poll_add" onclick="Board.incPoll(\'bnt\')">' +
                getLang('voting_variants_add') +
                '</a><span class="sdivide">|</span><a id="bnt_poll_delete" class="bnt_disabled" onclick="Board.decPoll(\'bnt\')">' + getLang(
                    'voting_variants_del') + '</a>\
</nobr>\
<div class="bnt_header">' + getLang('voting_variants') +
                '</div>\
<div id="bnt_poll_answers"></div>\
    ...
    incPoll: function(prefix) {
        ...
        if (el) {
            ge(prefix + '_poll_delete')
                .className = '';
            show(el);
        }
        if (!el || !el.nextSibling) {
            ge(prefix + '_poll_add')
                .className = prefix + '_disabled';
        }
    },
    decPoll: function(prefix) {
        var answers = ge(prefix + '_poll_answers'),
            first = answers.firstChild;
    ...
    topicAction: function(key) {
        ge('privacy_edit_topic_action')
            .innerHTML = getLang('global_edit');
        var act = cur.privacy[key][0];
        cur.privacy[key] = [0, 0, []];
        if (act == 'edit') {
            showFastBox(getLang('board_edit_topic'), '\
<div class="bte_label fl_l">' + getLang('board_new_topic_title') +
                '</div>\
<div class="bte_labeled fl_l">\
  <input id="bte_title" class="text" type="text" onchange="curBox().changed = true;" onkeypress="if (event.keyCode == 10 || event.keyCode == 13) Board.saveTopic()" />\
      ...
      ',
            ...
            showBox('al_board.php', {
                act: 'create_voting',
                topic: cur.topic
            });
        } else if (act == 'delete') {
            showFastBox(getLang('board_edit_topic'), '<div>' + getLang('board_sure_delete_topic') + '</div>', getLang('global_delete'), Board.deleteTopic, getLang(
                'global_cancel'));
        } else if (act == 'open' || act == 'close') {
            ajax.post('al_board.php', {
                act: (act == 'open') ? 'open_topic' : 'close_topic',
                topic: cur.topic,
                hash: cur.hash,
            ...
            var t = cur.topic;
            ajax.post('al_board.php', {
                act: (act == 'fix') ? 'fix_topic' : 'unfix_topic',
                topic: t,
                hash: cur.hash
            }, {
    ...
    initTopic: function(opts, preload) {
        ...
        if (cur.addField) {
            cur.boardAddMedia = initAddMedia(ge('ba_add_media')
                .firstChild, 'ba_media_preview', cur.mediaTypes, {
                    limit: 10,
                    editable: 1,
                    sortable: 1,
                    teWidth: 280,
    ...
    getAbsDate: function(ts) {
        ...
        return cur.updates.date_format.replace('{am_pm}', ampm)
            .replace('{hour}', hours)
    ...
    topicOnScroll: function(e, st, pp) {
        ...
        if (cur.docked && e && e.type == 'resize') {
            if (browser.msie6) {
                ...
                cur.addBlock.style.left = (ge('page_layout')
    ...
    loadedPosts: function(count, from, rows, offset, pages, preload, names) {
        Board.topicOnScroll(false, false, true);
        ge('bt_summary')
            .innerHTML = count ? getLang('board_msgs_in_topic', count) : getLang('board_no_msgs_in_topic');
    ...
    sendPost: function(ev, stickerId) {
        ...
        var last = ((cur.pgCont.childNodes[cur.pgNodesCount - 1].id || '')
            .match(/\d+$/) || [0])[0];
  ...
  ' + text +
    ...
    editPost: function(post) {
        ...
        var bottom = geByClass1('bp_bottom', postNode),
            prg = geByClass1('bp_progress', bottom);
        ajax.post('al_board.php', {
            act: 'edit_comment',
            post: post
        }, {
            onDone: function(text, media, add) {
                ...
                var textField = dataNode.parentNode.insertBefore(ce('div', {
                        className: 'bpe_wrap',
                        innerHTML: '\
<textarea id="bpe_text" onkeyup="Board.topicOnScroll(false, false, true); checkTextLength(cur.postLimit, this, \'bpe_warn\')" onkeypress="onCtrlEnter(event, Board.savePost)">' +
                            text + '</textarea>\
<div id="bpe_warn"></div>\
...
<div class="bpe_buttons">\
  <div id="bpe_add_media" class="fl_r"><span class="add_media_lnk">' + getLang(
                                'board_add_attach') + '</span></div>\
  ...
  <div class="progress fl_l" id="bpe_progress"></div>\
                ...
                setTimeout(function() {
                    ...
                    cur.boardEditMedia = initAddMedia(ge('bpe_add_media')
                        .firstChild, 'bpe_media_preview', cur.mediaTypes, {
                            limit: 10,
                            editable: 1,
                            sortable: 1,
                            teWidth: 280,
    ...
    votingUpdate: function(html, js) {
        ...
        if (html) {
            ...
            var newTypes = {
                edit: cur.privacy.topic_action_types.edit,
                voting: getLang('board_create_voting')
            };
    ...
    vote: function(el, oid, vid, option) {
        var pr = hasClass(el.firstChild, 'progress') ? el.firstChild : el.insertBefore(ce('span', {
    ...
    votingActionPerform: function(act, sure) {
        if (act == 105) {
            if (sure) {
                ...
                return showFastBox(getLang('global_warning'), getLang('voting_sure_del_quiz'), getLang('global_delete'), Board.votingActionPerform.pbind(act, true),
