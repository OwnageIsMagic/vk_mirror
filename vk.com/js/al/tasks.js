Tasks = {
    ...
    getPage: function(offset) {
        ...
        var _n = nav.objLoc,
            act = (cur.act == 'viewbug') ? 'get_comments' : cur.act;
    ...
    search: function() {
        ...
        if (q) {
            newLoc = {
                0: nav.objLoc[0],
                act: (cur.act == 'comments' ? cur.act : 'bugs'),
                q: q
            };
        } else {
            newLoc = {
    ...
    saveRedmineKey: function() {
        ...
        ajax.post(Tasks.address, {
            ...
            onDone: function(result) {
                var el = se('<div class="' + (result ? 'ok_msg' : 'error') + '" style="text-align:center">' + (result ? getLang('global_done') : getLang(
                    'global_error')) + '</div>');
                var next = domNS(ge('redmine_key'));
                if (next) {
                    domPN(ge('redmine_key'))
    ...
    saveTask: function(bid, hash) {
        ...
        if (limit_type == -1) {
            query.limit_date = ge('custom_date')
                .value;
        }
        if (!bid) {
    ...
    deleteTask: function(bid, hash) {
        var box = showFastBox({
            ...
            onHide: function() {
                if (cur.deleted) nav.go(Tasks.address);
            }
    ...
    editPriority: function(bid, hash) {
        ...
        }, getLang('tasks_sure_increase_priority'), getLang('tasks_increase'), function(btn) {
            ajax.post(Tasks.address, {
                act: 'edit_priority',
                bid: bid,
                hash: hash
    ...
    addComment: function(hash) {
        show('tasks_loading');
        var comment = ge('task_reply_field')
            .value;
        if (!comment) return;
        ajax.post(this.address, {
            act: 'add_comment',
            ...
            onDone: function(comments) {
                ...
                ge('task_reply_field')
                    .blur();
                autosizeSetup(ge('task_reply_field'), {
                    minHeight: 70,
                    maxHeight: 500
                });
                ge('answer_to')
    ...
    replyViewComment: function(uid, name) {
        ge('answer_to')
            .innerHTML = '<a onclick="return Tasks.cancelReplyComment();">' + name + '</a>';
        cur.replyTo = uid;
        return false;
    },
    ...
    editComment: function(cid, hash) {
        ...
        ajax.post(Tasks.address, {
            ...
            onDone: function(t) {
                cont.innerHTML = '<textarea class="tasks_edit_comment" id="comment' + cid + 'edit" onkeydown="Tasks.saveComment(event, ' + cid + ', \'' + hash +
                    '\')">' + t +
                    '</textarea>\
       <div style="margin: 5px 0 0 -2px; height: 23px">\
         <div class="fl_l button_blue">\
          <button id="save_but' +
                    cid + '" onclick="Tasks.doSaveComment(' + cid + ', \'' + hash + '\')">' + cur.lang.save +
                    '</button>\
         </div>\
         <div class="fl_l button_gray" style="margin-left: 6px;">\
          <button id="cancel_but' + cid +
                    '" onclick="Tasks.cancelEditComment(' + cid + ')">' + cur.lang.cancel + '</button>\
         </div>\
         <div id="editCommentProgress' +
                ...
                autosizeSetup(ge('comment' + cid + 'edit'), {
                    minHeight: 30
                });
                cur.editing = cid;
            }
    ...
    cancelEditComment: function(cid) {
        geByClass1('tasks_comm_text', ge('comment' + cid))
            .innerHTML = cur.comments[cid];
    ...
    deleteComment: function(bid, cid, hash) {
        ...
        ajax.post(Tasks.address, {
            ...
            onDone: function(res) {
                var cont = (cur.act == 'viewbug') ? ge('comment' + cid)
                    .firstChild : ge('comment' + bid + '_' + cid)
                    .firstChild;
                if (cont) {
            ...
            ajax.post(Tasks.address, {
                ...
                onDone: function(comments, links) {
                    ...
                    if (!cont) {
                        ...
                        ge('bug' + bid)
                            .parentNode.insertBefore(t, ge('bug' + bid)
                                .nextSibling);
    ...
    addReplyComment: function(bid, to, hash) {
        var comment = ge('reply_field' + bid)
            .value;
    ...
    checkFilter: function(option) {
        var filter_row = ge('filter_' + option),
            typeMask = cur.typeMask,
            value = cur.feedTypes[option];
        if (typeMask & (1 << value)) {
            typeMask &= ~(1 << value);
    ...
    setFilter: function(option) {
        var cont = ge('right_column'),
            filter_row = ge('filter_' + option),
            noChecked = true,
            typeMask = cur.typeMask,
            value = cur.feedTypes[option];
    ...
    showPhoto: function(url, photo_id) {
        ...
        var onload = function() {
            showFastBox({
                ...
                width: Math.min(pic.width, 1024) + 30,
                dark: 1
            }, '<img class="tasks_preview_img" src="' + url + '" />');
            hide('tasks_loading');
            ...
            onDone: function(res) {
                switch (cur.act) {
                    ...
                    case 'feed':
                        ...
                        setStyle(ge('update_hide' + full_id), {
                            opacity: 0.5
                        });
                        break;
                }
    ...
    changeTaskStatus: function(status, hash) {
        var doChangeStatus = function(status, hash, box) {
            ...
            if (cur.hideAutoanswer && ge('task_closed_autoanswer')) {
                ...
                query.answer_text = ge('task_closed_autoanswer')
                    .value;
            }
            ajax.post(Tasks.address, query, {
                onDone: function(summary, actions, comments) {
                    if (box) box.hide();
                    hide('tasks_loading');
                    if (summary) ge('task_summary')
                        .innerHTML = summary;
                    if (actions) ge('task_actions')
                        .innerHTML = actions;
                    if (comments) ge('task_comments')
                        .innerHTML = comments;
                }
            });
        }
        var sendReq = function(hash, box, start) {
            ...
            if (ge('task_closed_autoanswer_addressing_m')) {
                query.addressing_m = ge('task_closed_autoanswer_addressing_m')
                    .value;
            }
            if (ge('task_closed_autoanswer_addressing_f')) {
                query.addressing_f = ge('task_closed_autoanswer_addressing_f')
                    .value;
            }
            var attachs = [],
                chosen = cur.ticketsAutoMedia && cur.ticketsAutoMedia.chosenMedias;
            if (chosen) {
                for (var i in chosen) {
                    var att = chosen[i],
                        type = att[0],
            ...
            ajax.post(Tasks.address, query, {
                onDone: function(finished, progressWidth, newStart) {
                    animate(ge('task_status_progress'), {
                        width: progressWidth
                    }, 200);
                    if (finished) {
                        box.showProgress();
    ...
    showNotifyBox: function() {
        var box = showBox('al_friends.php', {
            ...
            params: {
                onHide: function() {
                    each(cur.flistScrollbar.destroy, function(k, f) {
                        f();
                    });
                },
                dark: 1
