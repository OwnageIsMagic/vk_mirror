var OwnerPhoto = {
    ...
    showError: function(step, err, wide) {
        ...
        switch (code) {
            ...
            case 'ERR_UPLOAD_BAD_IMAGE_SIZE':
                if (wide) {
                    msg = getLang('profile_custom_snippet_photo_error_size')
                        .replace('{width}', '537')
                        .replace('{height}', '240');
                    break;
                } else {
        ...
        msg = msg.replace('{sorry}', '<b>' + getLang('global_sorry_error') + '</b>') + '<br><a onclick="OwnerPhoto.detailsError(this);">' + getLang('global_error_details') +
            '</a><div class="unshown">Error: ' + code + (e[3] ? ('. Details: ' + e[3]) : '.') + '</div>';
        val(el, msg);
        if (isVisible(el)) {
            cssAnim(el, {
                backgroundColor: '#EEDB7F'
            }, {
                duration: 150
            }, cssAnim.pbind(el, {
                backgroundColor: '#F9F6E7' /*#F3E7AC*/
            }, {
                duration: 2000
            }));
        } else {
    ...
    showCameraSaveBtn: function() {
        ge('camera_button_yes')
            .innerHTML = getLang('profile_oph_camera_save');
        ge('camera_button_no')
            .innerHTML = getLang('profile_to_video_mode');
        this.updateCameraButtonsPos();
    },
    hideCameraSaveBtn: function(noUpdate) {
        ge('camera_button_yes')
            .innerHTML = getLang('profile_capture_image');
        ge('camera_button_no')
            .innerHTML = getLang('profile_oph_camera_back');
    ...
    hideCameraCaptureBtn: function() {
        hide(ge('camera_button_no')
            .parentNode);
        ge('camera_button_yes')
            .innerHTML = getLang('profile_no_camera_back');
        this.updateCameraButtonsPos();
    },
        ...
        cur.ownerPhotoUploadId = Upload.init('owner_photo_input', opts.url, {}, {
            ...
            onUploadComplete: function(i, res, errorAdd) {
                ...
                if (obj.error) {
                    ...
                    var txt = (res === false) ? '[FALSE]' : ((res === null) ? '[NULL]' : ((res === undefined) ? '[UNDEFINED]' : ('&laquo;' + clean(res.toString()
                        .substr(0, 1024)) + '&raquo;')));
                    OwnerPhoto.showError(1, 'ERR_CLIENT_BAD_RESPONSE: bad upload owner photo response, recv ' + txt);
                } else {
                    var url = Upload.options[cur.ownerPhotoUploadId].base_url + 'upload.php?act=owner_photo_edit&_query=' + encodeURIComponent(res) +
                        '&_origin=' + encodeURIComponent(locProtocol + '//' + locHost);

                    hide('owner_photo_upload', 'owner_photo_camera');
                    ge('owner_photo_upload_return')
                        .className = 'button_cancel inl_bl';
                    OwnerPhoto.edit({
            ...
            onUploadProgress: function(i, bytesLoaded, bytesTotal) {
                if (!ge('form' + i + '_progress')) {
                    ...
                    obj.appendChild(ce('div', {
                        innerHTML: '<div class="page_upload_progress_wrap">\
            <div id="form' + i +
    ...
    uploadReturn: function() {
        ...
        var cont = ge('owner_photo_upload'),
            wrap = ge('owner_photo_input')
        re('owner_photo_error');
        cont.insertBefore(ce('div', {
            className: 'msg',
            id: 'owner_photo_error'
        }), wrap);
    ...
    edit: function(opts) {
        ...
        val(wrap, '<div id="owner_photo_rotate"><div style="' + st + 'margin: 0px auto;"><img id="owner_photo_img" src="' + opts.thumb + '" style="' + st +
            '" onload="OwnerPhoto.editInit();" /></div></div>');
    ...
    editTagger: function() {
        ...
        var rect, mul1 = opts.square ? 1 : 1.5,
            mul2 = opts.square ? 1 : 2.5,
            div1 = opts.square ? 1 : 0.4;
        var mina = div1,
            maxa = 1;
        var minSize = [
        ...
        if (OwnerPhoto.canRotate()) {
            ...
            wrap.appendChild(ce('div', {
                id: 'owner_photo_rotate_c',
                innerHTML: '<div id="owner_photo_rotate_c1" onclick="OwnerPhoto.rotate(1)"></div><div id="owner_photo_rotate_c3" onclick="OwnerPhoto.rotate(3)"></div>'
            }));
        }
    },
    editDone: function() {
        ...
        if (opts.square || opts.no_crop) {
            ...
            clearTimeout(cur.ownerPhotoCropTimer);
            cur.ownerPhotoCropTimer = setTimeout(OwnerPhoto.cropSuccess.pbind(true, '{"error":"ERR_CLIENT_UPLOAD_TIMEOUT: no response on owner_photo_crop iframe request"}'),
                10000);
    ...
    crop: function(opts) {
        var cont = ge('owner_photo_crop'),
            wrap = ge('owner_photo_crop_thumb'),
            s = opts.thumbSize;
...
<div id="owner_photo_crop_wrap" class="clear_fix">\
  <div class="fl_l"><img id="owner_photo_crop_img" src="' + opts.thumb + '" style="width: ' + s[0] +
            'px; height: ' + s[1] +
            'px;" onload="stManager.add([\'tagger.css\', \'tagger.js\'], OwnerPhoto.cropInit)" onerror="OwnerPhoto.thumbError()" /></div>\
