var PhotosAdd = {
    ...
    genFile: function(i, oncancel, cancel) {
        return ce('div', {
            innerHTML: '\
<a class="photo_file_cancel" id="photo_cancel' + i + '" onclick="' + oncancel + '">' + cancel +
                '</a>\
<div class="photo_file_button">\
  <div class="file_button_gray">\
    <div class="file_button" id="photo_file_button' + i + '">' + getLang(
                    'photos_choose_file') + '</div>\
  </div>\
</div>\
    '
        });
...
<form target="photo_frame_all" id="photo_form_all" method="POST" action="' + cur.url +
...
<form target="photo_frame' + i + '" id="photo_form' + i + '" method="POST" action="' +
    ...
    '
        ...
        show('photo_cancel' + i);
        ge('photo_cancel' + i)
            .innerHTML = getLang('global_cancel');
    ...
    backupDesc: function(photo) {
        ...
        cur.descs[photo] = trim(ge('photo_caption' + photo)
            .value);
    },
    saveDesc: function(photo, hash) {
        var dsc = ge('photo_caption' + photo)
            .value,
            old = (cur.descs || {})[photo];
        ...
        ajax.post('al_photos.php', {
            ...
            onDone: function(text) {
                ge('photo_save_result' + photo)
                    .innerHTML = text;
            },
            onFail: function(text) {
                ge('photo_save_result' + photo)
                    .innerHTML = '<div class="photo_save_error">' + text + '</div>';
                return true;
            },
        ...
        if (cur.uplSelected) {
            if (ge('photos_add_bar')) {
                ...
                if (!cur.scrollBarFixedY) {
                    cur.scrollBarFixedY = getXY(ge('photos_add_bar')
                        .parentNode)[1];
                }
    ...
    thumbOver: function(obj, photo) {
        ...
        if (!control) { // create controls
            control = ce('div', {
                ...
                innerHTML: '<div class="photos_add_c_bar"><a class="photos_add_rl" onclick="PhotosAdd.rotateAngle(\'' + photo +
                    '\', 90);"></a><a class="photos_add_rr" onclick="PhotosAdd.rotateAngle(\'' + photo + '\', -90);"></a></div>'
            });
    ...
    rotateAngle: function(photo, angle) {
        ...
        if (this.transformAvailable()) {
            ...
            var imgDiv = obj.parentNode.parentNode,
                xy = getXY(imgDiv),
                sz = getSize(imgDiv),
                w = 46,
                h = 16,
                l = Math.floor((sz[0] - w) / 2),
                t = Math.floor((sz[1] - h) / 2);
            setStyle(imgDiv, {
                position: 'relative'
            });
            var el = ce('div', {
                    innerHTML: '<div id="rotating_image' + photo + '" class="progress_inv_img" style="display: block; position: absolute; width: ' + w + 'px; height: ' +
    ...
    rotateDone: function(data) {
        ...
        obj.parentNode.onclick = function(ev) {
            return showPhoto(photo, 'album' + ph['album'], {
                temp: {
                    base: data['base'],
                    x_: data['x_'],
                    y_: data['y_'],
    ...
    onUploadStart: function(info, res) {
        ...
        if (info.num == 0) {
            ...
            if (!ge('photos_add_wrap')) {
                cur.count = 0;
                var pc = ge('photos_container') || ge('photos_albums_container'),
                    pa = ge('photos_upload_area'),
                    tab = ge('photo_add_tab');
                pc.parentNode.insertBefore(ce('div', {
                        innerHTML: cur.photosAddWrap
                    })
                    .firstChild, pc);
                pa.parentNode.insertBefore(ce('div', {
                        innerHTML: cur.photosAddBar
                    })
                    .firstChild, pa);
                setStyle(ge('lite_photo_uploader'), {
                    width: '1px',
                    height: '1px'
                });
                ...
                re('albums_load_more');
                re('photos_load_more');
                re(geByClass1('summary_wrap', ge('photos_upload_area_wrap')
                    .parentNode));
                re(geByClass1('photos_summary', ge('photos_upload_area_wrap')
                    .parentNode));
                show(tab);
                var w = 180;
                ...
                if (cur.albums) {
                    ...
                    cur.moveddc = ge('photos_movedd_container')
                        .firstChild;
                }
            }
            ...
            show(ge('photos_add_bar')
                .parentNode);
            if (browser.msie) {
                setStyle(ge('photos_add_bar_form'), {
                    position: 'absolute',
                    top: '-5000px'
                });
            } else {
            ...
            if (info.totalCount > 1) {
                var txt = (cur.uploaderLang['photos_add_uploading_X'] || '')
                    .replace('%s', info.totalCount);
            } else {
                var txt = cur.uploaderLang['photos_add_uploading'];
            }
            ge('photos_add_p_text')
                .innerHTML = txt;
            cur.scrollBarFixedY = false;
            cur.uploadStarted = true;
            setStyle(ge('photos_add_p_inner'), {
                width: '0px'
            });
            ...
            ge('photos_add_p_text')
                .innerHTML = lng.replace('{count}', info.totalCount);
        }
        var classAdd = '';
        ...
        setStyle(ge('photos_add_p_inner'), {
            width: (info.loadedSize / info.totalSize * 175) + 'px'
        });
        ajax.post('al_photos.php', {
            act: 'start_add',
    ...
    onUploadProgress: function(info, start, all) {
        ...
        var oldWidth = intval(ge('photos_add_p_inner')
            .style.width);
        if (newWidth > oldWidth) {
            animate(ge('photos_add_p_inner'), {
                width: newWidth
            }, 200);
        }
    },
    ...
    onUploadComplete: function(info, res) {
        ...
        if (!obj.photos) {
            ...
            ge('photos_add_error')
                .innerHTML = cur.uploaderLang['photos_add_error'];
    ...
    onUploadCompleteAll: function(info, res) {
        ...
        this.onUploadProgress({
            loadedSize: 100,
            totalSize: 100
        }, cur.flash_lite ? 100 : 0, 100);
        PhotosAdd.makeTask(function() {
            var infoCont = ge('photo_upload_area_label'),
                infoText = infoCont.innerHTML;
            ...
            fadeIn(infoCont, 500, function() {
                setTimeout(function() {
                    fadeOut(infoCont, 300, function() {
                        if ((cur.savedPhotos || [])
                            .length >= (cur.maxFiles || 200)) {
                            slideUp('photos_upload_area_wrap', 200, function() {
                                re('photos_upload_area_wrap')
                            });
                        } else {
                            if (cur.flash_lite) {
