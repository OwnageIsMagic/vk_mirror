var ThumbsEdit = {
    ...
    getMedias: function(e) {
        ...
        for (var i = (ThumbsEdit.cache()[e.id] || {})
                .previews || [], a = 0, n = i.length; n > a; ++a) switch (i[a].type) {
            case "photo":
    ...
    thumbElement: function(e, t) {
        var i = {
            ...
            a = ce("div", {
                className: "thumb_wrap fl_l" + (e.lastColumn ? " last_column" : "") + (e.lastRow ? " last_row" : "") + (e.msize ? " thumb_market_album_wrap" : "") + (e
                    .image.src.match(/^\/images\//) ? " page_album_nocover" : "")
            }, i),
            n = ce("img", {
                className: "preview"
            }),
        ...
        addEvent(r, "click", function(e) {
            if (!ThumbsEdit.cur()
                .updating && !domClosest("ui_thumb_x_button", e.target)) {
        ...
        if (e.unsized ? setStyle(r, "backgroundImage", "url(" + c.src + ")") : (extend(n, {
            ...
            }), n.src = c.src), e.old_image && ThumbsEdit.loadAndDisplayImage(n, e), e.unsized || r.appendChild(n), r.appendChild(s), !e.isAlbum && !e.duration && intval(e
                .height) > 55 && r.appendChild(h), r.appendChild(o), a.appendChild(r), a.setAttribute("index", t), a.setAttribute("attachment", e.id), e.duration && (e.single &&
    ...
    crop: function(e, t, i) {
        ...
        if (n.width && n.height) {
            ...
            t / i > h ? (a && n.width < t && (t = n.width, i = Math.min(i, n.height)), s = t, o = s / h, o > i && (d = -intval((o - i) / 2))) : (a && n.height < i && (i =
                n.height, t = Math.min(t, n.width)), o = i, s = o * h, s > t && (r = -intval((s - t) / 2)))
        }
        return {
            width: s,
            height: o,
    ...
    startDrag: function(e) {
        ...
        if (!(i && !t || domClosest("ui_thumb_x_button", e.target) || ThumbsEdit.cur()
                .updating)) {
            var a = ThumbsEdit.getParent(e.target, "draggable_thumb"),
                n = ThumbsEdit.getParent(a, "editable_thumbs"),
                s = ThumbsEdit.cache()[n.id];
            if (s && !((s.previews || [])
                    .length < 2)) return ThumbsEdit.cur()
                .el = a, addEvent(window, "mousemove touchmove", ThumbsEdit.drag), addEvent(window, "mouseup touchend touchcancel", ThumbsEdit.drop), t || cancelEvent(
                    e)
        }
    },
    drag: function(e) {
        ...
        if (hasClass(i, "moving")) {
            ...
            var v = i.parentNode.parentNode,
                ...
                _ = t.getCoords(v, d);
            s.id = v.id, s.i = f, s.to_i = f, s.x = a - w[0], s.y = n - w[1], s.t = b, s.w = b.width, s.h = b.height, s.pos = w, s.wpos = d, s.crop = t.crop(b, b.width, b.height),
                s.coords = _;
    ...
    drop: function(e) {
        ...
        if (removeEvent(window, "mousemove touchmove", ThumbsEdit.drag), removeEvent(window, "mouseup touchend touchcancel", ThumbsEdit.drop), !hasClass(a, "moving"))
            return i.updating = !1, cancelEvent(e);
        if (i.to_i == i.i) {
            var o = (i.t, i.crop),
                r = {
                    duration: 120,
            ...
            d = t.move(d, i.i, i.to_i), t.update(i.id, d), (t.cache()[i.id].opts || {})
                .onMove && t.cache()[i.id].opts.onMove()
        }
    ...
    update: function(e, t) {
        if (e = ge(e)) {
            ...
            each(g, function(e, t) {
                for (var i = null, n = a.cur()
                        .coords, s = 0, r = n.length; r > s; ++s)
                    if (n[s].id == t.id) {
                        i = n[s];
                        break
                    }
                if (null != i) {
                    ...
                    each(c.thumbs, function(e, t) {
                        ...
                        left: 0,
                        top: 0
                    }), (i.height != t.height || i.width != i.width || t.x != i.x || t.y != i.y || i.index == a.cur()
                        .i) && (hasClass(d, "moving") && fadeOut(h, 150), setTimeout(function() {
                        animate(d, {
                            top: t.y - i.y,
                            left: t.x - i.x,
                            marginLeft: 0,
                ...
                h.onUpdate && h.onUpdate(), setStyle(n, {
                    ...
                    }), m.parentNode.removeChild(m), a.cur()
                    .updating = !1
            }, 150)
        }
    },
        ...
        if (i) {
            ...
            return each(e.childNodes, function(e, t) {
                    if (t.getAttribute && t.getAttribute("attachment") == n) {
                        var i = (geByClass1("ui_thumb_x_button", t) || {})
                            .tt;
                        i && i.destroy && i.destroy()
                    }
                }), ThumbsEdit.cur()
                .coords = ThumbsEdit.getCoords(e), 0 == i.previews.length ? ThumbsEdit.init(e, [], i.opts) : ThumbsEdit.update(e, i.previews), a[0] && a[0].remove && a[0].remove(),
                a
        }
    },
    removeById: function(e, t) {
        if (!(e = ge(e))) return null;
        for (var i = (ThumbsEdit.cache()[e.id] || {})
                .previews || [], a = 0, n = i.length; n > a; ++a)
            if ("photo" == i[a].type && i[a].photo.id == t || "video" == i[a].type && i[a].video.id == t || "album" == i[a].type && i[a].album.id == t || "market_album" ==
                i[a].type && i[a].market_album.id == t) return ThumbsEdit.removeMedia(e, a);
        return null
    },
    getRatio: function(e) {
        if (e.vid) return 1.8;
    ...
    getSize: function(e, t, i, a) {
        ...
        if (e.vid) {
            var n = "",
                ...
                h = 0,
                l = 0;
            return 130 >= s && 98 >= o ? (n = "s", h = 130, l = 98) : 320 >= s && 240 >= o ? (n = r ? "l" : "m", h = 320, l = 240) : (n = d ? "y" : r ? "l" : "m", h = 640,
                l = 480), {
                width: h,
                height: l,
                src: e.sizes[n][0]
        ...
        var f = !a && !!c.o,
            b = "",
            w = null;
        b = 75 > p ? "s" : 130 > p ? "m" : f && (w = c.o) && w[1] >= t && w[2] >= i ? "o" : f && (w = c.p) && w[1] >= t && w[2] >= i ? "p" : f && (w = c.q) && w[1] >= t &&
            w[2] >= i ? "q" : f && (w = c.r) && w[1] >= t && w[2] >= i ? "r" : "x";
        var _ = c[b];
    ...
    compute: function(e, t, i, a) {
        e.id = e.vid ? "video" + e.owner_id + "_" + e.vid : e.pid ? "photo" + e.owner_id + "_" + e.pid : e.msize ? "market_album" + e.owner_id + "_" + e.aid : "album" + e.owner_id +
            "_" + e.aid;
        var n = {
            id: e.id,
            width: intval(t),
    ...
    processThumbs: function(e, t, i, a) {
        ...
        var l = a.wide,
            ...
            v = [];
        each(i, function(e, t) {
            ("photo" == t.type || "video" == t.type || "album" == t.type || "market_album" == t.type) && (m[m.length] = t[t.type]), "video" == t.type && u.push(c),
                c++
        });
        var g = "",
            p = [0, 0, 0, 0],
        ...
        if (1 == b) {
            ...
            var k = o(m[0], T, z, M);
            !k.unsized && (k.image.width < T && k.image.height <= _ || k.image.height < z && k.image.width <= w) && (T = k.image.width, z = k.image.height, k = o(m[0], T,
                z, M)), v[0] = k
            ...
            for (var $ = clone(m), ee = clone(q), te = U.split(","), ie = j[U], ae = te.length - 1, I = 0; I < te.length; I++) {
                ...
                for (var de = w, he = 0; he < se.length; he++) {
                    var le = se[he],
                        ue = ee.shift(),
                        ce = a;
                    re == he ? (thumb_width = Math.ceil(de), ce.lastColumn = !0) : (thumb_width = intval(ue * oe), de -= thumb_width + x), v[v.length] = o(le, thumb_width,
                        oe, ce)
                }
            }
            T = w, z = F
