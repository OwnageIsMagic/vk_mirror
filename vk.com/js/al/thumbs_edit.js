var ThumbsEdit = {
    ...
    getMedias: function(el) {
        ...
        for (var arr = (ThumbsEdit.cache()[el.id] || {})
                .previews || [], i = 0, l = arr.length; i < l; ++i) {
            switch (arr[i].type) {
                case 'photo':
                    res.push(['photo', arr[i].photo.owner_id + '_' + arr[i].photo.pid]);
                    break;
                case 'video':
                    res.push(['video', arr[i].video.owner_id + '_' + arr[i].video.vid]);
                    break;
                case 'album':
    ...
    thumbElement: function(thumb, index) {
        ...
        var xbutn = ce('div', {
            className: 'thumb_x_button',
            innerHTML: '<div class="thumb_x"></div>'
        });
        var draggable = ce('div', {
            className: 'draggable_thumb clear_fix' + (thumb.unsized ? ' unsized' + (thumb.single ? ' unsized_single' : '') : '')
        }, style);

        addEvent(draggable, 'click', function(e) {
            if (ThumbsEdit.cur()
                .updating || ThumbsEdit.getParent(e.target, 'thumb_x_button')) return;
        ...
        if (!thumb.unsized) {
            ...
            setStyle(img, {
                marginLeft: cropped.marginLeft,
                marginTop: cropped.marginTop
            });
            img.src = image.src;
        } else {
        ...
        if (thumb.duration) {
            if (thumb.single) {
                draggable.appendChild(ce('div', {
                    className: 'page_post_video_play_inline'
                }));
            } else {
                draggable.appendChild(ce('div', {
                    className: 'duration',
                    innerHTML: thumb.duration
                }));
            }
        }
        ...
        if ((thumb.title || thumb.thumb) && !thumb.duration) {
            ...
            title.appendChild(ce('div', {
                innerHTML: thumb.height > 100 ? thumb.title : ' ',
                className: 'fl_l title_text'
            }, {
    ...
    crop: function(thumb, width, height) {
        ...
        return {
            ...
            height: img_h,
            marginLeft: x,
            marginTop: thumb.isAlbum && thumb.single ? 0 : y
        };
    },
    ...
    startDrag: function(e) {
        ...
        if ((clck && !touch) || ThumbsEdit.getParent(e.target, 'thumb_x_button') || ThumbsEdit.cur()
            .updating) {
            return;
        }
    ...
    drag: function(e) {
        ...
        if (!hasClass(el, 'moving')) {
            ...
            each(ecur.coords, function(k, n) {
                ...
                var mleft = cX >= ncX && ((next && ((cX <= next.x + (next.offsetX / 2) && ((next.offsetY == n.offsetY) || (cX < fnX))) || next.y != n.y)) || (!next &&
                    c.x <= fnX)) && cY > n.y && cY < fnY;
    ...
    drop: function(e) {
        ...
        if (ecur.to_i == ecur.i) {
            ...
            if ((_e.cache()[ecur.id].opts || {})
                .onMove) _e.cache()[ecur.id].opts.onMove();
        }
    ...
    update: function(el, attachments) {
        ...
        var opts = options === true || options === false ? {
            wide: options
        } : clone(options || {});
        var maxW = opts.width;
        var maxH = opts.height;
        ...
        var fake = ce('div', {
            className: 'editable_thumbs' + (cache.wide ? ' wide' : '')
        });
        ...
        each(coords, function(k, coord) {
            var obj = null;
            for (var crds = _e.cur()
                    .coords, i = 0, l = crds.length; i < l; ++i) {
            ...
            if (obj.height != coord.height || obj.width != obj.width || coord.x != obj.x || coord.y != obj.y || obj.index == _e.cur()
                .i) {
        ...
        setTimeout(function() {
            ...
            each(thumbs.thumbs, function(k, t) {
                wrap.appendChild(_e.thumbElement(t, k));
            });
    ...
    removeMedia: function(el, index) {
        ...
        var del = obj.previews.splice(index, 1),
            idr = del[0][del[0].type].id;
        each(el.childNodes, function(k, v) {
            if (v.getAttribute && v.getAttribute('attachment') == idr) {
                var tt = (geByClass1('thumb_x_button', v) || {})
                    .tt;
        ...
        ThumbsEdit.cur()
            .coords = ThumbsEdit.getCoords(el);
    ...
    removeById: function(el, attId) {
        ...
        var arr = (ThumbsEdit.cache()[el.id] || {})
            .previews || [];
    ...
    processThumbs: function(maxW, maxH, attachments, opts) {
        ...
        var max_w, max_h, margin_w = wide ? 6 : 3,
            margin_h = margin_w;
        ...
        if (cnt == 1) {
            ...
            case 'ww':
                if (avg_ratio > 1.4 * max_ratio && (ratios[1] - ratios[0]) < 0.2) {
                    ...
                    result[0] = compute(thumbs[0], w, h, {
                        lastColumn: 1
                    });
                    result[1] = compute(thumbs[1], w, h, {
                        lastColumn: 1,
                        lastRow: 1
                    });
            ...
            default:
                ...
                result[0] = compute(thumbs[0], w0, h, {
                    lastRow: 1
                });
                result[1] = compute(thumbs[1], w1, h, {
                    lastColumn: 1,
                    lastRow: 1
                });
            ...
            if ((ratios[0] > 1.2 * max_ratio || avg_ratio > 1.5 * max_ratio) && orients == 'www') {
                ...
                result[0] = compute(thumbs[0], w, h_cover, {
                    lastColumn: 1
                });
                if (orients == 'www') {
                    ...
                    result[2] = compute(thumbs[2], max_w - w - margin_w, h, {
                        lastColumn: 1,
                        lastRow: 1
                    });
                } else {
                    ...
                    result[1] = compute(thumbs[1], w0, h, {
                        lastRow: 1
                    });
                    result[2] = compute(thumbs[2], w0, h, {
                        lastRow: 1,
                        lastColumn: 1
                    });
                }
                ...
                result[0] = compute(thumbs[0], w_cover, h, {
                    lastRow: 1
                });
                ...
                result[1] = compute(thumbs[1], w, h0, {
                    lastColumn: 1
                });
                result[2] = compute(thumbs[2], w, h1, {
                    lastColumn: 1,
                    lastRow: 1
                });
            ...
            if ((ratios[0] > 1.2 * max_ratio || avg_ratio > 1.5 * max_ratio) && orients == 'wwww') {
                ...
                result[2] = compute(thumbs[2], w1, h, {
                    lastRow: 1
                });
                result[3] = compute(thumbs[3], w2, h, {
                    lastColumn: 1,
                    lastRow: 1
                });
                ...
                result[3] = compute(thumbs[3], w, h2, {
                    lastColumn: 1,
                    lastRow: 1
                });
        ...
        return {
            width: intval(thumbs_width),
            height: intval(thumbs_height),
            thumbs: result
        };
    }
}
