var EditDB = {
    ...
    filterCountries: function(q) {
        ...
        var childs = ge('edb_countries')
            .childNodes,
            hl = q ? EditDB.getHighlight(cur.index, q) : false;
        if (q) {
            var lst = cur.index.search(q),
                shown = {};
            if (lst.length) {
                for (var i in lst) {
                    shown[lst[i]] = 1;
                ...
                each(childs, function() {
                    ...
                    this.replaceChild(ce('span', {
                        innerHTML: cur.names[this.id].replace(hl.re, hl.val)
                    }), this.firstChild);
                });
                show('edb_countries');
