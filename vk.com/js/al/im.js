...
var IM = {
    ...
    updateCounts: function(cnts) {
        ...
        if (tabs && cnts[0] <= 0 && cur.gid) {
            hide(geByClass1('_tab_gfilter_unread', tabs)
                .parentNode);
        }
    ...
    scroll: function(toTop, toMsg, hl) {
        if (!cur.fixedScroll && !IM.r()) {
            ...
            var st = 0,
                to = toMsg > 0 ? ge('mess' + toMsg) : (toMsg < 0 ? ge('im_unread_bar' + cur.peer) : false);
    ...
    scrollAppended: function(appendedH, removeBar) {
        ...
        var winH = Math.max(intval(window.innerHeight), intval(document.documentElement.clientHeight)),
            ...
            contentSH = IM.contSH(),
            toUnread = false,
            row = tab && tab.firstUnr && ge('mess' + tab.firstUnr),
            atBottom = contentST > contentSH - winH - (appendedH || 0) - 200;
        if (cur.nu && atBottom) {
            if (removeBar === 2) re('im_unread_bar' + cur.peer);
            if (row) {
                var y = getXY(row)[1],
                    ymin = contentSH - winH + cur.imEl.head.clientHeight + cur.imEl.nav.offsetHeight + 10 + 49;
                if (y < ymin) {
                    toUnread = true;
                    atBottom = false;
                }
...
<div class="im_unread_bar">\
  <span class="im_unread_bar_wrap"><span class="im_unread_bar_text">' + getLang(
                            'mail_new_unread_msgs') + '</span></span>\
</div>'
                    });
    ...
    scrollOn: function(toTop, toMsg) {
        ...
        cur.scrollTO = setTimeout(IM.scroll.bind(IM)
            .pbind(toTop, toMsg), 100);
    },
    ...
    mkdate: function(raw) {
        ...
        return pad(result.getDate()) + '.' + pad(result.getMonth() + 1) + '.' + (result.getFullYear() + '')
            .substr(2);
    },

    mknotonline: function(sex, name) {
        if (!sex || sex == 2) {
            return lang.mail_im_not_online[1].replace('{user}', name)
                .replace(/\..+$/, '.');
        } else {
            return lang.mail_im_not_online[2].replace('{user}', name)
                .replace(/\..+$/, '.')
        }
    },
    ...
    addMsg: function(peer_id, after_id, msg_id, status, out, title, message, date, kludges, delayed) {
        ...
        if (!delayed && !wasRow && msg_id > 0) {
            ...
            if (!out && status > 1) {
                ...
                if (!willMark && cur.nu) {
                    var r = ge('im_dialog' + peer_id),
                        ur = r ? intval(r.getAttribute('data-unread')) : 0;
                    if (r) {
        ...
        if (cur.tabs[peer_id] && cur.tabs[peer_id].q_offset) {
            var last = intval(domLC(geByTag1('tbody', ge('im_log' + peer_id)))
                .id.replace(/^mess/, ''));
            if (last && last < msg_id) {
        ...
        if (is_sticker) {
            if (kludges.attach1_product_id) {
                ...
                media_html = '<div class="im_sticker_row"><img height="128" class="im_gift" src="/images/stickers/' + intval(kludges.attach1) + '/' + (window.devicePixelRatio >=
                    2 ? '256' : '128') + '.png"/></div>';
            }
        } else {
        ...
        if (srvMsg) {
            ...
            message = (message || '')
                .replace(
                    /(^|[^A-Za-z0-9�-��-���\-\_])(https?:\/\/)?((?:[A-Za-z\$0-9�-��-���](?:[A-Za-z\$0-9\-\_�-��-���]*[A-Za-z\$0-9�-��-���])?\.){1,5}[A-Za-z\$������������������������\-\d]{2,22}(?::\d{2,5})?)((?:\/(?:(?:\&amp;|\&#33;|,[_%]|[A-Za-z0-9�-��-���\-\_#%?+\/\$.~=;:]+|\[[A-Za-z0-9�-��-���\-\_#%?+\/\$.,~=;:]*\]|\([A-Za-z0-9�-��-���\-\_#%?+\/\$.,~=;:]*\))*(?:,[_%]|[A-Za-z0-9�-��-���\-\_#%?+\/\$.~=;:]*[A-Za-z0-9�-��-���\_#%?+\/\$~=]|\[[A-Za-z0-9�-��-���\-\_#%?+\/\$.,~=;:]*\]|\([A-Za-z0-9�-��-���\-\_#%?+\/\$.,~=;:]*\)))?)?)/ig,
                    function() { // copied to notifier.js:3401
                        ...
                        if (!susp && domain.match(/^([a-zA-Z0-9\.\_\-]+\.)?(vkontakte\.ru|vk\.com|vkadre\.ru|vshtate\.ru|userapi\.com|vk\.me)$/)) {
                            ...
                            return prefix + '<a href="' + (protocol + url)
                                .replace(/"/g, '&quot;')
                                .replace(/</g, '&lt;')
                                .replace(/>/g, '&gt;') + '" target="_blank"' + oncl + '>' + full + '</a>';
                        }
                        return prefix + '<a href="away.php?utf=1&to=' + encodeURIComponent(protocol + replaceEntities(url)) + '" target="_blank" onclick="return goAway(\'' +
                            clean(protocol + url) + '\', {}, event);">' + full + '</a>';
                    });
        ...
        var hist = IM.getTable(peer_id),
            maxIndex = hist.rows.length,
            index;
        if (after_id == -1) {
            index = maxIndex;
        } else {
        ...
        if (srvMsg) {
            extend(row.insertCell(0), {
                colSpan: 5,
                className: 'im_srv_msg_td',
                innerHTML: '<div class="im_srv_msg">' + srvMsg + '</div>'
            });
        } else {
            if (peer_data && actual_peer && (user = peer_data.members[actual_peer])) {
                ...
                if (out) {
                    ...
                    var photo = cur.tabs[peer_id].photo,
                        name = cur.tabs[peer_id].name;
                    if (fromId < -2e9) {
                        if (peer_id > 2e9) {
                            name = 'Unknown';
            ...
            author_html = '<div class="im_log_author_chat_thumb"><a href="' + user[0] + '" target="_blank"><img src="' + user[1] +
                '" class="im_log_author_chat_thumb" width="32" height="32"/></a></div>';
            ...
            if (msg_id > 0) {
                ...
                row.onclick = function(e) {
                    if (!IM.checkLogClick(this, e || window.event)) IM.checkLogMsg(msg_id)
                };
            } else {
            ...
            extend(row.insertCell(0), {
                ...
                innerHTML: actions_html
            });
            extend(row.insertCell(1), {
                className: 'im_log_author',
                innerHTML: author_html
            });
            extend(row.insertCell(2), {
                className: 'im_log_body',
                innerHTML: '<div class="wrapped">' + message + '</div>'
            });
            var msgLink = !cur.gid ? "mail?act=show&id=" + msg_id : "gim" + cur.gid + "?msgid=" + msg_id + "&sel=" + cur.peer;
            var dateLink = msg_id > 0 ? '<a class="im_important_toggler" onclick="return IM.toggleImportant(' + msg_id +
                ');" onmouseover="IM.showImportantTT(this);"></a><div class="im_date_link">' + whois + '<a href="/' + msgLink + '">' + full_date + '</a></div>' :
                '<span class="im_date_link">' + whois + full_date + '</span>';
            extend(row.insertCell(3), {
    ...
    notify: function(peer_id, msg) {
        ...
        peer_name = trim(replaceEntities(stripHTML((peer_name || '')
            .replace('&nbsp;', ' '))));
        Notifier.proxyIm({
            id: msg[4],
            text: message,
    ...
    loadMedia: function(msg_id, peer_id) {
        ajax.post('al_im.php', {
            ...
            onFail: function(a) {
                ...
                topError('IM media fail: ' + a + '; ' + peer_id + '_' + msg_id, {
                    dt: -1
                });
    ...
    chatPhotoSaved: function(res) {
        ...
        var msg_id = res[0];
        if (!msg_id) return;
        if (!ge('mess' + msg_id)) IM.addMsg(peer, -1, msg_id, 2, 1, '', '', res[2], {
            source_act: res[5] ? 'chat_photo_update' : 'chat_photo_removed',
            from: cur.id,
            attach1_type: 'photo',
            _no_media_load: 1
        });
        ...
        ajax.post('al_im.php', {
            ...
            onDone: function(html, msgs, all_shown, cnts, data) {
                ...
                var cur_rows = geByTag1('tbody', table),
                    ...
                    before_row = cur_rows.firstChild,
                    add_row, row_id;

                domPN(table)
                    .insertBefore(new_t, (more < 0) ? domNS(table) : table);
    ...
    deleteHistory: function(peer, hash) {
        ...
        }, peer > 2e9 ? getLang('mail_chat_sure_to_delete_all') : getLang('mail_sure_to_delete_all'), getLang('mail_delete'), succ, getLang('global_cancel'), box.hide);
    },
    ...
    showMediaHistory: function(peer, mediaType) {
        return showWiki({
            w: 'history' + IM.peerToId(peer) + '_' + mediaType
        });
    },
    ...
    extractEmoji: function(txt, peer) {
        ...
        for (var code in newRc) {
            ...
            rchtml += '<a id="im_rc_em_' + code + '" class="im_rc_emojibtn" onmousedown="Emoji.addEmoji(cur.emojiId[cur.peer], \'' + code +
                '\', this); return cancelEvent(event);">' + Emoji.getEmojiHTML(code, false, true) + '</a>';
            ml -= 22;
        }
        ...
        animate(rcCont, {
            ...
            onComplete: function() {
                var emjs = geByClass('im_rc_emojibtn', rcCont)
                    .slice(7);
                for (var i in emjs) {
                    re(emjs[i]);
                }
    ...
    send: function(btn, ev, sendPeer) {
        ...
        if (cur.editable) {
            ...
            var rowMsg = clean(msg)
                .replace(/\n/g, '<br>'),
                rowTitle = clean(title)
                .replace(/\n/g, '<br>');
        }
        ...
        ajax.post('al_im.php', params, {
            onDone: function(response) {
                ...
                msg_row.cells[3].innerHTML = '<a class="im_important_toggler" onclick="return IM.toggleImportant(' + new_msg_id +
                    ');" onmouseover="IM.showImportantTT(this);"></a><div class="im_date_link">' + whois + '<a href="">' + IM.mkdate(response.date + cur.tsDiff) +
                    '</a><input type="hidden" id="im_date' + new_msg_id + '" value="' + response.date + '" />';
                msg_row.id = 'mess' + new_msg_id;
                msg_row.cells[0].innerHTML = '<div id="ma' + new_msg_id + '" class="im_log_check_wrap"><div class="im_log_check" id="mess_check' + new_msg_id +
                    '"></div></div>';
                ...
                msg_row.onclick = function(e) {
                    if (!IM.checkLogClick(this, e || window.event)) IM.checkLogMsg(new_msg_id)
                };
            ...
            onFail: function(error) {
                ...
                if (!msg_row) return;
                re('mprg' + msg_id);
                msg_row.cells[3].innerHTML = '<span class="im_log_date_error">' + getLang('global_error') + '</span><input type="hidden" id="im_date' + msg_row
                    .id.substr(4) + '" value="0" />';
                IM.scroll();
    ...
    feed: function(peer, events) {
        ...
        for (var i in events) {
            ...
            if (!msg[0] && row) { // Existing message was deleted
                ...
                if (cur.tabs[peer].q_offset) {
                    ...
                    var last = intval(domLC(geByTag1('tbody', ge('im_log' + peer)))
                        .id.replace(/^mess/, ''));
                    if (last && last < i) {
        ...
        if (lastMsg && tab && !tab.loading) { // Updating dialogs rows
            var dlg = ge('im_dialog' + peer),
                upd = IM.updateDialog.pbind(peer, lastMsg);
            tab.lupd = lastMsg[0];
    ...
    updateDialog: function(peer, lastMsg, unreadCnt, cnts, folders) {
        ...
        repls.date = getLang('mail_im_row_date_format')
            .replace('{am_pm}', ampm)
            .replace('{hour}', hours)
            .replace('{num_hour}', numhours)
            .replace('{minute}', (minutes < 10 ? '0' : '') + minutes);
        ...
        if (!srvMsg) {
            ...
            if (kludges.attach1_type) {
                ...
                body += '<div class="im_row_attach"><div class="im_attach_mail"></div>' + (kludges.fwd.match(/,\(/) ? getLang('mail_added_msgs') : getLang('mail_added_msg')) +
                    '</div>';
        ...
        if (peer < 2e9) {
            repls.photo = '<a href="/id' + peer +
                '" target="_blank" onclick="event.cancelBubble = true; return nav.go(this, event);" onmousedown="event.cancelBubble = true;"><img src="' + tab.photo +
                '" width="50" height="50" /></a>';
            repls.user_link = '<a href="/id' + peer +
                '" target="_blank" onclick="event.cancelBubble = true; return nav.go(this, event);" onmousedown="event.cancelBubble = true;">' + tab.tab_text.replace(
                    '&nbsp;', ' ') + '</a>';
            if (!lastMsg[2] || cur.friends[peer + '_'] && cur.friends[peer + '_'][0]) {
                var onl = langSex((tab || {})
                        .sex || (cur.friends[peer + '_'] || {})[5], window.global_online),
                    onlP = cur.friends[peer + '_'] && cur.friends[peer + '_'][0] || 0;
                if (onlP > 0 && onlP < 6) {
                    onl += '<b class="mob_onl dialogs_mob_onl" onmouseover="mobileOnlineTip(this, {mid: ' + peer +
                        '})" onclick="mobilePromo(); return cancelEvent(event);"></b>';
                }
                repls.online = '<div class="dialogs_online">' + onl + muted + '</div>';
            }
        } else {
            // Complicated photo and user link
            repls.user_link = '<a href="/im?sel=' + peer + '" onclick="event.cancelBubble = true; if (!checkEvent(event)) {IM.addPeer(' + peer + '); return false;}">' +
                tab.name + '</a>';
            ...
            for (var i in response.updates) {
                ...
                if (code == 8 || code == 9) {
                    ...
                    if (!cur.peer) {
                        ...
                        IM.updateOnline(extra, (cur.tabs[cur.peer] || {})
            ...
            cur.emojiId[peer] = Emoji.init(txt, {
                ...
                onKeyAction: function(e) {
                    ...
                    if (e.type == 'keyup') {
                        IM.readLastMsgs();
                        if (cur.editable) {
                            if (!e.ctrlKey && !e.metaKey && (e.keyCode > 40 && !inArray(e.keyCode, [91, 92]) && !(e.keyCode >= 112 && e.keyCode <= 125) || e.keyCode ==
                                    32 || e.keyCode == 8)) {
                                IM.onMyTyping(peer);
                            }
                        } else {
    ...
    restoreDraft: function(peer) {
        ...
        if ((draft.medias || [])
            ...
            for (var i in draft.medias) {
                ...
                m.push(draft.medias[i].slice(0, 2)
                    .join(','));
            }
            ajax.post('al_im.php', {
                act: 'draft_medias',
    ...
    onMediaChange: function(type, media, data, url) {
        ...
        if (IM.r(cur.peer) || url && (input.innerText || input.textContent)
            .indexOf(url) == -1) {
        ...
        switch (type) {
            case 'photo':
                ...
                vkImage()
                    .src = data.thumb_s;
                preview = '<div onclick="if (cur.cancelClick) return (cur.cancelClick = false); return IM.showPhoto(\'' + media + '\', \'' + data.list + '\', ' + data.view_opts
                    .replace(/"/g, '&quot;') + ');" class="im_preview_photo"><img class="im_preview_photo" src="' + data.thumb_s + '" onload="IM.refr()" />';
                postview = '</div>';
                contIndex = 1;
            ...
            case 'video':
                ...
                preview = '<div onclick="if (cur.cancelClick) return (cur.cancelClick = false); return showVideo(\'' + media +
                    '\');" class="im_preview_video"><div class="im_preview_video_thumb" style="background-image:url(\'' + data.thumb + '\')" onload="IM.refr()"></div>';
                postview = '</div>';
                contIndex = 1;
            ...
            case 'doc':
                ...
                if (data.thumb && data.thumb_s) {
                    preview = '<a target="_blank" href="' + data.href +
                        '" class="fl_l pam_dpic" onclick="if (cur.cancelClick) return (cur.cancelClick = false);"><div class="im_preview_photo_doc"><img src="' + data.thumb_s +
                        '" align="middle"></div><div class="im_preview_doc_photo_hint">' + data.title + '</div>';
            ...
            case 'mail':
                preview = '<a onclick="if (cur.cancelClick) return (cur.cancelClick = false); IM.willForward()" class="medadd_h medadd_h_mail inl_bl">' + getLang(
                    'mail_im_forward') + '</a>';
            ...
            case 'map':
                preview = '<div class="fl_l"><a onclick="return showBox(\'al_places.php\', {act: \'geo_box\', lat: ' + data[0] + ', long: ' + data[1] +
                    '}, {dark: 1});"><div class="page_media_map_point"></div><img class="page_preview_map" width="174" height="70" src="/maps?lat=' + data[0] + '&lng=' +
                    data[1] + '&z=11&' + (window.devicePixelRatio >= 2 ? 'w=360&h=140' : 'w=174&h=70') + '" /></a></div>';
                contIndex = 3;
                cls = 'fl_l';
                var lnk = cur.imMedia.lnkId;
        ...
        if (type) {
            ...
            if (preview || postview) {
                mediaHtml = '<div class="im_preview_' + type + '_wrap im_preview_ind%ind% ' + cls + '"' + attrs + '>' + preview +
                    '<div nosorthandle="1" class="im_media_x inl_bl" ' + (browser.msie ? 'title' : 'tooltip') + '="' + getLang('dont_attach') +
    ...
    onMediaProgress: function(type, i, data) {
        ...
        <div id="upload' + ind + '_progress" class="page_attach_progress" style="width: ' + percent +
                '%;"></div>\
      </div></div>';
            ...
            setStyle(ge('upload' + ind + '_progress'), {
                width: percent + '%'
            });
    ...
    _showDialogTyping: function(dialogElId, names, noAnim) {
        ...
        if (!names) {
            ...
            cssAnim(typingEl, {
                ...
                setStyle(msgBodyEl, {
                    opacity: 1.0
                }); // ensure that opacity will be 1.0
            });
        ...
        each(cur.typingEvents, function(peer, tevent) {
            ...
            if (namesArr.length == 0) {
                ...
                if (toCutIndex !== false) {
                    ...
                    names = names.replace('{users}', firstNames.join(', '))
                        .replace('{users_count}', lastCount);
                } else {
                    var lastUser = namesArr.pop();
                    names = getLang('mail_im_multi_typing')
                        .replace('{users}', namesArr.join(', '))
                        .replace('{last_user}', lastUser);
                }
            } else if (namesArr[0]) {
                names = langSex(sex, cur.lang.mail_im_typing)
                    .replace('{user}', namesArr[0]);
            }
    ...
    updateTyping: function(forced) {
        ...
        if (typings.length == 1) {
            if (cur.lang.mail_im_typing) {
                val(el, langSex(sex, cur.lang.mail_im_typing)
                    .replace('{user}', typings[0]));
            }
        } else {
            var lastUser = typings.pop();
            val(el, getLang('mail_im_multi_typing')
                .replace('{users}', typings.join(', '))
                .replace('{last_user}', lastUser));
        }
        IM.hideLastAct(peer);
        if (forced) {
    ...
    attachedCount: function(loaded) {
        var previewEl = ge('im_media_preview'),
            ...
            progressNode = ge('im_progress_preview');
        return (previewEl.childNodes.length + dpreviewEl.childNodes.length + mpreviewEl.childNodes.length + docsEl.childNodes.length / (docsEl.sorter ? 2 : 1) + sdocsEl.childNodes
            .length + (loaded ? 0 : progressNode.childNodes.length));
    },
    toEnd: function() {
        if (nav.objLoc.msgid) {
            IM.updateLoc(); // remove msgid
    ...
    updateMoreNew: function(peer) {
        ...
        if (cur.peer == peer && cur.tabs[peer].q_offset) {
            var teWrap = ge('im_to_end_wrap'),
                ...
                newval = trim(getLang('mail_goto_history_end')
                    .replace('{new}', newtxt));
            if (
                domFC(teWrap)
                .className == 'progress_inline' &&
                domFC(teWrap)
                .getAttribute('data-for') == peer &&
                isVisible(teWrap)
            ) {
                return;
            }
    ...
    decHash: function(hash) {
        (function(_) {
            cur.decodedHashes[_] = (function(__) {
                var ___ = ge ? '' : '___';
                for (____ = 0; ____ < __.length; ++____) ___ += __.charAt(__.length - ____ - 1);
                return geByClass ? ___ : '___';
            })(_.substr(_.length - 5) + _.substr(4, _.length - 12));
        })(hash);
    },
    ...
    init: function(options) {
        ...
        var filter = ge('im_filter'),
            filterReset = ge('im_filter_reset');
        placeholderSetup(filter, {
            back: true
        });
        ...
        addEvent(filterReset, 'mouseover mouseout click', function(e) {
            if (e.type != 'click') {
                if (isVisible(filterReset))
                    animate(filterReset, {
                        opacity: e.type == 'mouseover' ? 1 : 0.5
                    }, 100);
                return;
            }
            val(filter, '');
        ...
        cur.destroy.push(function() {
            ...
            if (cur.fixedScroll) {
                each(geByClass('top_info_wrap', ge('page_wrap')), function() {
                    show(this);
                });
        ...
        if (options.text_data || options.media_data) {
            var txt = IM.getTxt(),
                text = replaceEntities(options.text_data || '');
            if (cur.editable) {
                val(txt, clean(text)
                    .replace(/\n/g, '<br/>'));
                Emoji.editableFocus(txt, false, true);
    ...
    getTextForPeer: function(peer) {
        ...
        var title = val('imw_title'),
            meds = cur.imwMedia ? cur.imwMedia.getMedias() : [];
        if (title) {
            show('im_title_wrap' + peer);
        ...
        for (var i = 0, l = meds.length; i < l; ++i) {
            var m = meds[i],
                media = cur.imwMediaSaved[m[0] + m[1]];
            if (media) {
    ...
    updateDialogLock: function(peer, dialogEl) {
        ...
        if (cur.blocks[peer] && !cur.blocks[peer][0]) {
            var text = getLang('mail_community_answering')
    ...
    addPeer: function(mid, events, dont_activate, getText, msgId) {
        ...
        if (cur.tabs[mid] && cur.tabs[mid].loading) {
            ...
            if (isArray(events) && events.length) {
                cur.tabs[mid].events = (cur.tabs[mid].events || [])
                    .concat(events);
            }
            return;
        }
    ...
    initTabEvents: function(tabEl) {
        ...
        if (!tabEl.className) {
            ...
            addEvent(tabEl, 'click', function() {
                IM.activateTab(mid);
            });
        }
        var labelEl = geByClass1('im_tab2', tabEl),
            xEl = geByClass1('im_tabx', labelEl);
    ...
    updateScroll: function(e, noev, force) {
        ...
        if (!cur.fixedScroll) {
            ...
            if (!IM.r(cur.peer) || cur.peer == -2 || cur.peer == -4 || cur.peer == -5) { // spam and search
                ...
                if (IM.r(cur.peer)) {
                    setStyle(rowsEl, {
                        minHeight: winH - imControlsH - contentY - 21
                    });
                }
            } else {
                setStyle(cur.imEl.rows, {
            ...
            var paddingBottom = Math.max(imControlsH, winH - contentY - cur.imEl.rowsWrap.offsetHeight - 1),
                ...
                prevPadd = (getStyle(cur.imEl.cont, 'padding') || '')
                .split(' ');
            ...
            if (cur.peer != -3) {
                setStyle(cur.imEl.cont, {
                    padding: contentY + 'px 0 ' + paddingBottom + 'px'
                });
            } else {
                setStyle(cur.imEl.cont, {
                    padding: ''
            ...
            if (!browser.mozilla && !browser.msie && (cur.lastWW !== lastWindowWidth || isResize || force)) {
                ...
                var goodLeft1 = ge('page_layout')
                    .offsetLeft,
                    goodLeft2 = goodLeft1 + cur.imEl.cont.offsetLeft;
                cur.imEl.head.style.left = ge('side_bar')
                    .style.left = goodLeft1 + 'px';
    ...
    panelToTop: function(noReset) {
        ...
        if (cur.isPanelToTop) {
            var diff = (lastWindowHeight - cur.lastWinH),
                h = Math.max(0, cur.oldResizableH + diff);
            if (diff) {
                IM.resizableSet(h);
            }
            return;
    ...
    onScroll: function() {
        ...
        if ((!IM.r() || cur.peer == -2 || cur.peer == -4 || cur.peer == -5) && !curBox() && !isVisible(layerBG)) {
            var moreEl = ge('im_more' + (cur.peer == -5 ? -2 : cur.peer)),
                moreNewEl = (cur.tabs[cur.peer] || {})
                .q_offset ? ge('im_morenew' + cur.peer) : false;
    ...
    activateTab: function(peer, from, msgId) {
        ...
        if (cur.imMedia) {
            cur.urlsCancelled[old_peer] = (cur.imMedia || {})
                .urlsCancelled || [];
        ...
        if (!IM.r(old_peer)) {
            ...
            if (progressNode.childNodes.length > 0) {
                showFastBox({
                    ...
                    dark: 1,
                    bodyStyle: 'padding: 20px; line-height: 160%;'
                }, getLang('mail_message_wait_until_uploaded'));
                return false;
            }
        ...
        if (!IM.r(peer)) {
            ...
            if (peer == -1) {
                var st = cur.lastDialogsY,
                    mid = cur.lastDialogsPeer;
                cur.lastDialogsY = 0;
        ...
        if (cur.nu && !IM.r(peer)) {
            ...
            if (cur.tabs[peer].unread) {
                ...
                if (firstId) {
                    var hist = ge('im_log' + peer),
                        row = ge('mess' + firstId);
                    if (hist && row) {
                        re('im_unread_bar' + peer);
                        extend(extend(hist.insertRow(row.rowIndex), {
                                id: 'im_unread_bar' + peer,
                                className: 'im_unread_bar_tr'
                            })
                            .insertCell(0), {
                                colSpan: 5,
  ...
  <span class="im_unread_bar_wrap"><span class="im_unread_bar_text">' + getLang(
    ...
    toggleInput: function(result, txt, peer, name, placeholder) {
        ...
        if (!result) {
            ...
            setTimeout(function() {
                if (name) {
                    if (!placeholder) {
                        placeholder = getLang('mail_community_answering')
                            .replace('{username}', name);
                    }
    ...
    applyPeer: function() {
        ...
        if (peer > 2e9) {
            ...
            val(geByClass1('im_tab3', ge('im_tab' + peer), 'div'), '<span class="im_tab4">' + user.name + '</span>' + '<span id="im_unread' + peer + '">' + val('im_unread' +
                peer) + '</span>');
        } else {
            if (peer < -2e9) {
            ...
            val('im_peer_holders', '<div class="im_peer_holder fl_l"><div class="im_photo_holder"><a href="' + peerLink + '" target="_blank"><img src="' + user.photo +
                '" width="50" height="50"/></a></div><div class="im_status_holder" id="im_status_holder"></div></div>');
    ...
    addTab: function(mid, tab_text, name, photo, hash, sex, data, block, href) {
        ...
        text = [
            '<a href="/write', mid, '?hist=1&offset=-1" target="_blank" class="im_more" id="im_more', mid, '" onclick="return IM.loadHistory(', mid,
            ...
            '<a href="/write', mid, '?hist=1&offset=-1" target="_blank" class="im_morenew" id="im_morenew', mid, '" onclick="return IM.loadHistory(', mid,
            ...
            ajax.post('al_im.php', {
                ...
                onDone: function(friendsOnline, cnts) {
                    ...
                    if (!cur.peer) {
                        ...
                        IM.updateOnline(intval(friendsOnline[cur.peer]), (cur.tabs[cur.peer] || {})
                            .sex || (cur.friends[cur.peer + '_'] || {})[5]);
                    }
                    IM.updateCounts(cnts);
                }
            ...
            onDone: function(options, rows, next_rows) {
                ...
                if (options.summary) {
                    ...
                    if (cur.gid) {
                        ...
                        var time = getLang('mail_average_response_time_admin')
                ...
                if (rows) {
                    var dlgs = ge('im_dialogs'),
                        moreEl = ge('im_more_dialogs') || cur.moreEl,
                        noneEl = ge('im_rows_none');

                    var st = bodyNode.scrollTop,
                        stChanged = false;
                ...
                if (options.folders && options.folders.unrespond == 0) {
                    hide(geByClass1('_tab_gfilter_unrespond')
                        .parentNode);
                }
    ...
    cacheFriends: function(q) {
        if (q) {
            ...
            for (var i in queries) {
                query = queries[i];
                var search_in = cur.friends_cache[query.substr(0, 1)
                    .toLowerCase()];
                if (search_in) {
                    ...
                    for (var i in search_in) {
                        ...
                        if ((new RegExp('^' + query + '|\\s' + query + '|\\(' + query, 'gi'))
                            .test(name.replace(/�/g, '�'))) {
    ...
    wrapFriends: function(list, online, to_match, simpleOver) {
        ...
        for (var i in list) {
            ...
            if (mem[0]) {
                ...
                if (mem[0] > 0 && mem[0] < 6) {
                    onl += '<b class="mob_onl im_friend_mob_onl" onmouseover="mobileOnlineTip(this, {mid: ' + j +
                        '})" onclick="mobilePromo(); return cancelEvent(event);"></b>'
                }
            ...
            ajax.post('hints.php', {
                ...
                onDone: function(correspondents) {
                    ...
                    each(correspondents || [], function(i) {
                        ...
                        text.push('<div class="im_friend' + cls + '" id="im_friend', id, '" ', 'onmousemove="IM.friendOver(this);"',
                            ' onclick="IM.selectPeer(', this[3], ')">', txt, '</div>');
                    });
                    ...
                    if (!cur.multi) {
                        if (cur.multi_appoint) {
                            ...
                            var cl = 'im_find_in_mail im_friend' + ((has || cur.friends_shown) ? '' : ' im_friend_over'),
                                srch = ce('a', {
                                    ...
                                    innerHTML: '<span class="im_find_mail">' + getLang('mail_im_search_query')
                                        .replace('{query}', '<b>' + clean(q) + '</b>') + '</span>',
    ...
    wrapEmail: function(name, to_match, user_id) {
        ...
        var text = ['<div class="im_friend im_chat', cls, '" id="im_friend', user_id, '" onmousemove="IM.friendOver(this);" onclick="IM.selectPeer(', user_id,
            ')"><div class="im_photo"><img src="/images/contact' + (window.devicePixelRatio >= 2 ? '_2x' : '') +
            '.png?1" class="fl_l" width="43" height="43" /></div><div class="fl_l name"><nobr>', clean(name), '</nobr></div></div>'
        ];
        return text.join('');
    },
    ...
    wrapEmails: function(list, to_match, selected) {
        ...
        for (var i in emails_list) {
            ...
            text.push('<div class="im_friend im_chat', cls, '" id="im_friend', user_id, '" ', over, ' onclick="IM.selectPeer(', user_id,
                ')"><div class="im_photo"><img src="/images/contact' + (window.devicePixelRatio >= 2 ? '_2x' : '') +
                '.png?1" class="fl_l" width="43" height="43" /></div><div class="fl_l name"><nobr>', email, '</nobr></div></div>');
        }
        return text.join('');
    },
    ...
    wrapChatPhoto: function(photos) {
        ...
        if (photos.length == 1) {
            ...
            var photo = '',
                photoMax = 4,
                photosNum = Math.min(4, photos.length);
            for (var i = 0; i < photos.length; i++) {
                ...
                var style = (photosNum == 4 && (photoMax == 2 || photoMax == 0) || photosNum == 3 && (photoMax == 1 || photoMax == 2) || photosNum == 2 && photoMax == 2) ?
                    marginStyle : '';
                text.push('<span><div class="dialogs_inline_chatter ', className, '" style="', style, '"><img class="dialogs_inline_chatter ', className, '" src="', photo,
                    '" width="', size, '" height="', size, '"/></div></span>');
            };
        }
    ...
    wrapChats: function(list, to_match) {
        var text = [],
            is_sel, chat_list = to_match ? list : cur.chats,
            limit = 20;
        for (var i in chat_list) {
            ...
            text.push('<div class="im_friend im_chat', cls, '" id="im_friend' + intval(i) + '" onmousemove="IM.friendOver(this);" onclick="IM.selectPeer(' + intval(i) +
                ')">' + txt + '</div>');
        }
        return text.join('');
    },
    ...
    wrapPeers: function(friends, to_match) {
        var text = '',
            corresp = '<div id="im_correspondents"></div>';
    ...
    selectPeer: function(mid, msgId) {
        if (!cur.multi) {
            if (mid == -2e9 && cur.addEmailPeer) {
                if (cur.multi_appoint) {
                    IM.updateChat(cur.multi_appoint, true, {
    ...
    updateChat: function(peer, force, options) {
        ...
        ajax.post('al_im.php', extend({
            act: 'a_get_chat',
            chat: peer - 2e9,
            cur_peers: curMems.join(','),
            cur_title: replaceEntities(curData.title),
            hash: curTab.hash
        }, options || {}), {
            onDone: function(evs, newTab) {
                IM.receivePeerData(peer, newTab);
                var hist = ge('im_log' + peer),
                    len = hist && hist.rows.length,
                    ev;
                if (!hist) {
                ...
                each(evs, function() {
                    ...
                    if (this.error) {
                        ...
                        extend(row.insertCell(3), {
                            innerHTML: '<span>' + IM.mkdate(date + cur.tsDiff) + '</span>',
                            className: 'im_log_date'
                        });
                        extend(row.insertCell(4), {
                            className: 'im_log_rspacer'
                        });
                    }
    ...
    inviteToChat: function() {
        ...
        var mems = cur.tabs[cur.peer].data.members,
            len = 0,
            i;
        for (i in mems) len++;
    ...
    changeChatTopic: function(force) {
        ...
        var onsubmit = function() {
            ...
            box = showFastBox({
                ...
                '</div><div class="im_change_topic_labeled fl_l"><input id="im_change_topic_val" class="text" /></div></div>', getLang('global_save'), onsubmit, getLang(
                    'global_cancel'),
                function() {
                    box.hide();
                }),
    ...
    searchMessages: function() {
        var flt = ge('im_filter'),
            q = trim(val(flt)),
            btn = ge('im_search_btn'),
            bck = geByClass1('input_back_content', domPS(flt));
            ...
            ajax.post('al_im.php', {
                ...
                onDone: function(options, rows) {
                    ...
                    if (rows) {
                        ...
                        ge('im_dialogs')
                            .appendChild(ge('im_more_dialogs'));
                        IM.onScroll();
                    }
                    IM.updateCounts(options.cnts);
                    IM._restoreDialogTypings();
    ...
    logMessState: function(state, msg_id) {
        ...
        var pos = indexOf(cur.selMsgs, msg_id),
            posSp = indexOf(cur.selSpam, msg_id),
            row = ge('mess' + msg_id);
                ...
                each(cur.selMsgs, function(k, msg_id) {
                    ...
                    var tr = ge('mess' + msg_id),
                        ...
                        mRes = ce('div', {
                            ...
                            innerHTML: restore.replace(/%s/, msg_id)
                        });
                    hide(mBody);
    ...
    checkSpamMsg: function(msg_id) {
        var pos = indexOf(cur.selSpam, msg_id),
            row = ge('mess' + msg_id);
        if (!row || cur.deletedRows[msg_id]) return;
        if (pos == -1) {
    ...
    flushSpam: function() {
        var onYes = function() {
            ...
            box = showFastBox({
                ...
                dark: 1,
                bodyStyle: 'padding: 20px; line-height: 160%;'
            }, getLang('mail_delete_all_spam'), getLang('mail_delete'), onYes, getLang('mail_close'), onNo),
            onNo = function() {
                box.hide();
            };
    },
    ...
    deleteLogMsg: function(msg_id) {
        ...
        ajax.post('al_mail.php', {
            ...
            onDone: function(res, restore, actions) {
                var mBody = geByClass1('wrapped', tr),
                    mres = ce('div', {
                        id: 'mres' + msg_id,
                        innerHTML: restore
                    });
    ...
    showToDialog: function(sel) {
        ...
        if (mid > 2e9) {
            val('im_to_dialog', '<a href="/im?sel=c' + (mid - 2e9) + '" onclick="if (checkEvent(event) === false) { IM.addPeer(' + mid +
                ', false, false, true); return false; }">' + getLang('mail_im_to_multidialog') + '</a>');
            show('im_to_dialog');
        } else {
            if (!mid || !sex || !text) return;
            text = getLang('mail_im_to_dialog', 3 - sex)
                .replace('{user}', text);
            val('im_to_dialog', '<a href="/im?sel=' + mid + '" onclick="if (checkEvent(event) === false) { IM.addPeer(' + mid + ', false, false, true); return false; }">' +
                text + '</a>');
            show('im_to_dialog');
        }
    },
    initWriteDD: function() {
        if (WideDropdown.init('imw_dd', {
                ...
                custom: function(q) {
                    return (q.split('@')
                        .length == 2) ? [
                        [clean(q), clean(q), getLang('mail_enter_email_address'), '/images/pics/contact50.gif', 0, '']
                    ] : false;
                },
    ...
    restoreWriteDraft: function() {
        ...
        if (draft = ls.get('im_draft' + vk.id + '_' + peer)) {
            if (Emoji.editableVal(txt)
                .length < 2) {
                if (cur.editable) {
                    val(txt, clean(draft.txt || '')
                        .replace(/\n/g, '<br/>'));
                } else {
                    val(txt, draft.txt || '');
                }
            ...
            if ((draft.medias || [])
                ...
                ajax.post('al_im.php', {
                    ...
                    onDone: function(resp) {
                        if (cur.peer != -3 || IM.getWritePeer() != peer || !(resp || [])
                            .length) return;
                        each(resp, function() {
    ...
    initWrite: function() {
        ...
        if (!cur.imwMedia) {
            cur.imwMediaSaved = {};
            cur.imwMedia = initAddMedia('imw_attach', 'imw_media_preview', [
                ['photo', getLang('profile_wall_photo')],
                ['video', getLang('profile_wall_video')],
                ['audio', getLang('profile_wall_audio')],
                ['doc', getLang('profile_wall_doc')],
            ...
            cur.imwMedia.onChange = function(type, value, data) {
                ...
                setTimeout(function() {
                    var el = ge('imw_media_preview'),
                        cnt = geByClass1('page_pics_preview', el)
                        .childNodes.length + geByClass1('page_docs_preview', el)
                        .childNodes.length;
    ...
    sendNewMsg: function() {
        var txt = IM.getNewTxt(),
            text = Emoji.editableVal(txt),
            media = cur.imwMedia ? cur.imwMedia.getMedias() : [],
            dd = cur.wdd && cur.wdd['imw_dd'];
        if (!dd || !dd.selCount) {
            return elfocus('imw_inp');
        }
    ...
    editableHasVal: function(cont) {
        ...
        return !!(geByTag1('IMG', cont) || stripHTML(val(cont))
            .replace(/[\s\xa0]/g, '')
            .length);
    },
    ...
    getServiceMsg: function(peerId, fromId, kludges) {
        ...
        var k = 0,
            f = tm[fromId],
            u = tm[sm],
            e = kludges.source_email,
            st = kludges.source_text;
        switch (sa) {
            case 'chat_create':
                k = 'mail_im_chat_created';
                break;
            case 'chat_title_update':
                k = 'mail_im_title_updated';
                break;
            case 'chat_invite_user':
                k = (sm == fromId) ? 'mail_im_returned_to_chat' : 'mail_im_invited';
                break;
            case 'chat_kick_user':
                k = (sm == fromId) ? 'mail_im_left' : 'mail_im_kicked_from_chat';
                break;
            case 'chat_photo_remove':
                k = 'mail_im_photo_removed';
                break;
            default:
                return false;
        };

        k = langSex(f.sex, cur.lang[k])
            .replace('{from}', '<a class="im_srv_lnk" target="_blank" href="' + f.link + '">' + f.name + '</a>');
        if (sm) {
    ...
    calendarUpdTip: function(ttip) {
        ...
        el.onmouseover = showTooltip.pbind(el, {
            text: ttip || getLang('mail_im_date_search'),
            black: 1,
            shift: [10, 3, 3]
        });
    },
    calendar: function() {
        stManager.add(['ui_controls.js', 'datepicker.js', 'datepicker.css'], function() {
            if (!cur.imDP) {
                var clLnk = '<td class="im_cal_clear" colspan="7"><a onclick="IM.calendarUpd(\'clear\');" class="im_cal_clear_lnk">' + getLang(
                    'wall_clear_date_filter') + '</a></td>';
            ...
            ge('im_datesearch_cal_box')
                .style[vk.rtl ? 'right' : 'left'] = ge('im_search_btn')[vk.rtl ? 'offsetRight' : 'offsetLeft'] + 'px';
            ge('im_datesearch_cal_box')
    ...
    addPeerMsg: function(peer, msg_id, rowTitle, rowMsg, kludges) {
        ...
        if (peerTab.q_offset) {
            ...
            IM.addMsg(peer, -1, msg_id, 2, 1, rowTitle, rowMsg, Math.floor((new Date())
                .getTime() / 1000), kludges);
            setTimeout(function() {
    ...
    photoUploaded: function(info, params) {
        ...
        ajax.post('al_photos.php', extend({
            ...
            onDone: function(media, data) {
                ...
                cur.imMedia.chooseMedia('photo', media, extend(data, {
    ...
    init: function() {
        ...
        cur.imUploadInd = Upload.init('im_upload', data.url, data.params, {
            ...
            onDragEnter: function(e) {
                var h = ge('im_write_form')
                    .offsetHeight - (browser.webkit || browser.chrome ? 2 : 0),
                    dropEl = ge('im_upload_dropbox')
                    .firstChild;
                setStyle(dropEl, {
                    height: h
                });
