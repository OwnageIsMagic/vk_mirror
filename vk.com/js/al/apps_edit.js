var AppsEdit = {
    ...
    switchTab: function(tab, event, noNavGo) {
        ...
        if (el) {
            ...
            if (leftNav && tabs[0]) {
                ...
                setStyle(mark, {
                    height: getSize(tabs[0])[1],
                    top: tabs[0].offsetTop || 0
                });
    ...
    animSubTab: function(newSel) {
        var subHeader = ge('app_edit_subheader'),
            oldSel = subHeader && geByClass1('subheader_sel', subHeader);

        if (subHeader && oldSel) {
            var mark = ge('app_edit_nav_mark'),
                fromX = oldSel && oldSel.offsetLeft || 0,
                toX = newSel.offsetLeft || 0,
                duration = (toX != fromX) ? intval(70 * Math.log(Math.abs(toX - fromX) / 20)) : 0;
            setStyle(mark, {
                width: getSize(oldSel)[0],
                left: fromX
            });
    ...
    loadCheckHistory: function(aid) {
        if (isVisible('apps_check_history')) {
            ...
            ge('apps_check_history')
                .innerHTML = '<div style="text-align: center; margin: 30px;"><img src="images/progress7.gif" /></div>';
    ...
    adminApp: function(act, id, hash, action, penalty, newCheck, banDomain, warnUsers, customOption, customField) {
        var box = showFastBox(cur.appEditAdminTitle, '<div id="apps_show_penalty" style="display:none;"><div style="color:#666;padding:5px 0;">' + cur.appEditAdminPenalty +
            '</div><input type="text" id="apps_penalty" class="text" style="width:310px" value="' + penalty + '"/></div><div style="color:#666;padding:5px 0;">' + cur.appEditAdminComment +
            '</div><textarea id="apps_check_comment" style="width:310px;height:100px;"></textarea>' + (banDomain || '') + (warnUsers || '') + (customOption || ''));
        box.setOptions({
            width: 350
        });
        if (penalty > 0) {
            ...
            onUploadProgress: function(i, bytesLoaded, bytesTotal) {
                if (!ge('form' + i + '_progress')) {
                    ...
                    obj.appendChild(ce('div', {
                        innerHTML: '<div class="apps_info_progress_wrap">\
          <div id="form' + i +
    ...
    ssOver: function(obj) {
        ...
        if (!nc) {
            var nc = se('<div class="apps_edit_screen_close" onmouseover="showTooltip(this, {black: 1, text: \'' + cur['remove_screenshot'] +
    ...
    bClick: function(obj, uplNum, newFormat) {
        ...
        img.src = newFormat ? (window.devicePixelRatio >= 2 ? '/images/dquestion_q.png' : '/images/dquestion_t.png') : (window.devicePixelRatio >= 2 ?
            '/images/dquestion_w.png' : '/images/dquestion_z.png');
    ...
    enableOrders: function(aid, hash, force) {
        ...
        ajax.post('editapp', {
            ...
            onDone: function(result, text) {
                if (result) {
                    ge('apps_new_api_notice')
                        .innerHTML = text;
                } else {
                    var confirmBox = showFastBox(getLang('global_action_confirmation'), text, getLang('box_yes'), function() {
                        confirmBox.hide();
                        AppsEdit.enableOrders(aid, hash, 1);
                    }, getLang('global_cancel'));
                }
            },
    ...
    removeFunc: function(id) {
        ...
        if (cur.funcIds.length == 0) {
            ...
            if (!hasVisible) {
                ...
                var msg = getLang('developers_no_funcs_found')
                    .split('{query}')
                    .join('<b>' + this.cleanStr(searchText) + '</b>');
    ...
    initVersionDropdown: function(id) {
        ...
        ge('apps_edit_const_v_' + id)
            .value = cur.lastVersion;
    ...
    cancelFuncChange: function(id) {
        ...
        var box = showFastBox({
            ...
            dark: 1
        }, cur.lang['developers_cancel_func_confrim'].replace('%s', clean(name)), cur.lang['developers_do_cancel'], function() {
            if (!func['name']) {
                AppsEdit.removeFunc(id);
            } else {
                if (codeEl && codeEl.ace) {
    ...
    runFunc: function(id, btn) {
        ...
        ajax.post('dev', params, {
            onDone: function(code) {
                ...
                }, '<div id="dev_result" onmousemove="Dev.resultMove(event.target);" onmouseout="Dev.resultMove(false);">' + html + '</div>');
            },
            onFail: function(msg) {
    ...
    adjustHeight: function(editor, el) {
        ...
        var newHeight = (editor.getSession()
            .getScreenLength() * lineHeight) + editor.renderer.scrollBar.width;
        var row = domPN(el);
    ...
    initEditor: function(el, id) {
        ...
        session.on('tokenizerUpdate', function() {
            AppsEdit.updateExecuteParams(editor, el)
        });
    },
    ...
    updateExecuteParams: function(editor, el) {
        ...
        if (newParams.length == 1 && removedParams.length == 1) {
            ...
            geByClass1("execute_field_name", oldEl)
                .innerHTML = newName;
        } else {
            var paramContainer = geByClass1("apps_edit_params", row);
            ...
            for (var i = 0; i < newParams.length; i++) {
                ...
                var newEl = ce('div', {
                    className: "execute_param_" + newName,
                    innerHTML: "<div class='execute_field_name'>" + newName + "</div><input class='text execute_field_input' type='text'>"
                });
    ...
    onDestroyFuncs: function(changed, old, n, opts) {
        ...
        var box = showFastBox({
            ...
            dark: 1
        }, message, getLang('global_continue'), function() {
            cur.leaving = true;
            box.hide();
            if (cur.onContinueCb) {
                cur.onContinueCb();
    ...
    saveFuncsPosition: function() {
        ...
        for (var i in elems) {
            if (elems[i] && elems[i].nodeType == 1) {
                var name = geByClass1('apps_edit_cont_name', elems[i])
                    .value;
            } else {
                name = null;
            }
    ...
    putVersionCode: function(id, v) {
        ...
        if (v == -1) { // default
            ge('func_remove_btn' + id)
                .innerHTML = cur.lang.developers_func_remove;
        } else {
            ge('func_remove_btn' + id)
                .innerHTML = cur.lang.developers_remove_func_version;
        }
    ...
    defaultActivityData: function() {
        var sample = unclean(cur.activitySamples)
            .split('<#>');
    ...
    addActivity: function() {
        ...
        geByClass1('apps_edit_activity_name', row)
            .select();
    ...
    showRequestStatusTT: function(el, id) {
        ...
        switch (request.status) {
            case 0:
                msg = getLang('apps_edit_request_created');
                break;
            case 1:
                msg = getLang('apps_edit_request_accepted');
                break;
            case 2:
                msg = getLang('apps_edit_request_declined');
                break;
        }
    ...
    showActivityStatusTT: function(el, id) {
        ...
        switch (activity.status) {
            case 0:
                msg = getLang('apps_edit_activity_created');
                break;
            case 1:
                msg = getLang('apps_edit_activity_accepted');
                break;
            case 2:
                msg = getLang('apps_edit_activity_declined');
                break;
        }
        window.tooltips && tooltips.hideAll();
        showTooltip(el, {
    ...
    cancelActivityChange: function(id) {
        ...
        var row = ge('activity_row_' + id),
            activity = cur.activities[id],
            content = geByClass1('apps_edit_content', row);
    ...
    cancelRequestChange: function(id) {
        ...
        var box = showFastBox({
            ...
            .replace('%s', clean(name)), getLang('developers_do_cancel'),
            function() {
                if (!request) {
                    AppsEdit.removeRequest(id);
                } else {
    ...
    removeActivityBox: function(id) {
        ...
        var box = showFastBox({
            ...
            .replace('%s', clean(name)), getLang('developers_do_remove'),
            function() {
                if (!activity) {
                    AppsEdit.removeActivity(id);
                } else {
    ...
    searchList: function(str) {
        ...
        if (str && !shownCount) {
            var msg = notFoundMsg.split('{query}')
                .join('<b>' + this.cleanStr(str) + '</b>');
            errorPanel.innerHTML = msg;
            show(errorPanel);
        } else {
            hide(errorPanel);
    ...
    uSearchUpdate: function() {
        if ((cur.searchInp || {})
            .id != 'apps_edit_search_inp') return;
    ...
    uShowMore: function(force) {
        ...
        var len = lst.length,
            cont = ge('apps_edit_users_rows'),
            more = ge('apps_edit_users_more');
        if (!len) {
    ...
    uSelAdminLevel: function(el, index, name) {
        ...
        var desc = ge('apps_admin_partial_level_desc')
    ...
    uSaveAdmin: function(mid, hash) {
        ...
        each(geByClass('checkbox', ge('apps_edit_admin_rights')), function(i, el) {
            if (isChecked(el)) {
                params.rights.push(el.getAttribute('id')
    ...
    cloneStoredFuncs: function(hash) {
        ...
        var box = showFastBox({
            ...
            ajax.post('al_apps_edit.php', {
                act: 'clone_funcs',
                from_aid: cur.aid,
                to_aid: self.funcsCloneChoser.val(),
                hash: hash
            }, {
                onDone: function() {
