    ...
    showEvent: function(ev, force) {
        ...
        ev.baloonWrapEl = ce('div', {
            ...
            innerHTML: '<div class="notifier_baloon ' + typeClassName +
                '"><div class="notifier_baloon_head clear_fix"><a class="notifier_close_wrap" role="link" title="' + getLang('global_close') + '" aria-label="' +
                getLang('global_close') + '"></a><h4 class="notifier_baloon_title">' + ev.title + '</h4></div><div class="notifier_baloon_body clear_fix">' + (ev.author_photo &&
                    ('<div class="notifier_image_wrap">' + (ev.author_link && ('<a href="' + ev.author_link + '">')) + thumbEl + (ev.author_link && '</a>') +
                        '</div>')) + (ev.add_photo && ('<div class="notifier_add_image_wrap"><img src="' + ev.add_photo + '" class="notifier_add_image" /></div>')) +
...
FastChat = {
    ...
    tabNotify: function(peer, evType, evData) {
        ...
        switch (evType) {
            ...
            case 'unavail':
                text = getLang('mail_im_not_online', 3 - tab.sex)
                    .replace(/\.$/, "");;
                break;
        }
    ...
    clistWrapPeer: function(id, data, re) {
        ...
        return '<a href="' + href + '" class="fc_contact clear_fix" id="fc_contact' + id + '" onclick="return FastChat.selectPeer(' + id +
            ...
            cls + '" ' + photoEvents + '>' + photoStr + '</span><span class="fc_contact_status"></span><span class="fc_contact_name">' + name +
            '<span id="fc_contact_unread' + id + '" class="fc_contact_unread">' + (unread ? ' <b>+' + unread + '</b>' : '') + '</span></span></a>';
    },
    ...
    addTabIcon: function(peer, data, noAnim) {
        ...
        var t = se('<a class="chat_tab_wrap' + (noAnim ? '' : ' chat_tab_beforeanim') + '" id="chat_tab_icon_' + peer + '" href="' + peerHref +
            ...
            '</div><div class="chat_tab_typing_wrap"><div class="chats_sp chat_tab_typing_icon"></div></div></a>');
    ...
    updateTyping: function(peer, force) {
        ...
        if (typings.length == 1) {
            val(txtEl, langSex(sex, lang.mail_im_typing)
                .replace('{user}', typings[0]));
        } else {
            var lastUser = typings.pop();
            val(txtEl, getLang('mail_im_multi_typing')
    ...
    getEditCont: function(emojiId) {
        ...
        return '<div class="emoji_cont _emoji_field_wrap">' + Emoji.tplSmile(getLang('mail_emoji_hint')) +
    ...
    initTab: function(peer, data, wrap) {
        ...
        stManager.add(['page.js', 'page.css', 'ui_media_selector.js', 'ui_media_selector.css'], function() {
            tab.imMedia = new MediaSelector(tab.addMediaBtn, tab.previewEl, [
...
var TopNotifier = {
    ...
    preload: function() {
        ...
        ajax.post('/al_feed.php', extend(this._qParams, {
            _preload: 1
        }), {
            cache: 1,
    ...
    show: function(ev) {
        ...
        if (!cont) {
            var c = ce('div', {
                innerHTML: '<div class="top_notify_header"><a href="/settings?act=notify" class="top_notify_prefs_lnk" onclick="return nav.go(this, event);" onmousedown="event.cancelBubble = true;" ontouchstart="event.cancelBubble = true;">' +
                    getLang('global_notifications_settings') + '</a>' + getLang('global_last_notifitications') +
        ...
        setStyle(cont, {
            'maxHeight': Math.max(wHeight - 200, 300)
        });
        TopNotifier.refresh();
    ...
    hideRow: function(el, item, hash) {
        ...
        slideUp(row, 200, function() {
            re(row);
            if (!geByClass('feed_row', 'top_notify_cont')
                .length) {
