    ...
    e.getListeners = function(n) {
        ...
        for (t in n)
            if (n.hasOwnProperty(t))
                for (i = n[t].length; i--;) e = n[t][i], e.once === !0 && this.removeListener(r, e.listener), s = e.listener.apply(this, o || []), s === this._getOnceReturnValue() &&
...
Notifier = {
    ...
    init: function(options) {
        ...
        if (!(curNotifier.cont = ge('notifiers_wrap'))) {
            bodyNode.insertBefore(curNotifier.cont = ce('div', {
                id: 'notifiers_wrap',
                className: 'fixed'
            }), ge('page_wrap'));
        }
    ...
    initFrameTransport: function() {
        ...
        curNotifier.post_message = Notifier.debug || !(browser.opera && intval(browser.version) < 15 || browser.msie || browser.mozilla && majorVersion >= 31 || browser.safari &&
            (majorVersion > 7 || majorVersion == 7 && minorVersion >= 1));
    ...
    pushEvent: function(msg, cnt) {
        ...
        switch (ev.type) {
            ...
            case 'mail':
                ...
                if (cur.module != 'im') {
                    FastChat.prepareTabIcon(intval(ev['author_id']), {
                        fixedLoad: 1
                    });
                }
                break;
    ...
    showEvent: function(ev, force) {
        ...
        ev.baloonWrapEl = ce('div', {
            ...
            innerHTML: '<div class="notifier_baloon clear_fix"><div class="notifier_baloon_head clear_fix"><div class="notifier_baloon_title fl_l">' + ev.title +
                '</div><div class="notifier_close_wrap fl_r"><a class="notifier_close" title="' + getLang('global_close') +
                '" href=""></a></div></div><div class="notifier_baloon_body"><table cellpadding="0" cellspacing="0" width="100%"><tr>' + (ev.author_photo && (
                    '<td class="notifier_image_wrap"><div class="notifier_image_wrap">' + (ev.author_link && ('<a href="' + ev.author_link +
                        '" onclick="return nav.go(this, event);">')) + thumbEl + (ev.author_link && '</a>') + '</div></td>')) +
                '<td class="notifier_baloon_msg"><div class="notifier_baloon_msg wrapped" style="width: ' + (300 - 60 * ((ev.author_photo && ev.add_photo) ? 2 : ((
                    ev.add_photo || ev.author_photo) ? 1 : 0))) + 'px;">' + ev.text + '</div></td>' + (ev.add_photo && (
                    '<td class="notifier_add_image_wrap"><div class="notifier_image_wrap' + imgClass + '"><img src="' + ev.add_photo +
                    '" class="notifier_image" /></div></td>')) + '</tr></table></div></div>'
        });
        ...
        ev.startFading = function() {
            ev.fading = animate(ev.baloonEl, {
                    opacity: 0
                }, 1000, Notifier.hideEvent.bind(Notifier)
                .pbind(ev, false));
            if (ev.over) {
                ev.fading.stop();
            }
    ...
    showEventUi: function(ev) {
        ...
        var title, text;
        if (ev.type === 'mail') {
            title = stripHTML(replaceEntities(ev.text)
                    .replace(/<br>/g, "\n")
                    .replace(/<span class='notifier_author_quote'.*?>(.*?)<\/span>.*$/, '$1'))
                .replace(/&laquo;|&raquo;/gi, '"');
            text = stripHTML(replaceEntities(ev.text)
                    .replace(/<br>/g, "\n")
                    .replace(/<span class='notifier_author_quote'.*<\/span>(.*?)/, '$1')
        ...
        notification.show();
        ev.closeTO = setTimeout(Notifier.hideEvent.bind(Notifier)
            .pbind(ev), 5000);
        return true;
    },
    ...
    lcStart: function() {
        if (Notifier.lcCheckServer()) {
            ...
            curNotifier.becomeServerTO = setTimeout(this.lcServer.bind(this)
                .pbind(true), 500);
        }
        ...
        curNotifier.lpMakeRequest(curNotifier.frame_url, params, function(text) {
            ...
            try {
                ...
                if (text && text.indexOf('Ad Muncher') == -1) {
                    topError('Notifier error: ' + e.message, {
                        dt: -1,
                        type: 5,
                        stack: e.stack,
...
function Sound(filename, opts) {
    ...
    if (audioObjSupport) {
        ...
        var wrap = ge('flash_sounds_wrap') || utilsNode.appendChild(ce('span', {
...
function updateWndVScroll() {
    ...
    each(curRBox.tabs, function(id) {
        if (this.toRight) {
            setStyle(this.wrap, {
                marginRight: vScroll ? sbWidth() + 1 : 0
            });
        }
    });
}
...
function defBox(options, callback) {
    var boxC = '<div class="' + (options.subClass || '') +
...
function RBox(content, options) {
    ...
    t.wrap = ce('div', {
        id: id,
        className: 'rb_box_wrap fixed' + (options.fixed ? ' fc_fixed' : '')
    });
    var pos = {};
    ...
    if (this.toRight) {
        setStyle(t.wrap, {
            marginRight: lastWndScroll[0] ? sbWidth() + 1 : 0
        });
    }
...
extend(RBox.prototype, {
    ...
    _focus: function() {
        ...
        var zIndex = 700 + curRBox.focused.length,
            first = true;
...
FastChat = {
        ...
        ajax.post('al_im.php', {
            ...
            onDone: function(data) {
                if (data.friends == -1) {
                    ...
                    ls.set('fcFriends' + vk.id, {
                        version: data.friends_version,
                        list: data.friends
                    });
                }
    ...
    sendSettings: function() {
        ...
        var settings = {},
            k = ['friends', 'friends_version', 'onlines', 'tpl', 'lang', 'me', 'version', 'im_queue', 'cl_queue'],
            i;
        for (i in k) {
        ...
        switch (act) {
            ...
            case 'needPeer':
                ...
                if (tab !== undefined) {
                    ...
                    peerData = {
                        ...
                        hash: mem[3],
                        data: mem[4],
                        online: curFastChat.onlines[peer]
                    };
                }
                if (peerData === false) {
                    break;
            ...
            case 'needMedia':
                var msgId = data.msgId,
                    msgMedia = curFastChat.gotMedia[msgId];
            ...
            case 'fetchingMedia':
                // if (!curFastChat.version) break;
                var msgId = data.msgId,
                    msgNeed = curFastChat.needMedia[msgId];
    ...
    clFeed: function(events) {
        ...
        each(events, function() {
            var ev = this.split('<!>'),
                ...
                onltype = ev[3] ? ev[3] : 1,
                tab = curFastChat.tabs[peer],
                wasOnline = curFastChat.onlines[peer];
                    ...
                    stManager.add(['im.js'], function() {
                        each(tab.sentmsgs, function(k, msgId) {
                            var row = ge('fc_msg' + msgId),
                                parent = row && row.parentNode;
                            if (re(row) && parent && !geByClass('fc_msg', parent)
                                .length) {
    ...
    tabNotify: function(peer, evType, evData) {
        ...
        var addClassTo = geByClass1('fc_tab', tab.wrap, 'div'),
            mob = (evData > 0 && evData < 6),
            cls = mob ? 'fc_tab_mobile' : 'fc_tab_online';
        if (evType == 'online') {
        ...
        setStyle(notify, {
            width: tab.logWrap.clientWidth - 8 /*, zIndex: 400*/
        });
        clearTimeout(tab.hideNotifyTO);
    ...
    initUI: function() {
        ...
        if (el.clistOnline) {
            var lShift, probe;
            bodyNode.appendChild(probe = ce('nobr', {
                className: 'fl_l',
    ...
    itemsTT: function(ev) {
        ...
        if (!tt) {
            Chat.userNameTT = ce('div', {
                className: 'chat_tab_info_wrap',
                innerHTML: '<div class="chat_tab_info_tt"><div class="chat_tab_info_p"></div><div id="chat_tab_info_text"></div></div>'
            }, {
                top: newTop - Chat.scrollNode.scrollTop + FastChat.itemsOffset,
                right: 70,
                opacity: 0
            });
    ...
    onStateChanged: function(data) {
        ...
        switch (data.op) {
            case 'added':
                ...
                if (data.fixed) {
                    FastChat.prepareTabIcon(data.peer, {
                        fixedLoad: true
                    })
                } else {
    ...
    clistRender: function(more) {
        ...
        each(curFastChat.friends, function(k) {
            var mid = intval(k),
                matches = !filterList || filterList[mid],
    ...
    clistWrapPeer: function(id, data, re) {
        ...
        return '<a href="' + href + '" class="fc_contact clear_fix' + cls + '" id="fc_contact' + id + '" onclick="return FastChat.selectPeer(' + id +
            ', event);" onmousedown="event.cancelBubble = true;" onmouseover="FastChat.clistPeerOver(this, 1, event);"  onmouseout="FastChat.clistPeerOver(this, 0, event);"><span class="fc_contact_photo fl_l" ' +
            photoEvents + '>' + photoStr + '</span><span class="fc_contact_name fl_l">' + name + '<span id="fc_contact_unread' + id + '" class="fc_contact_unread">' + (
                unread ? ' <b>+' + unread + '</b>' : '') + '</span></span><span class="fc_contact_status fl_l"></span></a>';
    },
    ...
    authorOver: function(obj, ev) {
        ...
        var offsetAuthor = obj.getBoundingClientRect()
            .top;
        var offsetContainer = container.getBoundingClientRect()
            .top;
        ...
        ajax.post('hints.php', {
            ...
            onDone: function(peers) {
                ...
                var wrap = el.parentNode,
                    div = ce('div', {
                        innerHTML: FastChat.wrapCorrespondents(correspondents, re)
                    }),
                    frag = document.createDocumentFragment();
                if (div.firstChild) {
                    while (div.firstChild) {
    ...
    onDocClick: function(e) {
        ...
        while (el) {
            if (el.className == 'fc_tab_wrap' || el.id == 'chat_onl_wrap' || el.id == 'custom_menu_cont' || el.id == 'layer_wrap' || el.id == 'box_layer_wrap' || el.id ==
                'wk_layer_wrap') {
                return true;
            }
            el = el.parentNode;
        ...
        if (tab) {
            if (trim(Emoji.editableVal(tab.txt)) || (tab.imMedia && tab.imMedia.getMedias()
                    .length)) {
                return true;
            }
        }
    ...
    clistCache: function(q) {
        if (q) {
            ...
            for (i in queries) {
                query = queries[i];
                if (cached = curFastChat.clistCache[' ' + query.charAt(0)
                        .toLowerCase()]) {
    ...
    clistUpdateTitle: function(rendered) {
        ...
        newVal = (cnt1 ? getLang('mail_im_X_onlines_title', cnt1) : getLang('mail_im_onlines_title'))
            .toString();
        ...
        if (curFastChat.clistBoxScroll) {
            if (!curFastChat.clHasMore && rendered) {
                ...
                cnt = intval((curFastChat.clistCache[curFastChat.q] || {})
                    ._num);
            }
    ...
    clistFilterKey: function(e) {
        ...
        switch (e.keyCode) {
            ...
            case KEY.UP:
                if (e.type != 'keyup') {
                    if (el = curFastChat.clSel && ge('fc_contact' + curFastChat.clSel)) {
                        var nextKey = e.keyCode == KEY.DOWN ? 'nextSibling' : 'previousSibling',
                            nextEl = el;
                        do {
            ...
            case KEY.ESC:
                if (e.type != 'keyup') {
                    var filter = ge('fc_clist_filter'),
                        prevVal = val(filter) || curFastChat.clSel;
                    filter.blur();
    ...
    addTabIcon: function(peer, data, noAnim) {
        ...
        var t = se('<a class="chat_tab_wrap' + (noAnim ? '' : ' chat_tab_beforeanim') + (data.online ? ' chat_tab_online' : '') + '" id="chat_tab_icon_' + peer +
            ...
            ', event);"><div class="chat_tab_imgcont"><div class="chats_sp chat_tab_online_icon"></div><div class="chat_tab_typing_wrap"><div class="chats_sp chat_tab_typing_icon"></div></div><div class="chat_tab_close" onclick="return FastChat.closeTabIcon(' +
            peer + ', event)"></div>' + imgRow + '</div></a>');
    ...
    scrollWrap: function(event) {
        ...
        if (Chat.ttPeer) {
            setStyle(Chat.userNameTT, {
                top: Chat.ttTop - Chat.scrollNode.scrollTop + FastChat.itemsOffset
            });
        }
    ...
    movePointer: function(peer, aminate) {
        ...
        if (aminate) {
            if (curFastChat.prevPointer) {
                ...
                setStyle(pOffset, {
                    bottom: curFastChat.prevPointer - tdiff + shift
                });
            }
            animate(pOffset, {
                bottom: tabDiff
    ...
    moveBoxesLeft: function(x, rec) {
        ...
        if (mostRight) {
            ...
            FastChat.stateChange({
                op: 'moved',
                peer: mostRight.box.options.peer,
                y: newY / wndInner[0],
                x: newX / wndInner[1]
            });
            if (newX) {
    ...
    pinTab: function(peer, ev, fast) {
        ...
        setStyle(t.wrap, {
            left: t.wrap.offsetLeft,
            top: t.wrap.offsetTop,
            right: 'auto',
            bottom: 'auto'
        });
        if (!fast) {
        ...
        if (peer == -1) {
            FastChat.stateChange({
                op: 'clist_toggled',
                val: 1,
                y: t.toBottom ? -1 : t.pos[0] / wndInner[0],
                x: t.toRight ? -1 : t.pos[1] / wndInner[1]
            });
        } else {
            FastChat.stateChange({
                op: 'unfixed',
    ...
    decHashCb: function(hash) {
        (function(_) {
            curFastChat.decodedHashes[_] = (function(__) {
                var ___ = ge ? '' : '___';
    ...
    updateTyping: function(peer, force) {
        ...
        if (typings.length == 1) {
            val(el, langSex(sex, lang.mail_im_typing)
        ...
        message = message.replace(
            ...
            function() { // copied to notifier.js:3401
                ...
                if (topDomain.length > 7 || indexOf(
                        ...
                        .split(','), topDomain) == -1) return matches[0];
    ...
    getEditCont: function(emojiId) {
        ...
        return '<div class="emoji_cont">' + Emoji.tplSmile(emojiId, getLang('mail_emoji_hint'), ' fc_emoji') +
            '<div class="fc_editable" tabindex="0" contenteditable="true"></div></div>';
    },
    getVal: function(obj) {
        //return obj.value;
    ...
    onTxtResize: function(peer) {
        ...
        if (h > 40) {
            ...
            setStyle(tab.box.resizeableH, {
                height: resH + (tab.hDiff || 0) - hDiff
            });
            tab.hDiff = hDiff;
    ...
    initTab: function(peer, data, wrap) {
        ...
        stManager.add(['page.js', 'page.css'], function() {
            tab.imMedia = initAddMedia(tab.addMediaBtn, tab.previewEl, [
    ...
    addBox: function(peer, data, options) {
        ...
        var wrap = se(rs(FastChat.tplBox, {
            id: peer,
            name: data.name,
            myphoto: Notifier.fixPhoto(curFastChat.me.photo, true),
            classname: data.online ? (data.online > 0 && data.online < 6 ? ' fc_tab_mobile' : ' fc_tab_online') : '',
            cont: editCont
        }));

        if (options.fixed && curFastChat.activeBox) {
            curFastChat.activeBox.hide(0, false, {
                noState: true
            });
        }
        ...
        wndInner = getWndInner(),
            opts = {
                ...
                onFocus: function(e) {
                    ...
                    if (tab.wrap.clientWidth) setStyle(tab.title, {
                        maxWidth: tab.wrap.clientWidth - 71
                    });
                    if (!tab.editable) {
                        setStyle(tab.txt.autosize.helper, {
                            width: getStyle(tab.txt, 'width', false)
                        });
                    }
                    tab.scroll && tab.scroll.update(false, true);
                ...
                onClose: function(pos) {
                    ...
                    var tabs = curFastChat.tabs,
                        posSeq = tabs[peer].posSeq;
                    delete tabs[peer];
                ...
                onResizeEnd: function(h, w) {
                    ...
                    FastChat.stateChange({
                        ...
                        h: h / wndInner[0],
                        w: w / wndInner[1],
                        y: tab.box.toBottom ? -1 : pos[0] / wndInner[0],
                        x: tab.box.toRight ? -1 : pos[1] / wndInner[1]
                    });
        ...
        if (opts.startLeft === undefined && opts.startRight === undefined) {
            var xs = [],
                minTop = wndInner[0] - 350,
                pos = curFastChat.clistBox.pos;
            var snapRight = false;
            ...
            var startX = lastWindowWidth - 262 - sbWidth(),
                endX = 0,
                minLayersX = false,
                minLayersCnt = false,
                curX, curCnt, j, sign = endX > startX ? 1 : -1;
    ...
    sendOnResponse: function(response, msgId, tab) {
        ...
        var row = ge('fc_msg' + msgId),
            realMsgId = response.msg_id,
            pos = indexOf(msgId, tab.newmsgs);
        if (!row) return;

        if (response.media) {
            var msgOpts = {
                sticker: intval(response.sticker)
            };
            FastChat.lcSend('gotMedia', {
                msgId: msgId,
                peer: tab.box.options.peer,
                text: response.media,
                msgOpts: msgOpts
            });
    ...
    blinkTab: function(peer) {
        ...
        var wrap = tab.box.wrap,
            className = wrap.className,
            zIndex = Math.min(700, intval(getStyle(wrap, 'zIndex')));
        setStyle(wrap, {
            zIndex: 700
        });
        removeClass(wrap, 'rb_inactive');
    ...
    createProgress: function(row, id, after, cls) {
        var el = ce('span', {
            className: 'fc_msg_progress progress ' + cls,
            id: 'fc_msg_progress' + id
        });
    ...
    error: function(peer, msg) {
        ...
        var tab = curFastChat.tabs[peer];
        re('fc_error' + peer);
        tab.log.appendChild(ce('div', {
            id: 'fc_error' + peer,
            className: 'fc_msgs_error',
            innerHTML: msg || getLang('global_error')
        }));
        FastChat.scroll(peer);
    },
    ...
    prepareMsgData: function(arr) {
        var peer = arr[0],
            flags = intval(arr[2]),
            from_id = flags & 2 ? curFastChat.me.id : (peer > 2e9 ? arr[5] : peer),
            date = intval(vkNow() / 1000),
            data = {
                id: arr[1],
                peer: peer,
                from_id: from_id,
        ...
        if (attFlags) { // Media
            ...
            if (attFlags & 6) {
                attText += rs(curFastChat.tpl.msg_fwd, {
                    msg_id: arr[1],
                    peer_nice: FastChat.nicePeer(peer),
                    label: getLang(attFlags & 2 ? 'mail_im_fwd_msg' : 'mail_im_fwd_msgs')
                });
            }
            if (attFlags & 8) {
                data.sticker = true;
