...
Notifier = {
    ...
    initFlashTransport: function(options) {
        ...
        if (!renderFlash(queueCont, {
                url: curNotifier.flash_url,
                id: 'queue_transport',
    ...
    showEventUi: function(ev) {
        ...
        var text = stripHTML(replaceEntities(ev.text)
                .replace(/<br>/g, "\n")
...
FastChat = {
        ...
        ajax.post('al_im.php', params, {
            ...
            showProgress: function() {
                ...
                tab.sendProgressTO = setTimeout(function() {
                    var row = ge('fc_msg' + msgId);
                    if (!row) return;
                    row.insertBefore(ce('span', {
                        className: 'fc_msg_progress progress',
                        id: 'fc_msg_progress' + msgId
                    }), row.firstChild);
                }, 2000);
            },
            hideProgress: function() {
    ...
    addMsg: function(data) {
        ...
        if (!last || !hasClass(last, 'fc_msgs_wrap') || last.getAttribute('data-from') != data.from_id || data.date - intval(last.getAttribute('data-date')) >= 300 || data
            .sticker || hasClass(last, 'fc_msg_sticker')) {
