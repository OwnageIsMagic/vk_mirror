...
AdsModer.saveFeatures = function(unionId, hash, featuresInfo, editBox) {
    ...
    for (var i in featuresInfo) {
        var featureInfo = featuresInfo[i];
        ajaxParams.features.push(featureInfo.key + ':' + intval(ge('ads_moder_feature_' + featureInfo.key)
            .value));
    }
...
AdsModer.openDomainCommentEditBox = function(domain, reload, buttons) {
    ...
    ajax.post('/adsmoder?act=a_get_domain_note', {
        ...
        onDone: function(data) {
            ...
            var info_string = '';
            if (data.exists) {
                info_string = '<div class="ads_premoderation_domain_info">����� <b>' + domain + '</b> � ��������� ��� �������������� ' + data.user + ' <b><nobr>' +
                    data.update_time + '</nobr></b></div>';
            } else {
...
AdsModer.openTemplateEditBox = function(template_id, buttons) {
    ...
    ajax.post('/adsmoder?act=a_get_template', {
        ...
        onDone: function(data) {
            var info_string = '';
            if (data.exists) {
                info_string = '<div class="ads_premoderation_domain_info">������ � ��������� ��� �������������� ' + data.user + ' <b><nobr>' + data.update_time +
                    '</nobr></b></div>';
            }
            var full_text_box;
            var short_title_box;
            ...
            short_title_box = ' <input id="ads_template_title_edit" type="text" placeholder="�������� ��������� �������" class="text" value="' + (data.template ?
                data.template.short_title : '') + '">';
            full_text_box = ' <textarea id="ads_template_text_edit" placeholder="������ ����� �������">' + (data.template ? data.template.full_text : '') +
                '</textarea>';
...
AdsModer.saveTemplate = function(action, template_id) {
    ...
    if (action == 'disable') {
        ...
        ajax_params.short_title = ge('ads_template_title_edit')
            .value;
        ajax_params.full_text = ge('ads_template_text_edit')
            .value;
    }
...
AdsModer.saveDomainComment = function(new_status, reload) {
    ...
    var ajax_params = {
        ...
        comment: ge('ads_domain_comment_edit')
            .value,
...
AdsModer.openNoteEditBox = function(ajaxParams, noteText, noteTextParams, boxTitle, editActionText, isEdit, isOtherUser) {
    ...
    if (isEdit) {
        cur.noteEditBox.setControlsText('<a href="#" onclick="AdsModer.saveNote(true); return false;">�������</a>');
    }
}
...
AdsModer.saveNote = function(isDelete) {
    ...
    ajaxParams.note_text = (isDelete ? '' : ge('ads_note_edit')
        .value);
...
AdsModer.premoderationProcessRequest = function(action, requestKey, requestKeyModer, onCompleteExternal) {
    ...
    if (action !== 'disapprove' ||
        (action === 'disapprove' && (moderComment.getValue()
            .trim()
            .length || cur.uiReasonsControls[requestKeyModer].getSelectedItems()
            .length))) {
    ...
    if (action === 'disapprove') {
        ajaxParams.moder_rules = cur.uiReasonsControls[requestKeyModer].getSelectedItems()
            .join(',');
    }
...
AdsModer.premoderationProcessRequestsMassCheck = function(action, requestKey) {
    ...
    var confirmText = '������� ���������� �� ������� ��������: ' + requestsKeys.length;
...
AdsModer.statSummaryDistrShowCountLabel = function(id, i, val) {
    ...
    setStyle(ge(id + '_max_label_out'), {
        width: ge(id + '_max_label')
            .offsetWidth + 2,
        height: ge(id + '_max_label')
            .offsetHeight + 2,
...
AdsModer.openDisableOfficeBox = function(ajaxParams, isCompany) {
    ...
    cur.lang.ads_disable_office_confirm_message =
        '�� ������� ��� ������ ������� ��������� �������?<br><br>����� �������� �������� ��������� ������ � ����������, �� ����� �������� �������� ����� ����������.';
    ...
    var messageBox = showFastBox(getLang('ads_disable_office_box_title'), message, getLang('ads_disable_office_box_button'), function() {
        AdsModer.disableOffice(ajaxParams, messageBox);
    }, getLang('box_cancel'));
}
