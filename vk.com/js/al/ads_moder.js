    ...
    var options = {
        tooltip: {
            ...
            formatter: function() {
                ...
                s += (is_cpc ? '������� ���������' : '������� �������') + ': ';
                s += langNumeric(Number(count * (is_cpc ? 1 : 1000))
                    .toFixed(0), "<b>%s</b>", true) + "<br>";
                if (ratio_index < this.points.length) {
                    s += "������� �������: " + getLang('ads_moder_bold_money_amount_rub_short', Number(count * this.points[ratio_index].y)
                        .toFixed(2)
                        .replace('.00', ''), true) + "<br>";
                }
                for (var i = 0; i < this.points.length; i++) {
                    var point = this.points[i];
                    s += '<br><span style="color:' + point.series.color + '">\u25CF</span> ' + point.series.name + ': ' + getLang(
                        'ads_moder_bold_money_amount_rub_short', Number(point.y)
                        .toFixed(2)
                        .replace('.00', ''), true);
                }
                return s;
...
AdsModer.searchAdsText = function() {
    var text = ge('ads_moder_search_ads_text')
        .value;
