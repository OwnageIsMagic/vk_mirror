var Market = {
    ...
    toggleExtendedControls: function(show) {
        var wrapEl = ge('market_search_wrap'),
            controlsEl = ge('market_search_controls');
        if (show == undefined) {
    ...
    uploadInit: function(cont, opts, isMain) {
        ...
        return Upload.init(cont, opts.url, cur.options.photoVars, extend(options, {
            ...
            onUploadComplete: function(info, res, errorAdd) {
                ...
                if (isMain) {
                    var url = Upload.options[cur.uploadId].base_url + 'upload.php?act=market_photo_crop&_query=' + encodeURIComponent(res) + '&_origin=' +
                ...
                if (isMain) {
                    if (!ge('form' + i + '_progress')) {
                        ...
                        obj.appendChild(ce('div', {
                            innerHTML: '<div class="market_upload_progress_wrap">\
              <div id="form' + i +
                                '_progress" class="market_upload_progress" style="width: 0%;"></div>\
            </div></div>'
                        }, {
                            height: tm + 'px',
                            marginTop: -tm + 'px'
    ...
    uploadExtraPhotos: function(ev) {
        if (cur.extraUploadId !== undefined && cur.uploadId !== undefined && window.Upload && Upload.checked && Upload.checked[cur.uploadId]) {
            geByTag1('input', 'market_ei_photo_upload')
                .click();
            return false;
        }
        return true;
    ...
    uploadFail: function(isMain, info, err) {
        ...
        switch (code) {
            ...
            case 'ERR_UPLOAD_BAD_IMAGE_SIZE':
                msg = getLang('profile_oph_err_size')
                    .replace('{min}', '400')
    ...
    choosePhoto: function(info, media, data) {
        ...
        var mediaHtml = '<div class="market_ei_photo_wrap market_ei_photo%ind% inl_bl" id="market_ei_photo%ind%">' + preview +
            '<div nosorthandle="1" class="market_x_button inl_bl" ' + (browser.msie ? 'title' : 'tooltip') + '="' + getLang('dont_attach') +
            '" onmouseover="if (browser.msie) return; showTooltip(this, {text: this.getAttribute(\'tooltip\'), shift: [14, 3, 3], black: 1})" onclick="Market.unchoosePhoto(%ind%); return cancelEvent(event);"><div class="market_x" nosorthandle="1"></div></div></div></div>',
  ...
  <div class="market_prg_wrap">\
    <div id="upload' + ind + '_progress" class="market_prg" style="width: ' + percent +
                '%;"></div>\
  </div></div>';
            var progressEl = ce('div', {
                id: 'upload' + ind + '_progress_wrap',
                innerHTML: '<div class="fl_l">' + progress + '</div>' + (label ? '<div class="market_prg_label fl_l">' + label + '</div>' : '') +
                    '<div class="market_prg_x fl_l" onmouseover="animate(this, {opacity: 1}, 200); showTooltip(this, {text: \'' + getLang('dont_attach') +
                    '\', shift: [14, 3, 3], black: 1})" onmouseout="animate(this, {opacity: 0.6}, 200);" onclick="Upload.terminateUpload(' + i + ', \'' + (fileName ||
                        i) + '\'); if (window.tooltips) tooltips.hide(this);"></div>',
                className: 'clear_fix'
            }, {
                marginTop: '6px'
            });
    ...
    cropPhoto: function(photo, uploadUrl) {
        ...
        box = showFastBox({
            ...
            html);
        val('market_photo_crop_thumb', '<div style="' + st + 'margin: 0px auto;"><img id="market_photo_crop_img" src="' + thumb + '" style="' + st +
    ...
    deleteItem: function() {
        var box = showFastBox({
            ...
            ajax.post('al_market.php?act=a_delete_item', {
                oid: cur.options.oid,
                id: cur.options.item_id,
                hash: cur.options.hash
            }, {
                onDone: function(text, albums) {
                    boxQueue.hideAll();
                    ...
                    if (cur.itemsCount > 0) {
                        ...
                        summary = getLang('market_summary_no_goods');
                        geByClass1('market_empty', cur.notFound)
                            .innerHTML = cur.aid ? getLang('market_album_empty') : getLang('market_catalog_empty');
                        hide(cur.listEl);
                        show(cur.notFound);
                    }
    ...
    itemBoxinit: function() {
        ...
        if (cur.mkComments[cur.options.itemRaw]) {
            domPN(cms)
                .replaceChild(cur.mkComments[cur.options.itemRaw], cms);
        }
        cur.mkYourComment = false;

        setTimeout(function() {
            var h = getSize('market_item_box')[1];
            setStyle('market_item_photo', {
                maxHeight: h
            });
            setStyle('market_item_photo_container', {
                height: h
            });
        }, 10);
        ...
        for (var i = 1; i < cur.options.photos.length && i <= 5; i++) {
            vkImage()
                .src = cur.options.photos[i].thumb_x;
        }
    ...
    switchPhoto: function(num, force) {
        ...
        ge('market_item_photo')
            .src = cur.options.photos[num].thumb_x;
    ...
    like: function() {
        ...
        ajax.post('like.php', {
            ...
            object: cur.options.like_obj,
            hash: cur.options.likehash
        }, {
    ...
    likeOut: function() {
        if (!cur.options.liked) {
            setTimeout(animate.pbind(ge('market_like_icon'), {
                opacity: 0.4
            }, 200, false), 1);
        }
    },
    ...
    updateComposer: function() {
        ...
        if (cur.mkYourComment) {
            domPN(yc)
                .replaceChild(cur.mkYourComment, yc);
        ...
        cur.destroy.push(function(c) {
            var field = c.mkYourComment && geByTag1('textarea', c.mkYourComment),
    ...
    comments: function(showcomm) {
        if (showcomm) {
            var frst = domFC(ge('market_comments'))
                .id || '';
    ...
    updateComms: function() {
        ...
        ge('market_comments_header')
            .innerHTML = getLang('market_show_previous_comments', commshown);
    ...
    commentClick: function(el, event, from) {
        var comm = el.id.replace('market_comment', ''),
            cmnt = comm.split('_');
    ...
    commentTo: function(comm, toId, event) {
        var cmnt = (comm || '')
            .split('_'),
            commId = cmnt[1],
            replyNameOld = cur.mkReplyTo && cur.options.names[cur.mkReplyTo[0]] || '',
            replyName = cur.options.names[toId] || '',
            fld = ge('market_reply_field'),
            tl = ge('market_reply_to_title'),
            asGroup = ge('market_reply_as_group');
        ...
        if (comm) {
            ...
            setStyle(tl, {
                maxWidth: ge('market_submit_reply')
                    .offsetWidth - domPN(ge('market_reply_button'))
                    .offsetWidth - (asGroup ? (asGroup.offsetWidth + 10) : 0) - ge('market_reply_media_lnk')
                    .offsetWidth - 21
            });
        } else {
            cur.mkReplyTo = false;
    ...
    commSaved: function(post) {
        var comms = ge('market_comments_wrap'),
            vd = comms ? cur.options.itemRaw : false,
    ...
    sendComment: function(ev, stickerId) {
        ...
        if (stickerId) {
            ...
            var params = comp ? Composer.getSendParams(comp, Market.sendComment) : {
