var friendsphotos = {
    ...
    loadedPhotos: function(album, off, label, rows) {
        ...
        ge('album_photos' + album)
            .appendChild(ce('div', {
                innerHTML: rows
            }));

        var lnk = ge('album_more_progress' + album)
            .parentNode;
        if (!label) {
            hide(lnk);
            return;
        ...
        ajax.post('al_photos.php', {
            act: 'friends_photos',
            album: album,
            offset: cur.albums[album].offset
        }, {
            cache: 1,
            onDone: function() {
                if (cur.albums[album].loading == 2) {
                    friendsphotos.loadedPhotos.pbind(album)
    ...
    loadPhotos: function(album) {
        ...
        ajax.post('al_photos.php', {
            ...
            onDone: friendsphotos.loadedPhotos.pbind(album),
    ...
    removeAlbum: function(album, hash, prog) {
        ...
        ajax.post('al_photos.php', {
            ...
            onDone: function(text) {
                ...
                el.parentNode.insertBefore(ce('div', {
                    innerHTML: text,
    ...
    removeAllAlbums: function(album, hash, prog) {
        ...
        ajax.post('al_photos.php', {
            ...
            onDone: function(text) {
                ...
                var alb = (ge('album_photos' + album) || {})
                    .parentNode;
                if (!alb) return;

                var owner = album.split('_')[0],
                    passed = false;
                var cont = browser.msie6 ? pageNode : ((browser.chrome || browser.safari) ? bodyNode : htmlNode);
                for (var el = ge('friends_albums')
