...
AdsEdit.updateTargetingGroups = function() {
    ...
    hiderInfoElem.innerHTML = (cur.targetingEditor.isUserDevicesHidden() ? getLang('ads_criteria_section_additional_info_retargeting_new') : getLang(
        'ads_criteria_section_additional_info_new'));
}
...
AdsEdit.initLastAdsBox = function(lastAdsBox, lastAds, lastAdsKeyMap) {
    if (!cur.lastAds) {
        ...
        cur.lastAdsIndex = new vkIndexer(lastAds, function(obj) {
            return se(obj[lastAdsKeyMap.indexer_text])
                .nodeValue;
        });
    }
    ...
    ge('ads_edit_last_ads_content')
        .scrollTop = 0;
    AdsEdit.searchLastAds(true);
}

AdsEdit.searchLastAds = function(initial) {
    var searchStr = ge('ads_edit_last_ads_search')
        .getValue();
    ...
    if (isShowAll || results.length != 0) {
        ...
        noResultElem.innerHTML = getLang('ads_edit_ad_choose_view_not_found')
            .replace('{query}', Ads.escapeValue(searchStr));
        show(noResultElem);
    }
}
        ...
        switch (intval(photoData.code)) {
            ...
            default:
                if (photoData.code !== undefined) {
                    message += '<br>' + getLang('ads_err_code')
                        .replace('{code}', photoData.code);
                }
                break;
        }
...
AdsEdit.saveCropPhoto = function(cropBox) {
    ...
    ge('ads_edit_crop_photo_crop')
        .value = cur.photoTagger.result()
        .join(',');
...
AdsEdit.onSaveCropPhotoComplete = function(result) {
    ...
    if (!photoData || !photoData.photo) {
        ...
        message += ((photoData.errcode !== undefined) ? ('<br>' + getLang('ads_err_code')
            .replace('{code}', photoData.errcode)) : '');
    ...
    var formatPhotoSize = ge('ads_edit_crop_format_photo_size')
        .value;
...
AdsEdit.parseVideoExternal = function(delayed) {
    ...
    ge('ads_edit_upload_video_external_form')
        .submit();
}
...
AdsEdit.onParseVideoComplete = function(uploadVideoBox, uploadVars, updateAfterUploadHash, result) {
    ...
    ge('ads_edit_upload_video_external_photo_url')
        .value = result.url;
    ge('ads_edit_upload_video_external_photo_extra')
        .value = result.extra;
...
AdsEdit.uploadVideoPhotoImages = function(imagesContext) {
    ...
    ge('ads_edit_upload_video_external_photo_image')
        .value = imagesContext.images[imagesContext.imageIndex];
    ge('ads_edit_upload_video_external_photo_form')
        .submit();
}
...
AdsEdit.initCreatingPostForm = function(creatingPostBox, html, js, postOwnerId, wallOptions) {
    ...
    ge('ads_edit_ad_creating_post_form')
        .innerHTML = html;
...
AdsViewEditor.prototype.init = function(options, editor, targetingEditor, params, paramsData, paramsParams) {
    ...
    // Init "format photo size"-specific keys
    {
        ...
        var formatPhotoSizes = [AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_PROMOTION_COMMUNITY, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_APP_HORIZONTAL, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_COMMUNITY_SQUARE,
        ...
        var formatPhotoSizes = [AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_PROMOTION_COMMUNITY, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_APP_HORIZONTAL, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_COMMUNITY_SQUARE,
            AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_APP_BIG, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_MOBILE
        ];
        for (var i in formatPhotoSizes) {
            this.params.title['value_' + formatPhotoSizes[i]] = '';
        }

        var formatPhotoSizes = [AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_SMALL, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_MEDIUM, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_BIG, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_PROMOTION_COMMUNITY,
            AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_APP_HORIZONTAL, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_COMMUNITY_SQUARE, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_APP_BIG, AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_MOBILE,
...
AdsViewEditor.prototype.initHelpParam = function(paramNameHelp) {
    ...
    switch (paramNameHelp) {
        ...
        case 'format_type_apps_only':
            ...
            break;
        case 'category1_id':
            targetElem = ge(this.options.targetIdPrefix + 'category1_id')
                .parentNode;
            var showTooltip = function() {
                AdsEdit.showHelpCriterionTooltip(paramNameHelp, targetElem, handler, this.help[paramNameHelp], helpText, shiftLeft, shiftTop, this.cur);
            }.bind(this);
        ...
        case 'title':
            ...
            break;
        case 'platform':
            targetElem = ge(this.options.targetIdPrefix + paramNameHelp)
                .parentNode;
            helpText = function() {
...
AdsViewEditor.prototype.initUiParam = function(paramName) {
    ...
    switch (paramName) {
        case 'format_type':
            ...
            var label = (inArray(this.params.link_type.value, AdsEdit.ADS_AD_LINK_TYPES_ALL_GROUP) ? getLang('ads_edit_ad_format_type_promotion_community') : getLang(
                'ads_edit_ad_format_type_square_image'));
    ...
    switch (paramName) {
        ...
        case '_link_type':
            ...
            this.params.link_type.complete = !!(false ||
                inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT, AdsEdit.ADS_AD_LINK_TYPE_PUBLIC, AdsEdit.ADS_AD_LINK_TYPE_MARKET,
                    AdsEdit.ADS_AD_LINK_TYPE_APP
                ]) && this.params.link_id.value ||
                inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_URL, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_ANDROID, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_IPHONE,
                    AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_WPHONE
                ]) && linkUrlOk ||
                this.params.link_type.value == AdsEdit.ADS_AD_LINK_TYPE_VIDEO && this.params.link_id.value && this.params.link_owner_id.value && (linkUrlOk || !this.params
                    .link_url.value) ||
        ...
        case 'platform':
            ...
            this.params[paramName].disabled_web = (this.params.campaign_type.value == AdsEdit.ADS_CAMPAIGN_TYPE_UI_USE_APPS_WITH_BUDGET || this.params.campaign_type.value ==
                AdsEdit.ADS_CAMPAIGN_TYPE_UI_USE_OLD && this.params.campaign_id.value_app && this.params.campaign_id.value == this.params.campaign_id.value_app || this.params
                .format_type.value != AdsEdit.ADS_AD_FORMAT_TYPE_TEXT_IMAGE || this.params.cost_type.value != AdsEdit.ADS_AD_COST_TYPE_CLICK || isDisclaimers);
            ...
            if (this.params.platform.disabled) {
                this.params[paramName].value = (this.params.platform.values_disabled[linkTypeForPlatform] ? this.params.platform.values_disabled[linkTypeForPlatform] : this.params
                    .platform.values_disabled[0]);
            } else {
                this.params[paramName].value = (this.params.platform.values_normal[linkTypeForPlatform] ? this.params.platform.values_normal[linkTypeForPlatform] : this.params
                    .platform.values_normal[0]);
            }
...
AdsViewEditor.prototype.getUiParamEnabled = function(paramName) {
    switch (paramName) {
        case 'category1_id':
            return !!(this.params[paramName].value != 125 || this.params.link_type.value != AdsEdit.ADS_AD_LINK_TYPE_APP || !this.params.link_id.app_game_links_ids[this.params
                .link_id.value]);
        case 'subcategory1_id':
            var data = this.getUiParamData(paramName);
...
AdsViewEditor.prototype.updateUiParamVisibility = function(paramName) {
    switch (paramName) {
        ...
        case '_link_id':
            var linkResult = this.getLink();
            var elemVisible = !!(linkResult.link_url && inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT, AdsEdit.ADS_AD_LINK_TYPE_PUBLIC,
                AdsEdit.ADS_AD_LINK_TYPE_MARKET, AdsEdit.ADS_AD_LINK_TYPE_APP
            ]));
            toggleClass(this.options.targetIdPrefix + 'link_id_go_wrap', 'unshown', !elemVisible);
            break;
        case '_link_url':
            var linkResult = this.getLink();
            var elemVisible = !!(linkResult.link_url && inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_URL, AdsEdit.ADS_AD_LINK_TYPE_VIDEO, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_ANDROID,
                    AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_IPHONE, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_WPHONE
                ]) ||
            ...
            break;
        case '_link_post':
            var visibleCreatePostButton = !!((this.params.link_type.value != AdsEdit.ADS_AD_LINK_TYPE_POST_STEALTH || !this.params.link_type.complete) && this.params.link_type
                .allow_edit_all && this.params.link_type.allow_create_post);
            ...
            toggleClass('ads_edit_ad_row_link_editing_post', 'unshown', !(inArray(this.params.link_type.value, AdsEdit.ADS_AD_LINK_TYPES_ALL_POST) && (visibleCreatePostButton ||
                visibleEditPostButton)));
            break;
        case '_link_video':
            var linkResult = this.getLink();
        ...
        case 'format_type':
            ...
            toggleClass(this.options.targetIdPrefix + paramName + '_text_image_wrap', 'unshown', elemsHidden[AdsEdit.ADS_AD_FORMAT_TYPE_TEXT_IMAGE] = !(inArray(linkTypeValue, [
                AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT, AdsEdit.ADS_AD_LINK_TYPE_PUBLIC, AdsEdit.ADS_AD_LINK_TYPE_APP, AdsEdit.ADS_AD_LINK_TYPE_URL
            ])));
            toggleClass(this.options.targetIdPrefix + paramName + '_big_image_wrap', 'unshown', elemsHidden[AdsEdit.ADS_AD_FORMAT_TYPE_BIG_IMAGE] = !(inArray(linkTypeValue, [
                AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT, AdsEdit.ADS_AD_LINK_TYPE_PUBLIC, AdsEdit.ADS_AD_LINK_TYPE_APP, AdsEdit.ADS_AD_LINK_TYPE_URL,
                AdsEdit.ADS_AD_LINK_TYPE_VIDEO
            ])));
            ...
            toggleClass(this.options.targetIdPrefix + paramName + '_app_in_news_wrap', 'unshown', elemsHidden[AdsEdit.ADS_AD_FORMAT_TYPE_APP_IN_NEWS] = !(inArray(linkTypeValue, [
                AdsEdit.ADS_AD_LINK_TYPE_APP
            ]) && this.params.link_id.app_in_news_links_ids[this.params.link_id.value] && this.params.format_type.allow_app_in_news));
            toggleClass(this.options.targetIdPrefix + paramName + '_apps_only_wrap', 'unshown', elemsHidden[AdsEdit.ADS_AD_FORMAT_TYPE_APPS_ONLY] = !(inArray(linkTypeValue, [
                AdsEdit.ADS_AD_LINK_TYPE_APP
            ]) && this.params.link_id.app_trusted_links_ids[this.params.link_id.value]));
            ...
            toggleClass(this.options.targetIdPrefix + paramName + '_promoted_post_wrap', 'unshown', elemsHidden[AdsEdit.ADS_AD_FORMAT_TYPE_PROMOTED_POST] = !(inArray(
                linkTypeValue, [AdsEdit.ADS_AD_LINK_TYPE_POST_WITH_SHADOW, AdsEdit.ADS_AD_LINK_TYPE_POST_STEALTH])));
            ...
            if (!this.params[paramName].hidden) {
                var label = (inArray(this.params.link_type.value, AdsEdit.ADS_AD_LINK_TYPES_ALL_GROUP) ? getLang('ads_edit_ad_format_type_promotion_community') : getLang(
                    'ads_edit_ad_format_type_square_image'));
                ge(this.options.targetIdPrefix + 'format_type_promotion_community_label')
                    .innerHTML = label;
            }
            ...
            break;
        case '_format_type':
            var headerTitle = ((this.params.format_type.value == AdsEdit.ADS_AD_FORMAT_TYPE_PROMOTED_POST) ? getLang('ads_edit_ad_header_setting_link_params') : getLang(
                'ads_edit_ad_header_setting_view'));
            ge('ads_edit_value_header_view')
        ...
        case 'description':
            ...
            break;
        case '_additional':
            toggleClass('ads_edit_ad_row_additional', 'unshown', !!(this.params.disclaimer_medical.hidden && this.params.disclaimer_specialist.hidden && this.params.disclaimer_supplements
                .hidden));
            break;
        case 'disclaimer_medical':
        case 'disclaimer_specialist':
        ...
        case 'subcategory1_id':
        case 'subcategory2_id':
            this.params[paramName].hidden = (this.params[paramName].hidden_normal || this.params.category1_id.value == 125 && !this.params.category2_id.value && this.params.link_type
                .value == AdsEdit.ADS_AD_LINK_TYPE_APP && this.params.link_id.app_game_links_ids[this.params.link_id.value]);
            this.initUiParam(paramName);
            toggleClass('ads_edit_ad_row_' + paramName, 'unshown', !!this.params[paramName].hidden);
            break;
        case 'stats_url':
        ...
        switch (paramName) {
            case 'format_type':
                ...
                this.params.disclaimer_medical.may_be_any = inArray(this.params.format_type.value, [AdsEdit.ADS_AD_FORMAT_TYPE_TEXT_IMAGE, AdsEdit.ADS_AD_FORMAT_TYPE_BIG_IMAGE,
                    AdsEdit.ADS_AD_FORMAT_TYPE_EXCLUSIVE, AdsEdit.ADS_AD_FORMAT_TYPE_PROMOTION_COMMUNITY, AdsEdit.ADS_AD_FORMAT_TYPE_GROUPS_ONLY
                ]);
                ...
                this.params.stats_url.hidden = !(this.params.format_type.value == AdsEdit.ADS_AD_FORMAT_TYPE_EXCLUSIVE && this.params.stats_url.allow_exclusive || this.params.format_type
                    .value == AdsEdit.ADS_AD_FORMAT_TYPE_PROMOTED_POST && this.params.stats_url.allow_promoted_post);
                this.params.views_limit_flag.hidden = (this.params.cost_type.value != AdsEdit.ADS_AD_COST_TYPE_VIEWS || this.params.format_type.value == AdsEdit.ADS_AD_FORMAT_TYPE_EXCLUSIVE &&
                    this.params.views_limit_exact.allow || this.params.format_type.value == AdsEdit.ADS_AD_FORMAT_TYPE_PROMOTED_POST);
                this.params.views_limit_exact.hidden = (this.params.format_type.value != AdsEdit.ADS_AD_FORMAT_TYPE_PROMOTED_POST && (this.params.cost_type.value != AdsEdit.ADS_AD_COST_TYPE_VIEWS ||
            ...
            case 'link_type':
                ...
                this.params.link_id.hidden = !inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT, AdsEdit.ADS_AD_LINK_TYPE_PUBLIC,
                    AdsEdit.ADS_AD_LINK_TYPE_MARKET, AdsEdit.ADS_AD_LINK_TYPE_APP
                ]);
                this.params.link_url.hidden = !inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_URL, AdsEdit.ADS_AD_LINK_TYPE_VIDEO, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_ANDROID,
                    AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_IPHONE, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_WPHONE, AdsEdit.ADS_AD_LINK_TYPE_POST_WITH_SHADOW, AdsEdit.ADS_AD_LINK_TYPE_POST_STEALTH
                ]);
                ...
                if (inArray(paramValue, AdsEdit.ADS_AD_LINK_TYPES_ALL_MOBILE_APP) || inArray(paramValueOld, AdsEdit.ADS_AD_LINK_TYPES_ALL_MOBILE_APP)) {
                    ...
                    this.params.link_domain.needed = (linkUrlInfo && (!linkUrlInfo.domain.match(/(^|\.)(vkontakte\.ru|vk\.com)$/) || inArray(this.params.link_type.value,
                        AdsEdit.ADS_AD_LINK_TYPES_ALL_MOBILE_APP)));
                ...
                this.params.platform.hidden = !!(!inArray(this.params.link_type.value, AdsEdit.ADS_AD_LINK_TYPES_ALL_POST) && (!this.params.platform.allow_web || !inArray(this
                    .params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT, AdsEdit.ADS_AD_LINK_TYPE_PUBLIC, AdsEdit.ADS_AD_LINK_TYPE_APP,
                ...
                if (!this.params.link_type.cancelling && inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT, AdsEdit.ADS_AD_LINK_TYPE_PUBLIC,
                        AdsEdit.ADS_AD_LINK_TYPE_MARKET, AdsEdit.ADS_AD_LINK_TYPE_APP
                    ])) {
            ...
            case 'link_domain':
                ...
                this.params.link_domain.is_ok = !!(linkDomainInfo && this.params.link_domain.value == linkDomainInfo.domain || inArray(this.params.link_type.value, AdsEdit.ADS_AD_LINK_TYPES_ALL_MOBILE_APP) &&
                    this.params.link_domain.value === '1');
                this.updateUiParam('_link_type');
                this.updateUiParam('_link_video');
                break;
            case 'title':
                if (this.params[paramName].update_value_max || !inArray(this.params.format_type.value, [AdsEdit.ADS_AD_FORMAT_TYPE_PROMOTION_COMMUNITY, AdsEdit.ADS_AD_FORMAT_TYPE_APP_IN_NEWS,
                        AdsEdit.ADS_AD_FORMAT_TYPE_APPS_ONLY, AdsEdit.ADS_AD_FORMAT_TYPE_GROUPS_ONLY, AdsEdit.ADS_AD_FORMAT_TYPE_MOBILE
                    ])) {
            ...
            case 'cost_per_click':
                ...
                for (var valueName in values) {
                    this.params.cost_per_click[valueName] = Number(values[valueName])
                        .toFixed(2)
                        .replace('.00', '');
                }
            ...
            case 'platform':
                ...
                break;
            case 'campaign_type':
                this.params.campaign_id.hidden = (this.params.campaign_type.value == AdsEdit.ADS_CAMPAIGN_TYPE_UI_CREATE_NEW || this.params.campaign_id.data.length == 0 || (
                    this.params.campaign_type.value == AdsEdit.ADS_CAMPAIGN_TYPE_UI_USE_APPS_WITH_BUDGET && !this.params.campaign_id.value_app));
...
AdsViewEditor.prototype.setUpdateData = function(data, result) {
    ...
    if (isObject(result) && AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_PROMOTION_COMMUNITY + '_title' in result) {
        ...
        if (inArray(this.params.format_type.value, [AdsEdit.ADS_AD_FORMAT_TYPE_PROMOTION_COMMUNITY, AdsEdit.ADS_AD_FORMAT_TYPE_GROUPS_ONLY]) && data.link_type == this.params.link_type
            .value && data.link_id == this.params.link_id.value && data.link_url == this.params.link_url.value && inArray(vkLinkType, [AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT,
                AdsEdit.ADS_AD_LINK_TYPE_PUBLIC, AdsEdit.ADS_AD_LINK_TYPE_APP
            ])) {
            ...
            this.params.link_domain['value_' + AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_PROMOTION_COMMUNITY] = result[AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_PROMOTION_COMMUNITY +
                '_link_domain'];
            this.updateUiParam('link_domain');
            this.updatePreview('domain');
        }
    ...
    if (isObject(result) && 'a_title' in result) {
        var vkLinkType = ((this.params.link_type.value == AdsEdit.ADS_AD_LINK_TYPE_URL && this.params.link_url_vk.link_type_value) ? this.params.link_url_vk.link_type_value :
            this.params.link_type.value);
        if (this.params.format_type.value == AdsEdit.ADS_AD_FORMAT_TYPE_APP_IN_NEWS && data.link_type == this.params.link_type.value && data.link_id == this.params.link_id.value &&
    ...
    if (isObject(result) && AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_COMMUNITY_SQUARE + '_title' in result) {
        var vkLinkType = ((this.params.link_type.value == AdsEdit.ADS_AD_LINK_TYPE_URL && this.params.link_url_vk.link_type_value) ? this.params.link_url_vk.link_type_value :
            this.params.link_type.value);
        if (this.params.format_type.value == AdsEdit.ADS_AD_FORMAT_TYPE_APPS_ONLY && data.link_type == this.params.link_type.value && data.link_id == this.params.link_id.value &&
            data.link_url == this.params.link_url.value && vkLinkType == AdsEdit.ADS_AD_LINK_TYPE_APP) {
...
AdsViewEditor.prototype.updatePhotoLink = function(formatPhotoSize) {
    ...
    var vkLinkType = ((this.params.link_type.value == AdsEdit.ADS_AD_LINK_TYPE_URL && this.params.link_url_vk.link_type_value) ? this.params.link_url_vk.link_type_value : this
...
AdsViewEditor.prototype.setTitle = function(title, noUpdateValueMax, titleValues) {
    ...
    if (clearTitleValues || isObject(titleValues)) {
        var valuesKeys = ['value_' + AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_PROMOTION_COMMUNITY, 'value_' + AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_APP_HORIZONTAL, 'value_' + AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_COMMUNITY_SQUARE,
            'value_' + AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_MOBILE, 'value_' + AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_APP_BIG
        ];
...
AdsViewEditor.prototype.updateLinkDomain = function(onCompleteNoError) {
    ...
    if (!inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_URL, AdsEdit.ADS_AD_LINK_TYPE_VIDEO, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_ANDROID, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_IPHONE,
            AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_WPHONE
        ])) {
        return true;
    }
...
AdsViewEditor.prototype.updateLinkDomainTry = function(updateContext) {
    ...
    if (updateContext.triesLeft > 0) {
        ...
        onError.bind(this)(inArray(updateContext.linkType, AdsEdit.ADS_AD_LINK_TYPES_ALL_MOBILE_APP) ? getLang('ads_error_mobile_app_unreachable') : getLang(
            'ads_error_url_unreachable'))
    }
    ...
    function fullStop() {
        ...
        var isLinkEqual2 = this.isLinkDomainLinkEqual(this.updateLinkDomainContext.linkType, this.params.link_type.value, this.updateLinkDomainContext.linkUrl, this.params.link_url
            .value);
...
AdsViewEditor.prototype.completeLink = function() {
    ...
    var isChangedImportant = (!this.params_old ||
        isChangedLinkType ||
        this.params.link_id.value != this.params_old.link_id.value && inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_GROUP, AdsEdit.ADS_AD_LINK_TYPE_EVENT,
            AdsEdit.ADS_AD_LINK_TYPE_PUBLIC, AdsEdit.ADS_AD_LINK_TYPE_MARKET, AdsEdit.ADS_AD_LINK_TYPE_APP
        ]) ||
        ...
        (this.params.link_id.value != this.params_old.link_id.value || this.params.link_owner_id.value != this.params_old.link_owner_id.value) && inArray(this.params.link_type
            .value, AdsEdit.ADS_AD_LINK_TYPES_ALL_POST)
    );
    ...
    if (isChangedImportant) {
        ...
        this.params.format_type.hidden = !!(inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_ANDROID, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_IPHONE,
            AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_WPHONE, AdsEdit.ADS_AD_LINK_TYPE_POST_WITH_SHADOW, AdsEdit.ADS_AD_LINK_TYPE_POST_STEALTH
        ]));
...
AdsViewEditor.prototype.getLink = function() {
    ...
    if (inArray(this.params.link_type.value, [AdsEdit.ADS_AD_LINK_TYPE_URL, AdsEdit.ADS_AD_LINK_TYPE_VIDEO, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_ANDROID, AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_IPHONE,
            AdsEdit.ADS_AD_LINK_TYPE_MOBILE_APP_WPHONE
        ]) && this.getLinkInfo(this.params.link_url.value)) {
...
AdsViewEditor.prototype.updatePreview = function(previewParamName) {
    switch (previewParamName) {
        ...
        case 'photo':
            ...
            break;
        case 'photo_icon':
            this.preview[previewParamName].src = (this.params.photo['value_' + AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_ICON] ? this.params.photo_link['value_' + AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_ICON] :
                this.params.photo_link['value_default_' + AdsEdit.ADS_AD_FORMAT_PHOTO_SIZE_ICON]);
            break;
        case 'play':
    ...
    switch (criterionName) {
        ...
        case 'tags':
            targetElem = ge(this.options.targetIdPrefix + criterionName)
                .parentNode;
            var showTooltip = function() {
                AdsEdit.showHelpCriterionTooltip(criterionName, targetElem, handler, this.criteria[criterionName], helpText, false, shiftTop, this.cur);
            }.bind(this);
            var hideTooltip = function() {
