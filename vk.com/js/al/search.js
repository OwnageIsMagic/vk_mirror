...
var searcher = {
        ...
        applyOptions: function(options, changeSection) {
            ...
            if (options.summary) {
                ge('summary')
                    .innerHTML = options.summary;
            }
            if (options.auto_rows !== undefined) {
                ge('search_auto_rows')
                    .innerHTML = options.auto_rows || '';
            }
        ...
        showMore: function() {
            ...
            var params = extend(searcher.getSectionParams(), {
                offset: cur.offset
            });
            params.edit = nav.objLoc.edit;
            params.sign = nav.objLoc.sign;
        ...
        checkFiltersHeight: function(extraH) {
            ...
            var st = scrollGetY(),
                ...
                filtPad = filtY + filtPos - sfY - lastPos - st;
            if (hasClass(sf, 'fixed') && filtPad < extraH) {
                animate(sf, {
                    top: (lastPos - extraH + filtPad) + 'px'
                }, 200, function() {
        ...
        init: function(options) {
            ...
            addEvent(el, 'focus', function() {
                addClass(ge('search_content'), 'highlight');
            });
            addEvent(el, 'blur', function() {
                removeClass(ge('search_content'), 'highlight');
            });
        ...
        bigphOver: function(obj, uid) {
            ...
            if (o.tagName != 'A' || o.className != 'search_bigph') {
                (o = obj.insertBefore(ce('a', {
                    className: 'search_bigph',
                    href: ch && ch._id ? ('/photo' + ch._id + '?all=1') : ('/albums' + uid),
                    innerHTML: '<span class="search_bigph_label">' + getLang('global_photo_full_size') + '</span>'
                }), obj.firstChild))
                .onclick = Searcher.bigphClick.pbind(uid);
                o._uid = uid;
            }
...
if (window.iSearch === undefined) {
    iSearch = {
        init: function(input, options) {
            ...
            if (ge('top_search')) ge('top_search')
                .onclick = function(e) {
                    if (hab.getLoc()
                        .indexOf('search')) {
...
window.searchActions = {
    ...
    groupAddOfficer: function(link, params) {
        ...
        ajax.post('al_groups.php', {
            ...
            onDone: function(title, html, mid, hash, btn) {
                cur.doAddOfficer = function() {
                    ajax.post('al_groups.php', {
                        ...
                        position: ge('group_p_admin_position')
                            .value,
                        context: 'search'
                    }, {
    ...
    animateAdded: function(el, speed) {
        ...
        animate(anim, {
            ...
            onComplete: function() {
                ...
                setStyle(anim.parentNode.parentNode, {
                    paddingLeft: '2px'
                });
                setStyle(anim, {
                    left: '-13px'
    ...
    addApp: function(link, params) {
        ajax.post('al_apps.php?act=join', params, {
            onDone: function(loc, label) {
                location.href = loc;
                return;
                var label = ce('span', {
                    innerHTML: label,
        ...
        ajax.post('groupsedit.php', {
            ...
            onDone: function(acts, lev) {
                ...
                if (lev || lev === '') {
                    ...
                    searchActions.uRemoveAdmin(mid)
