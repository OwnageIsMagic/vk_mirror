    ...
    var C, T, X, Y, S, L, H, R, W, Z = {},
        ...
        q = function(t, i) {
            if (P = extend(P, t), each(t, function(t) {
                    ...
                    n = "top" == i || "bottom" == i ? o / _ * t.height : o / N * t.width, this.style.left = h + t.left + ("right" == i ? t.width - n : 0) + "px", this.style.top =
                        a + t.top + ("bottom" == i ? t.height - n : 0) + "px", this.style.width = ("left" == i || "right" == i ? n : t.width) + "px", this.style.height = (
                            "top" == i || "bottom" == i ? n : t.height) + "px"
                }), !i)
                for (var n in I)
                    if (I[n]) {
                        var o = I[n],
                            ...
                            p = t.left + t.width / 2,
                            w = t.top + t.height / 2;
                        o.style.width = g(k * c / t.width) + "px", o.style.height = g(F * v / t.height) + "px", o.style.marginLeft = -f(p * c / t.width - s / 2) + "px", o.style.marginTop = -
                            f(w * v / t.height - r / 2) + "px"
                    }
        },
        B = 0,
        D = function(t, e) {
        ...
        K = function(e) {
            ...
            return e.target == T || hasClass(e.target, "tag_frame_zone") ? B = 1 : e.target == X || e.target == t ? B = 2 : "preview" == e.target.className ? B = -1 - e.target.id.substring(
                4) : each(Z, function(t) {
                if (e.target == this) {
                    B = t;
                    var i = Y - W[0],
        ...
        O = function(e) {
            ...
            if (1 == B) {
                ...
                var a = P.left,
                    ...
                    A = 0,
                    I = 0;
                2 == B.length ? (A = "n" == B.charAt(0) ? -1 : 1, I = "w" == B.charAt(1) ? -1 : 1) : (A = "n" == B ? -1 : "s" == B ? 1 : 0, I = "w" == B ? -1 : "e" == B ? 1 : 0),
                    Y = a + N * (0 > I), S = o + _ * (0 > A), (a > n || n > a + N) && l(n - Y) < l(n - (a + N * (I >= 0))) && (Y += d(m, intval(_ * z)) * I, I *= -1), (o > h || h >
                        o + _) && l(h - S) < l(h - (o + _ * (A >= 0))) && (S += d(x, intval(N / b)) * A, A *= -1);
                var T = l(I) * l(n - Y),
                    j = l(A) * l(h - S);
                ...
                var O, Q = 0,
                    ...
                    $ = c(o, F - o - j),
                    tt = T / j;
                p(tt, z) < 0 ? 0 == I ? (O = c(G, j * z), Q = (T - O) / 2, Q = v(Q) * c(g(l(Q)), V), T = c(T - 2 * Q, G), j = T / z) : 0 == A ? (O = d(T / z, x), U = (j - O) / 2,
                    U = v(U) * g(l(U)), j = d(j - 2 * U, x), T = j * z) : (T = c(j * z, G), j = T / z) : p(tt, b) > 0 && (0 == I ? (O = d(j * b, m), Q = (T - O) / 2, Q = v(Q) *
                    g(l(Q)), T = d(T - 2 * Q, m), j = T / b) : 0 == A ? (O = c(K, T / b), U = (j - O) / 2, U = v(U) * c(g(l(U)), $), j = c(j - 2 * U, K), T = j * b) : (j =
                    c(T / b, K), T = j * b)), T = intval(T), j = intval(j), a = I >= 0 ? Y : Y - T, o = A >= 0 ? S : S - j, a += Q, o += U, q({
                    left: a,
                    top: o,
                    width: T,
                    height: j
        ...
        if (!(0 > s)) {
            ...
            if (r.style.position = "relative", h = t.offsetLeft, a = t.offsetTop, N = c(k, N), i = intval(N / 2), _ = c(F, _), n = intval(_ / 2), m = c(m, N), x = c(x, _), e.icons)
                for (var l in e.icons) e.icons[l] && e.icons[l].width && e.icons[l].height && e.icons[l].box && (M.push({
                    ...
                    }), I.push(ge(e.icons[l].box)
                        .appendChild(ce("img", {
                            src: t.src,
                            className: "preview"
                        }))), ge(e.icons[l].box)
                    .style.overflow = "hidden", I[I.length - 1].style.cursor = "move", addEvent(I[I.length - 1], "mousedown", K));
