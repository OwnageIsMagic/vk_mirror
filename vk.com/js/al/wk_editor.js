function WkEditor(cont, opts) {
    ...
    if (opts.mode) {
        ...
        modeBtn = '<a id="wke_b_mode" class="fl_r wke_b" wiki="mode" tooltip="' + opts.mode[2] + '" onmousedown="' + this.inst + '.button(this, event);" onmouseup="' + this.inst +
            '.buttonUp(this);" onmouseover="' + this.inst + '.ttOver(this, true);" onmouseout="' + this.inst + '.ttOut(this);" style="background-position: ' + opts.mode[0] + 'px ' +
            opts.mode[1] + 'px;"></a>';
    }
    ...
    for (var i in this.buttons) {
        ...
        panel += '<a id="wke_b_' + i + '" class="fl_l wke_b" wiki="' + i + '" tooltip="' + itm[2] + '" onmousedown="' + this.inst + '.button(this, event);" onmouseup="' + this.inst +
            '.buttonUp(this);" onmouseover="' + this.inst + '.ttOver(this);" onmouseout="' + this.inst + '.ttOut(this);" style="background-position: ' + itm[0] + 'px ' + itm[1] +
            'px;"></a>';
    }
    panel += '</div></div>';
...
extend(WkEditor.prototype, {
                    ...
                    for (var i in imgs) {
                        ...
                        if (imageWrap.tagName != 'A' || !imageWrap.getAttribute('wiki')) {
                            ...
                            if (imageWrap.tagName != 'A') {
                                ...
                                var newWrap = ce('a', {
                                    innerHTML: '<img width="' + size[0] + '" height="' + size[1] + '" src="' + imgs[i].src + '" />',
    ...
    button: function(obj, e) {
        ...
        switch (n) {
            ...
            case 'blockquote':
                if (this.plainMode) {
                    this.plainInsert("<blockquote>", "</blockquote>", {
                        newline: 1
                    })
                } else {
    ...
    checkBrowser: function() {
        ...
        if (support && !this.plain) {
            ...
            this.plainMode = false;
            ge('wke_b_mode')
                .setAttribute('tooltip', this.modeBtn[2]);
        } else {
            hide(this.cont);
            show(this.textarea);
            this.plainMode = true;
            ge('wke_b_mode')
                .setAttribute('tooltip', this.modeBtn[3]);
            if (!support) {
                hide('wke_b_mode');
            }
        }
            ...
            if (range && range.parentElement() == this.textarea) {
                ...
                if (textInputRange.compareEndPoints("StartToEnd", endRange) > -1) {
                    ...
                    if (browser.msie && intval(browser.version) < 9) {
                        startPos += normalizedValue.slice(0, startPos)
                            .split("\n")
                            .length - 1;
                    }
                    ...
                    if (textInputRange.compareEndPoints("EndToEnd", endRange) > -1) {
                        ...
                        if (browser.msie && intval(browser.version) < 9) {
                            endPos += normalizedValue.slice(0, endPos)
                                .split("\n")
                                .length - 1;
                        }
                    }
    ...
    wrapList: function(tag, btn) {
        ...
        if (rangeEl[1] == 5 && geByTag('li', el)
            ...
            this.surround(ce(tag, {
                className: 'listing'
            }), btn[3], '<li><span class="l">', '</span></li>', true);
        }
        this.checkEditPlace();
    },
    ...
    fixHeader: function(el) {
        el.parentNode.replaceChild(ce('div', {
            ...
            innerHTML: clean(this.getText(el))
        }), el);
    },
    ...
    showUrlBox: function(rangeEl, btn) {
        if (this.plainMode) {
            ...
            cur.wkBoxRange = this.setFocus(el, {
                noCollapse: true
            });
            cur.wkLinkEl = el;
            if (btn) {
    ...
    insertLink: function(name, url, outer) {
        if (this.plainMode) {
            this.plainInsert(this.getLinkWiki(url, name), '', {
                replace: 1
            });
            return false;
        }
    ...
    switchMode: function(newMode) {
        ...
        if (newMode == 3) {
            ...
            setStyle(urlEl, {
                backgroundPosition: '0px -317px'
            });
            urlEl.setAttribute('tooltip', this.buttons.url[3]);
        } else if (this.mode == 3) {
            var urlEl = ge('wke_b_url');
            setStyle(urlEl, {
                backgroundPosition: this.buttons.url[0] + 'px ' + this.buttons.url[1] + 'px'
            });
    ...
    insertAudio: function(performer, title, duration, aid, url, durationInt) {
        ...
        if (this.plainMode) {
            this.plainInsert('[[audio' + aid + ']]', '', {
                replace: 1
            });
            return false;
        }

        var html = '<div class="audio" id="audio' + aid +
            '" onmouseover="addClass(this, \'over\');" onmouseout="removeClass(this, \'over\');" contenteditable="false">\
  <a name="' + aid +
            '"></a>\
  <div class="area clear_fix" onclick="playAudioNew(\'' + aid +
    ...
    <table cellspacing="0" cellpadding="0" width="100%">\
      <tr>\
        <td>\
          <div class="play_btn_wrap"><div class="sp_main play_new" id="play' +
            aid + '"></div></div>\
          <input type="hidden" id="audio_info' + aid + '" value="' + url + ',' + durationInt +
            '" />\
        </td>\
        <td class="info">\
          <div class="title_wrap fl_l" onmouseover="setTitle(this)"><b onclick="event.cancelBubble = true;"><a>' +
            performer + '</a></b> &ndash; <span class="title" id="title' + aid + '">' + title +
            '</span></div>\
          <div class="duration fl_r" onmousedown="if (window.audioPlayer) audioPlayer.switchTimeFormat(\'' + aid +
            '\', event);" onclick="cancelEvent(event)">' + duration + '</div>\
        </td>\
      </tr>\
    </table>\
    <div id="player' + aid +
            '" class="player" ondragstart="return false;" onselectstart="return false;" onclick="event.cancelBubble = true;">\
      <table cellspacing="0" cellpadding="0" border="0" width="100%">\
        <tbody><tr>\
          <td style="width: 100%;">\
            <div id="audio_pr' +
            aid +
            '" class="audio_pr" onmouseover="addClass(this, \'over\')" onmouseout="removeClass(this, \'over\'); removeClass(this, \'down\')" onmousedown="addClass(this, \'down\'); audioPlayer.prClick(event);" onmouseup="removeClass(this, \'down\')">\
    ...
    insertVideo: function(thumb, name, duration, link, videoId) {
        ...
        if (this.plainMode) {
            this.plainInsert('[[video' + videoId + ']]', '', {
                replace: 1
            });
            return false;
        }
        ...
        this.getImgSize(thumb, (function(w, h) {
                ...
                var videoEl = se('<a class="wk_video" href="' + link + '" contentEditable="false" onclick="return showVideo(\'' + videoId +
                    '\', \'\', {autoplay: 1}, event)"><img alt="' + name + '" title="' + name +
                    '" src="/images/play_video_wide.png?3" style="background-image: url(' + thumb + ');">');
                this.insert([videoEl]);
    ...
    insertPhoto: function(oid, pid, thumb, bThumb) {
        ...
        this.getImgSize(bThumb, (function(w, h) {
                ...
                if (this.plainMode) {
                    this.plainInsert('[[photo' + oid + '_' + pid + '|' + w + 'x' + h + 'px;noborder| ]]', '', {
                        replace: 1
                    });
                    return false;
                }
    ...
    ttOver: function(obj, right) {
        ...
        if (right) {
            ...
            l -= size[0] - 22;
            cur.wkRightTT = true;
            setStyle(geByClass1('wke_tt_b', this.tt), {
                marginLeft: size[0] - 16
            });
            setStyle(geByClass1('wke_tt_tb', this.tt), {
                marginLeft: size[0] - 16
            });
    ...
    getText: function(cont, multi) {
        ...
        if (multi) {
            ...
            while (el) {
                switch (el.nodeType) {
                    case 3:
                        var c = el.data.replace(this.re.n, ' ')
                            .replace(this.re.sp, ' ');
                        text += c;
                        break;
                    case 1:
