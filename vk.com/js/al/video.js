var Video = {
    ...
    init: function(obj) {
        ...
        placeholderSetup(cur.vSearch, {
            back: true
        });
        iSearch.destroy();
        if (vk.id) {
            iSearch.init(cur.vSearch, {
                onShow: function() {
    ...
    controlTooltip: function(el, text) {
        ...
        showTooltip(el, {
            ...
            onShowStart: function(tt) {
                geByClass1('tt_text', tt.container)
    ...
    privateClick: function(obj, ev) {
        geByClass1('video_row_icon_edit', obj.parentNode.parentNode.parentNode)
            .click();
    ...
    loadSilent: function(section, extended) {
        ...
        if (list && (list.silent || list.videos)) {
            ...
            ajax.post('al_video.php', {
                ...
                oid: oid,
                section: section,
                offset: list.silent ? 0 : list.videos.length,
                ...
                onDone: function(list, playlistsOverviewHtml, playlistsCount, playlistsData) {
                    ...
                    if (extended && !Video.isCurrentChannel()) {
                        ...
                        if (playlistsCount && playlistsCount > 2) {
                            ...
                            domPN(summaryEl)
                                .replaceChild(playlistsSummaryEl, summaryEl);
    ...
    searchAlbums: function(str) {
        if (str) {
            ...
            a = a.sort(function(i, j) {
                return i._order - j._order
            });
        ...
        if (hasResults) {
            ...
            if (ge('video_albums_show_more')) {
                ge('video_albums_show_more')
                    .innerHTML = langNumeric(a.length, cur.lang['video_show_all_albums']);
    ...
    searchVideos: function(str, force) {
        ...
        if (str && cur.vIndex) {
            ...
            if (!str.replace(/\|/g, '')
                ...
                cur.selection = {
                    re: new RegExp('(' + str.replace(/\|/g, '')
                        .replace(cur.vIndex.delimiter, '|')
                        .replace(/^\||\|$/g, '')
                        .replace(/([\+\*\)\(])/g, '\\$1') + ')', 'gi'),
                    val: '<em>$1</em>'
                };
    ...
    toggleFilter: function(obj, target) {
        if (hasClass(obj, 'filter_shut') || !isVisible(target)) {
            ...
            var extraH = slideDown(target, 200)
                .to.height;
        } else {
    ...
    toggleAdult: function() {
        ...
        var checkAgreed = function(val) {
            ...
            var box = showFastBox({
                title: getLang('video_adult_box_title'),
                onHide: cancelAgreed
            }, getLang('video_adult_box_text'), getLang('global_continue'), function() {
                cur.isAdult = true;
    ...
    changeSummary: function() {
        var str, searchStr, oldSearhSummary = false,
            htitle;
        ...
        if (cur.vSection == 'search' && cur.vStr) {
            ...
            htitle = getLang('video_title_search')
                .replace('{q}', cur.vStr) + (len ? ' | ' + getLang('video_title_search_X_found', len) : '');
        }
        ...
        if (cur.vSection == 'tagged') {
            htitle = cur.htitle_videos + (len ? ' | ' + langNumeric(len, cur.lang['video_title_X_videos_tagged'], true) : '')
                .replace('{user}', cur.htitle_tagged_user);
        }
        if (!htitle) {
    ...
    scrollResize: function() {
        ...
        if (channelHeaderEl) {
            ...
            setStyle(cur.videoContentTab, {
                marginTop: needFixed ? (getSize(cur.channelHeaderEl)[1] + 'px') : 0
            });
        } else {
            setStyle(cur.videoContentTab, {
                marginTop: 0
    ...
    loadFrom: function(url, params, section) {
        ...
        ajax.post(url, params, {
            onDone: (function(count, data, obj) {
                    removeClass(ge('video_section_' + section), 'loading');
                    if (obj) {
                        extend(cur, obj);
                    }
    ...
    generateList: function(section) {
        ...
        if (section == 'uploaded' || section == 'added' || section.substr(0, 6) == 'album_') {
            ...
            this.loadFrom('al_video.php', {
                act: 'get_tagged_video',
                mid: cur.oid
            }, section);
            return false;
    ...
    showCat: function() {
        ...
        if (cur.catWrap.children.length) {
            ...
            geByTag1('body')
                .scrollTop = cur.videoCatPageScroll || 0;
            return;
        }
            ...
            ajax.post('al_video.php', {
                ...
                onDone: function(list, playlistsOverviewHtml, playlistsCount) {
                    ...
                    if (playlistsCount > 0 && trim(playlistsOverviewHtml)) {
                        ...
                        domPN(summaryEl)
                            .replaceChild(channelSummaryEl, summaryEl);
    ...
    onSwitchTabs: function(section) {
        ...
        var tabTitleEl = geByClass1('tab_word', hiddenTab)
            .firstChild;
    ...
    updateDataSectionAttr: function() {
        ge('video_all')
            .setAttribute('data-section', cur.vSection);
    },
    ...
    updateTitle: function(row) {
        ...
        if (linkEl.innerHTML.length > 40 && !linkEl.hasAttribute('title')) {
            ...
            if (initialHeight < realHeight) {
                linkEl.setAttribute('title', trim(linkEl.innerHTML.replace(/(<em>)|(<\/em>)/g, '')
                    .replace(/&amp;/g, '&')));
            }
        }
    },
    ...
    updateTimes: function(albumsSearch, forceAlbumsUpdate) {
        ...
        if (albumsSearch || forceAlbumsUpdate || cur.vSection == 'playlists') {
            ...
            if (cur.vSection == 'search') {
                var hd = cur.vHD ? cur.vHD : 0;
                var searchData = cur.searchData[cur.vStr + hd.toString() + (cur.vOrder || '')
                    .toString() + cur.vDateAdded];
    ...
    showAlbums: function(obj) {
        while (ge('video_albums_more') && ge('video_albums_more')
            .firstChild) {
            var el = ge('video_albums_more')
                .firstChild;
            ge('video_albums_wrap')
                .appendChild(el);
        }
        hide(obj);
        if (cur.canEditAlbums && cur.albumsSorter && !(trim(cur.vStr) && cur.vStr != '""')) {
    ...
    xRow: function(event, opacity, row, hint) {
        if (hint) showTooltip(row, {
            text: hint,
            showdt: 500
        });
        animate(hasClass(row, 'video_close') ? row : geByClass1('video_close', row), {
            opacity: opacity
        }, 200);
    ...
    albumDrop: function(el, obj) {
        ...
        var videoId = obj.id.replace('video_cont', '')
            .split('_');
        var oid = videoId[0];
        var vid = videoId[1];
    ...
    switchPlaylistMode: function(button, mode, silent) {
        ...
        if (!silent && vk.id) {
            ajax.post('/al_video.php', {
                act: 'a_save_list_view_mode',
                mode: cur.listViewMode,
                hash: cur.listViewModeHash
            });
        }
    ...
    videoPlaylistsEditInit: function(box, playlists, video, gid, hash, inPlaylists) {
        var oid = video[0],
            vid = video[1];
        var checkboxes = [],
            body = ge('video_playlists_edit_box');
        var checkedPlaylists = {};
        each(playlists, function(i, pl) {
            var privateIcon = pl.private ? '<span class="video_album_private_icon" onmouseover="showTooltip(this,{black:1,text: \'' + getLang(
                'video_album_is_private_tt') + '\', shift:[13, 3, 0]})"></span>' : '';
        ...
        box.removeButtons()
            .addButton(getLang('Save'), function(btn) {
                ...
                ajax.post('/al_video.php', {
                    ...
                    oid: oid,
                    vid: vid,
                    gid: (gid == vk.id) ? void 0 : -gid,
                    playlists: pls.length ? pls : '0',
                    hash: hash
                }, {
    ...
    onVideoDelete: function(oid, vid, hash, cb) {
        var fastBox = showFastBox({
            title: getLang('video_delete_dialog_title'),
            dark: 1,
            forceNoBtn: true,
            bodyStyle: 'padding: 20px; line-height: 160%;'
        }, getLang('video_tc_sure_delete'), getLang('video_tc_delete'), function(button) {
            ajax.post('/al_video.php', {
                act: 'delete_video',
                oid: oid,
                vid: vid,
    ...
    toggleExtendedControls: function(show) {
        var videoSearchEl = ge('video_search'),
            videoSearchControlsEl = ge('video_search_controls');
    ...
    initSearchControls: function() {
        ...
        new Dropdown(ge('video_search_duration'), [
            [0, getLang('video_search_any_duration')],
            [2, getLang('video_search_long')],
            [1, getLang('video_search_short')]
        ], extend(options, {
        ...
        new Dropdown(ge('video_search_added'), [
            [0, getLang('video_search_any_time')],
            [86400, getLang('video_search_last_24')],
            [86400 * 7, getLang('video_search_last_week')],
            [86400 * 30, getLang('video_search_last_month')],
            [86400 * 365, getLang('video_search_last_year')]
        ], extend(options, {
        ...
        new Dropdown(ge('video_search_sort'), [
            [2, getLang('video_by_relevance')],
            [0, getLang('video_search_by_date')],
            [1, getLang('video_search_by_duration')]
        ], extend(options, {
    ...
    showTutorial: function() {
        showBox('/al_video.php', {
            ...
            params: {
                width: 600,
                bodyStyle: 'position: relative; padding: 0; border-radius: 5px',
                dark: 1,
                hideButtons: 1
            }
        });
    ...
    updateTutorialNavBullets: function() {
        ...
        addClass(geByClass1('video_info_tutorial_nav_bullets')
            .children[index], ACTIVE_BULLET_CLS);
    },
    ...
    hideBanner: function(event) {
        ajax.post('al_video.php', {
            act: 'hide_new_video_banner',
            hash: cur._videoBannerHideHash
        });
        var cb = curBox();
        cb && cb.hide();
