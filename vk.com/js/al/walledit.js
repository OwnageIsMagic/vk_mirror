var WallEdit = {
    ...
    editPost: function(post, text, mediaData, hash, opts, types) {
        ...
        cur.options = extend(cur.options || {}, {
            share: opts.share
        });
        cur.editHash = hash;
        ...
        node.parentNode.insertBefore(ce('div', {
            id: 'wpe_cont',
            innerHTML: '\
<div class="clear_fix"><div class="wpe_text_cont">\
<div id="pv_reply_smile" title="' + stripHTML(getLang('wall_reply_emoji_hint')) +
                '" class="emoji_smile fl_l" onmouseover="return WallEdit.emojiShowTT(this, event);" onmouseout="return WallEdit.emojiHideTT(this, event);" onmousedown="return cancelEvent(event);"><div class="emoji_smile_icon_on"></div><div class="emoji_smile_icon"></div></div>\
<div id="wpe_text" class="fl_l" contenteditable="true">' +
                text + '</div>\
</div></div>\
<div id="wpe_warn"></div>\
<div id="wpe_media_preview" class="clear_fix media_preview"></div>\
' + (opts.signed ?
                    ('<div id="wpe_signed" class="checkbox' + (opts.signed > 0 ? ' on' : '') + '" onclick="checkbox(this)"><div></div>' + getLang(
                        'wall_suggest_subscribe') + '</div>') : '') + '\
...
<div class="wpe_buttons">' +
    ...
    <button onclick="WallEdit.savePost()">' + getLang('global_save') +
                '</button>\
  </div>\
  <div class="button_cancel wpe_cancel fl_l">\
    <div class="button" onclick="WallEdit.cancelEditPost()">' + getLang(
                    'global_cancel') + '</div>\
  </div>\
  <div class="progress fl_l" id="wpe_prg"></div>\
    ...
    cancelEditPost: function(data, html, fronly) {
        ...
        if (data == -1) {
            ...
            setTimeout(animate.pbind(info, {
                opacity: 0
            }, 500, re.pbind(info)), 1500);
    ...
    savePost: function() {
        ...
        var composer = cur.wallEditComposer,
            ...
            from = cur.onepost ? 'one' : ((window.wkcur || {})
                .shown ? 'wk' : '');
