if (!window.Graph)
    (function(window) {
        ...
        var DEF_TIME = Math.round((new Date())
            .getTime() / 1000);
        var dateFormats = {
            hour: "{hour}:00",
            day: getLang('stats_day_mon')
                .split("{month}")
                .join("{dayMonth}"),
            dayFull: getLang('stats_day_mon')
                .split("{month}")
                .join("{dayMonth}"),
            dayFullYear: getLang('stats_day_month_year')
                .split("{month}")
                .join("{dayMonth}"),
                    ...
                    if (lines[l].b) {
                        ...
                        for (var i = stIdx; i <= enIdx; i++) {
                            ...
                            if (smooth && (i > stIdx) && (enIdx - stIdx + 1 > 2))
                                bezierCurveTo(ctx, Math.min(Math.max(px, xs1[i - stIdx - 1]), cx), Math.min(y + h, Math.max(ys1[i - stIdx - 1], 0)), Math.min(Math.max(px, xs2[
                                    i - stIdx - 1]), cx), Math.min(y + h, Math.max(ys2[i - stIdx - 1], 0)), cx, y + h - (ps[i].y + yDelta) * yfactor);
                            else
        ...
        function Graph(id, data, params, width, height) {
            ...
            this.graphDiv.innerHTML =
                '<div style="float: left"><canvas style="padding-top: 20px"></canvas></div>\
    <div style="float: left">\
      <div style="height: 20px; float: right;"></div>\
      <div style="height: 20px; padding-left: 10px;"></div>\
      <div style="position: relative; height: ' +
                this.viewHeight + 'px;">\
        <canvas height="' + (this.viewHeight * cs) + '" style="height: ' + this.viewHeight +
                'px;"></canvas>\
        <canvas height="' + ((this.viewHeight - 17) * cs) + '" style="position: absolute; top: 0; left: 0; height: ' + (this.viewHeight - 17) +
                'px;"></canvas>\
      ...
      <div style="float: left; width: 270px; padding-top: 8px; -moz-user-select: none; -khtml-user-select: none;"></div><div style="float: left; width: 270px; padding-top: 8px; -moz-user-select: none; -khtml-user-select: none;"></div>\
            ...
            if (typeof params.multiple == 'object' && params.multiple.items) {
                ...
                for (i in params.multiple.altitems) {
                    var item = se('<a class="graph_menu_item' + (i + params.multiple.altitems.length == params.multiple.def ? ' graph_menu_item_sel' : '') + '">' + params.multiple
                        .altitems[i][0] + '</a>');
                    var clickContext = {
                        clickNode: i,
                        graph: this,
            ...
            try {
                ...
                this.message.innerHTML = (cur.graphVars['lang.bad_browser'] || '��� ����������� ������� ���������� ��� ���������� {link}�������� �������{/link}.')
                    .replace('{link}', '<a onclick="Graph.prototype.showBadBrowserBox();">')
                    .replace('{/link}', '</a>');
                setStyle(this.graphDiv, {
        ...
        Graph.prototype = {
            ...
            downloadCSV: function(ev) {
                var data = ev.currentTarget.line.d,
                    fileName = ev.currentTarget.line.name.replace(/[^a-z�-�0-9\-_\(\)\.\,\;]/gi, "_")
                    .slice(0, 100),
                    CSV = '',
                    minDate, maxDate;
            ...
            showDots: function(ev) {
                ...
                for (var l = 0; l < g.lines.length; l++) {
                    ...
                    if (ni > -1) {
                        ...
                        if (!isVisible(g.lines[l].dotLabel) || !g.lines[l].dotLabel.dot || g.lines[l].dotLabel.dot.x != dot.x || parseInt(g.lines[l].dot.style.left) !=
                            Math.floor(dotX)) {
            ...
            highlightBar: function(ev) {
                ...
                for (var l = 0; l < g.lines.length; l++) {
                    for (var i = 0; i < g.lines[l].d.length; i++) {
                        ...
                        var ny = g.viewHeight - (g.isNegative ? (34 + (g.lines[l].d[i].y - g.localBottom) * g.yfactorIn) : (25 + (g.lines[l].d[i].y - (g.adjust ? g.localBottom :
                            0)) * g.yfactorIn));
            ...
            showTT: function(dotX, dotY, text) {
                ...
                if (this.mainLayout.tt && this.mainLayout.tt.container && (!isVisible(this.mainLayout.tt.container) || !this.dotLabelXY || this.dotLabelXY[0] != xy[0])) {
                    geByClass1('tt_text', this.mainLayout.tt.container)
                        .innerHTML = text;
                    if (window.tooltips) {
                        tooltips.hide(this.mainLayout, {
                            fasthide: 1
            ...
            updateLines: function() {
                ...
                if (this.isNegative)
                    drawLines(ctx, 0, 4, this.viewWidth, 32, this.lines, this.minTime, this.maxTime, this.xfactorOut, this.yfactorOut, -this.localBottom, true, null,
                        false);
                else
            ...
            redrawWindow: function() {
                ...
                if (this.lines.length == 0 || (localMax - localMin < 1e-6)) {
                    ...
                    this.ystep = Math.max(getYStep(localMax - (this.adjust ? localMin : 0), this.params.int_scale), this.isNegative ? getYStep(-localMin, this.params.int_scale) :
                        1e-6);
                ...
                for (var i = i_start;
                    ...
                    for (var j = 0; j < sc.format.length; j++) {
                        ...
                        var formatWidth = ctx.measureText(formatDate(-1, sc.format[j]))
                            .width;
                ...
                // graph
                if (this.isNegative)
                    drawLines(ctx, 0, 9, this.viewWidth, this.viewHeight - 43, this.lines, this.leftTime, this.rightTime, this.xfactorIn, this.yfactorIn, -this.localBottom,
                        false, this.activeLine, this.smoothLines);
                else
                    drawLines(ctx, 0, 9, this.viewWidth, this.viewHeight - 34, this.lines, this.leftTime, this.rightTime, this.xfactorIn, this.yfactorIn, this.adjust ?
                        -this.localBottom : 0, false, this.activeLine, this.smoothLines);
            ...
            background-color: ' +
                    ...
                    var legendDownloadCSVBox = this.params.download_csv ?
                        '<div style="position: absolute; top: 0px; left: 0px; padding: 2px 3px; white-space: nowrap; display: none;">.csv</div>' : '';
                    line.legend = ce('div', {
                        innerHTML: '<canvas width="' + (20 * cs) + '" height="' + (20 * cs) +
                            '" style="vertical-align: middle; padding-right: 2px; width: 20px; height: 20px;"></canvas>' +
                            '<span title="' + clean(line.name) + '">' + ((line.name.length >= 30) ? line.name.substr(0, 27) + '...' : line.name) + '</span>' +
                            '<div style="position: absolute; top: 0px; left: 0px; padding: 2px 3px; white-space: nowrap; display: none; background: ' + line.hexColor +
                            '; color: white; -webkit-border-radius: 2px; -khtml-border-radius: 2px; -moz-border-radius: 2px; border-radius: 2px;"></div>' +
                ...
                var maxWidth = Math.max(
                    ctx.measureText(formatValue(this.maxValue + getYStep(this.maxValue, this.params.int_scale)))
                    .width,
                    Math.max(ctx.measureText(formatValue(getYStep(this.minAbsValue, this.params.int_scale)))
                        .width,
                        ctx.measureText(formatValue(this.minValue))
                        .width));
            ...
            setBarChartData: function(data, adjust) {
                ...
                for (var l = 0; l < data.length; l++) {
                    ...
                    line.legend = ce('div', {
                        innerHTML: '<canvas width="' + (20 * cs) + '" height="' + (20 * cs) +
                            ...
                            '<div style="position: absolute; top: 0px; left: 0px; padding: 2px 3px; white-space: nowrap; display: none; background: ' + line.hexColor2 +
                            '; -webkit-border-radius: 2px; -khtml-border-radius: 2px; -moz-border-radius: 2px; border-radius: 2px;"></div>'
                    }, {
                        color: line.hexColor,
                        position: 'relative',
                        cursor: 'default',
                ...
                var maxWidth = Math.max(
                    ctx.measureText(formatValue(this.maxValue + getYStep(this.maxValue, this.params.int_scale) + yUnits))
                    .width,
                    ctx.measureText(formatValue(getYStep(this.minAbsValue, this.params.int_scale) + yUnits))
                    .width);
                ...
                if (this.lines.length == 0 || (this.maxValue - this.minValue < 1e-6)) {
                    ...
                    this.ystep = Math.max(getYStep(this.maxValue - (adjust ? this.minValue : 0), this.params.int_scale), this.isNegative ? getYStep(-this.minValue,
                        this.params.int_scale) : 1e-6);
                ...
                for (var i = 0; i < this.params.bar_chart.length; i++) {
                    while (ctx.measureText(this.params.bar_chart[i])
                        .width > this.xstep && fontSize >= 8) {
                        fontSize--;
                        ctx.font = getFont(fontSize, true);
                    }
                }
                ...
                if (this.isNegative)
                    ...
                    drawBars(ctx, 0, 9, this.viewWidth, this.viewHeight - 34, this.lines, this.params.bar_chart, this.barsWidth, this.xstep, this.yfactorIn, this.adjust ?
                        -this.localBottom : 0);
    ...
    <div id="stats_good_browsers" class="clear_fix">\
      <a href="http://www.mozilla-europe.org/" target="_blank" style="background: url(/images/firefox' +
                    s +
                    '.png) no-repeat 50% 17px;">Mozilla Firefox</a>\
      ...
      <a href="http://www.opera.com/" target="_blank" style="background: url(/images/opera' + s +
                    '.png) no-repeat 50% 15px;">Opera</a>\
      <a href="http://www.apple.com/safari/" target="_blank" style="background: url(/images/safari' + s +
                    '.png) no-repeat 50% 12px;">Safari</a>\
    </div>\
    <style>\
      #stats_good_browsers {\
        height: 136px;\
