var Fave = {
    switchTab: function(section) {
        ...
        if (section == 'likes_notes') {
            replaceClass(ge('fave_like_tab_likes_posts')
                .parentNode, 'summary_tab', 'summary_tab_sel');
        } else {
            replaceClass(ge('fave_like_tab_' + section)
                .parentNode, 'summary_tab', 'summary_tab_sel');
        }
        ...
        ajax.post('al_fave.php', {
            ...
            onDone: function(res, html) {
                if (res) {
                    ge('link' + link)
                        .innerHTML = html;
                    var num = geByClass('fave_link_item')
                        .length - 1;
                    if (!cur.linksNum) {
                        cur.linksNum = num;
                    } else {
    ...
    preloadTabs: function() {
        ajax.post('al_fave.php', {
            ...
            onDone: function(data, usersHtml, userRows, linksHtml, photosHtml, videosHtml, postsHtml, notesHtml, facesHtml) {
                var page = ge('content');
                if (cur.section != 'users') {
                    page.appendChild(ce('div', {
                        innerHTML: usersHtml
                    }));
                    ...
                    if (!ge('empty_users') && geByClass1('summary', ge('users'))) {
                        cur.lang['users_summary'] = geByClass1('summary', ge('users'))
                            .innerHTML;
                        Fave.indexAll();
                    }
                }
    ...
    scrollCheck: function() {
        if (browser.mobile || cur.isListLoading || cur.disableAutoMore || (cur.section || '')
            .indexOf('likes_')) return;
    ...
    init: function() {
        ...
        placeholderSetup(userSearch, {
            back: true
        });
        placeholderSetup('fave_new_link', {
            back: true
        });
        ...
        if (cur.faveData.faveUsers && cur.faveData.faveUsers.length > 0) {
            ...
            cur.qsorterEl = qsorter.init('users_content', {
                onReorder: Fave.reorderFave,
                xsize: 5,
                width: 120,
                height: 150 + (browser.msie8 ? 2 : 0)
            });
        }
    ...
    itemOver: function(post) {
        ...
        var lnk = ge('like_link' + post),
            icon = ge('like_icon' + post),
            no_likes = hasClass(icon, 'no_likes');
        if (!lnk) {
            return;
        }
        if (lnk.timeout) {
    ...
    remove: function(el, uid, user, hash, ev) {
        ...
        showFastBox({
            ...
            }, getLang('fave_sure_delete')
            .replace('{user}', user), getLang('global_delete'),
            function() {
                if (isVisible(curBox()
                        .progress)) return;
                ...
                ajax.post('al_fave.php', {
                    ...
                    onDone: function() {
                        ...
                        Fave.indexAll();
                        cur.lang['users_summary'] = geByClass1('summary', ge('users'))
                            .innerHTML = getLang('fave_N_users', cur.faveData.faveUsers.length);
                        Fave.drawUsers(cur.faveData.faveUsers);
                        show(ge('users_online'));
                        if (ge('fave_user_div_onl' + uid)) {
                            re('fave_user_div_onl' + uid);
                            var onlcnt = geByClass('fave_user_div', ge('users_online_content'))
                                .length;
                            if (onlcnt > 0) {
    ...
    bigphOver: function(obj, uid) {
        ...
        if (o.tagName != 'A' || o.className != 'fave_uph') {
            (o = obj.insertBefore(ce('a', {
                ...
                innerHTML: '<span class="fave_uph_label">' + getLang('global_photo_full_size') + '</span>'
            }), obj.firstChild))
            .onclick = Fave.bigphClick.pbind(uid);
            o._uid = uid;
        }
