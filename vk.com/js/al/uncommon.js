function doChangeMail(opts) {
    ...
    if (!/[a-z0-9\.\-_]+@[a-z0-9\.\-_]+/i.test(params.newmail)) {
        ge(opts.error)
            .innerHTML = cur.changeMailError;
        show(opts.error);
        elfocus(opts.input);
        return;
    }
    hide(opts.error);
    ajax.post('al_register.php', params, {
        onDone: function(res, t) {
            if (!res) {
...
function hideLeftOther() {
    ...
    window._hideLeftOtherTO = setTimeout(function() {
        animate(ge('left_other_rows'), {
            opacity: 0,
            height: 21
        }, 150, hide.pbind('left_other_rows'));
    }, 200);
}
