...
var Graffiti = {
    init: function() {
        ...
        function decodeHtmlText(htmlText) {
            var el = ce('div');
            el.innerHTML = htmlText.replace(/</g, '&lt;')
                .replace(/>/g, '&gt;');
    ...
    slideEventHandler: function(id, sliderXY, holder) {
        ...
        switch (id) {
            case "size":
                var _s = _curpos;
                if (_s < 1) _s = 1;
                Graffiti.brush.size = ((_s / 95 * 100) / 100)
                    .toFixed(2) * 64;
                Graffiti.updateSample();
                break;
            case "opacity":
                var _op = (Math.max(Math.min((_curpos / 95 * 100) / 100, 1), 0))
                    .toFixed(2);
    ...
    handleColorPickerEvents: function(e) {
        switch (e.type) {
            ...
            case "click":
                ...
                var _ = ctx.getImageData(pixelX, pixelY, 1, 1)
                    .data;
                var color = [].slice.call(_, 0, 3)
                    .join();
    ...
    resizeCanvases: function(w, h) {
        ...
        Graffiti.helpCanv.style.top = (-1 * (h * 2))
            .toFixed() + "px";
    ...
    drawSVG: function(params) {
        ...
        if (mouse.x.length < 2) {
            ...
            str += '" fill="none" stroke="rgb(' + color + ')" stroke-opacity="' + opacity + '" stroke-width="' + size +
