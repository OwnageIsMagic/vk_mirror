var Pagination = {
    init: function(e) {
        ...
        }), cur.pgFixedBack = cur.pgFixed.firstChild, cur.pgFixedPages = cur.pgFixedBack.nextSibling, cur.pgUrl && (Pagination.clearPreload(), cur.pgPreload && (
    ...
    reinit: function(e) {
        cur.initScrollFn(e), bodyNode.appendChild(cur.pgFixed), browser.mobile || (addEvent(cur.pgFixed, "mouseover", Pagination.fixedOver), addEvent(cur.pgFixed,
            "mouseout", Pagination.fixedOut), addEvent(window, "keydown", Pagination.keyNav))
    },
    pageTopUpdated: function() {
        cur.pgFixedStart = getXY(cur.pgCont)[1], cur.pgFixedDelta = -cur.pgFixed.offsetTop, cur.pgNodesCount && (cur.pgFixedDelta += cur.pgFixedStart - cur.pgCont.childNodes[
            0].offsetTop)
    },
    pageReady: function(e) {
        e && (cur.pgPage = cur.pgStartPage = Math.floor(cur.pgStart / cur.pgPerPage)), cur.pgNodesCount = cur.pgCont.childNodes.length, Pagination.pageTopUpdated(),
            setTimeout(Pagination.genFixed, 0)
    },
    deinit: function() {
        removeEvent(Pagination.scrollnode(), "scroll", Pagination.scrollResize), removeEvent(window, "resize", Pagination.scrollResize), removeEvent(window, "keydown",
            Pagination.keyNav), removeEvent(cur.pgFixed, "mouseover", Pagination.fixedOver), removeEvent(cur.pgFixed, "mouseout", Pagination.fixedOut), re(cur.pgFixed)
    },
    genFixed: function() {
        if (!(cur.pgStart + cur.pgCount <= cur.pgPerPage)) {
            var e, r, i, a, t = cur.pgHref ? ' href="' + cur.pgHref + '%d"' : "",
                ...
                p = [];
            n > c && (cur.pgByOne ? (e = (n - 1) * g, r = "&lsaquo;") : (e = 0, r = "&laquo;"), p.push('<a class="pg_flnk_nb"' + t.replace("%d", e) + o.replace("%d", e) +
                ">" + r + "</a>"));
            for (var d = Math.max(0, n - c); n > d; ++d) e = d * g, i = d >= cur.pgStartPage ? "pg_flnk_rd" : "pg_flnk", p.push('<a class="' + i + '"' + t.replace("%d", e) +
                o.replace("%d", e) + ">" + (d + 1) + "</a>");
            p.push('<a class="pg_flnk_sel"' + t.replace("%d", n * g) + o.replace("%d", n * g) + ">" + (n + 1) + "</a>");
            for (var d = n + 1; n + c + 1 > d && u > d; ++d) e = d * g, i = e >= cur.pgStart + cur.pgNodesCount ? "pg_flnk" : "pg_flnk_rd", p.push('<a class="' + i + '"' +
                t.replace("%d", e) + o.replace("%d", e) + ">" + (d + 1) + "</a>");
            u - c - 1 > n && (cur.pgByOne ? (e = (n + 1) * g, r = "&rsaquo;") : (e = (u - 1) * g, r = "&raquo;"), p.push('<a class="pg_flnk_nb"' + t.replace("%d", e) + o.replace(
                "%d", e) + ">" + r + "</a>")), cur.pgFixedPages.innerHTML = p.join(""), isVisible(cur.pgFixed) ? a = getSize(cur.pgFixedPages) : (show(cur.pgFixed), a =
                getSize(cur.pgFixedPages), hide(cur.pgFixed)), setStyle(cur.pgFixedBack, {
                width: a[0],
                height: a[1]
    ...
    updateFixed: function(e, r) {
        return void 0 === r && (r = scrollGetY()), Pagination.updateFixedLeft(e), !cur.pgOver && r < cur.pgFixedStart ? void(isVisible(cur.pgFixed) && !cur.pgHiding && (
            cur.pgHiding = !0, cur.pgShowing = !1, fadeOut(cur.pgFixed, 300, function() {
                cur.pgHiding = !1
            }))) : void(cur.pgShowing || !cur.pgHiding && isVisible(cur.pgFixed) || (cur.pgHiding && data(cur.pgFixed, "tween")
            .stop(!0), cur.pgShowing = !0, fadeIn(cur.pgFixed, 300, function() {
                cur.pgShowing = !1
            })))
    },
    scrollResize: function(e) {
        if (!(browser.mobile && !browser.safari_mobile || cur.pgPaused)) {
            ...
            if ("scroll" == e.type) {
                ...
                o == t - 1 && a[cur.pgNodesCount - 1] && i + lastWindowHeight > a[cur.pgNodesCount - 1].offsetTop + cur.pgFixedDelta && ++o, o > 0 && --o, cur.pgPage !=
                    cur.pgStartPage + o ? (cur.pgPage = cur.pgStartPage + o, Pagination.genFixed()) : Pagination.updateFixed(e)
    ...
    clearPreload: function() {
        var e = extend({
            ...
            i = new RegExp("^" + escapeRE(r)
                .replace(/([&#]offset=)/, "$1\\d+") + "$", "i");
    ...
    loaded: function(e, r, i, a, t, o) {
        ...
        if (n) cur.pgStart = a, cur.pgCont.innerHTML = i, cur.pgPages.innerHTML = t;
            ...
            for (var g = ge(cur.pgCont), c = (cur.pgPrepare || Pagination.prepare)
                    .apply(window, arguments); c.firstChild;) g.appendChild(c.firstChild);
    ...
    showMore: function() {
        ...
        if (!(!isVisible(cur.pgMore) || e && buttonLocked(cur.pgMore) || isVisible(cur.pgMorePrg))) return cur.pgLoading ? void(cur.pgLoading = 2) : void ajax.post(cur.pgUrl,
            extend({
                offset: cur.pgOffset,
                part: 1
            }, cur.pgParams || {}), {
