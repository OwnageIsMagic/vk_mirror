var Feed = {
    ...
    update: function(e) {
        if (!cur.feedUpdateLoading && !(cur.add_queue && window.Notifier && Notifier.addKey(cur.add_queue, feed.updated) && "news" != cur.section || "top" == cur.subsection ||
                inArray(cur.section, ["search", "photos_search", "mentions", "articles", "articles_search", "likes", "recommended"]))) {
            var t = Math.random();
    ...
    pushEvent: function(e, t) {
        ...
        switch (o) {
            ...
            case "edit_post":
                ...
                E && (E = isVisible(domNS(E))), (T = feed.needScrollPost(t, B)) && (c -= B.offsetHeight);
                var L = psr(rs(e[3], {
                        poll_hash: cur.wallTpl.poll_hash
                    })),
                    m = ge("post" + r);
                m && !isVisible(m.parentNode) && (L = wall.updatePostImages(L)), val(B, L), E && (E = geByClass1("wall_post_more", B), E && E.onclick()), ge("post_poll_id" +
                    r) && wall.updatePoll(r), T && (c += B.offsetHeight), nodeUpdated(B);
                break;
            case "edit_reply":
                var N = e[3],
                    B = ge("wpt" + N);
