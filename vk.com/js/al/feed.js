var Feed = {
                            ...
                            if (!(postEl = ge('post' + post_raw))) {
                                ...
                                if (cnt) {
                                    var stDelta = postEl.parentNode.offsetHeight,
                                        newCnt = geByClass('new_reply', repliesCont, 'div')
                                        .length;
    ...
    updated: function(key, data) {
        ...
        if (data.failed) {
            ...
            cur.lp_error_to = setTimeout(feed.getNewQKey.bind(feed)
                .pbind(timeout), timeout * 1000);
            return;
        }
            ...
            case 'new_post':
                {
                    ...
                    if (place) { // Client-side grouping posts
                        ...
                        if (grouped = geByClass1('feed_reposts_wrap' + place, cont, 'div')) {
                            first = geByClass1('feed_reposts_first', grouped, 'div')
                                .firstChild;
                            ...
                            newEl = se('<div class="feed_row"' + (showCollapsed ? ' style="display:none"' : '') + '><div class="feed_repost' + place + '">' + postHTML +
                                '</div></div>');
                    ...
                    if (showCollapsed && addedToStart) {
                        ...
                        ge('feed_new_posts')
                            .innerHTML = getLang('news_new_posts', cur.newPostsCount);;
                        show('feed_new_posts');
                    }
            ...
            case 'new_tagged':
                {
                    ...
                    if (grouped = geByClass1('post_photos' + place, cont, 'div')) {
                        ...
                        for (i = 0; i < thumbs.length; i += 3) {
                            thumbs_html.push(cur.wallTpl.photo_thumb.replace(/%photo_id%/g, thumbs[i])
                                .replace(/%thumb%/g, thumbs[i + 1])
                                .replace(/%temp%/g, thumbs[i + 2].replace(/"/g, '&quot;'))
                                .replace('%full_id%', ev[2]));
                        }
    ...
    feedPhotoRepl: function(thumbs, repl, ev) {
        ...
        ext = {
            ...
            count: cnt,
            full_id: ev[2],
            item_id: ev[2].replace(/^1_/, 'photos_')
                .replace(/^3_/, 'tagged_'),
            sec_name: stripHTML(ev[3])
        };
        return ext;
    ...
    notifyShowEditReply: function(item, opts) {
        if (cur.editing && cur.notifyReplyData && cur.notifyReplyData[cur.editing].disabled) {
            feed.notifyCheckHideReply(cur.editing, (window.event || {})
                .target);
        }
        ...
        var itemEl = ge('feedback_row_' + item),
            pEl = domPN(itemEl),
            acts = '',
            repl = {
                hint: opts.hint || '',
        ...
        if (opts.disabled) {
            itemEl.parentNode.appendChild(se(rs(cur.options.feedback_dis, {
                item: item,
                text: opts.disabled
            })));
            setTimeout(function() {
    ...
    notifyRowOver: function(item, state) {
        ...
        if (state && hasClass(el, 'feedback_row_clickable')) {
            ...
            if (likeEl = geByTag1('div', geByClass1('like_wrap', ge(el), 'div'))) {
                var to = data(likeEl, 'likeoutTO'),
                    tween = data(likeEl, 'tween');
                to && clearTimeout(to);
    ...
    expandAppend: function(item, cont, data, right, onlyClient) {
        ...
        if (data.date) {
            ...
            if (ge('feedback_grouped_' + item)
                .getAttribute('data-mid') == data.mid) {
                return;
            }
        }
        if (onlyClient) return;
        var cl = (geByClass1('feedback_grouped_row', ge('feedback_grouped_' + item)) || {})
            .className || 'feedback_grouped_row';
        cont.appendChild(domFC(ce('div', {
            innerHTML: '\
<div class="feedback_grouped_row_wrap" id="feedback_grouped_' + item + '_' + data.mid + '" data-mid="' + data.mid +
                '" data-sex="' + data.sex + '"' + (data.date ? '' : ' data-auto="1"') + '>\
  <div class="' + cl +
                '">\
      ...
      <tr>\
        <td class="feedback_row_photo">\
          <div class="feedback_row_photo" onclick="event.cancelBubble = true;">\
            <a class="feedback_row_photo_lnk" href="' +
                data.href +
        ...
        <td class="feedback_row_content">\
          <div class="feedback_row_group_names">' +
                langSex(data.sex, langs)
                .replace('{names}', data.lnk) +
                '</div>\
          <div class="feedback_row_group_photos clear_fix"></div>\
          <div class="feedback_row_date">' + (data.date ||
                    '&nbsp;') + '</div>\
        </td>\
        ' + right + '\
      </tr>\
    ...
    expanded: function(item, data, onlyClient) {
        ...
        var phs = geByClass1('feedback_row_group_photos', el),
            tmp = ce('div');
        if (domFC(phs)) return;
        ...
        var cls = geByClass1('feedback_row_t', el)
            .rows[0].cells,
    ...
    expand: function(ev, item) {
        ...
        if (thumbs) {
            var exp = ce('div', {
                    className: 'feed_row_fb_expanded'
                }),
                langs = ['', val('fbgr_' + item + '_1'), val('fbgr_' + item + '_2')],
                cls = geByClass1('feedback_row_t', el)
                .rows[0].cells,
                right = (cls.length > 2) ? '<td class="' + cls[2].className + '">' + val(cls[2]) + '</td>' : '';
            domPN(domPN(el))
                .insertBefore(exp, domNS(domPN(el)));
            for (var e = domFC(phs); e; e = domFC(phs)) {
                if (hasClass(e, 'feedback_group_photo_wrap')) {
                    var mid = e.getAttribute('data-mid'),
                        ...
                        img = (domFC(a) || {})
                        .src || '/images/camera_c.gif';
                    feed.expandAppend(item, exp, {
                        mid: mid,
                        sex: sex,
                        href: href,
                        img: img,
            ...
            ajax.post('al_feed.php', {
                ...
                onDone: feed.expanded.pbind(item),
                cache: 1
            });
        } else {
            var exp = domNS(domPN(el));
            ...
            for (var e = domFC(exp); e; e = domFC(exp)) {
                if (hasClass(e, 'feedback_grouped_row_wrap') && e.getAttribute('data-auto')) {
                    ...
                    phs.appendChild(domFC(ce('div', {
                        innerHTML: '\
<div class="feedback_group_photo_wrap fl_l" data-mid="' + e.getAttribute('data-mid') + '" data-sex="' + e.getAttribute(
                                'data-sex') + '">\
  <a href="' + name.href + '" class="feedback_group_photo" title="' + replaceEntities(val(name)) +
                            '"><img src="' + ((geByClass1('feedback_row_photo', geByClass1('feedback_row_photo_lnk', e)) || {})
                                .src || '/images/camera_c.gif') + '" class="feedback_group_photo" /></a>\
</div>'
                    })));
                }
                re(e);
        ...
        var names = geByClass1('feedback_row_group_names', el),
            tmp = val(names),
    ...
    ungroup: function(ev, item) {
        ...
        var hid = domNS(domPN(el)),
            names = geByClass1('feedback_row_group_names', el),
            tmp = val(names),
    ...
    ignoreItem: function(post_raw, feed_raw, hash) {
        var adData = ge('post' + post_raw)
            .getAttribute('data-ad');
    ...
    ignoreOwner: function(post_raw, owner_id, hash, btn) {
        ...
        ajax.post('/al_feed.php?misc', {
            ...
            owner_id: owner_id,
            hash: hash,
            list: cur.section == 'list' && cur.list || 0
        }, {
            onDone: function(html) {
                val('post' + post_raw, html);
    ...
    unsubscribe: function(post_raw, hash) {
        ...
        ajax.post('al_feed.php', {
            ...
            onDone: function(html) {
                ge('post' + post_raw)
                    .innerHTML = html.replace('%post_raw%', post_raw);
            }
        });
    },
    ...
    restorePost: function(post_raw) {
        ge('post' + post_raw)
            .innerHTML = cur.feedEntriesHTML[post_raw];
    ...
    sliderSelectChanged: function(e) {
        var percent = ge(cur.section == 'articles' ? 'feed_period_select' : 'feed_rate_select')
            .value;
        feed.sliderUpdate(percent);
        feed.sliderApply();
    },
    ...
    deleteList: function(list_id, force) {
        ...
        if (!force) {
            var box = showFastBox({
                title: getLang('news_delete_list_confirm_title'),
                dark: 1,
                bodyStyle: 'padding: 20px; line-height: 160%;'
            }, getLang('news_delete_list_confirm'), getLang('global_delete'), function() {
    ...
    infoTopFeedNotification: function(hash) {
        ...
        setTimeout(function() {
            Feed.hideTopFeedNotification(hash, true);
        }, 2500);
    },
        ...
        if (m = f.id.match(r)) {
            ...
            if (m = c.match(/feed_reposts_wrap(-?\d+_\d+)/)) {
                ...
                if (hasClass(f, 'post_photos')) {
                    ...
                    if (p) {
                        p = domFC(p);
                        if (p && (m = p.getAttribute('data-post-id')
                                .match(/^(-?\d+_p?\d+)$/))) {
                            post_id = m[1];
                        }
                    }
    ...
    searchChooseGeoPoint: function() {
        ...
        if (currentPointMatches) {
            ...
            params.zoom = [8, 11, 14, 17, 20][(currentPointMatches[3] || '')
                .length
            ]
        }
    ...
    searchChooseFilter: function(filter, item) {
        ...
        switch (filter) {
            case 'type':
                uiDropdown = cur.uiSearchPostType;
                break;
            case 'has':
                uiDropdown = cur.uiSearchPostAttach;
                break;
            case 'likes':
                uiDropdown = cur.uiSearchPostLikes;
                break;
    ...
    searchContentOnChange: function() {
        ...
        var domainMatches = url && url['domain'].match(/(^|\.|\/\/)vkontakte\.ru|vk\.com/),
            mediaMatches = domainMatches && url['query'].match(
                /(?:#(photo)|^\/(photo|video|album|page|audio|wall)|z=(album|photo|video)|w=(page|wall))(-?\d+_)?(\d+)(?:\?reply=(\d+))?$/),
            newTag = '';
    ...
    sinceOwnerBigphOver: function(obj, oid) {
        ...
        if (o.tagName != 'A' || o.className != 'feed_since_ownerph') {
            o = obj.insertBefore(ce('a', {
                className: 'feed_since_ownerph',
                href: ch && ch._id ? ('/photo' + ch._id + '?all=1') : ('/albums' + oid),
                innerHTML: '<span class="feed_since_ownerph_label">' + cur.lang.global_photo_full_size + '</span>'
            }), obj.firstChild);
    ...
    init: function(options) {
        ...
        cur.nav.push(function(changed, old, n, opts) {
            ...
            delete(params[0]);
            var splitt = !!ge('feedtab_updates') || !ge('feedtab_news');
            if (splitt && changed.section !== undefined && ((cur.section == 'notifications' || cur.section == 'replies') && changed.section != 'notifications' &&
                    changed.section != 'replies' || cur.section != 'notifications' && cur.section != 'replies' && (changed.section == 'notifications' || changed.section ==
                        'replies'))) {
                return;
            }
        ...
        if (browser.mobile) {
            setStyle('feed_rate_slider_wrap', {
                width: 150
            });
            hide('feed_rate_full');
    ...
    applyUi: function() {
        ...
        cur.searchSortMenu.setOptions({
            title: cur.search_sort[1]
        });
    ...
    mentionClick: function(el, ev) {
        var post = ((el.getAttribute('mention') || '')
            .match(/^bp(-?\d+_\d+)$/) || {})[1];
    ...
    toggleCustomFeedTab: function(el, section, hash, htitle) {
        ...
        if (!hidden) {
            ...
            var box_confirm = getLang('news_hide_named_list')
                .replace('{title}', getLang(htitle));
            var onYes = function() {
    ...
    init: function(input, options) {
        ...
        addEvent(input, 'keyup click mouseup', function(e) {
            ...
            cur.requestTimeout = setTimeout(function() {
                ajax.post('/hints.php?act=a_feed_hints', {
                    q: term,
                    section: section
                }, {
