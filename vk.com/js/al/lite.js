...
window.locHash = location.hash.replace('#/', '')
    .replace('#!', '');
window.locDomain = locHost.toString()
        ...
        var waiter = function() {
            ...
            for (var i in wait) {
                ...
                if (!StaticFiles[f].l && StaticFiles[f].t == 'css' && getStyle(StaticFiles[f].n, 'display') == 'none') {
                    if (stVersions[f] < 0) {
                        topMsg(
                            '<b>Warning:</b> Something is bad, please <b><a href="/techsupp.php?fid=1&act=t&tid=497998">clear your cache</a></b> and restart your browser.',
                            10);
                    }
                    StaticFiles[f].l = 1;
                }
            ...
            switch (code) {
                ...
                case 2: // captcha
                    if (intval(answer[1]) === 2) {
                        ...
                        if (vk.nophone == 1 && !vk.nomail) {
                            addText = getLang('global_try_to_activate')
                                .replace('{link}', '<a class="phone_validation_link">')
                                .replace('{/link}', '</a>');
                ...
                case 12:
                    ...
                    cur.box = showFastBox({
                        ...
                        }, getLang('global_mobile_need_validation')
                        .replace('{link}', '<br/><a onclick="cur.box.hide();" href="' + vk.loginscheme + '://vk.com/settings?z=validatebox">')
                        .replace('{/link}', '</a>'));
    ...
    var getLoc = function(skipFrame) {
        ...
        if (vk.al == 3) {
            ...
            if (withFrame && !skipFrame) {
                ...
                loc = browser.msie6 ? ((location.toString()
                    .match(/#(.*)/) || {})[1] || '') : location.hash.replace(/^#/, '');
...
function addClass(obj, name) {
    if ((obj = ge(obj)) && !hasClass(obj, name = trim(name)
            .replace(window.whitespaceRegex, ' '))) {
...
window.Widgets = {
    ...
    popupBoxOpen: function(url, params, name, opts) {
        ...
        window.activePopup = window.open(url + '?' + ajx2q(params), name, 'width=' + opts.width + ',height=' + opts.height + ',left=' + left + ',top=' + top +
    ...
    popupBoxAdjust: function(startWidth, startHeight, opts) {
        ...
        function adjust() {
            if ((interruptedCentering || opts.nocenter) && interruptedResize) {
                return;
            }
            ...
            if (width != popupWidth || height != popupHeight) {
                ...
                interruptedCentering || opts.nocenter || window.moveTo(Math.floor((screen.width - popupWidth - panelsWidth) / 2), Math.floor((screen.height - popupHeight +
                    panelsHeight) / 2));
