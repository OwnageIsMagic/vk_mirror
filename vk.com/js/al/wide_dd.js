(function() {
    ...
    var wdd = {
        _textEvent: function(e) {
            ...
            switch (e.type) {
                ...
                case 'keypress':
                    ...
                    if (e.keyCode == KEY.UP) {
                        if (el && domPS(el)) {
                            wdd.over(id, domPS(el)
                                .id.replace(/^wddi/, '')
                                .replace(new RegExp('_' + id + '$', ''), ''), true);
                        }
                        return cancelEvent(e);
                    } else if (e.keyCode == KEY.DOWN) {
                        if (el && domNS(el)) {
                            wdd.over(id, domNS(el)
                                .id.replace(/^wddi/, '')
                                .replace(new RegExp('_' + id + '$', ''), ''), true);
                        }
                        return cancelEvent(e);
        ...
        _widenTextInput: function(dd) {
            if (vk.rtl) {
                dd.text.style.width = (Math.max((dd.text.offsetTop > 20 ? dd.fullWidth : dd.partWidth) - dd.fullWidth + (dd.text.offsetLeft + dd.text.offsetWidth) - 2,
                    dd.addWidth) - dd.textDelta) + 'px';
            } else {
                dd.text.style.width = (Math.max((dd.text.offsetTop > 20 ? dd.fullWidth : dd.partWidth) - (dd.text.offsetLeft - dd.textOffset) - 2, dd.addWidth) - dd.textDelta) +
                    'px';
            }
            wdd._showList(dd);
        },
        ...
        _indexItem: function(dd, k, v) {
            ...
            for (i = 0, l = keys.length; i < l; ++i) {
                words += ' ' + (v[keys[i]] || '')
                    .replace(dd.opts.delimeter, ' ')
                    .replace(/<[^>]*>/g, '');
            }
            words = trim(words.toLowerCase())
                .split(/\s+/);
        ...
        _renderList: function(dd, data, showEmpty, append) {
            ...
            for (var i = 0, l = data.length; i < l; ++i) {
                ...
                var img = item[3] ? '<b class="fl_l wddi_thumb"><img class="wddi_img" src="' + (isArray(item[3]) ? '/images/icons/multichat.png' : item[3]) + '" />' +
                    mark + '</b>' : '',
                    cl;
                if (shown) {
                    cl = 'wddi';
                ...
                html.push('\
<div class="' + cl + '" onmousedown="WideDropdown.select(\'' + dd.id + '\', event)" onmouseover="WideDropdown.over(\'' + dd.id + '\', \'' +
                    clean(id) + '\')" id="wddi' + id + '_' + dd.id + '" onclick="">\
  <div class="wddi_data">' + img + '\
    <div class="wddi_text">' + text +
                    '</div>\
    <div class="wddi_sub">' + sub + '</div>\
        ...
        _updateImgs: function(dd, noAnim) {
            ...
            if (noAnim === true) {
                ...
                cssAnim(n ? dd.img.insertBefore(o, n) : dd.img.appendChild(o), {
                    opacity: 1
                }, {
                    duration: 150
                }, function() {
...
<div class="wdd_add2">\
  <table cellspacing="0" cellpadding="0"><tr>\
    <td><div class="wdd_add3">\
      <nobr>' +
                    getLang('global_add') + '</nobr>\
    </div></td>\
    <td><div class="wdd_add_plus" onmousedown="WideDropdown.focus(\'' + id +
                    '\')"></div></td>\
  </table>\
</div>'
            }), dd.text);
            ...
            dd.listWrap = el.insertBefore(ce('div', {
                className: 'wdd_lwrap' + (opts.toup ? ' wdd_toup' : ''),
                innerHTML: '<div class="wdd_list"></div><div class="wdd_bottom1"></div><div class="wdd_bottom2"></div>'
            }, {
                display: 'none',
        ...
        over: function(id, item, check) {
            ...
            if (el = ge('wddi' + dd.over + '_' + id)) {
                ...
                if (check && dd.scroll) {
                    var et = el.offsetTop,
                        eh = el.offsetHeight,
                        lt = dd.list.scrollTop,
                        lh = dd.list.offsetHeight;
                    if (et < lt) {
...
<div class="summary_tab2">\
  <table cellspacing="0" cellpadding="0"><tr>\
    ...
    <td><div class="summary_tab_x" onmousedown="WideDropdown.deselect(\'' + id + '\', \'' + clean(
                        sel + '') + '\', event)"></div></td>\
  </table>\
</div>'
            }));
