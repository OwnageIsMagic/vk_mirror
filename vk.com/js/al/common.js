...
if (!window.locDomain) {
    var locDomain = location.host.toString()
...
(function() {
    ...
    if (navigator.plugins && navigator.mimeTypes && navigator.mimeTypes.length) {
        ...
        if (x && x.description) {
            var ver = x.description.replace(/([a-zA-Z]|\s)+/, '')
                .replace(/(\s+r|\s+b[0-9]+)/, '.')
                .split('.');
...
window.locHash = location.hash.replace('#/', '')
    .replace('#!', '');
window.locBase = location.toString()
    .replace(/#.+$/, '');
...
function getLang() {
    try {
        ...
        debugLog('lang error:' + e.message + '(' + Array.prototype.slice.call(arguments)
            .join(', ') + ')');
    }
}
...
function debugEl(el) {
    return el && (((el.tagName || '')
        .toLowerCase() + (el.className ? '.' + el.className.replace(/\s+/g, '.') : '') + (el.id && !/^__vk/.test(el.id) ? '#' + el.id : '')) || el.toString()) || '[NULL]';
}
        ...
        do {
            ...
            if (pos == 'fixed' || pos == 'absolute' || (pos == 'relative')) {
                ...
                if (pos == 'fixed' && !forFixed) {
                    left += ((obj.offsetParent || {})
                        .scrollLeft || bodyNode.scrollLeft || htmlNode.scrollLeft);
                    top += ((obj.offsetParent || {})
                        .scrollTop || bodyNode.scrollTop || htmlNode.scrollTop);
                }
            }
...
function setStyle(elem, name, value) {
    ...
    if (name == 'opacity') {
        ...
        try {
            ...
            if (isN && (/height|width/i)
                .test(name)) value = Math.abs(value);
            elem.style[name] = isN && !(/z-?index|font-?weight|opacity|zoom|line-?height/i)
                .test(name) ? value + 'px' : value;
        } catch (e) {
...
function _eventHandle(event) {
    ...
    var eventHandlers = (handlers[event.type] || [])
        .slice();
...
function updGlobalPlayer() {
    ...
    var isDev = hasClass(bodyNode, 'dev'),
        topMargin = isDev ? 11 : 9,
        leftMargin = isDev ? 4 : 6;
    var gpW = 154,
        gpL = vk.rtl ? pbxy[0] - gpW + leftMargin : pbxy[0] + pbsz[0] - leftMargin;
    var barHeight = getXY('page_layout')[1],
        menuHeight = getSize(ge('page_header'))[1] || 0;
    if (vk.id && (gpL + gpW <= dwidth && !vk.rtl || vk.rtl && gpL >= 0)) {
        ...
        if (browser.mac) {
            ...
            var gpTop = headFixed ? menuHeight + topMargin + barHeight : Math.max(Math.min(menuHeight + 9 - scrollGetY(), menuHeight + topMargin) + barHeight, 20),
                oldStyle = getStyle(_gp, 'top'),
                oldTop = parseInt(oldStyle);
...
function onDocumentClick(e) {
    ...
    if (_pads.shown) {
        ...
        for (var el = e.target; el != bodyNode; el = domPN(el)) {
            if (!el || el.id && el.id.match(/^(box_|mv_|wk_|pad_)?(layer|pad)_(bg|wrap)$/) || (el.className || '')
                .match(/(^|\s)tt(\s|$)/)) break;
        }
...
function initFixedMenu() {
    var lnks = {
        ...
        i, el;
    for (i in lnks) {
        text.push('<a class="fmenu_item fl_r" onmouseover="Pads.preload(\'' + i + '\')" onmousedown="return Pads.show(\'' + i + '\', event)" href="' + lnks[i] +
            '"><span class="fmenu_text inl_bl"></span><span id="fmenu_' + i + '" class="fmenu_icon inl_bl"></span></a>');
    }
    re(_fMenu);
    extend(window, {
...
function domReady() {
    ...
    if (!vk.id && ls.checkVersion() && ls.get('last_reloaded')) {
        try {
            var permanent = ['sound_notify_off'],
                lsData = {};
                                ...
                                if (c > __stm.lowlimit && stVersions[f] > 0 || c > __stm.highlimit) {
                                    if (stVersions[f] < 0) {
                                        topError('<b>Error:</b> Could not load <b>' + f + '</b>.', {
                                            dt: 5,
                                            type: 1,
                                            msg: 'Failed to load with ' + __stm.lowlimit + '/' + __stm.highlimit + ' limits (' + ((vkNow() - vk.started) / 100) +
                                                ' ticks passed)',
                                            file: f
                                        });
                                        StaticFiles[f].l = 1;
...
var ajax = {
    ...
    _post: function(url, q, o) {
        ...
        if (window.debuglogSent) {
            o._reqid = debuglogSent(url + (q ? ': ' + ajx2q(q)
                .replace(/&/g, '&amp;') : ''));
            if (o.frame) {
...
function HistoryAndBookmarks(params) {
    ...
    var setFrameContent = function(loc) {
        try {
            ...
            d.write(
                '<script type="text/javascript">var u=navigator.userAgent,d=location.host.toString().match(/[a-zA-Z]+\\.[a-zA-Z]+$/)[0];if(/opera/i.test(u)||!/msie 6/i.test(u)||document.domain!=d)document.domain=d;</script>' +
                '<div id="loc">' +
                loc.replace('&', '&amp;')
                .replace('"', '&quot;')
                .replace('>', '&gt;')
...
function leftBlockFriendHide(blockId, blockData, hash) {
    ...
    ajax.post('al_index.php', query, {
        onDone: function(html) {
            if (html) {
                var block = se(html);
                if (geByClass('left_friend_block', container)
                    .length >= 2) {
                    hide(block);
                }
...
function leftBlockToggleFriend(mid, type, hash, act, ev) {
    ...
    ajax.post('al_friends.php', {
        ...
        showProgress: function() {
            var tt = (ge('left_friend_subscribed') || {})
                .tt;
            if (tt) {
                tt.hide({
                    fasthide: 1
...
function comScoreUDM(url, referrer) {
    ...
    vkImage()
        .src = locProtocol + '//' + (locProtocol == 'https:' ? 'sb' : 'b') + '.scorecardresearch.com/p?c1=2&c2=13765216&c3=&c4=' + escape(url) + '&c5=&c7=' + escape(url) + '&c9=' +
        escape(referrer) + '&c15=&cv=2.0&cj=1&rn=' + Math.random();
}
...
function updateOtherCounters(url, referrer) {
    ...
    vkImage()
        .src = locProtocol + '//www.tns-counter.ru/V13a***R>' + referrer.replace(/\*/g, '%2a') + '*vk_com/ru/UTF-8/tmsec=vksite_total/' + Math.round(Math.random() * 1E9);
...
function handlePageView(params) {
    ...
    if (width != vk.width || widthDec != vk.width_dec) {
        ...
        ge('page_body')
            .style.width = (width - widthDec) + 'px';
        if (ge('ts_wrap') && hasClass(ge('ts_wrap'), 'vk')) ge('ts_wrap')
            .style.width = (width - 191) + 'px';
...
function handleSetCount(e, v, id) {
    var newCnt = '+',
        attr = '',
        maxCntLen = (id == 'spr') ? 5 : ((vk.fs > 11) ? 2 : 3),
        plus = '+',
...
function handlePageParams(params) {
    ...
    var confMail = ge('confirm_mail_wrap'),
        chPhone = ge('change_phone_wrap'),
        mv = ge('mvk_footer_lnk');
    if (mv) {
    ...
    var i = 0,
        ...
        sh = false,
        ids = ['fr', 'ph', 'vid', 'msg', 'nts', 'gr', 'ev', 'wsh', 'ap', 'ads', 'nws', 'wk', 'docs'];
    var lnks = ['friends', 'albums' + vk.id, 'video', '', 'notes', 'groups', 'events', 'gifts.php?act=wishlist', 'apps', ads_menu_link, 'feed' + (ge('l_nwsf') ?
        '?section=notifications' : ''), 'pages', 'docs'];
...
var nav = {
    ...
    go: function(loc, ev, opts) {
        ...
        if (loc.tagName && loc.tagName.toLowerCase() == 'a') {
            ...
            loc = loc.href || '';
            if (ev && !(loc || '')
                .match(new RegExp('^' + locProtocol + '//' + locHost, 'i'))) {
                return;
            }
        }
        var strLoc = '',
...
if (vk.time && !browser.opera_mobile) setTimeout(function() {
    ...
    if (time[1] > vk.time[1] && time[2] == 1) {
        if (vk.time[2] == 31 || (vk.time[1] == 4 || vk.time[1] == 6 || vk.time[1] == 9 || vk.time[1] == 11) && vk.time[2] == 30 || vk.time[1] == 2 && (vk.time[2] == 29 ||
                vk.time[2] == 28 && (vk.time[0] % 4))) {
            vk.time[2] = 0;
        } else {
            vk.time[2] = time[2] = 0;
        }
    } else if (vk.time[1] > time[1] && vk.time[2] == 1) {
        if (time[2] == 31 || (time[1] == 4 || time[1] == 6 || time[1] == 9 || time[1] == 11) && time[2] == 30 || time[1] == 2 && (time[2] == 29 || time[2] == 28 && (vk.time[
                0] % 4))) {
            time[2] = 0;
        } else {
            time[2] = vk.time[2] = 0;
...
function parseCyr(text) {
    ...
    for (i = 0; i < rus1.length; i++) {
        outtext = outtext.split(rus1[i])
            .join(lat1[i]);
    }
    for (i = 0; i < rus2.length; i++) {
        outtext = outtext.split(rus2.charAt(i))
            .join(lat2.charAt(i));
    }
    return (outtext == text) ? null : outtext;
}
...
function parseLatKeys(text) {
    ...
    for (i = 0; i < lat.length; i++) {
        outtext = outtext.split(lat.charAt(i))
            .join(rus.charAt(i));
    }
    return (outtext == text) ? null : outtext;
}
...
function __phCheck(el, opts, focus, blur) {
    ...
    if (editable) {
        var v = (el.textContent !== undefined ? el.textContent : el.innerText) || geByTag('img', el)
            .length;
    } else {
        var v = el.value;
    }
...
function placeholderSetup(id, opts) {
    ...
    var b1 = el.phcont = el.parentNode.insertBefore(ce('div', {
        ...
        innerHTML: '<div class="input_back"><div class="input_back_content' + (o.big ? ' big' : '') + '" style="width: ' + (getSize(el)[0] - 20) + 'px;">' + ph +
            '</div></div>'
    }), el);
    var b = domFC(b1);
    var c = domFC(b);
    ...
    if (el.value || (o.editable && ((el.textContent !== undefined ? el.textContent : el.innerText) || geByTag('img', el)
            .length))) {
        el.phshown = false;
        hide(b1);
    }
...
var __bq = boxQueue = {
    ...
    hideLast: function(check, e) {
        if (__bq.count()) {
            ...
            if (check === true && (box.changed || __bq.skip || e && e.target && e.target.tagName && e.target.tagName.toLowerCase() != 'input' && cur.__mdEvent && e.target !=
                    cur.__mdEvent.target)) {
                __bq.skip = false;
                return;
            }
            box.hide();
...
<div class="box_title_wrap"><div class="box_x_button">' + (options.dark ? getLang('global_close') :
                '') + '</div><div class="box_title"></div></div>\
<div class="box_body" style="' + options.bodyStyle + '"></div>\
<div class="box_controls_wrap"' +
            controlsStyle + '><div class="box_controls">\
...
function showBox(url, params, options, e) {
    ...
    box.removeButtons()
        .addButton(getLang('global_close'));
...
<div class="captcha">\
  ...
  <div><input type="text" class="big_text" maxlength="7" placeholder="' + getLang(
            'global_captcha_input_here') + '" /><div class="progress" /></div></div>\
</div>' + (o.addText || '');
        box = showFastBox({
    ...
    el.autosize = {
        ...
        update: function(e) {
            ...
            var newStyle = {
                ...
                curOverflow = getStyle(el, 'overflow')
...
function renderFlash(cont, opts, params, vars) {
    ...
    ge(cont)
        .innerHTML = browser.flashwrap(opts, params);
    return true;
}

function playAudioNew() {
    var args = arguments;
    if (args[args.length - 1] !== false) args = Array.prototype.slice.apply(arguments)
        .concat([true]);
...
function showAudioClaimWarning(owner_id, id, delete_hash, claim_id, title, reason) {
    ...
    if (reason == 'crap') {
        claimText = getLang(claim_id >= 0 ? 'audio_crap_warning_text' : 'audio_crap_warning') || getLang(claim_id > 0 ? 'audio_claim_warning_objection' : (claim_id == 0 ?
            'audio_claim_warning_text' : 'audio_claim_warning'));
    ...
    claimText = claimText.split('{objection_link}')
        .join('<a href="/help?act=cc_objection&claim=' + claim_id + '&content=audio' + owner_id + '_' + id + '">' + getLang('audio_claim_objection') + '</a>');
    claimText = claimText.split('{delete_link}')
        .join('<a href="#" onclick="deleteAudioOnClaim(' + owner_id + ',' + id + ',\'' + delete_hash + '\'); return false;">' + getLang('audio_claim_delete') + '</a>');
...
function deleteAudioOnClaim(owner_id, id, delete_hash) {
    ...
    ajax.post('/audio', {
        ...
        onDone: function(text, delete_all) {
            ...
            cur.deletedAudios[id] = ge('audio' + id)
                .innerHTML;
            el.innerHTML = text;
...
var gSearch = new(function() {
    ...
    this.go = function(e) {
        var url = '/gsearch.php?section=' + (self.last_section || 'people') + '&q=' + trim(ge('search_input')
            .value) + '&name=1';
...
function zNav(changed, opts, fin) {
    ...
    if (zt) {
        ...
        switch (zt[1]) {
            ...
            case 'single':
                if (w === undefined) {
                    stManager.add(['single_pv.css', 'single_pv.js'], ge(z)
                        .onclick);
                }
                return false;
                break;
...
function animateCount(el, newCount, opts) {
    ...
    var incr = opts.str ? (curCount.length == newCount.length ? curCount < newCount : curCount.length < newCount.length) : curCount < newCount,
        big = (incr ? newCount : curCount)
        .toString(),
        small = (incr ? curCount : newCount)
        .toString(),
        constPart = [],
        constEndPart = [],
        bigPart = '',
    ...
    if (!opts.str) {
        small = ((new Array(big.length - small.length + 1))
            .join('0')) + small;
    }
    ...
    constPart = constPart.join('')
        .replace(/\s$/, '&nbsp;');
    constEndPart = constEndPart.join('')
        .replace(/^\s/, '&nbsp;');
    ...
    var bigW = geByClass1('counter_anim_big_c', animEl, 'span')
        .offsetWidth,
        smallW = vert ? (smallPart == '&nbsp;' ? bigW : geByClass1('counter_anim_small_c', animEl, 'span')
            .offsetWidth) : 0;
...
TopSearch = {
    ...
    toggleTsWrap: function(s) {
        var tsWrap = ge('ts_wrap'),
            isVis = isVisible(tsWrap);
    ...
    searchCache: function(listName, q) {
        ...
        for (i in queries) {
            query = queries[i];
            if (cached = this.cache[listName][' ' + query.charAt(0)
                    .toLowerCase()
                ]) {
    ...
    row: function(mid, href, photo, name, online, re, hintType, info) {
        ...
        return '<a href="' + href + '" class="ts_contact ' + (online ? (mobPlatforms[online] ? 'ts_contact_mobile' : 'ts_contact_online') : '') +
            ...
            hintType + '"><span class="ts_contact_photo fl_l"><img src="' + photo + '"/></span><span class="ts_contact_name fl_l">' + name +
            '<div class="ts_contact_info">' + info + '</div></span><div class="ts_contact_status"></div></a>';
    },
    ...
    prepareRows: function(q) {
        ...
        each(searchLists, function(i, v) {
            ...
            var results = TopSearch.listSearch(i, q, v, excludeIds),
                items = [],
                row;
        ...
        if (q) {
            ...
            html.unshift('<a href="/search?c[section]=auto&c[q]=' + encodeURIComponent(q) +
                ...
                langKey.replace('{query}', '<span class="ts_query">' + clean(show_q) + '</span>') + '</span><div class="ts_contact_status "></div></a>');
        }
    ...
    hintsSearch: function(q, re, limit, noSep) {
        ...
        ajax.post('al_search.php', {
            ...
            onDone: function(pages) {
                ...
                var rows = [],
                    hasSep = ge('ts_search_sep') ? true : false;
...
(function() {
    ...
    AdsLight.initUserHandlers = function() {
        ...
        function onActiveTab(eventStub) {
            if (isVkDomain && window.vkNow && window.vk && vk.ads_rotate_interval && !afterClickLink && vk__adsLight.activeTab < 0 && vkNow() + vk__adsLight.activeTab >=
                15000 && isTimeToUpdate()) {
                clearTimeout(updateTimer);
                updateTimer = setTimeout(function() {
                    if (isTimeToUpdate()) {
                        __adsLoaded = 0;
    ...
    AdsLight.onAdsShowed = function(adsShowedAll) {
        ...
        // Add known showed ads to message
        {
            ...
            for (var windowId in vk__adsLight.adsShowedAll) {
                if ((!adsShowedAll[windowId] || vk__adsLight.adsShowedAll[windowId].publish_time > adsShowedAll[windowId].publish_time) && nowTime - vk__adsLight.adsShowedAll[
                        windowId].publish_time < 10000) {
    ...
    AdsLight.setNewBlock = function(adsHtml, adsSection, adsCanShow, adsShowed, adsParams) {
        ...
        if (adsHtml && (adsHtml.slice(0, adsExperimentMarker.length) === adsExperimentMarker)) {
            if (Math.random() < 0.01) {
                ajax.post('/wkview.php?act=mlet&mt=9004', {}, {
                    onFail: function() {
                        return true;
                    }
                });
    ...
    AdsLight.updateExternalStats = function(containerElem) {
        ...
        for (var i = 0, elem; elem = elems[i]; i++) {
            ...
            vkImage()
                .src = elem.getAttribute('external_stats_src');
        }
    }
        ...
        function check() {
            for (var elemId in adsIdsApplyNeeded) {
                ...
                if (elemRect.bottom > 0 && elemRect.top < lastWindowHeight) {
                    ...
                    var isElemVisible = (pointElem1 && (pointElem1 === elem || isAncestor(pointElem1, elem)) || pointElem2 && (pointElem2 === elem || isAncestor(pointElem2,
                        elem)));
                    if (isElemVisible) {
    ...
    AdsLight.restoreVisibility = function(elem, tempStyles) {
        ...
        var cssStyles = {
            display: function(element) {
                ...
                if (
                    /^(:?textarea|acronym|strong|select|object|button|small|label|input|time|span|samp|code|cite|abbr|var|sup|sub|map|kbd|img|dfn|big|bdo|tt|em|br|q|i|b|a)$/i
        ...
        function getChildrenClasses(elem) {
            ...
            for (var i = 0; i < elem.childNodes.length; i++) {
                if (elem instanceof Element) {
                    ...
                    if (child.className) {
                        ownClasses = child.className.split(' ')
