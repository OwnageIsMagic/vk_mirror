...
! function(t) {
    ...
    var e = t.HTMLCanvasElement && t.HTMLCanvasElement.prototype,
        ...
        n = o && t.Uint8Array && function() {
            try {
                return 100 === new Blob([new Uint8Array(100)])
                    .size
            } catch (t) {
                return !1
            }
        ...
        i = (o || r) && t.atob && t.ArrayBuffer && t.Uint8Array && function(t) {
            ...
            for (i = e[2] ? e[1] : "text/plain" + (e[3] || ";charset=US-ASCII"), l = !!e[4], u = t.slice(e[0].length), b = l ? atob(u) : decodeURIComponent(u), c = new ArrayBuffer(
                    b.length), d = new Uint8Array(c), B = 0; B < b.length; B += 1) d[B] = b.charCodeAt(B);
...
function rs(html, repl) {
    each(repl, function(k, v) {
        html = html.replace(new RegExp('%' + k + '%', 'g'), (typeof v === 'undefined' ? '' : v)
            .toString()
            .replace(/\$/g, '&#036;')); // fix in case input contains '$' which will be interpreted as control symbold
    });
    return html;
}
...
function domOverflows(overflowEl, el) {
    ...
    var fixEl = domClosestPositioned(el, {
            positions: ['fixed'],
            fromEl: el
        }),
...
function placeholderInit(id, opts) {
    ...
    var ph;
    var o = opts ? clone(opts) : {};
    var custom = typeof(ce("input")
        .placeholder) === 'undefined' || el && el.getAttribute && el.getAttribute('contenteditable');
    if (!el || (el.phevents && !o.reload) || !(ph = (el.getAttribute ? el.getAttribute('placeholder') : el.placeholder))) {
        return;
    }
    el.getValue = function() {
...
function showAudioClaimWarning(owner_id, id, claim_id, title, reason) {
    ...
    claimText = claimText.split('{delete_link}')
        .join('<a onclick="deleteAudioOnClaim(' + owner_id + ',' + id + '); return false;">' + getLang('audio_claim_delete') + '</a>');
...
function deleteAudioOnClaim(owner_id, id) {
    ...
    AudioPage(audioEl)
        .deleteAudio(audioEl, audioId, event, true);
...
function moneyTransferBox(txId, hash, ev, btn, decline, force) {
    ...
    if (decline) {
        if (!force) {
            ...
            cur.confirmBox = showFastBox(getLang('global_action_confirmation'), confirmText, confirmBtn, moneyTransferBox.pbind(txId, hash, ev, btn, decline, 1), getLang(
                'global_cancel'));
            return;
        }
...
TopSearch = {
    ...
    row: function(mid, href, photo, name, online, re, hintType, info, verified) {
        ...
        if (!info) info = '';
        verified = verified ? '<div class="page_verified' + ((mid == -128932034) ? ' ph_verified' : '') + '" onmouseover="pageVerifiedTip(this, {' + (mid > 0 ? ('mid:' +
            mid) : ('gid:' + Math.abs(mid))) + '})"></div>' : '';
        return '<a href="' + href + '" class="ts_contact clear_fix" id="ts_contact' + mid + '" onclick="return TopSearch.select(this, event, ' + peer +
            ...
            hintType + '"><span class="ts_contact_photo ' + onlinePlatformClass(online) + '"><img class="ts_contact_img" src="' + photo +
    ...
    htmlRows: function(q) {
        ...
        if (q) {
            ...
            html += '<a href="/search?c[section]=' + section + '&c[q]=' + encodeURIComponent(q) +
                '" class="ts_search_link clear_fix active" id="ts_search_link" onclick="return TopSearch.select(this, event);" onmousedown="event.cancelBubble = true;" onmouseover="TopSearch.itemOver(this, 1, event);"  onmouseout="TopSearch.itemOver(this, 0, event);" ><span class="ts_contact_name fl_l">' +
                label + '</span><div class="ts_contact_status "></div></a>';
        }
    ...
    hintsSearch: function(q, re) {
        ...
        ajax.post('al_search.php', {
            ...
            onDone: function(pages) {
                ...
                var limit = _t.maxItems - geByClass('ts_contact', tsWrap)
                    .length - 1,
                    groupedPages = {};
...
function toggleAudioLyrics(event, ref, audioId, lyricsId) {
    ...
    if (lyricsId) {
        ...
        if (cur.audioPage) {
            isTop = cur.audioPage.getCurrentPlaylist()
                .getType() == AudioPlaylist.TYPE_POPULAR;
            topGenre = cur.audioPage.getCurrentPlaylist()
                .getAlbumId();
        }
...
function toggleOnline(obj, platform) {
    ...
    var onlineCls = onlinePlatformClass(platform)
        .split(' ');
...
function setWorkerTimeout(cb, delay) {
    if (window.Worker && window.Blob) {
      ...
      onmessage = function(e) { \
        clearTimeout(timeout); \
        if (e.data == 'start') { \
          timeout = setTimeout(function() { postMessage({}); }, " +
            delay + "); \
        } \
      } \
    "
