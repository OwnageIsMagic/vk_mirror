FAQ = {
    ...
    appendExtraField: function(btn) {
        var list = ge('faq_optional_extra_fields_list'),
            origin = ge('faq_optional_extra_field_example');
        if (!list) {
            return;
        }
        var block = origin.cloneNode(true),
            ind = 0,
            idPrefix = 'faq_optional_extra_field_';
    ...
    prepareExtraField: function(block, typeInp, requiredInp, titleInp, noteInp) {
        ...
        var chb = new Checkbox(requiredInp, {
            label: requiredInp.title,
            width: 120
        });
    ...
    saveFAQ: function(hash) {
        ...
        var language = cur.langsDD && cur.langsDD.val() || 0,
            query = {
                ...
                imgs: imgs,
                faq_id: cur.id,
                fixed: cur.fixFAQ.val(),
                urgent: cur.urgentFAQ.val(),
        ...
        if (ge('faq_optional_extra_field_add') && (!cur.sectionSelector || cur.sectionSelector.val() == 0 || cur.sectionSelector.val() == 39)) {
            ...
            for (var i = 0; i < blocks.length; i++) {
                var b = blocks[i];
                d['ef_' + i + '_type'] = data(b, 'typeSelector')
                    .val();

                d['ef_' + i + '_title'] = geByClass1('faq_optional_extra_field__title', b)
                    .value;
                d['ef_' + i + '_note'] = geByClass1('faq_optional_extra_field__note', b)
                    .value;
                d['ef_' + i + '_required'] = data(b, 'requiredCheckbox')
                    .val();
            }
        ...
        if (cur.faqFromAllCheckbox && cur.faqFromCheckboxes) {
            var from_list = [],
                all_val = cur.faqFromAllCheckbox.val();
            if (all_val) {
...
<div class="fis_box">\
  <div class="info_msg fis_about">' + getLang('support_screen_you_can') +
            '</div>\
  <div id="fis_add_data"></div>\
  <div class="fis_warn_text">' + getLang('support_screen_warn') +
            '</div>\
  <div id="fis_dropbox" class="dropbox">\
    <div class="dropbox_wrap">\
      <div class="dropbox_area">' + getLang('drop_files_here') +
            '</div>\
    </div>\
  </div>\
</div>\
  ');
    ...
    choose: function(ind, forEdit, media, data) {
        ...
        vkImage()
            .src = data.thumb_s;
        preview = '<div onclick="return showPhoto(\'' + media + '\', \'' + data.list + '\', ' + data.view_opts.replace(/"/g, '&quot;') +
            ');" class="fl_l fis_preview"><img class="fis_photo" src="' + data.thumb_s + '" /></div>';
        var mediaEl = ce('div', {
                innerHTML: '<div id="fis_preview' + ind + '" class="fis_preview_wrap">' + preview + '<div class="fis_x fl_l" ' + (browser.msie ? 'title' : 'tooltip') +
                    '="' + getLang('dont_attach') +
                    '" onmouseover="if (browser.msie) return; showTooltip(this, {text: this.getAttribute(\'tooltip\'), shift: [6, 3, 3]})" onclick="FAQ.unchoose(\'' +
                    ind + '\'' + (forEdit ? ', 1' : '') + ')"></div></div>'
            })
            .firstChild;
    ...
    chooseUploaded: function(info, params) {
        ...
        ajax.post('al_photos.php', extend({
            ...
            progress: (Upload.types[i] == 'form' && curBox()) ? curBox()
                .progress : null
        });
    },
...
<div class="fis_progress_wrap">\
  <div id="upload' + ind + '_progress" class="fis_progress" style="width: ' + percent +
                '%;"></div>\
</div></div>';
            var progressEl = ce('div', {
                id: 'upload' + ind + '_progress_wrap',
                innerHTML: '<div class="fl_l">' + progress + '</div>' + (label ? '<div class="fis_label fl_l">' + label + '</div>' : '') +
        ...
        Upload.init('fis_add_data', cur.uploadData.url, cur.uploadData.vars, {
            ...
            onUploadStart: function(info, res) {
                ...
                if (Upload.types[i] == 'form') {
                    if (curBox()) show(curBox()
                        .progress);
                    geByClass1('file', ge('fis_add_data'))
                        .disabled = true;
                }
                if (Upload.types[i] == 'fileApi') {
                    if (cur.notStarted) {
    ...
    saveTilesTop: function(button, language, hash) {
        ...
        ajax.post(nav.objLoc[0], query, {
            showProgress: function() {
                addClass(button, 'flat_btn_lock');
            },
            hideProgress: function() {
                removeClass(button, 'flat_btn_lock');
            }
        });
    },
    ...
    tilesShowSearch: function(evt, categoryId) {
        ...
        show(searchInput);
        geByClass1('selector_input', searchInput)
            .focus();
        return false;
    },
    ...
    tilesQuestionRemove: function(questionId, event) {
        var question = ge('faq_tiles_editor_tile_question' + questionId),
            questions = question.parentNode,
            categoryId = questions.id.replace('faq_tiles_editor_tile__questions', '');
        ...
        if (geByClass('faq_tiles_editor_tile_question', questions)
            .length < cur.perCategoryLimit) {
    ...
    tilesQuestionAdd: function(categoryId, questionId, questionTitle) {
        ...
        question.innerHTML =
            '<span class="faq_tiles_editor_tile_question__title">' + questionTitle +
            '</span>\
