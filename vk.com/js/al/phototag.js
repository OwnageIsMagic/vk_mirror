...
<div class="list_wrap"><a href="/" onclick="return Phototag.addTag(' +
                    vk.id + ', event)">' + getLang('photos_tags_me') +
                    '</a>\
  ...
  <div class="box_controls">\
    ...
    <div id="pv_tag_progress" class="progress"></div>\
      ...
      '
            ...
            ge('pv_add_tag')
                .onclick = Phototag.addTag;
            ge('pv_cancel_tag')
                .onclick = Phototag.stopTag;
        }
    ...
    updateFriends: function() {
        ...
        var q = trim(cur.pvFriendName.value)
            .toLowerCase()
            .replace(/[�]/, '�'),
            frs = [];
    ...
    cacheFriends: function(q) {
        if (q) {
            ...
            for (var i in queries) {
                query = queries[i];
                var searchIn = cur.pvFriendsCache[query.substr(0, 1)
                    .toLowerCase()];
                if (searchIn) {
    ...
    showFriends: function() {
        ...
        var h = getSize(cur.pvFriends)[1],
            y = xy[1] + layerWrap.scrollTop + r[1] - ((layerWrap.offsetParent || {})
