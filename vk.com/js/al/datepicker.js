(function() {
    ...
    window.Calendar = function(params) {
        ...
        var mode = (modes[params.mode] || params.mode || 'd')
            .toString()
            .replace(/(this|next|prev)/, '');
            ...
            switch (mode) {
                case 'm':
                    ...
                    for (var i = 1; i <= 12; i++) {
                        ...
                        t.push('<td class="' + clDay + leftStyle + '" style="width:50%" id="day' + i + '_' + rnd + '" onclick="return cals.getDay(' + guid + ', 1, ' +
                            i + ', ' + y + ');" onmouseover="addClass(this, \'hover\')"  onmouseout="removeClass(this, \'hover\')">' + mn[i - 1] + '</td>');
                    }
                    t.push('</tr>');
                ...
                default:
                    ...
                    var monthYear = monthFormat.replace('{month}', mn[m - 1])
                        .replace('{year}', y);
                    ...
                    var headerNormal = '' +
                        ...
                        '<td align="center" class="month"><a class="cal_month_sel" onclick="return cals.getMonth(' + guid + ',' + m + ',' + y + ',1);">' + monthYear +
                        '</a></td>' +
                    ...
                    for (var i = 1; i <= 42; i++) {
                        ...
                        if (x > 0) {
                            dayPos[i] = x1;
                            t.push('<td id="day' + x + '_' + rnd + '" class="' + clDay + '" onclick="return cals.getDay(' + guid + ', ' + x1 + ', ' + m + ', ' + y +
                                ');" onmouseover="addClass(' + hoverEl + ', \'hover\')"  onmouseout="removeClass(' + hoverEl + ', \'hover\')">' + x + '</td>');
                        } else {
                            if (i != 36) {
                                if (!dontDoLine) {
                    ...
                    headerNormal = '' +
                        ...
                        '<td align="center" class="month"><a class="cal_month_sel" onclick="return cals.getMonth(' + guid + ',' + m + ',' + y + ');">' + y +
                        '</a></td>' +
                    ...
                    for (var i = 1; i <= 12; i++) {
                        ...
                        t.push('<td class="' + clDay + leftStyle + '" style="width:50%" id="day' + i + '_' + rnd + '" onclick="return cals.getMonth(' + guid + ', ' + i +
                            ', ' + y + ');" onmouseover="addClass(this, \'hover\')"  onmouseout="removeClass(this, \'hover\')">' + mn[i - 1] + '</td>');
                    }
                    t.push('</tr>' + addRowsM);
    ...
    window.Datepicker = function(el, options) {
        ...
        var updateDate = function(date, mode, init) {
            ...
            if (mode === 'h') {
                ...
                if (mode === 'm') {
                    ge(inputId)
                        .value = monthFormat.replace('{month}', winToUtf(l.mn[date.m - 1]))
                        .replace('{year}', date.y);
                } else {
                    ge(inputId)
                        .value = dateFormat.replace('{day}', date.d)
                        .replace('{month}', winToUtf(l.mnOf[date.m - 1]))
                        .replace('{year}', date.y);
                }
            }
            _t.hide();
            if (fmt === 'plain') {
                ge(id)
                    .value = date.d + '.' + date.m + '.' + date.y + (options.time ? (' ' + hour + ':' + min) : '');
            } else if (fmt === 'ts') {
                ge(id)
                    .value = Math.floor(new Date(date.y, date.m - 1, date.d, hour, min)
                        .getTime() / 1000) - ((new Date())
                        .getTimezoneOffset() * 60 + intval(vk.tz)) - intval(vk.dt);
            }
        ...
        if (options.day || options.month || options.year) {
            ...
            var ts = parseInt(el.value) + ((new Date())
                .getTimezoneOffset() * 60 + intval(vk.tz)) + intval(vk.dt);
            d = new Date(ts * 1000);
        } else {
            d = new Date();
        ...
        var html = '<input type="hidden" name="' + n + '" id="' + id + '"/>' + '<div class="datepicker_control"><input readonly="1" style="width:' + (w - 25) +
            'px;" type="text" class="datepicker_text" id="' + inputId + '"/></div>' + '<div id="' + calBox + '" class="cal_box"><iframe id="' + calFrame +
            '" class="cal_frame"></iframe><div id="' + calDiv + '" class="cal_div"></div></div>';
        var wrap = ce('div', {
    ...
    window.Timepicker = function(el, options) {
        ...
        var html = '<input type="hidden" name="' + n + '" id="' + id + '" value="' + v + '"/>' + o.format.replace('{hour}', '<div class="fl_l"><input type="hidden" id="' +
                id + '_hour_input" value="' + h + '"/></div>')
    ...
    window.Daypicker = function(el, options) {
        ...
        var html = '<div class="fl_l"><input type="hidden" name="' + n + '" id="' + id + '" value="' + v + '"/>' + o.format.replace('{year}',
                '<div class="fl_l"><input type="hidden" id="' + id + '_year_input" value="' + o.year + '"/></div>')
            .replace('{month}', '<div class="fl_l"><input type="hidden" id="' + id + '_month_input" value="' + o.month + '"/></div>')
            .replace('{day}', '<div class="fl_l"><input type="hidden" id="' + id + '_day_input" value="' + o.day + '"/></div>') + '</div>';
        var wrap = ce('div', {
        ...
        var daysInMonth = function(month, year) {
            var days = (new Date(year ? year : 2004, month, 0))
                .getDate(),
                res = [
