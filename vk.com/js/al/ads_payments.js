...
AdsPayments.checkBudgetSumInc = function(isRequest) {
    ...
    var strBudgetInc;
    if (window.curLegal) {
        strBudgetInc = ge('union_invoice_sum')
            .value;
    } else {
        strBudgetInc = ge('union_budget_sum')
            .value;
    }
    ...
    if (window.curIsMoney && (window.curOfficeLegal || window.userMoney > cur.unionsLimits.budget_increase_rubles_min) && !window.curLegal && window.userMoneyStr &&
...
AdsPayments.onBudgetRublesChanged = function(delayed) {
    ...
    if (!window.curLegal) {
        ...
        ge('union_invoice_sum_currency')
            .innerHTML = getLang(window.langMoneyAmountText, ge('union_invoice_sum')
                .value)
        if (window.curInvoiceFeeFract) {
            var budgetSum = parseFloat(ge('union_invoice_sum')
                .value);
            var feeSum = (isNaN(budgetSum) ? 0 : (budgetSum / (1 - window.curInvoiceFeeFract)))
                .toFixed(2);
            ge('fee_money_sum_full')
                .innerHTML = getLang('ads_payments_legal_sum_with_fee', feeSum);;
        }
    }
}
            ...
            if (response.error) {
                ...
                if (window.curLegal || window.curPaymentPage) {
                    if (window.location.toString()
                        .indexOf('budget') != -1 && !window.curPaymentPage) {
                        nav.reload();
                    } else {
    ...
    function onComplete(response) {
        ...
        if (isOk && response) {
            if (response.company) {
                ...
                message = message.replace('{link}', '<a href="#" onclick="AdsPayments.reqsApplyFreeCompany(); return false;">')
                    .replace('{/link}', '</a>')
...
AdsPayments.reqsApplyFreeCompany = function() {
    ...
    if ('contract_day' in cur.freeCompany) {
        ...
        if (cur.freeCompany.contract_day) {
            AdsPayments.reqsReinitContractDay(Math.round(cur.freeCompany.contract_day / 10000), Math.round(cur.freeCompany.contract_day / 100) % 100, Math.round(cur.freeCompany
                .contract_day) % 100);
        } else {
    ...
    var langKey = (cur.params.is_ads_posts ? 'ads_error_reqs_inn_will_fix_posts' : (cur.params.is_ads_offers ? 'ads_error_reqs_inn_will_fix_offers' :
