(function(self, w) {
    ...
    self.searchFunc = function(inputStr, array, key, showCount, caseSensitive, lat, startWords) {
        ...
        var defaultEqual = function(regExp) {
            replaceStr = keyStr.replace(/�/gi, '�')
                .replace(regExp, rChars);
            return (keyStr != replaceStr);
        }
        var startWordsEqual = function(regExp) {
            keyStr = keyStr.replace(/�/gi, '�')
                .split(/\s/);
    ...
    function keyDown(e) {
        ...
        if (e.keyCode == KEY.TAB) {
            ...
            elem.value = trim(geByClass('s_title', (qsCurrItem) ? qsCurrItem : subMenuCont)[0].innerHTML.replace(/<\/?span>/g, '')
                .replace(/<.*\/.*>/g, ''));
            return false;
        }
