var GroupsEdit = {
    ...
    uStart: function() {
        var tab = cur.tab,
            ...
            nd = d,
            rnd = cur.rnd,
            ph = getLang('groups_users_search');
        cur.qShown = false;
        if (cur.opts.counts[tab] > 1000 || d == 'lazy') {
            ...
            if (!d) {
                cur.offsets[tab] = ge('gedit_users_rows_' + tab)
                    .childNodes.length;
                nd = 'notavail';
    ...
    uScroll: function() {
        ...
        var st = scrollGetY(),
            lnk = ge('gedit_users_more_' + cur.tab);
    ...
    uSearch: function() {
        ...
        if (tab == 'members' || tab == 'unsure' || tab == 'admins') {
            return nav.go('/search?c[section]=people&gid=' + cur.opts.id + '&c[q]=' + encodeURIComponent(q) + '&from=' + tab, false, {
                noback: false
            });
        }
    ...
    uGetAddr: function(lnk) {
        var m = lnk.match(/^(https?:\/\/)?([a-z0-9]+\.)*(vkontakte\.ru|vk\.com)\/(.+)$/),
            result = m[4].substr(m[4].indexOf('#') + 1)
            .replace(/^[\/\!]*/, '');
    ...
    uSearchUpdate: function(force) {
        if ((cur.searchInp || {})
            .id != 'gedit_users_search_inp') return;
            ...
            if (q) {
                ...
                domPN(cont)
                    .replaceChild(ce('div', {
                        ...
                        innerHTML: GroupsEdit.uGenEmpty(getLang((tab == 'members' || tab == 'unsure' || tab == 'admins' ? 'groups_too_many_enter' :
                ...
                if (cur.htmls[tab]) {
                    ...
                    domPN(el)
                        .replaceChild(cur.htmls[tab], el);
                    debugLog(domPN(ge(domFC(el)
                        .id)));
                    if (domFC(el)
                        .className == 'gedit_user' && domPN(ge(domFC(el)
                            .id)) == cur.htmls[tab]) {
                        cur.htmls[tab].replaceChild(domFC(el), ge(domFC(el)
                            .id));
                    }
    ...
    uUpdateSummary: function() {
        var tab = cur.tab,
            add = (tab == 'requests') ? '<span class="divide">|</span><span><a onclick="GroupsEdit.uApproveAll()">' + getLang('groups_approve_all') + '</a></span>' : '';
    ...
    uShowMore: function(force) {
        ...
        if (q) {
            if (q.match(/^(https?:\/\/)?([a-z0-9]+\.)*(vkontakte\.ru|vk\.com)\/.+/)) {
                ...
                if (lst === undefined) {
                    var tmp = cur.index[tab].search(q),
                        mp = {};
                    lst = [];
        ...
        var start = force ? 0 : cont.childNodes.length,
    ...
    uGenRow: function(tab, row, name) {
        ...
        if (infoArr[1]) { // online
            info += '<div class="gedit_user_online">' + (getLang('global_online', sex) + (infoArr[1] > 0 && infoArr[1] < 6 ?
                '<b class="mob_onl gedit_mob_onl" onmouseover="mobileOnlineTip(this, {mid: ' + oid + '})" onclick="mobilePromo()"></b>' : '')) + '</div>';
        }
        ...
        switch (tab) {
            case 'requests':
                if (level == -3) {
                    ...
                    btns = [
                        ...
                        '<div class="button" onclick="GroupsEdit.uAction(this, ', oid, ', \'', hash, '\', -1)">', getLang('groups_members_application_decline'),
                        '</div>',
                        '</div>',
                        '</div>'
                    ].join('');
            ...
            case 'admins':
                ...
                if (level < 0) {
                    actions += '<a class="gedit_user_action" onclick="GroupsEdit.uAction(this, ' + oid + ', \'' + hash + '\', 0)">' + getLang('groups_restore_member') +
                        '</a>';
                } else if (level < 3) {
                    actions += '<a class="gedit_user_action" onclick="GroupsEdit.uAction(this, ' + oid + ', \'' + hash + '\', -1)">' + getLang('groups_members_delete') +
                        '</a>';
                }
                break;
            ...
            case 'invites':
                if (level < 0) {
                    actions += '<a class="gedit_user_action" onclick="GroupsEdit.uAction(this, ' + oid + ', \'' + hash + '\', 0)">' + getLang('groups_send_invitation') +
                        '</a>';
                } else {
                    actions += '<a class="gedit_user_action" onclick="GroupsEdit.uAction(this, ' + oid + ', \'' + hash + '\', -1)">' + getLang(
                        'groups_members_invitations_cancel') + '</a>';
                }
                break;
        }
    ...
    bigphOver: function(obj, uid) {
        ...
        if (o.tagName != 'A' || o.className != 'gedit_bigph') {
            o = obj.insertBefore(ce('a', {
                className: 'gedit_bigph',
                href: ch && ch._id ? ('/photo' + ch._id + '?all=1') : ('/albums' + uid),
                innerHTML: '<span class="gedit_bigph_label">' + getLang('global_photo_full_size') + '</span>'
            }), obj.firstChild);
        }
    ...
    bigphClick: function(uid, ev) {
        ...
        if (!ch) return;
        return showPhoto(ch._id, 'album' + uid + '_0/rev', extend({
            jumpTo: {
                z: 'albums' + uid
            }
        }, ch), ev);
    ...
    uDoneAdmin: function(mid, hash) {
        ...
        ajax.post('groupsedit.php', {
            ...
            onDone: function(msg, row) {
                var tabs = ['members', 'unsure', 'admins'],
                    i;
                ...
                for (i = 0; i < 3; ++i) {
                    ...
                    if (t == 'admins' && !found && level) {
                        ...
                        if (el = ge('gedit_user_' + t + mid)) {
                            domPN(el)
                                .replaceChild(se(GroupsEdit.uGenRow(t, row)), el);
                        }
                        if (!found) {
                            ajaxCache = {};
                        }
    ...
    uInitAdmin: function(box, hosts, extcur, lng) {
        ...
        box.removeButtons()
            .addButton(getLang('global_cancel'), box.hide, 'no');
            ...
            onDone: function(row, admrows, msg) {
                if (cur.hostFrom == 'page') {
                    curBox()
                        .hide();
                    showDoneBox(msg);
                    domPN(ge('event_admin'))
                        .replaceChild(se(row), ge('event_admin'));
                    return;
                }
                var t = 'admins';
                ...
                if (row[0] > 0 && last[0] < 0) {
                    ...
                    if (el = ge('gedit_user_' + t + last[0])) {
                        domPN(el)
                            .appendChild(se(GroupsEdit.uGenRow(t, row)), el);
                    }
                } else {
                ...
                if (cur.tab != 'admins') nav.go(extend(nav.objLoc, {
                    tab: 'admins'
                }));
            },
            showProgress: curBox()
    ...
    uAction: function(el, mid, hash, act) {
        if (!curBox() && (buttonLocked(el) || (domFC(el) || {})
                .className == 'progress_inline')) return;
        var tab = cur.tab;
        ajax.post('groupsedit.php', {
            act: 'user_action',
            id: cur.opts.id,
            ...
            onDone: function(row) {
                if (curBox()) curBox()
                    .hide();
                if (isArray(row)) {
                    var tabs = (tab == 'requests' || tab == 'declined' || tab == 'invites') ? [tab] : ['members', 'unsure', 'admins'],
                        cnt = tabs.length,
                        i;
                    for (i = 0; i < cnt; ++i) {
                        var t = tabs[i],
                    ...
                    GroupsEdit.uRemoveAdmin(mid)
                        .uRemove = [mid, hash, act];
                }
            },
        ...
        ajax.post('al_groups.php', {
            ...
            onDone: function(url) {
                if (url) {
                    ...
                    ge('group_status_export_data')
                        .innerHTML = getLang('groups_authorize_please');
...
<center id="group_status_export_data">' + getLang(
                'groups_external_site_request') + '</center>\
    ');
        cur.twitterBox.setOptions({
            onHide: clearTimeout.bind(window)
                .pbind(cur.twitterTimer)
        });
        cur.twitterBox.removeButtons()
            .addButton(getLang('global_cancel'), cur.twitterBox.hide, 'no');
    ...
    removeTwitter: function() {
        var box = showFastBox({
            ...
            dark: 1,
            bodyStyle: 'padding: 20px; line-height: 160%;'
        }, getLang('groups_status_confirm'), getLang('global_continue'), function() {
            ajax.post('al_groups.php', {
                act: 'clear_twitter',
                gid: cur.gid,
                hash: cur.hash
    ...
    saveInfo: function(state) {
        var name = trim(ge('group_edit_name')
                .value),
            addr = trim(ge('group_edit_addr')
                .value);
        if (!state) state = 0;
        ...
        if (cur.cls == 0 || cur.cls == 2) {
            ...
            each(['enable_replies', 'enable_topics', 'enable_photos', 'enable_video', 'enable_audio', 'enable_links', 'enable_events', 'enable_places', 'enable_contacts'],
                function(i, v) {
        ...
        ajax.post('al_groups.php', params, {
            onDone: function(result, oldaddr) {
                ...
                if (oldaddr != addr) {
                    each(geByTag('a'), function() {
                        this.href = this.href.replace(new RegExp('/' + oldaddr + '\\?', 'g'), '/' + addr + '?')
                            .replace(new RegExp('/' + oldaddr + '$', 'g'), '/' + addr);
                    });
                    nav.setLoc({
                        0: addr,
                        act: 'edit'
                    ...
                    if (ge('group_edit_addr_print_text')) {
                        if (addr != 'club' + cur.gid && addr != 'public' + cur.gid && addr != 'event' + cur.gid) {
                            ge('group_edit_addr_print_text')
                                .innerHTML = cur.lang['groups_print_text'].replace('{link}', '<a href="/' + addr + '?act=edit&amp;w=print">')
                                .replace('{/link}', '</a>');
                        } else {
                            ge('group_edit_addr_print_text')
    ...
    init: function(selData) {
        ...
        if (cur.cls == 1) {
            extend(cur, {
                pcategoryDD: new Dropdown(ge('public_type'), selData.pcategories, {
                    ...
                    onChange: function(v) {
                        v = intval(v);
                        if (v && (selData.psubcategories[v] || {})
                            .length > 1) {
                            cur.psubcategoryDD.setOptions({
                                defaultItems: selData.psubcategories[v]
                            });
            ...
            var width = 140 + 2 * 47 + 10,
                el = geByClass1('group_edit_at', ge('group_edit_start_time'));

            width += el.offsetWidth;
            var toAdd = (266 - width),
                toAddLeft = Math.floor(toAdd / 2),
                toAddRight = toAdd - toAddLeft;
            var st = {
                paddingLeft: toAddLeft + 'px',
                paddingRight: toAddRight + 'px'
            };
            setStyle(el, st);
        ...
        cur.onPrivacyChanged = function(key, dt) {
            ...
            switch (key) {
                ...
                case 'g_messages':
                    var value = parseInt(GroupsEdit.getFields('messages')
                        .messages);
    ...
    doSearchBlacklist: function() {
        var inp = ge('group_bl_search'),
            name = trim(inp.value),
            btn = ge('group_bl_submit');
        if (!name) {
            return elfocus(inp);
        }
    ...
    updateBlacklist: function(gid, msg, mid, html, delta) {
        ...
        if (ge('group_bl_rows') && cur.gid == gid) {
            var row = ge('group_bl' + mid),
                newrow = se(html);
            if (!row && !(delta > 0)) return;

            if (row) {
                domPN(row)
                    .replaceChild(newrow, row);
            } else if (delta > 0) {
    ...
    updateBlacklistSummary: function(delta) {
        ...
        if (cur.count > 0) {
            ge('group_bl_summary')
                .innerHTML = getLang('groups_X_banned_members', cur.count);
        } else {
            ge('group_bl_summary')
                .innerHTML = getLang('group_no_banned_users');
        }
    },
    ...
    load: function() {
        ...
        ajax.post(nav.objLoc[0], extend({
            ...
            showProgress: function() {
                ...
                hide(ge('group_users_more')
                    .firstChild);
            },
            hideProgress: function() {
                show(ge('group_users_more')
                    .firstChild);
    ...
    initLinks: function() {
        placeholderSetup('group_l_search', {
            back: true
        });
    ...
    linkAction: function(el, act, id) {
        ...
        if (!del) { // say after what link is that one
            var after = ge('group_l_row' + id)
                .previousSibling
            for (; after && (!after.id || isVisible('group_l_restore' + after.id.replace('group_l_row', '')));) {
                after = after.previousSibling;
            }
            params.after = (after && after.id || '')
                .replace('group_l_row', '');
        }
        ajax.post('al_groups.php', params, {
            onDone: function() {
    ...
    parseLink: function(url, cuteurl) {
        ...
        re(cur.lnkParse);
        cur.lnkParse = cont.insertBefore(ce('div', {
            innerHTML: '\
<iframe class="upload_frame" name="link_parse_iframe"></iframe>\
   '
        }), cont.firstChild);

        var parseForm = cur.lnkParse.appendChild(ce('form', {
            action: cur.parseUrl,
            method: 'post',
...
<div class="group_al_thumb select_fix fl_l" onclick="GroupsEdit.rotateImgs()">\
  <img src="' + img +
            '" id="group_al_thumb_img" />\
</div>\
<div class="group_al_info fl_l">\
  <input type="text" class="text" id="group_al_title" value="' + title +
            '" onkeypress="if (event.keyCode == 10 || event.keyCode == 13) GroupsEdit.doAddLink()" placeholder="' + getLang('group_link_add_title') +
            '" />\
  <div class="group_al_position">' + GroupsEdit.getDomain(url) + '</div>\
</div>\
<br class="clear" />', getLang('global_add'), GroupsEdit.doAddLink,
            getLang('global_cancel'));
    ...
    addLink: function() {
        var lnk = trim(ge('group_l_search')
                .value)
            .replace(/\s/g, '+');
        if (!lnk) {
            ...
            onDone: function(code, innerlnk, html, js) {
                if (code > 0) {
                    ...
                    cur.addBox = showFastBox({
                        ...
                        dark: 1,
                        bodyStyle: 'padding: 20px; line-height: 160%;'
                    }, html, getLang('global_add'), GroupsEdit.doAddLink, getLang('global_cancel'));
                    cur.addBox.evalBox(js);
                }
            },
            showProgress: lockButton.pbind(ge('group_l_submit')),
            hideProgress: unlockButton.pbind(ge('group_l_submit'))
        });
    },
    uploadImg: function() {
        show(cur.addBox.progress);
        var cont = ge('group_l_bar');
        var uploadCont = cont.appendChild(ce('div', {
    ...
    linksCount: function(count) {
        cur.count = count;
        ge('group_l_summary')
            .innerHTML = count ? getLang('links_count', count) : getLang('links_no_count');
    ...
    doAddLink: function(force) {
        ...
        var str = (title ? title : ge('group_al_position'))
            .value;
        var params = {
                lnk: cur.lnk,
                index: cur.lnkIndex,
        ...
        if (cur.editing) {
            ...
            callback = function(lid, thumb, title, position) {
                cur.addBox.hide();
                GroupsEdit.invalidateBack();
                if (thumb !== false) ge('group_l_photo' + lid)
                    .src = thumb;
                if (title !== false) ge('group_l_title' + lid)
                    .innerHTML = title;
                if (position !== false) ge('group_l_position' + lid)
                    .innerHTML = position;
            }
        } else {
            params.act = 'add_link';
        ...
        setTimeout(ajax.post.pbind('al_groups.php', params, {
            onDone: callback,
    ...
    go: function(el, ev) {
        var result = nav.go(el, ev, {
            onFail: GroupsEdit.failed.pbind(GroupsEdit.getsect())
        });
        if (result === false) {
    ...
    switchAdSubTab: function(el, wrap, link, evt, params) {
        ...
        addClass(el, 'active');
        if (params.part) {
            var obj = nav.fromStr(link),
                url = obj[0];
            delete obj[0];
            ajax.post(url, extend(obj, {
                part: 1
            }), {
                cache: 1,
    ...
    getAdPage: function(offset, wrap) {
        ...
        ajax.post(url, extend(obj, {
            ...
            onDone: function(res) {
                ge(wrap || 'group_ad_requests_table_warp')
                    .innerHTML = res;
    ...
    getAdWithdrawalPage: function(offset) {
        ajax.post('al_groups.php', {
            ...
            onDone: function(res) {
                ge('group_ad_withdrawal_table_wrap')
                    .innerHTML = res;
            }
        });
        return false;
    ...
    toggleAdOtherSettings: function() {
        if (!isVisible('group_ad_other_settings')) {
            ge('group_ad_other_settings_lnk')
                .innerHTML = getLang('groups_adspost_hide_other_settings');
            slideDown(ge("group_ad_other_settings"), 200);
            GroupsEdit.initAdOtherSettings();
        } else {
            ge('group_ad_other_settings_lnk')
                .innerHTML = getLang('groups_adspost_show_other_settings');
    ...
    showAgeLimitsInfo: function() {
        showFastBox({
            ...
            dark: 1,
            width: 500
        }, '<div class="group_edit_age_limits_box">' + getLang('groups_age_limits_description') + '</div>');
    },
    ...
    callbackShowCurlResult: function(curlId, response) {
        ...
        if (!result) return;
        showFastBox({
            title: response ? getLang('groups_api_request_result') : getLang('groups_api_request_body'),
            dark: 1,
            width: 900,
            bodyStyle: 'padding: 0;'
        }, '<div class="group_api_result"><pre class="group_api_result_pre"">' + result + '</pre></div>');
    },
