            ...
            onDone: function(keyValueHtml) {
                if (keyValueHtml && query.key) {
                    ...
                    if (rowEl) {
                        ...
                        geByClass1('_tr_key_inner', rowEl)
                            .innerHTML = keyValueHtml;
                    }
                } else {
                    nav.reload();
                }
                    ...
                    function checkLink(link) {
                        ajax.post(TR_ADDRESS, {
                            ...
                            onDone: function(userId, photo, href) {
                                ...
                                if (userId) {
                                    domPN(linkInput)
                                        .appendChild(se('<a href="' + href + '" class="tr_mem_ava ow_ava ow_ava_small" style="background-image: url(\'' +
                                            photo + '\')"></a>'));
...
window.translation = window.translation || {
    ...
    menu: function(ev, sections, admin_href, admin_name, hash) {
        ...
        if (admin_href == 'super') {
            ...
            invitation = '<a href="' + admin_href + '">' + admin_name +
                '</a> has invited you to translate this page. <a onclick="return translation.invite();">Invite friend &raquo;</a>';
        }
      ...
      <div class="translation_box">\
        <div class="button_blue flat_button" id="translation_toggle">' + inline_translation +
            '</div>\
        ' + show_all_phrases +
            '\
        ...
        ge('translation_toggle')
            .onclick = translation.toggle;
        ge('translation_to_page')
            .onclick = function() {};
        if (section_id) {
    ...
    update: function(box) {
        var nativeText = ge('native_text'),
            history = ge('historyWrap');
        var items = geByTag('textarea', ge('translation_form')),
            tokens;
                ...
                onDone: function(response) {
                    ...
                    if (result.friends && result.friends.length) {
                        stManager.add(['ui_controls.css', 'ui_controls.js'], function() {
                            translation.uiFriends = new Dropdown(ge('translation_inv_id'), result.friends, {
                                width: 150,
                                autocomplete: true,
    ...
    _updateKeysList: function(sectionId, keysHtml, untranslatedCounts, totalUntranslatedCount) {
        ...
        if (sectionTotal > 0) {
            ge("keys-summary-text")
                .innerHTML = langNumeric(sectionTotal, cur.lang.tran_untranslated_keys_summary);
        } else {
            ge("keys-summary-text")
                .innerHTML = getLang('tran_no_untranslated_keys_summary');
        }
    },
    ...
    selectSection: function(sectionId, langId, noScroll) {
        ...
        show('progress-spinner');
        var self = this;
        ajax.post('al_translation.php', extend({
            act: 'get_section_keys'
        }, query), {
            onDone: self._updateKeysList.bind(this)
        });
    ...
    t: function(ev, el, leftBtn) {
        ...
        var box = showBox('al_translation.php', {
            act: 'edit_key_dialog',
            key: langKey
        }, {
            params: {
    ...
    editKey: function(key, langId) {
        ...
        var box = showBox('al_translation.php', {
            act: 'edit_key_dialog',
            key: key,
            section_id: nav.objLoc.section_id,
            lang_id: langId ? langId : nav.objLoc.lang_id
        }, {
            params: {
                dark: true,
                bodyStyle: 'padding: 0',
    ...
    _initTextareasSMSCounter: function() {
        function _updateSMSCounter(textarea) {
            ge('sms-count-text')
                .innerHTML = 'SMS count: ' + translation._getSMSCount(val(textarea));
        }
    ...
    initEditKeyDialog: function(key, sectionId, hash, sectionsList, isDeletedKey, canSave) {
        ...
        if (isAddingNewKey) {
            ...
            if (!geByClass1('value', descriptionInfo)
                .innerHTML) {
                re(descriptionInfo);
            }
            ...
            ajax.post('al_translation.php', query, {
                onDone: function(html1, html2) {
                    if (html2 && ge('section-en-translation')) {
                        ge('section-ru-translation')
                            .innerHTML = html1;
                        ge('section-en-translation')
                            .innerHTML = html2;
                    } else {
                        ge('section-translation')
                            .innerHTML = html1;
                    }
        ...
        // Extended controls
        var disableInlineCheckbox = new Checkbox(ge('disable_inline'), {
            label: 'Disable inline translating'
        });
        var extendedWikiCheckbox = new Checkbox(ge('extended_wiki'), {
            label: 'Extended Wiki markup',
        ...
        var hasCaseCheckbox = new Checkbox(ge('has_case'), {
            label: 'Depends on case',
            onChange: function(value) {
                toggle('translation-case', !!value);
            }
        });
        var markUntranslatedCheckbox = new Checkbox(ge('mark_untranslated'), {
            label: 'Mark as untranslated'
        });
        var exportToJSCheckbox = new Checkbox(ge('export'), {
            label: 'Export to JS'
        });
        ...
        curBox()
            .removeButtons()
            .addButton(getLang('box_cancel'), function() {
                curBox()
                    .hide();
            }, "no");
        ...
        if (canSave) {
            if (isDeletedKey) {
                curBox()
                    .addButton(getLang('tran_restore_key_button_label'), function(button) {
                ...
                function _onDialogKeyPress(event) {
                    if ((event.ctrlKey || event.metaKey) && event.which == 13) {
                        ...
                        { // find next key
                            ...
                            if (rowEl) {
                                nextKey = rowEl.getAttribute('id')
                                    .slice(4); // remove first 'key_' prefix
                            }
                        }
                ...
                function _onSave(button, callback) {
                    ...
                    var validator = new RegExp(/%s\b/gi),
                        isError = false;
                    each(query, function(k, v) {
                        if (k.indexOf('Value_') == 0) {
                            ...
                            if (match && match.length > 1) {
                                ge('key-save-error')
                                    .innerHTML = 'More than one %s tag is forbidden';
                                show('key-save-error');
                                isError = true;
                                return false;
                            }
                ...
                curBox()
                    .addButton(isAddingNewKey ? getLang('tran_create_key') : getLang('box_save'), _onSave);
            }
    ...
    _initActions: function(key, sectionId, hash, sectionsList) {
        ...
        new Dropdown(ge('key_actions'), [
            [-1, getLang('tran_choose_key_action')],
            [0, getLang('tran_delete_key')],
            [1, getLang('tran_copy_key')],
            [2, getLang('tran_move_key')]
        ], {
            width: 160,
            selectedItems: '-1',
            multiselect: false,
    ...
    doCloneKey: function(curKey, hash, isMove) {
        var newKey = ge((isMove ? 'move' : 'clone') + '_key_name')
            .value;
        var self = this;
    ...
    initLangChooser: function() {
        ...
        var ddEl = ce('div', {
            innerHTML: '<div class="mdd-value">' + items[0][1] + '</div>' +
    ...
    translatorsTab: {
        ...
        onLangStatLangChoose: function(ref, langId) {
            ...
            ajax.post('al_translation.php', {
                act: 'a_lang_stat_info',
                lang_id: langId
            }, {
                onDone: function(html) {
                    geByClass1('lang_stat_info')
                        .innerHTML = html;
                }
            });
        },
        ...
        _showTranslations: function(translatorId) {
            ...
            showBox('al_translation.php', {
                act: 'show_translations_box',
                translator_id: translatorId,
                date: selectedDate
            }, {
                params: {
                ...
                (function(_editEl) {
                    var dropdown = new InlineDropdown(_editEl, {
                        ...
                        onSelect: function(action) {
                            var translatorId = dropdown._iddEl.getAttribute('id')
                                .split('id')[1];
        ...
        initTab: function(languages, parentLangs, targetLangs) {
            ...
            var translatorMemlinkInput = ge('translator_memlink'),
                phEl = geByClass1('translator_photo');;
        ...
        onAddTranslator: function(button, hash) {
            ...
            ajax.post('al_translation.php', {
                act: 'a_add_translator',
                hash: hash,
                mem_link: memLink,
                parent_lang: parentLang,
        ...
        translatorsTableFilter: function(immediate, force) {
            ...
            this.updateTranslatorsTable = this.updateTranslatorsTable || function(showCoordsFilter, textFilter, langId, selectedStatDate, force) {
                ...
                ajax.post('al_translation.php', {
                    act: 'get_translators_table',
                    show_coords_filter: +showCoordsFilter,
                    text_filter: textFilter,
                    lang_id: langId,
                    stat_date: selectedStatDate
                }, {
                    onDone: function(html) {
                        hideMask(tableEl);
    ...
    batchEditTab: {
        ...
        updatePreview: function() {
            ...
            each(geByClass('batch_edit_item_value'), function(i, el) {
                ...
                geByClass1('batch_edit_item_result', el.parentNode)
                    .innerHTML = value;
            });
        },

        saveChanged: function(button, hash) {
            var itemsCount = geByClass('batch_edit_item_result')
                .length,
                ChunkSize = 4;
        ...
        _save: function(offset, limit, callback, hash) {
            ...
            each(geByClass('batch_edit_item_result'), function(i, el) {
                ...
                var value = el.innerHTML,
                    langId = el.getAttribute('data-language-id');
                var originalValue = self.batchKeysValues[langId];
                originalValue = ce('div', {
                        innerHTML: originalValue
                    })
                    .innerHTML;
    ...
    glossaryTab: {
        initTab: function() {
            ...
            addEvent(searchInput, 'change input', function() {
                var searchStr = trim(val(searchInput))
                    .toLowerCase();;
                var regex = searchStr ? new RegExp(searchStr, 'g') : null;

                each(geByClass('item', geByClass1('glossary_items')), function(i, itemEl) {
                    if (regex) {
                        var str = geByClass1('original', itemEl)
                            .innerHTML;
                        str += geByClass1('alternative_original', itemEl)
                            .innerHTML;
                        str += geByClass1('translation', itemEl)
                            .innerHTML;
                        str += geByClass1('comments', itemEl)
                            .innerHTML;
                        str = str.toLowerCase();
        ...
        _resortItems: function() {
            ...
            itemsArr.sort(function(a, b) {
                var ta = geByClass1('original', a)
                    .innerHTML;
                var tb = geByClass1('original', b)
                    .innerHTML;
        ...
        editItem: function(ref) {
            ...
            var ruPhrase = '',
                ...
                id = '';
            if (ref) {
                ruPhrase = geByClass1('original', ref)
                    .innerHTML;
                enPhrase = geByClass1('alternative_original', ref)
                    .innerHTML;
                tranPhrase = geByClass1('translation', ref)
                    .innerHTML;
                comments = geByClass1('comments', ref)
                    .innerHTML.split('<br>')
                    .join('\n');
            ...
            var editEl = se(
                ...
                '<div class="edit_buttons"><button id="glossary_save_item" class="flat_button" onclick="translation.glossaryTab.saveItem(this, ' + id +
                ')">Save</button><button id="cancel" class="flat_button secondary" onclick="translation.glossaryTab.cancelSaveItem()">Cancel</button></div>' +
                '</div>');
        ...
        saveItem: function(button, itemId, withOpenNext) {
            ...
            var self = this;
            ajax.post('al_translation.php', {
                act: 'glossary_save_item',
                item_id: itemId,
                ru_phrase: ruPhrase,
                en_phrase: enPhrase,
                tran_phrase: tranPhrase,
                comments: comments
            }, {
                onDone: function(itemsHtml) {
                    unlockButton(button);
...
(function() {
    ...
    window.SmsCounter = SmsCounter = (function() {
        ...
        SmsCounter.count = function(text) {
            ...
            var codeCount = (text.match(/\{count\}/g) || [])
                .length;
            length += codeCount * 2;
