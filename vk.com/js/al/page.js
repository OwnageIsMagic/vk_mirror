var Page = {
        ...
        inviteToGroup: function(gid, mid, invited, hash) {
            var setInvited = function(invited) {
                ...
                geByClass('actions', row)[0].innerHTML = invited ? '<a href="" onclick="return page.inviteToGroup(' + gid + ', ' + mid + ', 1, \'' + hash + '\')">' +
                    getLang('friends_cancel_event_invite') + '</a>' : '<a href="" onclick="return page.inviteToGroup(' + gid + ', ' + mid + ', 0, \'' + hash + '\')">' +
                    getLang('friends_send_event_invite') + '</a>';
            }
            if (invited) {
                ...
                ajax.post('/al_page.php', {
                    ...
                    onDone: function(res, message) {
                        if (!res) {
                            setInvited(0);
                            ge('res' + mid)
                                .innerHTML = '<div class="res">' + message + '</div>';
                            var row = ge('member_row' + mid);
        ...
        ownerPhotoFast: function() {
            ...
            if (!inp) inp = ge('owner_photo_wrap')
        ...
        playCurrent: function(el, audioId, hash) {
            var prg = geByClass1('current_audio_prg', el.parentNode) || el.parentNode.appendChild(ce('span', {
                className: 'progress_inline current_audio_prg'
            }));
        ...
        playLive: function(audioId, hash, ajaxOpts) {
            ...
            stManager.add(['audioplayer.css', 'audioplayer.js'], ajax.post.pbind('audio', {
        ...
        audioStatusUpdate: function(hash) {
            ...
            var audioId = (_a && _a.player && _a.player.paused && !_a.player.paused()) ? aid : '',
                top = (_a && (_a.playbackParams.top_audio || _a.playbackParams.top)) ? 1 : '';
        ...
        audioListenersOver: function(el, oid) {
            var valueEl = geByClass1('value', el),
                pointerShift = 92,
            ...
            var b = showBox('al_page.php', {
                ...
                .setButtons(getLang('global_save'), function() {
                    ...
                    function onSearch() {
                        ...
                        params.hash = hash;
                        if (!hash) params.hash = ge('public_contact_hash')
                            .value;
                        if (ge('public_contact_memlink')) params.page = ge('public_contact_memlink')
                            .value;
                        params.title = ge('public_contact_position')
                            .value;
                        params.phone = ge('public_contact_phone')
                            .value;
                        params.email = ge('public_contact_email')
                            .value;
                        ...
                        ajax.post('al_page.php', params, {
                            onDone: function(res, script) {
                                ...
                                if (box) {
                                    box.content(res);
                                    if (ge('public_contacts_list') && ge('public_contacts_list')
                                        .sorter) {
                                        ge('public_contacts_list')
                                            .sorter.destroy();
                                    }
                                    if (script) {
                                        eval(script);
                                    }
                                    toggle('public_add_contact', ge('public_contacts_list')
                    ...
                    if (!mid && cur.lastContact != ge('public_contact_memlink')
                        .value) {
                        page.searchContact(oid, ge('public_contact_memlink')
                            .value, onSearch);
                    } else {
                        onSearch();
                    }
        ...
        searchContact: function(oid, page, onSearch) {
            ...
            ajax.post('al_page.php', {
                ...
                onDone: function(uid, img, name, hash) {
                    ...
                    ge('public_contact_name')
                        .innerHTML = name;
                    ge('public_contact_image')
                        .innerHTML = img;
                    ge('public_contact_hash')
                        .value = hash;
                    if (!uid) {
        ...
        reorderContacts: function(oid, hash, user, before, after) {
            ...
            var before_id = (before && before.id || '')
                .replace('public_contact_cell', '');
            var after_id = (after && after.id || '')
                .replace('public_contact_cell', '');
            cur.reloadAfterClose = true;
            ajax.post('/al_page.php', {
        ...
        infoEdit: function(audio) {
            ...
            show(ed, ge('page_current_info')
                .firstChild.nextSibling);
            hide(ge('page_current_info')
                .firstChild);
            ...
            ge('currinfo_save')
                .onclick = Page.infoCheckSave;
        },
        infoCancel: function() {
            hide('currinfo_editor', ge('page_current_info')
                .firstChild.nextSibling);
            show(ge('page_current_info')
                .firstChild);
        ...
        infoSave: function(txt) {
            ...
            ajax.post('al_page.php', {
                ...
                onDone: function() {
                    var c = txt ? 'my' : 'no',
                        t = txt ? ('<span class="current_text">' + Emoji.emojiToHTML(txt.replace(/&/g, '&amp;')
                            .replace(/</g, '&lt;')
                            .replace(/"/g, '&quot;'), true) + '</span>') : getLang('change_current_info');
                    ge('current_info')
                        .innerHTML = ge('page_current_info')
                        .firstChild.nextSibling.innerHTML = '<span class="' + c + '_current_info">' + t + '</span>';
                    Page.infoCancel();
                    var el = ge('current_info'),
                        tt = el.tt;
                    if (tt && tt.el) {
        ...
        infoCheckSave: function(e) {
            ...
            Page.infoSave(ge('currinfo_input')
                .value);
        },
        ...
        toggleFixedPost: function(postRaw, e) {
            ...
            if (cur.fixedWide) {
                ...
                cur.wallType = ge('page_wall_posts')
                    .className = 'all';
            } else {
                if (cur.topHeaderBack) {
        ...
        showGif: function(obj, ev, dontHideActive) {
            ...
            var acts = '<div class="page_gif_share" onmouseover="showTooltip(this, {text: \'' + shareTxt +
                '\', black: 1, center: 1, shift: [1, 2, 0]})" onclick="return Page.shareGif(this, \'' + doc + '\', \'' + hash +
                '\', event)"><div class="page_gif_share_icon"></div></div>';;
            if (addHash) {
                acts += '<div class="page_gif_add" onmouseover="return Page.overGifAdd(this, \'' + addTxt + '\', \'' + doc +
                    '\', event);" onclick="return Page.addGif(this, \'' + doc + '\', \'' + hash + '\', \'' + addHash +
                    '\', event);"><div class="page_gif_add_icon"></div></div>';
            }
        ...
        albumOver: function(obj, id) {
            ...
            var desc = geByClass1('page_album_description', obj),
                title = geByClass1('page_album_title_wrap', obj),
                descY = getSize(desc)[1];
            animate(title, {
                marginTop: 98 - (descY ? descY + 5 : 0)
            }, {
                duration: 200,
        ...
        addAudioPreview: function(media, data) {
            ...
            var rnd = Math.floor((1 + Math.random()) * 1000000000),
                aid = media + '_' + rnd,
                nfo = data.info;
            return '\
<div class="medadd_aud inl_bl" id="audio' + aid +
                '"><table cellspacing="0" cellpadding="0" width="100%"><tr>\
  <td rowspan="2"><div class="medadd_aud_playwrap" onclick="playAudioNew(\'' + aid +
                '\') + data">\
  ...
  <td class="info medadd_aud_td medadd_aud_wid"><div class="title_wrap fl_l" onmouseover="setTitle(this)">\
    <b class="medadd_aud_perf">' +
                data.performer + '</b> &ndash; <span class="title" id="title' + aid + '">' + data.title +
                ' </span>\
  </div><div class="duration fl_r" onmousedown="if (window.audioPlayer) audioPlayer.switchTimeFormat(\'' + aid + '\', event)">' + data.duration +
                '</div></td></tr><tr>\
  <td class="medadd_aud_td"><div class="audio_back_line medadd_aud_ph"></div><div id="player' + aid +
                '" class="player" ondragstart="return false;" onselectstart="return false;">\
    <table cellspacing="0" cellpadding="0" border="0" width="100%"><tr>\
      <td class="medadd_aud_td medadd_aud_wid">\
        <div id="audio_pr' +
                aid +
                '" class="audio_pr" onmouseover="addClass(this, \'over\'); if (cur.hideTipTO) clearTimeout(cur.hideTipTO);" onmouseout="removeClass(this, \'over\'); removeClass(this, \'down\'); cur.hideTipTO = setTimeout(hide.pbind(\'audio_tip_wrap\'), 100);" onmousedown="addClass(this, \'down\'); audioPlayer.prClick(event);" onmouseup="removeClass(this, \'down\')">\
          <div id="audio_white_line' +
                aid + '" class="audio_white_line" onmousedown="audioPlayer.prClick(event);"></div>\
          <div id="audio_back_line' + aid +
                '" class="audio_back_line" onmousedown="audioPlayer.prClick(event);"><!-- --></div>\
          <div id="audio_load_line' + aid +
                '" class="audio_load_line" onmousedown="audioPlayer.prClick(event);"><!-- --></div>\
          <div id="audio_pr_line' + aid +
                '" class="audio_progress_line" onmousedown="audioPlayer.prClick(event);">\
            <div id="audio_pr_slider' + aid +
                '" class="audio_slider"><!-- --></div>\
          </div>\
        </div>\
      </td><td class="medadd_aud_td">\
        <div id="audio_vol' + aid +
                '" class="audio_vol" onmouseover="addClass(this, \'over\')" onmouseout="removeClass(this, \'over\'); removeClass(this, \'down\')" onmousedown="addClass(this, \'down\'); audioPlayer.volClick(event)" onmouseup="removeClass(this, \'down\')">\
          <div id="audio_vol_white_line' +
                aid + '" class="audio_vol_white_line" onmousedown="audioPlayer.volClick(event);"><!-- --></div>\
          <div id="audio_vol_back_line' + aid +
                '" class="audio_load_line" onmousedown="audioPlayer.volClick(event);"><!-- --></div>\
          <div id="audio_vol_line' + aid +
                '" class="audio_progress_line" onmousedown="audioPlayer.volClick(event);">\
            <div id="audio_vol_slider' + aid +
                '" class="audio_slider" onmousedown="audioPlayer.volClick(event);"><!-- --></div>\
          </div>\
        </div>\
      </td>\
    </tr></table>\
...
var Wall = {
    ...
    switchWall: function(ev) {
        ...
        if (ge('page_wall_count_all')) cnts.all = intval(ge('page_wall_count_all')
            .value);
        if (ge('page_wall_count_own')) cnts.own = intval(ge('page_wall_count_own')
            .value);
        if (!cnts.own || cnts.own >= cnts.all) {
            return cancelEvent(ev);
        }
        cur.wallType = ge('page_wall_posts')
            .className = (cur.wallType == 'own') ? 'all' : 'own';
        Wall.update();
        return cancelEvent(ev);
    },
    suggest: function(ev) {
        if (!cur.oid) return cancelEvent(ev);
        var cont = ge('page_suggest_post'),
            posts = domPN(ge('page_wall_posts')),
            vis = isVisible(cont);
        toggle(posts, vis);
        toggle(cont, !vis);
    ...
    suggestMore: function() {
        var cont = ge('page_suggestions'),
            pr = ge('page_suggest_prg');
        if (isVisible(pr)) return;
        ajax.post('al_wall.php', {
            act: 'get_suggests',
            owner_id: cur.oid,
            offset: cont.childNodes.length - geByClass('dld', cont)
                .length - 1
        }, {
    ...
    checkTextLen: function(inp, warn, force) {
        ...
        var val = trim(Emoji.editableVal(inp)
            .replace(/\n\n\n+/g, '\n\n'));
                    ...
                    case 'share':
                        ...
                        params = extend(params, {
                            ...
                            open_graph_data: (share.openGraph || {})
                                .data,
                            open_graph_hash: (share.openGraph || {})
                                .hash
                        });
                        break;
                    case 'page':
        ...
        if (!data(input, 'composer')) {
            if (!cur.composerAdded) {
                stManager.add(['wide_dd.css', 'wide_dd.js'], function() {
                    ...
                    callback && callback();
                    cur.destroy.push(Composer.destroy.bind(Composer)
                        .pbind(composer));
                });
            } else {
                composer = Composer.init(input, options);
    ...
    sendPost: function(inputType) {
        ...
        var pType = cur.suggesting ? 'suggest' : cur.wallType,
            params = {
                act: 'post',
                message: msg,
                to_id: cur.postTo,
    ...
    _repliesLoaded: function(post, hl, replies, names, data) {
        ...
        if (post == cur.wallLayer) {
            ...
            extend(wkcur, {
                ...
                loaded: data.num || geByClass('reply', r, 'div')
                    .length,
                count: data.count
            });
    ...
    repliesSideSetup: function(post) {
        ...
        if (cur.wallMyOpened[post] && header) {
            if (!side) {
                var sideWrap = se('<div class="replies_side_wrap"><div id="replies_side' + post +
                    '" class="replies_side"><div class="replies_side_icon" id="replies_side_icon' + post + '"></div></div></div>')
    ...
    highlightReply: function(el) {
        ...
        var xy = getXY(el),
            top = xy[1] - (bodyNode.scrollTop || htmlNode.scrollTop || 0);
        if (top < 0) {
            var cont = browser.msie6 ? pageNode : ((browser.chrome || browser.safari) ? bodyNode : htmlNode);
            animate(cont, {
                scrollTop: cont.scrollTop + top - 50
            }, 300, hlfunc);
        } else {
            hlfunc();
        }
    ...
    showReplies: function(post, count, hl, ev) {
        ...
        if (!browser.msie && count > 0 && count < 10) {
            ...
            if (slice.length == count) {
                var total = geByClass('reply', cont, 'div')
                    .length;
    ...
    moreReplies: function(post, offset, count, opts) {
        ...
        ajax.post('al_wall.php', params, {
            onDone: function(replies, names, data) {
                ...
                extend((post == cur.wallLayer ? wkcur : cur)
                    .options.reply_names, names);
                if (opts.onDone) {
    ...
    getReplyName: function(id) {
        return (((cur.wallLayer ? wkcur : cur)
                .options || {})
            .reply_names || {})[id] || [];
    },
    ...
    showEditReply: function(post, ev, fixed) {
        ...
        if (!data(rf, 'composer') && (fakeBox || cur.fixedWide || cur.wallLayer)) {
            var mediaTypes = [];
            each((cur.wallLayer == post ? wkcur : cur)
                .options.rmedia_types || cur.options.media_types || [],
                function() {
    ...
    replySubmitTooltip: function(post, over, place) {
        ...
        if (!point) {
            point = cur.replySubmitSettings = ce('div', {
                className: 'reply_multiline_tt_point'
            });
        }
        if (!over) {
    ...
    sendReply: function(post, ev, stickerNum) {
        ...
        var post_hash = ge('post_hash' + post) ? ge('post_hash' + post)
            .value : cur.options.post_hash,
        ...
        if (repliesEl && !isVisible(repliesEl) || ge('reply_link' + post)) {
            ...
            var headerEl = geByClass1('wr_header', repliesEl, 'a'),
                shown = geByClass('reply', repliesEl, 'div')
                .length + 1,
                total = shown;
            if (headerEl) {
                total = intval(headerEl.getAttribute('offs')
                    .split('/')[1]) + 1;
            }
    ...
    deletePost: function(post, hash, root, force) {
        ...
        ajax.post('al_wall.php', {
            ...
            onDone: function(msg, res, need_confirm) {
                ...
                hide(t);
                if (domNS(t)
                    .className == 'post_publish') hide(domNS(t));
        ...
        ajax.post('al_wall.php', {
            ...
            onDone: function(msg, js) {
                if (el) {
                    ...
                    var r = ge('post' + post),
                        t = geByClass1('post_table', r) || geByClass1('reply_table', r) || geByClass1('feedback_row_t', r);
    ...
    checkPostClick: function(el, event) {
        ...
        var target = event.target || event.srcElement,
            ...
            foundGood = false,
            classRE =
            /wall_post_text|published_comment|reply_link_wrap|post_media|event_share|public_share|group_share|feed_friends|feed_gifts|feed_videos|feed_explain_list|explain|feed_photos|feedback_row/;
        do {
            if (!target ||
                target == el ||
                target.onclick ||
    ...
    copyHistory: function(ev, el, post, offset) {
        ...
        ajax.post('al_wall.php', {
            ...
            onDone: function(rows) {
                ...
                domPN(after)
                    .insertBefore(cf(rows), domNS(after));
    ...
    likeOut: function(post, opts) {
        ...
        opts = opts || {};
        if (!hasClass(icon, 'my_like') && !hasClass(icon, 'fw_my_like')) {
            data(icon, 'likeoutTO', setTimeout(animate.pbind(icon, {
    ...
    likeUpdate: function(post, my, count, title) {
        ...
        var countInput = ge('like_real_count_' + like_obj) || {},
            rows = ge('like_table_' + like_obj);
        var titleNode = ge('like_title_' + like_obj),
            countNode = ge('like_count' + post);
        if (!countNode) {
            return;
        }
        var icon = ge('like_icon' + post);
        var tt = countNode.parentNode.tt || {},
            opts = clone(tt.opts || {}),
            newleft = (my ? 0 : -36);
    ...
    likeShare: function(post, hash) {
        ...
        var matches = post.match(/(-?\d+)(_?)(photo|video|note|topic|market|wall_reply|note_reply|photo_comment|video_comment|topic_comment|market_comment|)(\d+)/),
            ...
            was = isChecked(el),
            ref = cur.wallType ? (cur.wallType == 'feed' ? 'feed_' + cur.section : ('wall_' + (cur.onepost ? 'one' : (!(cur.wallType || '')
                .indexOf('full_') ? 'full' : 'page')))) : cur.module;
    ...
    albumCoverOver: function(obj, id, h) {
        ...
        animate(title, {
            marginTop: Math.max(0, getSize(obj)[1] - 22 - (descY + 7))
        }, {
            duration: 200,
            transition: Fx.Transitions.easeOutCirc
        });
    ...
    albumCoverOut: function(obj, id) {
        ...
        cur.wallAlbumTO[id] = setTimeout(function() {
            animate(geByClass1('wall_album_caption', obj), {
                marginTop: getSize(obj)[1] - 22
            }, 200);
        }, 150);
    },
    ...
    scrollCheck: function(ev, st, noScrollToY) {
        ...
        if (cur.wallPage && !cur.fixedWide) {
            ...
            if (cur.wallPageWide != pageWide && (!pageWide || rowsCont.offsetHeight > 3 * lastWindowHeight || isVisible('wall_more_link')) && !layers.visible && !fsElement) {
                ...
                var mapWrap = ge('profile_map_cont'),
                    map = cur.placesPhotoMap,
                    mapOpts = cur.placesPhotoOpts;
                if (mapWrap && map) {
                    setStyle(mapWrap, pageWide ? {
                        width: 585,
                        height: 270
                    } : {
                        width: 390,
            ...
            if (
                ...
                for (el = cur.topRow; el; el = nel) {
                    ...
                    if (((nel || {})
                            .id || '')
                        .match(/page_wall_count_/)) nel = null;
    ...
    postsGetRaws: function(el) {
        ...
        if (m = el.id.match(/^post(-?\d+_\d+)$/)) {
            res[m[1]] = 1;
            if (m = (el.getAttribute('data-copy') || '')
                .match(/^(-?\d+_\d+)$/)) {
                res[m[1]] = -1;
            }
        }
    ...
    pollOver: function(el, post, opt) {
        ...
        if (el != ttel && !el.mo) {
            ...
            addEvent(el, 'mouseout', function(e) {
                triggerEvent(ttel, 'mouseout', e, true);
            });
        }
        showTooltip(ttel, {
            url: 'al_wall.php',
    ...
    update: function() {
        ...
        var h = ge('page_wall_header'),
            cnt = cnts[cur.wallType];
        ...
        h.onclick = function(event) {
            return cnt ? nav.go(this, event) : false;
        };
        ge('page_wall_header')
            .href = '/wall' + cur.oid + ((cur.wallType == 'own') ? '?own=1' : '');
        var morelnk = ge('wall_more_link'),
            del = intval(cur.deletedCnts[cur.wallType]),
            count = geByClass(cur.wallType, ge('page_wall_posts'))
            .length - del;
    ...
    getAbsDate: function(ts, cur) {
        ...
        return cur.wallTpl.date_format.replace('{am_pm}', ampm)
            .replace('{hour}', hours)
            .replace('{num_hour}', numhours)
            .replace('{minute}', minutes);
    },
    getNowRelTime: function(cur) {
        cur = cur || window.cur;
        var ts = vkNow();
        return '<span class="rel_date rel_date_needs_update" time="' + intval(ts / 1000 - (cur.tsDiff || 0)) + '" abs_time="' + Wall.getAbsDate(ts, cur) + '">' + getLang(
            'wall_just_now') + '</span>';
    },
    getNewPostHTML: function(ev, adminLevel, extendCb, cur) {
        ...
        var nameStr = ev[3].replace('mem_link', 'author')
    ...
    getNewReplyHTML: function(ev, adminLevel, extendCb, cur) {
        ...
        if (cur.onepost) {
            ...
            acts = rs(cur.wallTpl.post_actions, {
                actions: acts.join('')
            });
        }
        var repls = {
    ...
    updatePostImages: function(html) {
        return html.replace(/<img[^>]+>/g, function(str) {
            if (str.match(/class=/)) {
                return str.replace('src=', 'data-src=')
                    .replace('class="', 'class="__need_img ');
            }
            return str;
        });
    },
    ...
    openNewComments: function(post_raw) {
        ...
        if (headerEl) {
            total = newCnt + intval(headerEl.getAttribute('offs')
                .split('/')[1]);
        }
    ...
    updateRepliesHeader: function(post_raw, headerEl, shown, total) {
        ...
        headerEl.innerHTML = '<div class="wrh_text" id="wrh_text' + post_raw + '">' + headerText + '</div><div class="progress wrh_prg" id="wrh_prg' + post_raw +
            '"></div>';
    ...
    updatePollResults: function(post_raw, newPollDataTxt) {
        ...
        var codeLink = geByClass1('page_poll_code', pollWrapEl, 'a'),
            totalEl = geByClass1('page_poll_total', pollWrapEl, 'span');
                ...
                case 'new_post':
                    {
                        ...
                        var cont = ge('page_wall_posts'),
                            ...
                            adminLevel = cur.options.is_admin !== undefined ? cur.options.is_admin : (cur.options.wall_oid < 0 ? ((flgs & 8) ? 2 : ((flgs & 2) ? 1 :
                                0)) : 0),
                ...
                case 'new_reply':
                    {
                        ...
                        var repliesEl = onepost ? ge('fw_replies_rows') : ge('replies' + post_id),
                            ...
                            extendCb = !onepost ? false : function(repls) {
                                return (repls.acts ? {
                                    acts: '<span class="divide">|</span>' + repls.acts
                                } : {})
                            },
                            flgs = intval(ev[ev.length - 1]),
                            adminLevel = cur.options.is_admin !== undefined ? cur.options.is_admin : (cur.options.wall_oid < 0 ? ((flgs & 8) ? 2 : ((flgs & 2) ? 1 :
                        ...
                        if (!isVisible(repliesEl) || !isVisible(repliesWrap) || isVisible('reply_link' + post_id)) {
                            ...
                            var openEl = repliesEl.nextSibling,
                                newCnt = geByClass('new_reply', repliesEl, 'div')
                                .length + 1;
                ...
                case 'upd_ci':
                    {
                        ...
                        switch (ev[3]) {
                            case 'audio':
                                ...
                                var attr = edit ? '' : (' onmouseover="showTooltip(this, {forcetoup: true, text: \'' + cur.options.ciAudioTip +
                                    '\', black: 1, shift: [13, 0, 0]})" onclick="Page.playCurrent(this, this.getAttribute(\'data-audio\'), \'' + cur.options.ciAudioHash +
                                    '\')"');
                                ...
                                info += '<div class="current_audio_cnt' + ci_cnt_class +
                                    ' fl_r" onmouseover="Page.audioListenersOver(this, cur.oid)" onclick="Page.showAudioListeners(cur.oid, event)"><div class="value fl_l">' +
                                    ci_cnt + '</div><div class="label fl_r"></div></div>';
                                wall.updateOwnerStatus(info, el, ev, edit);
                                break;

                            case 'app':
                                var shift = ev[6] ? '[11, 0, 0]' : '[13, 0, 0]',
                                    addCls = ev[6] ? ' current_app_icon' : '';
                                var attr = edit ? (' onclick="cur.ciApp = ' + ev[4] + '"') : (' onmouseover="showTooltip(this, {forcetoup: true, text: \'' + cur.options
                                    .ciAppTip + '\', black: 1, shift: ' + shift + '})" href="' + ev[5] + '?ref=14" onclick="return showApp(event, ' + ev[4] +
                                    ', 1, 14, cur.oid)"');
    ...
    init: function(opts) {
        ...
        if (opts.media_types) {
            cur.wallAddMedia = initAddMedia(ge('page_add_media')
                .firstChild, 'media_preview', opts.media_types, extend({
        ...
        cur.withUpload = window.WallUpload && !(browser.msie111 || browser.safari_mobile) && (cur.wallType == 'all' || cur.wallType == 'own' || cur.wallType == 'feed') &&
            Wall.withMentions && cur.wallUploadOpts;
...
var wall = extend(Wall, {
    ...
    activeDeletePost: function(post, tt, action) {
        ...
        if (tt) showTooltip(ge((action || 'delete_post') + post), {
            text: tt,
            showdt: 0,
            black: 1,
            shift: [14, 3, 3]
...
WallUpload = {
    photoUploaded: function(info, params) {
        ...
        ajax.post('al_photos.php', extend({
            ...
            onDone: function(media, data) {
                cur.wallAddMedia.chooseMedia('photo', media, extend(data, {
    ...
    init: function() {
        ...
        submitBox.insertBefore(ce('div', {
            ...
            innerHTML: '<div class="post_upload_dropbox_inner noselect"><span class="post_upload_drop_label">' + (data.opts.lang.wall_drop_photos_here ||
                'Drop files here') + '</span><span class="post_upload_release_label">' + (data.opts.lang.wall_release_photos_here ||
                'Release button to attach files') + '</span></div>'
    ...
    var _hideTimer, mediaMenu = {
        ...
        show: function(touched, ev) {
            ...
            if (menuNode && !isVisible(menuNode)) {
                ...
                var h = mediaMenu._updatePosition(),
                    el = menuNode.firstChild;
        ...
        _updatePosition: function(visible) {
            var coords = getXY(lnk, mediaMenu.fixed),
                pointerShift = 0,
                xShift = opts.leftOffset || 0,
                yShift = opts.topOffset || 0,
                top = coords[1] + yShift - 4 + (browser.msie && browser.version < 8 ? 1 : 0);
            var rowsEl = menuNode.firstChild,
                more = geByClass1('add_media_more', menuNode);
            if (vk.rtl) {
                var right =
            ...
            var countSize = getSize(rowsEl),
                st = (mediaMenu.fixed ? 0 : scrollGetY()),
                size = countSize;
            if (!visible) {
                if (more) {
                    show(more);
            ...
            if (needReverse) {
                var els = itemsNode.childNodes,
                    len = els.length,
                    el = (mediaMenu.moreWrap || {})
                    .lastChild || {};
                while (len--) {
...
function initAddMedia(lnk, previewId, mediaTypes, opts) {
    ...
    each(mediaTypes || [], function(i, v) {
        ...
        switch (v[0]) {
            case 'graffiti':
                ...
                break;
            case 'photos_list':
                handler = showBox.pbind('al_photos.php', extend(params, {
                    act: 'choose_photo'
                }), {
                    cache: 1,
            ...
            case 'photo':
                ...
                break;
            case 'video':
                handler = showBox.pbind('al_video.php', extend(params, {
                ...
                break;
            case 'audio':
                handler = showBox.pbind('audio', extend(params, {
            ...
            case 'doc':
                ...
                break;
            case 'map':
                handler = showBox.pbind('al_places.php', extend(params, {
    ...
    if (multi) {
        previewEl.innerHTML = '<div id="page_pics_preview' + lnkId + '" class="page_pics_preview media_preview clear_fix"></div><div id="page_dpics_preview' + lnkId +
            '" class="page_pics_preview page_media_sortable media_preview clear_fix"></div><div id="page_docs_preview' + lnkId +
            '" class="page_docs_preview page_media_sortable media_preview clear_fix"></div><div id="page_pdocs_preview' + lnkId +
            '" class="page_docs_preview media_preview clear_fix"></div><div id="page_ldocs_preview' + lnkId +
            '" class="page_docs_preview media_preview clear_fix"></div><div id="page_mpics_preview' + lnkId +
            '" class="page_pics_preview media_preview clear_fix"></div><div id="page_ppdocs_preview' + lnkId +
            '" class="page_docs_preview media_preview clear_fix"></div><div id="page_progress_preview' + lnkId + '" class="page_progress_preview media_preview clear_fix"></div>';
        ...
        chooseMedia: function(type, media, data, url, noboxhide, isGraffiti) {
            ...
            switch (type) {
                ...
                case 'photo':
                    ...
                    oncl = opts.nocl ? '' : ' onclick="return cur.addMedia[' + addMedia.lnkId + '].showPhoto(\'' + media + '\', \'' + data.list + '\', {' + _vopts.replace(
                        /"/g, '&quot;') + '});"';
                    preview = '<div ' + oncl + ' class="fl_l page_preview_photo' + (isGraffiti ? ' page_preview_ph_graff' : '') + '"><img class="page_preview_photo" src="' +
                        data.thumb_m + '" /></div>';
                    toPics = 1;
                    toEl = picsEl;
                    break;
                ...
                case 'app':
                    ...
                    each(geByClass('add_media_type_' + lnkId + '_app', menu.menuNode, 'a'), function() {
                        hide(this);
                    });
                    break;
                ...
                case 'doc':
                    ...
                    if (data.thumb && data.thumb_s) {
                        preview = '<a onclick="if (cur.cancelClick) return (cur.cancelClick = false);" target="_blank" href="' + data.href +
                            '" class="fl_l pam_dpic"><div class="page_preview_doc_photo"><img src="' + data.thumb_s +
                            '" align="center"></div><div class="page_preview_doc_photo_hint">' + data.title + '</div>';
                ...
                case 'share':
                    ...
                    preview = '<a target="_blank" href="/away.php?to=' + encodeURIComponent(data.url) + '" class="medadd_h medadd_h_link inl_bl">' + data.lang.profile_choose_link +
                        '</a>';
                ...
                case 'map':
                    preview = '<div class="fl_l"><a onclick="return showBox(\'al_places.php\', {act: \'geo_box\', lat: ' + data[0] + ', long: ' + data[1] + ', provider: ' +
                        intval(data[3]) + '}, {dark: 1});"><div class="page_media_map_point"></div><img class="page_preview_map" width="174" height="70" src="/maps?lat=' +
                        data[0] + '&lng=' + data[1] + '&z=11&' + ((window.devicePixelRatio >= 2 || true) ? 'w=360&h=140' : 'w=174&h=70') + '" /></a></div>';
                    toEl = toPics = mpicsEl;
                ...
                case 'page':
                    ...
                    preview = '<a href="/page' + data.media + '" onclick="return showWiki({oid: ' + lst[0] + ', id: ' + lst[1] +
                        '}, false, event, {queue: 1})" class="medadd_h medadd_h_page inl_bl">' + data.lang.profile_choose_page + '</a>';
                    toEl = ldocsEl;
                    break;
...
' + (data.thumb ? '<img class="wall_album_cover" src="' + data.thumb + '"/>' : '') +
  ...
  <div class="wall_album_caption">\
    <div class="wall_album_title_wrap clear_fix">\
      <div class="wall_album_title fl_l">' + data.title +
                        '</div>\
      <div class="wall_album_count fl_r">' + data.count + '</div>\
                ...
                case 'note':
                    if (!data.lang) return false;
                    preview = '<a onclick="showWiki({w: \'note' + data.raw + '\', edit: 1}, true, event, {queue: 1})" class="medadd_h medadd_h_note inl_bl">' + data.lang.profile_choose_note +
                        '</a>';
                    postview = '<div class="medadd_c medadd_c_note"><a onclick="showWiki({w: \'note' + data.raw +
                        '\', edit: 1}, true, event, {queue: 1})" id="share_note_title' + data.raw + '">' + data.title + '</a></div>';
                    toEl = ldocsEl;
                    break;

                case 'market':
                    preview = '<div class="medadd_c_market fl_l"><a target="_blank" href="' + data.href + '"><img class="medadd_c_market_thumb fl_l" src="' + data.thumb +
                        '" /></a><div class="medadd_c_market_info fl_l"><a class="medadd_c_market_title" target="_blank" href="' + data.href + '">' + data.title +
                        '</a><div class="medadd_c_market_price">' + data.price + '</div></div>';
                    hide(lnk);
                    break;
...
' + (data.thumb ? '<img class="wall_album_cover" src="' + data.thumb + '"/>' : '') +
  ...
  <div class="wall_album_caption">\
    <div class="wall_album_title_wrap clear_fix">\
      <div class="wall_album_count fl_r">' + data.count +
                        '</div>\
      <div class="wall_album_title">' + data.title + '</div>\
                ...
                case 'postpone':
                    ...
                    if (cur.editingPost && !multi) {
                        ...
                        geByTag1('button', geByClass1('button_blue', ge('post' + cur.editingPost[0])))
                            .innerHTML = getLang('global_save');
                    } else if (cur.editingPost && domPN(ppdocsEl)
                        .id == 'wpe_media_preview') {
                        media = intval(media);
                        if (media) {
                            data.date = media;
                        } else {
        ...
        singleAdded: function(mediaEl, type) {
            ...
            each(geByClass('add_media_item', menu.menuNode, 'a'), function(i, v) {
                if (hasClass(v, 'add_media_type_' + lnkId + '_postpone') && (addMedia.postponePreview || type === 'postpone')) {
                    ...
                    hide(v);
                } else if (!inArray(type, ['postpone', 'mark_as_ads']) && !hasClass(v, 'add_media_type_' + lnkId + '_postpone') && !hasClass(v, 'add_media_type_' +
                        lnkId + '_mark_as_ads')) {
                    hide(v);
                } else if (isVisible(v)) {
                    menuItemsVisible++;
        ...
        showPhoto: function(photoId, listId, opts, ev) {
            ...
            for (var i in ajaxCache) {
                if (i.toString()
                    .match(/^\/al_photos\.php\#act=show&draft_photos/)) {
                    delete ajaxCache[i];
                }
            }
            var m = addMedia.getMedias(),
                allPhotos = [];
            each(m, function(k, v) {
                if (v && v[0] == 'photo') {
  ...
  <div id="upload' + ind +
        ...
        attachCount: function() {
            ...
            var num = (editable && window.ThumbsEdit ? ((ThumbsEdit.cache()['thumbs_edit' + lnkId] || {})
                ...
                .length + dpicsEl.childNodes.length + mpicsEl.childNodes.length + docsEl.childNodes.length / (docsEl.sorter ? 2 : 1) + progressEl.childNodes.length;
        ...
        createPoll: function(data) {
            var h = (browser.msie6 || data.question) ? '' : '1px',
                html = [],
                ans;
            ...
            addMedia.pollPreview = pdocsEl.appendChild(ce('div', {
                ...
                innerHTML: '\
<input onkeydown="cur.addMedia[' + lnkId + '].keyPoll(this, event)" class="text medadd_c_pollq" id="create_poll_question' + lnkId +
                    '" value="' + (data.question || '') + '" />\
<div class="medadd_c_pollh">' + data.lang.a +
                    '</div>\
...
<div class="medadd_c_polladd_wr" id="create_poll_add' +
  ...
  <div class="medadd_c_polladd" onclick="cur.addMedia[' + lnkId + '].incPoll()">' + data.lang.i + '</div>\
                        ...
                        '' : '<div class="checkbox medadd_c_pollcb' + (data.anon ? ' on' : '') + '" id="create_poll_anonymous' + lnkId +
                        '" onclick="checkbox(this);cur.addMedia[' + lnkId + '].changedPoll();"><div></div>' + data.lang.c + '</div>')
            }));
        ...
        pollData: function(silentCheck) {
            ...
            for (var el = domFC(answers); el; el = domNS(el)) {
                a = trim(val(domFC(el)));
                if (a) {
                    var id = -intval((domFC(el)
                        .id.match(/^create_poll_ans(\d+)$/) || [0, -(ind++)])[1]); // -id or ind
                    result['answers[' + id + ']'] = a;
                    found = true;
                }
            }
        ...
        checkMessageURLs: function(message, inactive) {
            ...
            for (var i in urls) {
                ...
                if (domain.match(/(^|\.|\/\/)(vkontakte\.ru|vk\.com)/)) {
                    valid = query.match(
                        /(#photo|^\/(photo|video|album|page|audio|doc)|z=(album|photo|video)|w=(page|product))(-?\d+_)?\d+|\.(jpg|png|gif)$|market-?\d+\?section=album_\d+|^\/stickers\/.+$|^\/vk2016+$|^\/blog\/.+$|^http:\/\/instagram\.com\/p\/.+/
                    ) ? true : false;
                }
                if (valid) {
...
<div class="medadd_c_linkcon"><div></div>' + (progress ? '<div class="progress medadd_c_linkprg"></div>' : '') + '</div>'
            ...
            if (data.failed) {
                ...
                if (data.images && data.images[cur.shareShowImg] && imgUrl) {
                    ...
                    if (data.images.length > 0) {
                        var leftScroller = ((data.images.length > 1) ? ('<div class="medadd_c_linkimg_scroll_wrap medadd_c_linkimg_scroll_wrap_left ' + ((cur.shareShowImg ==
                            0) ? 'medadd_c_linkimg_scroll_wrap_left_first' : '') + '" onclick="' + ((cur.shareShowImg == 0) ? 'Page.ownerPhoto(\'' + data.media +
                        ...
                        var imgControls = (!data.media || data.media === '_') ? '' : '<div class="medadd_c_linkimg_controls">' +
                            ...
                            '    <div class="medadd_c_linkimg_controls_btn ' + (availableImagesCount > 1 ? 'medadd_c_btn_side_padd' : '') + '" id="medadd_ctrl_upload" ' +
                            uploadTooltip + ' onclick="Page.ownerPhoto(\'' + data.media + '\');"></div>' +
                            '  </div>' +
                            '  <div class="medadd_c_linkimg_controls_btn_group clear_fix fl_r">' +
                            '    <div class="medadd_c_linkimg_controls_btn" id="medadd_ctrl_remove" ' + removeTooltip +
                            ' onclick="tooltips.hide(this);cur.removeLinkImage(this)"></div>' +
                            '  </div>' +
                            '</div>';
        ...
        setupPostpone: function(data, export_row) {
            ...
            var ed = (cur.editingPost && domPN(toEl)
                    .id == 'wpe_media_preview'),
                h = (browser.msie6 || ed || !multi) ? '' : '1px',
                addedhtml = false;
            var html = '<div class="clear_fix">\
<div class="fl_l"><input type="hidden" id="postpone_date' + lnkId + '" value="' + (data.date || '') +
                '" /></div>\
...
<div class="fl_l"><input type="hidden" id="postpone_time' +
            ...
            if (cur.editingPost && data.friends_only != undefined) {
                ...
                if (data.status_export != undefined) {
                    html += '<div class="checkbox_status_export' + (data.status_export ? ' on' : '') + ' fl_l" id="status_export' + lnkId +
                        '" onclick="checkbox(this)" onmouseover="showTooltip(this, {text: \'' + data.lang.export_to_twitter +
                        '\', black: 1, shift: [12,4,0]});"><div></div></div>';
                }
                if (data.facebook_export != undefined) {
                    html += '<div class="checkbox_facebook_export' + (data.facebook_export ? ' on' : '') + ' fl_l" id="facebook_export' + lnkId +
                        '" onclick="checkbox(this)" onmouseover="showTooltip(this, {text: \'' + data.lang.export_to_facebook +
                        '\', black: 1, shift: [12,4,0]});"><div></div></div>';
                }
                html += '<div class="checkbox' + (data.friends_only ? ' on' : '') + ' fl_l" id="friends_only' + lnkId +
                    '" onclick="checkbox(this);checkbox(\'status_export' + lnkId + '\',!isChecked(this));checkbox(\'facebook_export' + lnkId +
                    '\',!isChecked(this));"><div></div>' + data.lang.friends_only + '</div></div>';
                addedhtml = true;
