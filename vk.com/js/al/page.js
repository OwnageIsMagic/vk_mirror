var Page = {
            ...
            var b = showBox('al_page.php', {
                ...
                .setButtons(getLang('global_save'), function(btn) {
                    ...
                    function onSearch() {
                        ...
                        params.hash = hash;
                        if (!hash) params.hash = ge('group_contact_hash')
                            .value;
                        ...
                        ajax.post('al_page.php', params, {
                            onDone: function(res, script) {
                                ...
                                if (box) {
                                    ...
                                    toggle('group_add_contact', ge('public_contacts_list')
        ...
        searchContact: function(oid, page, onSearch) {
            ...
            ajax.post('al_page.php', {
                ...
                onDone: function(uid, img, name, hash) {
                    ...
                    ge('group_contact_name')
                        .innerHTML = name;
                    ge('group_contact_image')
                        .innerHTML = img;
                    ge('group_contact_hash')
                        .value = hash;
                    if (!uid) {
        ...
        infoSave: function() {
            ...
            var input = ge('currinfo_input'),
                txt = trim((window.Emoji ? Emoji.editableVal : val)(input))
                .replace(/\n/g, ' ');
            ...
            ajax.post('al_page.php', {
                ...
                onDone: function() {
                    ...
                    ge('current_info')
                        ...
                        .innerHTML = '<span class="' + c + '_current_info">' + t + '</span>';
                    Page.infoCancel();
                    var el = ge('current_info'),
                        tt = el.tt;
                    if (tt && tt.el) {
        ...
        showGif: function(obj, ev, dontHideActive) {
            ...
            var acts = '<div class="page_gif_share" onmouseover="showTooltip(this, {text: \'' + shareTxt +
                '\', black: 1, shift: [7, 6, 6], toup: 0, needLeft: 1})" onclick="return Page.shareGif(this, \'' + doc + '\', \'' + hash +
        ...
        autoplayPinnedVideo: function(postId, videoRaw, videoHash) {
            ...
            showInlineVideo(videoRaw, videoHash, {
                autoplay: 1,
                addParams: {
                    post_id: postId,
                    from_autoplay: 1
        ...
        feedListsDDShow: function() {
            ...
            for (var i in cur.options.feedLists) {
                ...
                elems.push('<a id="page_feed_item' + i + '" class="page_actions_item page_actions_subitem' + (cur.options.feedListsSet && cur.options.feedListsSet[i] ?
                    ' checked' : '') + '" onclick="Page.feedListsCheck(this, ' + i + ');">' + lname + '</a>');
            }
...
var Wall = {
    ...
    suggestMore: function() {
        ...
        ajax.post('al_wall.php', {
            act: 'get_suggests',
            owner_id: cur.oid,
            offset: geByClass('post', cont)
                .length - geByClass('dld', cont)
                .length
        }, {
            onDone: function(rows, notAll) {
                var el = ce('div', {
    ...
    suggestPublished: function(post, text, postponed) {
        ...
        if (!intval(val('page_suggests_count'))) {
            ...
            if (wallMenu) {
                geByClass1('ui_rmenu_item', wallMenu)
                    .click();
    ...
    showPostponedFull: function(rows) {
        ...
        val(cont, rows);
        FullWall.updateSummary(geByClass('post', cont)
            .length);
    },
    ...
    setDraft: function(data) {
        ...
        Emoji.val(field, clean(replaceEntities(data[0] || ''))
            .replace(/\n/g, '<br/>'));
    ...
    focusOnEnd: function() {
        ...
        if (document.selection) {
            ...
            window.getSelection()
                .collapse(el, el.childNodes.length);
        }
    },
            ...
            var optId = Emoji.init(txt, {
                ...
                ctrlSend: function() {
                    return Wall.customCur()
                        .wallTpl.reply_multiline || Wall.composerListShown(txt);
                },
    ...
    showEditReply: function(post, ev, fixed) {
        ...
        if (fakeBox) {
            var postHash = ge('post_hash' + post),
                ...
                ownerPhoto = fakeBox.getAttribute('data-owner-photo') || (geByClass1('post_img', postEl) || {})
                .src || '',
                ownerHref = fakeBox.getAttribute('data-owner-href') || (geByClass1('post_image', postEl) || {})
                .href || '';
    ...
    replyNamesRE: function() {
        var names = ((cur.wallLayer ? wkcur : cur)
                .options || {})
            .reply_names;
    ...
    replyTo: function(post, toMsgId, toId, event) {
        ...
        replyNameFirst = '<a class="reply_to_mem" onclick="return wall.showReply(this, \'' + post + '\', \'' + replyOid + replyType + '_' + toMsgId + replyAt +
            '\', event);">' + replyNameFirst + '</a>';
        var value = '<span class="reply_to_cancel" onclick="return Wall.cancelReplyTo(\'' + post + '\', event);"></span><div class="reply_to_label">' + langStr(getLang(
            'global_reply_to'), 'user', replyNameFirst) + '</div>';
    ...
    replySubmitTooltip: function(post, over, place) {
        ...
        var ctrlSubmit = Wall.customCur()
            .wallTpl.reply_multiline ? 1 : 0,
            hint = rs(Wall.customCur()
    ...
    onReplySubmit: function(post, e) {
        ...
        if (e.keyCode == KEY.RETURN || e.keyCode == 10) {
            ...
            if (Wall.customCur()
                .wallTpl.reply_multiline && (e.ctrlKey || browser.mac && e.metaKey) ||
                !Wall.customCur()
                .wallTpl.reply_multiline && !e.shiftKey && !(e.ctrlKey || browser.mac && e.metaKey) && !isListVisible) {
                Wall.sendReply(post);
    ...
    sendReply: function(post, ev, options) {
        ...
        var message = Emoji.emojiToHTML(clean(params.message), true),
            toName = params.reply_to_user < 0 ? getLang('wall_replied_to_group') : cur.options.reply_names[params.reply_to_user] && cur.options.reply_names[params.reply_to_user]
            [0],
    ...
    markAsAds: function(post, hash, el, fullPost) {
        ajax.post('al_wall.php', {
            ...
            onDone: function(msg) {
                ge('ui_actions_menu_item_mark_as_ads' + post)
                    .innerHTML = msg;
            }
        });
    },
    ...
    postFull: function(post, event, opts) {
        if (post.match(/^wall-?\d+_\d+$/) && !(opts || {})
            .nolist && !(cur.pgParams && (cur.pgParams.owners_only || cur.pgParams.q))) {
            switch (cur.wallType) {
                case 'all':
                case 'full_all':
                    post += '/all';
                ...
                case 'upd_ci':
                    {
                        ...
                        switch (ev[3]) {
                            case 'audio':
                                ...
                                if (!edit) {
                                    attr += ' onmouseover="showTooltip(this, {forcetoup: true, text: \'' + cur.options.ciAudioTip +
                                        '\', black: 1, shift: [14, 5, 5]})" onclick="Page.playCurrent(this, \'' + ev[4] + '\')"';
                                }
    ...
    init: function(opts) {
        ...
        cur.withUpload = window.WallUpload && !browser.safari_mobile && inArray(cur.wallType, ['all', 'own', 'feed', 'full_all', 'ads_promoted_stealth']) && Wall.withMentions &&
            cur.wallUploadOpts;
    ...
    replyAsGroupOver: function(obj, tt_user, tt_group) {
        ...
        if (!ttChooser) {
            ...
            <div class="radiobtn ' +
                    onCls[0] + '" data-as="me">' + getLang('global_on_behalf_me') + '</div> \
            <div class="radiobtn ' + onCls[1] + '" data-as="group">' +
...
WallUpload = {
    photoUploaded: function(info, params) {
        ...
        ajax.post('al_photos.php', extend({
            ...
            onDone: function(media, data) {
                WallUpload.addMedia()
                    .chooseMedia('photo', media, extend(data, {
                        upload_ind: i + '_' + fileName
                    }));
            },
            ...
            onUploadProgress: function(info, bytesLoaded, bytesTotal) {
                ...
                if (Upload.types[i] == 'fileApi') {
                    ...
                    if (lnkId === undefined || lnkId && cur.addMedia[lnkId].chosenMedia || cur.imMedia) {
                        ...
                        WallUpload.addMedia()
                            .showMediaProgress('photo', i, data);
                    }
                }
            },
            onUploadError: WallUpload.uploadFailed,
...
<div class="medadd_c_polladd_wr" id="create_poll_add' +
  ...
  <div class="medadd_c_polladd" onclick="cur.addMedia[' + lnkId + '].incPoll()">' + data.lang.i + '</div>\
                        ...
                        '" onclick="checkbox(this);cur.addMedia[' + lnkId + '].changedPoll();">' + data.lang.c + '</div>')
            }));
            ...
            if (cur.editingPost && data.friends_only != undefined) {
                ...
                if (data.status_export != undefined) {
                    html += '<div class="checkbox_status_export' + (data.status_export ? ' on' : '') + ' fl_l" id="status_export' + lnkId +
                        '" onclick="checkbox(this)" onmouseover="showTooltip(this, {text: \'' + data.lang.export_to_twitter + '\', black: 1, shift: [12,4,0]});"></div>';
                }
                if (data.facebook_export != undefined) {
                    html += '<div class="checkbox_facebook_export' + (data.facebook_export ? ' on' : '') + ' fl_l" id="facebook_export' + lnkId +
                        '" onclick="checkbox(this)" onmouseover="showTooltip(this, {text: \'' + data.lang.export_to_facebook + '\', black: 1, shift: [12,4,0]});"></div>';
                }
                html += '<div class="checkbox' + (data.friends_only ? ' on' : '') + ' fl_l" id="friends_only' + lnkId +
                    '" onclick="checkbox(this);checkbox(\'status_export' + lnkId + '\',!isChecked(this));checkbox(\'facebook_export' + lnkId + '\',!isChecked(this));">' +
                    data.lang.friends_only + '</div></div>';
                addedhtml = true;
