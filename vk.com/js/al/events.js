window.events = {
                ...
                switch (type) {
                    ...
                    case 'decline':
                        ...
                        if (cur.curTab == 'invitations' && acts.childNodes.length < 3) {
                            acts.innerHTML += '<span class="divider">|</span><a onclick="events.reportSpam(this.parentNode, ' + eid + ', \'' + hash + '\')">' + getLang(
                                'its_spam') + '</a>';
                        }
                        break;
                }
    ...
    changePast: function(eid, type, hash, el) {
        ...
        if (cur.convert) {
            geByClass1('status', el.parentNode.parentNode)
                .innerHTML = getLang('events_past_moved');
            el.innerHTML = '';
        } else {
    ...
    changeStatus: function(eid, hash, norequest) {
        var row = ge('event_row' + eid);
        if (!row) return;
        var html = '<div class="button_blue fl_l"><button class="join_button">' + getLang('events_user_will_participate') +
            '</button></div>\
    <div class="button_blue fl_l"><button class="unsure_button">' + getLang('events_user_unsure') +
            '</button></div>\
    ...
    subscribe: function(eid, action, hash, el) {
        ...
        ajax.post('al_events.php', {
            ...
            onDone: function(followersModule, unsureModule, likeModule, js) {
                ...
                if (getModules) {
                    ge('event_like_module_wrap')
                        .innerHTML = likeModule;
                    ge('event_followers_module_wrap')
                        .innerHTML = followersModule;
                    ge('event_unsure_module_wrap')
                        .innerHTML = unsureModule;
                    if (js) {
                        eval(js);
                    }
    ...
    switchTab: function(tab, onEnd, onHeader) {
        ...
        ge('info_module_header_content')
            .innerHTML = cur.tabHeaders[tab];
        //var hh = getSize(ge('info_module_header_content'))[1];

        ge('public_manage_page_link')
            .innerHTML = cur.tabActions[tab];
...
function initCalendar(month, year, events, holidays) {
    ...
    extend(cur, {
        ...
        calGetCurEvents: function(day) {
            ...
            for (var i in events) {
                var year = (new Date(events[i][3] * 1000))
                    .getFullYear();
        ...
        calShowMore: function(el, i, day) {
            ...
            e.innerHTML = '<a href="' + item[2] + '" onclick="return nav.go(this, event);"><img src="' + item[4] + '" onmouseover="cur.calTrailOn(this, ' + item[0] +
                ', ' + day + ');" onmouseout="cur.calTrailOff(this, ' + item[0] + ');"/></a>';
        },
        calTrailOn: function(el, fid, day) {
            ...
            if (fid < 0) {
                ...
                text = getLang('events_calendar_future_event')
                    .replace('{time}', time);
            } else {
                if (bDay.getFullYear() != nowDay.getFullYear()) {
                    var years = curDay.getFullYear() - bDay.getFullYear();
                    text = getLang((nowDay > curDay) ? 'events_calendar_past_birthday' : ((nowDay.toString() == curDay.toString()) ? 'events_calendar_recent_birthday' :
                        'events_calendar_future_birthday'), years);
                }
            }
            ...
            var showTT = function() {
                ...
                showTooltip(el, {
                    content: '<div class="wrap"><div class="content"><a href="' + item[2] + '" onclick="return nav.go(this, event);"><img src="' + item[5] +
                        '"/></a><div class="name">' + name + '</div>' + text + '</div></div>',
        ...
        calGetMonth: function(shift) {
            ...
            ge('calendar_header')
                .innerHTML = getLang('Month' + cur.calMon) + ' ' + cur.calYear;


            var days = (new Date(cur.calYear, cur.calMon, 0))
                .getDate();
                ...
                for (var weekDay = 0; weekDay < 7; weekDay++) {
                    ...
                    if (day > 0 && day <= days) {
                        ...
                        if (calEvents[0]) {
                            ...
                            dayEvents = '<a href="' + href + '" onclick="return nav.go(this, event);"><img src="' + photo + '" onmouseover="cur.calTrailOn(this, ' +
                                fid + ', ' + day + ');\" onmouseout="cur.calTrailOff(this, ' + fid + ');"/></a>';
            ...
            <div class="day_num fl_l">' + day +
                            '</div><div class="day_text fl_l">' + dayText + '</div>\
            <div class="day_events clear">' + dayEvents +
                            '</div>\
            ...
            ge('calendar_table_wrap')
                .innerHTML = '<table class="day_table" cellpadding="0" cellspacing="0" align="center">\
      <tr>\
       <td class="day_head day1">' + getLang(
                    'events_mon') + '</td>\
       <td class="day_head day2">' + getLang('events_tue') + '</td>\
       <td class="day_head day3">' + getLang(
                    'events_wed') + '</td>\
       <td class="day_head day4">' + getLang('events_thu') + '</td>\
       <td class="day_head day5">' + getLang(
                    'events_fri') + '</td>\
       <td class="day_head day6">' + getLang('events_sat') + '</td>\
       <td class="day_head day7">' + getLang(
                    'events_sun') + '</td>\
      </tr>' + rows + '</table>';
...
var PagedList = function(container, data, options) {
    ...
    this.getPage = function(page, search, force) {
        ...
        if (!filtered_data.length) {
            ge(container)
                .innerHTML = options.emptyRow(search);
