var SinglePhotoview = {
    ...
    updateArrows: function() {
        var e = sbWidth() + 2;
        cur.spvLeft.style.left = "20px", cur.spvLeftNav.style.width = Math.floor((lastWindowWidth - e - cur.spvActualWidth - 52) / 2) + "px", cur.spvRightNav.style.left =
            Math.floor((lastWindowWidth - e + cur.spvActualWidth + 52) / 2) + "px", cur.spvRightNav.style.width = Math.floor((lastWindowWidth - e - cur.spvActualWidth - 52) /
                2) + "px", cur.spvClose.style.left = lastWindowWidth - e - 2 - 37 + "px"
    },
    ...
    showByIndex: function(e, t) {
        if (!t || 2 != t.button && 3 != t.which) {
            ...
            if (i) {
                ...
                } else o = !0, addEvent(window, "resize", SinglePhotoview.onResize), addEvent(document, "keydown", SinglePhotoview.onKeyDown), addEvent(layerWrap, "click",
                    SinglePhotoview.onClick), boxQueue.hideAll(), setStyle(layerBG, {
                    opacity: ""
                ...
                if (s && s.src) {
                    ...
                    return cur.spvIndex = e, cur.spvShown = !0, cur.spvFixed || (cur.spvFixed = bodyNode.appendChild(ce("div", {
                            ...
                            })), cur.spvLeft = cur.spvFixed.firstChild, cur.spvClose = cur.spvLeft.nextSibling, addClass(layerWrap, "spv_dark"), addClass(layerBG,
                                "spv_dark"), vkImage()
                            .src = "/images/upload.gif", layer.innerHTML =
    ...
    doShow: function() {
        ...
        if (cur.spvShown) {
            var t = cur.spvCurPhoto,
                ...
                v = e.width || t.w,
                r = e.height || t.h;
            r * o >= 453 ? i.style.height = Math.floor(r * o) + "px" : i.style.height = "453px", s = positive(Math.floor((453 - r * o) / 2)), cur.spvPhWidth = Math.floor(v *
                    o), cur.spvPhHeight = Math.floor(r * o), cur.spvActualWidth = Math.max(cur.spvPhWidth, 604), cur.spvCont.style.width = cur.spvActualWidth + 154 + "px",
                cur.spvSummary.parentNode.style.width = cur.spvActualWidth - 4 + "px", i.innerHTML = '<img style="width: ' + cur.spvPhWidth + "px; height: " + cur.spvPhHeight +
                "px; margin-top: " + s + 'px;" src="' + e.src + '" />', layerWrap.scrollTop = 0, t.desc ? (cur.spvWide.innerHTML = '<div id="spv_desc">' + t.desc +
                    "</div>", show(cur.spvCommentsData)) : hide(cur.spvCommentsData), SinglePhotoview.updateArrows(), setTimeout(SinglePhotoview.afterShow, 2)
        }
    },
    afterShow: function() {
    ...
    doHide: function() {
        ...
        layers.hide(), layers.fullhide = !1, each(["spvLeft", "spvClose", "spvFixed"], function() {
                var e = this + "";
                re(cur[e]), cur[e] = !1
            }), browser.mobile && (ge("footer")
                .style.height = ""), removeClass(layerWrap, "spv_dark"), removeClass(layerBG, "spv_dark"), layerBG.style.opacity = "", cur.spvShown = cur.spvClicked = !1,
            removeEvent(window, "resize", SinglePhotoview.onResize), removeEvent(document, "keydown", SinglePhotoview.onKeyDown), removeEvent(layerWrap, "click",
                SinglePhotoview.onClick), cur.spvOnHide && cur.spvOnHide(), window.wkcur && wkcur.shown && WkView.showLayer()
    },
    onClick: function(e) {
    ...
    onKeyDown: function(e) {
        return e.returnValue === !1 ? !1 : e.keyCode == KEY.ESC ? (SinglePhotoview.hide(), cancelEvent(e)) : void(boxQueue.count() || (e.keyCode == KEY.RIGHT ?
            SinglePhotoview.showByIndex(cur.spvIndex + 1) : e.keyCode == KEY.LEFT && SinglePhotoview.showByIndex(cur.spvIndex - 1)))
    },
    onResize: function() {
