...
function showAboutAds() {
    ...
    adBox.removeButtons()
        ...
        .content("<div>" + getLang('market_can_promote') + "<br /><br /><b>" + getLang('market_features') + "</b><br /><br /></div><ul class='listing'><li><span>" + getLang(
                'market_determine_cost') + "</span></li><li><span>" + getLang('market_pay_for_result') + "</span></li></ul><div>" + getLang('market_more_info') +
            "<br /><br /><div style='text-align:right'><a href='market.php?act=cat'>" + getLang('market_create_ad') + " &raquo;</a></div></div>")
        .show();
    return false;
}
...
function showNoBox() {
    ...
    noBox.removeButtons()
        ...
        .content("<div>" + getLang('market_available_after') + "</div>")
        .show();
    return false;
}
...
function submitItemForm(form) {
    ...
    if (ge('captcha')) {
        params += '&captcha=' + encodeURIComponent(ge('captcha')
            .value);
    }
    ...
    Ajax.postWithCaptcha('market.php?act=save', params, {
        onSuccess: function(ajaxObj, responseText) {
            ...
            if (response['error']) {
                ...
                ge('errorMessage')
                    .innerHTML = '<div class="alertmsg">' + response['error'] + '</div>';
                slideDown(ge('errorMessage'), 200);
                if (response['sid']) {
                    ge('captchaRow')
                        .style.display = (browser.msie ? 'block' : 'table-row');
                    ge('captchaImg')
                        .src = '/captcha.php?sid=' + response['sid'];
                    ge('current_sid')
                        .value = response['sid'];
                    ge('captcha')
                        .focus();
                ...
                if (!selected_files) {
                    if (photosForm.file1.value) {
                        ...
                        location.replace('market.php?act=view&id=' + response.item_id + '&m=1' + ge('referrer')
                            .value);
                    }
                } else {
...
function update_page(obj, text) {
    var old_section = 0;
    if (ge('section_index')) {
        old_section = ge('section_index')
            .innerHTML;
    } else {
        old_section = ge('section')
            .value;
    }
    if (old_section) {
    ...
    var new_section = ge('section_index')
        .innerHTML;
    if (new_section) {
    ...
    if (new_section != old_section) {
        ...
        selectCategory(new_section, parseInt(ge('category_index')
            .innerHTML));
    }
    ...
    if ($('#searchForm')
        ...
        var data = {
            cat: ge('category')
                .options[ge('category')
                    .selectedIndex].value,
            price_min: ge('minPrice')
                .value,
            price_max: ge('maxPrice')
                .value,
            section: section,
            show: ge('show')
                .value,
...
function deleteItem(id, where) {
    ...
    } else if (parseInt(ge('deleting_' + id)
            .value)) {
        return false;
    } else {
        ge('deleting_' + id)
...
function reportItem(id) {
    ...
    reportBox.removeButtons()
        ...
        .content("" + getLang('market_select_ban_reason') +
            "<br /><br /><div id='marketReport'><div><input type='radio' name='reason' id='r1' onclick='reason=1' checked><label for='r1'>" + getLang('market_reason_spam') +
            "</label></div><div><input type='radio' name='reason' id='r2' onclick='reason=2'><label for='r2'>" + getLang('market_reason_porn') +
            "</label></div><div><input type='radio' name='reason' id='r3' onclick='reason=3'><label for='r3'>" + getLang('market_reason_cheat') +
            "</label></div><div><input type='radio' name='reason' id='r4' onclick='reason=4'><label for='r4'>" + getLang('market_reason_offtop') +
            "</label></div><div><input type='radio' name='reason' id='r5' onclick='reason=5'><label for='r5'>" + getLang('market_reason_duplicate') +
            "</label></div><div><input type='radio' name='reason' id='r6' onclick='reason=6'><label for='r6'>" + getLang('market_reason_extr') +
            "</label></div><div><input type='radio' name='reason' id='r7' onclick='reason=7'><label for='r7'>" + getLang('market_reason_nonsense') +
            "</label></div><div><input type='radio' name='reason' id='r8' onclick='reason=8'><label for='r8'>" + getLang('market_reason_ortho') + "</label></div></div>")
        .show();
    return false;
}
...
function showPhoto(url, photo_id) {
    ...
    if (!pic.width) {
        ...
        ge('imageProgress')
            .style.height = ge('photo' + photo_id)
            .offsetHeight + "px";
        ge('imageProgress')
...
function showNextPhoto(url, afterWaited) {
    ...
    if (!pic.width) {
        ...
        ge('imageProgressBig')
            .style.width = ge('currentPhoto')
            .offsetWidth + "px";
        ge('imageProgressBig')
            .style.height = ge('currentPhoto')
            .offsetHeight + "px";
        ge('imageProgressBig')
            .firstChild.style.marginTop = ((ge('currentPhoto')
                .offsetHeight - 8) / 2) + "px";
        ge('imageProgressBig')
            .style.left = coords[0] + "px";
        ge('imageProgressBig')
...
function renderPhoto(url) {
    ...
    var title = getLang('market_photo_one_of_photo')
        .replace('{index}', index)
        .replace('{count}', photosCount);
    ...
    photoBox.content('<a href="#next" onclick="showNextPhoto(\'' + photosOfItem[index] + '\'); return false;"><img id="currentPhoto" src="' + url + '" /></a>')
        .show();
...
function addToFavesSearch(id) {
    ge('to_faves' + id)
        .innerHTML = "<a onclick=\"return deleteFromFavesSearch(" + id + ");\" href='#' >" + market_defave + "</a>";
    $('#faved' + id)
        .load('market.php?act=add_to_faves&mode=1', {
            id: id,
            hash: ge('faveHash')
                .value
        });
...
function plusFave() {
    var fave_num = parseInt(ge('faveNum')
        .value);
...
function deleteFromFavesSearch(id) {
    ge('to_faves' + id)
        .innerHTML = "<a onclick=\"return addToFavesSearch(" + id + ");\" href='#' >" + market_enfave + "</a>";
    $('#faved' + id)
        .load('market.php?act=delete_from_faves&mode=1', {
            id: id,
            hash: ge('faveHash')
                .value
        });
...
function user_moderation_selected(id) {
    ...
    if (!ge('sending_to_' + id)) {
        ...
        node.value = 1;
        document.body.appendChild(node);
    } else if (parseInt(ge('sending_to_' + id)
            .value)) {
        return;
    } else {
        ge('sending_to_' + id)
    ...
    var status = ge('user_moderation_select')
        .options[ge('user_moderation_select')
            .selectedIndex].value;
    ajax.onDone = function(obj, txt) {
        ge('sending_to_' + id)
            .value = 0;
        ge('user_moderation_progress')
            .style.display = 'none';
        if (txt.substr(0, 1) != 'g') {
    ...
    var back_link = ge('back_link') ? ge('back_link')
        .value : '';
    ...
    ajax.onDone = function(obj, txt) {
        ...
        if (txt.substr(0, 1) != 'g') {
            ...
            if (status == 0) {
                ...
                ge('user_is_ok')
                    .innerHTML = "<a href='javascript: user_moderation_status(" + id + ", 1)'>�����������</a>";
                ge('user_attention')
                    .style.fontWeight = 'normal';
                ge('user_attention')
                    .innerHTML = "<a href='javascript: user_moderation_status(" + id + ", 2)'>��������������</a>";
                if (ge('warn_added_counter')) {
                    ge('warn_added_counter')
                        .innerHTML = '';
                }
            } else if (status == 1) {
                ge('user_clear')
                ...
                ge('user_warn')
                    .innerHTML = '������������';
                var btn =
                    '<ul class="nNav" style="float: right"><li><b class="nc"><b class="nc1"></b><b class="nc2"></b></b><span class="ncc"><a href="javascript: user_moderation_status(' +
                    id + ', -2)">������</a></span><b class="nc"><b class="nc2"></b><b class="nc1"></b></b></li></ul>';
                ge('itemsList')
                    .innerHTML = '<div id="user_warned_message" style="padding: 0px 10px 10px 10px"><div class="message">' + btn +
                    '<div style="padding: 5px 0px">������������ ������������ � ��������� ������.</div></div></div>' + ge('itemsList')
                    .innerHTML;
            } else if (status == -2) {
...
function generate_message(text, cancel_handler) {
    var buttons =
        '<div style="float: right; margin-left: 10px"><ul class="nNav"><li style="margin: 0px"><b class="nc"><b class="nc1"></b><b class="nc2"></b></b><span class="ncc"><a href="' +
        cancel_handler + '">' + market_photos_cancel + '</a></span><b class="nc"><b class="nc2"></b><b class="nc1"></b></b></li></ul></div>';
...
function photo_file_selected() {
    var new_photo = ge('photo_file_inputs')
        .firstChild;
    if (new_photo.value.length) {
        ...
        new_div.innerHTML = '<table cellpadding="0" cellspacing="0" style="padding: 0"><tr style="padding: 0"><td id="file' + new_index +
            'cell" style="padding: 0"><span id="flag_for_' + new_index + '">&nbsp;</span></td><td style="vertical-align: middle; padding: 0;"><a id="delete_photo_file_link_' +
            new_index + '" href="javascript: delete_photo_file(' + new_index + ')">' + market_photo_delete + '</a></td></tr></table>';
        ge('selected_files')
            .appendChild(new_div);
        ge('photo_file_inputs')
            .removeChild(new_photo);
        ge('file' + new_index + 'cell')
            .insertBefore(new_photo, ge('flag_for_' + new_index));
        new_photo.setAttribute('id', 'file' + new_index);
        new_photo.setAttribute('name', 'file' + new_index);
        selected_files[new_index] = true;
        ge('photo_file_inputs')
            .innerHTML = '<input type="file" id="new_photo" name="new_photo" class="upload" onchange="photo_file_selected()"/>';
        ++photos_count;
        if (photos_count >= 5) {
            ge('adding_photos')
                .style.display = 'none';
...
function show_message_box(cls, msg) {
    ge('messageWrap')
        .style.display = 'block';
    ge('messageWrap')
        .innerHTML = '<div id="' + cls + '">' + msg + '</div>';
    setTimeout(function() {
        (new effects.fader())
        .fade(ge(cls), [0xFFFFFF, 0xDDDDDD], 3000);
    }, 2500);
}
...
function unblacklist_user(user_id) {
    ...
    ajax.onDone = function(obj, text) {
        if (user_id) {
            ge('memberOptions_' + user_id)
                .innerHTML = '<span style="font-weight: bold">������������ ������ �� ������� ������</span>';
            effects.fadeToDld(ge('memberRow_' + user_id));
            setTimeout(function() {
                new effects.fader()
                    .fade(ge('memberRow_' + user_id), [0xFFFFFF, 0xD8DFEA], 3000);
            }, 2500);
    ...
    if (user_id) {
        ...
        ge('removing_progress')
            .style.display = 'inline';
    }
    ajax.post('/market.php', {
        'act': 'a_unblacklist_user',
        'user_id': user_id,
        'page': ge('removing_page')
            .value,
        'hash': ge('hash')
            .value
    });
...
function reload_captcha() {
    ...
    ge('captchaImg')
        .src = '/captcha.php?sid=' + ge('current_sid')
        .value + '&rfrsh=' + Math.random();
    captcha_loading = true;
...
function unspamlist_words(id) {
    ...
    ajax.onDone = function(obj, text) {
        if (id) {
            ge('memberOptions_' + id)
                .innerHTML = '<span style="font-weight: bold">����� ������� �� ����-�������</span>';
            effects.fadeToDld(ge('memberRow_' + id));
            setTimeout(function() {
                new effects.fader()
                    .fade(ge('memberRow_' + id), [0xFFFFFF, 0xD8DFEA], 3000);
            }, 2500);
...
function change_select(item_id, ev) {
    ...
    if (selected_items.length) {
        ...
        ge('delete_warn_link')
            .href = '/market.php?act=delete_warn&items=' + selected_items.join('_') + '&leave=' + not_selected_items.join('_');
    } else {
