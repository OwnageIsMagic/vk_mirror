function saveContacts() {
    ...
    if (ge('forum')
        ...
        params['forum'] = ge('forum')
            .value;
    }
    each(['country', 'city'], function() {
        var value = intval(ge('' + this)
            .value);
        if (value) {
            params['' + this] = value;
        }
    ...
    var doneHandler = function(obj, text) {
        ...
        selData.city = ge('city')
            .value;
        selData.country = ge('country')
            .value;

        contacts.email = ge('email')
            .value;
        contacts.mobile = ge('mobile')
            .value;
        contacts.home = ge('hometel')
            .value;
        contacts.icq = ge('icq')
            .value;
        contacts.website = ge('website')
            .value;
        contacts.nick = ge('nick')
            .value;
        contacts.forum = (ge('forum')
                .value == ge('forum')
                .getAttribute('placeholder')) ? '' : ge('forum')
            .value;
...
function onFriendsLoaded(type, selected) {
    ...
    var html = '<div class="relation_input"><div class="fl_l"><input type="hidden" id="' + id + '" name="' + id +
        '"/></div><div class="fl_r" style="padding-top:3px;"><a href="" onclick="removeRelation(\'' + type + '\', ' + rel_guid + ');return false;">' + global_delete + '</a></div>';
    var t = 0;
    if (withDate) {
        t = selected ? parseInt(selected[3]) : 0;
        html += '<div class="relation_date"><input class="dateField" type="hidden" id="' + id + '_date" value="' + t + '"><div class="bday"><input type="hidden" id="' + id +
            '_day"/></div><div class="bmonth"><input type="hidden" id="' + id + '_month"/></div><div class="byear"><input type="hidden" id="' + id + '_year"/></div></div>';
    }
...
function saveGeneral() {
    ...
    var doneHandler = function(obj, text) {
        ...
        general_data.home_town = ge('home_town')
            .value;
        ...
        ge('status_detail_content')
            .innerHTML = response.relation_text;
...
function savePersonal() {
    ...
    var doneHandler = function(obj, text) {
        ...
        each(['activities', 'interests', 'music', 'movies', 'tv', 'books', 'games', 'quotes', 'about'], function() {
            ge('' + this)
                .value = personal_data['' + this] = winToUtf(response['' + this]);
        ...
        var handlePartnerChange = function(value) {
            ...
            if (intval(value) && uiStatus.val() > 1 && uiStatus.val() < 6) {
                if (intval(value) == general_data.partner) {
                    ...
                    ge('status_help_content')
                        .innerHTML = general_data.status_helps[3 - uiSex.val()];
                    ...
                    ge('status_help_wrap')
                        .style.left = left + 'px';
                    ge('status_help_wrap')
                        .style.paddingLeft = (real_left - left) + 'px';
                ...
                if (value > 1 && value < 6) {
                    update_friends_list(function() {
                        ...
                        if (general_data.prepositions[value][uiSex.val()].length) {
                            ge('status_prep')
                                .innerHTML = general_data.prepositions[value][uiSex.val()];
                            show('status_prep');
                        } else {
                            hide('status_prep');
                        }
        ...
        if (general_data.sex) {
            ...
            if (general_data.status > 1 && general_data.status < 6) {
                if (general_data.prepositions[general_data.status][general_data.sex].length) {
                    ge('status_prep')
                        .innerHTML = general_data.prepositions[general_data.status][general_data.sex];
                    show('status_prep');
                }
                show('status_partner');
            }
...
function showMessage(message, cl) {
    ge('messageWrap')
        .innerHTML = '<div class="' + cl + '">' + message + '</div>';
    show('messageWrap');
    setTimeout(function() {
        animate(ge('messageWrap')
            .firstChild, {
...
function wereChanges() {
    if (act == 'contacts') {
        return intval(ge('country')
                .value) != selData.country || intval(ge('city')
                .value) != selData.city ||
            ge('email')
            .value != contacts.email || ge('mobile')
            .value != contacts.mobile ||
            ge('hometel')
            .value != contacts.home || ge('icq')
            .value != contacts.icq ||
            ge('website')
            .value != contacts.website || ge('nick')
            .value != contacts.nick ||
            (ge('forum')
                .value != contacts.forum && (contacts.forum || ge('forum')
                    .value != ge('forum')
                    .getAttribute('placeholder')));
    } else if (act == 'general') {
        return ge('sex')
            .value != general_data.sex || ge('status')
            .value != general_data.status ||
            ge('partner')
            .value != general_data.partner || ge('bday')
            .value != general_data.bday ||
            ge('bmonth')
            .value != general_data.bmonth || ge('byear')
            .value != general_data.byear ||
            ge('bday_visibility')
            .value != general_data.bday_visibility || ge('political')
            .value != general_data.political ||
            ge('home_town')
            .value != general_data.home_town ||
        ...
        var result = false;
        each(['activities', 'interests', 'music', 'movies', 'tv', 'books', 'games', 'quotes', 'about'], function() {
            result |= (personal_data['' + this].replace(/\x0d/g, '') != ge('' + this)
                .value.replace(/\x0d/g, ''));
        });
        return result;
    }
