...
function show_del_link(type, id) {
    var old_visible = parseInt(ge('is_visible_' + type + '_' + id)
        .value);
    if (old_visible == 0) {
        ge('is_visible_' + type + '_' + id)
            .value = '1';
        if (!parseInt(ge('is_loading_' + type + '_' + id)
                .value)) {
            if (parseInt(ge('is_deleted_' + type + '_' + id)
                    .value))
                ge('a_del_links_' + type + '_' + id)
                .innerHTML = mail_cancel;
            else
                ge('a_del_links_' + type + '_' + id)
                .innerHTML = mail_delete;
        }
    }
}
...
function delete_all_messages(box_title, box_text, peer_id, hash) {
    ...
    window.deleteAllMessagesBox.content(box_text)
        .show();
}

function delete_message(type, id) {
    ge('del_links_' + type + '_' + id)
        .innerHTML = '<img src="/images/upload.gif" />';
    ge('is_loading_' + type + '_' + id)
        .value = '1';
    var outbox = parseInt(ge('is_out_' + type + '_' + id)
        .value);
    var ajax = new Ajax();
    ajax.onDone = function(resp, text) {
        ge('is_loading_' + type + '_' + id)
        ...
        if (text == "error") {
            ...
            ge('del_links_' + type + '_' + id)
                .innerHTML = '<a id="a_del_links_' + type + '_' + id + '" href="javascript: delete_message(\'' + type + '\', ' + id + ')"></a>';
            if (parseInt(ge('is_visible_' + type + '_' + id)
                    .value))
                ge('a_del_links_' + type + '_' + id)
                .innerHTML = mail_delete;
        } else {
            ge('message_' + type + '_' + id)
                .className = "message_deleted";
            ge('del_links_' + type + '_' + id)
                .innerHTML = '<a id="a_del_links_' + type + '_' + id + '" href="javascript: restore_message(\'' + type + '\', ' + id + ')"></a>';
            ge('is_deleted_' + type + '_' + id)
                .value = '1';
            if (parseInt(ge('is_visible_' + type + '_' + id)
                    .value))
                ge('a_del_links_' + type + '_' + id)
