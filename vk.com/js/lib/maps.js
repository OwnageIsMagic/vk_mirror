...
function YCustomZoomControl(offices) {
    this.onAddToMap = function(map, position) {
        this.container = YMaps.jQuery(
            "<div class=\"ymap_zoom_wrap\"><div class=\"ymap_zoom_btn\" id=\"ymap_zoom_wrap_p\" data-z=\"1\">+</div><div class=\"ymap_zoom_btn\" id=\"ymap_zoom_wrap_m\" data-z=\"-1\">-</div></div>"
        );
        this.map = map;
        ...
        this.container.find('.ymap_zoom_btn')
            .bind('click', function() {
    ...
    VKMap: {
        init: function(element, api) {
            vkMaps.load(api, (function() {
                    ...
                    YMaps.Events.observe(yandexMap, yandexMap.Events.Click, (function(map, mouseEvent) {
                            var lat = mouseEvent.getCoordPoint()
                                .getY(),
                                lon = mouseEvent.getCoordPoint()
                                .getX();
                            var _this = this;
        ...
        getZoomLevelForBoundingBox: function(bbox) {
            ...
            var sw = bbox.getSouthWest()
                .toProprietary(this.api);
            var zoom = new YMaps.GeoBounds(ne, sw)
                .getMapZoom(map);
    ...
    Marker: {
        toProprietary: function() {
            ...
            YMaps.Events.observe(ymarker, ymarker.Events.DragEnd, function(ymarker) {
                var latLon = new vkMaps.LatLonPoint()
                    .fromProprietary('yandex', ymarker.getGeoPoint());
...
vkMaps.register('yandex2', {
    VKMap: {
        ...
        addMapTypeControls: function() {
            var TypeLayout = ymaps.templateLayoutFactory.createClass(
                '<div class="places_map_type" style="right:0px;top:0px;position: absolute;backgroundColor:#FFF;padding:0;margin:10px;"><input id="yandex_type_dd" style="padding:0px; margin: 0px;"></input></div>', {
                    build: function() {
            ...
            TypeSelector.prototype = {
                setParent: function(parent) {
                    ...
                    if (parent) {
                        ...
                        this.layout.setParentElement(map.panes.get('controls')
                            .getElement());
                    } else {
    ...
    Marker: {
        toProprietary: function() {
            ...
            ymarker.events.add('dragend', function(e) {
                var latLon = new vkMaps.LatLonPoint()
                    .fromProprietary('yandex2', ymarker.geometry);
        ...
        setMapType: function(type) {
            ...
            switch (type) {
                case vkMaps.VKMap.ROAD:
                    ...
                    break;
                case vkMaps.VKMap.SATELLITE:
                    this.tileLayer = L.mapbox.tileLayer(vkMaps.mapboxSatellite)
                        .addTo(map);
                    break;
                case vkMaps.VKMap.HYBRID:
                    this.tileLayer = L.mapbox.tileLayer(vkMaps.mapboxMixed)
                        .addTo(map);
                    break;
                default:
                    this.tileLayer = L.mapbox.tileLayer(vkMaps.mapboxApiId)
    ...
    Marker: {
        toProprietary: function() {
            ...
            lmarker.on('dragend', function(e) {
                var latLon = new vkMaps.LatLonPoint()
                    .fromProprietary('mapbox', lmarker.getLatLng());
                this.VKMap_marker.location = latLon;
                if (this.VKMap_marker.dragend) {
                    this.VKMap_marker.dragend.fire(latLon);
                }
