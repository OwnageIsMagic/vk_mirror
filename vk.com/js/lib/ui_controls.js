...
createChildClass('Selector', UiControl, {
    ...
    initDOM: function(input, data) {
        ...
        this.container = ce('div', {
            ...
            className: 'selector_container' + (!self.options.autocomplete ? ' dropdown_container' : '') + (self.options.big ? ' big' : '') + (self.options.withIcons ?
                ' with_icons' : '') + (browser.mobile ? ' mobile_selector_container' : '') + (self.options.noArr ? ' dropdown_noarr' : ''),
    ...
    <tr>\
      <td class="selector">\
        ...
        <input type="text" class="selector_input" ' +
                this.readOnly + ' />\
        <input type="hidden" name="' + self.options.resultField + '" id="' + self.options.resultField +
                '" value="" class="resultField">\
        <input type="hidden" name="' + self.options.customField + '" id="' + self.options.customField +
                '" value="" class="customField">\
      </td>' + (self.options.dropdown ? '<td id="dropdown' + this.guid +
                    '" class="selector_dropdown">&nbsp;</td>' : '') +
                '\
    </tr>\
  </table>\
  <div class="results_container">\
    ...
    afterInit: function() {
        ...
        if (this.options.selectedItems !== undefined) {
            if (isArray(this.options.selectedItems)) {
                ...
                each((this.options.selectedItems + '')
                    .split(','),
                    function(i, x) {
    ...
    handleKeyboardEvent: function(e) {
        ...
        switch (e.type) {
            ...
            case 'keypress':
                if (e.which == KEY.RETURN && browser.opera && self.options.enableCustom && (self.options.multiCustom || self.select.selectedItem() === null || self.select
            ...
            case 'keydown':
                switch (e.keyCode) {
                    ...
                    case KEY.DEL:
                        ...
                        break;
                    case KEY.RETURN:
                        if (!browser.opera && self.options.enableCustom && (self.options.multiCustom || self.select.selectedItem() === null || self.select.selectedItem() ===
                                undefined)) {
                            self.select.hide();
                            if (!self.options.noBlur) {
                                self.input.blur();
        ...
        if (self.options.enableCustom && (self.input.value.length || forceVal && forceVal.length)) {
            ...
            if (self._selectedItems.length == 0 || self.options.multiCustom) {
                ...
                if (self.options.multiCustom) {
                    ...
                    if (custom_val.length > 1) multiValue = custom_val.slice(1)
                        .join(',');
    ...
    setOptions: function(new_options) {
        ...
        if (('width' in new_options) || ('autocomplete' in new_options) || ('dropdown' in new_options) || ('placeholder' in new_options) || ('disabledText' in
                new_options)) {
            this.updateInput();
        }
    },
    ...
    initDOM: function(input, options) {
        this.container = ce('div', {
            ...
            innerHTML: '<table cellpadding=0 cellspacing=0><tr><td class="radiobtn"><div class="radiobtn_off"><div></div></div></td><td class="radiobtn_label">' +
                this.options.label + '<input type="radio" id="' + input.id + '" name="' + this.options.resultField + '" value="' + (this.options.checked ?
...
createChildClass('Autosize', UiControl, {
    ...
    updateSize: function(event) {
        ...
        if (self.options.helperTA) {
            ...
            self.helper.innerHTML = trim(replaceChars(value))
                .replace(/<br>$/, '<br>&nbsp;');
...
createChildClass('DropdownMenu', UiControl, {
    ...
    afterInit: function(items, options) {
        ...
        var self = this;
        onDomReady(function() {
            (self.common.pageContainer || window.pageNode)
            .appendChild(self.container);
            var header = self.header;
            var body = self.body;
            var target = self.options.target;
            ge('dd_rows_' + self.guid)
                .appendChild(self.rows);
    ...
    moveTo: function(left, top) {
        ...
        if (this.options.columnsCount && !browser.msie) {
            ...
            var itemsCount = geByTag('a', this.rows)
                .length;
...
createChildClass('Indexer', UiUtil, {
    ...
    indexItem: function(k, v) {
        ...
        for (i = 0; i < this.options.indexkeys.length; i++) {
            ...
            current_words += ' ' + v[this.options.indexkeys[i]].replace(this.options.delimeter, ' ')
                .replace(/<[^>]*>/, '');
        }
        current_words = trim(winToUtf(current_words)
                .toLowerCase())
            .split(/\s+/);
    ...
    search: function(pattern) {
        ...
        pattern = trim(pattern.toLowerCase()
            .replace(this.options.delimeter, ' '));
        ...
        each(self.storage.index[min_pattern.substr(0, self.options.chars)], function(k, v) {
            ...
            for (i = 0; i < self.options.indexkeys.length; i++) {
                current_words += ' ' + item[self.options.indexkeys[i]].replace(self.options.delimeter, ' ')
                    .replace('<b>', '')
                    .replace('</b>', '');
            }
            current_words = winToUtf(current_words)
                .toLowerCase();
...
function Slider(el, options) {
    ...
    if (options.color) {
        setStyle(this.amountEl, {
            backgroundColor: options.color
        });
        setStyle(this.handlerEl, {
            backgroundColor: options.color
        });
    }
    if (options.backColor) {
        setStyle(this.slideEl, {
    ...
    var currHandler = null,
        currHintEl, _this = this;
    ...
    function updateHint() {
        if (_this.options.formatHint) {
            ...
            setStyle(currHintEl, {
                top: handlerPos[1] - hintSize[1] - 9,
                left: Math.round(handlerPos[0] - hintSize[0] / 2 + handlerSize[0] / 2)
            });
    ...
    function onMouseMove(event) {
        ...
        setStyle(_this.amountEl, {
            width: left
        });
        setStyle(currHandler, {
            left: left
        });
        ...
        addEvent(this._els.currCascade, 'click', function(event) {
            ...
            if (item) {
                var res = true,
                    ...
                    item_name = geByClass1('idd_item_name', item)
                    .innerHTML;
                if (sublist.onSelect) {
