var loc = {
    is_opera: (navigator.userAgent.toLowerCase()
        .indexOf('opera') != -1),
    is_ie: (!this.isOpera && (navigator.userAgent.toLowerCase()
        .indexOf('msie') != -1)),
    is_ie7: (!this.isOpera && (navigator.userAgent.toLowerCase()
        .indexOf('msie 7') != -1)),
    is_ff: (navigator.userAgent.toLowerCase()
        .indexOf('firefox') != -1),
    is_gc: (navigator.userAgent.toLowerCase()
        .indexOf('chrome') != -1),
    ...
    init: function() {
        ...
        if (this.use_frame) {
            ...
            frame_div.innerHTML = '<iframe id="history_frame" name="history_frame" src="/blank.html?url=' + hash +
                '"></iframe><form id="history_form" target="history_frame" method="GET" action="/blank.html"><input type="hidden" name="url" value=""/></form>';
