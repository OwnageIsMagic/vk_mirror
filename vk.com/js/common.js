...
function replaceEntities(str) {
    return se('<textarea>' + (str.replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
            .replace(/"/g, '&quot;')) + '</textarea>')
        .value;
}
...
function hasClass(obj, name) {
    obj = ge(obj);
    return obj && (new RegExp('(\\s|^)' + name + '(\\s|$)'))
        .test(obj.className);
}
...
function setStyle(elem, name, value) {
    ...
    if (name == 'opacity') {
        ...
        var isNum = typeof(value) == 'number' && !(/z-?index|font-?weight|opacity|zoom|line-?height/i)
            .test(name);
        if (isNum && value < 0 && (/^(width|height)$/i)
            .test(name)) {
            value = 0; //fix for IE;
        }
...
function showCaptcha(sid, img, onClick, onShow, onHide) {
    ...
    if (window.need_mobile_act == 1 && !ge('please_confirm_mail')) {
        about_mobile_text = global_try_to_activate.replace('{link}', '<a href="javascript: activate_mobile(true)">')
            .replace('{/link}', '</a>');
    ...
    box.content('<div style="text-align: center; height: 76px"><a href="#" id="refreshCaptcha"><img id="captchaImg" class="captchaImg" src="' + img +
        '"/></a><div></div><input id="captchaKey" class="inputText" name="captcha_key" type="text" style="width: 120px; margin: 3px 0px 0px;" maxlength="7"/><img id="captchaLoader" src="' +
        base_domain + 'images/progress7.gif" style="display:none; margin-top: 13px;" /></div>' + about_mobile_text);
    box.show();
...
function attachScript(id, c) {
    var i, new_id = c.substr(c.indexOf('/') + 1, c.indexOf('.') - c.indexOf('/') + 2)
        .replace(/[\/\.]/g, '_');
    var newreqs = [];
    for (reqnum in reqs) {
        req = reqs[reqnum];
        if (req) {
            if (req.running == 0) {
                ge('req' + req.num)
                    .parentNode.removeChild(ge('req' + req.num));
                reqs[reqnum] = null;
            } else {
                newreqs[reqnum] = req;
            }
...
function MessageBox(options) {
    ...
    boxContainer = ce('div', {
        ...
        innerHTML: '<div class="box_layout"><div class="box_title_wrap">' + x_button + full_page_button +
            '<div class="box_title"></div></div><div class="box_body" style="' + options.bodyStyle +
            '"></div><div class="box_controls_wrap"><div class="box_controls"></div></div></div>'
    }, {
        display: 'none'
    });
    ...
    if (options.progress) {
        boxControls.innerHTML = '<img src="' + base_domain + 'images/upload' + (window.devicePixelRatio >= 2 ? '_2x' : '') + '.gif" width="32" height="8" id="' + options.progress +
            '" style="display: none" />';
...
function TabbedBox(curTab, tabs, options) {
    ...
    var mb = options.onDone ? new AlertBox(options.title || '', '', options.onDone, extend(options, {
        ...
        contents = {},
        scripts = {},
        contentBox, tabsBox, tabHeader, tabFooter, w, html = '<div class="mb_header">' + (options.header || '') +
        '</div><div class="mb_tab_header"><div class="mb_tabs"><div class="mb_tabs_list fl_l">',
        tmp = {},
        curContent = '';
    options = extend({
        showOneTab: false,
    ...
    function sh(o, m, c) {
        return noTab ? '' : '<div class="mb_shadow' + (c || '') + '" style="opacity:' + (o / 100) + ';filter:alpha(opacity=' + o + ');margin-top:' + m + 'px;width:' + w +
            'px"></div>';
    }
    ...
    html += '</div><div class="fl_r mb_search">' + (options.search || '') + '</div><div class="clear"></div></div>' + sh(30, 0) + sh(11, 1) + sh(7, 2) + sh(3, 3) +
        '</div><div class="mb_tab_content" style="' + (options.tabStyle || '') + '"></div><div class="mb_tab_footer">' + sh(3, -4) + sh(7, -3) + sh(11, -2) + sh(30, -1) + sh(80, 0,
            ' mb_shadow_last') + '</div>';
    mb.content(html);
...
function changeLang(forceAll) {
    ...
    langBox.setOptions(options)
        .addControlsText('');
    langBox.removeButtons()
        .addButton({
            label: getLang('box_close'),
            onClick: function() {
                langBox.hide(200)
...
(function() {
    ...
    window.checkTextLength = function(max_len, val, warn, nobr, display) {
        ...
        var n_len = replaceChars(val, nobr)
            .length;
...
function im_popup(peer_id) {
    ...
    var dom = window.loc_host.toString()
...
TopSearch = {
    ...
    tsListCache: function(q, listName) {
        ...
        if (q) {
            ...
            for (i in queries) {
                query = queries[i];
                if (cached = TopSearch[listCache][' ' + query.charAt(0)
                        .toLowerCase()
                    ]) {
    ...
    row: function(mid, href, photo, name, online, re, type) {
        ...
        if (mid > 0 && mid < 2e9) {
            photoEvents =
                'onmousemove="TopSearch.itemOver(this.parentNode, 2, event);"  onmouseout="TopSearch.itemOver(this.parentNode, 1, event);" onclick="event.cancelBubble = true; return TopSearch.select(this.parentNode, event);"';
            peer = mid;
        }
        if (type == 'hint') {
            style = 'ts_hint';
        }
        return '<a href="' + href + '" class="ts_contact ' + style + ' clear_fix' + (online ? (online != 1 ? ' ts_contact_mobile' : ' ts_contact_online') : '') +
            '" id="ts_contact' + mid + '" onclick="return TopSearch.select(this, event, ' + peer +
            ');" onmousedown="event.cancelBubble = true;" onmouseover="TopSearch.itemOver(this, 1, event);"  onmouseout="TopSearch.itemOver(this, 0, event);"><span class="ts_contact_photo fl_l" ' +
            photoEvents + '><img src="' + photo + '"/></span><span class="ts_contact_name fl_l">' + name + '</span><span class="ts_contact_status fl_l"></span></a>';
    },
    prepareRows: function(q) {
        ...
        if (q) {
            ...
            html.push('<a href="/search?c[section]=auto&c[q]=' + encodeURIComponent(q) +
                ...
                getLang('top_search')
                .replace('{query}', '<span class="ts_query">' + clean(show_q) + '</span>') + '</span><span class="ts_contact_status fl_l"></span></a>');
        }

        TopSearch.friendsRows = tsWrap.innerHTML = html.reverse()
            .join('');
