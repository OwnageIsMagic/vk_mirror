...
function initMilitaryRow(military, elem) {
    ...
    g('military' + military.id)
        .style.display = 'block';
    if (intval(military.country)) {
        g('details' + military.id)
            .style.display = 'block';
    }
    ...
    if (!isVisible('militaries')) {
        ...
        if (militariesCount) {
            for (var i = 0; i < militaries.length; ++i) {
                ge('militaries')
                    .appendChild(genMilitaryRow(militaries[i].id));
...
function addMilitary() {
    ...
    ge('militaries')
        .appendChild(genMilitaryRow(new_military.id));
...
function deleteMilitary(id) {
    --militariesCount;
    show('add_military_link');
    if (intval(ge('unit' + id)
            .value)) {
        hide('content' + id);
        ge('deleted' + id)
            .firstChild.innerHTML = lang.profileEdit_military_will_be_deleted;
        show('deleted' + id);
    } else {
        ...
        militaries.pop();
        ge('military' + id)
            .parentNode.removeChild(ge('military' + id));
...
function saveMilitaries() {
    ...
    for (var i = 0; i < militaries.length; ++i) {
        ...
        params['id' + i] = id;
        if (isVisible('content' + id) && intval(ge('unit' + id)
                .value)) {
            params = addFields(params, id, i, 'country', 'unit', 'start', 'finish');
            if (ge('unit' + id + '_custom')
                .value.length) {
                params['unit' + i + '_custom'] = ge('unit' + id + '_custom')
                    .value;
            }
        } else {
            params['deleted' + i] = 1;
    ...
    var doneHandler = function(obj, text) {
        ...
        for (var i = 0; i < to_remove.length; ++i) {
            ...
            ge('military' + militaries[index].id)
                .parentNode.removeChild(ge('military' + militaries[index].id));
            militaries[index] = militaries[militaries.length - 1];
...
function checkChanges(showBox) {
    ...
    for (var i = 0; i < militaries.length; ++i) {
        ...
        } else if (militaries[i].id < 0 && isVisible('content' + militaries[i].id) && intval(ge('unit' + militaries[i].id)
                .value)) {
            military_change = true;
        }
    }
