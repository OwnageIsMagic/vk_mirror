...
function showError(container, text) {
    var old_text = ge(container + '_old') ? ge(container + '_old')
        .innerHTML : ge(container)
        .innerHTML;
    ge(container)
        .innerHTML = '<span class="error">' + text + '</span><div id="' + container + '_old" style="display: none">' + old_text + '</div>';
    ge(container)
...
function noEmailReg() {
    no_email_reg = true;
    ge('email_details')
        .innerHTML = lang_reg_no_email_desc;
    ge('regemail')
        .value = '';
    ge('emailLabel')
        .innerHTML = lang_reg_login;
    ge('regemail')
        .focus();
    return false;
}
...
function prepareParams() {
    ...
    result.first_name = ge('first_name')
        .value;
    result.last_name = ge('last_name')
        .value;
    if (!/^\s*$/.test(hash)) {
        result.hash = hash;
        if (own_password) {
            result.regpass = ge('regpass')
                .value;
        }
        if (own_email) {
            result.regemail = ge('regemail')
                .value;
        }
    } else {
        result.regemail = ge('regemail')
    ...
    result.sex = intval(ge('sex')
        .value);
    timezone_offset = new Date()
        .getTimezoneOffset();
        ...
        var options = {
            onSuccess: function(obj, text) {
                ...
                if (response.error) {
                    ...
                    ge('email_after_reg')
                        .value = response.mail ? response.mail : reg_login;
                    ge('pass_after_reg')
                        .value = response.pass ? response.pass : reg_pass;
                    ge('login_after_reg')
                        .submit();
