...
function updatePage(res, text) {
    ...
    ge('audio_summary')
        .innerHTML = res.summary;
    ge('header')
        .getElementsByTagName('h1')[0].innerHTML = res.title;
    ge('pagesTop')
        .innerHTML = res.pages;
    ge('pagesBottom')
        .innerHTML = res.pages;
    ...
    var filters = ge('side_filters')
        .childNodes;
    each(filters, function(i, v) {
        if ((v.id || '')
            .substr(0, 9) != 'list_item') return;
...
function switchTab(n) {
    ...
    for (var i = 0; i < tabs.length; i++) {
        if (obj.id == tabs[i]) {
            ge(tabs[i])
                .className = "t_filter_selected";
        } else {
            ge(tabs[i])
                .className = "t_filter_off";
        }
    }
    current_tab = n;
}
...
function ListEditBox() {
    ...
    function initBox() {
        ...
        listBox.content('<div class="alist_outer_wrap">' +
            ...
            '<div id="alist_wrap"><span id="list_edit_box_alist_name"></span><a id="list_change_name" href="#">' + getLang('audio_change_album_name') +
            '</a></div><div id="alist_edit_wrap"><input id="alist_name_edit" ' + input_css_hack + ' placeholder="' + getLang('audio_enter_album_name') +
            '" class="inputText" /></div></div><div class="audio_list_top">' +
    ...
    function onAudioClick(obj, n) {
        ...
        ge('list_edit_box_list_selected_count')
            .innerHTML = selectedAudios.length;
    }
    ...
    function showList(windowTitle, listId, listName, selectedIds) {
        ...
        ge('list_edit_box_audio_list_lookup')
            .value = '';
        placeholderSetup('list_edit_box_audio_list_lookup');
        if (!search_def) search_def = ge('list_edit_box_audio_list_lookup')
            .value.toLowerCase();
        ge('list_edit_box_list_selected_count')
            .innerHTML = 0;
        ge('list_edit_box_list_tab')
            .className = 't_filter_selected';
        ge('list_edit_box_list_selected_tab')
            .className = 't_filter_off';
        this.listId = listId;
            ...
            return new PagedList('list_edit_box_alist_data', data, {
                getRow: function(row) {
                    ...
                    return '<div class="audio_row" id="audio_choose' + row[0] + '">' +
                        '<div onmouseout="alistOff(this)" onmousemove="alistOver(this)" onclick="onAudioClick(this,' + row[0] + ')" class="alist_cell" id="alist' +
                        row[0] + '">' +
                ...
                setPages: function(page, pages, side) {
                    ...
                    html.push('</ul>');
                    if (ge('af_pages_' + side)) ge('af_pages_' + side)
                        .innerHTML = html.join('');
                },
                onStart: function() {
                    ge('af_pages_top')
                ...
                onEnd: function() {
                    animate(ge('list_edit_box_alist_data')
                        .parentNode, {
                            scrollTop: 0
                        }, 200);
                },
                emptyRow: function(search) {
                    var txt = getLang('audio_search_not_found')
                        .replace('{search}', replaceChars(search[0]));
        ...
        if (!audio_data) {
            ge('list_edit_box_alist_data')
                .innerHTML = '<div class="listProgress"><img src="http://vkontakte.ru/images/progress7.gif"></div></div>';
            Ajax.Post({
                url: 'audio.php',
                query: {
    ...
    var getFilter = function() {
        var value = ge('list_edit_box_audio_list_lookup')
            .value.toLowerCase();
            ...
            if (elem) {
                ...
                animate(scrollElem, {
                    ...
                    try {
                        ...
                        else window.eval(elem.getAttribute('onclick')
                            .replace('return ', ''));
                    } catch (error) {}
                });
            }
