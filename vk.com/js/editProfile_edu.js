...
function genUniRow(uni_id, no_delete) {
    ...
    elem.innerHTML = '<div id="uni_content' + uni_id + '">' +
        ...
        genOneRow('u', 'university', uni_id, lang.select_university, '',
            '<span class="add_item_link fl_l" id="u_add_uni_to_db' + uni_id + '"><b><a href="#add" onclick="return addUniversityToDB(' + uni_id + ')">' + lang.profileEdit_uni_not_in_db +
            '</a></b></span>') +
        genOneRow('u', 'faculty', uni_id, lang.select_faculty, '',
            '<span class="add_item_link fl_l" id="u_add_fac_to_db' + uni_id + '"><b><a href="#add" onclick="return addFacultyToDB(' + uni_id + ')">' + lang.profileEdit_fac_not_in_db +
            '</a></b></span>') +
        genOneRow('u', 'chair', uni_id, lang.select_chair, '',
            '<span class="add_item_link fl_l" id="u_add_chair_to_db' + uni_id + '"><b><a href="#add" onclick="return addChairToDB(' + uni_id + ')">' + lang.profileEdit_chair_not_in_db +
            '</a></b></span>') +
...
function genOneSchoolRow(id) {
    ...
    return '<div class="row" id="row_' + key + '"><div class="label fl_l ta_r">' + lang.select_school + '</div>' +
        ...
        '</div>' +
        '<span class="add_item_link fl_l" id="s_add_school_to_db' + id + '"><b><a href="#add" onclick="return addSchoolToDB(' + id + ')">' + lang.profileEdit_school_not_in_db +
        '</a></b></span>' +
        '</div>';
}
...
function addToDB(obj, title, id) {
    ...
    addBox.content('<div class="box_loader"></div>')
        .show();
    var ajax = new Ajax();
...
function filterUni() {
    ...
    ge('summary')
        .innerHTML = lang.profileEdit_can_edit_unis;

    if (!isVisible('primary_uni')) {
        ge('primary_uni')
            .appendChild(genUniRow(primary_uni.id, true));
        ...
        if (unisCount) {
            for (var i = 0; i < unis.length; ++i) {
                ge('unis')
                    .appendChild(genUniRow(unis[i].id));
...
function filterSchool() {
    ...
    ge('summary')
        .innerHTML = lang.profileEdit_can_edit_schools;
    ...
    if (!isVisible('schools')) {
        ...
        if (schoolsCount) {
            for (var i = 0; i < schools.length; ++i) {
                ge('schools')
                    .appendChild(genSchoolRow(schools[i].id));
...
function addUniversity() {
    ...
    ge('unis')
        .appendChild(genUniRow(new_uni.id));
...
function addSchool() {
    ...
    ge('schools')
        .appendChild(genSchoolRow(new_school.id));
...
function deleteUniversity(id) {
    --unisCount;
    show('add_uni_link');
    if (intval(ge('u_university' + id)
            .value) || (id > 0)) {
        hide('uni_content' + id);
        ge('uni_deleted' + id)
            .firstChild.innerHTML = lang.profileEdit_uni_will_be_deleted;
        show('uni_deleted' + id);
    } else {
        ...
        unis.pop();
        ge('uni' + id)
            .parentNode.removeChild(ge('uni' + id));
        if (unis.length == 0) {
            hide('unis');
        }
    }
...
function deleteSchool(id) {
    --schoolsCount;
    show('add_school_link');
    if (intval(ge('s_school' + id)
            .value) || (id > 0)) {
        hide('school_content' + id);
        ge('school_deleted' + id)
            .firstChild.innerHTML = lang.profileEdit_school_will_be_deleted;
        ...
        schools.pop();
        ge('school' + id)
            .parentNode.removeChild(ge('school' + id));
        if (schools.length == 0) {
            addSchool();
        }
    }
...
function addFields() {
    ...
    for (var i = 4; i < arguments.length; ++i) {
        if (intval(ge(prefix + '_' + arguments[i] + id)
                .value)) {
            params[key + arguments[i]] = ge(prefix + '_' + arguments[i] + id)
                .value;
        }
    }
    return params;
...
function addUniParams(params, uni, key) {
    ...
    if (isVisible('uni_content' + uni.id) && intval(ge('u_university' + uni.id)
            .value)) {
...
function addSchoolParams(params, school, key) {
    ...
    if (isVisible('school_content' + school.id) && intval(ge('s_school' + school.id)
            .value)) {
    ...
    if (isVisible('uni')) {
        ...
        doneHandler = function(response) {
            ...
            for (var i = 0; i < to_remove.length; ++i) {
                ...
                ge('uni' + unis[index].id)
                    .parentNode.removeChild(ge('uni' + unis[index].id));
                unis[index] = unis[unis.length - 1];
        ...
        doneHandler = function(response) {
            ...
            for (var i = 0; i < to_remove.length; ++i) {
                var index = to_remove[i];
                ge('school' + schools[index].id)
                    .parentNode.removeChild(ge('school' + schools[index].id));
                schools[index] = schools[schools.length - 1];
                for (var j = i + 1; j < to_remove.length; ++j) {
...
function updateSchool(old_school, new_school, parent) {
    ...
    old_school.spec = ge('s_spec' + old_school.id + '_custom')
        .value;
...
function schoolChanged(school) {
    return !isVisible('school_content' + school.id) ||
        ...
        school.school_class != school.uiClass.val() || school.spec != ge('s_spec' + school.id + '_custom')
        .value;
}
