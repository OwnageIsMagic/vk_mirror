...
function showEditOfficerBox(user_id, group_id) {
    ge('boxTitle')
        .innerHTML = ge('officer' + user_id + '_name')
        .innerHTML;
    ge('boxMessage')
        .innerHTML = "<table cellpadding=0><tr><td valign=top rowspan=2 style='padding-top: 5px'>" + groups_enter_post +
        "<td><td valign=top><input type='text' style='width: 182px' id='position' value='" + ge('position' + user_id)
        .value.replace(/'/g, "&#39;") + "'><br><div style='margin-top: 2px'><input style='margin: 1px 2px 0 0; padding: 0px;' type='checkbox' id='admin' " + ge('is_admin' +
            user_id)
        .value + ">" + groups_group_admin + "</div></td></tr></table>";
    ge('button1')
        .innerHTML = groups_edit;
    ge('button2')
        .innerHTML = groups_cancel;
    showBoxOld(function() {
        if (ge('position')
            .value.length > 0) {
            var ajax = new Ajax();
            var position = ge('position')
                .value;
            var admin = ge('admin')
                .checked ? 1 : 0;
            var admin2 = ge('admin')
            ...
            ge('boxMessage')
                .innerHTML = '<div style="text-align: center"><img valign="middle" src="/images/progress7.gif"></div>';
        } else {
            ge('position')
                .focus();
        }
...
function showDeleteOfficerBox(user_id, group_id) {
    ge('boxTitle')
        .innerHTML = groups_delete_manager;
    ge('boxMessage')
        .innerHTML = groups_sure_delete_manager;
    ge('button1')
        .innerHTML = groups_yes;
    ge('button2')
        .innerHTML = groups_no;
    showBoxOld(function() {
        var ajax = new Ajax();
        ...
        ajax.onDone = function() {
            ge('officer' + user_id)
                .parentNode.removeChild(ge('officer' + user_id));
            hideBoxOld();
        };
        ajax.post('/groups_ajax.php', {
            'act': 'adeleteofficer',
...
function showEditLinkBox(link_id, group_id) {
        ...
        onClick: function() {
            if (ge('position')
                ...
                Ajax.Post({
                    ...
                    onDone: function() {
                        ge('pos' + link_id)
                            .innerHTML = ', ' + replaceChars(position);
                        ge('position' + link_id)
                            .value = position;
                        mb.hide();
                    }
    ...
    var text = ge('position' + link_id)
        .value.replace(/'/g, "&#39;")
        .replace(/^, /, '');
    mb.content('<table cellpadding="0"><tr><td valign="top" rowspan="2" style="padding-top: 5px">' + groups_enter_name +
            '<td><td valign="top"><input type="text" style="width: 182px" id="position" value="' + text + '"></td></tr></table>')
        .show();
    ge('position')
        .focus();
    return false;
...
function showDeleteLinkBox(link_id, group_id) {
    ...
    mb.addButton({
        ...
        onClick: function() {
            Ajax.Post({
                ...
                onDone: function() {
                    ge('link' + link_id)
                        .parentNode.removeChild(ge('link' + link_id));
                    mb.hide();
                }
            });
            mb.content('<div style="text-align: center"><img valign="middle" src="/images/progress7.gif"></div>');
    ...
    mb.content(groups_sure_delete_link)
        .show();
}
...
function addToInvite(id) {
    ...
    node.id = "toinvite" + id;
    node.innerHTML = "<div class='mName'><a href='/id" + id + "'>" + ge("friend" + id)
        .innerHTML + "</a></div><div class='mOption'><a href='#' onClick='return delToInvite(" + id + ");'>" + groups_cancel2 + "</a></div><input type='hidden' name='" + id +
        "' value='1'>";
...
function doAjax(uid, gid, act, message, fadefunc) {
    ...
    ajax.onDone = function() {
        ...
        ge('memberOptions_' + uid)
            .innerHTML = message;
        setTimeout(function() {
            new effects.fader()
                .fade(ge('memberRow_' + uid), [0xFFFFFF, 0xD8DFEA], 3000);
        }, 2500);
    };
    ge('memberOptions_' + uid)
        .innerHTML = '<img style="overflow: hidden;" valign="middle" src="/images/upload_mini.gif"> ';
    ajax.post('/groups_ajax.php', {
        'act': act,
        'id': uid,
        'gid': gid
...
function showAddOfficerBox(user_id, group_id, hash, success_style) {
    ge('boxTitle')
        .innerHTML = groups_assigning_manager;
    ge('boxMessage')
        .innerHTML = "<div id='addOfficerFormCont'><table cellpadding=0><tr><td valign=top rowspan=2 style='padding-top: 5px'>" + groups_enter_post +
        "<td><td valign=top><input type='text' style='width: 182px' id='position' name='position" + user_id +
        "'><br><div style='margin-top: 2px'><input style='margin: 1px 2px 0 0; padding: 0px;' type='checkbox' id='admin' name='admin" + user_id + "'>" + groups_group_admin +
        "</div></td></tr></table></div>";
    ge('button1')
        .innerHTML = groups_assign;
    ge('button2')
        .innerHTML = groups_cancel;
    showBoxOld(function() {
        if (ge('position')
            ...
            ge('boxMessage')
                .innerHTML = "<div style='text-align: center; height: 13px'><img valign='middle' src='images/progress7.gif'></div>";
        } else {
            ge('position')
                .focus();
        }
...
function hideReplyBox(full_id) {
    ...
    if (el.show) {
        if (!status) {
            ...
            if (ge('reply_to_name')) ge('reply_to_name')
                .innerHTML = '';
        } else {
            hide('submit_status');
        }
...
function postGroupStatus(to_id, hash) {
    ...
    var type = ge('wall_type') ? ge('wall_type')
        .value : 2;
    var reply_to = ge('group_status')
        .value == 1 ? -1 : 0;
    var params = {
        act: 'a_post_wall',
    ...
    Ajax.Send('wall.php', params, function(o, t) {
        ...
        if (type == 2) {
            ge('status_top_box_wrap')
                .innerHTML = wallHistory[0];
        } else {
            ge('status_top_box_wrap')
                .innerHTML = wallHistory[1];
        }
        el.style.color = '#777';
...
function AutosizeMod(ta, options) {
    ...
    function updateSize(he) {
        return function(e) {
            ...
            oldValue = value;
            helper.innerHTML = trim(replaceChars(value))
                .replace(/<br>$/, '<br>&nbsp;');
...
function postWall(to_id, hash, top_id, type) {
    ...
    show('progr2');
    if (type === undefined) type = ge('wall_type') ? ge('wall_type')
        .value : 0;
    ...
    var params = {
        ...
        message: txt,
        to_id: to_id,
        reply_to: (ge('reply_to' + full_id) || {})
            .value,
        top_id: top_id,
        type: type
    };
    if (full_id) {
        params.start_id = (ge('start_reply' + full_id) || {})
            .value;
    }
    var callback = function(o, t) {
        ...
        if (r.replies) {
            ge('replies' + full_id)
                .innerHTML = r.replies;
            if (type == 2) {
                wallHistory[0] = ge('status_top_box_wrap')
                    .innerHTML;
                wallHistory[1] = false;
            } else {
                wallHistory[0] = false;
                wallHistory[1] = ge('status_top_box_wrap')
                    .innerHTML;
            }
        } else {
            wallHistory[1] = r.wall;
...
function toWallComments(to_id, wall_hash, status_hash) {
    ...
    var onDone = function() {
        ...
        setupReply();
        ge('right_wall_link')
            .innerHTML = '<a href="" onclick="cancelEvent(event);return toStatusBox(' + to_id + ', \'' + wall_hash + '\', \'' + status_hash + '\');return false;">' + getLang(
                'group_to_group_posts') + '</a>';
    };
    if (!wallHistory[1]) {
        Ajax.Send('wall.php', {
            act: 'a_get_wall',
...
function toStatusBox(to_id, wall_hash, status_hash) {
    ...
    var onDone = function() {
        ...
        setupReply();
        ge('right_wall_link')
            .innerHTML = '<a href="" onclick="cancelEvent(event);return toWallComments(' + to_id + ', \'' + wall_hash + '\', \'' + status_hash + '\');return false;">' +
            getLang('groups_to_all_posts') + '</a>';
    };
    if (!wallHistory[0]) {
        Ajax.Send('wall.php', {
            act: 'a_get_wall',
...
function deleteStatus(cid, oid, hash) {
    ...
    Ajax.Send('/wall.php', {
        ...
        function(o, t) {
            restoreCache[full_id] = ge('status' + cid)
                .innerHTML;
            ge('status' + cid)
                .innerHTML = t;
        }
...
function replyTo(reply_id, msg_id, to_id, to_href, to_name, top_id) {
    ...
    //if (!full_id) {
    ge('reply_to_name' + full_id)
        .innerHTML = '<a href="' + to_href + '">' + to_name + '</a>';
...
onDomReady(function() {
    ...
    if (ge('wall_preload') && ge('wall_preload')
        .innerHTML) {
        var sd = ge('wall_default');
        if (sd && sd.value == 1) {
            wallHistory[1] = ge('status_top_box_wrap')
                .innerHTML;
            wallHistory[0] = ge('wall_preload')
                .innerHTML;
        } else {
            wallHistory[0] = ge('status_top_box_wrap')
                .innerHTML;
            wallHistory[1] = ge('wall_preload')
                .innerHTML;
        }
        ge('wall_preload')
            .innerHTML = '';
