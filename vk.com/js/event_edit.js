...
datePicker = {
    ...
    init: function(el, pref, formId, w, mode) {
        ...
        if (!this.calendarDiv) {
            ...
            ge(this.formId)
                .appendChild(this.calendarBox);
            this.calendarBox.style.display = "none";
            this.calendarBox.style.position = "absolute";
            this.calendarBox.innerHTML =
                "<iframe id='calendarFrame' style='border:0;position:absolute;z-index:100'></iframe><div id='calendarDiv' style='position:absolute;z-index:200'></div>";
        ...
        if (!w) w = 145;
        html.push("<input readonly style='width:" + w + "px;padding-left:6px;cursor:pointer;' class='inputText' onclick='datePicker.click(\"" + pref +
            "\")' onblur='datePicker.blur()' type='text' name='" + id + "' id='" + id + "'/>");
        html.push("<span style='height:20px;margin-right:-7px'><img src='/images/calendar.gif' style='margin:3px 6px -" + ((core.isIE) ? "1" : "3") +
            "px -18px;cursor:pointer;' onclick='datePicker.click(\"" + pref + "\")' id='" + imgId + "'/></span>");
    ...
    */
    ...
    update: function(pref, date, mode) {
        ...
        if (mode == "m") {
            ge(pref + "_date_input")
                .value = ebrowse_mn[date["m"] - 1] || "";
        } else {
            ge(pref + "_date_input")
                .value = date["d"] + " " + this.months[date["m"] - 1] + " " + date["y"];
        }
        datePicker.hide();
