    ...
    if (!VK.Cookie) {
        VK.Cookie = {
            ...
            setRaw: function(val, ts, domain, time) {
                ...
                var exp = time ? (new Date()
                    .getTime() + time * 1000) : ts * 1000;
                rawCookie += (val && ts === 0 ? '' : '; expires=' + new Date(exp)
                    .toGMTString());
                rawCookie += '; path=/';
            ...
            set: function(session, resp) {
                if (session) {
                    this.setRaw(this.encode(session), session.expire, window.location.hostname, (resp || {})
                        .time);
                } else {
                    this.clear();
                }
        ...
        VK.Api = {
            ...
            checkMethod: function(method, params, cb, queryTry) {
                ...
                if (m == 'wall.post' || m == 'activity.set') {
                    ...
                    var query = VK._protocol + '//vk.com/al_apps.php?act=wall_post_box&widget=4&method=' + m + '&aid=' + parseInt(VK._apiId, 10) + '&text=' +
                        encodeURIComponent(text);
                    if (m == 'wall.post') {
                        query += '&owner_id=' + parseInt(params.owner_id || 0, 10) + '&attachments=' + (params.attachments || params.attachment || '') + '&publish_date=' +
                            (params.publish_date || '');
                    }
                    var method_access = '_' + (Math.random())
                        .toString(16)
                        .substr(2);
                ...
                if (m == 'messages.allowmessagesfromgroup') {
                    var method_access = '_' + (Math.random())
                        .toString(16)
                        .substr(2);
                    var query = VK._protocol + '//vk.com/widget_allow_messages_from_community.php?act=allow_box&group_id=' + parseInt(params.group_id, 10) + '&app_id=' +
                        parseInt(VK._apiId, 10) + '&method_access=' + method_access;
            ...
            queryLength: function(query) {
                var len = 100,
                    i; // sid + sig
                for (i in query) {
                    len += i.length + encodeURIComponent(query[i])
                        .length + 1;
                }
                return len;
            }
            ...
            revokeGrants: function(cb) {
                ...
                if (VK._session && VK._session.sid) {
                    var url = 'https://login.vk.com/?act=openapi&oauth=1&aid=' + parseInt(VK._apiId, 10) + '&location=' + encodeURIComponent(window.location.hostname) +
                        '&do_logout=1&token=' + VK._session.sid;
...
if (!VK.UI) {
    VK.UI = {
        ...
        button: function(el, handler) {
            ...
            html = (
                '<table cellspacing="0" cellpadding="0" id="openapi_UI_' + index + '" onmouseover="VK.UI._change(1, ' + index + ');" onmouseout="VK.UI._change(0, ' +
                index + ');" onmousedown="VK.UI._change(2, ' + index + ');" onmouseup="VK.UI._change(1, ' + index +
        ...
        _change: function(state, index) {
            var row = document.getElementById('openapi_UI_' + index)
                .rows[0];
...
if (!VK.Widgets) {
    ...
    VK.Widgets.showBoxUrl = function(domain, url) {
        domain = (domain || VK._protocol + '//vk.com')
            .replace(/\/?\s*$/, '');
        url = url.replace(/^\s*\/?/, '');
        return domain + '/' + url;
    };
    ...
    VK.Widgets.Like = (function(Like) {
        ...
        Like = function(objId, options, page) {
            ...
            var verticalBtnHeightWidth = {
                ...
                width = autoWidth ? 153 : (type == 'full' ? Math.max(200, options.width || 350) : (type == 'button' ? 180 : (type == 'mini' ? 115 :
    ...
    VK.Widgets.Community = VK.Widgets.Group = function(objId, options, gid) {
        ...
        options.mode = parseInt(options.mode, 10)
            .toString();
        var params = {
                gid: gid,
    ...
    VK.Widgets.Ads = function(objId, options, paramsExtra) {
        ...
        adsParamsLocal.ads_ad_unit_width = (parseInt(adsParams.ads_ad_unit_width) || adsParams.ads_ad_unit_width === 'auto' && adsParams.ads_ad_unit_width_auto ||
            adsParamsDefault.ads_ad_unit_width);
        adsParamsLocal.ads_ad_unit_height = (parseInt(adsParams.ads_ad_unit_height) || adsParams.ads_ad_unit_height === 'auto' && adsParams.ads_ad_unit_height_auto ||
