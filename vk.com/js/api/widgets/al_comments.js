    ...
    switchSection: function(section) {
        ...
        ajax.post('al_widget_comments.php', WComments.getSectionParams(), {
            onDone: function(options, rows) {
                ...
                replaceClass(cur.heightEl,
                    'wcomments_section_posts wcomments_section_admin_browse wcomments_section_admin_bl wcomments_section_admin_updates wcomments_section_browse',
                    'wcomments_section_' + section);
                cur.options.fixed_height && setStyle(cur.postsOuterEl, {
                    maxHeight: cur.options.fixed_height - getSize(geByClass1('_wcomments_head'))[1] - getSize(cur.section == 'posts' || cur.section ==
                        'browse' ? geByClass1('_wcomments_form') : geByClass1('_wcomments_admin_tabs'))[1]
                });
        ...
        ajax.post('al_widget_comments.php', {
            ...
            onDone: function(response, earliest_post_id) {
                earliest_post_id && each(geByClass('wcomments_post', ge('wcomments_posts'), 'div'), function() {
                    !this.id.indexOf('post' + mid) && this.id.split('_')[1] >= earliest_post_id && this.id != 'post' + post_id && isVisible(this) &&
                        hide(this);
                });
                ge('post_del' + post_id)
                    .innerHTML = response;
                ...
                if (isVisible('reply_link' + post_id)) {
                    ...
                    if (cur.wallMyOpened[post_id]) {
                        ...
                        var headerEl = geByClass1('wr_header', repliesEl, 'a'),
                            ...
                            .length + 1,
                            total = shown;
                        if (headerEl) total = intval(headerEl.getAttribute('offs')
                            .split('/')[1]) + 1;
        ...
        switch (file) {
                ...
                extend(Wall, {
                                        ...
                                        ajax.post('al_widget_comments.php', params, {
                                            onDone: function(options, rows) {
                                                if (cur.section == 'posts' && cur.Rpc) {
                                                    cur.Rpc.callMethod('publish', 'widgets.comments.new_comment', options.count,
                                                        options.last_comment, options.date, options.full_hash, options.pageId
                                                    );
                                                }
                    ...
                    sendReply: function(post, ev, options) {
                        ...
                        var message = Emoji.emojiToHTML(clean(params.message), true),
                            toName = params.reply_to_user < 0 ? getLang('wall_replied_to_group') : cur.options.reply_names[params.reply_to_user] && cur.options
                            .reply_names[params.reply_to_user][0],
                        ...
                        ajax.post('al_wall.php', {
                            ...
                            onDone: function(msg, res, need_confirm) {
                                ...
                                if (res && cur.section == 'posts' && cur.Rpc) {
                                    cur.Rpc.callMethod('publish', 'widgets.comments.delete_comment', res.count, res.last_comment, res.date, res.full_hash,
                                        res.pageId);
                                }
            ...
            case 'ui_media_selector.js':
                window.MediaSelector = function(lnk, previewId, mediaTypes, opts) {
                    ...
                    if (multi) {
                        previewEl.innerHTML = '<div id="page_pics_preview' + lnkId +
                            '" class="page_pics_preview media_preview clear_fix"></div><div id="page_dpics_preview' + lnkId +
                            '" class="page_pics_preview post_thumbed_media page_media_sortable media_preview clear_fix"></div><div id="page_docs_preview' + lnkId +
                            '" class="page_docs_preview post_thumbed_media page_media_sortable media_preview clear_fix"></div><div id="page_pdocs_preview' + lnkId +
                    ...
                    addMedia = {
                        ...
                        chooseMedia: function(type, media, data, url, noboxhide, isGraffiti) {
                            ...
                            if (addMedia.attachCount() >= limit && data.upload_ind === undefined && type !== 'postpone' && type !== 'mark_as_ads' || geByClass1(
                                    'medadd_c_market', docsEl)) {
                                if (multi) {
                                    return false;
                                } else {
                            ...
                            switch (type) {
                                case 'graffiti':
                                    ...
                                    preview = '<div class="fl_l page_preview_graffiti"><img class="page_preview_graffiti" src="' + clean(data.thumb) +
                                        '" /></div>';
                                    toEl = toPics = mpicsEl;
                                    break;
                                ...
                                case 'photo':
                                    ...
                                    oncl = opts.nocl ? '' : ' onclick="return showPhoto(\'' + clean(media) + '\', \'' + data.list + '\', {' + _vopts.replace(
                                        /"/g, '&quot;') + '});"';
                                    preview = '<div ' + oncl + ' class="fl_l page_preview_photo' + (isGraffiti ? ' page_preview_ph_graff' : '') +
                                        '"><img class="page_preview_photo" src="' + clean(data.thumb_m) + '" /></div>';
                                    toPics = 1;
                                    toEl = picsEl;
                                    break;
                                ...
                                case 'video':
                                    ...
                                    preview = '<div' + oncl + ' class="fl_l page_preview_video"><img class="page_preview_video" src="' + clean(data.thumb) +
                                        '" /></div>';
                                    toPics = 1;
                                    toEl = picsEl;
                                    break;
                                ...
                                case 'share':
                                    ...
                                    preview = '<a target="_blank" href="/away.php?to=' + encodeURIComponent(data.url) +
                                        '" class="medadd_h medadd_h_link inl_bl">' + clean(data.lang.profile_choose_link) + '</a>';
                                ...
                                case 'album':
                                    ...
                                    vkImage()
                                        .src = thumb;
                                    oncl = opts.nocl ? '' : ' href="/album' + clean(media) + '" onclick="return nav.change({z: \'album' + clean(media) +
                                        '\'}, event)"';
                                    var cls = 'fl_l page_album_link' + (thumb ? '' : ' page_album_nocover');
                                    preview = '<a class="' + cls + '" ' + oncl + '>\
                    ' + (thumb ?
                                            '<div class="page_album_thumb_wrap"><img class="page_album_thumb" src="' + clean(thumb) + '"/></div>' : '') +
                                        '\
                      <div class="page_album_title">\
                        <div class="page_album_size">' + clean(
                                            data.count) + '</div>\
                        <div class="page_album_title_text">' + clean(data.title) +
                                        '</div>\
                      </div>\
                    </a>';
                                    toPics = 1;
                            ...
                            if (multi) {
                                var medias = addMedia.chosenMedias,
                                    ...
                                    mediaEl = (editable && toPics === 1) ? false : ((type == 'photos_list') ?
                                        se('<div class="page_preview_' + type + '_wrap' + wrap_class + '" style="position: relative">' + preview +
                                            '<div class="page_photos_count">' + media.split(',')
                                            .length + '</div></div>') :
                                        se('<div class="page_preview_' + type + '_wrap' + wrap_class + '"' + (opts.nocl ? ' style="cursor: default"' : '') +
                                            attrs + '>' + preview + '<div nosorthandle="1" class="page_media_x_wrap inl_bl" data-title="' + getLang(
                                                'dont_attach') + '" onmouseover="showTitle(this)" onclick="cur.addMedia[' + addMedia.lnkId + '].unchooseMedia(' +
                                            ind + '); return cancelEvent(event);"><div class="page_media_x" nosorthandle="1"></div></div>' + postview +
                                            '</div>'));
                                ...
                                var mediaEl = se('<div class="' + (toPics === false ? 'page_docs_preview' : 'page_pics_preview') + (ind ? '' :
                                        ' post_thumbed_media') + '"><div class="page_preview_' + type + '_wrap"' + (opts.nocl ? ' style="cursor: default"' :
                                        '') + attrs + '>' + preview + '<div nosorthandle="1" class="page_media_x_wrap inl_bl" data-title="' + getLang(
                                        'dont_attach') + '" onmouseover="showTitle(this)" onclick="cur.addMedia[' + addMedia.lnkId + '].unchooseMedia(' +
                                    ind + '); return cancelEvent(event);"><div class="page_media_x" nosorthandle="1"></div></div>' + postview +
                                    '</div></div>');
                            ...
                            if ((!cur.fileApiUploadStarted || data.upload_ind === undefined) && !cur.preventBoxHide && noboxhide !== true && !inArray(type, [
                                    'poll', 'share', 'page', 'postpone', 'mark_as_ads'
                                ])) {
                                boxQueue.hideLast();
                            }
                  ...
                  <div class="fl_l"> \
                    <div class="page_attach_progress_wrap" style="margin-top: 3px; margin-bottom: 4px;"> \
                      <div id="upload' +
                                    ind +
                                    '_progress" class="page_attach_progress ui_progress"> \
                        ...
                        <div class="ui_progress_bar"></div> \
                                    ...
                                    labelEl +
                                    '<div class="progress_x fl_l" onmouseover="animate(this, {opacity: 1}, 200); showTooltip(this, {text: \'' + getLang(
                                        'dont_attach') +
                                    '\', shift: [6, 3, 3]})" onmouseout="animate(this, {opacity: 0.6}, 200);" onclick="Upload.terminateUpload(' + i + ', \'' +
                                    (fileName || i) + '\', this);"></div>';
                        ...
                        attachCount: function() {
                            ...
                            var num = (editable && window.ThumbsEdit ? ((ThumbsEdit.cache()['thumbs_edit' + lnkId] || {})
                                ...
                                .length + dpicsEl.childNodes.length + mpicsEl.childNodes.length + docsEl.childNodes.length / (docsEl.sorter ? 2 : 1) +
                                progressEl.childNodes.length;
                        ...
                        createPoll: function(data) {
                            ...
                            addMedia.pollPreview = pdocsEl.appendChild(ce('div', {
                                ...
                                innerHTML: '\
                <input onkeydown="cur.addMedia[' + lnkId +
                                    '].keyPoll(this, event)" class="text dark medadd_c_pollq" id="create_poll_question' + lnkId + '" value="' + (
                                        data.question || '') + '" />\
                <div class="medadd_c_pollh">' + data.lang.a +
                                    '</div>\
                <div class="medadd_c_pollans" id="create_poll_answers' + lnkId +
                                    '"></div>\
                <div class="medadd_c_polladd_wr" id="create_poll_add' + lnkId +
                                    '">\
                  <div class="medadd_c_polladd fakeinput dark" onclick="cur.addMedia[' + lnkId +
                                    '].incPoll()">' + data.lang.i + '</div>\
                </div>' + (data.edit ? '' :
                                        '<div class="checkbox medadd_c_pollcb' + (data.anon ? ' on' : '') + '" id="create_poll_anonymous' + lnkId +
                                        '" onclick="checkbox(this);cur.addMedia[' + lnkId + '].changedPoll();">' + data.lang.c + '</div>') + (data.pollSettings ||
                                        '')
                            }));
                            ...
                            cur.pollAnswerTemplate =
                                ...
                                clean(stripHTML(unclean(data.lang.d))) +
                                '" onmouseover="showTitle(this)" onclick="cur.addMedia[%lnkid%].decPoll(this)"><div class="page_media_x"></div></div>';
                        ...
                        checkMessageURLs: function(message, inactive, withTimeout) {
                            if (cur.noCheckMessageURLs || addMedia.chosenMedia || addMedia.urlAttachmentLoading && addMedia.urlAttachmentLoading[0] > vkNow() -
                                10000 || addMedia.attachCount() >= limit) {
                                return;
                            }
                        ...
                        addPreview: function(progress) {
                            return (addMedia.sharePreview = ldocsEl.appendChild(ce('div', {
                                ...
                                innerHTML: '<div class="medadd_c_linkcon"><div></div>' + (progress ?
                                    '<div class="progress medadd_c_linkprg"></div>' : '') + '</div>'
                            })));
                        },
                        ...
                        showPreview: function(fast) {
                            ...
                            if (data.failed) {
                                ...
                                var onloadStr = fast ? '' :
                                    'onload="if (this.width < 130 && !cur.onLoadSwitched) {cur.onLoadSwitched=1;setTimeout(cur.shareShowNext, 0);}"';
                                var imghtml = '';
                                ...
                                if (data.images && data.images[cur.shareShowImg] && imgUrl) {
                                    ...
                                    if (data.images.length > 0) {
                                        var leftScroller = ((data.images.length > 1) ? (
                                            '<div class="medadd_c_linkimg_scroll_wrap medadd_c_linkimg_scroll_wrap_left ' + ((cur.shareShowImg == 0) ?
                                                'medadd_c_linkimg_scroll_wrap_left_first' : '') + '" onclick="' + ((cur.shareShowImg == 0) ?
                                                'Page.ownerPhoto(\'' + data.media + '\');' : 'cur.shareShowNext(true);') +
                                            '"><div class="medadd_c_linkimg_scroll"></div></div>') : '');
                                        var rightScroller = '';
                                        var closeButton = '';
                                        ...
                                        var uploadTooltip = 'onmouseover="showTooltip(this, {text: \'' + getLang('global_link_choose_own_photo') +
                                            '\', black: 1, shift: [7, 11, 8], appendParentCls: \'post\'})"';
                                        var removeTooltip = 'onmouseover="showTooltip(this, {text: \'' + getLang('global_link_remove_photo') +
                                            '\', black: 1, shift: [7, 11, 8], appendParentCls: \'post\'})"';
                                        ...
                                        var imgControls = (!data.media || data.media == '_') ? '' : '<div class="medadd_c_linkimg_controls">' +
                                            ...
                                            '    <div class="medadd_c_linkimg_controls_btn ' + (availableImagesCount > 1 ? 'medadd_c_btn_side_padd' : '') +
                                            '" id="medadd_ctrl_upload" ' + uploadTooltip + ' onclick="Page.ownerPhoto(\'' + data.media + '\');"></div>' +
                                            '  </div>' +
                ...
                <div class="fl_l"><input type="hidden" id="postpone_date' + lnkId +
                                '" value="' + (data.date || '') + '" /></div>\
                <div class="fl_l medadd_c_timerat">' + data.lang.profile_wall_postpone_at +
                                '</div>\
                <div class="fl_l"><input type="hidden" id="postpone_time' + lnkId + '"/></div></div>';
                            if (cur.editingPost && data.friends_only != undefined) {
                                ...
                                html += '<div class="checkbox' + (data.friends_only ? ' on' : '') + ' fl_l" id="friends_only' + lnkId +
                                    ...
                                    '\',!isChecked(this));">' + data.lang.friends_only + '</div></div>';
                                addedhtml = true;
                        ...
                        var _hideTimer, mediaMenu = {
                            ...
                            setItems: function(types) {
                                ...
                                each(types, function(i, v) {
                                    ...
                                    row = (moreNode ? moreNode : menuNode)
                                        .appendChild(ce('a', {
