WComments = {
    ...
    switchSection: function(newSection) {
        ...
        ge('wcomments_posts')
            .innerHTML = '<div class="wcomments_posts_loading"></div>';
        hide('wcomments_more_link');
        var params = extend(WComments.getSectionParams(), {
            // part: 1
        });
    ...
    showLikesBox: function(obj) {
        cur.Rpc.callMethod('showBox', 'widget_like.php?' + ajx2q({
            act: 'a_stats_box',
            obj: obj,
            from: 'wcomments',
            app: cur.options.app,
        ...
        switch (ev_type) {
            case 'new_post':
                ...
                var cont = ge('wcomments_posts'),
                    newEl = ce('div', {
                        innerHTML: Wall.getNewPostHTML(ev, cur.options.is_admin)
                    })
                    .firstChild,
                    newCnt = intval(ev[8]),
            ...
            case 'new_reply':
                ...
                var repliesEl = ge('replies' + post_id),
                    newEl = ce('div', {
                        innerHTML: Wall.getNewReplyHTML(ev, cur.options.is_admin)
                    })
                    .firstChild,
                    highlight = false;
    ...
    init: function(options) {
        ...
        if (options.fixed_height) {
            if (browser.msie6) {
                setStyle('wcomments_posts_wrap', {
                    height: options.fixed_height - 127
                });
            } else {
                setStyle('wcomments_posts_wrap', {
                    maxHeight: options.fixed_height - 127
                });
            }
        ...
        timeUpdateInt = setInterval(function() {
            WComments.updateTimes();
        }, 10000);
...
var Wall = {
        ...
        if (cur.withMentions && !cur.mentionsAdded) {
            ...
            stManager.add(['ui_controls.css', 'ui_controls.js', 'mentions.js'], function() {
                ...
                cur.postMention = new MentionAutocomplete('post_field', {
                    ...
                    onValueChange: (cur.wallAddMedia || {})
                        .checkMessageURLs
                });
    ...
    sendPost: function() {
        var addmedia = cur.wallAddMedia || {},
            ...
            medias = addmedia.getMedias ? addmedia.getMedias() : [],
            share = (addmedia.shareData || {});
        var msg = ge('post_field')
            .getValue();
        var attachI = 0;
                    ...
                    ajax.post('al_widget_comments.php', params, {
                        onDone: function(options, rows) {
                            if (cur.section == 'posts' && cur.Rpc) {
                                cur.Rpc.callMethod('publish', 'widgets.comments.new_comment', options.count, options.last_comment, options.date,
                                    options.full_hash, options.pageId);
    ...
    sendReply: function(post, ev, stickerId) {
        ...
        newEl = se(rs(cur.wallTpl.reply_fast, {
            reply_id: '0_' + replyId,
            message: Emoji.emojiToHTML(msg, true)
                .replace(/\n/g, '<br/>'),
    ...
    deletePost: function(post, hash) {
        ...
        ajax.post('al_wall.php', {
            ...
            onDone: function(msg, options) {
                ...
                if (cur.section == 'posts' && cur.Rpc) {
                    cur.Rpc.callMethod('publish', 'widgets.comments.delete_comment', options.count, options.last_comment, options.date, options.full_hash,
                        options.pageId);
                    // cur.Rpc.callMethod('onChange', options.count, options.last_comment, options.date, options.full_hash, options.pageId);
                }
                hide(t);
    ...
    markAsSpam: function(post, hash) {
        ...
        ajax.post('al_wall.php', {
            ...
            onDone: function(msg, js) {
                var r = ge('post' + post),
                    t = geByClass1('post_table', r) || geByClass1('reply_table', r);
                var pd = ge('post_del' + post);
                if (pd) {
                    pd.innerHTML = msg;
                    show(pd);
    ...
    checkPostClick: function(el, event) {
        ...
        var target = event.target || event.srcElement,
            ...
            foundGood = false,
            classRE =
            /wall_post_text|post_media|event_share|public_share|group_share|feed_friends|feed_gifts|feed_videos|feed_explain_list|explain|feed_photos|feedback_row/;
        do {
            if (!target ||
                target == el ||
                target.onclick ||
    ...
    likeOut: function(post, tthide) {
        ...
        if (!icon) return;
        if (!hasClass(icon, 'my_like') && !hasClass(icon, 'fw_my_like')) {
            setTimeout(animate.pbind(ge('like_icon' + post), {
                opacity: 0.4
            }, 200, false), 1);
        }
        if (tthide) {
    ...
    likeUpdate: function(post, my, count, title) {
        ...
        var m = post.match(/(-?\d+)_(photo|video|note|topic|wall_reply|)(\d+)/),
            like_obj = (m[2] || 'wall') + m[1] + '_' + m[3];
    ...
    like: function(post, hash) {
        ...
        var count = countInput ? countInput.value : ge('like_count' + post)
            .innerHTML;
        Wall.likeUpdate(post, !my, intval(count) + (my ? -1 : 1));
    },
    likeShare: function(post, hash) {
        var el = ge('like_share_wall' + post),
            was = isChecked(el);
        checkbox(el);
        ajax.post('like.php', {
    ...
    update: function(count) {
        ...
        ge('page_wall_posts_count')
            .innerHTML = cnts[cur.wallType] ? langNumeric(cnts[cur.wallType], cur.options.wall_counts) : '';
        ge('page_wall_header')
    ...
    getNowRelTime: function() {
        var ts = vkNow();
        return '<span class="rel_date rel_date_needs_update" time="' + intval(ts / 1000 - (cur.tsDiff || 0)) + '" abs_time="' + Wall.getAbsDate(ts) + '">' + getLang(
            'wall_just_now') + '</span>';
    },
    getNewPostHTML: function(ev, isAdmin, extendCb) {
        var post_id = ev[2],
            html = cur.wallTpl.post,
            actions = rs(cur.wallTpl.post_actions, {
                actions: (isAdmin || !ev[2].indexOf(vk.id + '_')) ? cur.wallTpl.del : cur.wallTpl.spam
            }),
            repls = {
    ...
    init: function(opts) {
        ...
        each(geByTag('textarea', ge('page_wall_posts')), function() {
        ...
        if (opts.media_types && ge('page_add_media')) {
            cur.wallAddMedia = initAddMedia(ge('page_add_media')
                .firstChild, 'media_preview', opts.media_types);
            cur.wallAddMedia.onChange = function() {
                Wall.checkPostLen(ge('post_field')
                    .value, true);
            }
        }
    }
    ...
    var _hideTimer, mediaMenu = {
        ...
        setItems: function(types) {
            ...
            each(types, function(i, v) { // [id, name, bg-position, onclick, href, bg-url, customStyle]
                ...
                row = (moreNode ? moreNode : rowsNode)
...
function initAddMedia(lnk, previewId, mediaTypes, opts) {
    ...
    each(mediaTypes || [], function(i, v) {
        ...
        switch (v[0]) {
            case 'graffiti':
                handler = function() {
                    cur.Rpc.callMethod('showBox', 'al_wall.php?' + ajx2q({
                        act: 'canvas_draw_box',
                        to_id: cur.postTo,
                        widget: 1
                    }), preps);
            ...
            case 'photo':
                handler = function() {
                    stManager.add(['page.css'], function() {
                        cur.Rpc.callMethod('showBox', 'al_photos.php?' + ajx2q({
                            act: 'choose_photo',
                            to_id: cur.postTo,
            ...
            case 'video':
                handler = function() {
                    cur.Rpc.callMethod('showBox', 'al_video.php?' + ajx2q({
            ...
            case 'audio':
                handler = function() {
                    stManager.add(['page.css'], function() {
                        cur.Rpc.callMethod('showBox', 'al_audio.php?' + ajx2q({
    ...
    val(previewEl, '<div id="page_pics_preview' + lnkId + '" class="page_pics_preview media_preview clear_fix"></div><div id="page_dpics_preview' + lnkId +
        ...
        chooseMedia: function(type, media, data, url, noboxhide, isGraffiti) {
            ...
            switch (type) {
                ...
                case 'share':
                    ...
                    preview = '<a target="_blank" href="/away.php?to=' + encodeURIComponent(data.url) + '" class="medadd_h medadd_h_link inl_bl">' + clean(data.lang.profile_choose_link) +
                        '</a>';
...
' + (data.thumb ? '<img class="wall_album_cover" src="' + clean(data.thumb) + '"/>' : '') +
  ...
  <div class="wall_album_caption">\
    <div class="wall_album_title_wrap clear_fix">\
      <div class="wall_album_title fl_l">' + clean(data.title) +
                        '</div>\
      <div class="wall_album_count fl_r">' + clean(data.count) + '</div>\
            ...
            if (multi) {
                ...
                val(previewEl, '<div class="fl_l">' + preview + '</div><div class="x fl_l" onmouseover="showTooltip(this, {text: \'' + getLang('dont_attach') +
                    '\', shift: [0, 3, 3]})" onclick="cur.addMedia[' + lnkId + '].unchooseMedia()"></div>');
                show(previewEl);
        ...
        unchooseMedia: function(ind) {
            ...
            if (multi) {
                ...
                each([addMedia.sharePreview, addMedia.pollPreview, addMedia.postponePreview], function() {
                    re(this);
                });
...
<div class="fl_l"><div class="page_attach_progress_wrap" style="margin-top: 3px; margin-bottom: 4px;">\
  <div id="upload' + ind +
                    ...
                    '\', shift: [6, 3, 3]})" onmouseout="animate(this, {opacity: 0.6}, 200);" onclick="cur[\'terminate_upload_' + i + '\'](\'' + (fileName || i) +
                    '\');"></div>';
...
<div class="medadd_c_polladd_wr" id="create_poll_add' +
  ...
  <div class="medadd_c_polladd" onclick="cur.addMedia[' + lnkId + '].incPoll()">' + data.lang.i + '</div>\
                        ...
                        '" onclick="checkbox(this)"><div></div>' + data.lang.c + '</div>')
            }));
        ...
        checkMessageURLs: function(message, inactive) {
            ...
            while (message && (matches = message.match(rx))) {
                ...
                if ((matches[6] || '')
                    .length > 1 || inArray(url, addMedia.urlsCancelled) || inArray(initialUrl, addMedia.urlsCancelled)) {
                    continue;
                }
                var valid = true;
...
<div class="medadd_c_linkcon"><div></div>' + (progress ? '<div class="progress medadd_c_linkprg"></div>' : '') + '</div>'
        ...
        showPreview: function(fast) {
            ...
            if (data.failed) {
                ...
                var html = (data.images && data.images[cur.shareShowImg] ? '<img class="medadd_c_linkimg fl_l" src="' + clean(data.images[cur.shareShowImg]) + '" ' +
                    onloadStr + (data.imagesStyles && data.imagesStyles[cur.shareShowImg] || '') + ' />' : '') + (data.title ? '<h4 class="medadd_c_linkhead">' + data.title +
                    '</h4>' : '') + (data.description ? '<div class="medadd_c_linkdsc">' + data.description + '</div>' : '') + '<div class="clear"></div>';
            }
        ...
        setupPostpone: function(data, export_row) {
            var ed = (cur.editingPost && domPN(ppdocsEl)
                    .id == 'wpe_media_preview'),
                h = (browser.msie6 || ed) ? '' : '1px',
                addedhtml = false;
...
<div class="fl_l"><input type="hidden" id="postpone_time' +
            ...
            addMedia.postponePreview = ppdocsEl.appendChild(ce('div', {
...
function showCaptchaBox(sid, dif, box, o) {
    ...
    cur.Rpc.callMethod('showBox', 'al_apps.php?' + ajx2q({
    ...
    setTimeout((function() {
            ...
            function keydown(event) {
                switch ((event || window.event)
                    .keyCode) {
                    case 40:
                        self.obj[self.scrollProp] += 40;
                        break;
                    case 38:
                        self.obj[self.scrollProp] -= 40;
                        break;
                    case 34:
                        self.obj[self.scrollProp] += self[self.scrollDimensionProp];
                        break;
                    case 33:
                        self.obj[self.scrollProp] -= self[self.scrollDimensionProp];
                        break;
...
function showInlineVideo(videoId, listId, options, ev, thumb) {
    ...
    options.params = options.params || {
        ...
        module: options.module || cur.module || '_ac'
    };
