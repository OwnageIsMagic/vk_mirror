var Community = {
    ...
    init: function() {
        cur.Rpc = new fastXDM.Client({
            ...
            authorised: function(args) {
                ...
                if (href.indexOf('fieldText=') != -1) href = href.replace(/fieldText=.+?(&|$)/, 'fieldText=' + winToUtf(ge('commentFiled')
                    .getValue()) + '\1');
                else href = href + '&fieldText=' + winToUtf(ge('commentFiled')
                    .getValue());
        ...
        if (cur.noAuth) {
            ...
            if (!cur.justAuth && ge('community_anim_row')) {
                if (state) {
                    ...
                    ge('members_count')
                        .innerHTML = cur.count_in;
                } else {
                    animate(ge('community_anim_row'), {
                        marginLeft: -cur.mWidth
                    }, 200);
                    ge('members_count')
                        .innerHTML = cur.count_out;
                }
            }
    ...
    subscribeBox: function(state, callback, isEvent) {
        var
            ...
            outerHeight = typeof window.outerHeight != 'undefined' ? window.outerHeight : (document.body.clientHeight - 22),
            features = 'width=655,height=479,left=' + parseInt(screenX + ((outerWidth - 655) / 2), 10) + ',top=' + parseInt(screenY + ((outerHeight - 479) / 2.5), 10);
        window.activePopup = window.open(location.protocol + '//' + location.host + '/widget_community.php?act=a_subscribe_box&oid=' + cur.oid + '&state=' + state + (
            isEvent ? '&is_event=1' : ''), 'vk_subscribe', features);
    ...
    toggleStat: function(oid, width) {
        if (cur.statShown) {
            setStyle(ge('community_groups_main'), {
                height: 'auto'
            });
            hide('hide_stat');
            show('show_stat');
            ...
            if (!cur.statLoaded) {
                setStyle(ge('community_groups_main'), {
                    height: cur.oldHeight - 8
                });
                setStyle(ge('wcomments_progress'), {
                    marginTop: cur.oldHeight / 2 - 40
                });
                ajax.post('/widget_community.php', {
                    act: 'a_get_stat',
                    width: width,
    ...
    membersBox: function(oid) {
        cur.Rpc.callMethod('showBox', 'widget_community.php?act=members_box&oid=' + oid, {
            height: 442,
            width: 498
        });
    },
...
function showVideo(video, list) {
    cur.Rpc.callMethod('showBox', 'al_video.php?' + ajx2q({
        ...
        widget_width: 654,
        widget: 1,
        module: cur.module || '_alcommunity'
    }), {
        height: window.outerHeight || screen.availHeight || 768,
        width: window.outerWidth || screen.availWidth || 1028
    });
...
function showFriends(oid) {
    ...
    cur.Rpc.callMethod('showBox', 'al_page.php?' + ajx2q({
...
function showInlineVideo(videoId, listId, options, ev, thumb) {
    ...
    options.params = options.params || {
        ...
        module: options.module || cur.module || '_acm'
    };
