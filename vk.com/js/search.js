...
function setSection(new_section) {
    ...
    if (ns) {
        ...
        if (selected[0]) {
            ...
            selected.innerHTML = "<div onmouseover=\"this.className='sec_pad_over'\" onmouseout=\"this.className='sec_pad'\" onclick='return setSection(\"" + selected.id +
                "\");' class='sec_pad'><a href='#' onclick='return setSection(\"" + selected.id + "\")'>" + cs_word + "</a></div>";
        }
...
function updatePage(ajaxObj, responseText) {
    ...
    try {
        ...
        */
        ...
        show_error = 1;
        ge('results')
            .innerHTML = result.rows || 'Server error';
        show_error = 2;
        ge('pagesTop')
            .innerHTML = result.pages || '';
        show_error = 3;
        ge('pagesBottom')
            .innerHTML = result.pages || '';
        show_error = 4;
        ...
        if (result.sections) {
            ge('sectionsFilter')
                .innerHTML = result.sections;
        }
        setupReply();
        show_error = 6;

        var section = ge('section')
            .value;

        if (result.filters) {
            ge('searchFilters')
                .innerHTML = result.filters || '';
            section = ge('section')
                .value;
            show_error = 7;
            if (result.q) {
                var inp_el = ge('search_input');
                inp_el.setValue(ge('q')
                    .value = result.q);
        ...
        if (show_error > 0 && show_error < 12) {
            ge('results')
                .innerHTML = 'Error ' + show_error + ': ' + e.message;
        } else if (show_error == 12) {
            ge('results')
                .innerHTML = 'Script error: <br>' + result.script;
...
var iSearch = {
    init: function(options) {
        ...
        addEvent('search_input', 'keyup focus mouseup', function(e) {
            ...
            var section = ge('section') ? ge('section')
                .value : '',
