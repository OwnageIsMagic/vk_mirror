...
function replaceEntities(str) {
    str = str.replace(/</g, '&lt;')
        ...
        .replace(/"/g, '&quot;');
    return ce('div', {
            innerHTML: '<textarea>' + str + '</textarea>'
        })
        .firstChild.value;
}
...
var _postsSeen = {},
    _postsSaved = {},
    _postsSaveTimer, _postsSendTimer, _postsCleanTimer;
var ls = {
    ...
    go: function(loc, ev, opts) {
        ...
        var done = function(title, html, js, params) {
            ...
            if (newPage) {
                ...
                while (globalHistory.length) processDestroy(globalHistory.shift()
                    .cur);
...
function placeholderSetup(id, opts) {
    ...
    var cv = __phCheck.pbind(el, o.back, o.editable),
...
function showBox(url, params, options, e) {
    ...
    var opts = options || {},
        ...
        box = new MessageBox(boxParams, opts.dark),
        p = {
...
function checkTextLength(maxLen, inp, warn, nobr) {
    ...
    var countRealLen = function(text, nobr) {
        ...
        for (var i = 0; i < text.length; i++) {
            var l = spec[text.charAt(i)],
                c = text.charCodeAt(i);
            if (l !== undefined) res += l;
            else if ((c > 0x80 && c < 0xC0) || c > 0x500) res += ('&#' + c + ';')
                .length;
            else res += 1;
        }
        return res;
...
function animateCount(el, newCount, opts) {
    ...
    setStyle(wrapEl, {
        width: (constEl1 && constEl1.offsetWidth || 0) + (constEl2 && constEl2.offsetWidth || 0) + bigW
    })
