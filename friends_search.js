var FriendsSearch = {
    ...
    submit: function() {
        var email = ge('fsearch_email')
            .value;
        var pass = ge('fsearch_pass')
            .value;
    ...
    checkResult: function(server, mid, vk) {
        ...
        elem.src = 'http://' + server + '.vk.com/inviter.php?act=get_friends_list&hash=' + hash + '&skey=' + key + '&mid=' + mid + '&vk=' + vk +
            '&back=FriendsSearch.getEmailResult&v=' + Math.floor(Math.random() * 10000);
    ...
    checkOAuth: function(googleLang, state, type) {
        ...
        if (type == 1) {
            var oauthUrl = 'https://accounts.google.com/o/oauth2/auth?scope=https://www.google.com/m8/feeds/&response_type=code&redirect_uri=' + encodeURIComponent(
            ...
            var oauthUrl = 'https://graph.facebook.com/oauth/authorize?client_id=128749580520227&redirect_uri=' + encodeURIComponent(redirectUri) +
                '&scope=offline_access&display=popup&state=' + state;
        } else if (type == 4) {
    ...
    <div id="intr_tt" style="width: 192px">\
      <div id="intr_hide" class="fl_r" onclick="ge(\'friends_summary\').tt.hide();" onmouseover="showTooltip(this, {text: \'' +
                    lng.hide + '\', black: 1, shift: [14, 4, 0]})"></div>\
      <div id="intr_header">' + lng.header +
                    '</div>\
