...
var AudioObject = {
    ...
    changeState: function(id, state, value) {
        ...
        switch (state) {
            case 'play':
                ...
                ge("imgbutton" + id)
                    .src = this.images[mode].pause;
                if (window.isValidFlash) ge("player")
                    .playTrack();
    ...
    showPlayer: function(id) {
        var player = this.checkPlayer(id)
            .player;
        if (!player) return;
        var mode = (window.audioEditMode) ? "editmode" : "playmode";
        if (window.audioEditMode) {
            ge("audio" + id)
                .className = "audioEditRowPlaying";
...
function showLyrics(id, lid) {
    ...
    if (lyricsDiv.shown != 1) {
        ...
        lyricsDiv.innerHTML =
            "<div style='text-align: center; height: 50px; padding: 30px 10px 10px 10px'><img valign='middle' src='http://vkontakte.ru/images/progress7.gif'></div>";
    } else {
        lyricsDiv.innerHTML = "";
        lyricsDiv.shown = 0;
        hide(lyricsDiv);
    ...
    Ajax.Send('audio.php', {
        ...
        onSuccess: function(obj, text) {
            ...
            if (wid) {
                ...
                var prev_node = ge('adding' + wid + 'link')
                    .previousSibling,
                    next_node = ge('adding' + wid + 'link')
                    .nextSibling;
...
function showVideoBox(vars, elem, description, to_comments_text, add_text, add_hash, thumb, player_available, allow_html5, player_version) {
    ...
    var title = decodeURIComponent(vars['md_title'])
        .replace(/\+/g, ' ');
    var author = decodeURIComponent(vars['md_author'])
        .replace(/\+/g, ' ');
    ...
    var controls = '';
    controls += '<span id="video_controls"><a href="/video' + vars['oid'] + '_' + vars['vid'] + '">' + to_comments_text + '</a> | <a href=\"javascript:reportVideo(' + vars['oid'] +
        ', ' + vars['vid'] + ')\">' + getLang('global_Complain_video') + '</span>';
    window.videoBox.addControlsText(controls);
    window.videoBox.setOptions({
        title: title
    });
...
function loadFlashPlayer(vars, player_available, allow_html5, player_version) {
    ...
    if (swfobject.hasFlashPlayerVersion('9')) {
        ...
        ge('flash_player_container')
            .style.display = player_available ? 'block' : 'none';
        ge('video_player')
            .style.padding = intval(allow_html5) ? '135px 0 0 0' : '160px 0 0 0';
        ge('player_html5_msg')
            .style.display = intval(allow_html5) ? 'block' : 'none';
    }
