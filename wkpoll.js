var WkPoll = {
    ...
    tab: function(opt, one) {
        ...
        var act = geByClass1('summary_tab_sel', ge('wk_poll_tabs')),
            cnt = domPN(ge('wk_poll_opt' + opt));
        if (cnt != act) {
        ...
        if (opt) {
            ...
            ajax.post('al_wall.php', cur.wkPollQuery[opt], {
                onDone: function(html, offset, pHtml, pOffset) {
    ...
    graphFilter: function(obj, percent, amount) {
        var act = geByClass1('summary_tab_sel', ge('wk_poll_grtabs')),
            cnt = domPN(obj);
        if (cnt != act) {
            removeClass(act, 'summary_tab_sel');
            addClass(act, 'summary_tab');
        ...
        if (type == 'country') {
            ...
            if (isChecked('wk_poll_extra_country')) {
                ...
                if (citiesLen > 1) {
                    ...
                    val('privacy_edit_poll_dmgr_city_wrap',
                        '<a id="privacy_edit_poll_dmgr_city" href="#" onclick="if (checkEvent(event) === false) Privacy.show(this, event, \'poll_dmgr_city\'); return false;">' +
                        cityName + '</a>');
                } else if (citiesLen) {
    ...
    updateDemography: function(callback) {
        ...
        var onUpdate = function(options) {
            ...
            each(options, function(optId, optData) {
                ...
                animate(ge('wk_poll_row' + optId), {
                    width: (optData.width * 534 / 100)
                }, 200, function() {
    ...
    more: function() {
        var opt = cur.wkPollOpt,
            ind = cur.wkPollInd,
            data = cur.wkPollCache[opt][ind + 1];
    ...
    scroll: function() {
        ...
        if (isVisible(cur.wkPollTabs)) {
            if (getXY(cur.wkPollTabsWrap, true)[1] < 0) {
                if (!cur.wkPollFixed) {
                    setStyle(cur.wkPollTabsWrap, 'height', domFC(cur.wkPollTabs)
                        .offsetHeight);
    ...
    bigphOver: function(obj, uid) {
        ...
        var o = obj.firstChild,
            ch = cur.wkPollPhCache[uid];
        if (o.tagName != 'A' || o.className != 'wk_pollph') {
            (o = obj.insertBefore(ce('a', {
                className: 'wk_pollph',
                href: ch && ch._id ? ('/photo' + ch._id + '?all=1') : ('/albums' + uid),
                innerHTML: '<span class="wk_pollph_label">' + getLang('global_photo_full_size') + '</span>'
            }), obj.firstChild))
            .onclick = WkPoll.bigphClick.pbind(uid);
            o._uid = uid;
        }
    ...
    bigphClick: function(uid, ev) {
        ...
        var ch = cur.wkPollPhCache[uid],
            o = cur.wkPollPhShown[uid],
            obj = domPN(o);
        if (!o || !obj) return;
        if (ch === undefined) {
