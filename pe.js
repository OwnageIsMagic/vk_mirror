function PhotoEdit(parentEl, photoSrc, bigPhotoSrc, options, initCallback, sizeUpdateCallback) {
    ...
    function onImageUpdate() {
        ...
        if (!originalImageCanvas) {
            ...
            originalImageCanvas.getContext('2d')
                .drawImage(imageCanvas, 0, 0, imageCanvas.width * pixelRatio, imageCanvas.height * pixelRatio);
        }
        ...
        for (var i = 0; i < lines.length || lastWrap; i++) {
            ...
            if (metrics.width > maxWidth) {
                ...
                while (ch < line.length) {
                    while (ch < line.length) {
                        var fits = context.measureText(curr)
                            .width < maxWidth;
        ...
        if (fontType == 0) {
            ...
            for (var i = lines.length - 1; i >= 0; i--) {
                ...
                (function me(fontSizes, min, max) {
                    ...
                    var textWidth = context.measureText(lines[i])
                        .width;
                    if (min > max) {
                        return textWidth;
                    }
...
! function(a) {
    ...
    var b = a.HTMLCanvasElement && a.HTMLCanvasElement.prototype,
        ...
        f = (c || e) && a.atob && a.ArrayBuffer && a.Uint8Array && function(a) {
            var b, f, g, h, i, j;
            for (b = a.split(",")[0].indexOf("base64") >= 0 ? atob(a.split(",")[1]) : decodeURIComponent(a.split(",")[1]), f = new ArrayBuffer(b.length), g = new Uint8Array(f), h =
                0; h < b.length; h += 1) g[h] = b.charCodeAt(h);
