Claims = {
    ...
    claimContent: function(cid, type, owner_id, id, hash) {
        ge('claim' + cid + type + owner_id + '_' + id)
            .innerHTML = '';
        ajax.post('/claims', {
            act: 'a_claim',
            claim_id: cid,
            ...
            onDone: function(response) {
                ge('claim' + cid + type + owner_id + '_' + id)
                    .innerHTML = '<a href="#" onclick="Claims.unclaimContent(' + cid + ',\'' + type + '\',' + owner_id + ',' + id + ',\'' + hash +
                    '\');">�������</a>';
            }
        });
    },
    unclaimContent: function(cid, type, owner_id, id, hash) {
        ...
        ajax.post('/claims', {
            ...
            onDone: function(response) {
                ge('claim' + cid + type + owner_id + '_' + id)
                    .innerHTML = '<a href="#" onclick="Claims.claimContent(' + cid + ',\'' + type + '\',' + owner_id + ',' + id + ',\'' + hash +
                    '\');">������</a>';
            }
        });
    },
    ...
    updateClaimButtons: function(cid, status) {
        if (status == 0) {
            ge('claim_status')
                .innerHTML = "�������";
            ge('claim_buttons')
                .innerHTML = "<div class=\"button_blue fl_l\"><button onclick=\"Claims.setClaimStatus(" + cid + ", 1, Claims.updateClaimButtons);\">�������</button></div>" +
        ...
        if (status == 1 || status == 2) {
            ge('claim_status')
                .innerHTML = (status == 1 ? "�������" : "���������");
            ge('claim_buttons')
                .innerHTML = "<div class=\"button_blue fl_l\"><button onclick=\"Claims.setClaimStatus(" + cid + ", 0, Claims.updateClaimButtons);\">�������</button></div>";
        }
        ge('claim_updated_msg')
            .innerHTML = "<b>������</b><br/>������ ������ ������� �������.";
    ...
    updateClaimLinks: function(cid, status) {
        if (status == 0) {
            ge('claim' + cid + '_status')
                .innerHTML = "�������<br/><a href='#' onclick='Claims.setClaimStatus(" + cid + ", 1, Claims.updateClaimLinks); return cancelEvent(event);'>�������</a>" +
                " | <a href='#' onclick='Claims.setClaimStatus(" + cid + ", 2, Claims.updateClaimLinks); return cancelEvent(event);'>���������</a>";
        } else
        if (status == 1 || status == 2) {
            ge('claim' + cid + '_status')
                .innerHTML = ((status == 1) ? "�������" : "���������") + "<br/><a href='#' onclick='Claims.setClaimStatus(" + cid +
                ", 0, Claims.updateClaimLinks); return cancelEvent(event);'>�������</a>";
        }
    },
        ...
        if (str) {
            ...
            if (str) {
                ...
                str = trim(escapeRE(str.replace(/\)/g, ''))
                    ...
                    .join('&amp;'));
                cur.selection = {
                    re: new RegExp('(' + str.replace(cur.titlesIndex.delimiter, '|')
                        .replace(/(^\||\|$|\?)/g, '') + ')', 'gi'),
                    val: '<span>$1</span>'
                };
            }
    ...
    showTitleRows: function(start, end) {
        ...
        if (!list || !list.length) {
            var msg = (cur.blacklist.length) ? getLang('claim_no_audios_found')
                .split('{query}')
                .join('<b>' + cur.searchStr.replace(/([<>&#]*)/g, '') + '</b>') : getLang('claim_no_blacklist_audios'),
                link = se(cur.linkTpl);
            ...
            for (i in titles) {
                ...
                if (cur.selection) {
                    row.title = row.title.replace(cur.selection.re, cur.selection.val)
                        .replace(/&#(\d*)<span>(\d+)<\/span>(\d*);/g, "&#$1$2$3;");
                }
