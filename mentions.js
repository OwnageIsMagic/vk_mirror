    ...
    createChildClass('MentionAutocomplete', UiControl, {
        ...
        initEvents: function() {
            ...
            addEvent(this.textareaEl, 'focus', function(e) {
                ...
                self.rtaEl.innerHTML = v ? v.replace(/&/g, '&amp;')
                    .replace(/</g, '&lt;')
                    .replace(/>/g, '&gt;')
                    .replace(/"/g, '&quot;')
                    .replace(/\n/g, '<br/>')
                    .replace(/\[((?:id|club)[1-9]\d*)\|([^\]]+)\]/g, '<a href="/$1">$2</a>') : (browser.mozilla ? '<br/>' : ' ');
                setTimeout(function() {
            ...
            addEvent(this.rtaEl, 'keyup mouseup', function(e) {
                ...
                if (browser.opera || browser.mozilla) {
                    ...
                    if (anchor && anchor.tagName == 'A' && !focusOffset) {
                        ...
                        if (a && a.nodeType == Node.TEXT_NODE && (a.nodeValue.charAt(a.nodeValue.length - 1) == '\u00A0' || a.nodeValue.charAt(a.nodeValue.length -
                                1) == ' ')) a.nodeValue = a.nodeValue.substr(0, a.nodeValue.length - 1);
                ...
                if (window.getSelection) { // normal browsers
                    ...
                    if (focusNode) {
                        if (focusNode == self.rtaEl && self.rtaEl.childNodes) {
                            ...
                            anchor.normalize();
                            if (anchor.tagName == 'A') window.getSelection()
                                .collapse(anchor, 1);
        ...
        getTerm: function() {
            ...
            if (pos < 0 || focusValue.substr(pos, focusOffset - pos)
                .match(/([\s,.:?! ]|&nbsp;)/)) return self.select.hide();

            var term = trim(focusValue.slice(pos, focusOffset))
                .substr(1)
                .toLowerCase();
        ...
        fixContents: function() {
            ...
            var fixEl = function() {
                ...
                if (!inArray(this.tagName, ['A', 'P', 'BR', 'DIV']) || this.parentNode != self.rtaEl && this.parentNode.tagName != 'P' && this.parentNode.parentNode !=
                    self.rtaEl) {
                    extractChilds(this);
                    counter++;
                }
                    ...
                    if ((data = this.href.match(/^https?:\/\/(?:[a-z0-9\.]+\.)?(vkontakte\.ru|vk\.com)\/(.*)/i)) && html) {
                        ...
                        if (mid = query.match(/^id(\d+)/)) {
                            if (html.match(/((https?:\/\/)?)(?:[a-z0-9\.]+\.)?(vkontakte\.ru|vk\.com)\/id(\d+)/ig)) {
                                ...
                                wikiValue += '[id' + mid[1] + '|' + stripHTML(html)
                                    .replace(/\]/g, '')
                                    .replace(/\n/g, ' ') + ']';
                            }
                            return;
                            ...
                            if (html.match(/((https?:\/\/)?)(?:[a-z0-9\.]+\.)?(vkontakte\.ru|vk\.com)\/(?:club|event|public)(\d+)/ig)) {
                                ...
                                wikiValue += '[club' + gid[1] + '|' + stripHTML(html)
                                    .replace(/\]/g, '')
                                    .replace(/\n/g, ' ') + ']';
                            }
                            return;
