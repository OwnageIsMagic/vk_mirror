...
var TypeAhead = function(rootEl, formEl, textBoxEl, idEl, defaultOptions, instructions, useFilter, onSuccessHandler, onInputChangeHandler, onUpHandler, onDownHandler,
    onListElMouseDownHandler, placeholderText, showNoMatches, override_resize, positionEl) {
    ...
    this.getMatchSingleTerm = function(term, document) {
        ...
        while ((index = curDocument.toUpperCase()
        ...
        if (filter.length > 0) {
            ...
            if (!matchFound) {
                for (var i = 0; i < this.defaultDropDownCount; i++) {
                    if (this.defaultDropDownEl.childNodes[i]._value.toUpperCase()
                        .indexOf(filter) == 0) {
                        matchFound = true;
                        this.selectedIndex = i;
