...
document.onmouseup = function() {
    if (audio_moveState == 1 && !ge('theAudio')
        .paused)
...
function updVol() {
    ...
    ge('audio_volSlider')
        .style.left = audio_curVolX + 'px';
    ge('audio_volume-line')
...
function updTime() {
    ...
    if (p && !isNaN(p.duration) && p.duration > 0 && audio_moveState != 1) {
        ge('audio_prSlider')
            .style.left = ((audio_prLineW * (p.currentTime - p.startTime)) / p.duration) + 'px';
