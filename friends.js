...
function FriendsFilter(dataObj, input, options) {
    ...
    function initIndex() {
        ...
        for (var i in dataObj.friends) {
            var obj = dataObj.friends[i],
                addedChars = [],
                str = obj[1] + (dataObj.universities ? ' ' + (dataObj.universities[obj[8]] || '')
                    .replace('(', ' ') : '');
            var cursor = 0,
                charac;
            dataObj.friendsIndex[obj[0]] = i;
            while (1) {
                charac = str.charAt(cursor)
                    .toLowerCase();
    ...
    function matchSubset(s, sub) {
        ...
        sub = escapeRE(sub);
        return ((new RegExp("^" + sub + "|\\s" + sub + "|\\(" + sub, "gi"))
            .test(trim(s)));
    }
...
function PrivacyBox(options) {
    ...
    function initBox() {
        ...
        saveListBox.content(getLang('friends_you_can_save_this_list') +
            '<div style="margin: 18px 0px 3px 0px; text-align:center"><input id="privacy_box_flist_name" type="text" placeholder="' + getLang('friends_enter_list_name') +
            '" class="inputText" style="width: 180px;font-size:12px"/></div>');
    ...
    function showFriendsFilter(offset) {
        ...
        if (!filteredItems.length) {
            if (tabListSelected && !filterQuery)
                ...
                flist.innerHTML = '<div class="flist_empty">' + getLang('friends_nothing_found_by_query')
                .replace('{query}', replaceChars(filterQuery)) + '</div>';
            return;
        }
        var html = "";
        for (var i = offset; i < filteredItems.length; i++) {
            ...
            if (i && ((i % 60) == 59) && (i != filteredItems.length - 1)) {
                html += "<span><div class='flist_more clear'><a onclick=\"this.parentNode.parentNode.innerHTML='';showFriendsFilter(" + (i + 1) + ");return false;\">" + getLang(
                    'friends_show_more_friends') + " &raquo;</div></span>";
                break;
            }
        }
    ...
    function selectAll() {
        ...
        ge('privacy_box_list_selected_count')
    ...
    function onListTabChange(showSelected) {
        ge('privacy_box_friend_list_lookup')
            .value = '';
    ...
    this.editPrivacy = function(item, selectedIds) {
        ...
        ge('privacy_box_list_selected_count')
            .innerHTML = 0;
        ge('privacy_box_list_tab')
            .className = 't_filter_selected';
        ge('privacy_box_list_selected_tab')
...
function listRow(n, name, photo, state) {
    ...
    return '<div onmouseout="flistOff(this)" onmousemove="flistOver(this)" onclick="onFriendClick(this,' + n + ')" class="flist_cell' + current_state + '" id="flist' + n +
        '"><div class="flist_border_wrap"><div class="flist_wrap">' +
        '<div class="flist_div">' +
