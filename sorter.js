var sorter = {
    ...
    evCoords: function(ev, s) {
        return browser.android ? [ev.touches[0].pageX + (ev.pageX || 0), ev.touches[0].pageY + (ev.pageY || 0) + (s.scrollNode && s.scrollNode.scrollTop || 0)] : [ev.pageX,
            ev.pageY + (s.scrollNode && s.scrollNode.scrollTop || 0)
        ];
    },
    ...
    mousemove: function(ev) {
        ...
        var cancel = true,
            evc = (ev.type == 'scroll') ? [sorter.lastX, sorter.lastY + scrollNode.scrollTop + (s.scrollNode && s.scrollNode.scrollTop || 0) - sorter.lastS] : sorter.evCoords(
                ev, s);
        if (ev.type == 'scroll') {
            cancel = false;
        } else {
