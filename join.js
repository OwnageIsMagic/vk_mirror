var Join = {
    ...
    scrollResize: function(e, pp) {
        ...
        if (cur.docked && e && e.type == 'resize') {
            if (browser.msie6) {
                ...
                cur.nextButton.style.left = (ge('page_layout')
    ...
    init: function(section, selData) {
        ...
        if (cur.preload[0]) {
            ajax.preload('join.php', extend(cur.params, {
                from: cur.from
            }), cur.preload);
        }
        Join.initScroll();
    ...
    focusWithoutDropdown: function() {
        ...
        dd.focus();
        dd.select.hide();
        Join.tipShow(cur.section + '_' + cur.section + '_tip', 'join_' + cur.section + '_tip', [-35, (cur.section == 'school') ? -52 : -40, 3], 'join_' + cur.section +
            '_tt');
    },
    yearTip: function() {
    ...
    updateUniversity: function() {
        if (!intval(cur.uiCountry.val())) {
            ...
            ajax.post('join.php', {
                act: 'university_last',
                university: cur.uiUniversity.val(),
                faculty: cur.uiFaculty.val(),
                chair: cur.uiChair.val()
            });
    ...
    go: function(el, section) {
        return nav.go('/join?act=' + section, false, {
    ...
    skip: function(el, ev) {
        var prg = ce('div', {
                className: 'progress fl_r join_skip_prg'
            }),
            prnt = el.parentNode;
        return nav.go(el, ev, {
            showProgress: function() {
    ...
    addFriend: function(el, mid) {
        ...
        ajax.post('join.php', extend(params, {
            act: 'add_friend',
            mid: mid,
            year: cur.uiYear.val(),
            hash: cur.hash
    ...
    updateStrength: function() {
        var pwd = val('join_pass'),
            level = -1,
            pwds = ['qwerty', '������', 'gfhjkm', '������', 'password', 'abc123', 'fuckyou', '123abc', 'baseball', 'football', 'soccer', 'monkey', 'liverpool', 'princess',
                'jordan', 'slipknot', 'superman', 'iloveyou'
            ];
        ...
        if (pwd.length < 6) {
            level = 0;
        } else if (pwd.match(/^\d+$/) || indexOf(pwds, pwd) >= 0 || pwd.substr(-1)
            .match(/\d/) && pwds.indexOf(pwd.substr(0, pwd.length - 1)) >= 0) {
            level = 1;
        } else {
            ...
            if (level < 0 && big > 1) {
                ...
                if (symbs.length > 1 && symbs.replace(new RegExp(escapeRE(symbs.charAt(0)), 'g'), '')
                    .length) {
                    level = 4;
                }
            }
        ...
        ge('join_about_pass')
            .className = 'join_pwd_level' + level;
        ge('join_pass_strength')
            .innerHTML = pwd ? getLang('join_pwd_level' + level) : '&nbsp;';
    ...
    initPhoneCode: function() {
        ...
        var el = utilsNode.appendChild(ce('div', {
            innerHTML: '\
<form method="POST" action="' + vk.loginscheme + '://login.vk.com/?act=check_code&_origin=' + locProtocol + '//' + locHost +
                '" id="join_code_form" name="join_code_form" target="join_code_frame">\
...
'
    ...
    getPhone: function() {
        var phone = trim(val('join_phone')),
            prefix = trim(val((ge('join_phone_prefix') || {})
                .firstChild) || '');
        return prefix + phone;
    },
    ...
    askCaptcha: function(sid, dif) {
        if (!cur.codeForm) return;
        unlockButton('join_send_code');
        window.badCodeBox = showCaptchaBox(sid, dif, window.badCodeBox, {
    ...
    submitPasswordSure: function() {
        showFastBox(getLang('join_new_page_sure_title'), '<div class="join_sure_new_page">' + getLang('join_new_page_sure') + '</div>', getLang('join_new_page_sure_submit'),
            Join.submitPassword.pbind(-1), getLang('global_cancel'));
    },
    submitPassword: function(toAlready, fbLoginTo) {
        ...
        if (cur.sureBoxText && toAlready !== 1 && toAlready !== -1) {
            showFastBox({
                    ...
                    width: 430,
                    noCloseButton: true,
                    bodyStyle: 'padding: 16px 14px 5px'
                }, cur.sureBoxText)
                .changed = true;
            ge('join_submit_old')
                .style.position = ge('join_submit_new')
    ...
    codeFailed: function(triesLeft) {
        ...
        if (triesLeft < 0) {
            ...
            text += '<br>' + getLang('join_tries_left')
                .replace('{count}', '<b>' + triesLeft + '</b>');
        }
    ...
    resendUpdate: function() {
        if (cur.resendDelay > 0) {
            ge('join_resend')
                .innerHTML = getLang('join_resend_code_time')
                .replace('%s', Math.floor(cur.resendDelay / 60) + ':' + (cur.resendDelay % 60 < 10 ? '0' : '') + (cur.resendDelay % 60));
            cur.resendDelay--;
        } else {
            ge('join_resend')
                .innerHTML = '<a id="join_resend_lnk" onclick="return Join.noCode()">' + getLang('join_no_code') + '</a>';
            clearInterval(cur.resendInt);
        }
    },
    noCode: function() {
    ...
    codeTip: function() {
        if (ge('join_code')
            .readOnly || cur.strongCode > 0) return;
