...
function removeMainTopic(id) {
    ...
    ajax.onDone = doneFunction('edit_main_topic_progress', function(response) {
        if (response.success) {
            ge('main_topic_text' + id)
                .style.textDecoration = 'line-through';
            ge('main_topic_del_link' + id)
                .href = 'javascript: restoreMainTopic(' + id + ')';
        } else {
            showMessage({
                text: '����������� ������: ' + response.error
            });
...
function restoreMainTopic(id) {
    ...
    ajax.onDone = doneFunction('edit_main_topic_progress', function(response) {
        if (response.success) {
            ...
            ge('main_topic_del_link' + id)
                .href = 'javascript: removeMainTopic(' + id + ')';
        } else {
            showMessage({
                text: '����������� ������: ' + response.error
            });
...
function initMoveTopicSections(index, parent_id) {
    ...
    ajax.onDone = doneFunction('section_progress' + index, function(response) {
        if (!response.error) {
            next_section.innerHTML = '<a id="move_topic_link' + index + '" href="javascript: moveTopic(ge(\'section' + index +
                '\').value)">���������</a><input id="section' + index + '" name="section' + index + '" />';
            var sections_json = [
                [0, ' - ��������� �� ������ - ']
            ];
...
function moveTopic(parent_id) {
    var topic_id = ge('move_topic_id')
        .value;
    var ajax = new Ajax();
    ajax.onDone = doneFunction('box_loader', function(response) {
        if (response.success) {
            if (ge('topic_parent' + topic_id)) {
                ge('topic_parent' + topic_id)
