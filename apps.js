...
function makeFRequest(url, n) {
    ...
    if (n == 3) {
        var seed = ge('reply_field')
            .value;
        var hash = '&hash=' + ge('hash')
            .value;
    }
...
function showComms() {
    ...
    ge("showcommlink")
        .innerHTML = "<a href='javascript: hideComms()'>" + app_comments_hidecomms + "</a>";
}
...
function hideComms() {
    ge('allcomments')
        .style.display = 'none';
    ge("showcommlink")
        .innerHTML = "<a href='javascript: showComms()'>" + app_comments_showcomms + "</a>";
}
...
function disable_app(id) {
    ...
    ajax.onDone = function() {
        ge('disable_' + id + '_href')
            .href = 'javascript: enable_app(' + id + ')';
        ge('disable_' + id + '_href')
            .innerHTML = 'разблокировать';
    };
    ajax.post('apps.php', {
        'act': 'disable',
        'id': id
...
function enable_app(id) {
    ...
    ajax.onDone = function() {
        ge('disable_' + id + '_href')
            .href = 'javascript: disable_app(' + id + ')';
        ge('disable_' + id + '_href')
            .innerHTML = 'заблокировать';
    };
    ajax.post('apps.php', {
        'act': 'enable',
        'id': id
...
function editComment(cid) {
    ...
    ajax.onDone = function(obj, text) {
        ...
        commtext.innerHTML = '<textarea id="comment' + cid + 'edit" style="' + padding_left + 'width: ' + width + '; height: ' + commtext.offsetHeight + 'px; margin-top: ' +
            ...
            '); else if (event.keyCode == 13 && event.ctrlKey) doEditComment(' + cid + ')">' + text + '</textarea>' +
