var Places = {
    ...
    initPhotoMap: function(opts) {
        ...
        if (opts.provider == 'mapbox') {
            ...
            if (!window.yandexMapsInited) {
                ...
                headNode.appendChild(ce('script', {
                    ...
                    src: (window.locProtocol || 'http:') + '//api-maps.yandex.ru/2.1/?lang=' + (cur.vkLngCode == 'ru' ? 'ru_RU' : 'en_US') +
                        '&onload=yandexMapInit&load=package.standard'
                }));
                return false;
            }
        }
                ...
                while (len--) {
                    ...
                    if (subPoint.overlay) {
                        ...
                        if (isMapbox) {
                            ...
                            subPoint.overlay.on('click', (pointClick)
                                .pbind(subPoint));
                        } else {
        ...
        function checkPointsReq(points, mapBounds) {
            ...
            for (i in points) {
                ...
                var inside = (point.lat + diff > mapBounds.neLat && point.lat - diff < mapBounds.swLat && point.lng + diff > mapBounds.neLng && point.lng - diff <
                    mapBounds.swLng);
        ...
        function getIconHtml(imgSrc, cnt, diff, iconNum, addClass) {
            ...
            html = '<div' + (len ? '' : ' id="profile_map_icon_' + iconNum + '"') + ' class="profile_map_photo profile_map_first" style="background: url(' + imgSrc +
                ') center center no-repeat;' + (len ? ' margin-left: -2px; margin-top: -4px;' : '') + '">' + html + '</div>';
            while (len--) {
                html = '<div' + (len ? '' : ' id="profile_map_icon_' + iconNum + '"') + ' class="profile_map_photo"' + (len ?
                    ' style="margin-left: -2px; margin-top: -4px;"' : '') + '>' + html + '</div>';
            }
            return html;
        }
        ...
        if (isMapbox) {
            ...
            if (opts.points) {
                ...
                while (len--) {
                    ...
                    point.overlay.on('click', (pointClick)
                        .pbind(point));
                    if (point.points) {
                        expandPoint(point);
                    }
    ...
    selectPhoto: function(pids, photoSrc, lat, lng) {
        ...
        placeholderSetup(ge('place_map_point_search'), {
            back: true
        });
    ...
    setPlaceStr: function(loc, noRemember) {
        ...
        vkMaps.load('yandex', function() {
            var geocoder = new vkMaps.Geocoder('yandex', function(place) {
                ...
                ge('place_map_edit_add')
                    .innerHTML = getLang('places_add_to_point')
                    .replace('%s', str);
            });
            geocoder.geocode({
                lat: loc.lat,
                lon: loc.lon,
                language: cur.vkLngCode
            })
        });
    },
    ...
    onPhotoUploadStart: function(info, res) {
        ...
        if (info.totalCount > 1) {
            ge('places_add_p_str')
                .innerHTML = langNumeric(info.num ? info.num + 1 : 1, cur.lang['photos_add_uploading_num_X'])
                .replace('{count}', info.totalCount);
        } else {
            ge('places_add_p_str')
                .innerHTML = getLang('photos_add_uploading');
        }
    },
    ...
    onPhotoUploadProgress: function(ind, upl, need) {
        ...
        if (ind.totalCount > 1) {
            ge('places_add_p_str')
                .innerHTML = langNumeric(ind.num + 1, cur.lang['photos_add_uploading_num_X'])
                .replace('{count}', ind.totalCount);
        }
        if (w > oldWidth) {
            animate(inner, {
                width: w
