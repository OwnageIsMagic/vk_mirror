window['public'] = window.Public = {
    toggleFave: function(btn, hash, act, ev) {
        ...
        ajax.post('fave.php', {
            act: act ? 'a_add_group' : 'a_delete_group',
            gid: -cur.oid,
            hash: hash
        }, {
    ...
    showMapBox: function(address, zoom, link) {
        ...
        if (!cur.boxForMap) {
            var boxOptions = {
                ...
                dark: 1,
                onShow: function() {
                    if (cur.boxMap) cur.boxMap.redraw();
                }
            };
    ...
    changeLikeBox: function(from, to) {
        from = ge(from);
        to = ge(to);
        var oldH = getSize(from)[1],
            newH = getSize(to)[1];
        setStyle('public_like_module', {
            height: oldH
        });
        setStyle(from, {
            position: 'absolute',
        ...
        if (oldH != newH) animate('public_like_module', {
            height: newH
        }, 200);
    },
    ...
    feedListsDDShow: function(loaded) {
        ...
        for (var i in cur.options.feedLists) {
            ...
            elems.push('<a id="public_feed_item' + i + '" class="public_actions_item public_feed_item' + (cur.feedListsSet && cur.feedListsSet[i] ? ' checked' : '') +
                '" onclick="Public.feedListsCheck(this, ' + i + ');">' + lname + '</a>');
        }
    ...
    showEdit: function(tab) {
        ...
        hide('public_edit_link');
        show('public_edit_progress');
        var self = this;
        ajax.post('al_public.php', {
            act: 'a_settings',
            pid: cur.options.public_id
        }, {
            onDone: function(html, headers, js, extra) {
                ...
                var onEnd = function() {
                    show('public_edit_link');
                    setStyle(ge('info_module_wrap'), {
    ...
    searchApp: function(page, onSearch) {
        ...
        ajax.post('al_public.php', {
            ...
            onDone: function(err, aid, img, info, hash, newHref) {
                if (err) {
                    ge('public_app_error_msg')
                        .innerHTML = err;
                    cur.appId = false;
                    return;
                }
    ...
    uploadPhotos: function(el, event) {
        var hasHTML5 = (window.XMLHttpRequest || window.XDomainRequest) && (window.FormData || window.FileReader && (window.XMLHttpRequest && XMLHttpRequest.sendAsBinary ||
...
function initPublicNewSection(options) {
    extend(cur, {
        newPageSubmit: function(el) {
            var name = trim(ge('new_public_name')
                .value);
            if (!name) {
                notaBene('new_public_name');
                return;
            }
            var publicType = ge('new_public_type')
                .value;
            if (publicType < 0) {
                ge('new_public_error')
                    .innerHTML = getLang('public_new_no_type_error');
                show('new_public_error_wrap');
                return;
            }
            var publicConfirm = ge('new_public_confirm')
                .value;
            if (!publicConfirm) {
                ge('new_public_error')
                    .innerHTML = getLang('public_new_no_agreement_error');
                show('new_public_error_wrap');
                return;
            }
            lockButton(el);
