            ...
            onDone: function(ajaxObj, responseText) {
                ...
                if (vars) {
                    ...
                    showVideoBox(vars, elem,
                        decodeURIComponent(vars.desc)
                        .replace(/\+/g, ' '),
                        decodeURI(vars.to_comments_text)
                        .replace(/\+/g, ' '),
                        decodeURI(vars.add_text)
                        .replace(/\+/g, ' '),
                        vars.add_hash,
...
(function() {
        ...
        switch (type) {
            case 'photo':
                ...
                if (!wikiImgCache[src]) {
                    ...
                    wikiMb.content('<div class="box_loader"></div>')
                        .show();
                    setTimeout(function() {
                        img.src = src;
                    }, 100);
    ...
    var showPhoto = function(img) {
        ...
        wikiMb.content('<div style="height: ' + img.height + 'px; text-align: center;" onclick="wikiMb.hide()"><img  src="' + img.src + '" /></div>')
            .show();
    };
...
function renderPhoto(url, photo_index, noteIndex) {
    ...
    if (notePhotosCount > 1) {
        if (typeof(photoNotesCount) != 'undefined') {
            photoBox.content('<a href="#next" onclick="showNextPhoto(\'' + photosOfItem[noteIndex][nextIndex] + '\', null, \'' + nextIndex + '\', ' + noteIndex +
                    '); return false;"><img id="currentPhoto" src="' + url + '" /></a>')
                .show();
        } else {
            photoBox.content('<a href="#next" onclick="showNextPhoto(\'' + photosOfItem[nextIndex] + '\', null, \'' + nextIndex + '\', ' + noteIndex +
                    '); return false;"><img id="currentPhoto" src="' + url + '" /></a>')
                .show();
        }
    } else {
            ...
            if (!window.wikiPopupBox) {
                ...
                wikiPopupBox.loadPage = function(gid, page, isById) {
                    if (isById) {
                        ...
                        wikiPopupBox.loadContent('/page-' + gid + '_' + page)
                            .show();
                    } else {
                        wikiPopupBox.setOptions({
                            title: page
                        });
                        wikiPopupBox.loadContent('/pages.php?o=-' + gid + '&p=' + page)
                            .show();
                    }
                }
