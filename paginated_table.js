...
(function() {
    ...
    var setInnerHTML = (function() {
        ...
        return function(par, propName, html) {
            ...
            tempNode.innerHTML = ['<', node.tagName, ' ', (node.id ? 'id="' + node.id + '" ' : ''), (node.className ? 'class="' + node.className + '" ' : ''), '>',
                html, '</', node.tagName, '>'
            ].join('');
    ...
    var ceRowHTML = (function() {
        ...
        return function(params) {
            var newHTML = ['<tr ', (params.id ? 'id="' + params.id + '" ' : ''), (params.className ? 'class="' + params.className + '" ' : ''), '>', params.innerHTML,
                '</tr>'
            ].join('');
        ...
        if (this.parent.tableSize) {
            ...
            if (this.smartFromTo) {
                ...
                if (typeof f == 'function') {
                    ...
                    this.smartFromTo.innerHTML = f.replace('{from}', from)
                        .replace('{to}', to);
                }
            }
            if (this.smartTotal) {
    ...
    window.PaginatedTable.prototype.addContent = function(content) {
        ...
        _t.content.data = (_t.content.data || [])
            .concat(content.data);
        _t.content.types = (_t.content.types || [])
            .concat(content.types);
        _t.content.eventListeners = (_t.content.eventListeners || [])
            .concat(content.eventListeners);
        ...
        if (keyN == -1) {
            ...
            for (i = 0; i < sortParams.length; i++) {
                if (sortParams[i].indexOf('secondary=') != -1) {
                    var dataTree = sortParams[i].substring(10)
                        .split('.');
    ...
    window.PaginatedTable.prototype._constructTable = function(visible) {
        ...
        if (browser.msie && parseFloat(browser.version) < 9.0) {
            cell = ce('<td colspan="' + (content.header ? content.header.length + (_t.options.checkboxPickingEnabled & !_t.options.representation.HIDE_CHECKBOX_ON_EMPTY) :
                1) + '">', {
    ...
    window.PaginatedTable.prototype._constructFooter = function() {
        ...
        function calculateFooterValue(formula, arguments) {
            ...
            switch (formula) {
                ...
                case 'average':
                    ...
                    break;
                case 'label':
                    text = getLang('ads_paginated_table_footer_total')
                        .replace('{total}', content.data.length);
                    break;
                default:
                    text = formula;
                    break;
        ...
        for (i = 0; i < deferred_cells.length; ++i) {
            ...
            footerRow.childNodes.item(deferred_cells[i].cell_index)
                .innerHTML = text;
        }
    ...
    window.PaginatedTable.prototype._constructRow = function(i) {
        ...
        for (j = 0; j < content.data[i].length; j++) {
            ...
            curHTML.push(['<td id="cell_', i, '_', j, '" class="', getCellClass(_t, j + jadd, n), '"', _t.options.noBrEnabled ? 'style="white-space: nowrap;"' : '', '>',
                dataStr, '</td>'
            ].join(''));
        }
        var curRow = null;
        ...
        switch (format) {
            ...
            case 'delim_float':
                s = data.toString()
                    .replace(',', '.');
                s = Number(s)
                    .toFixed(FLOAT_FORMAT_PRECISION);
                var tn = s.indexOf('.');
            ...
            case 'percent_float':
                ...
                s = data.toString()
                    .replace(',', '.');
                s = Number(s)
                    .toFixed(PERCENT_FORMAT_PRECISION);
                return s + '&nbsp;%';
