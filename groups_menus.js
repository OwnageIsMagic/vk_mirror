...
function create_group() {
    create_box.content('<div class="ta_l"><div style="margin-bottom: 5px">' + groups_create_enter +
        '</div><input id="new_name" name="new_name" type="text" style="width: 290px; margin: 3px 0px 0px;"/><div id="groups_new_public_tip">' + getLang('groups_new_public_tip') +
        '</div>');
    create_box_show();
}
...
var refinedSearch = function(searchItems, itemsContainer, itemContainerId, itemNameId, summaryRefined, noItemsFoundCont, noItemsFoundContText) {
    ...
    var prepare = function() {
        ...
        if (t) {
            ...
            lookupResults.id = 'refinedSearchResults' + Math.random()
                .toString()
                .replace(".", "");
    ...
    var performLookup = function(input, event) {
        ...
        for (i = 0; i < preparedArray.length; i++) {
            ...
            var matchPos = rawName_lc.toLowerCase()
                .indexOf(inputVal_lc);
            if (matchPos != -1) {
                if (!item[3]) {
