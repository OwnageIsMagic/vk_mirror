...
function set_default_params() {
    params['forum_id'] = ge('forum_id')
        .innerHTML;
    params['leader'] = ge('leader')
        .innerHTML;
    params['supporter'] = parseInt(ge('supporter')
        .value);
    params['act'] = ge('cur_act')
        .innerHTML;

    if ((params['act'] == 's') || (params['act'] == 'default')) {
        params['section_id'] = ge('section_id')
            .innerHTML;
        params['parent_id'] = ge('parent_id')
            .innerHTML;
        params['path'] = ge('path')
            .innerHTML;
        ...
        params['page'] = '';
        params['count'] = ge('results_count')
            .innerHTML;
        params['pages_count'] = ge('pages_count')
            .innerHTML;
        params['per_page'] = ge('per_page')
            .innerHTML;
        ...
        params['page'] = '';
        params['count'] = ge('posts_count')
            .innerHTML;
        params['per_page'] = ge('posts_per_page')
            .innerHTML;
        params['pages_count'] = ge('pages_count')
            .innerHTML;
        params['topic_id'] = ge('topic_id')
            .innerHTML;
        params['path'] = ge('path')
            .innerHTML;

        params['topic_closed'] = ge('topic_closed')
            .innerHTML;
        if (parseInt(params['topic_closed'])) {
            if (ge('open_topic_button')) {
                ge('open_topic_button')
                    .style.display = 'inline';
            }
            ge('topic_closed_notification')
                .style.display = 'block';
        } else {
            if (ge('close_topic_button')) {
                ge('close_topic_button')
                    .style.display = 'inline';
            }
        }

        params['topic_sticked'] = ge('topic_sticked')
            .innerHTML;
        if (parseInt(params['topic_sticked'])) {
            if (ge('unstick_topic_button')) {
                ge('unstick_topic_button')
                    .style.display = 'inline';
            }
        } else {
            if (ge('stick_topic_button')) {
                ge('stick_topic_button')
                    .style.display = 'inline';
            }
        }
        ...
        if (!ge('filter_link')) {
            ge('results_header')
                .innerHTML = "<a style='float: right; margin-top: 7px' id='filter_link' href='javascript: show_section_tree()'>������&#9660;</a>" + ge('results_header')
                .innerHTML;
        }
...
function update_page_links() {
    ...
    if ((last_page == 1) && (cur_page == last_page)) {
        ...
        ge('page_links_up')
            .innerHTML = '<li><img style="display: none" id="loading_pages_up" src="/images/upload.gif"/></li>' + links;
        ge('page_links_down')
            .innerHTML = '<li><img style="display: none" id="loading_pages_down" src="/images/upload.gif"/></li>' + links;
    }
}
///////////////////////////////////////// Loading pages }
///////////////////////////////////////// Working with AJAX #... params }
...
function show_answer(id) { // In section
    ...
    ge("topic" + id + "answershow")
        .style.display = "none";
    ge("topic" + id + "answerhide")
        .style.display = "inline";
    ge("topic" + id + "answerlink")
        .href = "javascript: hide_answer(" + id + ");";
}
...
function hide_answer(id) { // In section
    ...
    ge("topic" + id + "answerhide")
        .style.display = "none";
    ge("topic" + id + "answerlink")
        .href = "javascript: show_answer(" + id + ");";
}
///////////////////////////////////////// Working with right answer }
...
function show_section_tree() {
    ge('filter_link')
        .innerHTML = '������&#9650;';
    ge('filter_link')
        .href = 'javascript: hide_section_tree()';
    ge('section_tree')
        .style.display = 'block';
}
...
function hide_section_tree() {
    ge('filter_link')
        .innerHTML = '������&#9660;';
    ge('filter_link')
        .href = 'javascript: show_section_tree()';
    ge('section_tree')
        .style.display = 'none';
}

function check_filter(section_id) {
    ge('filter' + section_id)
        .className = 'forum_filter_checked';
    ge('filter' + section_id)
        .href = 'javascript: uncheck_filter(' + section_id + ')';
    var filtered = ge('forum_filter')
        .innerHTML.split(','),
    ...
    ge('forum_filter')
        .innerHTML = new_filtered.join(',');
}

function uncheck_filter(section_id) {
    ge('filter' + section_id)
        .className = 'forum_filter_unchecked';
    ge('filter' + section_id)
        .href = 'javascript: check_filter(' + section_id + ')';
    var filtered = ge('forum_filter')
        .innerHTML.split(',');
    for (var i = 0, count = filtered.length; i < count; ++i) {
        if (parseInt(filtered[i]) == section_id) {
...
function commentshow(index) {
    ge('comment' + index)
        .style.display = 'block';
    ge('commentshow' + index)
        .style.display = 'none';
    ge('commenthide' + index)
        .style.display = 'block';
    return false;
}
...
function show_details(id) { // In section
    ge("topic" + id + "details")
        .style.display = "block";
    ge("topic" + id + "details")
        .innerHTML = '<div style="text-align: center; margin: 20px 0px"><img src="/images/progress7.gif"/></div>';
    ...
    ge("topic" + id + "detailsshow")
        .style.display = "none";
    ge("topic" + id + "detailshide")
        .style.display = "inline";
    ge("topic" + id + "detailslink")
        .href = "javascript: hide_details(" + id + ");";
}
...
function hide_details(id) { // In section
    ...
    ge("topic" + id + "detailshide")
        .style.display = "none";
    ge("topic" + id + "detailslink")
        .href = "javascript: show_details(" + id + ");";
}
...
function showAutoAnswersBox(tid) {
    ...
    str +=
        "<div><table style='margin-top:5px;margin-bottom:5px;' cellpadding=5><tbody><tr><td style='width:97px;'><input id='answerName' type='text' style='text-align:left; width:55px; padding: 2px 10px 0px;' value=''></td><td><a href='javascript:addAutoAnswer();'>��������</a></td></tr></tbody></table></div>";
...
function addAutoAnswer() {
    ...
    ajax.onDone = getAutoAnswersSuccess;
    //ajax.onFail = opFailed;
    answer_name = ge('answerName')
        .value;
    answer_text = ge('answerText')
        .value;
...
function can_be_notified(value) {
    ...
    if (value) {
        ge('can_be_notified')
            .href = 'javascript: can_be_notified(0)';
        ge('can_be_notified')
            .innerHTML = '��������� ���������';
    } else {
        ge('can_be_notified')
            .href = 'javascript: can_be_notified(1)';
        ge('can_be_notified')
            .innerHTML = '��������� ���������';
    }
}
