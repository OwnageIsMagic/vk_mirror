(function() {
    ...
    window.inlineMenu = function(sections, admin_href, admin_name, hash) {
        ...
        if (!menuBox) {
            ...
            if (section_id) {
                show_all_phrases = '<div style="margin-top:8px" class="button_gray button_wide" ><button onclick="showAllPhrases(\'' + sections +
                    '\');">Show all phrases</button></div>';
            }
            if (admin_href == 'super') {
                invitation = '<div style="text-align:center;">You are super user.<br /><a href="' + base_domain +
                    'translation.php?act=translators">Add translators &raquo;</a></div>';
            } else if (admin_href == 'coordinator') {
                invitation = '<div style="text-align:center;">You are coordinator.<br /><a href="' + base_domain +
                    'translation.php?act=translators">Add translators &raquo;</a></div>';
            } else if (admin_href) {
                invitation = '<a href="' + admin_href + '">' + admin_name +
                    '</a> has invited you to translate this page. <a href="#" onclick="return showInvitationBox();">Invite friend &raquo;</a>';
                inviteHash = hash;
            }

            if (invitation) {
                invitation =
                    '<div id="invitationWrap" style="background:#F7F7F7;border-bottom:1px solid #CCC;padding:6px 9px;height:30px; overflow:hidden;line-height:130%"><div id="invitationText">' +
                    invitation + '</div><div id="inviteBox" style="display:none;text-align:center; margin-top:4px"><img src="' + base_domain +
                    'images/upload.gif" id="friendsProgress" style="margin-top:7px" /><table cellspacing="0" cellpadding="0" style="display:none" id="inviteControls"><tr><td><input type="hidden" class="inputText" id="inviteFriendId" name="invite_friend_id" /></td><td style="padding-left:6px"><a href="#" onclick="return doInvite();">Send</a></td></tr></table></div></div>';
            }

            menuBox.content(invitation + '<div style="padding:14px"><div class="button_blue button_wide"><button onclick="enableTranslation()">' + inline_translation +
                '</button></div>' + show_all_phrases + '<div style="margin-top:8px" class="button_gray button_wide"><button onclick="location.replace(\'' + base_domain +
                'translation.php?section_id=' + section_id +
                '\');">Go to translation page</button></div><div style="padding:5px 5px 0px 5px"><a href="club16000">Help &raquo;</a><br /><a href="translation.php?section_id=1000">Untranslated &raquo;</a></div></div>'
            );
        }
        menuBox.show();
        return false;
    ...
    window.doInvite = function() {
        ...
        Ajax.Post({
            ...
            onDone: function(ajaxObj, responseText) {
                ...
                invitationText.innerHTML = '<div style="text-align:center">' + text +
                    '<br /><a href="#back" onclick="return showInvitationBox();">Go back</a></div>';
                hide('inviteBox');
                show(invitationText);
            }
        });
...
window.batchSearchKey = function(force) {
    var key = trim(ge('batch_key_search_input')
        .value);
    ...
    Ajax.Post({
        ...
        onDone: function(ajaxObj, responseText) {
            ...
            var results = JSON.parse(JSON.parse(responseText)
                .values);
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                resultHTML += '<div class="batch_edit_item" >' + mcDiff + '<span class="batch_edit_item_value" data-language-id="' + lang.language_id + '">' + lang
                    .value + '</span>';
...
window.onPatternChange = function() {
    var patt = trim(ge('batch_pattern')
        .value);
    if (prevBatchPattern == patt) {
        return;
    }

    var globalFlag = ge('regexp_global')
        .checked ? 'g' : '';
...
window.onPatternSubstChange = function() {
    var subst = trim(ge('batch_pattern_subst')
        .value);
    var patt = trim(ge('batch_pattern')
        .value);
