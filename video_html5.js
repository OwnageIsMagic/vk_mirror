...
function updTime() {
    ...
    if (video) {
        ge('prSlider')
            .style.left = ((prLineW * video.currentTime) / video.duration) + 'px';
        ge('curtime')
            .innerHTML = formatTime(video.currentTime);
        ge('duration')
            .innerHTML = formatTime(video.duration);
    }
}
...
function showLoading() {
    ...
    ge('loadingGif2')
        .style.display = notLoaded ? 'block' : 'none';
}
...
function initHTML5Player() {
    ...
    ge('volume-line')
        .style.opacity = 0.75;
    ge('video_title')
        .innerHTML = decodeURIComponent(video_title)
        .replace(/\+/g, ' ');
    ge('video_author')
        .innerHTML = decodeURIComponent(video_author)
        .replace(/\+/g, ' ');
