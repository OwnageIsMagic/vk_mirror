...
var audioPlayer = {
    ...
    songInfos: {},
    shareMusic: function() {
        var mus = ((window._pads && _pads.shown == 'mus' && audioPlayer.isPlaylistGlobal() ? ls.get('audio_id') : null) || currentAudioId() || (audioPlayer.lastSong &&
                audioPlayer.lastSong.aid) || (cur.defaultTrack && cur.defaultTrack.id) || window.audioPlaylist && audioPlaylist.start || '')
            .match(/^(-?\d+_\d+)(?:_(s-?\d+(?:_h-?\d+)?))?/);
        if (!mus) return;
    ...
    initPlayer: function(id) {
        ...
        if (browser.flash >= 9 && !(browser.mac && (browser.chrome || browser.safari) && html5AudioSupport)) {
            ...
            var t = 'flash_audio';
            if (!ge(t)) document.body.appendChild(ce('div', {
                id: t,
                className: 'fixed'
            }));
            if (renderFlash(t, opts, params, vars)) {
                setTimeout(function() {
                    _a.checkFlashLoaded(id);
                }, 50);
            }
        } else {
            if (html5AudioSupport) {
                var t = 'html5_audio';
                if (!ge(t)) document.body.appendChild(ce('audio', {
                    id: t
                }));
        ...
        if (!ge('gp') && !_a.gpDisabled) {
            ...
            addEvent(ge('gp'), 'dragstart', function() {
                return false;
            });
            addEvent(ge('gp'), 'selectstart', function() {
                return false;
            });
            ge('gp')
                .innerHTML =
                '<div class="wrap" onmouseover="addClass(this, \'over\');" onmouseout="removeClass(this, \'over\');">\
  ...
  <div id="gp_wrap">\
    ...
    setPlayer: function(element, id) {
        if (element) {
            ...
            showFastBox({
                ...
                dark: 1
            }, message, getLang('share_go'), function() {
    ...
    getSongInfoFromDOM: function(aid) {
        var art, title, nfo = geByClass1('info', ge('audio' + aid)),
            full_id = aid,
            l;
        ...
        l = geByTag1('a', art);
        if (l) art = l;
        var reArr = ['<span>', '</span>', '<span class="match">', '<font>', '</font>'],
            re = new RegExp(reArr.join('|'), "gi");
        ...
        var data = ge('audio_info' + aid)
            .value.split(','),
            url = data[0],
            duration = parseInt(data[1]);
        dur = audioPlayer.formatTime(duration);
        data = aid.split('_');
        var oid = data[0],
            aid = data[1],
            canAdd = (oid != vk.id) ? 1 : 0;
        if (!canAdd && data[2] && data[3]) {
            var wpt = ge('wpt' + data[2] + '_' + data[3]),
                post = wpt,
                postFound = false;
            if (wpt) {
                while (post) {
    ...
    reloadAudio: function(id) {
        ...
        if (window.cur && (cur.audiosReloadedAll || cur.audiosReloaded && cur.audiosReloaded[id])) {
            setTimeout(showFastBox({
                    ...
                    dark: 1
                }, getLang('audio_error_loading'), getLang('global_close'))
                .hide, 2000);
        } else {
            ajax.post('audio', {
                act: 'reload_audio',
    ...
    sendNotification: function(data) {
        ...
        try {
            if (ge('video_player') && ge('video_player')
                .playVideo) ge('video_player')
                .playVideo(false);
        } catch (e) {}
    },
    setAid: function(aid) {
    ...
    setGraphics: function(act) {
        var _a = audioPlayer,
            head_play = ge('head_play_btn'),
            aid = currentAudioId(),
            _row = ge('audio' + aid),
            pad_row = ge('audio' + aid + '_pad'),
            gp_play = ge('gp_play');
        if (!aid) return;
        switch (act) {
            case 'play':
            ...
            case 'stop':
                ...
                if (_row) {
                    ...
                    if (aid) {
                        var t = _a.duration || _a.lastSong && _a.lastSong[3] || ge('audio_info' + aid)
                            .value.split(',')[1],
                            res = _a.formatTime(t),
                ...
                if (_row) {
                    ...
                    if (ge('player' + aid)) {
                        ...
                        if (ge('audio_vol_line' + aid)) {
                            setStyle(ge('audio_vol_line' + aid), {
                                width: (playerVol * 100) + '%'
                            });
                        }
                    }
                ...
                if (pad_row) {
                    ...
                    if (ge('player' + aid + '_pad')) {
                        ...
                        if (ge('audio_vol_line' + aid + '_pad')) {
                            setStyle(ge('audio_vol_line' + aid + '_pad'), {
                                width: (playerVol * 100) + '%'
                            });
                        }
                    }
    ...
    setPlaybackParams: function() {
        var _a = audioPlayer,
            ...
            ids = (aid || '')
            .split('_'),
            __cur = window._pads && _pads.shown == 'mus' && _pads.cur,
            params = {};
        ...
        if ((aid + '')
            .match(/^-?\d+_\d+_s(-?\d+)(?:_|$)/) && window.audioPlaylist && audioPlaylist[aid]) {
            params.status = 1;
        }
        ...
        if (cur.module == 'audio' && cur.curSection && cur.curSection.substr(0, 5) == 'album' && cur.album_id || __cur && __cur.curSection && __cur.curSection.substr(0, 5) ==
    ...
    isAudioFriend: function(oid, full_id) {
        return (oid != vk.id && cur.allAudiosIndex && cur.allAudiosIndex != 'all' && cur.audioFriend && cur.curSection != 'recommendations' && cur.curSection != 'popular') ||
            ge('audio_playback' + full_id) || (cur.isAudioFriend && cur.curSection != 'recommendations' && cur.allAudiosIndex && cur.allAudiosIndex == 'all');
    },
    playbackSent: 0,
    statusSent: 0,
    playback: function(paused) {
        ...
        if (_a.hasStatusExport() && ids[1] != _a.statusSent) {
            setTimeout(ajax.post.pbind('audio', {
                act: 'audio_status',
                full_id: aid,
                hash: _a.addHash,
                top: _a.playbackParams && (_a.playbackParams.top_audio || _a.playbackParams.top) ? 1 : ''
            }), 0);
            _a.statusSent = ids[1];
        }
    ...
    keyPressed: function(otherTab, e) {
        e = normEvent(e);
        if (!e.keyCode) return;
        var keyCodes = [176, 177, 178, 179, KEY.LEFT, KEY.RIGHT],
            inArr = false;
        ...
        if (!inArr) return;
        var cn = window.curNotifier,
            pData = ls.get('pad_pldata'),
            anotherTab = cn && pData && cn.instance_id && pData.instance && cn.instance_id !== pData.instance;
        if (!e.noBroadcast) {
            if (anotherTab) {
                var _n = window.Notifier;
                if (_n) _n.lcSend('audio_mediakey', {
                    keyCode: e.keyCode,
                    noBroadcast: 1
                });
                return;
    ...
    canChangePr: function(e) {
        ...
        return !e.shiftKey && !e.ctrlKey && !e.metaKey && (cur.module == 'audio' || window._pads && _pads.shown == 'mus') && !cur.pvShown && !(window.wkcur || {})
            .shown && !(window.mvcur || {})
            .mvShown && !curBox() && !(inp && trim(val(inp)) && !(chatInput && trim(val(inp)) == '<br>'));
    },
    ...
    showTip: function(text, x, y, fixed, pad) {
        if (!ge('audio_tip_wrap')) {
            bodyNode.appendChild(se(
                '<div id="audio_tip_wrap" onmouseover="audioPlayer.tipOver();" onmouseout="audioPlayer.tipOut();"><div id="audio_tip"></div><div id="audio_tip_arrow"></div></div>'
            ));
        }
        ge('audio_tip')
            .innerHTML = text;
        var sz = getSize('audio_tip_wrap'),
            w = sz[0] || 32,
            h = sz[1] || 18,
            l = x - (w + 1) / 2,
            a = w / 2 - 3;
        setStyle(ge('audio_tip_wrap'), {
            left: l,
            top: y - h - 7,
            position: fixed ? 'fixed' : 'absolute',
            zIndex: curBox() ? 1001 : (pad ? 401 : 101)
        });
        setStyle(ge('audio_tip_arrow'), {
            left: a
        });
        show('audio_tip_wrap');
    },
    mouseMove: function(e) {
        ...
        if (_a.controls) {
            for (var i in _a.controls) {
                ...
                if (obj.progressArea && hasClass(obj.progressArea, 'over')) {
                    overFound = true;
                    var time = intval(_a.getPrPos(e, i) * _a.duration / 100),
                        fixed = _a.controls[i].fixed,
        ...
        if (!overFound && aid && hasClass(ge('audio_pr' + aid), 'over')) {
            overFound = true;
            var time = intval(_a.getPrPos(e, null) * _a.duration / 100),
                timeText = _a.formatTime(time);
    ...
    prClick: function(e, objName) {
        ...
        if (aid && ge('player' + aid)) {
            setStyle(ge('audio_pr_line' + aid), {
                width: val + '%'
            });
        }
        if (aid && ge('player' + aid + '_pad')) {
            setStyle(ge('audio_pr_line' + aid + '_pad'), {
                width: val + '%'
            });
        }
        cancelEvent(e);
    ...
    setControlsTitle: function() {
        ...
        if (_a.controls) {
            for (var i in _a.controls) {
                var obj = _a.controls[i],
                    songInfo = obj.padPlaylist && _a.isPlaylistGlobal() ? ls.get('pad_lastsong') || _a.lastSong : _a.lastSong;
                if (obj.performer) {
                ...
                if (obj.titleWrap) {
                    if (obj.titleWrap.scrollWidth > obj.titleWrap.clientWidth) {
                        var el = ce('div', {
                            innerHTML: clean(replaceEntities(songInfo[5])) + ' &ndash; ' + clean(replaceEntities(songInfo[6]))
                        });
                ...
                if (obj.add) {
                    var aid = currentAudioId(),
                        cur_aids = aid && aid.split('_') || [],
                        raw_id = aid && cur_aids[0] + '_' + cur_aids[1] || null;
    ...
    loadGlobal: function() {
        var l, _a = audioPlayer,
            aid = currentAudioId(),
            cl = aid && _a.player && !_a.player.paused() || !aid && _a.lastSong ? 'playing' : '';
        ge('gp_play_btn')
            .innerHTML = '<a onmousedown="cancelEvent(event)" onclick="playAudioNew(\'' + aid + '\', false)"><div class="gp_play_wrap"><div id="gp_play" class="' + cl +
            '"></div></div></a>';
        var art, title;
    ...
    onPlayFinish: function() {
        ...
        var next_id, cur_id = currentAudioId(),
            m = (cur_id + '')
            .match(/^-?\d+_\d+_s(-?\d+)(?:_|$)/);
    ...
    nextTrack: function(no_scroll, fromPad) {
        ...
        var aid = fromPad ? ls.get('audio_id') || currentAudioId() : currentAudioId(),
            pList = fromPad ? ls.get('pad_playlist') || window.audioPlaylist : window.audioPlaylist;
    ...
    getNextSibling: function(aid) {
        var getAId = function(el) {
            return el && el.id && el.id.substr(0, 5) == 'audio' && parseInt(el.id.substr(5)) ? el.id.substr(5) : 0;
        }
    ...
    setPadPlaylist: function(pList) {
        if (pList) {
            if (!pList.htitle) {
                ...
                pList.htitle = htitle ? getLang('audio_current_playing_from')
                    .replace('{playlist}', htitle) : '';
                if (cur.nextPlaylist && !cur.nextPlaylist.htitle && cur.nextPlaylist.source == pList.source && cur.nextPlaylist.start == pList.start && cur.nextPlaylist.length ==
                    pList.length) {
    ...
    createDynamicPlaylist: function(aid, copyToMain, limit) {
        var _a = audioPlayer;
        if (!aid) return;
        var nextId = startId = aid,
            el = ge('audio' + aid),
            dynPl = [_a.getSongInfoFromDOM(startId)],
            k = 1;
        if (!limit) limit = 200;
        ...
        if (hasClass(el.parentNode, 'module_body')) {
            ...
            if (local_id && ge('post' + ids[2] + '_' + ids[3])) {
                ...
                var params = {
                    act: 'get_wall_playlist',
                    oid: ids[2],
                    wall_type: wallType,
                    local_id: local_id
                };
                if (cur.module == 'wall' && ge('wall_search') && ge('wall_search')
                    .value) {
                    params.q = ge('wall_search')
                        .value;
                }
    ...
    showCurrentTrack: function() {
        ...
        var _a = audioPlayer,
            aid = currentAudioId(),
            pad_aid = pad_aid_raw = ls.get('audio_id') || aid,
            cur_aids = aid && aid.split('_') || [],
            raw_id = aid && cur_aids[0] + '_' + cur_aids[1] || null;
        if (aid && _a.player) {
            _a.setGraphics('load');
    ...
    hideCurrentTrack: function() {
        var audios = geByClass('audio'),
            playBtns = geByClass('play_new');
    ...
    onLoadProgress: function(bLoaded, bTotal) {
        ...
        if (_a.controls) {
            for (var i in _a.controls) {
                var obj = _a.controls[i];
                if (obj.load) setStyle(obj.load, {
                    width: per + '%'
                });
            }
        }
        if (ge('player' + aid)) {
            setStyle(ge('audio_load_line' + aid), {
                width: per + '%'
            });
        }
        if (ge('player' + aid + '_pad')) {
            setStyle(ge('audio_load_line' + aid + '_pad'), {
                width: per + '%'
            });
        }
    },
    ...
    shuffleAudios: function(fromPad) {
        ...
        if (fromPad && window.Pads) {
            pl = ((!window.audioPlaylist || window.curNotifier && (padPlData = ls.get('pad_pldata')) && padPlData.source == curNotifier.instance_id) ? ls.get(
                'pad_playlist') || window.audioPlaylist : padAudioPlaylist());
    ...
    registerPlayer: function(name, obj) {
        ...
        if (obj.play) {
            addEvent(obj.play, 'click', function(e) {
                var aid = (obj.padPlaylist && audioPlayer.isPlaylistGlobal() ? ls.get('audio_id') : null) || currentAudioId() || (audioPlayer.lastSong &&
                    audioPlayer.lastSong.aid) || (cur.defaultTrack && cur.defaultTrack.id) || window.audioPlaylist && audioPlaylist.start || '';
                if (obj.padPlaylist) {
                    window.padPlClicked = true;
                }
