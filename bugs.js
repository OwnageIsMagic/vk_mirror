Bugs = {
    ...
    subscribeBug: function(btn, subscribe) {
        ajax.post('bugs', {
            ...
            onDone: function(html) {
                if (html) ge('bugs_view_subscribe')
                    .innerHTML = se(html)
                    .innerHTML;
            },
    ...
    saveBug: function(bug_id) {
        ...
        var attachs = [],
            chosen = cur.bugsNewMedia.chosenMedias;
        if (attachs) {
            for (var i in chosen) {
                var att = chosen[i],
                    type = att[0],
        ...
        ajax.post('bugs', query, {
            ...
            onDone: function(description, data, msg) {
                if (description) ge('bugs_view_description')
                    .innerHTML = description;
                if (data) ge('bugs_view_data')
                    .innerHTML = data;
    ...
    choosePhotoUploaded: function(info, params) {
        ...
        ajax.post('al_photos.php', extend({
            ...
            onDone: function(media, data) {
                cur.bugsNewMedia.chooseMedia('photo', media, extend(data, {
                ...
                if (Upload.types[i] == 'fileApi') {
                    ...
                    if (!ge('form' + i + '_progress')) {
                        ...
                        obj.appendChild(ce('div', {
                            innerHTML: '<div class="bugs_progress_wrap">\
            <div id="form' + i +
        ...
        each(mediaTypes || [], function(i, v) {
            ...
            switch (v[0]) {
                case 'photo':
                    handler = function() {
                        if (opts.oneClick) {
                            var inp = ge('bug_photo_input'),
                                par = ge('bugs_view_comments') || curBox() && geByClass1('bugs_new_cont', curBox()
                                    .bodyNode);
                            ...
                            Bugs.addScreen(Bugs.initPhotoUpload.pbind('bas_add_data', {
                                hideOnStart: true
                            }));
                        }
                    }
                ...
                case 'doc':
                    handler = function() {
                        if (opts.oneClick) {
                            ...
                            Bugs.addDoc(Bugs.initDocUpload.pbind('bas_add_data', {
                                hideOnStart: true
                            }));
                        }
                    }
...
<div class="fl_l"><div class="page_attach_progress_wrap" style="margin-top: 3px; margin-bottom: 4px;">\
  <div id="upload' +
                        ind + '_progress" class="page_attach_progress"></div>\
                            ...
                            'bugs_dont_attach') + '\', shift: [6, 3, 3]})" onmouseout="animate(this, {opacity: 0.6}, 200);" onclick="Upload.terminateUpload(' + i +
                        ', \'' + (fileName || i) + '\');"></div>';
        ...
        ajax.post('bugs', {
            ...
            onDone: function(t) {
                ...
                cont.parentNode.insertBefore(ce('div', {
                    id: 'bug_update_edit' + replyId,
                    innerHTML: '\
<textarea class="bug_edit_update" id="bug_update' + replyId + 'edit" onkeydown="Bugs.saveReply(event, \'' +
                        replyId + '\', \'' + hash + '\')" style="width: ' + wdt + '; margin: ' + mrg + ';">' + t +
                        '</textarea>\
<div style="margin: ' + btn_mrg +
                        ' 0 8px -3px; height: 23px">\
  <div class="fl_l button_blue">\
    <button id="save_butn' + replyId +
                        '" onclick="Bugs.doSaveReply(\'' + replyId + '\', \'' + hash + '\')">' + getLang('global_save') +
                        '</button>\
  </div>\
  <div class="fl_l button_gray" style="margin-left: 10px;">\
    <button id="cancel_butn' + replyId +
                        '" onclick="Bugs.cancelEditReply(\'' + replyId + '\')">' + getLang('global_cancel') +
                        '</button>\
  </div>\
  <div id="edit_progress' + replyId +
                        '" style="margin: 5px 0px 0px 10px; vertical-align: 0px; float: left; display: none"><img src="/images/upload.gif"/></div>\
</div>'
                }, {
                    display: 'none'
                }), cont);
                autosizeSetup(ge('bug_update' + replyId + 'edit'), {
                    minHeight: 17
                });
    ...
    doSaveReply: function(replyId, hash) {
        ...
        ajax.post('bugs', {
            ...
            onDone: function(text) {
                var cont = geByClass1('bugs_update_text', ge('bug_update' + replyId)),
                    acts = geByClass1('bugs_update_info', ge('bug_update' + replyId));
                cont.innerHTML = text;
                show(cont, acts);
                re('bug_update_edit' + replyId);
                delete cur.editing;
    ...
    selectTag: function(tag, e) {
        ...
        ge('bugs_selected_tags')
            .appendChild(ge('filter_tag' + tag));
        toggle('selected_tags_wrap', geByClass('bugs_filter_tag', ge('bugs_selected_tags'))
            .length);
        toggle('tags_wrap', geByClass('bugs_filter_tag', ge('bugs_tags'))
            .length);
