var mail = {
    ...
    showMessage: function(msg_id) {
        ...
        mail.initThread(extend(msg.options, {
    ...
    showMsgsList: function(params, back, addLoc) {
        ...
        if (params !== false) {
            var next_section = params.section || 'inbox',
                next_filter = params.filter || 'all';
    ...
    doPost: function() {
        ...
        if (!inp) return;
        var msg = trim(val(inp)),
            chosenMedias = cur.addMailMedia && cur.addMailMedia.chosenMedias || [];
        ...
        ajax.post('al_mail.php', params, {
            onDone: function(msg_id, row, top_msg, loc) {
                if (cur._back_local) {
                    ...
                    if (cur.section == 'outbox') {
                        var r = ce('div', {
                                innerHTML: '<table><tbody>' + row + '</tbody></table>'
                            })
                            .firstChild.rows[0];
                        ge('mail_rows_t')
                            .insertRow(r, 0);
    ...
    showMoreThread: function() {
        ...
        var show_more_link = ge('show_more_link'),
            main_t = geByTag1('tbody', ge('mail_history_t')),
            next_t = ge('mail_history_t_next') && geByTag1('tbody', ge('mail_history_t_next'));
    ...
    showFullHistory: function() {
        ...
        var show_more_link = ge('show_more_link'),
            main_t = geByTag1('tbody', ge('mail_history_t')),
            next_t = ge('mail_history_t_next') && geByTag1('tbody', ge('mail_history_t_next')),
            progress = ge('mail_history_full'),
            prev_progress = val(progress),
            new_row;
        if (show_more_link && isVisible(show_more_link) && next_t) {
            while (new_row = next_t.firstChild) {
                main_t.appendChild(new_row);
    ...
    applyThreadOptions: function(options) {
        ...
        var cached_next = (ge('mail_history_t_next') || {
                rows: []
            })
            .rows.length > 0;
    ...
    showMore: function() {
        ...
        ajax.post('/al_mail.php', extend(mail.getListParams(), {
            offset: cur.offset
        }), {
            onDone: function(options, rows) {
                if (browser.msie || browser.opera) {
    ...
    applyOptions: function(options, switched) {
        ...
        var cached_next = (ge('mail_rows_t_next') || {
                rows: []
            })
            .rows.length > 0;
        setStyle('show_more_link', 'display', options.has_more || cached_next ? 'block' : 'none');
    ...
    checkChange: function(td, mid) {
        ...
        if (cur.messChecked[mid]) {
            ...
            if (intval(mid)) {
                ...
                cur.messCheckedNum = ge('mail_rows_t')
                    .rows.length;
            }
        }
        mail.toggleActions();
    ...
    initThread: function(options) {
        ...
        if (!browser.mobile) {
            autosizeSetup(ge('mail_reply_field'), {
                maxHeight: 300,
                padding: 3
            });
            setTimeout(function() {
        ...
        stManager.add(['page.js', 'page.css'], function() {
            cur.addMailMedia = initAddMedia('mail_add_link', 'mail_media_preview', options.media_types, {
                mail: 1
            });
        });
    },
    initWrite: function(options) {
        ...
        cur.toPeople = new Autocomplete(ge('mail_to'), 'hints.php?act=a_json_friends&from=mail', {
            ...
            customSearch: function(term) {
                return (term.indexOf('@') != -1) ? [
                    [clean(term), clean(term), cur.lang['mail_enter_email_address'], '/images/pics/contact_info.png', 0, '']
                ] : false
            },
        ...
        if (!browser.mobile) {
            autosizeSetup(ge('mail_write_field'), {
                maxHeight: 300,
                padding: 3
            });
            setTimeout(function() {
            ...
            if (itemData !== undefined && typeof(itemData[3]) == 'string' && itemData[3].length) {
                if (itemData[3] == 'none') {
                    ...
                    if (cur.thread.id != itemValue) {
                        ...
                        img.parentNode.href = (itemData[0] > 2e9) ? '/im?sel=c' + (itemData[0] - 2e9) : (itemData[0] > 0 ? '/id' + itemData[0] : '/write?email=' +
                            itemData[0]);
                        ...
                        if (itemData[4]) {
                            ...
                            if (itemData[4] > 0 && itemData[4] < 6) {
                                onl += '<b class="mob_onl mail_show_mob_onl" onmouseover="mobileOnlineTip(this, {mid: ' + itemData[0] +
                                    '})" onclick="mobilePromo()"></b>';
                            }
                            onl = '<div class="mail_envelope_online">' + onl + '</div>';
                        }
                        val('to_online', onl);
                        var history_label = getLang('mail_show_history_user')
                            .replace('{user}', itemData[5]);
                        val('mail_history', '<a id="mail_history_open" href="#" onclick="mail.showHistory(\'' + clean(itemValue) + '\'); return false;">' +
                            history_label + '</a><div id="mail_history_open_progress"><img src="images/upload.gif" /></div>');
    ...
    deleteMsg: function(msg_id) {
        ...
        ajax.post('al_mail.php', {
            ...
            onDone: function(res, body, actions) {
    ...
    restoreSpamMsg: function(msg_id, show, out) {
        ...
        ajax.post('al_mail.php', {
            act: 'a_restore_spam',
            id: msg_id,
            from: suspected ? 'spam' : cur.section,
            hash: cur.mark_hash
        }, {
            onDone: function(res, body, actions) {
                if (cur.section == 'spam' || suspected) {
    ...
    markMsgs: function(act, btn) {
        ...
        var fail = mail.updateList,
            emptyCheckHub = new callHub(function() {
                mail.scrollCheck();
                mail.updateSummary();
                if (!ge('mail_rows_t')
                    .rows.length) {
