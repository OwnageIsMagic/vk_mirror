var notes = {
    ...
    editComment: function(oid, nid, cid) {
        ...
        var onDone = function(text) {
            ...
            commtext.innerHTML = '<textarea id="comment' + cid + 'edit" style="' + padding_left + 'width: ' + width + '; height: ' + commtext.offsetHeight +
                'px; margin-top: ' + margin_top + '; margin-left: ' + margin_left +
                '; line-height: 14px; margin-bottom: 0px;" onkeydown="if (event.keyCode == 27) notes.cancelEditComment(' + cid +
                '); else if (event.keyCode == 13 && event.ctrlKey) notes.doEditComment(' + oid + ', ' + cid + ')">' + text + '</textarea>' +
    ...
    cancelEditComment: function(cid) {
        ...
        geByClass1('notes_comment_text', comment)
            .innerHTML = ge('comment' + cid + 'text')
            .innerHTML;
    },

    doEditComment: function(oid, cid) {
        if (!ge('comment' + cid + 'edit')
            .value) {
            ge('comment' + cid + 'edit')
                .focus();
            return;
    ...
    postComment: function() {
        var comment = ge('notes_comment_input')
            .value;
        if (comment) {
            notes.showError();
            ge('notes_progr')
            ...
            var callback = function(responseText) {
                ge('notes_progr')
                    .style.display = 'none';
                ge('notes_comment_input')
                    .value = '';
                ge('blogcommentsandform')
            ...
            ajax.post('/notes', {
                ...
                'blog_id': ge('blog_id')
                    .value,
                'reply_to': ge('reply_to')
                    .value,
                'comment': comment,
                'hash': ge('hash')
                    .value,
                'add_bookmark': ge('add_bookmark')
                    .value
            }, options);
        } else {
            ge('notes_comment_input')
    ...
    replyTo: function(mid, mname) {
        ge('reply_to')
            .value = mid;
        ge('notes_reply_name')
            .innerHTML = " <a href='javascript:notes.cancelReply()'>" + mname + "</a>";
        ge('notes_comment_input')
            .focus();
    },
    ...
    showDeleteNoteBox: function(nid, hash, lj) {
        var add = lj ?
            '<div style="padding-top: 10px; padding-bottom: 10px;" class="clear_fix"><div class="fl_l checkbox notes_checkbox on" id="lj_delete" onclick="checkbox(this)" style="height: 11px;">' +
    ...
    showNote: function(oid, nid, ncom, toComments) {
        ...
        var dateTextNode = geByClass1('notes_note_byline')
        ...
        geByClass1('notes_comments_count_wide')
            .innerHTML = '';
        ...
        if (oid != cur.viewer_id) {
            var userNotesText = ge('whos' + oid + '_' + nid)
                .innerHTML;
            tabs.children[tabs.children.length - 1].innerHTML = '<li class="tr_r"><a href="/notes' + oid + '" onclick="return nav.go(this, event)">' + userNotesText +
                '</a></li>';
    ...
    showMoreComments: function(oid, nid) {
        var comments = ge('notes_comments_body')
            .children;
    ...
    applyOptions: function(options) {
        ...
        if (cur.all_shown_text) {
            ge('notes_all_shown')
                .innerHTML = cur.all_shown_text;
        }

        if (cur.empty_text) {
            ge('notes_blog_empty')
                .innerHTML = cur.empty_text;
        }
    ...
    wysiwygInit: function() {
        ...
        extendedToolBar +=
            'table,table_delete,insert_row_before,insert_row_after,insert_col_before,insert_col_after,delete_row,delete_col,col_width,link,unlink,citate,character,image,video,audio,doc';
        var editorParams = {
            ...
            html: ge('post')
                .value,
            wiki: ge('post_wiki') ? ge('post_wiki')
                .value : '',
            height: 300,
    ...
    saveNote: function(mid) {
        ...
        if (editor) {
            ge('post')
                .value = editor.convertToWiki();
        }
        if (mid == undefined) {
            mid = '';
        }
        ge('privacy_note')
            .value = Privacy.getValue('note' + mid);
        ge('privacy_notecomm')
            .value = Privacy.getValue('notecomm' + mid);
    ...
    showPhotosBox: function(url, photo_id, photo_index, target) {
        ...
        if (!cur.pic.width) {
            ...
            ge('imageProgressAnimation')
                .style.marginLeft = (target.offsetWidth / 2 - ge('imageProgressAnimation')
                    .width / 2) + "px";
            ge('imageProgressAnimation')
                .style.marginTop = (target.offsetHeight / 2 - ge('imageProgressAnimation')
                    .height / 2) + "px";
            show('imageProgress');
            ge('imageProgress')
                .style.left = (coords[0] - ge('imageProgress')
                    .offsetParent.offsetLeft) + "px";
            ge('imageProgress')
                .style.top = (coords[1] - ge('imageProgress')
                    .offsetParent.offsetTop) + "px";
            //show('imageProgress');
    ...
    renderPhoto: function(url, photo_index, noteIndex) {
        ...
        var title = (notePhotosCount == 1) ? '����������' : getLang('market_photo_one_of_photo')
            .replace('{index}', nextIndex)
            .replace('{count}', notePhotosCount);
        ...
        if (notePhotosCount > 1) {
            if (typeof(cur.photoNotesCount) != 'undefined') {
                cur.photoBox.content('<a href="#next" onclick="notes.showNextPhoto(\'' + cur.photosOfItem[noteIndex][nextIndex] + '\', null, \'' + nextIndex + '\', ' +
                        noteIndex + '); return false;"><img id="currentPhoto" src="' + url + '" /></a>')
                    .show();
            } else {
                cur.photoBox.content('<a href="#next" onclick="notes.showNextPhoto(\'' + cur.photosOfItem[nextIndex] + '\', null, \'' + nextIndex + '\', ' + noteIndex +
                        '); return false;"><img id="currentPhoto" src="' + url + '" /></a>')
                    .show();
            }
        } else {
            cur.photoBox.content('<img src="' + url + '" />')
                .show();
        }
