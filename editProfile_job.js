...
function initWorkRow(work, elem) {
    ...
    g('company' + work.id)
        .value = winToUtf(work.company_name);
    g('position' + work.id)
        .value = winToUtf(work.position);
    ...
    if (!isVisible('works')) {
        ...
        if (worksCount) {
            for (var i = 0; i < works.length; ++i) {
                ge('works')
                    .appendChild(genWorkRow(works[i].id));
...
function addWork() {
    ...
    ge('works')
        .appendChild(genWorkRow(new_work.id));
...
function deleteWork(id) {
    ...
    show('add_work_link');
    if ((ge('company' + id)
            .value.length) || (ge('position' + id)
            .value.length) || (id > 0)) {
        hide('content' + id);
        ge('deleted' + id)
            .firstChild.innerHTML = lang.profileEdit_work_will_be_deleted;
        show('deleted' + id);
    } else {
        ...
        works.pop();
        ge('work' + id)
            .parentNode.removeChild(ge('work' + id));
        if (works.length == 0) {
            addWork();
        }
    }
...
function addFields() {
    ...
    for (var i = 3; i < arguments.length; ++i) {
        if (intval(ge(arguments[i] + id)
                .value)) {
            params[arguments[i] + index] = ge(arguments[i] + id)
                .value;
        }
    }
    return params;
...
function saveWorks() {
    ...
    for (var i = 0; i < works.length; ++i) {
        var id = works[i].id;
        params['id' + i] = id;
        if (isVisible('content' + id) && (ge('company' + id)
                .value.length || ge('position' + id)
                .value.length)) {
    ...
    var doneHandler = function(obj, text) {
        ...
        for (var i = 0; i < to_remove.length; ++i) {
            ...
            ge('work' + works[index].id)
                .parentNode.removeChild(ge('work' + works[index].id));
            works[index] = works[works.length - 1];
...
function updateWork(old_work, new_work_id, parent) {
    ...
    old_work.company_name = ge('company' + old_work.id)
        .value;
    old_work.start = old_work.uiStart.val();
    old_work.finish = old_work.uiFinish.val();
    old_work.position = ge('position' + old_work.id)
        .value;
...
function workChanged(work) {
    return !isVisible('content' + work.id) ||
        ...
        winToUtf(work.company_name) != ge('company' + work.id)
        .value || winToUtf(work.position) != ge('position' + work.id)
        .value ||
...
function checkChanges(showBox) {
    ...
    for (var i = 0; i < works.length; ++i) {
        if (works[i].id > 0 && workChanged(works[i])) {
            work_change = true;
        } else if (works[i].id < 0 && isVisible('content' + works[i].id) && (ge('company' + works[i].id)
                .value.length || ge('position' + works[i].id)
                .value.length)) {
            work_change = true;
        }
    }
