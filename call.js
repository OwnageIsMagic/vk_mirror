...
var Call = {
    ...
    playRing: function() {
        if (window.ls && ls.get("videocall_sound") && (ls.get("videocall_sound") + 6000 > (new Date())
                .getTime())) {
            return;
        }
        if (window.ls) {
            ls.set("videocall_sound", (new Date())
                .getTime());
        }
    ...
    <div class="call_invitation_wrap clear_fix">\
      <div class="call_title noselect">' + getLang('calls_invitation_title') +
            '</div>\
      <div class="call_text noselect">' + text + '</div>\
      <div class="call_user_pic"><img src="' + photo +
            '" /></div>\
      <div class="call_buttons">\
        <div class="button_green"><button onclick="Call.incomingReply(' + call_id + ', ' + far_uid + ', \'' +
            far_id + '\', \'' + hash + '\', \'' + stream + '\', \'' + rtmp + '\', \'' + rtmfp + '\', \'' + prefer + '\', Call.enableCamera)">' + getLang('calls_reply') +
            '</button></div>\
        <div class="button_red"><button onclick="Call.incomingReject(3,' + call_id + ',' + far_uid + ',' + far_uid + ',\'' + hash +
            '\');">' + getLang('calls_reject') +
            '</button></div>\
        <button class="hover_button" onclick="Call.incomingToggleCam(this);"><div class="call_camera_btn"></div>' + getLang(
                'calls_camera_on') + '</button>\
      </div>\
    </div></div>');
    ...
    videochatBox: function(title) {
      ...
      <span id="call_chat_toggle">' +
                getLang('calls_open_chat') +
                '</span></div>\
      <div id="call_cancel" class="call_cancel">\
        <div class="button_red"><button onclick="Call.endCall(true, 0)">' +
                getLang('calls_finish') + '</button></div>\
      </div>\
    </div>'
    ...
    incomingReceive: function(ev) {
        ...
        if (!Call.devices) {
            ...
            window.__vchatDevicesCB = function(cams, mics) {
                ...
                if (Call.devices > 0) {
                    Call.incomingDoReceive(ev.custom.call_id, ev.author_id, ev.custom.near_id, ev.custom.rtmp_hash, ev.custom.hash, ev.custom.stream, ev.custom.rtmp,
                        ev.custom.rtmfp, ev.custom.prefer, ev.custom.video);
                }
            };
            renderFlash(dv, {
        ...
        if (Call.devices > 0) {
            Call.incomingDoReceive(ev.custom.call_id, ev.author_id, ev.custom.near_id, ev.custom.rtmp_hash, ev.custom.hash, ev.custom.stream, ev.custom.rtmp, ev.custom.rtmfp,
                ev.custom.prefer, ev.custom.video);
        }
    },
